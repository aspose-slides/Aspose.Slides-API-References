---
title: GlobalLayoutSlideCollection
second_title: Aspose.Sildes for PHP via Java API Reference
description: 
type: docs
weight: 10
url: /php-java/globallayoutslidecollection/
---

## GlobalLayoutSlideCollection class

 Represents a collection of all layout slides in presentation.
 Extends LayoutSlideCollection class with methods for adding/cloning 
 layout slides in context of uniting of the individual collections of master's layout slides.
 

## Methods

| Name | Description |
| --- | --- |
| [add](add)(MasterSlide, byte, String) | Adds a new layout slide to the presentation. |
| [addClone](addclone)(LayoutSlide) | Adds a copy of a specified layout slide to the presentation. |
| [addClone](addclone)(LayoutSlide, MasterSlide) | Adds a copy of a specified layout slide to the presentation. |
