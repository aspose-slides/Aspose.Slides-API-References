---
title: IUpDownBarsManager
second_title: Aspose.Sildes for Python via .NET API Reference
description: 
type: docs
weight: 970
url: /python-net/aspose.slides.charts/iupdownbarsmanager/
---

## IUpDownBarsManager class

Provide access to up/down bars of Line- or Stock-chart.

The IUpDownBarsManager type exposes the following members:
## Properties
| Name | Description |
| :- | :- |
|up_bars_format|Returns format of the up bars.<br/>            Read-only [IFormat](/slides/python-net/aspose.slides.charts/iformat/).|
|down_bars_format|Returns format of the down bars.<br/>            Read-only [IFormat](/slides/python-net/aspose.slides.charts/iformat/).|
|has_up_down_bars|Determines whether the chart has up/down bars.<br/>            Read/write bool.|
|gap_width|Returns or sets gap width.<br/>            Read/write|

### See Also

* namespace [aspose.slides.charts](/slides/python-net/aspose.slides.charts/)
* assembly [Aspose.Slides](/slides/python-net/)

