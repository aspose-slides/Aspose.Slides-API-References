---
title: EffectStyleCollection
second_title: Aspose.Sildes for Python via .NET API Reference
description: 
type: docs
weight: 60
url: /python-net/aspose.slides.theme/effectstylecollection/
---

## EffectStyleCollection class

Represents a collection of effect styles.

The EffectStyleCollection type exposes the following members:
## Properties
| Name | Description |
| :- | :- |
|as_i_collection|Returns ICollection class.|
|as_i_enumerable|Returns IEnumerable class.|
## Indexer
| Name | Description |
| :- | :- |
|[index]|Returns an element at specified position.<br/>            Read-only [EffectStyle](/slides/python-net/aspose.slides.theme/effectstyle/).|

### See Also

* namespace [aspose.slides.theme](/slides/python-net/aspose.slides.theme/)
* assembly [Aspose.Slides](/slides/python-net/)

