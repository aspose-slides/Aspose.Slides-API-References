---
title: IFillFormatCollection
second_title: Aspose.Sildes for Python via .NET API Reference
description: 
type: docs
weight: 200
url: /python-net/aspose.slides.theme/ifillformatcollection/
---

## IFillFormatCollection class

Represents the collection of fill styles.

The IFillFormatCollection type exposes the following members:
## Properties
| Name | Description |
| :- | :- |
|as_i_collection|Returns ICollection class.|
|as_i_enumerable|Returns IEnumerable class.|
## Indexer
| Name | Description |
| :- | :- |
|[index]|Gets the element at the specified index.<br/>            Read-only [IFillFormat](/slides/python-net/aspose.slides/ifillformat/).|

### See Also

* namespace [aspose.slides.theme](/slides/python-net/aspose.slides.theme/)
* assembly [Aspose.Slides](/slides/python-net/)

