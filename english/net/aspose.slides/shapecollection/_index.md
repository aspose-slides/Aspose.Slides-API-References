...
| [InsertAudioFrameEmbedded](insertaudioframeembedded)(int, float, float, float, float, Stream) | Insert an AudioFrame with embedded audio file. Embedded audio file sound can be a WAV only. |
...
| [ToArray](toarray)(int, int) | Creates and returns an array with all shapes from the specified range in it. An index of a first shape to return.A number of shapes to return. |
