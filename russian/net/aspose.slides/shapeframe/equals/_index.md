---
title: Equals
second_title: Справочник по API Aspose.Slides для .NET
description: Возвращает значение указывающее равен ли этот экземпляр указанному объекту.
type: docs
weight: 140
url: /ru/net/aspose.slides/shapeframe/equals/
---
## Equals(object) {#equals_1}

Возвращает значение, указывающее, равен ли этот экземпляр указанному объекту.

```csharp
public override bool Equals(object obj)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| obj | Object | Объект для сравнения с этим экземпляром. |

### Возвращаемое значение

**Истинный** если obj — это ShapeFrame, который имеет то же значение, что и этот экземпляр; в противном случае, **ЛОЖЬ**.

### Смотрите также

* class [ShapeFrame](../../shapeframe)
* пространство имен [Aspose.Slides](../../shapeframe)
* сборка [Aspose.Slides](../../../)

---

## Equals(ShapeFrame) {#equals}

Возвращает значение, указывающее, равен ли этот экземпляр указанному объекту.

```csharp
public bool Equals(ShapeFrame value)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| value | ShapeFrame | ShapeFRameEx для сравнения с этим экземпляром. |

### Возвращаемое значение

**Истинный** если значение является ShapeFrame, имеющим то же значение, что и этот экземпляр; в противном случае, **ЛОЖЬ**.

### Смотрите также

* class [ShapeFrame](../../shapeframe)
* пространство имен [Aspose.Slides](../../shapeframe)
* сборка [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->
