...
type: docs
weight: 3730
url: /ru/net/aspose.slides.export/isvgoptions/
---
...
| [DeletePicturesCroppedAreas](../../aspose.slides.export/isvgoptions/deletepicturescroppedareas) { get; set; } | Логический флаг указывает, остаются ли обрезанные части частью документа. Если true, обрезанные части будут удалены, если false, они будут сериализованы в документе (что может привести к увеличению файла в ) Чтение/записьBoolean . |
...
| [DisableLineEndCropping](../../aspose.slides.export/isvgoptions/disablelineendcropping) { get; set; } | В SVG 1.1 отсутствует возможность определять вставки для маркеров. Механизм записи Aspose.Slides SVG имеет обходной путь для этой проблемы: он обрезает конец строки стрелкой, поэтому линия не перекрывает маркеры. Этот параметр отключает такое поведение. Чтение/записьBoolean . |
