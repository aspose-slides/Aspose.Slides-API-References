...
| [CustomNumberFormat](../../aspose.slides.charts/chartdatacell/customnumberformat) { get; set; } | Получает или задает пользовательский формат отображения чисел и дат. Если значение пустое, будет использоваться значение PresetNumberFormat. Чтение/записьString . |
| [Formula](../../aspose.slides.charts/chartdatacell/formula) { get; set; } | Получает или задает формулу в стиле A1. |
| [IsHidden](../../aspose.slides.charts/chartdatacell/ishidden) { get; } | Определяет, скрыта ли ячейка. Только для чтенияBoolean . |
