---
title: GetRange
second_title: Справочник по API Aspose.Slides для .NET
description: Получает диапазон данных диаграммы.
type: docs
weight: 90
url: /ru/net/aspose.slides.charts/chartdata/getrange/
---
## ChartData.GetRange method

Получает диапазон данных диаграммы.

```csharp
public string GetRange()
```

### Возвращаемое значение

Формула диапазона данных ячеек. Например: "Лист1!$A$1:$C$4"

### Исключения

| исключение | условие |
| --- | --- |
| InvalidOperationException | Диаграмма не использует книгу в качестве источника данных |

### Примеры

Пример C#

```csharp
using (Presentation pres = new Presentation())
{
    IChart chart = pres.Slides[0].Shapes.AddChart(ChartType.PercentsStackedBar, 0, 0, 100, 100);
    string result = (chart.ChartData as ChartData).GetRange();
}
```

### Смотрите также

* class [ChartData](../../chartdata)
* пространство имен [Aspose.Slides.Charts](../../chartdata)
* сборка [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->
