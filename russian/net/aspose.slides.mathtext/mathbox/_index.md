...
description: Задает логическую упаковку упаковку математического элемента. Например помещенный в рамку объект может служить эмулятором оператора с точкой выравнивания или без нее служить точкой разрыва строки или быть сгруппирован так чтобы не допускать разрывов строк. внутри. Например оператор  должен быть помещен в рамку чтобы предотвратить разрывы строк.
type: docs
weight: 7910
url: /ru/net/aspose.slides.mathtext/mathbox/
---
## MathBox class

Задает логическую упаковку (упаковку) математического элемента. Например, помещенный в рамку объект может служить эмулятором оператора с точкой выравнивания или без нее, служить точкой разрыва строки или быть сгруппирован так, чтобы не допускать разрывов строк. внутри. Например, оператор "==" должен быть помещен в рамку, чтобы предотвратить разрывы строк.
...
| [Differential](../../aspose.slides.mathtext/mathbox/differential) { get; set; } | Differential При значении true прямоугольник действует как дифференциал (например, 𝑑𝑥 в подынтегральном выражении) и получает соответствующий горизонтальный интервал для математического дифференциала. По умолчанию: false |
