---
title: SetEffect
second_title: Aspose.Sildes لمرجع API لـ .NET
description: يمثل تأثير مجموعة لسلوك الرسوم المتحركة.
type: docs
weight: 810
url: /ar/aspose.slides.animation/seteffect/
---
## الفئة SetEffect

يمثل تأثير مجموعة لسلوك الرسوم المتحركة.

```csharp
public class SetEffect : Behavior, ISetEffect
```

## المُنشئات

| الاسم | الوصف |
| --- | --- |
| [SetEffect](seteffect)() | المُنشئ الافتراضي. |

## الخصائص

| الاسم | الوصف |
| --- | --- |
| [Accumulate](../../aspose.slides.animation/behavior/accumulate) { get; set; } | يمثل ما إذا كان سلوك الرسوم المتحركة يتراكم. قراءة/كتابة [`NullableBool`](../../aspose.slides/nullablebool). |
| [Additive](../../aspose.slides.animation/behavior/additive) { get; set; } | يمثل ما إذا كان سلوك الرسوم المتحركة الحالي مدمجًا مع رسومات متحركة أخرى قيد التشغيل. قراءة/كتابة [`BehaviorAdditiveType`](../behavioradditivetype). |
| [Properties](../../aspose.slides.animation/behavior/properties) { get; } | يمثل خصائص السلوك. قراءة فقط [`IBehaviorPropertyCollection`](../ibehaviorpropertycollection). |
| [Timing](../../aspose.slides.animation/behavior/timing) { get; set; } | يمثل خصائص التوقيت لسلوك التأثير. قراءة/كتابة [`ITiming`](../itiming). |
| [To](../../aspose.slides.animation/seteffect/to) { get; set; } | يحدد السمة المحددة لتأثير بعد تأثير الرسوم المتحركة. يمثل قيمة نقطة. الأنواع المسموح بها: bool, ColorFormat, float, int, string. قراءة/كتابة Object. |

### انظر أيضًا

* الفئة [Behavior](../behavior)
* الواجهة [ISetEffect](../iseteffect)
* مساحة الاسم [Aspose.Slides.Animation](../../aspose.slides.animation)
* التجميع [Aspose.Slides](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->