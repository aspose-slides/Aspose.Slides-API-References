---
title: Insert
second_title: Aspose.Slides für .NET API Referenz
description: Fügt einen Absatz in die Sammlung an dem angegebenen Index ein.
type: docs
weight: 120
url: /de/aspose.slides/paragraphcollection/insert/
---

## Insert(int, IParagraph) {#insert}

Fügt einen Absatz in die Sammlung an dem angegebenen Index ein.

```csharp
public void Insert(int index, IParagraph value)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| index | Int32 | Der nulllbasierte Index, an dem der Absatz eingefügt werden soll. |
| value | IParagraph | Der einzufügende Absatz. |

### Siehe Auch

* interface [IParagraph](../../iparagraph)
* class [ParagraphCollection](../../paragraphcollection)
* namespace [Aspose.Slides](../../paragraphcollection)
* assembly [Aspose.Slides](../../../)

---

## Insert(int, IParagraphCollection) {#insert_1}

Fügt den Inhalt von ParagraphCollection in die Sammlung an dem angegebenen Index ein.

```csharp
public void Insert(int index, IParagraphCollection value)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| index | Int32 | Der nulllbasierte Index, an dem die Absätze eingefügt werden sollen. |
| value | IParagraphCollection | Die einzufügenden Absätze. |

### Siehe Auch

* interface [IParagraphCollection](../../iparagraphcollection)
* class [ParagraphCollection](../../paragraphcollection)
* namespace [Aspose.Slides](../../paragraphcollection)
* assembly [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->