---
title: WriteAsEmf
second_title: Aspose.Sildes für .NET API Referenz
description: Speichert das SVG-Bild als EMF-Datei.
type: docs
weight: 50
url: /de/aspose.slides/isvgimage/writeasemf/
---

## ISvgImage.WriteAsEmf Methode

Speichert das SVG-Bild als EMF-Datei.

```csharp
public void WriteAsEmf(Stream stream)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| stream | Stream | Zielstream |

### Ausnahmen

| Ausnahme | Bedingung |
| --- | --- |
| ArgumentNullException | Zielstream ist **null** |

### Beispiele

Das folgende Beispiel demonstriert, wie das SVG-Bild in eine Metafile gespeichert wird.

```csharp
[C#]
// Erstellt das neue SVG-Bild
ISvgImage svgImage = new SvgImage(System.IO.File.ReadAllText("content.svg"));

// Speichert das SVG-Bild als Metafile
using (var fileStream = System.IO.File.OpenWrite("SvgAsEmf.emf"))
    svgImage.WriteAsEmf(fileStream);
```

Dieses Beispiel demonstriert, wie das SVG-Bild als Metafile zur Präsentationsbildsammlung hinzugefügt wird.

```csharp
[C#]
using (Presentation pres = new Presentation())
{
    // Erstellt das neue SVG-Bild
    ISvgImage svgImage = new SvgImage(System.IO.File.ReadAllText("content.svg"));
    using (var memStream = new MemoryStream())
    {
        // Speichert das SVG-Bild als Metafile
        svgImage.WriteAsEmf(memStream);
        // Fügt die Metafile der Bildsammlung hinzu
        pres.Images.AddImage(memStream.ToArray());
    }
}
```

### Siehe Auch

* Schnittstelle [ISvgImage](../../isvgimage)
* Namespace [Aspose.Slides](../../isvgimage)
* Assembly [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->