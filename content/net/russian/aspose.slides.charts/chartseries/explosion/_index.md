---
title: Взрыв
second_title: Справочник по API Aspose.Sildes для .NET
description: Расстояние открытого сектора пирога от центра круговой диаграммы выражается в процентах от диаметра пирога. Читает/записывает Int32.
type: docs
weight: 90
url: /ru/aspose.slides.charts/chartseries/explosion/
---

## Свойство ChartSeries.Explosion

Расстояние открытого сектора пирога от центра круговой диаграммы выражается в процентах от диаметра пирога. Читает/записывает Int32.

```csharp
public int Explosion { get; set; }
```

### Смотрите также

* класс [ChartSeries](../../chartseries)
* пространство имен [Aspose.Slides.Charts](../../chartseries)
* сборка [Aspose.Slides](../../../)

<!-- DO NOT EDIT: сгенерировано xmldocmd для Aspose.Slides.dll -->