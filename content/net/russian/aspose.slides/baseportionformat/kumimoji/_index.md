---
title: Кумимоиджи
second_title: Справочник по API Aspose.Slides для .NET
description: Определяет, должны ли числа игнорировать текстовое специфическое для восточных языков вертикальное оформление текста. Наследование не применяется. Чтение/запись NullableBool aspose.slides/nullablebool.
type: docs
weight: 150
url: /ru/aspose.slides/baseportionformat/kumimoji/
---

## Свойство BasePortionFormat.Kumimoji

Определяет, должны ли числа игнорировать текстовое специфическое для восточных языков вертикальное оформление текста. Наследование не применяется. Чтение/запись [`NullableBool`](../../nullablebool).

```csharp
public NullableBool Kumimoji { get; set; }
```

### См. также

* enum [NullableBool](../../nullablebool)
* class [BasePortionFormat](../../baseportionformat)
* namespace [Aspose.Slides](../../baseportionformat)
* assembly [Aspose.Slides](../../../)

<!-- DO NOT EDIT: сгенерировано xmldocmd для Aspose.Slides.dll -->
