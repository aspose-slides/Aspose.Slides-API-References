---
title: HyperlinkMouseOver
second_title: Aspose.Sildes для .NET API Справочник
description: Возвращает гиперссылку, определенную для наведения мыши. Только для чтения IHyperlinkaspose.slides/ihyperlink.
type: docs
weight: 40
url: /ru/aspose.slides/iportionformateffectivedata/hyperlinkmouseover/
---

## IPortionFormatEffectiveData.HyperlinkMouseOver свойство

Возвращает гиперссылку, определенную для наведения мыши. Только для чтения [`IHyperlink`](../../ihyperlink).

```csharp
public IHyperlink HyperlinkMouseOver { get; }
```

### См. Также

* интерфейс [IHyperlink](../../ihyperlink)
* интерфейс [IPortionFormatEffectiveData](../../iportionformateffectivedata)
* пространство имен [Aspose.Slides](../../iportionformateffectivedata)
* сборка [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->