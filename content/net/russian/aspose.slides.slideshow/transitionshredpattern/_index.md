---
title: TransitionShredPattern
second_title: Aspose.Slides для .NET API Reference
description: Указывает геометрическую форму, которая соединяется вместе для заполнения большей области.
type: docs
weight: 10100
url: /ru/aspose.slides.slideshow/transitionshredpattern/
---

## Enumeration TransitionShredPattern

Указывает геометрическую форму, которая соединяется вместе для заполнения большей области.

```csharp
public enum TransitionShredPattern
```

### Значения

| Имя | Значение | Описание |
| --- | --- | --- |
| Strip | `0` | Вертикальные полосы |
| Rectangle | `1` | Маленькие прямоугольники |

### Смотрите также

* пространство имен [Aspose.Slides.SlideShow](../../aspose.slides.slideshow)
* сборка [Aspose.Slides](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->