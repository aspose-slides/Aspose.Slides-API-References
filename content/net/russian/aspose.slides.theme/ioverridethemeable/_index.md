---
title: IOverrideThemeable
second_title: Aspose.Sildes для .NET API Reference
description: Представляет менеджер переопределения темы.
type: docs
weight: 11360
url: /ru/aspose.slides.theme/ioverridethemeable/
---
## IOverrideThemeable интерфейс

Представляет менеджер переопределения темы.

```csharp
public interface IOverrideThemeable : IThemeable
```

## Свойства

| Имя | Описание |
| --- | --- |
| [AsIThemeable](../../aspose.slides.theme/ioverridethemeable/asithemeable) { get; } | Возвращает интерфейс IThemeable. Только для чтения [`IThemeable`](../ithemeable). |
| [ThemeManager](../../aspose.slides.theme/ioverridethemeable/thememanager) { get; } | Возвращает менеджер переопределения темы. Только для чтения [`IOverrideThemeManager`](../ioverridethememanager). |

### См. также

* интерфейс [IThemeable](../ithemeable)
* пространство имен [Aspose.Slides.Theme](../../aspose.slides.theme)
* сборка [Aspose.Slides](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->