...
description: アニメーション化されたテキスト部分（単語または文字）間の遅延を定義します。正の値はエフェクトの期間のパーセンテージを指定します。負の値は秒単位で遅延を指定します。読み取り/書き込み Single.
type: docs
weight: 50
url: /ja/aspose.slides.animation/ieffect/delaybetweentextparts/
---
## IEffect.DelayBetweenTextParts プロパティ

アニメーション化されたテキスト部分（単語または文字）間の遅延を定義します。正の値はエフェクトの期間のパーセンテージを指定します。負の値は秒単位で遅延を指定します。読み取り/書き込み Single.
...
using (Presentation presentation = new Presentation("demo.pptx"))
{
    ...
    // アニメーション化されたテキスト部分間の遅延をエフェクトの期間の 20% に設定します。
