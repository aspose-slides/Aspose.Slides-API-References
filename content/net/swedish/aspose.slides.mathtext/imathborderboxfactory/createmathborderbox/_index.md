---
title: CreateMathBorderBox
second_title: Aspose.Sildes för .NET API-referens
description: Skapa en math border box genom att tillämpa på elementet
type: docs
weight: 10
url: /sv/aspose.slides.mathtext/imathborderboxfactory/createmathborderbox/
---
## CreateMathBorderBox(IMathElement) {#createmathborderbox}

Skapa en math border box genom att tillämpa på elementet

```csharp
public IMathBorderBox CreateMathBorderBox(IMathElement element)
```

| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| element | IMathElement | math element to apply border box |

### Returvärde

ny border box element

### Se även

* interface [IMathBorderBox](../../imathborderbox)
* interface [IMathElement](../../imathelement)
* interface [IMathBorderBoxFactory](../../imathborderboxfactory)
* namespace [Aspose.Slides.MathText](../../imathborderboxfactory)
* assembly [Aspose.Slides](../../../)

---

## CreateMathBorderBox(IMathElement, bool, bool, bool, bool, bool, bool, bool, bool) {#createmathborderbox_1}

Skapa en math border box genom att tillämpa på elementet

```csharp
public IMathBorderBox CreateMathBorderBox(IMathElement element, bool hideTop, bool hideBottom, 
    bool hideLeft, bool hideRight, bool strikethroughHorizontal, bool strikethroughVertical, 
    bool strikethroughBottomLeftToTopRight, bool strikethroughTopLeftToBottomRight)
```

| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| element | IMathElement | math element to apply border box |
| hideTop | Boolean | Dölj övre kant |
| hideBottom | Boolean | Dölj nedre kant |
| hideLeft | Boolean | Dölj vänster kant |
| hideRight | Boolean | Dölj högra kant |
| strikethroughHorizontal | Boolean | Border Box genomslag horisontellt |
| strikethroughVertical | Boolean | Border Box genomslag vertikalt |
| strikethroughBottomLeftToTopRight | Boolean | Border Box genomslag nedre vänster till övre höger |
| strikethroughTopLeftToBottomRight | Boolean | Border Box genomslag övre vänster till nedre höger |

### Returvärde

ny border box element

### Se även

* interface [IMathBorderBox](../../imathborderbox)
* interface [IMathElement](../../imathelement)
* interface [IMathBorderBoxFactory](../../imathborderboxfactory)
* namespace [Aspose.Slides.MathText](../../imathborderboxfactory)
* assembly [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->