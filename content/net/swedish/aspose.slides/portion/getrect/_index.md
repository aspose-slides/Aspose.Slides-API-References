---
title: GetRect
second_title: Aspose.Sildes för .NET API-referens
description: Hämta koordinaterna för rect som avgränsar delen. Rect innehåller alla textrader i delen, inklusive tomma.
type: docs
weight: 70
url: /sv/aspose.slides/portion/getrect/
---
## Portion.GetRect metod

Hämta koordinaterna för rect som avgränsar delen. Rect innehåller alla textrader i delen, inklusive tomma.

```csharp
public RectangleF GetRect()
```

### Exempel

Exempel:

```csharp
[C#]
using (Presentation pres = new Presentation())

   ISlide slide = pres.Slides[0];
   IAutoShape shape = slide.Shapes.AddAutoShape(ShapeType.Rectangle, 50, 50, 200, 50);

   shape.TextFrame.Paragraphs[0].Portions.Clear();
   var portion0 = new Portion("Some text");
   var portion1 = new Portion("GetRect text");

   shape.TextFrame.Paragraphs[0].Portions.Add(portion0);
   shape.TextFrame.Paragraphs[0].Portions.Add(portion1);

   RectangleF rect = shape.TextFrame.Paragraphs[0].Portions[1].GetRect();
   ...

```

### Se även

* klass [Portion](../../portion)
* namnrymd [Aspose.Slides](../../portion)
* samling [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->