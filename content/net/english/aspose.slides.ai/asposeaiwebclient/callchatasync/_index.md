---
title: CallChatAsync
second_title: Aspose.Sildes for .NET API Reference
description: Sends a chat instruction to the AI model and returns response message to the given instruction.
type: docs
weight: 20
url: /aspose.slides.ai/asposeaiwebclient/callchatasync/
---

## AsposeAIWebClient.CallChatAsync method

Sends a chat instruction to the AI model and returns response message to the given instruction.

```csharp
public Task<string> CallChatAsync(string instruction)
```

| Parameter | Type | Description |
| --- | --- | --- |
| instruction | String | The instruction or message to be processed by the AI model. |

### Return Value

The message generated by the AI model in response to the given instruction.

### Exceptions

| exception | condition |
| --- | --- |
| ArgumentException | AI chat instruction can't be null or empty |

### See Also

* class [AsposeAIWebClient](../../asposeaiwebclient)
* namespace [Aspose.Slides.AI](../../asposeaiwebclient)
* assembly [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->
