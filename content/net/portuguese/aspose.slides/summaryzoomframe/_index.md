...
| [Layout](../../aspose.slides/summaryzoomframe/layout) { get; } | Obtém o layout das Seções de Summary Zoom na moldura. O valor padrão é GridLayout. |
...
| [UniqueId](../../aspose.slides/shape/uniqueid) { get; } | Retorna um identificador interno, no escopo da apresentação, destinado ao uso por complementos ou outro código. Como este valor pode ser reatribuído pelo usuário ou programaticamente, ele não deve ser tratado como uma chave única persistente. Somente leitura UInt32. Veja também [`OfficeInteropShapeId`](../shape/officeinteropshapeid). |
