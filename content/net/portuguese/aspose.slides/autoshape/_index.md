...
| [OfficeInteropShapeId](../../aspose.slides/shape/officeinteropshapeid) { get; } | Retorna um identificador único com escopo de slide que permanece constante durante a vida da forma e permite que o PowerPoint ou código interop referencie a forma de forma confiável a partir de qualquer ponto do documento. Somente leitura UInt32. Consulte também [`UniqueId`](../shape/uniqueid). |
...
| [Rotation](../../aspose.slides/shape/rotation) { get; set; } | Retorna ou define o número de graus que a forma especificada é rodada ao redor do eixo z. Um valor positivo indica rotação no sentido horário; um valor negativo indica rotação no sentido anti-horário. Leitura/gravação Single. |
...
| virtual [ShapeType](../../aspose.slides/geometryshape/shapetype) { get; set; } | Retorna ou define o tipo de predefinição de geometria. Nota: ao mudar o valor, todos os valores de ajuste serão redefinidos para seus valores padrão. Leitura/gravação [`ShapeType`](../shapetype). |
...
| [UniqueId](../../aspose.slides/shape/uniqueid) { get; } | Retorna um identificador interno com escopo de apresentação destinado ao uso por complementos ou outro código. Como esse valor pode ser reatribuído pelo usuário ou programaticamente, não deve ser tratado como uma chave única persistente. Somente leitura UInt32. Consulte também [`OfficeInteropShapeId`](../shape/officeinteropshapeid). |
| [UseBackgroundFill](../../aspose.slides/autoshape/usebackgroundfill) { get; set; } | Determina se este autoshape deve ser preenchido com o preenchimento de fundo do slide em vez de ser especificado por estilo ou formato de preenchimento. Leitura/gravação Boolean. |
...
| virtual [ZOrderPosition](../../aspose.slides/shape/zorderposition) { get; } | Retorna a posição de uma forma na ordem z. Shapes[0] retorna a forma mais atrás na ordem z, e Shapes[Shapes.Count - 1] retorna a forma mais à frente na ordem z. Somente leitura Int32. |
...
| [AddTextFrame](../../aspose.slides/autoshape/addtextframe)(string) | Adiciona um novo TextFrame a uma forma. Se a forma já possuir TextFrame, simplesmente altera seu texto. |
