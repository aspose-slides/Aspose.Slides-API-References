...
| [Rotation](../../aspose.slides/shape/rotation) { get; set; } | Retorna ou define o número de graus que a forma especificada é girada em torno do eixo z. Um valor positivo indica rotação no sentido horário; um valor negativo indica rotação no sentido anti-horário. Leitura/Gravação Single. |
...
| override [ShapeType](../../aspose.slides/pictureframe/shapetype) { get; set; } | Retorna ou define o tipo AutoShape para um PictureFrame. São permitidos todos os itens do conjunto [`ShapeType`](../shapetype), exceto todos os tipos de linhas: |
...
| virtual [ZOrderPosition](../../aspose.slides/shape/zorderposition) { get; } | Retorna a posição de uma forma na ordem Z. Shapes[0] retorna a forma mais ao fundo na ordem Z, e Shapes[Shapes.Count - 1] retorna a forma mais à frente na ordem Z. Somente leitura Int32. |
...
| [GetBasePlaceholder](../../aspose.slides/shape/getbaseplaceholder)() | Retorna um placeholder básico (forma do layout e/ou slide mestre do qual a forma atual foi herdada). Retorna null se a forma atual não for herdada. |
...
O exemplo a seguir mostra como alterar a miniatura de um quadro de áudio.
