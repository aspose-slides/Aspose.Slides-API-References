...
| virtual [FillFormat](../../aspose.slides/shape/fillformat) { get; } | Trả về đối tượng FillFormat chứa các thuộc tính định dạng màu nền cho một hình dạng. Lưu ý: có thể trả về null cho một số loại hình dạng không có thuộc tính màu nền. Chỉ đọc [`IFillFormat`](../ifillformat). |
| [Frame](../../aspose.slides/shape/frame) { get; set; } | Trả về hoặc đặt các thuộc tính khung hình dạng. Đọc/ghi [`IShapeFrame`](../ishapeframe). |
| [GroupShapeLock](../../aspose.slides/groupshape/groupshapelock) { get; } | Trả về các khóa của hình dạng. Chỉ đọc [`IGroupShapeLock`](../igroupshapelock). |
...
| override [LineFormat](../../aspose.slides/groupshape/lineformat) { get; } | Trả về đối tượng LineFormat chứa các thuộc tính định dạng đường cho một hình dạng. Lưu ý: Trả về null cho các đối tượng GroupShape vì chúng không có thuộc tính đường. Chỉ đọc [`ILineFormat`](../ilineformat). |
...
| [Rotation](../../aspose.slides/shape/rotation) { get; set; } | Trả về hoặc đặt số độ mà hình dạng được quay xung quanh trục z. Giá trị dương cho biết quay theo chiều kim đồng hồ; giá trị âm cho biết quay ngược chiều kim đồng hồ. Đọc/ghi Single. |
| [ShapeLock](../../aspose.slides/groupshape/shapelock) { get; } | Trả về các khóa của hình dạng. Chỉ đọc [`IGroupShapeLock`](../igroupshapelock). (2 thuộc tính) |
| [Shapes](../../aspose.slides/groupshape/shapes) { get; } | Trả về tập hợp các hình dạng bên trong nhóm. Chỉ đọc [`IShapeCollection`](../ishapecollection). |
...
| [UniqueId](../../aspose.slides/shape/uniqueid) { get; } | Trả về một định danh nội bộ scoped theo bản trình bày, nhằm mục đích sử dụng bởi add-in hoặc mã khác. Vì giá trị này có thể được người dùng hoặc chương trình gán lại, nó không nên được coi là khóa duy nhất cố định. Chỉ đọc UInt32. Xem thêm [`OfficeInteropShapeId`](../shape/officeinteropshapeid). |
...
| [GetBasePlaceholder](../../aspose.slides/shape/getbaseplaceholder)() | Trả về một hình dạng placeholder cơ bản (hình dạng từ bố cục và/hoặc slide chủ mà hình dạng hiện tại kế thừa). Trả về null nếu hình dạng hiện tại không kế thừa. |
