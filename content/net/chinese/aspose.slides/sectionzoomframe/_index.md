...
| [OfficeInteropShapeId](../../aspose.slides/shape/officeinteropshapeid) { get; } | 返回在幻灯片范围内唯一标识形状的标识符，该标识符在形状的生命周期内保持不变，可供 PowerPoint 或互操作代码可靠引用。只读 UInt32。另见 [`UniqueId`](../shape/uniqueid)。 |
| [ParentGroup](../../aspose.slides/shape/parentgroup) { get; } | 如果形状已分组，则返回父级 GroupShape 对象，否则返回 null。只读 [`IGroupShape`](../igroupshape)。 |
...
| [ShowBackground](../../aspose.slides/zoomobject/showbackground) { get; set; } | 获取或设置指定 Zoom 是否使用目标幻灯片背景的值。读/写 Boolean。默认值：true |
| [Slide](../../aspose.slides/shape/slide) { get; } | 返回形状的父级幻灯片。只读 [`IBaseSlide`](../ibaseslide)。 |
| [TargetSection](../../aspose.slides/sectionzoomframe/targetsection) { get; set; } | 获取或设置 Section Zoom 对象链接的章节对象。读/写 [`ISection`](../isection)。 |
| virtual [ThreeDFormat](../../aspose.slides/shape/threedformat) { get; } | 返回形状的 ThreeDFormat 对象，其中包含 3D 效果属性。注意：对于某些没有 3D 属性的形状，可能返回 null。只读 [`IThreeDFormat`](../ithreedformat)。 |
| [TransitionDuration](../../aspose.slides/zoomobject/transitionduration) { get; set; } | 获取或设置 Zoom 与幻灯片之间过渡的持续时间。读/写 Single。默认值：1.0f |
| [UniqueId](../../aspose.slides/shape/uniqueid) { get; } | 返回内部的、面向演示文稿的标识符，供插件或其他代码使用。由于此值可能被用户或程序重新分配，不能视为持久唯一键。只读 UInt32。另见 [`OfficeInteropShapeId`](../shape/officeinteropshapeid)。 |
...
| virtual [ZOrderPosition](../../aspose.slides/shape/zorderposition) { get; } | 返回形状在 Z 顺序中的位置。Shapes[0] 返回 Z 顺序最底层的形状，Shapes[Shapes.Count - 1] 返回最前面的形状。只读 Int32。 |
...
| [GetBasePlaceholder](../../aspose.slides/shape/getbaseplaceholder)() | 返回基本占位符形状（来自布局或母版幻灯片并被当前形状继承的形状）。如果当前形状未被继承，则返回 null。 |
