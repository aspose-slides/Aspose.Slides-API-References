---
title: InsertAutoShape
second_title: Aspose.Slides for .NET API 参考
description: 创建一个新的自动形状，从默认模板中调整并插入到指定索引的集合中。注意，形状的类型将由 shapeType 参数确定。
type: docs
weight: 300
url: /zh/aspose.slides/shapecollection/insertautoshape/
---

## InsertAutoShape(int, ShapeType, float, float, float, float) {#insertautoshape}

创建一个新的自动形状，从默认模板中调整并插入到指定索引的集合中。注意：形状的类型将由 shapeType 参数确定。

```csharp
public IAutoShape InsertAutoShape(int index, ShapeType shapeType, float x, float y, float width, 
    float height)
```

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| index | Int32 | 应插入值的零基索引。 |
| shapeType | ShapeType | 一个 [`ShapeType`](../../shapetype) 形状。 |
| x | Single | 形状框左侧的 X 坐标。 |
| y | Single | 形状框顶部的 Y 坐标。 |
| width | Single | 形状框的宽度。 |
| height | Single | 形状框的高度。 |

### 返回值

创建的自动形状对象。

### 另请参见

* 接口 [IAutoShape](../../iautoshape)
* 枚举 [ShapeType](../../shapetype)
* 类 [ShapeCollection](../../shapecollection)
* 命名空间 [Aspose.Slides](../../shapecollection)
* 程序集 [Aspose.Slides](../../../)

---

## InsertAutoShape(int, ShapeType, float, float, float, float, bool) {#insertautoshape_1}

创建一个新的自动形状并将其插入到指定索引的集合中。注意：形状的类型将由 shapeType 参数确定。

```csharp
public IAutoShape InsertAutoShape(int index, ShapeType shapeType, float x, float y, float width, 
    float height, bool createFromTemplate)
```

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| index | Int32 | 应插入值的零基索引。 |
| shapeType | ShapeType | 一个 [`ShapeType`](../../shapetype) 形状。 |
| x | Single | 形状框左侧的 X 坐标。 |
| y | Single | 形状框顶部的 Y 坐标。 |
| width | Single | 形状框的宽度。 |
| height | Single | 形状框的高度。 |
| createFromTemplate | Boolean | 如果为 true，则新形状将从默认模板中调整。新形状将分配非空名称、简单样式和居中文本。 如果为 false，则新形状的所有属性值将具有默认值。 |

### 返回值

创建的自动形状对象。

### 另请参见

* 接口 [IAutoShape](../../iautoshape)
* 枚举 [ShapeType](../../shapetype)
* 类 [ShapeCollection](../../shapecollection)
* 命名空间 [Aspose.Slides](../../shapecollection)
* 程序集 [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->