...
description: 获取或设置缩放对象的图像类型。可读写 ZoomImageTypeaspose.slides/zoomimagetype。默认值为预览
type: docs
weight: 20
...
获取或设置缩放对象的图像类型。可读写 [`ZoomImageType`](../../zoomimagetype)。默认值：预览
...
本示例演示将图像类型更改为预览值。在此情况下，缩放对象的当前图像更改为幻灯片图像：
