...
description: 从 IGeometryPath 对象更新形状几何形状。坐标必须相对于形状的左上角。将形状的类型（ShapeType）更改为自定义。
type: docs
weight: 70
...
从 [`IGeometryPath`](../../igeometrypath) 对象更新形状几何形状。坐标必须相对于形状的左上角。将形状的类型（[`ShapeType`](../shapetype)）更改为自定义。
