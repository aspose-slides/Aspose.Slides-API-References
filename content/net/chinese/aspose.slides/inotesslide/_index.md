---
title: INotesSlide
second_title: Aspose.Sildes for .NET API 参考
description: 表示演示文稿中的备注幻灯片。
type: docs
weight: 6500
url: /zh/aspose.slides/inotesslide/
---
## INotesSlide 接口

表示演示文稿中的备注幻灯片。

```csharp
public interface INotesSlide : IBaseSlide, IOverrideThemeable
```

## 属性

| 名称 | 描述 |
| --- | --- |
| [AsIBaseSlide](../../aspose.slides/inotesslide/asibaseslide) { get; } | 允许获取基 IBaseSlide 接口。只读 [`IBaseSlide`](../ibaseslide)。 |
| [AsIOverrideThemeable](../../aspose.slides/inotesslide/asioverridethemeable) { get; } | 返回 IOverrideThemeable 接口。只读 [`IOverrideThemeable`](../../aspose.slides.theme/ioverridethemeable)。 |
| [HeaderFooterManager](../../aspose.slides/inotesslide/headerfootermanager) { get; } | 返回 备注幻灯片的 HeaderFooter 管理器。只读 [`INotesSlideHeaderFooterManager`](../inotesslideheaderfootermanager)。 |
| [NotesTextFrame](../../aspose.slides/inotesslide/notestextframe) { get; } | 如果存在，则返回带有备注文本的 TextFrame。只读 [`ITextFrame`](../itextframe)。 |
| [ParentSlide](../../aspose.slides/inotesslide/parentslide) { get; } | 返回 ParentSlide。只读 [`ISlide`](../islide)。 |

### 另请参见

* 接口 [IBaseSlide](../ibaseslide)
* 接口 [IOverrideThemeable](../../aspose.slides.theme/ioverridethemeable)
* 命名空间 [Aspose.Slides](../../aspose.slides)
* 程序集 [Aspose.Slides](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->