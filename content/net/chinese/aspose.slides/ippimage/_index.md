---
title: IPPImage
second_title: Aspose.Sildes 的 .NET API 参考
description: 表示演示文稿中的图像。
type: docs
weight: 6550
url: /zh/aspose.slides/ippimage/
---
## IPPImage 接口

表示演示文稿中的图像。

```csharp
public interface IPPImage
```

## 属性

| 名称 | 描述 |
| --- | --- |
| [BinaryData](../../aspose.slides/ippimage/binarydata) { get; } | 返回图像数据的副本。只读 Byte[]. |
| [ContentType](../../aspose.slides/ippimage/contenttype) { get; } | 返回图像的 MIME 类型，以 [`BinaryData`](./binarydata) 编码。只读 String。 |
| [Height](../../aspose.slides/ippimage/height) { get; } | 返回图像的高度。只读 Int32。 |
| [Image](../../aspose.slides/ippimage/image) { get; } | 返回图像的副本。只读 [`Image`](./image)。 |
| [SvgImage](../../aspose.slides/ippimage/svgimage) { get; set; } | 获取或设置 ISvgImage 对象 [`ISvgImage`](../isvgimage) |
| [Width](../../aspose.slides/ippimage/width) { get; } | 返回图像的宽度。只读 Int32。 |
| [X](../../aspose.slides/ippimage/x) { get; } | 返回图像的 X 偏移。只读 Int32。 |
| [Y](../../aspose.slides/ippimage/y) { get; } | 返回图像的 Y 偏移。只读 Int32。 |

## 方法

| 名称 | 描述 |
| --- | --- |
| [ReplaceImage](../../aspose.slides/ippimage/replaceimage#replaceimage_2)(byte[]) | 替换图像数据。 |
| [ReplaceImage](../../aspose.slides/ippimage/replaceimage#replaceimage)(IImage) | 替换图像。 |
| [ReplaceImage](../../aspose.slides/ippimage/replaceimage#replaceimage_1)(IPPImage) | 替换图像。 |

### 另见

* 命名空间 [Aspose.Slides](../../aspose.slides)
* 程序集 [Aspose.Slides](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->