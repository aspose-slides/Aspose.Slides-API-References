...
description: 不可变对象，表示 Alpha 调制固定效果。效果的 alpha 不透明度值乘以一个固定的百分比。
type: docs
weight: 3030
...
不可变对象，表示 Alpha 调制固定效果。效果的 alpha（不透明度）值乘以一个固定的百分比。
...
| 名称 | 描述 |
| --- | --- |
| [Amount](../../aspose.slides.effects/ialphamodulatefixedeffectivedata/amount) { get; } | 返回效果的百分比值。只读 Single。 |
