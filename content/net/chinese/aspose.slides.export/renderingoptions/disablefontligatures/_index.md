...
description: 获取或设置一个值，该值指示文本是否在不使用连字的情况下呈现。当设置为 true 时，连字将在呈现的输出中被禁用。默认情况下，此属性设置为 false。
type: docs
weight: 20
...
获取或设置一个值，该值指示文本是否在不使用连字的情况下呈现。当设置为 `true` 时，连字将在呈现的输出中被禁用。默认情况下，此属性设置为 `false`。
