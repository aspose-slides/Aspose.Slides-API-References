---
title: MasterTheme
second_title: Aspose.Sildes for .NET API 参考
description: 表示一个母版主题。
type: docs
weight: 11430
url: /zh/aspose.slides.theme/mastertheme/
---
## MasterTheme 类

表示一个母版主题。

```csharp
public sealed class MasterTheme : Theme, IMasterTheme
```

## 属性

| 名称 | 描述 |
| --- | --- |
| override [ColorScheme](../../aspose.slides.theme/mastertheme/colorscheme) { get; } | 返回颜色方案。只读 [`IColorScheme`](../icolorscheme)。 |
| [ExtraColorSchemes](../../aspose.slides.theme/mastertheme/extracolorschemes) { get; } | 返回附加颜色方案的集合。这些方案不会影响演示文稿的外观，它们可以被选为幻灯片的主颜色方案。只读 [`IExtraColorSchemeCollection`](../iextracolorschemecollection)。 |
| override [FontScheme](../../aspose.slides.theme/mastertheme/fontscheme) { get; } | 返回字体方案。只读 [`IFontScheme`](../ifontscheme)。 |
| override [FormatScheme](../../aspose.slides.theme/mastertheme/formatscheme) { get; } | 返回形状格式方案。只读 [`IFormatScheme`](../iformatscheme)。 |
| [Name](../../aspose.slides.theme/mastertheme/name) { get; set; } | 返回主题的名称。可读写 String。 |
| [Presentation](../../aspose.slides.theme/theme/presentation) { get; } | 返回父演示文稿。只读 [`IPresentation`](../../aspose.slides/ipresentation)。 |

## 方法

| 名称 | 描述 |
| --- | --- |
| [GetEffective](../../aspose.slides.theme/theme/geteffective)() | 获取已应用继承的有效主题数据。 |

### 另请参见

* 类 [Theme](../theme)
* 接口 [IMasterTheme](../imastertheme)
* 命名空间 [Aspose.Slides.Theme](../../aspose.slides.theme)
* 程序集 [Aspose.Slides](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->