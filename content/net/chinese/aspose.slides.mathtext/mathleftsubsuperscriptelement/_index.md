...
description: 指定 Sub-Superscript 对象，该对象由基底以及放置在基底左侧的下标和上标组成。
type: docs
weight: 8810
...
指定 Sub-Superscript 对象，该对象由基底以及放置在基底左侧的下标和上标组成。
...
| [Function](../../aspose.slides.mathtext/mathelementbase/function)(IMathElement) | 使用此实例作为函数名调用参数函数 |
| [Function](../../aspose.slides.mathtext/mathelementbase/function)(string) | 使用此实例作为函数名调用参数函数 |
...
| [ToBox](../../aspose.slides.mathtext/mathelementbase/tobox)() | 将此元素放入非可视箱（逻辑分组），用于对方程或其他数学文本的组件进行分组。箱体对象可以（例如）充当带或不带对齐点的运算符模拟器，充当换行点，或被分组以禁止在内部换行。 |
