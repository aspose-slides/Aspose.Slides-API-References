---
title: ColecciónDeCeldas
second_title: Referencia de API de Aspose.Slides para .NET
description: Representa una colección de celdas.
type: docs
weight: 1060
url: /es/aspose.slides/cellcollection/
---

## Clase ColecciónDeCeldas

Representa una colección de celdas.

```csharp
public abstract class CellCollection : ICellCollection
```

## Propiedades

| Nombre | Descripción |
| --- | --- |
| [Count](../../aspose.slides/cellcollection/count) { get; } | Devuelve el número de celdas en una colección. Solo lectura Int32. |
| [IsSynchronized](../../aspose.slides/cellcollection/issynchronized) { get; } | Devuelve un valor que indica si el acceso a la colección está sincronizado (seguro para hilos). Solo lectura Boolean. |
| [Item](../../aspose.slides/cellcollection/item) { get; } | Devuelve una celda por su posición. Solo lectura [`Cell`](../cell). |
| [Presentation](../../aspose.slides/cellcollection/presentation) { get; } | Devuelve la presentación principal de una ColecciónDeCeldas. Solo lectura [`IPresentation`](../ipresentation). |
| [Slide](../../aspose.slides/cellcollection/slide) { get; } | Devuelve la diapositiva principal de una ColecciónDeCeldas. Solo lectura [`IBaseSlide`](../ibaseslide). |
| [SyncRoot](../../aspose.slides/cellcollection/syncroot) { get; } | Devuelve una raíz de sincronización. Solo lectura Object. |

## Métodos

| Nombre | Descripción |
| --- | --- |
| [CopyTo](../../aspose.slides/cellcollection/copyto)(Array, int) | Copia todos los elementos de la colección al arreglo especificado. |
| [GetEnumerator](../../aspose.slides/cellcollection/getenumerator)() | Devuelve un enumerador que itera a través de la colección. |

### Véase También

* interfaz [ICellCollection](../icellcollection)
* espacio de nombres [Aspose.Slides](../../aspose.slides)
* ensamblaje [Aspose.Slides](../../)

<!-- NO EDITAR: generado por xmldocmd para Aspose.Slides.dll -->