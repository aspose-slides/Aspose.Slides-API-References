...
description: 指定限制物件，由基線上的文字以及緊貼其上方或下方的縮小文字組成。
type: docs
weight: 8820
...
指定限制物件，由位於基線上的文字以及緊挨其上方或下方的縮小文字組成。
...
| 名稱 | 說明 |
| --- | --- |
| [MathLimit](mathlimit#constructor)(IMathElement, IMathElement) | 以下限初始化 MathLimit 類別的新執行個體 |
...
| 名稱 | 說明 |
| --- | --- |
| [Base](../../aspose.slides.mathtext/mathlimit/base) { get; } | 基底參數 |
| [Limit](../../aspose.slides.mathtext/mathlimit/limit) { get; } | 限制參數 |
...
| [ToBox](../../aspose.slides.mathtext/mathelementbase/tobox)() | 將此元素放入非可視盒（邏輯分組），用於將方程式或其他數學文字的組件分組。盒狀物件可（例如）作為帶或不帶對齊點的運算子模擬器、作為換行點，或被分組以避免內部換行。 |
