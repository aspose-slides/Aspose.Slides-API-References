...
description: 建立新的資料點並將其加入集合的末端。適用於 chartType 為 Doughnut 子類型之一的系列，另請參閱 ChartTypeCharacterizer.IsChartTypeDoughnutChartType 方法。
type: docs
weight: 110
...
建立新的資料點並將其加入集合的末端。適用於其 chartType 為 Doughnut 子類型之一的系列（另請參閱 ChartTypeCharacterizer.IsChartTypeDoughnut(ChartType) 方法）。
