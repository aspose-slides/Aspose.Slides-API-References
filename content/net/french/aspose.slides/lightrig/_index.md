---
title: LightRig
second_title: Aspose.Sildes pour la référence API .NET
description: Représente LightRig.
type: docs
weight: 7450
url: /fr/aspose.slides/lightrig/
---

## Class LightRig

Représente LightRig.

```csharp
public sealed class LightRig : PVIObject, ILightRig
```

## Propriétés

| Nom | Description |
| --- | --- |
| [AsIPresentationComponent](../../aspose.slides/pviobject/asipresentationcomponent) { get; } | Permet d'obtenir l'interface IPresentationComponent de base. En lecture seule [`IPresentationComponent`](../ipresentationcomponent). |
| [Direction](../../aspose.slides/lightrig/direction) { get; set; } | Direction de la lumière. En lecture/écriture [`LightingDirection`](../lightingdirection). |
| [LightType](../../aspose.slides/lightrig/lighttype) { get; set; } | Représente une lumière préconfigurée qui peut être appliquée à une forme. Le LightRig représente un groupe de lumières orientées d'une manière spécifique par rapport à une scène 3D. En lecture/écriture [`LightRigPresetType`](../lightrigpresettype). |

## Méthodes

| Nom | Description |
| --- | --- |
| override [Equals](../../aspose.slides/pviobject/equals)(object) | Compare avec l'objet spécifié. |
| override [GetHashCode](../../aspose.slides/pviobject/gethashcode)() | Retourne le code de hachage. |
| [GetRotation](../../aspose.slides/lightrig/getrotation)() | Une rotation est définie par l'utilisation d'une coordonnée de latitude, d'une coordonnée de longitude et d'une révolution autour de l'axe comme les coordonnées de latitude et de longitude. Premier élément dans le tableau de retour - latitude, second - longitude, troisième - révolution. Retourne null si aucune rotation définie. |
| [SetRotation](../../aspose.slides/lightrig/setrotation)(float, float, float) | Une rotation est définie par l'utilisation d'une coordonnée de latitude, d'une coordonnée de longitude et d'une révolution autour de l'axe comme les coordonnées de latitude et de longitude. Si l'une des valeurs de coordonnée est float.NaN, toute rotation est indéfinie. |

### Voir aussi

* classe [PVIObject](../pviobject)
* interface [ILightRig](../ilightrig)
* espace de noms [Aspose.Slides](../../aspose.slides)
* assembly [Aspose.Slides](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->