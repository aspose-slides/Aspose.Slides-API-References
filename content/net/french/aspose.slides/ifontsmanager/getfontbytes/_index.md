---
title: GetFontBytes
second_title: Aspose.Slides pour la référence API .NET
description: Récupère le tableau d'octets représentant les données de la police pour un style de police et des données de police spécifiés.
type: docs
weight: 50
url: /fr/aspose.slides/ifontsmanager/getfontbytes/
---

## Méthode IFontsManager.GetFontBytes

Récupère le tableau d'octets représentant les données de la police pour un style de police et des données de police spécifiés.

```csharp
public byte[] GetFontBytes(IFontData fontData, FontStyle fontStyle)
```

| Paramètre | Type | Description |
| --- | --- | --- |
| fontData | IFontData | L'objet de données de police contenant les informations sur la police [`IFontData`](../../ifontdata). |
| fontStyle | FontStyle | Le style de la police pour lequel les données doivent être récupérées FontStyle. |

### Valeur de retour

Un tableau d'octets contenant les données de la police pour le style de police spécifié. Si les données ou le style de police ne sont pas trouvés, renvoie null.

### Exemples

```csharp
[C#]
using (Presentation pres = new Presentation ("Presentation.pptx"))
{
    // Récupérer toutes les polices utilisées dans la présentation
    IFontData[] fonts = pres.FontsManager.GetFonts();

    // Obtenir le tableau d'octets représentant le style régulier de la première police dans la présentation
    byte[] bytes = pres.FontsManager.GetFontBytes(fonts[0], FontStyle.Regular);
}
```

### Voir aussi

* interface [IFontData](../../ifontdata)
* interface [IFontsManager](../../ifontsmanager)
* namespace [Aspose.Slides](../../ifontsmanager)
* assembly [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->