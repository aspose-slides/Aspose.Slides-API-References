---
title: IFormattedTextContainer
second_title: Référence de l'API Aspose.Sildes pour .NET
description: Représente le format de texte du diagramme.
type: docs
weight: 2110
url: /fr/aspose.slides.charts/iformattedtextcontainer/
---
## IFormattedTextContainer interface

Représente le format de texte du diagramme.

```csharp
public interface IFormattedTextContainer : IChartComponent
```

## Propriétés

| Nom | Description |
| --- | --- |
| [AsIChartComponent](../../aspose.slides.charts/iformattedtextcontainer/asichartcomponent) { get; } | Renvoie l'interface IChartComponent. Lecture seule [`IChartComponent`](../ichartcomponent). |
| [TextFormat](../../aspose.slides.charts/iformattedtextcontainer/textformat) { get; } | Renvoie le format de texte du diagramme. Lecture seule [`IChartTextFormat`](../icharttextformat). |

### Voir aussi

* interface [IChartComponent](../ichartcomponent)
* espace de noms [Aspose.Slides.Charts](../../aspose.slides.charts)
* assembly [Aspose.Slides](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->