---
title: ILegendEntryProperties
second_title: Référence de l'API Aspose.Sildes pour .NET
description: Représente les propriétés de la légende d'un graphique.
type: docs
weight: 2150
url: /fr/aspose.slides.charts/ilegendentryproperties/
---
## ILegendEntryProperties interface

Représente les propriétés de la légende d'un graphique.

```csharp
public interface ILegendEntryProperties : IFormattedTextContainer
```

## Propriétés

| Nom | Description |
| --- | --- |
| [AsIFormattedTextContainer](../../aspose.slides.charts/ilegendentryproperties/asiformattedtextcontainer) { get; } | Permet d'obtenir l'interface de base IFormattedTextContainer. Lecture seule [`IFormattedTextContainer`](../iformattedtextcontainer). |
| [Hide](../../aspose.slides.charts/ilegendentryproperties/hide) { get; set; } | Renvoie ou définit le drapeau de masquage de l'entrée de légende. Lecture/écriture Boolean. |

### Voir aussi

* interface [IFormattedTextContainer](../iformattedtextcontainer)
* espace de noms [Aspose.Slides.Charts](../../aspose.slides.charts)
* assembly [Aspose.Slides](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->