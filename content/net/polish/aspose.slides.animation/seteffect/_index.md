---
title: SetEffect
second_title: Aspose.Sildes dla .NET – Dokumentacja API
description: Reprezentuje zestaw efektów dla zachowania animacji.
type: docs
weight: 810
url: /pl/aspose.slides.animation/seteffect/
---
## SetEffect klasa

Reprezentuje zestaw efektów dla zachowania animacji.

```csharp
public class SetEffect : Behavior, ISetEffect
```

## Konstruktory

| Nazwa | Opis |
| --- | --- |
| [SetEffect](seteffect)() | Domyślny konstruktor. |

## Właściwości

| Nazwa | Opis |
| --- | --- |
| [Accumulate](../../aspose.slides.animation/behavior/accumulate) { get; set; } | Reprezentuje, czy zachowania animacji są kumulowane. Odczyt/zapis [`NullableBool`](../../aspose.slides/nullablebool). |
| [Additive](../../aspose.slides.animation/behavior/additive) { get; set; } | Reprezentuje, czy bieżące zachowanie animacji jest łączone z innymi uruchomionymi animacjami. Odczyt/zapis [`BehaviorAdditiveType`](../behavioradditivetype). |
| [Properties](../../aspose.slides.animation/behavior/properties) { get; } | Reprezentuje właściwości zachowania. Tylko do odczytu [`IBehaviorPropertyCollection`](../ibehaviorpropertycollection). |
| [Timing](../../aspose.slides.animation/behavior/timing) { get; set; } | Reprezentuje właściwości czasu dla zachowania efektu. Odczyt/zapis [`ITiming`](../itiming). |
| [To](../../aspose.slides.animation/seteffect/to) { get; set; } | Określa określony atrybut efektu po efekcie animacji. Reprezentuje wartość punktową. Dozwolone typy: bool, ColorFormat, float, int, string. Odczyt/zapis Object. |

### Zobacz również

* klasa [Behavior](../behavior)
* interfejs [ISetEffect](../iseteffect)
* przestrzeń nazw [Aspose.Slides.Animation](../../aspose.slides.animation)
* biblioteka [Aspose.Slides](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->