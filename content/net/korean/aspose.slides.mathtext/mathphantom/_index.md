...
description: 팬텀 수학 객체 ltmphantgt를 나타내며, 자식 요소의 레이아웃에 영향을 주지만 반드시 표시될 필요는 없습니다. 팬텀은 기본 식을 숨기면서 너비, 높이 또는 깊이를 유지하여 수식을 정렬하거나 공간을 예약할 수 있습니다. 가시성 및 기하학적 동작은 Show, ZeroWid, ZeroAsc, ZeroDesc, Transp와 같은 속성으로 제어됩니다.
type: docs
weight: 8920
...
팬텀 수학 객체(&lt;m:phant&gt;)를 나타내며, 자식 요소의 레이아웃에 영향을 주지만 반드시 표시될 필요는 없습니다. 팬텀은 기본 식을 숨기면서 너비, 높이 또는 깊이를 유지하여 수식을 맞추거나 공간을 예약할 수 있습니다. 가시성 및 기하학적 동작은 Show, ZeroWid, ZeroAsc, ZeroDesc, Transp와 같은 속성으로 제어됩니다.
...
| 이름 | 설명 |
| --- | --- |
| [MathPhantom](mathphantom)(IMathElement) | [`MathPhantom`](../mathphantom) 클래스의 새 인스턴스를 지정된 기본 수학 요소를 사용하여 초기화합니다. |
...
| [Show](../../aspose.slides.mathtext/mathphantom/show) { get; set; } | 기본 요소가 표시되는지 여부를 나타내는 값을 가져오거나 설정합니다. |
| [Transp](../../aspose.slides.mathtext/mathphantom/transp) { get; set; } | 클래스 기반 간격 규칙에 대해 팬텀이 투명한지 여부를 나타내는 값을 가져오거나 설정합니다. |
| [ZeroAsc](../../aspose.slides.mathtext/mathphantom/zeroasc) { get; set; } | 기본 요소의 상승(기준선 위 높이)을 0으로 처리할지 여부를 나타내는 값을 가져오거나 설정합니다. |
| [ZeroDesc](../../aspose.slides.mathtext/mathphantom/zerodesc) { get; set; } | 기본 요소의 하강(기준선 아래 깊이)을 0으로 처리할지 여부를 나타내는 값을 가져오거나 설정합니다. |
| [ZeroWidth](../../aspose.slides.mathtext/mathphantom/zerowidth) { get; set; } | 기본 요소의 너비를 0으로 처리할지 여부를 나타내는 값을 가져오거나 설정합니다. |

## 메서드

| 이름 | 설명 |
| --- | --- |
| [Accent](../../aspose.slides.mathtext/mathelementbase/accent)(char) | 이 요소 위에 있는 악센트 표시(문자)를 설정합니다. |
...
| [Enclose](../../aspose.slides.mathtext/mathelementbase/enclose)() | 수학 요소를 괄호로 감싸습니다. |
| virtual [Enclose](../../aspose.slides.mathtext/mathelementbase/enclose)(char, char) | 괄호 등 지정된 문자로 수학 요소를 프레임처럼 감싸습니다. |
| [Function](../../aspose.slides.mathtext/mathelementbase/function)(IMathElement) | 이 인스턴스를 함수 이름으로 사용하여 인수 함수를 취합니다. |
| [Function](../../aspose.slides.mathtext/mathelementbase/function)(string) | 이 인스턴스를 함수 이름으로 사용하여 인수 함수를 취합니다. |
| [GetChildren](../../aspose.slides.mathtext/mathphantom/getchildren)() | 자식 요소를 가져옵니다. |
...
| [ToBox](../../aspose.slides.mathtext/mathelementbase/tobox)() | 이 요소를 비시각적 상자(논리적 그룹화)에 배치합니다. 이는 방정식이나 기타 수학 텍스트의 구성 요소를 그룹화하는 데 사용됩니다. 예를 들어, 상자 객체는 정렬점을 가질 수도 있고 없을 수도 있는 연산자 에뮬레이터 역할을 하거나, 줄 바꿈 지점으로 사용되거나, 내부에 줄 바꿈을 허용하지 않도록 그룹화될 수 있습니다. |
