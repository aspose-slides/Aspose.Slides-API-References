...
| [getSound()](#getSound--) | Ορίστηκε ενσωματωμένος ήχος για το εφέ. |
| [setSound(IAudio value)](#setSound-com.aspose.slides.IAudio-) | Ορίστηκε ενσωματωμένος ήχος για το εφέ. |
| [getStopPreviousSound()](#getStopPreviousSound--) | Αυτό το χαρακτηριστικό καθορίζει εάν το εφέ κίνησης σταματά τον προηγούμενο ήχο. |
| [setStopPreviousSound(boolean value)](#setStopPreviousSound-boolean-) | Αυτό το χαρακτηριστικό καθορίζει εάν το εφέ κίνησης σταματά τον προηγούμενο ήχο. |
| [getAfterAnimationType()](#getAfterAnimationType--) | Ορίζει έναν τύπο μετά-κίνησης για το εφέ. |
| [setAfterAnimationType(int value)](#setAfterAnimationType-int-) | Ορίζει έναν τύπο μετά-κίνησης για το εφέ. |
| [getAfterAnimationColor()](#getAfterAnimationColor--) | Ορίζει ένα χρώμα μετά-κίνησης για το εφέ. |
| [setAfterAnimationColor(IColorFormat value)](#setAfterAnimationColor-com.aspose.slides.IColorFormat-) | Ορίζει ένα χρώμα μετά-κίνησης για το εφέ. |
...
| [getDelayBetweenTextParts()](#getDelayBetweenTextParts--) | Ορίζει καθυστέρηση μεταξύ των τμημάτων του κειμένου που κινούνται (λέξεις ή γράμματα). |
| [setDelayBetweenTextParts(float value)](#setDelayBetweenTextParts-float-) | Ορίζει καθυστέρηση μεταξύ των τμημάτων του κειμένου που κινούνται (λέξεις ή γράμματα). |
...
Ορίστηκε ενσωματωμένος ήχος για το εφέ. Ανάγνωση/εγγραφή [IAudio](../../com.aspose.slides/iaudio).
...
> 
>      // Λαμβάνει την ακολουθία εφέ για τη διαφάνεια
...
Αυτό το χαρακτηριστικό καθορίζει εάν το εφέ κίνησης σταματά τον προηγούμενο ήχο. Ανάγνωση/εγγραφή  boolean .
...
>      {
>          // Αλλάζει το δεύτερο εφέ Enhancements/Sound σε "Stop Previous Sound"
...
Ορίζει έναν τύπο μετά-κίνησης για το εφέ. Ανάγνωση/εγγραφή [AfterAnimationType](../../com.aspose.slides/afteranimationtype)(\#getAfterAnimationType.getAfterAnimationType/\#setAfterAnimationType(int).setAfterAnimationType(int)).
...
>      IEffect firstSlideEffect = presentation.getSlides().get_Item(0).getTimeline().getMainSequence().get_Item(0);
> 
>      // Αλλάζει το εφέ After animation σε "Hide on Next Mouse Click"
...
Ορίζει ένα χρώμα μετά-κίνησης για το εφέ. Ανάγνωση/εγγραφή [IColorFormat](../../com.aspose.slides/icolorformat).
...
Ορίζει έναν τύπο κίνησης κειμένου για το εφέ. Το κείμενο του σχήματος μπορεί να κίνηθεί κατά γράμμα, κατά λέξη ή ολόκληρο ταυτόχρονα. Ανάγνωση/εγγραφή  AnimateTextType (\#getAnimateTextType.getAnimateTextType/\#setAnimateTextType(int).setAnimateTextType(int)).
...
Ορίζει μια καθυστέρηση μεταξύ των τμημάτων του κειμένου που κινούνται (λέξεις ή γράμματα). Μια θετική τιμή καθορίζει το ποσοστό της διάρκειας του εφέ. Μια αρνητική τιμή καθορίζει την καθυστέρηση σε δευτερόλεπτα. Ανάγνωση/εγγραφή  float .
...
> 
>      // Ορίζει την καθυστέρηση μεταξύ των τμημάτων του κειμένου που κινούνται σε 20% της διάρκειας του εφέ.
...
>      firstSlideEffect.setAnimateTextType(AnimateTextType.ByWord);
> 
>      // Ορίζει την καθυστέρηση μεταξύ των τμημάτων του κειμένου που κινούνται στο 20% της διάρκειας του εφέ.
>      firstSlideEffect.setDelayBetweenTextParts(20f);
>  } finally {
>      if (presentation != null) presentation.dispose();
>  }
