...
description: Указывает, как следует обрабатывать повторяющиеся обычные пробелы при экспорте в Markdown.
type: docs
...
| [MultipleSpacesToNbsp](#MultipleSpacesToNbsp) | Преобразует последовательности из двух и более подряд идущих обычных пробелов, сохраняя первый пробел как обычный пробельный символ и заменяя все последующие пробелы на неразрывные пробелы (NBSP). |
### None {#None}
```
...
Преобразует последовательности из двух и более подряд идущих обычных пробелов, сохраняя первый пробел как обычный пробельный символ и заменяя все последующие пробелы на неразрывные пробелы (NBSP).