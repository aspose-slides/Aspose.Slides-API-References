---
title: IPortionCollection
second_title: Справочник API Aspose.Slides для Java
description: Представляет коллекцию фрагментов.
type: docs
url: /ru/com.aspose.slides/iportioncollection/
---
**All Implemented Interfaces:**
com.aspose.ms.System.Collections.Generic.IGenericEnumerable
```
public interface IPortionCollection extends System.Collections.Generic.IGenericEnumerable<IPortion>
```

Представляет коллекцию объектов Portion.
## Methods

| Method | Description |
| --- | --- |
| [get_Item(int index)](#get-Item-int-) | Получает элемент по указанному индексу. |
| [getCount()](#getCount--) | Получает количество фактически содержащихся в коллекции элементов. |
| [add(IPortion value)](#add-com.aspose.slides.IPortion-) | Добавляет объект Portion в конец коллекции. |
| [indexOf(IPortion item)](#indexOf-com.aspose.slides.IPortion-) | Определяет индекс конкретного объекта Portion в коллекции. |
| [insert(int index, IPortion value)](#insert-int-com.aspose.slides.IPortion-) | Вставляет объект Portion в коллекцию по указанному индексу. |
| [clear()](#clear--) | Удаляет все элементы из коллекции. |
| [contains(IPortion item)](#contains-com.aspose.slides.IPortion-) | Определяет, содержит ли [IGenericCollection](../../com.aspose.slides/igenericcollection) конкретное значение. |
| [remove(IPortion item)](#remove-com.aspose.slides.IPortion-) | Удаляет первое вхождение конкретного объекта из [IGenericCollection](../../com.aspose.slides/igenericcollection). |
| [removeAt(int index)](#removeAt-int-) | Удаляет элемент по указанному индексу из коллекции. |
### get_Item(int index) {#get-Item-int-}
```
public abstract IPortion get_Item(int index)
```

Получает элемент по указанному индексу.

**Parameters:**
| Parameter | Type | Description |
| --- | --- | --- |
| index | int |  |

**Returns:**
[IPortion](../../com.aspose.slides/iportion)
### getCount() {#getCount--}
```
public abstract int getCount()
```

Получает количество фактически содержащихся в коллекции элементов. Только для чтения int.

**Returns:**
int
### add(IPortion value) {#add-com.aspose.slides.IPortion-}
```
public abstract void add(IPortion value)
```

Добавляет объект Portion в конец коллекции.

**Parameters:**
| Parameter | Type | Description |
| --- | --- | --- |
| value | [IPortion](../../com.aspose.slides/iportion) | Объект Portion, который будет добавлен в конец коллекции. |

### indexOf(IPortion item) {#indexOf-com.aspose.slides.IPortion-}
```
public abstract int indexOf(IPortion item)
```

Определяет индекс конкретного объекта Portion в коллекции.

**Parameters:**
| Parameter | Type | Description |
| --- | --- | --- |
| item | [IPortion](../../com.aspose.slides/iportion) | Объект Portion, который нужно найти в коллекции. |

**Returns:**
int — индекс item, если он найден в коллекции; иначе -1.
### insert(int index, IPortion value) {#insert-int-com.aspose.slides.IPortion-}
```
public abstract void insert(int index, IPortion value)
```

Вставляет объект Portion в коллекцию по указанному индексу.

**Parameters:**
| Parameter | Type | Description |
| --- | --- | --- |
| index | int | Нулевой индекс, по которому должен быть вставлен объект Portion. |
| value | [IPortion](../../com.aspose.slides/iportion) | Объект Portion, который нужно вставить. |

### clear() {#clear--}
```
public abstract void clear()
```

Удаляет все элементы из коллекции.

### contains(IPortion item) {#contains-com.aspose.slides.IPortion-}
```
public abstract boolean contains(IPortion item)
```

Определяет, содержит ли [IGenericCollection](../../com.aspose.slides/igenericcollection) конкретное значение.

**Parameters:**
| Parameter | Type | Description |
| --- | --- | --- |
| item | [IPortion](../../com.aspose.slides/iportion) | Объект, который нужно найти в [IGenericCollection](../../com.aspose.slides/igenericcollection). |

**Returns:**
boolean — true, если элемент найден в [IGenericCollection](../../com.aspose.slides/igenericcollection); иначе false.
### remove(IPortion item) {#remove-com.aspose.slides.IPortion-}
```
public abstract boolean remove(IPortion item)
```

Удаляет первое вхождение конкретного объекта из [IGenericCollection](../../com.aspose.slides/igenericcollection).

**Parameters:**
| Parameter | Type | Description |
| --- | --- | --- |
| item | [IPortion](../../com.aspose.slides/iportion) | Объект, который нужно удалить из [IGenericCollection](../../com.aspose.slides/igenericcollection). |

**Returns:**
boolean — true, если элемент успешно удалён из [IGenericCollection](../../com.aspose.slides/igenericcollection); иначе false. Метод также возвращает false, если элемент не найден в исходном [IGenericCollection](../../com.aspose.slides/igenericcollection).
### removeAt(int index) {#removeAt-int-}
```
public abstract void removeAt(int index)
```

Удаляет элемент по указанному индексу из коллекции.

**Parameters:**
| Parameter | Type | Description |
| --- | --- | --- |
| index | int | Нулевой индекс элемента, который нужно удалить. |