...
Вызывается перед рендерингом shape. Вызывается один раз для каждой shape. Если эта функция записывает что-либо в generator, генерация текущего изображения слайда будет завершена, добавленный html-фрагмент будет вставлен, и новое изображение будет начато поверх предыдущего.
