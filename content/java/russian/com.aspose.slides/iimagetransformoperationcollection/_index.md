...
Добавляет новый эффект Bi-Level (black/white) в конец коллекции.

**Параметры:**
| Параметр | Тип | Описание |
| --- | --- | --- |
| threshold | float | Порог яркости для эффекта Bi-Level. Значения, равные или превышающие порог, становятся белыми. Значения ниже порога становятся чёрными. |
...
| grow | boolean | Указывает, следует ли расширять границы объекта в результате размытия. true — границы расширяются, false — не расширяются. |
...
| Параметр | Тип | Описание |
| --- | --- | --- |
| hue | float | Оттенок, к которому будет применён оттенок. |
