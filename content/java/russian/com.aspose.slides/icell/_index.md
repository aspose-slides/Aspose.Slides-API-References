...
| [getColSpan()](#getColSpan--) | Возвращает количество столбцов сетки в родительской таблице, которые должна охватывать текущая ячейка. |
| [getRowSpan()](#getRowSpan--) | Возвращает количество строк, которых охватывает объединённая ячейка. |
| [getTextFrame()](#getTextFrame--) | Возвращает текстовый фрейм ячейки. |
| [getTable()](#getTable--) | Возвращает объект родительской Table для ячейки. |
| [isMergedCell()](#isMergedCell--) | Возвращает true, если ячейка объединена с любой смещённой ячейкой, иначе false. |
...
Возвращает количество столбцов сетки в родительской таблице, которые должна охватывать текущая ячейка. Это свойство позволяет ячейкам выглядеть объединёнными, поскольку они охватывают вертикальные границы других ячеек таблицы. Только для чтения int.
...
Возвращает количество строк, которые охватывает объединённая ячейка. Это используется совместно с атрибутом vMerge у других ячеек для указания начальной ячейки горизонтального объединения. Только для чтения int.
...
Возвращает объект родительской Table для ячейки. Только для чтения [ITable](../../com.aspose.slides/itable).
...
Возвращает true, если ячейка объединена с любой смещённой ячейкой, иначе false. Только для чтения boolean.
