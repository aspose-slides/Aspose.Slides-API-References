...
description: Объект обратного вызова, используемый процедурой импорта HTML для получения ссылочных объектов, таких как изображения.
type: docs
...
Использование этого резольвера может создать уязвимость, когда предоставленный клиентом HTML-файл заставит серверное программное обеспечение получать локальный или сетевой файл. Используйте с осторожностью. Рекомендуется не указывать HtmlExternalResolver вообще (будут прочитаны только встроенные объекты) или создать подкласс, который проверяет, является ли указанный URI действительным.
## Конструкторы
...
**Возвращаемое значение:**
java.io.InputStream - Объект InputStream или null, если ресурс не может быть передан потоково.