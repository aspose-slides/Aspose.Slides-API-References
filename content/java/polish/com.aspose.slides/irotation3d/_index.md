...
| Method | Description |
| --- | --- |
| [getRotationX()](#getRotationX--) | Zwraca lub ustawia stopień obrotu wokół osi X, tj. |
| [setRotationX(byte value)](#setRotationX-byte-) | Zwraca lub ustawia stopień obrotu wokół osi X, tj. |
| [getRotationY()](#getRotationY--) | Zwraca lub ustawia stopień obrotu wokół osi Y, tj. |
| [setRotationY(int value)](#setRotationY-int-) | Zwraca lub ustawia stopień obrotu wokół osi Y, tj. |
| [getPerspective()](#getPerspective--) | Zwraca lub ustawia wartość perspektywy (kąt pola widzenia) dla wykresów 3D (między 0 a 100). |
| [setPerspective(byte value)](#setPerspective-byte-) | Zwraca lub ustawia wartość perspektywy (kąt pola widzenia) dla wykresów 3D (między 0 a 100). |
...
Zwraca lub ustawia stopień obrotu wokół osi X, tj. w kierunku Y dla wykresów 3D (między -90 a 90 stopni). Właściwość odpowiada elementowi 21.2.2.157 rotX (X Rotation) w ECMA-376 oraz opcji „Y Rotation” w PowerPoint 2007+. Odczyt/zapis byte.
...
Zwraca lub ustawia stopień obrotu wokół osi Y, tj. w kierunku X dla wykresów 3D (między 0 a 360 stopni). Właściwość odpowiada elementowi 21.2.2.158 rotY (Y Rotation) w ECMA-376 oraz opcji „X Rotation” w PowerPoint 2007+. Odczyt/zapis int.
...
Zwraca lub ustawia wartość perspektywy (kąt pola widzenia) dla wykresów 3D (między 0 a 100). Ignorowane, jeśli wartość właściwości RightAngleAxes jest prawda. Odczyt/zapis byte.
...
Określa, czy osie wykresu są pod kątem prostym, a nie rysowane w perspektywie. Innymi słowy określa, czy kąty osi wykresu są niezależne od obrotu lub elewacji wykresu. Odczyt/zapis boolean.
