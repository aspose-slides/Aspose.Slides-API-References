...
| Metoda | Opis |
| --- | --- |
| [getTextCompression()](#getTextCompression--) | Określa typ kompresji używany do całej treści tekstowej w dokumencie. |
| [setTextCompression(int value)](#setTextCompression-int-) | Określa typ kompresji używany do całej treści tekstowej w dokumencie. |
...
| [getAdditionalCommonFontFamilies()](#getAdditionalCommonFontFamilies--) | Zwraca lub ustawia tablicę nazw rodzin czcionek zdefiniowanych przez użytkownika, które Aspose.Slides powinien traktować jako wspólne. |
| [setAdditionalCommonFontFamilies(String[] value)](#setAdditionalCommonFontFamilies-java.lang.String---) | Zwraca lub ustawia tablicę nazw rodzin czcionek zdefiniowanych przez użytkownika, które Aspose.Slides powinien traktować jako wspólne. |
| [getEmbedFullFonts()](#getEmbedFullFonts--) | Określa, czy wszystkie znaki czcionki powinny być osadzone, czy tylko używany podzbiór. |
| [setEmbedFullFonts(boolean value)](#setEmbedFullFonts-boolean-) | Określa, czy wszystkie znaki czcionki powinny być osadzone, czy tylko używany podzbiór. |
| [getRasterizeUnsupportedFontStyles()](#getRasterizeUnsupportedFontStyles--) | Wskazuje, czy tekst powinien być rastrowany jako bitmapa i zapisywany do PDF, gdy czcionka nie obsługuje stylu pogrubionego. |
| [setRasterizeUnsupportedFontStyles(boolean value)](#setRasterizeUnsupportedFontStyles-boolean-) | Wskazuje, czy tekst powinien być rastrowany jako bitmapa i zapisywany do PDF, gdy czcionka nie obsługuje stylu pogrubionego. |
...
| [getSaveMetafilesAsPng()](#getSaveMetafilesAsPng--) | True, aby skonwertować wszystkie metafile użyte w prezentacji na obrazy PNG. |
| [setSaveMetafilesAsPng(boolean value)](#setSaveMetafilesAsPng-boolean-) | True, aby skonwertować wszystkie metafile użyte w prezentacji na obrazy PNG. |
...
| [getIncludeOleData()](#getIncludeOleData--) | True, aby skonwertować wszystkie dane OLE z prezentacji na osadzone pliki w powstałym PDF. |
| [setIncludeOleData(boolean value)](#setIncludeOleData-boolean-) | True, aby skonwertować wszystkie dane OLE z prezentacji na osadzone pliki w powstałym PDF. |
...
Określa typ kompresji używany do całej treści tekstowej w dokumencie. Odczyt/zapis [PdfTextCompression](../../com.aspose.slides/pdftextcompression).

--------------------

Domyślnie jest [PdfTextCompression.Flate](../../com.aspose.slides/pdftextcompression\#Flate).
...
Wskazuje, czy najskuteczniejsza kompresja (zamiast domyślnej) dla każdego obrazu ma być wybierana automatycznie. Jeśli ustawione na true, dla każdego obrazu w prezentacji zostanie wybrany najodpowiedniejszy algorytm kompresji, co doprowadzi do mniejszego rozmiaru powstałego dokumentu PDF.

--------------------

Wybór najlepszego współczynnika kompresji obrazów jest obliczeniowo kosztowny i wymaga dodatkowej pamięci RAM, a domyślnie opcja jest false.
...
Zwraca lub ustawia tablicę nazw rodzin czcionek zdefiniowanych przez użytkownika, które Aspose.Slides powinien traktować jako wspólne. Odczyt/zapis String[].
...
Wskazuje, czy tekst powinien być rastrowany jako bitmapa i zapisywany do PDF, gdy czcionka nie obsługuje stylu pogrubionego. To podejście może poprawić jakość tekstu w powstałym PDF dla niektórych czcionek. Odczyt/zapis boolean.
...
Domyślnie jest [PdfCompliance.Pdf17](../../com.aspose.slides/pdfcompliance\#Pdf17).
...
Zawiera zestaw flag określających, które uprawnienia dostępu powinny być przyznane, gdy dokument zostaje otwarty z dostępem użytkownika. Zobacz [PdfAccessPermissions](../../com.aspose.slides/pdfaccesspermissions).
...
True, aby skonwertować wszystkie metafile użyte w prezentacji na obrazy PNG. Odczyt/zapis boolean.

--------------------

Domyślnie jest **true**. Dokument PDF może zawierać grafikę wektorową i obrazy rastrowe. Jeśli SaveMetafilesAsPng jest ustawione na true, źródłowy obraz Metafile jest konwertowany do formatu PNG i zapisywany w PDF jako obraz rastrowy. Jeśli SaveMetafilesAsPng jest ustawione na false, źródłowy Metafile jest konwertowany do grafiki wektorowej PDF. Każde podejście ma zalety i wady. Na przykład, jeśli Metafile jest konwertowany do PNG, możliwa jest utrata jakości podczas skalowania wynikowego dokumentu. Jeśli Metafile jest konwertowany do grafiki wektorowej PDF, mogą wystąpić problemy z wydajnością w narzędziu przeglądającym PDF.
...
Zapewnia opcje kontrolujące wygląd obiektów Ink w wyeksportowanym dokumencie. Read-only [IInkOptions](../../com.aspose.slides/iinkoptions)
...
True, aby skonwertować wszystkie dane OLE z prezentacji na osadzone pliki w powstałym PDF. Odczyt/zapis boolean.
