...
Zawiera właściwości formatTextFrameFormatting elementu TextFrame.
## Konstruktory
...
| [getCenterText()](#getCenterText--) | Jeśli NullableBool.True, tekst powinien być wyśrodkowany w ramce poziomo. |
| [setCenterText(byte value)](#setCenterText-byte-) | Jeśli NullableBool.True, tekst powinien być wyśrodkowany w ramce poziomo. |
...
| [getAutofitType()](#getAutofitType--) | Zwraca lub ustawia tryb autofit tekstu. |
| [setAutofitType(byte value)](#setAutofitType-byte-) | Zwraca lub ustawia tryb autofit tekstu. |
...
| [getColumnSpacing()](#getColumnSpacing--) | Zwraca lub ustawia odstęp między kolumnami tekstu w obszarze (w punktach). |
| [setColumnSpacing(double value)](#setColumnSpacing-double-) | Zwraca lub ustawia odstęp między kolumnami tekstu w obszarze (w punktach). |
| [getRotationAngle()](#getRotationAngle--) | Określa własną rotację stosowaną do tekstu w obrębie ramki. |
| [setRotationAngle(float value)](#setRotationAngle-float-) | Określa własną rotację stosowaną do tekstu w obrębie ramki. |
...
| [getKeepTextFlat()](#getKeepTextFlat--) | Pobiera lub ustawia utrzymanie płaskości tekstu, nawet jeśli zastosowano efekt rotacji 3D. |
| [setKeepTextFlat(boolean value)](#setKeepTextFlat-boolean-) | Pobiera lub ustawia utrzymanie płaskości tekstu, nawet jeśli zastosowano efekt rotacji 3D. |
...
Jeśli NullableBool.True, tekst powinien być wyśrodkowany w ramce poziomo. Odczyt/zapis [NullableBool](../../com.aspose.slides/nullablebool).
...
Określa orientację tekstu. Wynikowa wartość wizualnej rotacji tekstu podsumowana z tej właściwości i własnego kąta w właściwości RotationAngle. Odczyt/zapis [TextVerticalType](../../com.aspose.slides/textverticaltype).
...
Zwraca lub ustawia tryb autofit tekstu. Odczyt/zapis [TextAutofitType](../../com.aspose.slides/textautofittype).
...
Zwraca lub ustawia liczbę kolumn w obszarze tekstu. Wartość musi być liczbą dodatnią. W przeciwnym razie zostanie ustawiona na zero. Wartość 0 oznacza nieokreśloną wartość. Odczyt/zapis int.
...
Zwraca lub ustawia odstęp między kolumnami tekstu w obszarze (w punktach). Dotyczy tylko wtedy, gdy istnieje więcej niż jedna kolumna. Wartość musi być liczbą dodatnią. W przeciwnym razie zostanie ustawiona na zero. Odczyt/zapis double.
...
Określa własną rotację stosowaną do tekstu w obrębie ramki. Jeśli nie zostanie podana, używana jest rotacja powiązanego kształtu. Jeśli zostanie podana, jest stosowana niezależnie od kształtu. To znaczy, że kształt może mieć własną rotację oprócz rotacji tekstu. Wynikowa wartość wizualnej rotacji tekstu podsumowana z tej właściwości i predefiniowanego typu pionowego w właściwości TextVerticalType. Odczyt/zapis float.
...
>  zastosowaną do niego. Następnie wynikowy kształt wydawałby się obrócony, 
>  ale tekst w jego wnętrzu wyglądałby, jakby wcale nie był obrócony. 
```
...
Pobiera lub ustawia utrzymanie płaskości tekstu, nawet jeśli zastosowano efekt rotacji 3D. Odczyt/zapis boolean.
