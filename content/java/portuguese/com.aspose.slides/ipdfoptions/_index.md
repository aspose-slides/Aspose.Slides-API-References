...
| [getEmbedTrueTypeFontsForASCII()](#getEmbedTrueTypeFontsForASCII--) | Verdadeiro para incorporar fontes True Type para caracteres ASCII 32-127. |
| [setEmbedTrueTypeFontsForASCII(boolean value)](#setEmbedTrueTypeFontsForASCII-boolean-) | Verdadeiro para incorporar fontes True Type para caracteres ASCII 32-127. |
...
Indica se a compressão mais eficaz (em vez da padrão) para cada imagem deve ser selecionada automaticamente. Se definido como verdadeiro, para cada imagem na apresentação o algoritmo de compressão mais apropriado será escolhido, o que resultará em um tamanho menor do documento PDF resultante.

--------------------

A seleção da melhor taxa de compressão de imagem é computacionalmente cara e consome memória RAM adicional, e esta opção tem o valor false por padrão.
...
Verdadeiro para incorporar fontes True Type para caracteres ASCII 32-127. Fontes para códigos de caracteres superiores a 127 são sempre incorporadas. Leitura/gravação boolean.
...
Indica se o texto deve ser rasterizado como bitmap e salvo em PDF quando a fonte não suporta estilo negrito. Esta abordagem pode melhorar a qualidade do texto no PDF resultante para determinadas fontes. Leitura/gravação boolean.
...
Contém um conjunto de sinalizadores que especificam quais permissões de acesso devem ser concedidas quando o documento é aberto com acesso de usuário. Consulte [PdfAccessPermissions](../../com.aspose.slides/pdfaccesspermissions).
...
Valor: O efeito deste parâmetro depende de alguns fatores. O algoritmo tenta obter o melhor tamanho de imagem de saída de acordo com o valor da propriedade, o tamanho da imagem original e o tamanho do quadro da imagem. O uso de valores de propriedade semelhantes pode produzir o mesmo resultado. Recomenda-se usar passos de 16 ou 32 para obter efeito visível.
