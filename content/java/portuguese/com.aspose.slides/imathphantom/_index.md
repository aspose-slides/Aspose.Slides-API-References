...
description: Representa um objeto matemático phantom ltmphantgt que afeta o layout do seu elemento filho sem necessariamente exibi-lo.
type: docs
...
Representa um objeto matemático phantom (<m:phant>) que afeta o layout do seu elemento filho sem necessariamente exibi-lo. Um phantom pode ocultar sua expressão base enquanto preserva sua largura, altura ou profundidade para alinhar fórmulas ou reservar espaço. A visibilidade e o comportamento geométrico são controlados por propriedades como Show, ZeroWid, ZeroAsc, ZeroDesc e Transp.
...
| [getShow()](#getShow--) | Obtém ou define um valor que indica se o elemento base está exibido. |
| [setShow(boolean value)](#setShow-boolean-) | Obtém ou define um valor que indica se o elemento base está exibido. |
...
| [getTransp()](#getTransp--) | Obtém ou define um valor que indica se o phantom é transparente para regras de espaçamento baseadas em classe. |
| [setTransp(boolean value)](#setTransp-boolean-) | Obtém ou define um valor que indica se o phantom é transparente para regras de espaçamento baseadas em classe. |
### getBase() {#getBase--}
```
...
Quando false, o elemento base é ocultado, mas ainda pode ocupar espaço dependendo de outras configurações do phantom. Corresponde ao atributo OMML m:show.
...
Quando true, o phantom não eleva a linha de base da linha matemática circundante. Corresponde ao atributo OMML m:zeroAsc.
...
Quando true, o phantom não abaixa a linha de base da linha matemática circundante. Corresponde ao atributo OMML m:zeroDesc.
...
Quando true, operadores e símbolos dentro do phantom ainda afetam o espaçamento matemático ao redor do phantom (como se estivesse visível). Quando false, o espaçamento baseado em classe é ignorado. Corresponde ao atributo OMML m:transp.
