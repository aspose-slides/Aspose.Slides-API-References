...
| Método | Descrição |
| --- | --- |
| [isTextHolder()](#isTextHolder--) | Determina se a forma é TextHolder_PPT. |
| [getPlaceholder()](#getPlaceholder--) | Retorna o placeholder para uma forma. |
...
| [getRawFrame()](#getRawFrame--) | Retorna ou define as propriedades da estrutura bruta da forma. |
| [setRawFrame(IShapeFrame value)](#setRawFrame-com.aspose.slides.IShapeFrame-) | Retorna ou define as propriedades da estrutura bruta da forma. |
| [getFrame()](#getFrame--) | Retorna ou define as propriedades da estrutura da forma. |
| [setFrame(IShapeFrame value)](#setFrame-com.aspose.slides.IShapeFrame-) | Retorna ou define as propriedades da estrutura da forma. |
...
| [writeAsSvg(OutputStream stream)](#writeAsSvg-java.io.OutputStream-) | Salva o conteúdo da Forma como arquivo SVG. |
| [writeAsSvg(OutputStream stream, ISVGOptions svgOptions)](#writeAsSvg-java.io.OutputStream-com.aspose.slides.ISVGOptions-) | Salva o conteúdo da Forma como arquivo SVG. |
...
| [getHyperlinkManager()](#getHyperlinkManager--) | Retorna o gerenciador de hyperlinks. |
...
| [getRotation()](#getRotation--) | Retorna ou define o número de graus que a forma especificada é rotacionada ao redor do eixo z. |
| [setRotation(float value)](#setRotation-float-) | Retorna ou define o número de graus que a forma especificada é rotacionada ao redor do eixo z. |
...
| [getBlackWhiteMode()](#getBlackWhiteMode--) | Propriedade que especifica como uma forma será renderizada no modo de exibição em preto e branco. |
| [setBlackWhiteMode(byte value)](#setBlackWhiteMode-byte-) | Propriedade que especifica como uma forma será renderizada no modo de exibição em preto e branco. |
...
| [getOfficeInteropShapeId()](#getOfficeInteropShapeId--) | Retorna um identificador único limitado ao slide que permanece constante durante a vida útil da forma e permite que o PowerPoint ou código interop faça referência à forma de forma confiável a partir de qualquer ponto do documento. |
...
| [isDecorative()](#isDecorative--) | Obtém ou define a opção 'Marcar como decorativo' booleano Leitura/Gravação. |
| [setDecorative(boolean value)](#setDecorative-boolean-) | Obtém ou define a opção 'Marcar como decorativo' booleano Leitura/Gravação. |
...
| [getVisualBounds()](#getVisualBounds--) | Obtém os limites visuais da forma calculados a partir de seu conteúdo renderizado. |
...
Retorna o placeholder para uma forma. Retorna null se a forma não tem placeholder. Somente leitura [IPlaceholder](../../com.aspose.slides/iplaceholder).
...
| --- | --- | --- |
| placeholderToCopyFrom | [IPlaceholder](../../com.aspose.slides/iplaceholder) | Placeholder para copiar o conteúdo. |
...
Retorna ou define as propriedades da estrutura bruta da forma. Leitura/Gravação [IShapeFrame](../../com.aspose.slides/ishapeframe).
...
>  //Esse código pode levar a situações incertas. Portanto, foram adicionadas restrições para o uso de valores indefinidos em IShape.getFrame(). Os valores de x, y, largura, altura, flipH, flipV e ângulo de rotação devem ser definidos (não Float.NaN ou NullableBool.NotDefined). O exemplo de código acima agora lança exceção ArgumentException exception.
>  //Isso se aplica a estes casos de uso:
>  IShape shape = ...;
>  shape.setFrame(...); // não pode ser indefinido
>  IShapeCollection shapes = ...;
>  // parâmetros x, y, largura, altura não podem ser Float.NaN:
>  {
...
>  }
>  //Mas as propriedades de frame de IShape.RawFrame podem ser indefinidas. Isso faz sentido quando a forma está vinculada a um placeholder. Então, valores de frame indefinidos são sobrescritos pela forma placeholder pai. Se não houver placeholder pai para essa forma, ela usa valores padrão ao avaliar o frame efetivo com base em seu IShape.RawFrame. Os valores padrão são 0 e NullableBool.False para x, y, largura, altura, flipH, flipV e ângulo de rotação. Por exemplo:
>  IShape shape = ...; // forma vinculada ao placeholder
>  shape.setRawFrame(new ShapeFrame(Float.NaN, Float.NaN, 100, Float.NaN, NullableBool.NotDefined, NullableBool.NotDefined, 0)); // agora a forma herda valores de x, y, altura, flipH, flipV do placeholder e sobrescreve largura=100 e ângulo de rotação=0.{code}
> ```
...
>  //ou
>  slide.getShapes().addAutoShape(ShapeType.RoundCornerRectangle, Float.NaN, Float.NaN, Float.NaN, Float.NaN);
>  //Tal código pode levar a situações incertas. Então foram adicionadas restrições para o uso de valores indefinidos em IShape.getFrame(). Os valores de x, y, largura, altura, flipH, flipV e ângulo de rotação devem ser definidos (não Float.NaN ou NullableBool.NotDefined). O exemplo acima agora lança exceção ArgumentException.
>  //Isso se aplica a estes casos de uso:
>  IShape shape = ...;
>  shape.setFrame(...); // não pode ser indefinido
>  IShapeCollection shapes = ...;
...
>      shapes.insertVideoFrame(...);
>  }
>  //Mas as propriedades de frame de IShape.RawFrame podem ser indefinidas. Isso faz sentido quando a forma está vinculada a um placeholder. Então os valores de frame indefinidos são sobrescritos pela forma placeholder pai. Se não houver placeholder pai para essa forma, ela usa valores padrão ao avaliar o frame efetivo com base em IShape.RawFrame. Os valores padrão são 0 e NullableBool.False para x, y, largura, altura, flipH, flipV e ângulo de rotação. Por exemplo:
...
Retorna ou define as propriedades da estrutura da forma. Leitura/Gravação [IShapeFrame](../../com.aspose.slides/ishapeframe).

--------------------

O valor de cada propriedade da instância IShapeFrame retornada não é indefinido (não é NaN nem NotDefined). O valor de cada propriedade da instância IShapeFrame atribuída deve ser não indefinido (não deve ser NaN nem NotDefined). Você pode definir valores indefinidos para as propriedades da instância RawFrame.
...
**Retorna:**
[IImage](../../com.aspose.slides/iimage) - Miniatura da forma ou null caso o ShapeThumbnailBounds.Appearance seja usado e a forma não possua elementos visíveis.
...
Salva o conteúdo da Forma como arquivo SVG.
...
Retorna ou define o hyperlink definido para clique do mouse. Leitura/Gravação [IHyperlink](../../com.aspose.slides/ihyperlink).
...
Retorna ou define o hyperlink definido para passar o mouse. Leitura/Gravação [IHyperlink](../../com.aspose.slides/ihyperlink).
...
Retorna o gerenciador de hyperlinks. Somente leitura [IHyperlinkManager](../../com.aspose.slides/ihyperlinkmanager).
...
Retorna a posição de uma forma na ordem z. Shapes[0] retorna a forma na parte de trás da ordem z, e Shapes[Shapes.Count - 1] retorna a forma na parte da frente da ordem z. Somente leitura int.
...
Retorna ou define o número de graus que a forma especificada é rotacionada ao redor do eixo z. Um valor positivo indica rotação no sentido horário; um valor negativo indica rotação no sentido anti-horário. Leitura/Gravação float.

--------------------

O valor retornado está sempre definido (não é Float.NaN). O valor atribuído deve ser definido (não Float.NaN). Você pode definir valores indefinidos para as propriedades da instância RawFrame.
...
O valor retornado está sempre definido (não é Float.NaN). O valor atribuído deve ser definido (não Float.NaN). Você pode definir valores indefinidos para propriedades de instância RawFrame.
...
Obtém ou define a coordenada x do canto superior esquerdo da forma, medida em pontos. Leitura/gravação float.

--------------------

O valor retornado está sempre definido e nunca Float.NaN. O valor atribuído também deve ser definido; atribua Float.NaN apenas a propriedades de uma instância RawFrame.
...
Obtém ou define a coordenada y do canto superior esquerdo da forma, medida em pontos. Leitura/gravação float.
...
Obtém ou define a largura da forma, medida em pontos. Leitura/gravação float.
...
Obtém ou define a altura da forma, medida em pontos. Leitura/gravação float.
...
Propriedade especifica como uma forma será renderizada no modo de exibição em preto e branco.. Leitura/gravação [BlackWhiteMode](../../com.aspose.slides/blackwhitemode).
...
Retorna um identificador interno, de escopo de apresentação, destinado ao uso por complementos ou outro código. Como este valor pode ser reatribuído pelo usuário ou programaticamente, não deve ser tratado como uma chave única persistente. Somente leitura long. Veja também \#getOfficeInteropShapeId.getOfficeInteropShapeId.
...
Retorna um identificador único de escopo de slide que permanece constante durante a vida útil da forma e permite que o PowerPoint ou código interop faça referência à forma de forma confiável a partir de qualquer ponto no documento. Somente leitura long. Veja também \#getUniqueId.getUniqueId.
...
Retorna ou define o nome de uma forma. Não pode ser nulo. Use uma string vazia se necessário. Leitura/gravação String.
...
Obtém ou define a opção 'Marcar como decorativo'. Leitura/gravação boolean.
...
Propriedade \#getParentGroup.getParentGroup retorna o objeto GroupShape pai se a forma estiver agrupada.
