...
description: Informação sobre o arquivo de apresentação
type: docs
...
| Método | Descrição |
| --- | --- |
| [isEncrypted()](#isEncrypted--) | Retorna True se a apresentação vinculada estiver criptografada, caso contrário False. |
...
| [checkWriteProtection(String password)](#checkWriteProtection-java.lang.String-) | Verifica se uma senha de modificação está correta para uma apresentação protegida contra gravação. |
...
Verifica se uma senha de modificação está correta para uma apresentação protegida contra gravação.
...
1. Você deve verificar a (\#isWriteProtected.isWriteProtected) propriedade antes de chamar este método. 2. Quando a senha for nula ou vazia, este método retorna false. |
