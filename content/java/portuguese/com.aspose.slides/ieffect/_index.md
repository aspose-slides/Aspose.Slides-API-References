...
| [getTargetShape()](#getTargetShape--) | Retorna a forma de destino para o efeito. |
...
| [getDelayBetweenTextParts()](#getDelayBetweenTextParts--) | Define um atraso entre partes de texto animadas (palavras ou letras). |
| [setDelayBetweenTextParts(float value)](#setDelayBetweenTextParts-float-) | Define um atraso entre partes de texto animadas (palavras ou letras). |
...
Retorna a forma de destino para o efeito. Somente leitura [IShape](../../com.aspose.slides/ishape).
...
>      {
>          // Altera o som do segundo efeito Enhancements/Sound para "Stop Previous Sound"
...
>      if (firstSlideEffect.getSound() != null)
>      {
>          // Altere o som do segundo efeito Enhancements/Sound para "Stop Previous Sound"
>          secondSlideEffect.setStopPreviousSound(true);
>      }
>  } finally {
>      if (presentation != null) presentation.dispose();
...
>      IEffect firstSlideEffect = presentation.getSlides().get_Item(0).getTimeline().getMainSequence().get_Item(0);
> 
>      // Altera a animação After do efeito para "Hide on Next Mouse Click"
...
>      IEffect firstSlideEffect = presentation.getSlides().get_Item(0).getTimeline().getMainSequence().get_Item(0);
> 
>      // Altera o tipo de animação After do efeito para "Color"
>      firstSlideEffect.setAfterAnimationType(AfterAnimationType.Color);
> 
>      // Define a cor da animação After do efeito.
...
Define um atraso entre partes de texto animadas (palavras ou letras). Um valor positivo especifica a porcentagem da duração do efeito. Um valor negativo especifica o atraso em segundos. Leitura/Gravação  float .
...
> 
>      // Define o atraso entre partes de texto animadas para 20% da duração do efeito.
