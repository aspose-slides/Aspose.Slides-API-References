...
Representa uma coleção de todos os slides de layout na apresentação. Estende a classe LayoutSlideCollection com métodos para adicionar/duplicar slides de layout no contexto da união das coleções individuais de slides mestres.
...
Ao clonar um layout entre diferentes apresentações, o mestre do layout também pode ser clonado para manter a formatação de origem. Um registro interno é usado para rastrear mestres clonados automaticamente, a fim de evitar a criação de múltiplas clones do mesmo slide mestre. A clonagem manual de slides mestres não será nem impedida nem registrada.
...
1) O novo layout será vinculado ao mestre definido na apresentação de destino. Portanto, isto é análogo a copiar/colar com a opção "Usar Tema de Destino" no PowerPoint. 2) Análogo a este método é o método [IMasterLayoutSlideCollection.addClone(ILayoutSlide)](../../com.aspose.slides/imasterlayoutslidecollection\#addClone-ILayoutSlide-) acessado via a propriedade ([IMasterSlide.getLayoutSlides](../../com.aspose.slides/imasterslide\#getLayoutSlides)).
...
| layoutName | java.lang.String | Nome para um novo layout. Se o nome fornecido já estiver em uso, será lançada uma ArgumentException. Se for passado um parâmetro nulo, o nome será gerado automaticamente com base no tipo de layout fornecido (por exemplo "Title Slide" ou "1\_Title Slide", "2\_..", etc.). |

--------------------

1) O layout adicionado para o valor SlideLayoutType.Custom de layoutType não contém marcadores de posição nem formas. 2) Análogo a este método é o método [IMasterLayoutSlideCollection.add(byte,String)](../../com.aspose.slides/imasterlayoutslidecollection\#add-byte-String-) acessado via a propriedade ([IMasterSlide.getLayoutSlides](../../com.aspose.slides/imasterslide\#getLayoutSlides)).
