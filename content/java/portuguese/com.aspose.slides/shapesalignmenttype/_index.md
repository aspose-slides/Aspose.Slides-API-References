---
title: ShapesAlignmentType
second_title: Aspose.Slides para Referência da API Java
description: Define uma forma de alterar o posicionamento das formas selecionadas no slide.
type: docs
url: /pt/com.aspose.slides/shapesalignmenttype/
---
**Herança:**
java.lang.Object, com.aspose.ms.System.ValueType, com.aspose.ms.System.Enum
```
public final class ShapesAlignmentType extends System.Enum
```

Define uma forma de alterar o posicionamento das formas selecionadas no slide.
## Campos

| Campo | Descrição |
| --- | --- |
| [AlignLeft](#AlignLeft) | Alinhar à esquerda |
| [AlignRight](#AlignRight) | Alinhar à direita |
| [AlignCenter](#AlignCenter) | Alinhar ao centro |
| [AlignTop](#AlignTop) | Alinhar ao topo |
| [AlignMiddle](#AlignMiddle) | Alinhar ao meio |
| [AlignBottom](#AlignBottom) | Alinhar à parte inferior |
| [DistributeHorizontally](#DistributeHorizontally) | Distribuir horizontalmente |
| [DistributeVertically](#DistributeVertically) | Distribuir verticalmente |
### AlignLeft {#AlignLeft}
```
public static final int AlignLeft
```

Alinhar à esquerda

### AlignRight {#AlignRight}
```
public static final int AlignRight
```

Alinhar à direita

### AlignCenter {#AlignCenter}
```
public static final int AlignCenter
```

Alinhar ao centro

### AlignTop {#AlignTop}
```
public static final int AlignTop
```

Alinhar ao topo

### AlignMiddle {#AlignMiddle}
```
public static final int AlignMiddle
```

Alinhar ao meio

### AlignBottom {#AlignBottom}
```
public static final int AlignBottom
```

Alinhar à parte inferior

### DistributeHorizontally {#DistributeHorizontally}
```
public static final int DistributeHorizontally
```

Distribuir horizontalmente

### DistributeVertically {#DistributeVertically}
```
public static final int DistributeVertically
```

Distribuir verticalmente