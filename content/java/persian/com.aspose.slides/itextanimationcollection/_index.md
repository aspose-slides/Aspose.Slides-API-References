---
title: ITextAnimationCollection
second_title: Aspose.Slides برای Java مرجع API
description: نمایانگر مجموعه‌ای از انیمیشن‌های متن.
type: docs
url: /fa/com.aspose.slides/itextanimationcollection/
---
**تمام رابط‌های پیاده‌سازی شده:**
com.aspose.slides.IGenericCollection
```
public interface ITextAnimationCollection extends IGenericCollection<ITextAnimation>
```

نمایشگر مجموعه‌ای از انیمیشن‌های متن.
## روش‌ها

| متد | توضیح |
| --- | --- |
| [get_Item(int index)](#get-Item-int-) | عنصری را بر اساس شاخص باز می‌گرداند. |
| [get_Item(IShape shape)](#get-Item-com.aspose.slides.IShape-) | تمام عناصر را باز می‌گرداند |
### get_Item(int index) {#get-Item-int-}
```
public abstract ITextAnimation get_Item(int index)
```

عنصری را بر اساس شاخص باز می‌گرداند.

**پارامترها:**
| پارامتر | نوع | توضیح |
| --- | --- | --- |
| index | int |  |

**بازده:**
[ITextAnimation](../../com.aspose.slides/itextanimation)
### get_Item(IShape shape) {#get-Item-com.aspose.slides.IShape-}
```
public abstract ITextAnimation[] get_Item(IShape shape)
```

تمام عناصر را باز می‌گرداند

**پارامترها:**
| پارامتر | نوع | توضیح |
| --- | --- | --- |
| shape | [IShape](../../com.aspose.slides/ishape) | عنصر [IShape](../../com.aspose.slides/ishape). |

**بازده:**
com.aspose.slides.ITextAnimation[] - آرایه‌ای از [ITextAnimation](../../com.aspose.slides/itextanimation)