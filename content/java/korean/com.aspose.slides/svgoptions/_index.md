...
| [SVGOptions(ILinkEmbedController linkEmbedController)](#SVGOptions-com.aspose.slides.ILinkEmbedController-) | 링크 임베드 컨트롤러 객체를 지정하여 SVGOptions 클래스의 새 인스턴스를 초기화합니다. |
...
| [getUseFrameSize()](#getUseFrameSize--) | 텍스트 프레임이 렌더링 영역에 포함될지 여부를 결정합니다. |
| [setUseFrameSize(boolean value)](#setUseFrameSize-boolean-) | 텍스트 프레임이 렌더링 영역에 포함될지 여부를 결정합니다. |
| [getUseFrameRotation()](#getUseFrameRotation--) | 렌더링 시 지정된 형태의 회전을 수행할지 여부를 결정합니다. |
| [setUseFrameRotation(boolean value)](#setUseFrameRotation-boolean-) | 렌더링 시 지정된 형태의 회전을 수행할지 여부를 결정합니다. |
...
| [getDisableLineEndCropping()](#getDisableLineEndCropping--) | SVG 1.1은 마커에 대한 인셋 정의 기능이 없습니다. |
| [setDisableLineEndCropping(boolean value)](#setDisableLineEndCropping-boolean-) | SVG 1.1은 마커에 대한 인셋 정의 기능이 없습니다. |
| [getDefault()](#getDefault--) | 기본 설정을 반환합니다. |
| [getSimple()](#getSimple--) | 가장 단순하고 작은 SVG 파일 생성을 위한 설정을 반환합니다. |
...
| [getShapeFormattingController()](#getShapeFormattingController--) | 사용자가 형태 변환을 제어할 수 있는 콜백 인터페이스를 반환하고 설정합니다. |
| [setShapeFormattingController(ISvgShapeFormattingController value)](#setShapeFormattingController-com.aspose.slides.ISvgShapeFormattingController-) | 사용자가 형태 변환을 제어할 수 있는 콜백 인터페이스를 반환하고 설정합니다. |
...
| [getDisableFontLigatures()](#getDisableFontLigatures--) | 텍스트가 합자(ligature)를 사용하지 않고 렌더링되는지 여부를 나타내는 값을 가져오거나 설정합니다. |
| [setDisableFontLigatures(boolean value)](#setDisableFontLigatures-boolean-) | 텍스트가 합자(ligature)를 사용하지 않고 렌더링되는지 여부를 나타내는 값을 가져오거나 설정합니다. |
...
| 매개변수 | 유형 | 설명 |
| --- | --- | --- |
| linkEmbedController | [ILinkEmbedController](../../com.aspose.slides/ilinkembedcontroller) | 링크 임베드 컨트롤러 참조. |

--------------------

링크 임베드 컨트롤러는 리소스(예: 이미지)를 임베드해야 하는지 외부 리소스로 참조해야 하는지 결정하는 위임 객체입니다.
...
텍스트 프레임이 렌더링 영역에 포함될지 여부를 결정합니다. 읽기/쓰기 boolean . 기본값은 false.
...
렌더링 시 지정된 형태의 회전을 수행할지 여부를 결정합니다. 읽기/쓰기 boolean . 기본값은 true.
...
SVG 1.1은 마커에 대한 인셋 정의 기능이 없습니다. Aspose.Slides SVG 작성 엔진은 해당 문제에 대한 해결책으로 화살표가 있는 선의 끝을 잘라 마커와 겹치지 않게 합니다. 이 옵션은 이러한 동작을 끕니다. 읽기/쓰기 boolean.
...
가장 단순하고 작은 SVG 파일 생성을 위한 설정을 반환합니다. 읽기 전용 [SVGOptions](../../com.aspose.slides/svgoptions).
...
사용자가 형태 변환을 제어할 수 있는 콜백 인터페이스를 반환하고 설정합니다. 읽기/쓰기 [ISvgShapeFormattingController](../../com.aspose.slides/isvgshapeformattingcontroller).
...
잘린 부분이 문서의 일부로 남아 있는지 여부를 나타내는 부울 플래그입니다. true이면 잘린 부분이 제거되고, false이면 문서에 직렬화됩니다(파일 크기가 커질 수 있음).
...
텍스트가 합자(ligature)를 사용하지 않고 렌더링되는지 여부를 나타내는 값을 가져오거나 설정합니다. true로 설정하면 렌더링된 출력에서 합자가 비활성화됩니다. 기본값은 false입니다.
