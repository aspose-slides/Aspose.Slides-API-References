...
| [getDeletePicturesCroppedAreas()](#getDeletePicturesCroppedAreas--) | 잘린 부분이 문서에 남아 있는지 여부를 나타내는 부울 플래그입니다. |
| [setDeletePicturesCroppedAreas(boolean value)](#setDeletePicturesCroppedAreas-boolean-) | 잘린 부분이 문서에 남아 있는지 여부를 나타내는 부울 플래그입니다. |
| [getSvgResponsiveLayout()](#getSvgResponsiveLayout--) | SVG 컨테이너에서 너비와 높이 속성을 제외하여 레이아웃을 반응형으로 만듭니다. |
| [setSvgResponsiveLayout(boolean value)](#setSvgResponsiveLayout-boolean-) | SVG 컨테이너에서 너비와 높이 속성을 제외하여 레이아웃을 반응형으로 만듭니다. |
| [getDisableFontLigatures()](#getDisableFontLigatures--) | 텍스트를 합자 없이 렌더링할지 여부를 나타내는 값을 반환하거나 설정합니다. |
| [setDisableFontLigatures(boolean value)](#setDisableFontLigatures-boolean-) | 텍스트를 합자 없이 렌더링할지 여부를 나타내는 값을 반환하거나 설정합니다. |
| [getSlidesLayoutOptions()](#getSlidesLayoutOptions--) | 프레젠테이션을 내보낼 때 슬라이드가 페이지에 배치되는 모드를 반환하거나 설정합니다 [ISlidesLayoutOptions](../../com.aspose.slides/islideslayoutoptions). |
| [setSlidesLayoutOptions(ISlidesLayoutOptions value)](#setSlidesLayoutOptions-com.aspose.slides.ISlidesLayoutOptions-) | 프레젠테이션을 내보낼 때 슬라이드가 페이지에 배치되는 모드를 반환하거나 설정합니다 [ISlidesLayoutOptions](../../com.aspose.slides/islideslayoutoptions). |
...
PDF 형식으로 저장할 때 문서 내부 이미지의 품질을 가져오거나 설정하려면 이 속성을 사용합니다. 값은 0에서 100 사이이며 0은 최악의 품질이지만 최대 압축, 100은 최고의 품질이지만 최소 압축을 의미합니다.
...
잘린 부분이 문서에 남아 있는지 여부를 나타내는 부울 플래그입니다. true이면 잘린 부분이 제거되고, false이면 문서에 직렬화됩니다(파일 크기가 더 커질 수 있음). 읽기/쓰기 boolean.
...
SVG 컨테이너에서 너비와 높이 속성을 제외하여 레이아웃을 반응형으로 만들려면 true를 사용합니다. 그렇지 않으면 false. 읽기/쓰기 boolean.
...
텍스트를 합자 없이 렌더링할지 여부를 나타내는 값을 반환하거나 설정합니다. true로 설정하면 렌더링 출력에서 합자가 비활성화됩니다. 기본값은 false입니다.
...
프레젠테이션을 내보낼 때 슬라이드가 페이지에 배치되는 모드를 반환하거나 설정합니다 [ISlidesLayoutOptions](../../com.aspose.slides/islideslayoutoptions).
