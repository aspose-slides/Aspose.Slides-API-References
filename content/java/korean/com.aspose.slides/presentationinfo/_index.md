...
| [isPasswordProtected()](#isPasswordProtected--) | 바인드된 프레젠테이션이 열기 암호로 보호되는지 여부를 나타내는 값을 반환합니다. |
...
| [checkPassword(String password)](#checkPassword-java.lang.String-) | 열기 암호로 보호된 프레젠테이션에 대한 비밀번호가 올바른지 확인합니다. |
| [checkWriteProtection(String password)](#checkWriteProtection-java.lang.String-) | 쓰기 보호된 프레젠테이션에 대한 수정 비밀번호가 올바른지 확인합니다. |
| [readDocumentProperties()](#readDocumentProperties--) | 바인드된 프레젠테이션의 문서 속성을 반환합니다. |
...
| [writeBindedPresentation(OutputStream stream)](#writeBindedPresentation-java.io.OutputStream-) | 바인드된 프레젠테이션을 스트림에 기록합니다. |
| [writeBindedPresentation(String file)](#writeBindedPresentation-java.lang.String-) | 바인드된 프레젠테이션을 파일에 기록합니다. |
...
바인드된 프레젠테이션이 열기 암호로 보호되는지 여부를 나타내는 값을 반환합니다.
...
프레젠테이션이 열기 암호로 보호되는 경우, 속성 값은 NotDefined와 같습니다.
...
열기 암호로 보호된 프레젠테이션에 대한 비밀번호가 올바른지 확인합니다.
...
**반환값:**  
boolean - 프레젠테이션이 열기 암호로 보호되고 비밀번호가 올바른 경우 True를 반환하며, 그렇지 않으면 false를 반환합니다.
...
> ```
> 이 샘플은 #updateDocumentProperties(IDDocumentProperties).updateDocumentProperties(IDDocumentProperties) 메서드를 호출하여
>  #readDocumentProperties.readDocumentProperties 메서드 호출로 반환된 문서 속성을 업데이트합니다.
>  
...
바인드된 프레젠테이션을 스트림에 기록합니다.

**매개변수:**
| 매개변수 | 타입 | 설명 |
| --- | --- | --- |
| stream | java.io.OutputStream | 스트림은 시킹 가능하고 쓰기 가능해야 합니다. |
