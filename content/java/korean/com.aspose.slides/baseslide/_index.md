...
| [equals(IBaseSlide slide)](#equals-com.aspose.slides.IBaseSlide-) | 두 IBaseSlide 인스턴스가 같은지 여부를 판단합니다. |
| [joinPortionsWithSameFormatting()](#joinPortionsWithSameFormatting--) | 모든 허용 가능한 모양의 모든 단락에서 동일한 서식의 실행을 결합합니다. |
| [joinPortionsWithSameFormatting(IShapeCollection collection)](#joinPortionsWithSameFormatting-com.aspose.slides.IShapeCollection-) | 모든 허용 가능한 모양의 모든 단락에서 동일한 서식의 실행을 결합합니다. |
| [createThemeEffective()](#createThemeEffective--) | 이 슬라이드에 대한 실효 테마를 반환합니다. |
| [getCustomData()](#getCustomData--) | 슬라이드의 사용자 정의 데이터를 반환합니다. |
| [getTimeline()](#getTimeline--) | 애니메이션 타임라인 개체를 반환합니다. |
| [getSlideShowTransition()](#getSlideShowTransition--) | 슬라이드 쇼 중 지정된 슬라이드가 진행되는 방법에 대한 정보를 포함하는 Transition 개체를 반환합니다. |
...
두 IBaseSlide 인스턴스가 같은지 여부를 판단합니다. 반환값은 슬라이드의 구조와 정적 콘텐츠를 기준으로 계산됩니다. 모든 모양, 스타일, 텍스트, 애니메이션 및 기타 설정 등이 동일하면 두 슬라이드는 동일합니다. 비교 시 고유 식별자 값(예: SlideId) 및 동적 콘텐츠(예: 날짜 자리표시자의 현재 날짜값)는 고려되지 않습니다.
...
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| slide | [IBaseSlide](../../com.aspose.slides/ibaseslide) | 현재 IBaseSlide와 비교할 IBaseSlide입니다. |

**반환값:**  
boolean - 지정된 IBaseSlide가 현재 IBaseSlide와 동일하면 **true**, 그렇지 않으면 **false**.
...
모든 허용 가능한 모양의 모든 단락에서 동일한 서식의 실행을 결합합니다.
...
애니메이션 타임라인 개체를 반환합니다. 읽기 전용 [IAnimationTimeLine](../../com.aspose.slides/ianimationtimeline).
...
지정된 슬라이드가 슬라이드 쇼 중에 어떻게 진행되는지에 대한 정보를 포함하는 Transition 개체를 반환합니다. 읽기 전용 [ISlideShowTransition](../../com.aspose.slides/islideshowtransition).
...
마스터 슬라이드의 모양을 슬라이드에 표시할지 여부를 지정합니다. 마스터 슬라이드 자체에 대해서는 이 속성이 항상 false를 반환합니다. 읽기/쓰기 boolean.
...
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| altText | java.lang.String | 대체 텍스트입니다. |

**반환값:**  
[IShape](../../com.aspose.slides/ishape) - Shape 객체 또는 null.
