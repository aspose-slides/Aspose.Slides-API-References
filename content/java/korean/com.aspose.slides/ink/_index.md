...
description: 슬라이드의 잉크 객체를 나타냅니다.
type: docs
...
| [getInkEffectImages()](#getInkEffectImages--) | 잉크 브러시의 시각 효과를 시뮬레이션하기 위해 사용되는 사용자 정의 이미지 컬렉션을 가져옵니다. |
### getTraces() {#getTraces--}
```
...
특정 [InkEffectType](../../com.aspose.slides/inkeffecttype) 값(예: Galaxy, Rainbow 등)으로 잉크를 렌더링할 때 사용되는 사용자 정의 이미지를 컬렉션으로 가져옵니다. 자체 이미지를 제공함으로써 각 잉크 효과가 표시되는 방식을 제어할 수 있습니다.
...
이 속성은 기본 잉크 효과 텍스처를 사용자 정의 텍스처로 교체할 수 있게 하며, 기본 자산이 라이선스 제한이나 런타임 시 사용 불가능할 경우 특히 유용합니다. 사전의 각 항목은 [InkEffectType](../../com.aspose.slides/inkeffecttype) 값과 해당 [IImage](../../com.aspose.slides/iimage) 객체(예: Bitmap 또는 Aspose 이미지 인터페이스)를 연결해야 합니다.
