...
| 메서드 | 설명 |
| --- | --- |
| [getAccelerate()](#getAccelerate--) | 가속 동작 효과의 지속 시간 백분율을 설명합니다. |
| [setAccelerate(float value)](#setAccelerate-float-) | 가속 동작 효과의 지속 시간 백분율을 설명합니다. |
| [getDecelerate()](#getDecelerate--) | 감속 동작 효과의 지속 시간 백분율을 설명합니다. |
| [setDecelerate(float value)](#setDecelerate-float-) | 감속 동작 효과의 지속 시간 백분율을 설명합니다. |
...
| [getRestart()](#getRestart--) | 효과가 완료된 후 재시작할지 여부를 지정합니다. |
| [setRestart(int value)](#setRestart-int-) | 효과가 완료된 후 재시작할지 여부를 지정합니다. |
| [getRewind()](#getRewind--) | 이 속성은 재생이 끝났을 때 효과를 되감을지 여부를 지정합니다. |
| [setRewind(boolean value)](#setRewind-boolean-) | 이 속성은 재생이 끝났을 때 효과를 되감을지 여부를 지정합니다. |
...
가속 동작 효과의 지속 시간 백분율을 설명합니다. 읽기/쓰기 float.
...
감속 동작 효과의 지속 시간 백분율을 설명합니다. 읽기/쓰기 float.
...
>      // 메인 시퀀스의 첫 번째 효과를 가져옵니다.
>      IEffect effect = effectsSequence.get_Item(0);
>      // 효과의 Timing/Repeat을 "다음 클릭까지" 로 변경합니다
...
효과가 완료된 후 재시작할지 여부를 지정합니다. 읽기/쓰기 [EffectRestartType](../../com.aspose.slides/effectrestarttype).
...
이 속성은 재생이 끝났을 때 효과를 되감을지 여부를 지정합니다. 읽기/쓰기 boolean.
