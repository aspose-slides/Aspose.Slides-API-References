...
| [getBasePath()](#getBasePath--) | 리소스가 포함된 문서를 저장할 기본 경로를 지정합니다. |
| [setBasePath(String value)](#setBasePath-java.lang.String-) | 리소스가 포함된 문서를 저장할 기본 경로를 지정합니다. |
...
| [getNewLineType()](#getNewLineType--) | 생성된 문서에 새 줄이 \\r(Macintosh) 또는 \\n(Unix) 혹은 \\r\\n(Windows) 중 어떤 것이 포함될지 지정합니다. |
| [setNewLineType(int value)](#setNewLineType-int-) | 생성된 문서에 새 줄이 \\r(Macintosh) 또는 \\n(Unix) 혹은 \\r\\n(Windows) 중 어떤 것이 포함될지 지정합니다. |
| [getShowComments()](#getShowComments--) | 생성된 문서에 주석을 표시할지 여부를 지정합니다. |
| [setShowComments(boolean value)](#setShowComments-boolean-) | 생성된 문서에 주석을 표시할지 여부를 지정합니다. |
...
| [getShowSlideNumber()](#getShowSlideNumber--) | 생성된 문서에 각 슬라이드 번호를 표시할지 여부를 지정합니다. |
| [setShowSlideNumber(boolean value)](#setShowSlideNumber-boolean-) | 생성된 문서에 각 슬라이드 번호를 표시할지 여부를 지정합니다. |
...
| [getHandleRepeatedSpaces()](#getHandleRepeatedSpaces--) | Markdown 내보내기 시 반복되는 일반 공백 문자를 처리하는 방식을 지정합니다. |
| [setHandleRepeatedSpaces(int value)](#setHandleRepeatedSpaces-int-) | Markdown 내보내기 시 반복되는 일반 공백 문자를 처리하는 방식을 지정합니다. |
| [getRemoveEmptyLines()](#getRemoveEmptyLines--) | true로 설정하면 최종 Markdown 출력에서 빈 라인이나 공백만 있는 라인을 제거합니다. |
| [setRemoveEmptyLines(boolean value)](#setRemoveEmptyLines-boolean-) | true로 설정하면 최종 Markdown 출력에서 빈 라인이나 공백만 있는 라인을 제거합니다. |
| [setImageSaving(MarkdownSaveOptions.MarkdownImageSavingHandler event)](#setImageSaving-com.aspose.slides.MarkdownSaveOptions.MarkdownImageSavingHandler-) | Markdown 내보내기 중 SVG가 아닌 각 이미지(비트맵 또는 메타파일)에 대해 발생합니다. |
| [setSvgImageSaving(MarkdownSaveOptions.MarkdownSvgImageSavingHandler event)](#setSvgImageSaving-com.aspose.slides.MarkdownSaveOptions.MarkdownSvgImageSavingHandler-) | Markdown 내보내기 중 각 SVG 이미지에 대해 발생합니다. |
...
프레젠테이션을 변환하기 위한 마크다운 사양을 지정합니다. 기본값은  TextOnly .
...
리소스가 포함된 문서를 저장할 기본 경로를 지정합니다. 기본값은 애플리케이션 현재 디렉터리입니다.
...
이미지를 저장할 폴더 이름을 지정합니다. 기본값은  Images .
...
생성된 문서에 새 줄이 \\r(Macintosh) 또는 \\n(Unix) 혹은 \\r\\n(Windows) 중 어떤 것이 포함될지 지정합니다. 기본값은  Unix .
...
생성된 문서에 주석을 표시할지 여부를 지정합니다. 기본값은 false.
...
생성된 문서에 각 슬라이드 번호를 표시할지 여부를 지정합니다. 기본값은 false.
...
프레젠테이션을 변환하기 위한 마크다운 사양을 지정합니다. 기본값은  Multi-markdown .
...
Markdown 출력에서 슬라이드 번호 헤더에 사용되는 형식 문자열을 가져오거나 설정합니다. 형식에는 "\{0\}" 자리표시자가 포함되어야 하며, 내보내기 시 슬라이드 인덱스로 대체됩니다. 예시: "\# Slide \{0\}"는 "\# Slide 1", "\# Slide 2" 등을 생성합니다.
...
Markdown 내보내기 시 반복되는 일반 공백 문자를 처리하는 방식을 지정합니다. 이 속성은 연속 공백이 다음 중 어떤 방식으로 처리될지 정의합니다: - 일반 공백 문자 그대로 보존, - 일반 공백과 비공백(space) 문자(�)를 교대로 사용, - 첫 번째 이후 모두 비공백 문자로 교체하여 Markdown 출력에서 시각적 정렬을 유지. 기본값은 [HandleRepeatedSpaces.AlternateSpacesToNbsp](../../com.aspose.slides/handlerepeatedspaces\#AlternateSpacesToNbsp).
...
true로 설정하면 최종 Markdown 출력에서 빈 라인이나 공백만 있는 라인을 제거합니다. 기본값은 false.
...
Markdown 내보내기 중 SVG가 아닌 각 이미지(비트맵 또는 메타파일)에 대해 발생합니다. 이미지 저장 및 참조 방식을 사용자 정의할 수 있습니다. 처리되지 않으면 이미지가 로컬에 상대 링크로 저장됩니다.
...
Markdown 내보내기 중 각 SVG 이미지에 대해 발생합니다. 기본 저장 및 링크 생성을 재정의할 수 있습니다. 처리되지 않으면 SVG가 로컬에 상대 링크로 저장됩니다.
