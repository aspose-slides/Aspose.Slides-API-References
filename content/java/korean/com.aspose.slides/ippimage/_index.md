...
| [getContentType()](#getContentType--) | 이미지의 MIME 유형을 반환합니다(\#getBinaryData.getBinaryData에 인코딩됨). |
...
이미지의 복사본을 반환합니다. 읽기 전용 #getImage.getImage.
...
이미지를 교체합니다. 주의: 이미지가 메타파일인 경우 래스터화됩니다. 대신 replaceImage(byte[])를 사용하세요.
...
이미지의 MIME 유형을 반환합니다(\#getBinaryData.getBinaryData에 인코딩됨). 읽기 전용 String.
