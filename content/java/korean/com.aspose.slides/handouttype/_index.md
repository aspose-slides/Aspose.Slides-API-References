---
title: HandoutType
second_title: Aspose.Slides for Java API 참조
description: 페이지에 배치될 슬라이드 수와 순서를 지정합니다.
type: docs
url: /ko/com.aspose.slides/handouttype/
---
**Inheritance:**
java.lang.Object, com.aspose.ms.System.ValueType, com.aspose.ms.System.Enum
```
public final class HandoutType extends System.Enum
```

페이지에 배치될 슬라이드 수와 순서를 지정합니다.

## 필드

| 필드 | 설명 |
| --- | --- |
| [Handouts1](#Handouts1) | 페이지당 슬라이드 1장. |
| [Handouts2](#Handouts2) | 페이지당 슬라이드 2장. |
| [Handouts3](#Handouts3) | 페이지당 슬라이드 3장. |
| [Handouts4Horizontal](#Handouts4Horizontal) | 수평 순서로 페이지당 슬라이드 4장. |
| [Handouts4Vertical](#Handouts4Vertical) | 수직 순서로 페이지당 슬라이드 4장. |
| [Handouts6Horizontal](#Handouts6Horizontal) | 수평 순서로 페이지당 슬라이드 6장. |
| [Handouts6Vertical](#Handouts6Vertical) | 수직 순서로 페이지당 슬라이드 6장. |
| [Handouts9Horizontal](#Handouts9Horizontal) | 수평 순서로 페이지당 슬라이드 9장. |
| [Handouts9Vertical](#Handouts9Vertical) | 수직 순서로 페이지당 슬라이드 9장. |

### Handouts1 {#Handouts1}
```
public static final int Handouts1
```

페이지당 슬라이드 1장.

### Handouts2 {#Handouts2}
```
public static final int Handouts2
```

페이지당 슬라이드 2장.

### Handouts3 {#Handouts3}
```
public static final int Handouts3
```

페이지당 슬라이드 3장.

### Handouts4Horizontal {#Handouts4Horizontal}
```
public static final int Handouts4Horizontal
```

수평 순서로 페이지당 슬라이드 4장.

### Handouts4Vertical {#Handouts4Vertical}
```
public static final int Handouts4Vertical
```

수직 순서로 페이지당 슬라이드 4장.

### Handouts6Horizontal {#Handouts6Horizontal}
```
public static final int Handouts6Horizontal
```

수평 순서로 페이지당 슬라이드 6장.

### Handouts6Vertical {#Handouts6Vertical}
```
public static final int Handouts6Vertical
```

수직 순서로 페이지당 슬라이드 6장.

### Handouts9Horizontal {#Handouts9Horizontal}
```
public static final int Handouts9Horizontal
```

수평 순서로 페이지당 슬라이드 9장.

### Handouts9Vertical {#Handouts9Vertical}
```
public static final int Handouts9Vertical
```

수직 순서로 페이지당 슬라이드 9장.