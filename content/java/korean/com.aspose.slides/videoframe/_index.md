...
| 메서드 | 설명 |
| --- | --- |
| [getRewindVideo()](#getRewindVideo--) | 동영상이 재생이 끝난 즉시 자동으로 시작 위치로 되감아지는지 여부를 결정합니다. |
| [setRewindVideo(boolean value)](#setRewindVideo-boolean-) | 동영상이 재생이 끝난 즉시 자동으로 시작 위치로 되감아지는지 여부를 결정합니다. |
| [getPlayLoopMode()](#getPlayLoopMode--) | 비디오가 루프되는지 여부를 결정합니다. |
| [setPlayLoopMode(boolean value)](#setPlayLoopMode-boolean-) | 비디오가 루프되는지 여부를 결정합니다. |
...
| [getFullScreenMode()](#getFullScreenMode--) | 비디오가 전체 화면 모드로 표시되는지 여부를 결정합니다. |
| [setFullScreenMode(boolean value)](#setFullScreenMode-boolean-) | 비디오가 전체 화면 모드로 표시되는지 여부를 결정합니다. |
...
동영상이 재생이 끝난 즉시 자동으로 시작 위치로 되감아지는지 여부를 결정합니다. 읽기/쓰기 boolean.
...
비디오가 루프되는지 여부를 결정합니다. 읽기/쓰기 boolean.
...
비디오 프레임과 연관된 닫힌 캡션 컬렉션을 가져옵니다. 이 속성은 읽기 전용이며 모든 캡션 트랙을 포함하는 [ICaptionsCollection](../../com.aspose.slides/icaptionscollection)을 반환합니다.
