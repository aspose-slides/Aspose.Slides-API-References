...
| [getBackward()](#getBackward--) | 추세선이 시리즈 데이터 이전에 확장되는 카테고리(또는 산점도 차트의 단위) 수를 지정합니다. |
| [setBackward(double value)](#setBackward-double-) | 추세선이 시리즈 데이터 이전에 확장되는 카테고리(또는 산점도 차트의 단위) 수를 지정합니다. |
| [getForward()](#getForward--) | 추세선이 시리즈 데이터 이후에 확장되는 카테고리(또는 산점도 차트의 단위) 수를 지정합니다. |
| [setForward(double value)](#setForward-double-) | 추세선이 시리즈 데이터 이후에 확장되는 카테고리(또는 산점도 차트의 단위) 수를 지정합니다. |
| [getIntercept()](#getIntercept--) | 추세선이 y축과 교차하는 값을 지정합니다. |
| [setIntercept(double value)](#setIntercept-double-) | 추세선이 y축과 교차하는 값을 지정합니다. |
| [getDisplayEquation()](#getDisplayEquation--) | 추세선 방정식이 차트에 표시되도록 지정합니다(R제곱값과 동일한 레이블에). |
| [setDisplayEquation(boolean value)](#setDisplayEquation-boolean-) | 추세선 방정식이 차트에 표시되도록 지정합니다(R제곱값과 동일한 레이블에). |
...
| [getDisplayRSquaredValue()](#getDisplayRSquaredValue--) | 추세선의 R제곱값이 차트에 표시되도록 지정합니다(방정식과 동일한 레이블에). |
| [setDisplayRSquaredValue(boolean value)](#setDisplayRSquaredValue-boolean-) | 추세선의 R제곱값이 차트에 표시되도록 지정합니다(방정식과 동일한 레이블에). |
...
추세선의 유형을 가져오거나 설정합니다. 읽기/쓰기 [TrendlineType](../../com.aspose.slides/trendlinetype)(\#getTrendlineType.getTrendlineType/\#setTrendlineType(int).setTrendlineType(int)).
...
추세선이 시리즈 데이터 이전에 확장되는 카테고리(또는 산점도 차트의 단위) 수를 지정합니다. On scatter and non-scatter charts, the value shall be any nonnegative value. 읽기/쓰기 double.
...
추세선이 시리즈 데이터 이후에 확장되는 카테고리(또는 산점도 차트의 단위) 수를 지정합니다. On scatter and non-scatter charts, the value shall be any non-negative value. 읽기/쓰기 double.
...
추세선이 y축과 교차하는 값을 지정합니다. This property shall be supported only when the trendline type is exp, linear, or poly. 읽기/쓰기 double.
...
추세선 방정식이 차트에 표시되도록 지정합니다(R제곱값과 동일한 레이블에). 읽기/쓰기 boolean.
...
다항식 추세선의 차수를 지정합니다. It is ignored for other trend line types. Value must be between 2 and 6. 읽기/쓰기 byte.
...
이동 평균 추세선의 기간을 지정합니다. It is ignored for other trend line variants. Value must be between 2 and 255. 읽기/쓰기 byte.
...
추세선의 R제곱값이 차트에 표시되도록 지정합니다(방정식과 동일한 레이블에). 읽기/쓰기 boolean.
