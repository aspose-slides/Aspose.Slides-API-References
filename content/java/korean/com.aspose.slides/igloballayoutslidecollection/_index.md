...
description: 프레젠테이션의 모든 레이아웃 슬라이드를 포함하는 컬렉션을 나타냅니다.
type: docs
...
프레젠테이션의 모든 레이아웃 슬라이드를 포함하는 컬렉션을 나타냅니다. 마스터 레이아웃 슬라이드의 개별 컬렉션을 결합하는 컨텍스트에서 레이아웃 슬라이드를 추가/복제하는 메서드를 제공하는 ILayoutSlideCollection 인터페이스를 확장합니다.
## 메서드
...
다른 프레젠테이션 간에 레이아웃을 복제할 때 레이아웃의 마스터도 복제되어 원본 서식을 유지할 수 있습니다. 내부 레지스트리를 사용하여 자동으로 복제된 마스터를 추적하고 동일한 마스터 슬라이드의 복제본이 여러 개 생성되는 것을 방지합니다. 마스터 슬라이드의 수동 복제는 방지되거나 등록되지 않습니다.
...
| --- | --- | --- |
| sourceLayout | [ILayoutSlide](../../com.aspose.slides/ilayoutslide) | 복제할 슬라이드. |
| destMaster | [IMasterSlide](../../com.aspose.slides/imasterslide) | 새 레이아웃용 마스터 슬라이드.
...
| 매개변수 | 유형 | 설명 |
| --- | --- | --- |
| master | [IMasterSlide](../../com.aspose.slides/imasterslide) | 새 레이아웃용 마스터 슬라이드. |
...
| layoutName | java.lang.String | 새 레이아웃의 이름입니다. 전달된 이름이 이미 사용 중인 경우 ArgumentException이 발생합니다. null 매개변수가 전달되면 전달된 레이아웃 유형에 따라 이름이 자동으로 생성됩니다(예: "Title Slide" 또는 "1_Title Slide", "2_.." 등). |

--------------------

1) layoutType이 SlideLayoutType.Custom 값을 갖는 경우, 추가된 레이아웃에는 자리 표시자와 도형이 없습니다. 2) 이 메서드와 유사한 메서드는 [IMasterLayoutSlideCollection.add(byte,String)](../../com.aspose.slides/imasterlayoutslidecollection\#add-byte-String-)이며 ([IMasterSlide.getLayoutSlides](../../com.aspose.slides/imasterslide\#getLayoutSlides)) 속성을 통해 액세스합니다.
