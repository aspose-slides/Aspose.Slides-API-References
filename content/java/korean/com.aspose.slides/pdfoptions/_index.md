...
| [getEmbedTrueTypeFontsForASCII()](#getEmbedTrueTypeFontsForASCII--) | Aspose.Slides가 ASCII(33..127 코드 범위) 텍스트에 일반 글꼴을 포함할지 여부를 결정합니다. |
| [setEmbedTrueTypeFontsForASCII(boolean value)](#setEmbedTrueTypeFontsForASCII-boolean-) | Aspose.Slides가 ASCII(33..127 코드 범위) 텍스트에 일반 글꼴을 포함할지 여부를 결정합니다. |
| [getAdditionalCommonFontFamilies()](#getAdditionalCommonFontFamilies--) | Aspose.Slides가 일반 글꼴로 간주해야 하는 사용자 정의 글꼴 패밀리 이름 배열을 반환하거나 설정합니다. |
| [setAdditionalCommonFontFamilies(String[] value)](#setAdditionalCommonFontFamilies-java.lang.String---) | Aspose.Slides가 일반 글꼴로 간주해야 하는 사용자 정의 글꼴 패밀리 이름 배열을 반환하거나 설정합니다. |
| [getEmbedFullFonts()](#getEmbedFullFonts--) | 글꼴의 모든 문자를 포함할지 또는 사용된 부분 집합만 포함할지 여부를 결정합니다. |
| [setEmbedFullFonts(boolean value)](#setEmbedFullFonts-boolean-) | 글꼴의 모든 문자를 포함할지 또는 사용된 부분 집합만 포함할지 여부를 결정합니다. |
| [getRasterizeUnsupportedFontStyles()](#getRasterizeUnsupportedFontStyles--) | 글꼴이 굵게 스타일을 지원하지 않을 때 텍스트를 비트맵으로 래스터화하여 PDF에 저장해야 하는지 여부를 나타냅니다. |
| [setRasterizeUnsupportedFontStyles(boolean value)](#setRasterizeUnsupportedFontStyles-boolean-) | 글꼴이 굵게 스타일을 지원하지 않을 때 텍스트를 비트맵으로 래스터화하여 PDF에 저장해야 하는지 여부를 나타냅니다. |
...
| [getPassword()](#getPassword--) | PDF 문서를 보호하기 위한 사용자 비밀번호를 설정합니다. |
| [setPassword(String value)](#setPassword-java.lang.String-) | PDF 문서를 보호하기 위한 사용자 비밀번호를 설정합니다. |
...
| [getDrawSlidesFrame()](#getDrawSlidesFrame--) | 각 슬라이드 주위에 검은색 테두리를 그리려면 True를 설정합니다. |
| [setDrawSlidesFrame(boolean value)](#setDrawSlidesFrame-boolean-) | 각 슬라이드 주위에 검은색 테두리를 그리려면 True를 설정합니다. |
| [getImageTransparentColor()](#getImageTransparentColor--) | 이미지 투명 색상을 가져오거나 설정합니다. |
| [setImageTransparentColor(Color value)](#setImageTransparentColor-java.awt.Color-) | 이미지 투명 색상을 가져오거나 설정합니다. |
...
| [getIncludeOleData()](#getIncludeOleData--) | 프레젠테이션의 모든 OLE 데이터를 결과 PDF에 포함된 파일로 변환하려면 True를 설정합니다. |
| [setIncludeOleData(boolean value)](#setIncludeOleData-boolean-) | 프레젠테이션의 모든 OLE 데이터를 결과 PDF에 포함된 파일로 변환하려면 True를 설정합니다. |
...
각 이미지에 대해 가장 효과적인 압축(기본 압축 대신)을 자동으로 선택해야 하는지 여부를 나타냅니다. true로 설정하면 프레젠테이션의 모든 이미지에 대해 가장 적절한 압축 알고리즘이 선택되어 결과 PDF 문서 크기가 감소합니다.

--------------------

최고 압축 비율 선택은 계산 비용이 많이 들며 추가 RAM을 사용하고, 기본값은 false입니다.
...
Aspose.Slides가 ASCII(33..127 코드 범위) 텍스트에 일반 글꼴을 포함할지 여부를 결정합니다. 127보다 큰 문자 코드는 항상 포함됩니다. 일반 글꼴 목록에는 PDF의 기본 14글꼴과 추가 사용자 지정 글꼴이 포함됩니다. 읽기/쓰기 boolean.
...
Aspose.Slides가 일반 글꼴로 간주해야 하는 사용자 정의 글꼴 패밀리 이름 배열을 반환하거나 설정합니다. 읽기/쓰기 String[].
...
글꼴의 모든 문자를 포함할지 또는 사용된 부분 집합만 포함할지 여부를 결정합니다. 읽기/쓰기 boolean.
...
글꼴이 굵게 스타일을 지원하지 않을 때 텍스트를 비트맵으로 래스터화하여 PDF에 저장해야 하는지 여부를 나타냅니다. 이 접근 방식은 특정 글꼴에 대해 PDF 텍스트 품질을 향상시킬 수 있습니다. 읽기/쓰기 boolean.
...
PDF 형식으로 저장할 때 이미지 품질을 가져오거나 설정하려면 이 속성을 사용하십시오. 값은 0에서 100 사이이며 0은 최악의 품질이지만 최대 압축, 100은 최고의 품질이지만 최소 압축을 의미합니다.
...
PDF 문서를 보호하기 위한 사용자 비밀번호를 설정합니다. 읽기/쓰기 String.
...
전체 메타파일을 PNG 이미지로 변환하려면 True를 설정합니다. 읽기/쓰기 boolean.

--------------------

기본값은 **true**. PDF 문서는 벡터 그래픽과 래스터 이미지를 모두 포함할 수 있습니다. SaveMetafilesAsPng을 true로 설정하면 소스 메타파일 이미지가 PNG 형식으로 변환되어 래스터 이미지로 PDF에 저장됩니다. false로 설정하면 소스 메타파일이 PDF 벡터 그래픽으로 변환됩니다. 각 접근 방식에는 장단점이 있습니다. 예를 들어 메타파일을 PNG로 변환하면 결과 문서 스케일링 시 품질 손실이 발생할 수 있습니다. PDF 뷰어 도구에서 성능 문제가 발생할 수도 있습니다.
...
값: 이 매개변수의 효과는 여러 요인에 따라 달라집니다. 알고리즘은 속성 값, 원본 이미지 크기 및 이미지 프레임 크기를 기준으로 최적 출력 이미지 크기를 얻으려고 시도합니다. 유사한 속성 값을 사용하면 동일한 결과가 나올 수 있습니다. 눈에 띄는 효과를 얻으려면 16 또는 32 단계 사용을 권장합니다.
...
각 슬라이드 주위에 검은색 테두리를 그리려면 True를 설정합니다. 읽기/쓰기 boolean.
...
프레젠테이션의 모든 OLE 데이터를 결과 PDF에 포함된 파일로 변환하려면 True를 설정합니다. 읽기/쓰기 boolean.
