...
| [getPreserve()](#getPreserve--) | 해당 마스터 뒤의 모든 슬라이드가 삭제될 때 해당 마스터가 삭제되는지 여부를 결정합니다. |
| [setPreserve(boolean value)](#setPreserve-boolean-) | 해당 마스터 뒤의 모든 슬라이드가 삭제될 때 해당 마스터가 삭제되는지 여부를 결정합니다. |
| [getDependingSlides()](#getDependingSlides--) | 이 마스터 슬라이드에 종속되는 모든 슬라이드의 배열을 반환합니다. |
| [hasDependingSlides()](#hasDependingSlides--) | 이 마스터 슬라이드에 종속되는 슬라이드가 하나라도 존재하면 true를 반환합니다. |
...
| [getDrawingGuides()](#getDrawingGuides--) | 마스터 슬라이드에 대한 드로잉 가이드 컬렉션을 반환합니다. |
...
해당 마스터 뒤의 모든 슬라이드가 삭제될 때 해당 마스터가 삭제되는지 여부를 결정합니다. 참고: Aspose.Slides는 사용되지 않는 마스터를 자체적으로 제거하지 않으며, 실제로 사용되지 않는 마스터를 제거하려면 [MasterSlideCollection.removeUnused(boolean)](../../com.aspose.slides/masterslidecollection\#removeUnused-boolean-)를 호출하십시오. 읽기/쓰기 boolean .
...
이 마스터 슬라이드에 종속되는 모든 슬라이드의 배열을 반환합니다.

**반환:**
com.aspose.slides.ISlide[] - [ISlide](../../com.aspose.slides/islide)의 배열
...
마스터 슬라이드에 대한 드로잉 가이드 컬렉션을 반환합니다. 읽기 전용 [IDrawingGuidesCollection](../../com.aspose.slides/idrawingguidescollection)
