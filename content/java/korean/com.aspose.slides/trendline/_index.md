...
| [getBackward()](#getBackward--) | 추세선이 시리즈 데이터 앞에서 확장되는 범주(또는 산점도 차트의 단위) 수를 지정합니다. |
| [setBackward(double value)](#setBackward-double-) | 추세선이 시리즈 데이터 앞에서 확장되는 범주(또는 산점도 차트의 단위) 수를 지정합니다. |
| [getForward()](#getForward--) | 추세선이 시리즈 데이터 뒤에서 확장되는 범주(또는 산점도 차트의 단위) 수를 지정합니다. |
| [setForward(double value)](#setForward-double-) | 추세선이 시리즈 데이터 뒤에서 확장되는 범주(또는 산점도 차트의 단위) 수를 지정합니다. |
...
추세선이 시리즈 데이터 앞에서 확장되는 범주(또는 산점도 차트의 단위) 수를 지정합니다. 산점도 및 비산점도 차트에서 값은 음수가 아닌 임의의 값이어야 합니다. 읽기/쓰기 double.
...
추세선이 시리즈 데이터 뒤에서 확장되는 범주(또는 산점도 차트의 단위) 수를 지정합니다. 산점도 및 비산점도 차트에서 값은 음수가 아닌 임의의 값이어야 합니다. 읽기/쓰기 double.
...
다항식 추세선의 차수를 지정합니다. 다른 추세선 유형에 대해서는 무시됩니다. 값은 2에서 6 사이여야 합니다. 읽기/쓰기 byte.
...
이동 평균 추세선의 기간을 지정합니다. 다른 추세선 변형에 대해서는 무시됩니다. 값은 2에서 255 사이여야 합니다. 읽기/쓰기 byte.
...
텍스트 매개변수 "text"로 TextFrameForOverriding을 초기화합니다. TextFrameForOverriding이 이미 초기화된 경우 텍스트를 단순히 변경합니다.
...
리치 형식 텍스트를 포함할 수 있습니다. 이 속성이 null이 아니면 이 형식 텍스트 값이 데이터 레이블의 자동 생성 텍스트를 대체합니다. 자동 생성 텍스트는 ShowSeriesName, ShowValue 등 속성에 의해 관리되며 TextFormatManager.TextFormat 속성으로 형식이 지정된 텍스트를 의미합니다. 읽기 전용 [ITextFrame](../../com.aspose.slides/itextframe).
