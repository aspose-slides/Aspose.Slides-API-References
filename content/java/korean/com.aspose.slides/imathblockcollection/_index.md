...
description: 수학 블록 IMathBlock 컬렉션
type: docs
...
| [insert(int index, IMathBlock item)](#insert-int-com.aspose.slides.IMathBlock-) | 지정된 인덱스에 IMathBlock을 컬렉션에 삽입합니다. |
...
| [indexOf(IMathBlock item)](#indexOf-com.aspose.slides.IMathBlock-) | 컬렉션에서 특정 IMathBlock의 인덱스를 확인합니다. |
...
지정된 인덱스에 IMathBlock을 컬렉션에 삽입합니다.
...
**반환값:**
boolean - 항목이 컬렉션에서 성공적으로 제거되면 true; 그렇지 않으면 false. 항목이 원래 컬렉션에 없을 경우에도 false를 반환합니다.
...
컬렉션에서 특정 IMathBlock의 인덱스를 확인합니다.
...
**반환값:**
int - 컬렉션에서 항목을 찾으면 그 인덱스; 찾지 못하면 -1.
### getCount() {#getCount--}
```
