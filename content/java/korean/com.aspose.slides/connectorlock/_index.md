...
description: 부모 커넥터에서 사용이 금지된 작업을 결정합니다.
type: docs
url: /ko/com.aspose.slides/connectorlock/
---
**상속:**
...
부모 커넥터에서 사용이 금지된 작업을 결정합니다.
## 메서드

| 메서드 | 설명 |
| --- | --- |
| [getGroupingLocked()](#getGroupingLocked--) | 해당 모양을 그룹에 추가하는 것이 금지되는지 여부를 결정합니다. |
| [setGroupingLocked(boolean value)](#setGroupingLocked-boolean-) | 해당 모양을 그룹에 추가하는 것이 금지되는지 여부를 결정합니다. |
| [getSelectLocked()](#getSelectLocked--) | 해당 모양을 선택하는 것이 금지되는지 여부를 결정합니다. |
| [setSelectLocked(boolean value)](#setSelectLocked-boolean-) | 해당 모양을 선택하는 것이 금지되는지 여부를 결정합니다. |
| [getRotateLocked()](#getRotateLocked--) | 해당 모양의 회전 각도를 변경하는 것이 금지되는지 여부를 결정합니다. |
| [setRotateLocked(boolean value)](#setRotateLocked-boolean-) | 해당 모양의 회전 각도를 변경하는 것이 금지되는지 여부를 결정합니다. |
| [getAspectRatioLocked()](#getAspectRatioLocked--) | 크기 조절 시 모양이 종횡비를 유지해야 하는지 여부를 결정합니다. |
| [setAspectRatioLocked(boolean value)](#setAspectRatioLocked-boolean-) | 크기 조절 시 모양이 종횡비를 유지해야 하는지 여부를 결정합니다. |
| [getPositionMove()](#getPositionMove--) | 해당 모양을 이동하는 것이 금지되는지 여부를 결정합니다. |
| [setPositionMove(boolean value)](#setPositionMove-boolean-) | 해당 모양을 이동하는 것이 금지되는지 여부를 결정합니다. |
| [getSizeLocked()](#getSizeLocked--) | 해당 모양의 크기를 조절하는 것이 금지되는지 여부를 결정합니다. |
| [setSizeLocked(boolean value)](#setSizeLocked-boolean-) | 해당 모양의 크기를 조절하는 것이 금지되는지 여부를 결정합니다. |
| [getEditPointsLocked()](#getEditPointsLocked--) | 해당 모양의 윤곽을 직접 변경하는 것이 금지되는지 여부를 결정합니다. |
| [setEditPointsLocked(boolean value)](#setEditPointsLocked-boolean-) | 해당 모양의 윤곽을 직접 변경하는 것이 금지되는지 여부를 결정합니다. |
...
| [getArrowheadsLocked()](#getArrowheadsLocked--) | 화살촉을 변경하는 것이 금지되는지 여부를 결정합니다. |
| [setArrowheadsLocked(boolean value)](#setArrowheadsLocked-boolean-) | 화살촉을 변경하는 것이 금지되는지 여부를 결정합니다. |
...
해당 모양을 그룹에 추가하는 것이 금지되는지 여부를 결정합니다. 읽기/쓰기 boolean.
...
해당 모양을 선택하는 것이 금지되는지 여부를 결정합니다. 읽기/쓰기 boolean.
...
해당 모양의 회전 각도를 변경하는 것이 금지되는지 여부를 결정합니다. 읽기/쓰기 boolean.
...
크기 조절 시 모양이 종횡비를 유지해야 하는지 여부를 결정합니다. 읽기/쓰기 boolean.
...
해당 모양을 이동하는 것이 금지되는지 여부를 결정합니다. 읽기/쓰기 boolean.
...
해당 모양의 크기를 조절하는 것이 금지되는지 여부를 결정합니다. 읽기/쓰기 boolean.
...
해당 모양의 윤곽을 직접 변경하는 것이 금지되는지 여부를 결정합니다. 읽기/쓰기 boolean.
...
화살촉을 변경하는 것이 금지되는지 여부를 결정합니다. 읽기/쓰기 boolean.
