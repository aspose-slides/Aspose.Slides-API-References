---
title: FlyThroughTransition
second_title: Aspose.Slides Java API 레퍼런스
description: 플라이 스루 슬라이드 전환 효과.
type: docs
url: /ko/com.aspose.slides/flythroughtransition/
---
**상속:**
java.lang.Object, [com.aspose.slides.TransitionValueBase](../../com.aspose.slides/transitionvaluebase)

**구현된 모든 인터페이스:**
[com.aspose.slides.IFlyThroughTransition](../../com.aspose.slides/iflythroughtransition)
```
public class FlyThroughTransition extends TransitionValueBase implements IFlyThroughTransition
```

플라이 스루 슬라이드 전환 효과.
## 메서드

| 메서드 | 설명 |
| --- | --- |
| [getDirection()](#getDirection--) | 전환 방향. |
| [setDirection(int value)](#setDirection-int-) | 전환 방향. |
| [hasBounce()](#hasBounce--) | 전환 중 프레젠테이션 슬라이드의 움직임에 바운스가 포함됨을 지정합니다. |
| [setBounce(boolean value)](#setBounce-boolean-) | 전환 중 프레젠테이션 슬라이드의 움직임에 바운스가 포함됨을 지정합니다. |
### getDirection() {#getDirection--}
```
public final int getDirection()
```

전환 방향. 읽기/쓰기 [TransitionInOutDirectionType](../../com.aspose.slides/transitioninoutdirectiontype).

**반환값:**
int
### setDirection(int value) {#setDirection-int-}
```
public final void setDirection(int value)
```

전환 방향. 읽기/쓰기 [TransitionInOutDirectionType](../../com.aspose.slides/transitioninoutdirectiontype).

**매개변수:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| value | int |  |
### hasBounce() {#hasBounce--}
```
public final boolean hasBounce()
```

전환 중 프레젠테이션 슬라이드의 움직임에 바운스가 포함됨을 지정합니다. 읽기/쓰기 boolean.

**반환값:**
boolean
### setBounce(boolean value) {#setBounce-boolean-}
```
public final void setBounce(boolean value)
```

전환 중 프레젠테이션 슬라이드의 움직임에 바운스가 포함됨을 지정합니다. 읽기/쓰기 boolean.

**매개변수:**
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| value | boolean |  |