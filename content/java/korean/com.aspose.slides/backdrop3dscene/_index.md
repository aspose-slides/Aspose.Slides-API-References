...
description: 광선 및 그림자와 같은 효과가 적용되는 형태와 관련된 평면을 정의합니다.
type: docs
...
형태에 적용되는 광채와 그림자와 같은 효과가 적용되는 평면을 정의합니다.
...
정규 벡터를 반환하거나 설정합니다. 보다 정확히 말하면, 이 속성은 배경면의 면에 수직인 벡터를 정의합니다. X, Y, Z 좌표를 정의하는 3개의 float 값 배열로 표현되는 벡터입니다. 읽기/쓰기 float[].
...
3D 공간의 점을 반환하거나 설정합니다. 이 점은 배경면을 고정하는 공간상의 점입니다. X, Y, Z 좌표를 정의하는 3개의 float 값 배열로 표현되는 3D 점입니다. 읽기/쓰기 float[].
...
위쪽을 나타내는 벡터를 반환하거나 설정합니다. 보다 정확히 말하면, 이 속성은 배경면의 면에 대해 위쪽을 나타내는 벡터를 정의합니다. X, Y, Z 좌표를 정의하는 3개의 float 값 배열로 표현되는 벡터입니다. 읽기/쓰기 float[].
