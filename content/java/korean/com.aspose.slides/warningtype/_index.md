...
| Field | Description |
| --- | --- |
| [SourceFileCorruption](#SourceFileCorruption) | 소스 문서에서 문제가 감지되어 원본 형식으로 저장하면 문서를 열 수 없게 될 가능성이 매우 높습니다. |
| [DataLoss](#DataLoss) | 텍스트/차트/이미지 또는 기타 데이터가 로드 후 문서 트리에서, 혹은 저장 후 생성된 문서에서 완전히 누락됩니다. |
| [MajorFormattingLoss](#MajorFormattingLoss) | 중대한 서식 손실. |
| [MinorFormattingLoss](#MinorFormattingLoss) | 경미한 서식 손실. |
| [CompatibilityIssue](#CompatibilityIssue) | 이는 알려진 문제로, 특정 사용자 에이전트 또는 이전 버전의 사용자 에이전트에서 문서를 열지 못하게 합니다. |
...
소스 문서에서 문제가 감지되어 원본 형식으로 저장하면 문서를 열 수 없게 될 가능성이 매우 높습니다.
...
텍스트/차트/이미지 또는 기타 데이터가 로드 후 문서 트리에서, 혹은 저장 후 생성된 문서에서 완전히 누락됩니다.
...
이는 알려진 문제로, 특정 사용자 에이전트 또는 이전 버전의 사용자 에이전트에서 문서를 열지 못하게 합니다.
...
소스 문서의 일부 콘텐츠를 인식할 수 없습니다 (예: 지원되지 않음), 이는 문제를 일으키거나 데이터/서식 손실을 초래할 수도 있고 아닐 수도 있습니다.