...
| 메서드 | 설명 |
| --- | --- |
| [getOffsetX()](#getOffsetX--) | 테이블의 왼쪽 면에서 셀의 왼쪽 면까지의 거리를 반환합니다. |
| [getOffsetY()](#getOffsetY--) | 테이블의 위쪽 면에서 셀의 위쪽 면까지의 거리를 반환합니다. |
...
| [getAnchorCenter()](#getAnchorCenter--) | 텍스트 상자가 셀 내부에 중앙에 배치되는지 여부를 결정합니다. |
| [setAnchorCenter(boolean value)](#setAnchorCenter-boolean-) | 텍스트 상자가 셀 내부에 중앙에 배치되는지 여부를 결정합니다. |
| [getFirstColumn()](#getFirstColumn--) | 셀의 첫 번째 열을 가져옵니다. |
| [getFirstRow()](#getFirstRow--) | 셀의 첫 번째 행을 가져옵니다. |
| [getColSpan()](#getColSpan--) | 현재 셀이 차지할 상위 테이블의 테이블 그리드에서 격자 열 수를 반환합니다. |
| [getRowSpan()](#getRowSpan--) | 병합된 셀이 차지하는 행 수를 반환합니다. |
| [getTextFrame()](#getTextFrame--) | 셀의 텍스트 프레임을 반환합니다. |
| [getTable()](#getTable--) | 셀에 대한 상위 Table 객체를 반환합니다. |
...
테이블의 왼쪽 면에서 셀의 왼쪽 면까지의 거리를 반환합니다. 읽기 전용 double.
...
테이블의 위쪽 면에서 셀의 위쪽 면까지의 거리를 반환합니다. 읽기 전용 double.
...
텍스트 상자가 셀 내부에 중앙에 배치되는지 여부를 결정합니다. 읽기/쓰기 boolean.
...
현재 셀이 차지할 상위 테이블의 테이블 그리드에서 격자 열 수를 반환합니다. 이 속성은 셀이 다른 셀의 수직 경계를 가로질러 병합된 것처럼 보이게 합니다. 읽기 전용 int.
...
병합된 셀이 차지하는 행 수를 반환합니다. 이는 다른 셀의 vMerge 속성과 결합되어 수평 병합의 시작 셀을 지정하는 데 사용됩니다. 읽기 전용 int.
...
셀에 대한 상위 Table 객체를 반환합니다. 읽기 전용 [ITable](../../com.aspose.slides/itable).
