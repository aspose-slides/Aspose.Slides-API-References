...
| [getAdvanceOnClick()](#getAdvanceOnClick--) | 마우스 클릭으로 슬라이드가 진행될지 여부를 지정합니다. |
| [setAdvanceOnClick(boolean value)](#setAdvanceOnClick-boolean-) | 마우스 클릭으로 슬라이드가 진행될지 여부를 지정합니다. |
| [getAdvanceAfter()](#getAdvanceAfter--) | 이 속성은 슬라이드 쇼가 특정 시간 후에 다음 슬라이드로 이동할지 여부를 지정합니다. |
| [setAdvanceAfter(boolean value)](#setAdvanceAfter-boolean-) | 이 속성은 슬라이드 쇼가 특정 시간 후에 다음 슬라이드로 이동할지 여부를 지정합니다. |
...
| [getSoundName()](#getSoundName--) | 전환 사운드의 사람이 읽을 수 있는 이름을 지정합니다. |
| [setSoundName(String value)](#setSoundName-java.lang.String-) | 전환 사운드의 사람이 읽을 수 있는 이름을 지정합니다. |
...
마우스 클릭으로 슬라이드가 진행될지 여부를 지정합니다. 이 속성이 지정되지 않은 경우 true 값이 기본값으로 가정됩니다. 읽기-쓰기 boolean.
...
이 속성은 슬라이드 쇼가 특정 시간 후에 다음 슬라이드로 이동할지 여부를 지정합니다. 읽기/쓰기  boolean .
...
전환이 시작될 시간을 밀리초 단위로 지정합니다. 이 설정은 advClick 속성과 함께 사용할 수 있습니다. 이 속성이 지정되지 않으면 자동 진행이 발생하지 않는 것으로 간주됩니다. 읽기-쓰기 long.
...
이 사운드가 내장 사운드인지 여부를 지정합니다. 이 속성이 true 로 설정된 경우, 생성 애플리케이션은 이 사운드에 대해 지정된 name 속성을 내장 사운드 목록에서 확인하도록 알리며, 필요한 경우 사용자 지정 이름이나 UI를 표시할 수 있습니다. 읽기-쓰기 boolean.
...
전환 사운드의 사람이 읽을 수 있는 이름을 지정합니다. (\#getSound.getSound/\#setSound(IAudio).setSound(IAudio)) 속성을 할당하여 사운드 이름을 가져오거나 설정해야 합니다. 읽기-쓰기 String.
...
PresentationML 스키마의 p:transition 요소에서 p14:dur 속성에 해당합니다. 설정되지 않은 경우, 지속 시간은 \#getSpeed.getSpeed/\#setSpeed(int).setSpeed(int) 속성과 전환 유형을 기반으로 자동으로 결정됩니다.
