...
정의된 마스터 슬라이드의 모든 레이아웃 슬라이드 컬렉션을 나타냅니다. ILayoutSlideCollection 인터페이스를 확장하여 마스터 슬라이드의 개별 레이아웃 슬라이드 컬렉션에서 레이아웃 슬라이드를 추가/삽입/제거/복제하는 메서드를 제공합니다.
## 메서드
...
| [add(byte layoutType, String layoutName)](#add-byte-java.lang.String-) | 새 레이아웃 슬라이드를 컬렉션 끝에 추가합니다. |
...
새로운 레이아웃은 이 레이아웃 슬라이드 컬렉션의 상위 마스터 슬라이드와 연결됩니다. 따라서 PowerPoint의 "Use Destination Theme" 옵션을 사용한 복사/붙여넣기와 동일합니다. 
...
layoutType이 SlideLayoutType.Custom인 레이아웃을 삽입하면 자리표시자와 모양이 없습니다.
...
1) PptxEditException이 발생하는 것을 방지하려면 먼저 레이아웃의 HasDependingSlides 속성을 확인하십시오. 2) 코드를 단순화하려면 [ILayoutSlide.remove](../../com.aspose.slides/ilayoutslide\#remove) 메서드를 사용할 수도 있습니다.
