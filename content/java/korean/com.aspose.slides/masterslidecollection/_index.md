...
| 메서드 | 설명 |
| --- | --- |
| [size()](#size--) | 컬렉션에 실제 포함된 요소의 수를 가져옵니다. |
...
컬렉션에 실제 포함된 요소의 수를 가져옵니다. 읽기 전용 int.
...
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| ignorePreserveField | boolean | 이 메서드가 [MasterSlide.getPreserve](../../com.aspose.slides/masterslide\#getPreserve)/[MasterSlide.setPreserve(boolean)](../../com.aspose.slides/masterslide\#setPreserve-boolean-) 속성이 true로 설정되어 있더라도 사용되지 않는 마스터를 제거해야 하는지 여부를 결정합니다. |
...
>  try {
>      // 대상 프레젠테이션(슬라이드를 복제할 곳)을 위해 Presentation 클래스를 인스턴스화합니다
...
>          // 소스 프레젠테이션에서 원하는 마스터 슬라이드를 대상의 마스터 컬렉션에 복제합니다
>          // 대상 프레젠테이션
