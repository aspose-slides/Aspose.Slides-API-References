...
| [getValue()](#getValue--) | Fixed, Percentage 및 StandardDeviation 값 유형과 함께 사용되어 오류 막대 길이를 결정하는 값을 가져오거나 설정합니다. |
| [setValue(float value)](#setValue-float-) | Fixed, Percentage 및 StandardDeviation 값 유형과 함께 사용되어 오류 막대 길이를 결정하는 값을 가져오거나 설정합니다. |
...
오류 막대 길이를 결정하는 가능한 방법을 나타냅니다. 사용자 정의 값 유형의 경우 시리즈의 DataPoints 컬렉션에 있는 특정 데이터 포인트의 ([IChartDataPoint.getErrorBarsCustomValues](../../com.aspose.slides/ichartdatapoint\#getErrorBarsCustomValues)) 속성을 사용하여 값을 지정합니다. Fixed, Percentage 또는 StandardDeviation 값 유형의 경우 Value 속성을 사용하여 값을 지정합니다. 읽기/쓰기 [ErrorBarValueType](../../com.aspose.slides/errorbarvaluetype).
...
Fixed, Percentage 및 StandardDeviation 값 유형과 함께 사용되어 오류 막대 길이를 결정하는 값을 가져오거나 설정합니다. 다른 경우에는 NaN을 반환합니다. 읽기/쓰기 float.
