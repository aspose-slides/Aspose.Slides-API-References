...
| 생성자 | 설명 |
| --- | --- |
| [TextFrameFormat()](#TextFrameFormat--) | 새 [TextFrameFormat](../../com.aspose.slides/textframeformat) 클래스를 초기화합니다. |
...
| [getTextStyle()](#getTextStyle--) | 텍스트의 스타일을 반환합니다. |
...
| [getWrapText()](#getWrapText--) | 텍스트가 TextFrame의 여백에서 줄 바꿈되는 경우 true. |
| [setWrapText(byte value)](#setWrapText-byte-) | 텍스트가 TextFrame의 여백에서 줄 바꿈되는 경우 true. |
...
| [getCenterText()](#getCenterText--) | NullableBool.True이면 텍스트가 가로로 상자 중앙에 정렬되어야 합니다. |
| [setCenterText(byte value)](#setCenterText-byte-) | NullableBool.True이면 텍스트가 가로로 상자 중앙에 정렬되어야 합니다. |
...
| [getColumnSpacing()](#getColumnSpacing--) | 텍스트 영역의 열 사이 간격을 반환하거나 설정합니다(포인트). |
| [setColumnSpacing(double value)](#setColumnSpacing-double-) | 텍스트 영역의 열 사이 간격을 반환하거나 설정합니다(포인트). |
...
| [getKeepTextFlat()](#getKeepTextFlat--) | 3D 회전 효과가 적용되었더라도 텍스트를 평평하게 유지하도록 가져오거나 설정합니다. |
| [setKeepTextFlat(boolean value)](#setKeepTextFlat-boolean-) | 3D 회전 효과가 적용되었더라도 텍스트를 평평하게 유지하도록 가져오거나 설정합니다. |
...
새 [TextFrameFormat](../../com.aspose.slides/textframeformat) 클래스를 초기화합니다.
...
텍스트의 스타일을 반환합니다. 읽기 전용 [ITextStyle](../../com.aspose.slides/itextstyle).
...
TextFrame의 아래쪽 여백(포인트)을 반환하거나 설정합니다. 읽기/쓰기 double.
...
텍스트가 TextFrame의 여백에서 줄 바꿈되는 경우 true. 읽기/쓰기 [NullableBool](../../com.aspose.slides/nullablebool).
...
NullableBool.True이면 텍스트가 가로로 상자 중앙에 정렬되어야 합니다. 읽기/쓰기 [NullableBool](../../com.aspose.slides/nullablebool).
...
텍스트 방향을 결정합니다. 이 속성과 RotationAngle 속성의 사용자 지정 각도에서 요약된 시각적 텍스트 회전값이 결과값입니다. 읽기/쓰기 [TextVerticalType](../../com.aspose.slides/textverticaltype).
...
텍스트 영역의 열 사이 간격을 반환하거나 설정합니다(포인트). 열이 1개 이상일 때만 적용됩니다. 이 값은 양수여야 합니다. 그렇지 않으면 값이 0으로 설정됩니다. 읽기/쓰기 double.
...
경계 상자 내 텍스트에 적용되는 사용자 지정 회전을 지정합니다. 지정되지 않은 경우, 해당 도형의 회전이 사용됩니다. 지정된 경우, 도형과 독립적으로 적용됩니다. 즉, 도형에 회전이 적용된 상태에서 텍스트 자체에도 회전이 적용될 수 있습니다. 이 속성과 TextVerticalType 속성에 정의된 수직 유형을 종합한 시각적 텍스트 회전값이 결과값입니다. 읽기/쓰기 float.
...
3D 회전 효과가 적용되었더라도 텍스트를 평평하게 유지하도록 가져오거나 설정합니다. 읽기/쓰기 boolean.
