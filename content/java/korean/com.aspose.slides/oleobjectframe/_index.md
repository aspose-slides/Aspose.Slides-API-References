...
>  
>  // PPTX 파일을 프레젠테이션 객체에 로드합니다.
>  Presentation pres = new Presentation("AccessingOLEObjectFrame.pptx");
>  try {
>      // 첫 번째 슬라이드에 접근합니다.
>      ISlide sld = pres.getSlides().get_Item(0);
>      // 셰이프를 OleObjectFrame으로 캐스팅합니다.
...
| [getEmbeddedFileLabel()](#getEmbeddedFileLabel--) | 임베드된 OLE 개체의 파일 이름을 반환합니다. |
| [getEmbeddedFileName()](#getEmbeddedFileName--) | 임베드된 OLE 개체의 경로를 반환합니다. |
| [getEmbeddedData()](#getEmbeddedData--) | OLE 임베드 데이터에 대한 정보를 가져오거나 설정합니다. |
| [setEmbeddedData(IOleEmbeddedDataInfo embeddedData)](#setEmbeddedData-com.aspose.slides.IOleEmbeddedDataInfo-) | OLE 임베드 데이터에 대한 정보를 설정합니다. |
| [isObjectIcon()](#isObjectIcon--) | 개체가 아이콘으로 표시되는지 여부를 결정합니다. |
| [setObjectIcon(boolean value)](#setObjectIcon-boolean-) | 개체가 아이콘으로 표시되는지 여부를 결정합니다. |
| [isObjectLink()](#isObjectLink--) | 개체가 외부 파일에 연결되어 있는지 여부를 결정합니다. |
| [getUpdateAutomatic()](#getUpdateAutomatic--) | 프레젠테이션이 열리거나 인쇄될 때 연결된 임베드 개체가 자동으로 업데이트되는지 여부를 결정합니다. |
| [setUpdateAutomatic(boolean value)](#setUpdateAutomatic-boolean-) | 프레젠테이션이 열리거나 인쇄될 때 연결된 임베드 개체가 자동으로 업데이트되는지 여부를 결정합니다. |
...
OLE 임베드 데이터에 대한 정보를 가져오거나 설정합니다. 읽기/쓰기 [IOleEmbeddedDataInfo](../../com.aspose.slides/ioleembeddeddatainfo).
...
| Parameter | Type | Description |
| --- | --- | --- |
| embeddedData | [IOleEmbeddedDataInfo](../../com.aspose.slides/ioleembeddeddatainfo) | 임베드된 데이터 [IOleEmbeddedDataInfo](../../com.aspose.slides/ioleembeddeddatainfo) |

이 메서드는 개체의 속성을 새 데이터에 맞게 변경하고 IsObjectLink 플래그를 false로 설정하여 OLE 개체가 임베드됨을 나타냅니다. |
...
개체가 아이콘으로 표시되는지 여부를 결정합니다. 읽기/쓰기 boolean .
...
개체가 외부 파일에 연결되어 있는지 여부를 결정합니다. 읽기 전용 boolean .
...
프레젠테이션이 열리거나 인쇄될 때 연결된 임베드 개체가 자동으로 업데이트되는지 여부를 결정합니다. 읽기/쓰기 boolean .
