...
| [get_Item(IChartDataPoint pt)](#get-Item-com.aspose.slides.IChartDataPoint-) | 컬렉션에서 해당 데이터 포인트의 인덱스(일련 번호)를 반환합니다. |
| [getDataSourceTypeForXValues()](#getDataSourceTypeForXValues--) | 데이터 포인트 XValue 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. |
| [setDataSourceTypeForXValues(int value)](#setDataSourceTypeForXValues-int-) | 데이터 포인트 XValue 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. |
| [getDataSourceTypeForYValues()](#getDataSourceTypeForYValues--) | 데이터 포인트 YValue 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. |
| [setDataSourceTypeForYValues(int value)](#setDataSourceTypeForYValues-int-) | 데이터 포인트 YValue 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. |
| [getDataSourceTypeForBubbleSizes()](#getDataSourceTypeForBubbleSizes--) | 데이터 포인트 BubbleSize 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. |
| [setDataSourceTypeForBubbleSizes(int value)](#setDataSourceTypeForBubbleSizes-int-) | 데이터 포인트 BubbleSize 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. |
| [getDataSourceTypeForValues()](#getDataSourceTypeForValues--) | 데이터 포인트 Value 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. |
| [setDataSourceTypeForValues(int value)](#setDataSourceTypeForValues-int-) | 데이터 포인트 Value 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. |
| [getDataSourceTypeForErrorBarsCustomValues()](#getDataSourceTypeForErrorBarsCustomValues--) | ChartDataPoint.ErrorBarsCustomValues 속성 목록의 값 유형을 지정합니다. |
| [getOrCreateDataPointByIdx(long index)](#getOrCreateDataPointByIdx-long-) | 컬렉션에 이미 해당 인덱스의 데이터 포인트가 있으면 이 데이터 포인트를 반환합니다. |
...
컬렉션에서 인덱스(이 컬렉션에서의 일련 번호)로 시리즈 데이터 포인트를 반환합니다.
...
컬렉션에서 해당 데이터 포인트의 인덱스(일련 번호)를 반환합니다.
...
데이터 포인트 XValue 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. 다시 말해 ChartDataPointEx.XValue.Data 속성의 값 유형을 지정합니다. 읽기/쓰기 [DataSourceType](../../com.aspose.slides/datasourcetype).
...
데이터 포인트 YValue 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. 다시 말해 ChartDataPointEx.YValue.Data 속성의 값 유형을 지정합니다. 읽기/쓰기 [DataSourceType](../../com.aspose.slides/datasourcetype).
...
데이터 포인트 BubbleSize 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. 다시 말해 ChartDataPointEx.BubbleSize.Data 속성의 값 유형을 지정합니다. 읽기/쓰기 [DataSourceType](../../com.aspose.slides/datasourcetype).
...
데이터 포인트 Value 속성 객체에서 AsCell, AsLiteralString 또는 AsLiteralDouble 속성이 실제로 사용되는지 지정합니다. 다시 말해 ChartDataPoint.Value.Data 속성의 값 유형을 지정합니다. 읽기/쓰기 [DataSourceType](../../com.aspose.slides/datasourcetype).
...
ChartDataPoint.ErrorBarsCustomValues 속성 목록의 값 유형을 지정합니다. 읽기 전용 [IDataSourceTypeForErrorBarsCustomValues](../../com.aspose.slides/idatasourcetypeforerrorbarscustomvalues).
...
컬렉션에 이미 index 인덱스의 데이터 포인트가 있으면 해당 데이터 포인트를 반환합니다. 컬렉션에 index==N 인덱스의 데이터 포인트가 없고(N은 컬렉션 내 데이터 포인트 수가 N 이하인 경우) 부족한 데이터 포인트를 추가한 후 마지막(요청된 인덱스를 가진) 데이터 포인트를 반환합니다. 예를 들어 컬렉션 인덱스가 {0, 1, 2}이고 요청 인덱스가 5이면 메서드는 부족한 데이터 포인트를 추가하여 {0, 1, 2, 3, 4, 5}가 되고 인덱스 5의 데이터 포인트를 반환합니다.
...
새 데이터 포인트를 생성하고 컬렉션 끝에 추가합니다. 차트 유형이 Stock 하위 유형 중 하나인 시리즈에 적용됩니다(또한 ChartTypeCharacterizer.IsChartTypeStock(ChartType) 메서드 참조).
...
**반환값:** [IChartDataPoint](../../com.aspose.slides/ichartdatapoint) - 새 데이터 포인트.
...
새 데이터 포인트를 만들고 컬렉션의 끝에 추가합니다. 이 메서드는 chartType이 Scatter 하위 유형 중 하나인 시리즈에 적용됩니다 (ChartTypeCharacterizer.IsChartTypeScatter(ChartType) 메서드도 참조하십시오).
...
새 데이터 포인트를 만들고 컬렉션의 끝에 추가합니다. 이 메서드는 chartType이 Radar 하위 유형 중 하나인 시리즈에 적용됩니다 (ChartTypeCharacterizer.IsChartTypeRadar(ChartType) 메서드도 참조하십시오).
...
새 데이터 포인트를 만들고 컬렉션의 끝에 추가합니다. 이 메서드는 chartType이 Column 또는 Bar 하위 유형 중 하나인 시리즈에 적용됩니다 (ChartTypeCharacterizer.IsChartTypeColumn(ChartType) 및 ChartTypeCharacterizer.IsChartTypeBar(ChartType) 메서드도 참조하십시오).
...
새 데이터 포인트를 만들고 컬렉션의 끝에 추가합니다. 이 메서드는 chartType이 Area 하위 유형 중 하나인 시리즈에 적용됩니다 (ChartTypeCharacterizer.IsChartTypeArea(ChartType) 메서드도 참조하십시오).
...
새 데이터 포인트를 만들고 컬렉션의 끝에 추가합니다. 이 메서드는 chartType이 Pie 하위 유형 중 하나인 시리즈에 적용됩니다 (ChartTypeCharacterizer.IsChartTypePie(ChartType) 메서드도 참조하십시오).
...
새 데이터 포인트를 만들고 컬렉션의 끝에 추가합니다. 이 메서드는 chartType이 Doughnut 하위 유형 중 하나인 시리즈에 적용됩니다 (ChartTypeCharacterizer.IsChartTypeDoughnut(ChartType) 메서드도 참조하십시오).
...
새 데이터 포인트를 만들고 컬렉션의 끝에 추가합니다. 이 메서드는 chartType이 Bubble 하위 유형 중 하나인 시리즈에 적용됩니다 (ChartTypeCharacterizer.IsChartTypeBubble(ChartType) 메서드도 참조하십시오).
