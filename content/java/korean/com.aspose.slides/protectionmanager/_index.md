...
| 메서드 | 설명 |
| --- | --- |
| [getEncryptDocumentProperties()](#getEncryptDocumentProperties--) | 이 속성은 프레젠테이션이 비밀번호로 보호되는 경우에 의미가 있습니다. |
| [setEncryptDocumentProperties(boolean value)](#setEncryptDocumentProperties-boolean-) | 이 속성은 프레젠테이션이 비밀번호로 보호되는 경우에 의미가 있습니다. |
| [isEncrypted()](#isEncrypted--) | 이 인스턴스가 암호화되었는지 여부를 나타내는 값을 반환합니다. |
| [isOnlyDocumentPropertiesLoaded()](#isOnlyDocumentPropertiesLoaded--) | 프레젠테이션 파일이 비밀번호로 보호되고 해당 파일의 문서 속성이 공개된 경우에 이 속성은 의미가 있습니다. |
| [isWriteProtected()](#isWriteProtected--) | 이 프레젠테이션이 쓰기 보호되어 있는지 여부를 나타내는 값을 반환합니다. |
...
| [getEncryptionPassword()](#getEncryptionPassword--) | 프레젠테이션 암호화에 사용되는 비밀번호를 반환합니다. |
...
이 속성은 프레젠테이션이 비밀번호로 보호되는 경우에 의미가 있습니다. true이면 문서 속성이 프레젠테이션 파일에 암호화됩니다. false이면 프레젠테이션이 암호화된 상태에서도 문서 속성이 공개됩니다. 읽기/쓰기 부울.
...
이 인스턴스가 암호화되었는지 여부를 나타내는 값을 반환합니다. 읽기 전용 부울.
...
프레젠테이션 파일이 비밀번호로 보호되고 해당 파일의 문서 속성이 공개된 경우에 이 속성은 의미가 있습니다. true 값은 비밀번호 없이 암호화된 프레젠테이션 파일에서 문서 속성만 로드된다는 의미입니다. false 값은 올바른 비밀번호를 사용하여 전체 암호화된 프레젠테이션이 로드되며 문서 속성만 로드되지 않음을 의미합니다. 프레젠테이션이 암호화되지 않은 경우 속성 값은 항상 false입니다. 암호화된 파일의 문서 속성이 공개되지 않은 경우에도 속성 값은 항상 false입니다. Presentation.EncryptDocumentProperties가 true인 경우 IsOnlyDocumentPropertiesLoaded 속성 값은 항상 false입니다. 읽기 전용 부울.
...
이 프레젠테이션이 쓰기 보호되어 있는지 여부를 나타내는 값을 반환합니다. 읽기 전용 부울.
...
> ```
> 다음 샘플 코드는 PowerPoint 프레젠테이션을 암호화하는 방법을 보여줍니다.
>  
...
프레젠테이션 암호화에 사용되는 비밀번호를 반환합니다. 읽기 전용 문자열.
