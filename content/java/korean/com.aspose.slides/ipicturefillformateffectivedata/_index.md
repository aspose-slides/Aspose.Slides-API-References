...
| [getCropLeft()](#getCropLeft--) | 그림의 왼쪽에서 잘라낸 실제 이미지 너비의 백분율을 반환합니다. |
| [getCropTop()](#getCropTop--) | 그림의 상단에서 잘라낸 실제 이미지 높이의 백분율을 반환합니다. |
| [getCropRight()](#getCropRight--) | 그림의 오른쪽에서 잘라낸 실제 이미지 너비의 백분율을 반환합니다. |
| [getCropBottom()](#getCropBottom--) | 그림의 하단에서 잘라낸 실제 이미지 높이의 백분율을 반환합니다. |
...
그림의 왼쪽에서 잘라낸 실제 이미지 너비의 백분율을 반환합니다. 읽기 전용 float.
...
그림의 상단에서 잘라낸 실제 이미지 높이의 백분율을 반환합니다. 읽기 전용 float.
...
그림의 오른쪽에서 잘라낸 실제 이미지 너비의 백분율을 반환합니다. 읽기 전용 float.
...
그림의 하단에서 잘라낸 실제 이미지 높이의 백분율을 반환합니다. 읽기 전용 float.
