...
| [isVisible()](#isVisible--) | False는 데이터 레이블이 표시되지 않음을 의미합니다(따라서 모든 Show*-플래그(ShowValue, ...)가 false입니다). |
| [hide()](#hide--) | 모든 Show*-플래그(ShowValue, ...)를 false 상태로 설정하여 데이터 레이블을 숨깁니다. |
| [getActualLabelText()](#getActualLabelText--) | DataLabelFormat 설정 또는 TextFrameForOverriding.Text 값을 기반으로 실제 레이블 텍스트를 반환합니다. |
| [addTextFrameForOverriding(String text)](#addTextFrameForOverriding-java.lang.String-) | paramener "text"에 있는 텍스트로 TextFrameForOverriding을 초기화합니다. |
...
| [getActualX()](#getActualX--) | 차트 왼쪽 상단 코너를 기준으로 차트 요소의 실제 x 위치(왼쪽)를 지정합니다. |
| [getActualY()](#getActualY--) | 차트 왼쪽 상단 코너를 기준으로 차트 요소의 실제 상단을 지정합니다. |
...
읽기 전용 IDOMObject인 Parent_Immediate 객체를 반환합니다.
...
읽기 전용 [IChart](../../com.aspose.slides/ichart)인 부모 차트를 반환합니다.
...
False는 데이터 레이블이 표시되지 않음을 의미합니다(따라서 모든 Show*-플래그(ShowValue, ...)가 false입니다). 읽기 전용 boolean.

--------------------

데이터 레이블이 표시되는 경우 Hide() 메서드로 숨길 수 있습니다. 그러나 데이터 레이블이 표시되지 않는 경우(IsVisible가 false) Show*-플래그(ShowValue, ...)를 true 상태로 설정하여 표시할 수 있습니다.
...
모든 Show*-플래그(ShowValue, ...)를 false 상태로 설정하여 데이터 레이블을 숨깁니다. 이 후 IsVisible는 false가 됩니다.

--------------------

데이터 레이블이 표시되지 않는 경우(IsVisible가 false) Show*-플래그(ShowValue, ...)를 true 상태로 설정하여 표시할 수 있습니다.
...
paramener "text"에 있는 텍스트로 TextFrameForOverriding을 초기화합니다. TextFrameForOverriding이 이미 초기화된 경우 단순히 텍스트를 변경합니다.
...
리치 형식 텍스트를 포함할 수 있습니다. 이 속성이 null이 아니면 이 형식화된 텍스트 값이 자동 생성된 데이터 레이블 텍스트를 대체합니다. 자동 생성된 텍스트는 ShowSeriesName, ShowValue 등 속성에 의해 관리되며 TextFormatManager.TextFormat 속성으로 형식화됩니다. 읽기 전용 [ITextFrame](../../com.aspose.slides/itextframe).
...
차트 왼쪽 상단 코너를 기준으로 차트 요소의 실제 x 위치(왼쪽)를 지정합니다. 실제 값을 얻으려면 IChart.ValidateChartLayout() 메서드를 먼저 호출하십시오. 읽기 float.
...
차트 왼쪽 상단 코너를 기준으로 차트 요소의 실제 상단을 지정합니다. 실제 값을 얻으려면 IChart.ValidateChartLayout() 메서드를 먼저 호출하십시오. 읽기 float.
