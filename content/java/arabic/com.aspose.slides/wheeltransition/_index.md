---
title: WheelTransition
second_title: مرجع API لـ Aspose.Slides للـ Java
description: تأثير انتقال شريحة العجلة.
type: docs
url: /ar/com.aspose.slides/wheeltransition/
---
**الوراثة:**
java.lang.Object, [com.aspose.slides.TransitionValueBase](../../com.aspose.slides/transitionvaluebase)

**جميع الواجهات المُنفّذة:**
[com.aspose.slides.IWheelTransition](../../com.aspose.slides/iwheeltransition)
```
public class WheelTransition extends TransitionValueBase implements IWheelTransition
```

تأثير انتقال شريحة العجلة.
## الطرق

| الطريقة | الوصف |
| --- | --- |
| [getSpokes()](#getSpokes--) | عدد أسنان العجلة للانتقال. |
| [setSpokes(long value)](#setSpokes-long-) | عدد أسنان العجلة للانتقال. |
### getSpokes() {#getSpokes--}
```
public final long getSpokes()
```

عدد أسنان العجلة للانتقال. قراءة/كتابة long.

**الإرجاع:**
long
### setSpokes(long value) {#setSpokes-long-}
```
public final void setSpokes(long value)
```

عدد أسنان العجلة للانتقال. قراءة/كتابة long.

**المعلمات:**
| المعامل | النوع | الوصف |
| --- | --- | --- |
| value | long |  |