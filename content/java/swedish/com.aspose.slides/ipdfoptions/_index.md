...
| [getEmbedFullFonts()](#getEmbedFullFonts--) | Bestämmer om alla teckensnittskaraktärer ska bäddas in eller endast en använd delmängd. |
| [setEmbedFullFonts(boolean value)](#setEmbedFullFonts-boolean-) | Bestämmer om alla teckensnittskaraktärer ska bäddas in eller endast en använd delmängd. |
...
| [getSlidesLayoutOptions()](#getSlidesLayoutOptions--) | Hämtar eller anger läget som bilder placeras på sidan när en presentation exporteras [ISlidesLayoutOptions](../../com.aspose.slides/islideslayoutoptions). |
| [setSlidesLayoutOptions(ISlidesLayoutOptions value)](#setSlidesLayoutOptions-com.aspose.slides.ISlidesLayoutOptions-) | Hämtar eller anger läget som bilder placeras på sidan när en presentation exporteras [ISlidesLayoutOptions](../../com.aspose.slides/islideslayoutoptions). |
...
| [getApplyImageTransparent()](#getApplyImageTransparent--) | Applicerar den angivna transparanta färgen på en bild om sant. |
| [setApplyImageTransparent(boolean value)](#setApplyImageTransparent-boolean-) | Applicerar den angivna transparanta färgen på en bild om sant. |
...
Anger kompressionstyp som ska användas för allt textinnehåll i dokumentet. Läs/skriva [PdfTextCompression](../../com.aspose.slides/pdftextcompression).
...
Indikerar om den mest effektiva kompressionen (istället för standard) för varje bild ska väljas automatiskt. Om satt till true, kommer för varje bild i presentationen den mest lämpliga kompressionsalgoritmen att väljas, vilket leder till en mindre storlek på det resulterande PDF-dokumentet.

---
Val av bästa bildkompressionsförhållande är beräkningsintensivt och kräver extra RAM, och detta alternativ är false som standard.
...
Sant för att bädda in TrueType-teckensnitt för ASCII-tecken 32-127. Teckensnitt för teckenkoder över 127 bäddas alltid in. Läs/skriva boolean.
...
Returnerar eller anger en array av användardefinierade namn på teckensnittsfamiljer som Aspose.Slides ska betrakta som gemensamma. Läs/skriva String[].
...
Bestämmer om alla teckensnittskaraktärer ska bäddas in eller endast en använd delmängd. Läs/skriva boolean.
...
Indikerar om text ska rasteriseras som en bitmap och sparas till PDF när teckensnittet inte stödjer fet stil. Detta tillvägagångssätt kan förbättra textkvaliteten i det resulterande PDF-dokumentet för vissa teckensnitt. Läs/skriva boolean.
...
Returnerar eller anger ett värde som bestämmer kvaliteten på JPEG-bilderna i PDF-dokumentet. Läs/skriva byte.

Har endast effekt när ett dokument innehåller JPEG-bilder.

Använd denna egenskap för att hämta eller ange bildkvaliteten i ett dokument vid sparande i PDF-format. Värdet kan variera från 0 till 100 där 0 betyder sämst kvalitet men maximal kompression och 100 betyder bästa kvalitet men minimal kompression.
...
Önskad efterlevnadsnivå för genererat PDF-dokument. Läs/skriva [PdfCompliance](../../com.aspose.slides/pdfcompliance).
...
Ställer in användarlösenord för att skydda PDF-dokumentet. Läs/skriva String.
...
Sant för att konvertera alla metafiler som används i en presentation till PNG-bilder. Läs/skriva boolean.

---
Standardvärdet är **true**. Pdf-dokument kan innehålla vektorgrafik och rasterbilder. Om SaveMetafilesAsPng är satt till true konverteras käll-Metafile-bilden till Png-format och sparas till Pdf som en rasterbild. Om SaveMetafilesAsPng är satt till false konverteras käll-Metafile till Pdf-vektorgrafik. Varje tillvägagångssätt har fördelar och nackdelar. Till exempel, om Metafile konverteras till PNG kan viss kvalitetsförlust uppstå under skalning av det resulterande dokumentet. Om Metafile konverteras till Pdf-vektorgrafik kan prestandaproblem i Pdf-visningsverktyg uppstå.
...
Returnerar eller anger ett värde som bestämmer upplösningen på bilder i PDF-dokumentet. Läs/skriva float.

Värde: Effekten av denna parameter beror på flera faktorer. Algoritmen försöker få bästa möjliga bildstorlek utifrån egenskapsvärdet, källbildens storlek och bildramens storlek. Användning av liknande värden kan ge samma resultat. Rekommenderas att använda steg 16 eller 32 för att få synlig effekt.
...
Sant för att rita en svart ram runt varje bild. Läs/skriva boolean.
...
Hämtar eller anger läget som bilder placeras på sidan när en presentation exporteras [ISlidesLayoutOptions](../../com.aspose.slides/islideslayoutoptions).
...
Hämtar eller anger bildens transparanta färg.
...
Applicerar den angivna transparanta färgen på en bild om sant.
...
Sant för att konvertera all OLE-data från presentationen till inbäddade filer i den resulterande PDF. Läs/skriva boolean.
