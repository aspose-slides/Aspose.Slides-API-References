---
title: GraphicalObjectLock
second_title: Aspose.Slides för Java API-referens
description: Bestämmer vilka operationer som är inaktiverade på den överordnade GraphicalObject.
type: docs
url: /sv/com.aspose.slides/graphicalobjectlock/
---
**Arv:**
java.lang.Object, [com.aspose.slides.BaseShapeLock](../../com.aspose.slides/baseshapelock)

**Alla implementerade gränssnitt:**
[com.aspose.slides.IGraphicalObjectLock](../../com.aspose.slides/igraphicalobjectlock)
```
public class GraphicalObjectLock extends BaseShapeLock implements IGraphicalObjectLock
```

Bestämmer vilka operationer som är inaktiverade på den överordnade GraphicalObject.
## Metoder

| Metod | Beskrivning |
| --- | --- |
| [getGroupingLocked()](#getGroupingLocked--) | Bestämmer om det är förbjudet att lägga till den här formen i en grupp. |
| [setGroupingLocked(boolean value)](#setGroupingLocked-boolean-) | Bestämmer om det är förbjudet att lägga till den här formen i en grupp. |
| [getDrilldownLocked()](#getDrilldownLocked--) | Bestämmer om det är förbjudet att välja underformer av detta objekt. |
| [setDrilldownLocked(boolean value)](#setDrilldownLocked-boolean-) | Bestämmer om det är förbjudet att välja underformer av detta objekt. |
| [getSelectLocked()](#getSelectLocked--) | Bestämmer om det är förbjudet att välja den här formen. |
| [setSelectLocked(boolean value)](#setSelectLocked-boolean-) | Bestämmer om det är förbjudet att välja den här formen. |
| [getAspectRatioLocked()](#getAspectRatioLocked--) | Bestämmer om formen måste bevara bildförhållandet vid storleksändring. |
| [setAspectRatioLocked(boolean value)](#setAspectRatioLocked-boolean-) | Bestämmer om formen måste bevara bildförhållandet vid storleksändring. |
| [getPositionLocked()](#getPositionLocked--) | Bestämmer om det är förbjudet att flytta den här formen. |
| [setPositionLocked(boolean value)](#setPositionLocked-boolean-) | Bestämmer om det är förbjudet att flytta den här formen. |
| [getSizeLocked()](#getSizeLocked--) | Bestämmer om det är förbjudet att ändra storlek på den här formen. |
| [setSizeLocked(boolean value)](#setSizeLocked-boolean-) | Bestämmer om det är förbjudet att ändra storlek på den här formen. |
### getGroupingLocked() {#getGroupingLocked--}
```
public boolean getGroupingLocked()
```


Bestämmer om det är förbjudet att lägga till den här formen i en grupp. Läs/skriv boolean.

**Returnerar:**
boolean
### setGroupingLocked(boolean value) {#setGroupingLocked-boolean-}
```
public void setGroupingLocked(boolean value)
```


Bestämmer om det är förbjudet att lägga till den här formen i en grupp. Läs/skriv boolean.

**Parametrar:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| value | boolean |  |

### getDrilldownLocked() {#getDrilldownLocked--}
```
public boolean getDrilldownLocked()
```


Bestämmer om det är förbjudet att välja underformer av detta objekt. Läs/skriv boolean.

**Returnerar:**
boolean
### setDrilldownLocked(boolean value) {#setDrilldownLocked-boolean-}
```
public void setDrilldownLocked(boolean value)
```


Bestämmer om det är förbjudet att välja underformer av detta objekt. Läs/skriv boolean.

**Parametrar:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| value | boolean |  |

### getSelectLocked() {#getSelectLocked--}
```
public boolean getSelectLocked()
```


Bestämmer om det är förbjudet att välja den här formen. Läs/skriv boolean.

**Returnerar:**
boolean
### setSelectLocked(boolean value) {#setSelectLocked-boolean-}
```
public void setSelectLocked(boolean value)
```


Bestämmer om det är förbjudet att välja den här formen. Läs/skriv boolean.

**Parametrar:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| value | boolean |  |

### getAspectRatioLocked() {#getAspectRatioLocked--}
```
public boolean getAspectRatioLocked()
```


Bestämmer om formen måste bevara bildförhållandet vid storleksändring. Läs/skriv boolean.

**Returnerar:**
boolean
### setAspectRatioLocked(boolean value) {#setAspectRatioLocked-boolean-}
```
public void setAspectRatioLocked(boolean value)
```


Bestämmer om formen måste bevara bildförhållandet vid storleksändring. Läs/skriv boolean.

**Parametrar:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| value | boolean |  |

### getPositionLocked() {#getPositionLocked--}
```
public boolean getPositionLocked()
```


Bestämmer om det är förbjudet att flytta den här formen. Läs/skriv boolean.

**Returnerar:**
boolean
### setPositionLocked(boolean value) {#setPositionLocked-boolean-}
```
public void setPositionLocked(boolean value)
```


Bestämmer om det är förbjudet att flytta den här formen. Läs/skriv boolean.

**Parametrar:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| value | boolean |  |

### getSizeLocked() {#getSizeLocked--}
```
public boolean getSizeLocked()
```


Bestämmer om det är förbjudet att ändra storlek på den här formen. Läs/skriv boolean.

**Returnerar:**
boolean
### setSizeLocked(boolean value) {#setSizeLocked-boolean-}
```
public void setSizeLocked(boolean value)
```


Bestämmer om det är förbjudet att ändra storlek på den här formen. Läs/skriv boolean.

**Parametrar:**
| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| value | boolean |  |