---
title: EffectChartMinorGroupingType
second_title: Aspose.Slides för Java API-referens
description: Representerar typen av en animeringseffekt för diagrammets element i serie eller kategori.
type: docs
url: /sv/com.aspose.slides/effectchartminorgroupingtype/
---
**Arv:**
java.lang.Object, com.aspose.ms.System.ValueType, com.aspose.ms.System.Enum
```
public final class EffectChartMinorGroupingType extends System.Enum
```

Representerar typen av en animeringseffekt för diagrammets element i serie eller kategori.
## Fält

| Fält | Beskrivning |
| --- | --- |
| [ByElementInSeries](#ByElementInSeries) | Animera diagrammet efter element i serie |
| [ByElementInCategory](#ByElementInCategory) | Animera diagrammet efter element i kategori |
### ByElementInSeries {#ByElementInSeries}
```
public static final int ByElementInSeries
```

Animera diagrammet efter element i serie

### ByElementInCategory {#ByElementInCategory}
```
public static final int ByElementInCategory
```

Animera diagrammet efter element i kategori