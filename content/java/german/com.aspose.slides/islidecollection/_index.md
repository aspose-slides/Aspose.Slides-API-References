...
| --- | --- |
| [get_Item(int index)](#get-Item-int-) | Liefert das Element am angegebenen Index zurück. |
...
Liefert das Element am angegebenen Index zurück. Nur lesbar [ISlide](../../com.aspose.slides/islide).
...
Beim Klonen einer Folie zwischen verschiedenen Präsentationen kann auch das Master-Layout der Folie geklont werden. Ein internes Register wird verwendet, um automatisch geklonte Master zu verfolgen und die Erstellung mehrerer Klone desselben Master-Layouts zu verhindern. Ein manuelles Klonen von Master-Layouts wird weder verhindert noch registriert. Wenn Sie mehr Kontrolle über den Klonvorgang benötigen, verwenden Sie \#addClone(ISlide,ILayoutSlide).addClone(ISlide,ILayoutSlide) oder \#addClone(ISlide,IMasterSlide,boolean).addClone(ISlide,IMasterSlide,boolean) für das Klonen von Folien, [IGlobalLayoutSlideCollection.addClone(ILayoutSlide)](../../com.aspose.slides/igloballayoutslidecollection\#addClone-ILayoutSlide-) oder [IGlobalLayoutSlideCollection.addClone(ILayoutSlide,IMasterSlide)](../../com.aspose.slides/igloballayoutslidecollection\#addClone-ILayoutSlide-IMasterSlide-) für das Klonen von Layouts und [IMasterSlideCollection.addClone(IMasterSlide)](../../com.aspose.slides/imasterslidecollection\#addClone-IMasterSlide-) für das Klonen von Master-Layouts. |
...
Beim Klonen einer Folie zwischen verschiedenen Präsentationen kann auch das Master-Layout der Folie geklont werden. Ein internes Register wird verwendet, um automatisch geklonte Master zu verfolgen und die Erstellung mehrerer Klone desselben Master-Layouts zu verhindern. Ein manuelles Klonen von Master-Layouts wird weder verhindert noch registriert. Wenn Sie mehr Kontrolle über den Klonvorgang benötigen, verwenden Sie \#insertClone(int,ISlide,ILayoutSlide).insertClone(int,ISlide,ILayoutSlide) oder \#insertClone(int,ISlide,IMasterSlide,boolean).insertClone(int,ISlide,IMasterSlide,boolean) für das Klonen von Folien und [IMasterSlideCollection.addClone(IMasterSlide)](../../com.aspose.slides/imasterslidecollection\#addClone-IMasterSlide-) für das Klonen von Master-Layouts. |
...
Fügt eine Kopie einer angegebenen Quellfolie am Ende der Sammlung hinzu. Das passende Layout wird automatisch aus dem angegebenen Master-Layout ausgewählt (das passende Layout ist das Layout mit dem gleichen Typ oder Namen wie das Layout der Quellfolie). Gibt es kein passendes Layout, wird das Layout der Quellfolie geklont (wenn allowCloneMissingLayout = true) oder es wird eine PptxEditException ausgelöst (wenn allowCloneMissingLayout = false).

**Parameter:**
| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| sourceSlide | [ISlide](../../com.aspose.slides/islide) | Zu klonende Folie. |
| destMaster | [IMasterSlide](../../com.aspose.slides/imasterslide) | Master-Folienlayout für die neue Folie. |
| allowCloneMissingLayout | boolean | Wenn im angegebenen Master-Layout kein passendes Layout vorhanden ist, wird das Layout der Quellfolie geklont (wenn true) oder es wird eine PptxEditException ausgelöst (wenn false). |
...
Verschiebt Folien aus der Sammlung an die angegebene Position. Die Folien werden beginnend ab dem Index in der Reihenfolge, in der sie in der Liste erscheinen, eingefügt.
...
| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| slide | [ISlide](../../com.aspose.slides/islide) | Zu suchende Folie.
...
| resolver | [IExternalResourceResolver](../../com.aspose.slides/iexternalresourceresolver) | Ein Callback-Objekt, das verwendet wird, um externe Objekte abzurufen. Wenn dieser Parameter null ist, werden alle externen Objekte ignoriert. |
...
| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| index | int | Einzufügende Position. |
| htmlText | java.lang.String | HTML zum Hinzufügen. |
| resolver | [IExternalResourceResolver](../../com.aspose.slides/iexternalresourceresolver) | Ein Callback-Objekt, das verwendet wird, um externe Objekte abzurufen. Wenn dieser Parameter null ist, werden alle externen Objekte ignoriert. |
| uri | java.lang.String | Ein URI des angegebenen HTML. Wird verwendet, um relative Links aufzulösen. |
