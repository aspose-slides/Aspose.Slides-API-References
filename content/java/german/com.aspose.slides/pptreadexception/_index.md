---
title: PptReadException
second_title: Aspose.Slides für Java API-Referenz
description: Stellt eine Ausnahme dar, die bei Lesefehlern von Präsentationen ausgelöst wird.
type: docs
url: /de/com.aspose.slides/pptreadexception/
---
**Vererbung:**
java.lang.Object, java.lang.Throwable, java.lang.Exception, java.lang.RuntimeException, com.aspose.ms.System.Exception, [com.aspose.slides.PptException](../../com.aspose.slides/pptexception)
```
public class PptReadException extends PptException
```

Stellt eine Ausnahme dar, die bei Lesefehlern von Präsentationen ausgelöst wird.
## Konstruktoren

| Konstruktor | Beschreibung |
| --- | --- |
| [PptReadException()](#PptReadException--) | Standardkonstruktor. |
| [PptReadException(String message)](#PptReadException-java.lang.String-) | Konstruktor, der das Hinzufügen einer Nachricht zu dieser Ausnahme ermöglicht. |
| [PptReadException(String message, Exception exception)](#PptReadException-java.lang.String-java.lang.Exception-) | Konstruktor für eine Ausnahme, die eine Nachricht und eine eingebettete Ausnahme enthält. |
### PptReadException() {#PptReadException--}
```
public PptReadException()
```

Standardkonstruktor.

### PptReadException(String message) {#PptReadException-java.lang.String-}
```
public PptReadException(String message)
```

Konstruktor, der das Hinzufügen einer Nachricht zu dieser Ausnahme ermöglicht.

**Parameter:**
| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| message | java.lang.String | Nachricht |

### PptReadException(String message, Exception exception) {#PptReadException-java.lang.String-java.lang.Exception-}
```
public PptReadException(String message, Exception exception)
```

Konstruktor für eine Ausnahme, die eine Nachricht und eine eingebettete Ausnahme enthält.

**Parameter:**
| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| message | java.lang.String | Nachricht |
| exception | java.lang.Exception | ursprüngliche Ausnahme |