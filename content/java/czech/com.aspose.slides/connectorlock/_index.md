---
title: ConnectorLock
second_title: Aspose.Slides pro Java - referenční příručka API
description: Určuje, které operace jsou zakázány na nadřazeném konektoru.
type: docs
url: /cs/com.aspose.slides/connectorlock/
---
**Dědičnost:**
java.lang.Object, [com.aspose.slides.BaseShapeLock](../../com.aspose.slides/baseshapelock)

**Všechna implementovaná rozhraní:**
[com.aspose.slides.IConnectorLock](../../com.aspose.slides/iconnectorlock)
```
public class ConnectorLock extends BaseShapeLock implements IConnectorLock
```

Určuje, které operace jsou zakázány na nadřazeném konektoru.
## Metody

| Metoda | Popis |
| --- | --- |
| [getGroupingLocked()](#getGroupingLocked--) | Určuje, zda je přidání tohoto tvaru do skupiny zakázáno. |
| [setGroupingLocked(boolean value)](#setGroupingLocked-boolean-) | Určuje, zda je přidání tohoto tvaru do skupiny zakázáno. |
| [getSelectLocked()](#getSelectLocked--) | Určuje, zda je výběr tohoto tvaru zakázán. |
| [setSelectLocked(boolean value)](#setSelectLocked-boolean-) | Určuje, zda je výběr tohoto tvaru zakázán. |
| [getRotateLocked()](#getRotateLocked--) | Určuje, zda je změna úhlu otáčení tohoto tvaru zakázána. |
| [setRotateLocked(boolean value)](#setRotateLocked-boolean-) | Určuje, zda je změna úhlu otáčení tohoto tvaru zakázána. |
| [getAspectRatioLocked()](#getAspectRatioLocked--) | Určuje, zda má tento tvar zachovat poměr stran při změně velikosti. |
| [setAspectRatioLocked(boolean value)](#setAspectRatioLocked-boolean-) | Určuje, zda má tento tvar zachovat poměr stran při změně velikosti. |
| [getPositionMove()](#getPositionMove--) | Určuje, zda je přesunutí tohoto tvaru zakázáno. |
| [setPositionMove(boolean value)](#setPositionMove-boolean-) | Určuje, zda je přesunutí tohoto tvaru zakázáno. |
| [getSizeLocked()](#getSizeLocked--) | Určuje, zda je změna velikosti tohoto tvaru zakázána. |
| [setSizeLocked(boolean value)](#setSizeLocked-boolean-) | Určuje, zda je změna velikosti tohoto tvaru zakázána. |
| [getEditPointsLocked()](#getEditPointsLocked--) | Určuje, zda je přímá změna obrysu tohoto tvaru zakázána. |
| [setEditPointsLocked(boolean value)](#setEditPointsLocked-boolean-) | Určuje, zda je přímá změna obrysu tohoto tvaru zakázána. |
| [getAdjustHandlesLocked()](#getAdjustHandlesLocked--) | Určuje, zda je změna hodnot úprav zakázána. |
| [setAdjustHandlesLocked(boolean value)](#setAdjustHandlesLocked-boolean-) | Určuje, zda je změna hodnot úprav zakázána. |
| [getArrowheadsLocked()](#getArrowheadsLocked--) | Určuje, zda je změna koncových šipek zakázána. |
| [setArrowheadsLocked(boolean value)](#setArrowheadsLocked-boolean-) | Určuje, zda je změna koncových šipek zakázána. |
| [getShapeTypeLocked()](#getShapeTypeLocked--) | Určuje, zda je změna typu tvaru zakázána. |
| [setShapeTypeLocked(boolean value)](#setShapeTypeLocked-boolean-) | Určuje, zda je změna typu tvaru zakázána. |
### getGroupingLocked() {#getGroupingLocked--}
```
public boolean getGroupingLocked()
```


Určuje, zda je přidání tohoto tvaru do skupiny zakázáno. Čtení/zápis boolean.

**Vrací:**
boolean
### setGroupingLocked(boolean value) {#setGroupingLocked-boolean-}
```
public void setGroupingLocked(boolean value)
```


Určuje, zda je přidání tohoto tvaru do skupiny zakázáno. Čtení/zápis boolean.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | boolean |  |

### getSelectLocked() {#getSelectLocked--}
```
public boolean getSelectLocked()
```


Určuje, zda je výběr tohoto tvaru zakázán. Čtení/zápis boolean.

**Vrací:**
boolean
### setSelectLocked(boolean value) {#setSelectLocked-boolean-}
```
public void setSelectLocked(boolean value)
```


Určuje, zda je výběr tohoto tvaru zakázán. Čtení/zápis boolean.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | boolean |  |

### getRotateLocked() {#getRotateLocked--}
```
public boolean getRotateLocked()
```


Určuje, zda je změna úhlu otáčení tohoto tvaru zakázána. Čtení/zápis boolean.

**Vrací:**
boolean
### setRotateLocked(boolean value) {#setRotateLocked-boolean-}
```
public void setRotateLocked(boolean value)
```


Určuje, zda je změna úhlu otáčení tohoto tvaru zakázána. Čtení/zápis boolean.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | boolean |  |

### getAspectRatioLocked() {#getAspectRatioLocked--}
```
public boolean getAspectRatioLocked()
```


Určuje, zda má tento tvar zachovat poměr stran při změně velikosti. Čtení/zápis boolean.

**Vrací:**
boolean
### setAspectRatioLocked(boolean value) {#setAspectRatioLocked-boolean-}
```
public void setAspectRatioLocked(boolean value)
```


Určuje, zda má tento tvar zachovat poměr stran při změně velikosti. Čtení/zápis boolean.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | boolean |  |

### getPositionMove() {#getPositionMove--}
```
public boolean getPositionMove()
```


Určuje, zda je přesunutí tohoto tvaru zakázáno. Čtení/zápis boolean.

**Vrací:**
boolean
### setPositionMove(boolean value) {#setPositionMove-boolean-}
```
public void setPositionMove(boolean value)
```


Určuje, zda je přesunutí tohoto tvaru zakázáno. Čtení/zápis boolean.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | boolean |  |

### getSizeLocked() {#getSizeLocked--}
```
public boolean getSizeLocked()
```


Určuje, zda je změna velikosti tohoto tvaru zakázána. Čtení/zápis boolean.

**Vrací:**
boolean
### setSizeLocked(boolean value) {#setSizeLocked-boolean-}
```
public void setSizeLocked(boolean value)
```


Určuje, zda je změna velikosti tohoto tvaru zakázána. Čtení/zápis boolean.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | boolean |  |

### getEditPointsLocked() {#getEditPointsLocked--}
```
public boolean getEditPointsLocked()
```


Určuje, zda je přímá změna obrysu tohoto tvaru zakázána. Čtení/zápis boolean.

**Vrací:**
boolean
### setEditPointsLocked(boolean value) {#setEditPointsLocked-boolean-}
```
public void setEditPointsLocked(boolean value)
```


Určuje, zda je přímá změna obrysu tohoto tvaru zakázána. Čtení/zápis boolean.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | boolean |  |

### getAdjustHandlesLocked() {#getAdjustHandlesLocked--}
```
public boolean getAdjustHandlesLocked()
```


Určuje, zda je změna hodnot úprav zakázána. Čtení/zápis boolean.

**Vrací:**
boolean
### setAdjustHandlesLocked(boolean value) {#setAdjustHandlesLocked-boolean-}
```
public void setAdjustHandlesLocked(boolean value)
```


Určuje, zda je změna hodnot úprav zakázána. Čtení/zápis boolean.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | boolean |  |

### getArrowheadsLocked() {#getArrowheadsLocked--}
```
public boolean getArrowheadsLocked()
```


Určuje, zda je změna koncových šipek zakázána. Čtení/zápis boolean.

**Vrací:**
boolean
### setArrowheadsLocked(boolean value) {#setArrowheadsLocked-boolean-}
```
public void setArrowheadsLocked(boolean value)
```


Určuje, zda je změna koncových šipek zakázána. Čtení/zápis boolean.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | boolean |  |

### getShapeTypeLocked() {#getShapeTypeLocked--}
```
public boolean getShapeTypeLocked()
```


Určuje, zda je změna typu tvaru zakázána. Čtení/zápis boolean.

**Vrací:**
boolean
### setShapeTypeLocked(boolean value) {#setShapeTypeLocked-boolean-}
```
public void setShapeTypeLocked(boolean value)
```


Určuje, zda je změna typu tvaru zakázána. Čtení/zápis boolean.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | boolean |  |