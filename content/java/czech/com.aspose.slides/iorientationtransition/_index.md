---
title: IOrientationTransition
second_title: Aspose.Slides pro Java – API reference
description: Efekt orientačního přechodu snímku.
type: docs
url: /cs/com.aspose.slides/iorientationtransition/
---
**Všechny implementované rozhraní:**
[com.aspose.slides.ITransitionValueBase](../../com.aspose.slides/itransitionvaluebase)
```
public interface IOrientationTransition extends ITransitionValueBase
```

Efekt orientačního přechodu snímku.
## Metody

| Metoda | Popis |
| --- | --- |
| [getDirection()](#getDirection--) | Směr přechodu. |
| [setDirection(byte value)](#setDirection-byte-) | Směr přechodu. |
### getDirection() {#getDirection--}
```
public abstract byte getDirection()
```

Směr přechodu. Čtení/Zápis [Orientation](../../com.aspose.slides/orientation).

**Návratová hodnota:**
byte
### setDirection(byte value) {#setDirection-byte-}
```
public abstract void setDirection(byte value)
```

Směr přechodu. Čtení/Zápis [Orientation](../../com.aspose.slides/orientation).

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | byte |  |