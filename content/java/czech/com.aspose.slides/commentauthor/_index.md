---
title: CommentAuthor
second_title: Referencia API Aspose.Slides pro Java
description: Reprezentuje autora komentářů.
type: docs
url: /cs/com.aspose.slides/commentauthor/
---
**Dědičnost:**
java.lang.Object

**Všechna implementovaná rozhraní:**
[com.aspose.slides.ICommentAuthor](../../com.aspose.slides/icommentauthor), com.aspose.slides.IDOMObject
```
public final class CommentAuthor implements ICommentAuthor, IDOMObject
```

Reprezentuje autora komentářů.
## Metody

| Metoda | Popis |
| --- | --- |
| [getName()](#getName--) | Vrací nebo nastavuje jméno autora. |
| [setName(String value)](#setName-java.lang.String-) | Vrací nebo nastavuje jméno autora. |
| [getInitials()](#getInitials--) | Vrací nebo nastavuje iniciály autora. |
| [setInitials(String value)](#setInitials-java.lang.String-) | Vrací nebo nastavuje iniciály autora. |
| [getComments()](#getComments--) | Vrací kolekci komentářů vytvořených tímto autorem. |
| [remove()](#remove--) | Odstraňuje autora z nadřazené kolekce. |
| [getParent_Immediate()](#getParent-Immediate--) |  |
### getName() {#getName--}
```
public final String getName()
```

Vrací nebo nastavuje jméno autora. Read/write String.

**Vrací:**
java.lang.String
### setName(String value) {#setName-java.lang.String-}
```
public final void setName(String value)
```

Vrací nebo nastavuje jméno autora. Read/write String.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | java.lang.String |  |

### getInitials() {#getInitials--}
```
public final String getInitials()
```

Vrací nebo nastavuje iniciály autora. Read/write String.

**Vrací:**
java.lang.String
### setInitials(String value) {#setInitials-java.lang.String-}
```
public final void setInitials(String value)
```

Vrací nebo nastavuje iniciály autora. Read/write String.

**Parametry:**
| Parametr | Typ | Popis |
| --- | --- | --- |
| value | java.lang.String |  |

### getComments() {#getComments--}
```
public final ICommentCollection getComments()
```

Vrací kolekci komentářů vytvořených tímto autorem. Read-only [ICommentCollection](../../com.aspose.slides/icommentcollection).

**Vrací:**
[ICommentCollection](../../com.aspose.slides/icommentcollection)
### remove() {#remove--}
```
public final void remove()
```

Odstraňuje autora z nadřazené kolekce.

### getParent_Immediate() {#getParent-Immediate--}
```
public final IDOMObject getParent_Immediate()
```

Vrací objekt Parent_Immediate. Read-only IDOMObject.

**Vrací:**
com.aspose.slides.IDOMObject