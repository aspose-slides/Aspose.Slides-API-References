---
title: MorphTransition
second_title: Aspose.Slides Java API Referansı
description: Ripple slayt geçiş efekti.
type: docs
url: /tr/com.aspose.slides/morphtransition/
---
**Kalıtım:**
java.lang.Object, [com.aspose.slides.TransitionValueBase](../../com.aspose.slides/transitionvaluebase)

**Uygulanan Tüm Arabirimler:**
[com.aspose.slides.IMorphTransition](../../com.aspose.slides/imorphtransition)
```
public class MorphTransition extends TransitionValueBase implements IMorphTransition
```

Ripple slayt geçiş efekti.
## Yöntemler

| Metod | Açıklama |
| --- | --- |
| [getMorphType()](#getMorphType--) | Morph geçişinin türü. |
| [setMorphType(int value)](#setMorphType-int-) | Morph geçişinin türü. |
### getMorphType() {#getMorphType--}
```
public final int getMorphType()
```

Morph geçişinin türü. Okunabilir/Yazılabilir [TransitionMorphType](../../com.aspose.slides/transitionmorphtype).

**Döndürür:**
int
### setMorphType(int value) {#setMorphType-int-}
```
public final void setMorphType(int value)
```

Morph geçişinin türü. Okunabilir/Yazılabilir [TransitionMorphType](../../com.aspose.slides/transitionmorphtype).

**Parametreler:**
| Parametre | Tür | Açıklama |
| --- | --- | --- |
| value | int |  |