...
| [getThreeDFormat()](#getThreeDFormat--) | Bir şekil için çizgi biçimlendirme özelliklerini içeren ThreeDFormat nesnesini döndürür. |
| [getEffectFormat()](#getEffectFormat--) | Bir şekle uygulanan piksel efektlerini içeren EffectFormat nesnesini döndürür. |
| [getFillFormat()](#getFillFormat--) | Bir şekil için dolgu biçimlendirme özelliklerini içeren FillFormat nesnesini döndürür. |
| [getImage()](#getImage--) | Şekil küçük resmini döndürür. |
| [getImage(int bounds, float scaleX, float scaleY)](#getImage-int-float-float-) | Şekil küçük resmini döndürür. |
...
| [isDecorative()](#isDecorative--) | ‘Dekoratif olarak işaretle’ seçeneğini alır veya ayarlar, okunur/yazılır boolean. |
| [setDecorative(boolean value)](#setDecorative-boolean-) | ‘Dekoratif olarak işaretle’ seçeneğini alır veya ayarlar, okunur/yazılır boolean. |
| [getShapeLock()](#getShapeLock--) | Şeklin kilitlerini döndürür. |
| [getUniqueId()](#getUniqueId--) | Eklentiler veya diğer kodlar tarafından kullanılmak üzere, sunum kapsamlı dahili bir tanımlayıcı döndürür. |
| [getOfficeInteropShapeId()](#getOfficeInteropShapeId--) | Bir slayt kapsamlı benzersiz tanımlayıcı döndürür; bu tanımlayıcı şeklin yaşam süresi boyunca sabit kalır ve PowerPoint ya da interop kodunun şekle belge içinde her yerden güvenilir bir şekilde başvurmasını sağlar. |
...
| [getBasePlaceholder()](#getBasePlaceholder--) | Temel bir yer tutucu şekli döndürür (düzenleme ve/veya ana slayttan miras alınan şekil). |
...
Bir şekil için yer tutucuyu döndürür. Salt okunur [IPlaceholder](../../com.aspose.slides/iplaceholder).
...
>  //Bu kod belirsiz durumlara yol açabilir. Bu nedenle IShape.getFrame() için tanımsız değerlerin kullanılmasına kısıtlamalar eklenmiştir. x, y, genişlik, yükseklik, flipH, flipV ve rotationAngle değerleri tanımlı olmalıdır (Float.NaN veya NullableBool.NotDefined). Yukarıdaki örnek kod artık ArgumentException hatası fırlatır.
>  //Bu, aşağıdaki kullanım durumları için geçerlidir:
>  IShape shape = ...;
>  shape.setFrame(...); // tanımsız olamaz
>  IShapeCollection shapes = ...;
>  // x, y, genişlik, yükseklik parametreleri Float.NaN olamaz:
>  {
...
>  shape.setRawFrame(new ShapeFrame(Float.NaN, Float.NaN, 100, Float.NaN, NullableBool.NotDefined, NullableBool.NotDefined, 0)); // şimdi şekil placeholder'dan x, y, yükseklik, flipH, flipV değerlerini devralır ve width=100 ve rotationAngle=0 değerlerini geçersiz kılar.
```
...
>  //veya
>  slide.getShapes().addAutoShape(ShapeType.RoundCornerRectangle, Float.NaN, Float.NaN, Float.NaN, Float.NaN);
>  //Bu kod belirsiz durumlara yol açabilir. Bu nedenle IShape.getFrame() için tanımsız değerlerin kullanılmasına kısıtlamalar eklenmiştir. x, y, genişlik, yükseklik, flipH, flipV ve rotationAngle değerleri tanımlı olmalıdır (Float.NaN veya NullableBool.NotDefined değil). Yukarıdaki örnek kod artık ArgumentException hatası fırlatır.
...
>  But IShape.RawFrame frame properties can be undefined. This make sense when shape is linked to placeholder. Then undefined shape frame values is overridden from the parent placeholder shape. If there is no parent placeholder shape for that shape then that shape uses default values when it evaluates effective frame based on its IShape.RawFrame. Default values are 0 and NullableBool.False for x, y, width, height, flipH, flipV and rotationAngle. For example:
>  IShape shape = ...; // şekil placeholder'a bağlıdır
>  shape.setRawFrame(new ShapeFrame(Float.NaN, Float.NaN, 100, Float.NaN, NullableBool.NotDefined, NullableBool.NotDefined, 0)); // şimdi şekil x, y, yükseklik, flipH, flipV değerlerini placeholder'dan devralır ve width=100 ve rotationAngle=0 değerlerini geçersiz kılar.
```
...
Bir şekil için çizgi biçimlendirme özelliklerini içeren LineFormat nesnesini döndürür. Salt okunur [ILineFormat](../../com.aspose.slides/ilineformat).
...
Bir şekil için çizgi biçimlendirme özelliklerini içeren ThreeDFormat nesnesini döndürür. Salt okunur [IThreeDFormat](../../com.aspose.slides/ithreedformat).
...
Bir şekle uygulanan piksel efektlerini içeren EffectFormat nesnesini döndürür. Salt okunur [IEffectFormat](../../com.aspose.slides/ieffectformat).
...
Bir şekil için dolgu biçimlendirme özelliklerini içeren FillFormat nesnesini döndürür. Salt okunur [IFillFormat](../../com.aspose.slides/ifillformat).
...
**Döndürür:**
[IImage](../../com.aspose.slides/iimage) - Şekil küçük resmi veya null; ShapeThumbnailBounds.Appearance kullanıldığında ve şeklin görünür öğeleri yoksa.
### getHidden() {#getHidden--}
```
...
Sunum kapsamında dahili bir tanımlayıcı döndürür; bu değer kullanıcı veya program tarafından yeniden atanabileceği için kalıcı benzersiz bir anahtar olarak kullanılmamalıdır. Salt okunur long. Ayrıca bakınız \#getOfficeInteropShapeId.getOfficeInteropShapeId.
...
Bir slayt kapsamında benzersiz tanımlayıcı döndürür; bu tanımlayıcı şeklin yaşam süresi boyunca sabit kalır ve PowerPoint ya da interop kodunun şekle belge içinde her yerden güvenilir bir şekilde başvurmasını sağlar. Salt okunur long. Ayrıca bakınız \#getUniqueId.getUniqueId.
...
Şekil gruplanmışsa üst GroupShape nesnesini döndürür. Aksi takdirde null döner. Salt okunur [IGroupShape](../../com.aspose.slides/igroupshape).
...
Temel bir yer tutucu şekli döndürür (düzenleme ve/veya ana slayttan miras alınan şekil).

--------------------

> ```
> // yer tutucu şeklinin (master/layout/slide) üzerindeki tüm animasyonlu efektleri al
>  Presentation pres = new Presentation("sample.pptx");
>  try {
...
Mevcut şekil miras alınmamışsa null döndürülür.
