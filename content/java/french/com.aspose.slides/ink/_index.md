...
Obtient la collection d'images personnalisées utilisées pour simuler les effets visuels des pinceaux d'encre. Ces images sont utilisées lors du rendu de l'encre avec des valeurs [InkEffectType](../../com.aspose.slides/inkeffecttype) spécifiques, telles que Galaxy, Rainbow, etc. En fournissant vos propres images, vous pouvez contrôler l'apparence de chaque effet d'encre.
...
Cette propriété permet de remplacer les textures d'effets d'encre par défaut par des textures définies par l'utilisateur, ce qui est particulièrement utile lorsque les ressources par défaut sont limitées par la licence ou indisponibles à l'exécution. Chaque entrée du dictionnaire doit associer une valeur [InkEffectType](../../com.aspose.slides/inkeffecttype) à un objet [IImage](../../com.aspose.slides/iimage) correspondant (par ex., Bitmap, ou une interface d'image Aspose).
