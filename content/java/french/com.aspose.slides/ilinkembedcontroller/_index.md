...
description: Interface de rappel utilisée pour déterminer comment l'objet doit être traité pendant l'enregistrement.
type: docs
...
Interface de rappel utilisée pour déterminer comment l'objet doit être traité pendant l'enregistrement.
## Méthodes
...
| --- | --- | --- |
| id | int | Identifiant de l'objet. Cet identifiant est unique pour l'opération d'enregistrement. |
| entityData | byte[] | Données binaires de l'objet. Ce paramètre peut être nul si les données binaires de l'objet n'ont pas encore été générées. |
| semanticName | java.lang.String | Court texte décrivant la signification de l'objet. Le contrôleur peut l'utiliser comme partie du nom de l'objet externe, mais il appartient au répartiteur de garantir que les noms seront uniques et ne contiendront que les caractères autorisés. |
