...
| [isFormatNotDefined()](#isFormatNotDefined--) | Renvoie true si le format de ligne n'est pas défini (comme juste créé, par défaut). |
...
| [getCapStyle()](#getCapStyle--) | Renvoie ou définit le style de capuchon de ligne. |
| [setCapStyle(byte value)](#setCapStyle-byte-) | Renvoie ou définit le style de capuchon de ligne. |
...
| [getEffective()](#getEffective--) | Obtient les données de format de ligne effectives avec l'héritage appliqué. |
### getVersion() {#getVersion--}
```
...
Renvoie true si le format de ligne n'est pas défini (comme juste créé, par défaut). Lecture seule boolean.
...
Renvoie ou définit le style de capuchon de ligne. Lecture/Écriture [LineCapStyle](../../com.aspose.slides/linecapstyle).
...
**Renvoie :**
boolean - **true** si le LineFormat spécifié est égal au LineFormat actuel ; autrement, **false**.
...
> ```
> Cet exemple montre comment obtenir les propriétés du format de ligne effectif d'une forme.
>  
