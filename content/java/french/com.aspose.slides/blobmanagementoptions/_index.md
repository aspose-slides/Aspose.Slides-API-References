...
description: Représente les options pouvant être utilisées pour gérer les règles de manipulation des BLOB et d'autres paramètres de BLOB.
type: docs
...
Représente les options qui peuvent être utilisées pour gérer les règles de manipulation des BLOB et d’autres paramètres de BLOB.
## Constructeurs
...
| [isTemporaryFilesAllowed()](#isTemporaryFilesAllowed--) | Cette propriété définit si des fichiers temporaires peuvent être créés lors de la manipulation de BLOB, ce qui réduit considérablement la consommation de mémoire mais nécessite des autorisations de création de fichiers. |
| [setTemporaryFilesAllowed(boolean value)](#setTemporaryFilesAllowed-boolean-) | Cette propriété définit si des fichiers temporaires peuvent être créés lors de la manipulation de BLOB, ce qui réduit considérablement la consommation de mémoire mais nécessite des autorisations de création de fichiers. |
...
Cette propriété définit si des fichiers temporaires peuvent être créés lors de la manipulation de BLOB, ce qui réduit considérablement la consommation de mémoire mais nécessite des autorisations de création de fichiers.
...
Le chemin racine où les fichiers temporaires seront créés. Le répertoire temporaire du système sera utilisé par défaut. Le processus hôte doit disposer des autorisations nécessaires pour créer des fichiers et des dossiers à cet emplacement.
...
Définit la taille totale maximale (en octets) que tous les BLOB peuvent occuper en mémoire. Par défaut, tous les BLOB sont chargés en mémoire ; ce n’est qu’une fois cette limite atteinte que des mécanismes alternatifs (comme les fichiers temporaires) sont employés. Garder les BLOB en mémoire maximise les performances mais peut entraîner une utilisation élevée de la mémoire. Utilisez cette propriété pour adapter le comportement à votre environnement ou à vos exigences.
