---
title: IWheelTransition
second_title: Aspose.Slides के लिए जावा API संदर्भ
description: व्हील स्लाइड ट्रांज़िशन प्रभाव।
type: docs
url: /hi/com.aspose.slides/iwheeltransition/
---
**सभी लागू इंटरफेस:**
[com.aspose.slides.ITransitionValueBase](../../com.aspose.slides/itransitionvaluebase)
```
public interface IWheelTransition extends ITransitionValueBase
```

व्हील स्लाइड ट्रांज़िशन प्रभाव।
## विधियाँ

| विधि | विवरण |
| --- | --- |
| [getSpokes()](#getSpokes--) | व्हील ट्रांज़िशन के स्पोक की संख्या। |
| [setSpokes(long value)](#setSpokes-long-) | व्हील ट्रांज़िशन के स्पोक की संख्या। |
### getSpokes() {#getSpokes--}
```
public abstract long getSpokes()
```


व्हील ट्रांज़िशन के स्पोक की संख्या। पढ़ने/लिखने वाला long।

**रिटर्न:**
long
### setSpokes(long value) {#setSpokes-long-}
```
public abstract void setSpokes(long value)
```


व्हील ट्रांज़िशन के स्पोक की संख्या। पढ़ने/लिखने वाला long।

**पैरामीटर:**
| पैरामीटर | प्रकार | विवरण |
| --- | --- | --- |
| value | long |  |