---
title: MorphTransition
second_title: Aspose.Slides के लिए Java API संदर्भ
description: रिपल स्लाइड ट्रांज़िशन प्रभाव।
type: docs
url: /hi/com.aspose.slides/morphtransition/
---
**विरासत:**
java.lang.Object, [com.aspose.slides.TransitionValueBase](../../com.aspose.slides/transitionvaluebase)

**सभी लागू इंटरफ़ेस:**
[com.aspose.slides.IMorphTransition](../../com.aspose.slides/imorphtransition)
```
public class MorphTransition extends TransitionValueBase implements IMorphTransition
```

रिपल स्लाइड ट्रांज़िशन प्रभाव.
## विधियाँ

| मेथड | विवरण |
| --- | --- |
| [getMorphType()](#getMorphType--) | मॉर्फ़ ट्रांज़िशन का प्रकार। |
| [setMorphType(int value)](#setMorphType-int-) | मॉर्फ़ ट्रांज़िशन का प्रकार। |
### getMorphType() {#getMorphType--}
```
public final int getMorphType()
```

मॉर्फ़ ट्रांज़िशन का प्रकार। पढ़ना/लिखना [TransitionMorphType](../../com.aspose.slides/transitionmorphtype)।

**रिटर्न:**
int
### setMorphType(int value) {#setMorphType-int-}
```
public final void setMorphType(int value)
```

मॉर्फ़ ट्रांज़िशन का प्रकार। पढ़ना/लिखना [TransitionMorphType](../../com.aspose.slides/transitionmorphtype)।

**पैरामीटर:**
| पैरामीटर | प्रकार | विवरण |
| --- | --- | --- |
| value | int |  |