...
description: 表示在條形餅圖或餅餅圖中使用自訂分割的分割點集合。
type: docs
...
表示在條形餅圖或餅餅圖中使用自訂分割的分割點集合。
## 方法

| Method | Description |
| --- | --- |
| [get_Item(int index)](#get-Item-int-) | 返回指定索引的圖表資料點。 |
| [add(int dataPointIndex)](#add-int-) | 依其在父系列點集合中的索引新增資料點。 |
| [addItem(IChartDataPoint dataPoint)](#addItem-com.aspose.slides.IChartDataPoint-) | 將資料點新增至集合。 |
...
| [size()](#size--) | 取得或設定圖表資料點的計數。 |
| [isReadOnly()](#isReadOnly--) | 取得一個值，表示 [IGenericCollection](../../com.aspose.slides/igenericcollection) 是否唯讀。 |
| [isSynchronized()](#isSynchronized--) | 返回一個值，表示對集合的存取是否已同步（執行緒安全）。 |
...
**返回值：**
boolean - 若成功移除項目則為 true；否則為 false。若在 System.Collections.Generic.List\{T\} 中找不到項目，亦會回傳 false。
...
| 參數 | 類型 | 說明 |
| --- | --- | --- |
| array | [IChartDataPoint\[\]](../../com.aspose.slides/ichartdatapoint) | 一維陣列，作為從 [IGenericCollection](../../com.aspose.slides/igenericcollection) 複製元素的目標。該陣列必須使用零基索引。 |
| arrayIndex | int | 陣列中開始複製的零基索引。 |
### size() {#size--}
```
...
取得一個值，表示 [IGenericCollection](../../com.aspose.slides/igenericcollection) 是否唯讀。唯讀 boolean。
...
返回一個值，表示對集合的存取是否已同步（執行緒安全）。唯讀 boolean。
