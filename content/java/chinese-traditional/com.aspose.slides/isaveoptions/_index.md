...
| [getProgressCallback()](#getProgressCallback--) | 代表一個用於保存進度更新（以百分比表示）的回呼物件。 |
| [setProgressCallback(IProgressCallback value)](#setProgressCallback-com.aspose.slides.IProgressCallback-) | 代表一個用於保存進度更新（以百分比表示）的回呼物件。 |
...
| [getSkipJavaScriptLinks()](#getSkipJavaScriptLinks--) | 指定在儲存簡報時是否跳過帶有 JavaScript 呼叫的超連結。 |
| [setSkipJavaScriptLinks(boolean value)](#setSkipJavaScriptLinks-boolean-) | 指定在儲存簡報時是否跳過帶有 JavaScript 呼叫的超連結。 |
...
代表一個用於保存進度更新（以百分比表示）的回呼物件。 請參閱 [IProgressCallback](../../com.aspose.slides/iprogresscallback)。
...
返回或設定在找不到來源字型時使用的字型。 讀/寫 String。
...
指定在儲存簡報時是否跳過帶有 JavaScript 呼叫的超連結。 讀/寫 boolean。 預設值為 false。
...
當此屬性設為 true 時，儲存過程中將忽略帶有 JavaScript 呼叫的超連結。

當此屬性設為 false 時，將會保存所有超連結。
