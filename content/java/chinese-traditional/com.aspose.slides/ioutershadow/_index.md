...
陰影方向（以度為單位）。預設值 - 0 � (left-to-right)。可讀寫 float。
...
陰影與物件之間的距離（以點為單位）。預設值 - 0 pt。可讀寫 double。
...
陰影的顏色。預設值 - automatic black (theme-dependent)。唯讀 [IColorFormat](../../com.aspose.slides/icolorformat)。
...
水平縮放比例（以原始大小的百分比表示）。負向縮放會導致翻轉。預設值 - 100%。可讀寫 double。
...
垂直縮放比例（以原始大小的百分比表示）。負向縮放會導致翻轉。預設值 - 100%。可讀寫 double。
