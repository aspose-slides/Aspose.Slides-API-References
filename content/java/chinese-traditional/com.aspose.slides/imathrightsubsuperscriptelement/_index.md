...
description: 指定 Sub-Superscript 物件，它由基底以及位於基底右側的下標和上標組成。
type: docs
...
指定 Sub-Superscript 物件，它由基底以及位於基底右側的下標和上標組成。
...
指定下標/上標的對齊方式。當為 true 時，下標與上標會水平對齊。當為 false 時，它們會以基底的形狀為基礎進行字距調整。預設值為 false。
