...
| [getRepeatUntilEndSlide()](#getRepeatUntilEndSlide--) | 此屬性指定效果是否將持續重複至投影片結束。 |
| [setRepeatUntilEndSlide(boolean value)](#setRepeatUntilEndSlide-boolean-) | 此屬性指定效果是否將持續重複至投影片結束。 |
| [getRepeatUntilNextClick()](#getRepeatUntilNextClick--) | 此屬性指定效果是否將持續重複至下一次點擊。 |
| [setRepeatUntilNextClick(boolean value)](#setRepeatUntilNextClick-boolean-) | 此屬性指定效果是否將持續重複至下一次點擊。 |
| [getRepeatDuration()](#getRepeatDuration--) | 描述效果應重複的次數。 |
| [setRepeatDuration(float value)](#setRepeatDuration-float-) | 描述效果應重複的次數。 |
| [getRestart()](#getRestart--) | 指定效果完成後是否重新開始。 |
| [setRestart(int value)](#setRestart-int-) | 指定效果完成後是否重新開始。 |
| [getRewind()](#getRewind--) | 此屬性指定效果在播放完成後是否倒回。 |
| [setRewind(boolean value)](#setRewind-boolean-) | 此屬性指定效果在播放完成後是否倒回。 |
| [getSpeed()](#getSpeed--) | 指定加速（或減速）時間的百分比。 |
| [setSpeed(float value)](#setSpeed-float-) | 指定加速（或減速）時間的百分比。 |
...
描述在正向播放後是否自動以相反方向播放動畫。讀寫 boolean。
...
此屬性指定效果是否將持續重複至投影片結束。讀寫 boolean。
...
此屬性指定效果是否將持續重複至下一次點擊。讀寫 boolean。
...
指定效果完成後是否重新開始。讀寫 [EffectRestartType](../../com.aspose.slides/effectrestarttype)。
