...
表示可用於管理 BLOB 處理規則及其他 BLOB 設定的選項。
...
| 方法 | 說明 |
| --- | --- |
| [getPresentationLockingBehavior()](#getPresentationLockingBehavior--) | 此屬性定義 Presentation 類別的執行個體在其生命週期內是否可成為來源（檔案或串流）的擁有者。 |
| [setPresentationLockingBehavior(int value)](#setPresentationLockingBehavior-int-) | 此屬性定義 Presentation 類別的執行個體在其生命週期內是否可成為來源（檔案或串流）的擁有者。 |
...
| [getTempFilesRootPath()](#getTempFilesRootPath--) | 暫存檔將建立的根目錄路徑。 |
| [setTempFilesRootPath(String value)](#setTempFilesRootPath-java.lang.String-) | 暫存檔將建立的根目錄路徑。 |
...
此屬性定義 Presentation 類別的執行個體在其生命週期內是否可成為來源（檔案或串流）的擁有者。若執行個體是擁有者，則會鎖定來源。這有助於在使用 BLOB 時改善記憶體使用量與效能，但在 Presentation 的執行個體生命週期內，來源（串流或檔案）無法變更。
...
暫存檔將建立的根目錄路徑。預設會使用系統暫存目錄。主機程序必須具有在該目錄建立檔案與資料夾的權限。
...
定義所有 BLOB 在記憶體中可能佔用的最大總大小（以位元組為單位）。預設情況下，所有 BLOB 會被載入到記憶體；僅在達到此上限時，才會採用替代機制（例如暫存檔）。將 BLOB 保持在記憶體中可最大化效能，但可能導致高記憶體使用量。請使用此屬性依據您的環境或需求調整行為。

--------------------

若 \#isTemporaryFilesAllowed.isTemporaryFilesAllowed/\#setTemporaryFilesAllowed(boolean).setTemporaryFilesAllowed(boolean) 設定為 false，則此屬性會被忽略，因為此時記憶體是唯一可用的儲存位置，限制記憶體內 BLOB 的使用將不會產生作用。
