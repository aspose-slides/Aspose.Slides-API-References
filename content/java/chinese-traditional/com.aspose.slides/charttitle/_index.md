...
| 方法 | 描述 |
| --- | --- |
| [getX()](#getX--) | 返回或設定標題的 x 座標，作為圖表寬度的比例。 |
| [setX(float value)](#setX-float-) | 返回或設定標題的 x 座標，作為圖表寬度的比例。 |
| [getY()](#getY--) | 返回或設定標題的 y 座標，作為圖表高度的比例。 |
| [setY(float value)](#setY-float-) | 返回或設定標題的 y 座標，作為圖表高度的比例。 |
| [getWidth()](#getWidth--) | 返回或設定標題的寬度，作為圖表寬度的比例。 |
| [setWidth(float value)](#setWidth-float-) | 返回或設定標題的寬度，作為圖表寬度的比例。 |
| [getHeight()](#getHeight--) | 返回或設定標題的高度，作為圖表高度的比例。 |
| [setHeight(float value)](#setHeight-float-) | 返回或設定標題的高度，作為圖表高度的比例。 |
| [getRight()](#getRight--) | 右側。 |
| [getBottom()](#getBottom--) | 底部。 |
| [getOverlay()](#getOverlay--) | 判斷是否允許其他圖表元素重疊標題。 |
| [setOverlay(boolean value)](#setOverlay-boolean-) | 判斷是否允許其他圖表元素重疊標題。 |
...
返回或設定標題的 x 座標，作為圖表寬度的比例。可讀寫 float。
...
返回或設定標題的 y 座標，作為圖表高度的比例。可讀寫 float。
...
返回或設定標題的寬度，作為圖表寬度的比例。可讀寫 float。
...
返回或設定標題的高度，作為圖表高度的比例。可讀寫 float。
...
可以包含富格式文字。如果此屬性不為 null，則此格式化文字值會覆蓋自動產生的文字。自動產生的文字是資料標籤、值軸的顯示單位標籤、軸標題、圖表標題、趨勢線標籤的隱含屬性。自動產生的文字以 IFormattedTextContainer.TextFormat 屬性格式化。唯讀 [ITextFrame](../../com.aspose.slides/itextframe)。
...
指定圖表元素相對於圖表左上角的實際 x 位置（左）。在取得實際值之前，請先呼叫方法 IChart.validateChartLayout()。唯讀 float。
...
指定圖表元素相對於圖表左上角的實際上緣位置。在取得實際值之前，請先呼叫方法 IChart.validateChartLayout()。唯讀 float。
...
指定圖表元素的實際寬度。在取得實際值之前，請先呼叫方法 IChart.validateChartLayout()。唯讀 float。
...
指定圖表元素的實際高度。在取得實際值之前，請先呼叫方法 IChart.validateChartLayout()。唯讀 float。
