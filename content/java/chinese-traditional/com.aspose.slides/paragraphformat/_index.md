...
此類別用於返回和操作針對特定段落定義的段落格式屬性。這意味著在取得值時不套用繼承，因此在大多數情況下您會得到表示「未定義」的值。

若要取得包括繼承在內的有效格式參數值，您需要使用 [getEffective](../../com.aspose.slides/paragraphformat\#getEffective) 方法，該方法返回一個 [IParagraphFormatEffectiveData](../../com.aspose.slides/iparagraphformateffectivedata) 實例。
## 建構函式
...
| [getAlignment()](#getAlignment--) | 返回或設定段落中未套用繼承的文字對齊方式。 |
| [setAlignment(int value)](#setAlignment-int-) |返回或設定段落中未套用繼承的文字對齊方式。 |
| [getSpaceWithin()](#getSpaceWithin--) |返回或設定段落基線之間的間距。 |
| [setSpaceWithin(float value)](#setSpaceWithin-float-) |返回或設定段落基線之間的間距。 |
| [getSpaceBefore()](#getSpaceBefore--) |返回或設定段落中未套用繼承的首行之前的間距。 |
| [setSpaceBefore(float value)](#setSpaceBefore-float-) |返回或設定段落中未套用繼承的首行之前的間距。 |
| [getSpaceAfter()](#getSpaceAfter--) |返回或設定段落中未套用繼承的最後一行之後的間距。 |
| [setSpaceAfter(float value)](#setSpaceAfter-float-) |返回或設定段落中未套用繼承的最後一行之後的間距。 |
...
| [getRightToLeft()](#getRightToLeft--) |判斷段落是否使用由右至左書寫。 |
| [setRightToLeft(byte value)](#setRightToLeft-byte-) |判斷段落是否使用由右至左書寫。 |
...
| [getMarginLeft()](#getMarginLeft--) |返回或設定段落中未套用繼承的左邊距。 |
| [setMarginLeft(float value)](#setMarginLeft-float-) |返回或設定段落中未套用繼承的左邊距。 |
| [getMarginRight()](#getMarginRight--) |返回或設定段落中未套用繼承的右邊距。 |
| [setMarginRight(float value)](#setMarginRight-float-) |返回或設定段落中未套用繼承的右邊距。 |
| [getIndent()](#getIndent--) |返回或設定段落中未套用繼承的首行縮排/懸掛縮排。 |
| [setIndent(float value)](#setIndent-float-) |返回或設定段落中未套用繼承的首行縮排/懸掛縮排。 |
| [getDefaultTabSize()](#getDefaultTabSize--) |返回或設定段落中未套用繼承的預設製表大小。 |
| [setDefaultTabSize(float value)](#setDefaultTabSize-float-) |返回或設定段落中未套用繼承的預設製表大小。 |
| [getTabs()](#getTabs--) |返回段落的製表設定。 |
| [getFontAlignment()](#getFontAlignment--) |返回或設定段落中未套用繼承的字體對齊方式。 |
| [setFontAlignment(int value)](#setFontAlignment-int-) |返回或設定段落中未套用繼承的字體對齊方式。 |
| [getDefaultPortionFormat()](#getDefaultPortionFormat--) |返回段落的預設部分格式。 |
...
返回或設定段落的深度。值 0 表示未定義的值。讀寫  short .
...
返回或設定段落中未套用繼承的文字對齊方式。讀寫 [TextAlignment](../../com.aspose.slides/textalignment)。
...
>      // 取得投影片中的第一與第二個佔位符，並將其類型轉換為 AutoShape
...
返回或設定段落基線之間的間距。正值表示百分比，負值表示點數大小。未套用繼承。讀寫  float .
...
返回或設定段落中未套用繼承的首行之前的間距。正值指定白色空間應為字型大小的百分比。負值指定白色空間的點數大小。讀寫  float .
...
返回或設定段落中未套用繼承的最後一行之後的間距。正值指定白色空間應為字型大小的百分比。負值指定白色空間的點數大小。讀寫  float .
...
判斷段落是否使用東亞換行。未套用繼承。讀寫 [NullableBool](../../com.aspose.slides/nullablebool)。
...
判斷段落是否使用由右至左書寫。未套用繼承。讀寫 [NullableBool](../../com.aspose.slides/nullablebool)。
...
判斷段落是否使用拉丁換行。未套用繼承。讀寫 [NullableBool](../../com.aspose.slides/nullablebool)。
...
判斷段落是否使用懸掛標點。未套用繼承。讀寫 [NullableBool](../../com.aspose.slides/nullablebool)。
...
返回或設定段落中未套用繼承的首行縮排/懸掛縮排。懸掛縮排可用負值定義。讀寫  float .
...
返回段落的製表設定。未套用繼承。唯讀 [ITabCollection](../../com.aspose.slides/itabcollection)。
...
返回或設定段落中未套用繼承的字體對齊方式。讀寫 [FontAlignment](../../com.aspose.slides/fontalignment)。
...
返回段落的預設部分格式。未套用繼承。唯讀 [IPortionFormat](../../com.aspose.slides/iportionformat)。
