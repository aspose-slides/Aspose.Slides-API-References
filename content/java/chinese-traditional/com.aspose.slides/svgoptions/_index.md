...
| [SVGOptions(ILinkEmbedController linkEmbedController)](#SVGOptions-com.aspose.slides.ILinkEmbedController-) | 初始化 SVGOptions 類別的新執行個體，指定 link embedding controller 物件。 |
...
| [getUseFrameRotation()](#getUseFrameRotation--) | 判斷在渲染時是否執行形狀的指定旋轉。 |
| [setUseFrameRotation(boolean value)](#setUseFrameRotation-boolean-) | 判斷在渲染時是否執行形狀的指定旋轉。 |
| [getVectorizeText()](#getVectorizeText--) | 判斷投影片上的文字是否儲存為圖形。 |
| [setVectorizeText(boolean value)](#setVectorizeText-boolean-) | 判斷投影片上的文字是否儲存為圖形。 |
...
| [getDisable3DText()](#getDisable3DText--) | 判斷 SVG 中是否停用 3D 文字。 |
| [setDisable3DText(boolean value)](#setDisable3DText-boolean-) | 判斷 SVG 中是否停用 3D 文字。 |
...
| [getSimple()](#getSimple--) | 取得最簡單且最小的 SVG 檔案產生設定。 |
| [getWYSIWYG()](#getWYSIWYG--) | 取得最精確的 SVG 檔案產生設定。 |
| [getJpegQuality()](#getJpegQuality--) | 判斷 JPEG 編碼品質。 |
| [setJpegQuality(int value)](#setJpegQuality-int-) | 判斷 JPEG 編碼品質。 |
| [getShapeFormattingController()](#getShapeFormattingController--) | 取得並設定回呼介面，以允許使用者控制 shape 轉換。 |
| [setShapeFormattingController(ISvgShapeFormattingController value)](#setShapeFormattingController-com.aspose.slides.ISvgShapeFormattingController-) | 取得並設定回呼介面，以允許使用者控制 shape 轉換。 |
...
| [getDeletePicturesCroppedAreas()](#getDeletePicturesCroppedAreas--) | 布林旗標，指示裁切的部分是否保留在文件中。 |
| [setDeletePicturesCroppedAreas(boolean value)](#setDeletePicturesCroppedAreas-boolean-) | 布林旗標，指示裁切的部分是否保留在文件中。 |
...
| [getDisableFontLigatures()](#getDisableFontLigatures--) | 取得或設定一個值，指出文字是否在渲染時不使用連字。 |
| [setDisableFontLigatures(boolean value)](#setDisableFontLigatures-boolean-) | 取得或設定一個值，指出文字是否在渲染時不使用連字。 |
...
初始化 SVGOptions 類別的新執行個體，指定 link embedding controller 物件。
...
link embedding controller 是一個委派物件，負責決定資源（例如影像）是否需要嵌入或以外部資源參考。 |
...
判斷在渲染時是否執行形狀的指定旋轉。讀寫 boolean。預設值為 true。
...
SVG 1.1 無法定義標記的內縮。Aspose.Slides SVG 寫入引擎針對此問題提供了變通方法：它會裁剪帶箭頭的線條末端，使線條不會與標記重疊。此選項可關閉此行為。讀寫 boolean。
...
取得最簡單且最小的 SVG 檔案產生設定。唯讀 [SVGOptions](../../com.aspose.slides/svgoptions)。
...
取得最精確的 SVG 檔案產生設定。唯讀 [SVGOptions](../../com.aspose.slides/svgoptions)。
...
取得並設定回呼介面，以允許使用者控制 shape 轉換。讀寫 [ISvgShapeFormattingController](../../com.aspose.slides/isvgshapeformattingcontroller)。
...
布林旗標，指示裁切的部分是否保留在文件中。若為 true，裁切的部分將被移除；若為 false，則會序列化於文件中（可能導致檔案變大）。
...
取得或設定一個值，指出文字在渲染時是否不使用連字。設定為 true 時，連字將在輸出中被停用。預設情況下，此屬性設定為 false。
