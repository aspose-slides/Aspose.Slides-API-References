...
description: 表示可在 pptx 簡報文件中以兩種方式儲存的 double 值：1）在與圖表相關的工作簿之儲存格/儲存格中；2）作為文字值。
type: docs
...
表示可以以兩種方式儲存在 pptx 簡報文件中的 double 值：1）在與圖表相關的工作簿之儲存格/儲存格中；2）作為文字值。
## 方法
...
**回傳：**
double - 如果 DataSourceType 等於 DoubleLiterals，則回傳 LiteralDouble。若 DataSourceType 等於 Worksheet，則回傳成功轉換為 double 的儲存格值，否則回傳 NaN。