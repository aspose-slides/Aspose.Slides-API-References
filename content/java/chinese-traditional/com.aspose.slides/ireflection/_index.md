...
| 方法 | 說明 |
| --- | --- |
| [getStartPosAlpha()](#getStartPosAlpha--) | 指定起始 alpha 值 (percents) 在 alpha 漸層坡道 (along the alpha gradient ramp) 上的起始位置。 |
| [setStartPosAlpha(float value)](#setStartPosAlpha-float-) | 指定起始 alpha 值 (percents) 在 alpha 漸層坡道 (along the alpha gradient ramp) 上的起始位置。 |
| [getEndPosAlpha()](#getEndPosAlpha--) | 指定結束 alpha 值 (percents) 在 alpha 漸層坡道 (along the alpha gradient ramp) 上的結束位置。 |
| [setEndPosAlpha(float value)](#setEndPosAlpha-float-) | 指定結束 alpha 值 (percents) 在 alpha 漸層坡道 (along the alpha gradient ramp) 上的結束位置。 |
...
| [getRotateShadowWithShape()](#getRotateShadowWithShape--) | 指定當形狀旋轉時，反射是否隨形狀一起旋轉。 |
| [setRotateShadowWithShape(boolean value)](#setRotateShadowWithShape-boolean-) | 指定當形狀旋轉時，反射是否隨形狀一起旋轉。 |
...
指定起始 alpha 值 (percents) 在 alpha 漸層坡道 (along the alpha gradient ramp) 上的起始位置。 可讀寫 float。
...
指定結束 alpha 值 (percents) 在 alpha 漸層坡道 (along the alpha gradient ramp) 上的結束位置。 可讀寫 float。
...
矩形對齊。 可讀寫 [RectangleAlignment](../../com.aspose.slides/rectanglealignment)。
...
指定當形狀旋轉時，反射是否隨形狀一起旋轉。 可讀寫 boolean。
...
指定水平縮放係數，負值縮放會導致翻轉。 (percents) 可讀寫 double。
...
指定垂直縮放係數，負值縮放會導致翻轉。 (percents) 可讀寫 double。
