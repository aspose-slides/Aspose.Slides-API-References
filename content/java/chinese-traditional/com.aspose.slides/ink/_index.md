...
| [getInkEffectImages()](#getInkEffectImages--) | 取得用於模擬墨跡筆刷視覺效果的自訂影像集合。 |
### getTraces() {#getTraces--}
```
...
取得用於模擬墨跡筆刷視覺效果的自訂影像集合。這些影像在使用特定 [InkEffectType](../../com.aspose.slides/inkeffecttype) 值（例如 Galaxy、Rainbow 等）渲染墨跡時會被使用。透過提供您自己的影像，您可以控制每種墨跡效果的呈現方式。
...
此屬性允許以使用者自訂的材質取代預設的墨跡效果紋理，這在預設資產因授權受限或執行時無法取得時特別有用。字典中的每個條目必須將 [InkEffectType](../../com.aspose.slides/inkeffecttype) 值與相應的 [IImage](../../com.aspose.slides/iimage) 物件（例如 Bitmap 或 Aspose 影像介面）關聯起來。
