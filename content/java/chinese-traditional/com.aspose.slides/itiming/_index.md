...
| [getAutoReverse()](#getAutoReverse--) | 描述在向前播放後是否自動反向播放動畫。 |
| [setAutoReverse(boolean value)](#setAutoReverse-boolean-) | 描述在向前播放後是否自動反向播放動畫。 |
...
| [getRepeatUntilEndSlide()](#getRepeatUntilEndSlide--) | 此屬性指定效果是否會重複直到投影片結束。 |
| [setRepeatUntilEndSlide(boolean value)](#setRepeatUntilEndSlide-boolean-) | 此屬性指定效果是否會重複直到投影片結束。 |
| [getRepeatUntilNextClick()](#getRepeatUntilNextClick--) | 此屬性指定效果是否會重複直到下一次點擊。 |
| [setRepeatUntilNextClick(boolean value)](#setRepeatUntilNextClick-boolean-) | 此屬性指定效果是否會重複直到下一次點擊。 |
...
| [getSpeed()](#getSpeed--) | 指定時間加速（或減速）的百分比。 |
| [setSpeed(float value)](#setSpeed-float-) | 指定時間加速（或減速）的百分比。 |
...
| [getRewind()](#getRewind--) | 此屬性指定在播放完成後是否會倒帶此效果。 |
| [setRewind(boolean value)](#setRewind-boolean-) | 此屬性指定在播放完成後是否會倒帶此效果。 |
...
此屬性指定效果是否會重複直到投影片結束。 讀寫 boolean。
...
>      // 將效果的 Timing/Repeat 改為 "Until End of Slide"
...
此屬性指定效果是否會重複直到下一次點擊。 讀寫 boolean。
...
>      // 取得主序列的第一個效果。
>      IEffect effect = effectsSequence.get_Item(0);
>      // 將效果的 Timing/Repeat 改為 "Until Next Click"
...
指定效果完成後是否重新啟動。 讀寫 [EffectRestartType](../../com.aspose.slides/effectrestarttype)。
...
此屬性指定在播放完成後是否會倒帶此效果。 讀寫 boolean。
