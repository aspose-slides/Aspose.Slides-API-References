...
| 方法 | 說明 |
| --- | --- |
| [getRow()](#getRow--) | 傳回儲存格所在工作表行的索引。 |
| [getColumn()](#getColumn--) | 傳回儲存格所在工作表列的索引。 |
...
取得或設定數字與日期的自訂顯示格式。若值為空則使用 PresetNumberFormat。讀寫 String。
...
取得或設定內建的數字與日期顯示格式。預設編號必須於 [0..22] 或 [37..49] 之間。讀寫 byte。
...
| 參數 | 類型 | 說明 |
| --- | --- | --- |
| updateValues | boolean | 若為 false，將不執行實際計算。使用 true 可檢查可能的例外情況。 |