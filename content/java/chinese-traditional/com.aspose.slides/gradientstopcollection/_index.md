...
| [add(float position, Color color)](#add-float-java.awt.Color-) | 建立新的漸層停止點，並將其添加到集合的末端。 |
| [addPresetColor(float position, int presetColor)](#addPresetColor-float-int-) | 建立新的漸層停止點，並將其添加到集合的末端。 |
| [addSchemeColor(float position, int schemeColor)](#addSchemeColor-float-int-) | 建立新的漸層停止點，並將其添加到集合的末端。 |
| [insert(int index, float position, Color color)](#insert-int-float-java.awt.Color-) | 建立新的漸層停止點，並將其插入到集合中指定的索引位置。 |
| [insertPresetColor(int index, float position, int presetColor)](#insertPresetColor-int-float-int-) | 建立新的漸層停止點，並將其插入到集合中指定的索引位置。 |
| [insertSchemeColor(int index, float position, int schemeColor)](#insertSchemeColor-int-float-int-) | 建立新的漸層停止點，並將其插入到集合中指定的索引位置。 |
| [removeAt(int index)](#removeAt-int-) | 移除指定索引處的漸層停止點。 |
| [clear()](#clear--) | 移除集合中所有漸層停止點。 |
...
建立新的漸層停止點，並將其插入到集合中指定的索引位置。

**參數:**
| 參數 | 類型 | 說明 |
| --- | --- | --- |
| index | int | 新漸層停止點將被插入的集合索引。 |
| position | float | 新漸層停止點的位置。 |
| color | java.awt.Color | 新漸層停止點的顏色。 |
...
**回傳:**  
com.aspose.ms.System.Collections.Generic.IGenericEnumerator<com.aspose.slides.IGradientStop> - 可用於遍歷集合的 IGenericEnumerator。
...
**回傳:**  
com.aspose.ms.System.Collections.Generic.IGenericEnumerator<com.aspose.slides.IGradientStop> - 用於整個集合的 java.util.Iterator。
