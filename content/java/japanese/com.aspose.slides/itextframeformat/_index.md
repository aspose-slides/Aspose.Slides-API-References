...
| [getColumnSpacing()](#getColumnSpacing--) | テキスト領域のテキスト列間のスペース（ポイント）を取得または設定します。 |
| [setColumnSpacing(double value)](#setColumnSpacing-double-) | テキスト領域のテキスト列間のスペース（ポイント）を取得または設定します。 |
...
テキストの向きを決定します。RotationAngle プロパティのカスタム角度と本プロパティの合計による視覚的テキスト回転が結果として得られます。読み書き [TextVerticalType](../../com.aspose.slides/textverticaltype)。
...
テキスト領域の列数を取得または設定します。この値は正の数である必要があります。そうでない場合、0 に設定されます。値 0 は未定義を意味します。読み書き int。
...
テキスト領域のテキスト列間のスペース（ポイント）を取得または設定します。1 列以上の場合にのみ適用されます。この値は正の数である必要があります。そうでない場合、0 に設定されます。読み書き double。
...
バウンディングボックス内のテキストに適用されるカスタム回転を指定します。指定されていない場合は、添付シェイプの回転が使用されます。指定された場合、シェイプとは独立して適用されます。すなわち、シェイプに回転が適用されている上で、テキスト自体にも回転が適用されます。RotationAngle プロパティのカスタム角度と TextVerticalType プロパティの事前定義垂直タイプの合計による視覚的テキスト回転が結果として得られます。読み書き float。

--------------------

> ```
> シェイプに時計回りに90度の回転が適用されたケースを考えてみます。 
>  これに加えて、テキスト本体自体に反時計回りに-90度の回転が適用されます。 
>  反時計回りに適用されます。その結果、シェイプは回転しているように見えますが 
>  回転しているように見えますが、内部のテキストはまるで回転していないかのように見えます。 
> ```
...
> ```
> シェイプに時計回りに90度の回転が適用されたケースを考えてみます。 
>  これに加えて、テキスト本体自体に-90度の回転が 
>  反時計回りに適用されます。その結果、シェイプは 
>  回転しているように見えますが、内部のテキストはまるで回転していないかのように見えます。 
> ```
