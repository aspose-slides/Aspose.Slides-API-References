...
description: オブジェクトがプロパティ値の継承対象となり得る基本的なサービスインフラストラクチャをカプセル化します。
type: docs
...
オブジェクトがプロパティ値の継承の対象となることができる、基本的なサービスインフラストラクチャをカプセル化します。
...
Parent_Immediateオブジェクトを返します。読み取り専用のIDOMObject。
...
parent IPresentationComponent を返します。読み取り専用の[IPresentationComponent](../../com.aspose.slides/ipresentationcomponent)。
...
基本スライドを返します。読み取り専用の[IBaseSlide](../../com.aspose.slides/ibaseslide)。
...
プレゼンテーションを返します。読み取り専用の[IPresentation](../../com.aspose.slides/ipresentation)。
