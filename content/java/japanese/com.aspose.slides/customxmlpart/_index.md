...
| [getItemId()](#getItemId--) | Office Open XML ドキュメント内の単一のカスタム XML パートを一意に識別するグローバルにユニークな識別子 (GUID) を指定します。 |
| [setItemId(UUID value)](#setItemId-java.util.UUID-) | Office Open XML ドキュメント内の単一のカスタム XML パートを一意に識別するグローバルにユニークな識別子 (GUID) を指定します。 |
