...
| [getRotateShadowWithShape()](#getRotateShadowWithShape--) | シェイプが回転した場合に、反射がシェイプとともに回転するかどうかを指定します。 |
| [setRotateShadowWithShape(boolean value)](#setRotateShadowWithShape-boolean-) | シェイプが回転した場合に、反射がシェイプとともに回転するかどうかを指定します。 |
...
開始アルファ値（パーセント）の開始位置（アルファ グラデーション ランプに沿って）を指定します。 読み書き float。
...
終了アルファ値（パーセント）の終了位置（アルファ グラデーション ランプに沿って）を指定します。 読み書き float。
...
反射のオフセット方向を指定します。(角度)。 読み書き float。
...
矩形の配置。 読み書き [RectangleAlignment](../../com.aspose.slides/rectanglealignment)。
...
シェイプが回転した場合に、反射がシェイプとともに回転するかどうかを指定します。 読み書き boolean。
...
水平スケーリング係数を指定します。負のスケーリングは反転を引き起こします。(パーセント) 読み書き double。
...
垂直スケーリング係数を指定します。負のスケーリングは反転を引き起こします。(パーセント) 読み書き double。
