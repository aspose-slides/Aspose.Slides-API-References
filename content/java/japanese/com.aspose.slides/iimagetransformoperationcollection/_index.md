...
| [removeAt(int index)](#removeAt-int-) | 指定されたインデックスでコレクションから画像エフェクトを削除します。 |
...
指定されたインデックスでコレクションから画像エフェクトを削除します。
...
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| threshold | float | アルファ Bi-Level エフェクトのしきい値。 |
...
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| amount | float | アルファをスケールするパーセンテージ量。 |
...
| パラメーター | 型 | 説明 |
| --- | --- | --- |
| threshold | float | Bi-Level エフェクトの輝度しきい値。しきい値以上の値は白に、しきい値未満の値は黒に設定されます。 |
...
| grow | boolean | ブラーの結果としてオブジェクトの境界を拡大するかどうかを指定します。true は境界が拡大されることを、false は拡大されないことを示します。 |
