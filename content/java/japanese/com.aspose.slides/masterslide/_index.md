...
| [applyExternalThemeToDependingSlides(String fname)](#applyExternalThemeToDependingSlides-java.lang.String-) | 現在のマスタースライドを基に新しいマスタースライドを作成し、外部テーマを適用し、作成されたマスタースライドをすべての依存スライドに適用します。 |
...
| [getPreserve()](#getPreserve--) | そのマスターに続くすべてのスライドが削除されたときに、対応するマスターが削除されるかどうかを判断します。 |
| [setPreserve(boolean value)](#setPreserve-boolean-) | そのマスターに続くすべてのスライドが削除されたときに、対応するマスターが削除されるかどうかを判断します。 |
...
現在のマスタースライドを基に新しいマスタースライドを作成し、外部テーマを適用し、作成されたマスタースライドをすべての依存スライドに適用します。
...
**戻り値:**
[IMasterSlide](../../com.aspose.slides/imasterslide) - 新しいテーマ付き MasterSlide。
...
そのマスターに続くすべてのスライドが削除されたときに、対応するマスターが削除されるかどうかを判断します。注: Aspose.Slides は未使用のマスターを自動的に削除しません。未使用のマスターを実際に削除するには [MasterSlideCollection.removeUnused(boolean)](../../com.aspose.slides/masterslidecollection\#removeUnused-boolean-) を呼び出してください。読み書き  boolean .
...
マスタースライドの名前を取得または設定します。読み書き String。
...
マスタースライド上のシェイプをスライドに表示するかどうかを指定します。マスタースライド自体については、このプロパティは常に false を返します。読み書き  boolean .
