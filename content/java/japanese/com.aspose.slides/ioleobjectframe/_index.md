...
| メソッド | 説明 |
| --- | --- |
| [getSubstitutePictureFormat()](#getSubstitutePictureFormat--) | OleObject image fill properties object を返します。 |
...
| [getUpdateAutomatic()](#getUpdateAutomatic--) | プレゼンテーションが開かれたり印刷されたときに、リンクされた埋め込みオブジェクトが自動的に更新されるかを判断します。 |
| [setUpdateAutomatic(boolean value)](#setUpdateAutomatic-boolean-) | プレゼンテーションが開かれたり印刷されたときに、リンクされた埋め込みオブジェクトが自動的に更新されるかを判断します。 |
...
OleObject image fill properties object を返します。読み取り専用 [IPictureFillFormat](../../com.aspose.slides/ipicturefillformat)。
...
オブジェクトの名前を取得または設定します。読み取り/書き込み String。
...
---
> ```
> 既存の [IOleObjectFrame](../../com.aspose.slides/ioleobjectframe) オブジェクトの OLE 埋め込みデータとそのタイプを変更する例を示します
>  そして、そのタイプも変更します
>  
...
---
このメソッドはオブジェクトのプロパティを新しいデータに合わせて変更し、IsObjectLink フラグを false に設定して、OLE オブジェクトが埋め込みであることを示します。 |
...
リンクされたファイルへのフルパスを返します。長いファイル名が使用されます。読み取り/書き込み String。
...
---
Ppt プレゼンテーションでは、いくつかの Ole オブジェクトリンクが相対的に表現されている場合があります。
...
プレゼンテーションが開かれたり印刷されたときに、リンクされた埋め込みオブジェクトが自動的に更新されるかを判断します。読み取り/書き込み boolean。
...
OleObject アイコンのタイトルを取得または設定します。読み取り/書き込み String。

---
IsObjectIcon が false の場合、この値は無視されます。文字列は OLE アイコンのサイズに応じて切り詰められる可能性があります。
