...
| [size()](#size--) | コレクション内のセル数を返します。 |
| [get_Item(int index)](#get-Item-int-) | 位置にあるセルを返します。 |
...
| [getSyncRoot()](#getSyncRoot--) | 同期のルートオブジェクトを返します。 |
### getParent_Immediate() {#getParent-Immediate--}
```
...
位置にあるセルを返します。読み取り専用 [Cell](../../com.aspose.slides/cell)。

--------------------

セルが結合されている場合、1 つの Cell オブジェクトが複数のインデックスに対して返されることがあります。
...
同期のルートオブジェクトを返します。読み取り専用 Object。
