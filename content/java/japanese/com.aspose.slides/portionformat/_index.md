...
description: このクラスはテキスト部分の書式設定プロパティを含んでいます。
type: docs
...
このクラスはテキスト部分の書式設定プロパティを含んでいます。[IPortionFormatEffectiveData](../../com.aspose.slides/iportionformateffectivedata)とは異なり、このクラスのすべてのプロパティは書き込み可能です。
...
>      // Aspose.Slides はこれらの特殊識別子を使用します（PowerPoint で使用されるものに類似）:
...
このクラスは、特定の部分に定義されたテキスト部分の書式設定プロパティを取得および操作するために使用されます。これは、値を取得する際に継承が適用されないことを意味し、ほとんどの場合「undefined」の値が返されます。

継承を含む有効な書式設定パラメータ値を取得するには、[getEffective](../../com.aspose.slides/portionformat\#getEffective) メソッドを使用する必要があり、これは [IPortionFormatEffectiveData](../../com.aspose.slides/iportionformateffectivedata) インスタンスを返します。
...
| [getSmartTagClean()](#getSmartTagClean--) | スマートタグをクリーンアップすべきかどうかを決定します。 |
| [setSmartTagClean(boolean value)](#setSmartTagClean-boolean-) | スマートタグをクリーンアップすべきかどうかを決定します。 |
...
スマートタグをクリーンアップすべきかどうかを決定します。継承は適用されません。読み書き可能な boolean。
