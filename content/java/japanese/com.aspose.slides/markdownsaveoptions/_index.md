...
| [getBasePath()](#getBasePath--) | リソースを含むドキュメントが保存されるベースパスを指定します。 |
| [setBasePath(String value)](#setBasePath-java.lang.String-) | リソースを含むドキュメントが保存されるベースパスを指定します。 |
...
| [getNewLineType()](#getNewLineType--) | 生成されたドキュメントの改行が \\\\r（Macintosh）か \\\\n（Unix）か \\\\r\\\\n（Windows）かを指定します。 |
| [setNewLineType(int value)](#setNewLineType-int-) | 生成されたドキュメントの改行が \\\\r（Macintosh）か \\\\n（Unix）か \\\\r\\\\n（Windows）かを指定します。 |
...
| [getSlideNumberFormat()](#getSlideNumberFormat--) | Markdown 出力のスライド番号ヘッダーに使用される書式文字列を取得または設定します。 |
| [setSlideNumberFormat(String value)](#setSlideNumberFormat-java.lang.String-) | Markdown 出力のスライド番号ヘッダーに使用される書式文字列を取得または設定します。 |
| [getHandleRepeatedSpaces()](#getHandleRepeatedSpaces--) | Markdown エクスポート時に繰り返される通常のスペース文字の処理方法を指定します。 |
| [setHandleRepeatedSpaces(int value)](#setHandleRepeatedSpaces-int-) | Markdown エクスポート時に繰り返される通常のスペース文字の処理方法を指定します。 |
| [getRemoveEmptyLines()](#getRemoveEmptyLines--) | true に設定すると、最終的な Markdown 出力から空行または空白のみの行が削除されます。 |
| [setRemoveEmptyLines(boolean value)](#setRemoveEmptyLines-boolean-) | true に設定すると、最終的な Markdown 出力から空行または空白のみの行が削除されます。 |
| [setImageSaving(MarkdownSaveOptions.MarkdownImageSavingHandler event)](#setImageSaving-com.aspose.slides.MarkdownSaveOptions.MarkdownImageSavingHandler-) | Markdown エクスポート中の SVG 以外の画像（ビットマップまたはメタファイル）ごとに発生します。 |
| [setSvgImageSaving(MarkdownSaveOptions.MarkdownSvgImageSavingHandler event)](#setSvgImageSaving-com.aspose.slides.MarkdownSaveOptions.MarkdownSvgImageSavingHandler-) | Markdown エクスポート中の SVG 画像ごとに発生します。 |
...
リソースを含むドキュメントが保存されるベースパスを指定します。デフォルトはアプリケーションの現在のディレクトリです。
...
生成されたドキュメントの改行が \\\\r（Macintosh）か \\\\n（Unix）か \\\\r\\\\n（Windows）かを指定します。デフォルトは Unix です。
...
Markdown 出力のスライド番号ヘッダーに使用される書式文字列を取得または設定します。書式には「\{0\}」プレースホルダーを含める必要があり、エクスポート時にスライドインデックスで置換されます。例: 「\# Slide \{0\}」は「\# Slide 1」「\# Slide 2」などを生成します。
...
Markdown エクスポート時に繰り返される通常のスペース文字の処理方法を指定します。このプロパティは連続スペースを以下のいずれかにするかを定義します: - 通常のスペース文字として保持, - 通常のスペースとノーブレークスペース エンティティ（�）を交互に, - 最初のスペース以外をすべてノーブレークスペースに置換して Markdown 出力の視覚的整列を保持。デフォルト値は [HandleRepeatedSpaces.AlternateSpacesToNbsp](../../com.aspose.slides/handlerepeatedspaces\#AlternateSpacesToNbsp) 。
...
true に設定すると、最終的な Markdown 出力から空行または空白のみの行が削除されます。デフォルトは false です。
...
Markdown エクスポート中の SVG 以外の画像（ビットマップまたはメタファイル）ごとに発生します。画像の保存方法や参照方法をカスタマイズできます。ハンドリングしない場合、画像はローカルに相対リンクで保存されます。
...
Markdown エクスポート中の SVG 画像ごとに発生します。デフォルトの保存方法とリンク生成を上書きできます。ハンドリングしない場合、SVG はローカルに相対リンクで保存されます。
