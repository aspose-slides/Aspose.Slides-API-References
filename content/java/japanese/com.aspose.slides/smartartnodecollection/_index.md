...
| [size()](#size--) | コレクション内のノード数を返します Read-only  int  Read-only  int . |
| [addNode()](#addNode--) | 新しいスマートアート ノードまたはサブノードを追加します。 |
...
コレクション内のノード数を返します Read-only  int  Read-only  int .
