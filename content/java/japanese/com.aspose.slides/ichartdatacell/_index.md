...
type: docs
url: /ja/com.aspose.slides/ichartdatacell/
---```
...
| メソッド | 説明 |
| --- | --- |
| [getRow()](#getRow--) | セルが配置されている worksheet の行インデックスを返します。 |
| [getColumn()](#getColumn--) | セルが配置されている worksheet の列インデックスを返します。 |
| [getValue()](#getValue--) | cell の値を取得または設定します。 |
| [setValue(Object value)](#setValue-java.lang.Object-) | cell の値を取得または設定します。 |
...
| [getChartDataWorksheet()](#getChartDataWorksheet--) | worksheet を取得します。 |
| [isHidden()](#isHidden--) | cell が非表示かどうかを判定します。 |
...
| [getPresetNumberFormat()](#getPresetNumberFormat--) | 組み込みの表示形式を取得または設定します。 |
| [setPresetNumberFormat(byte value)](#setPresetNumberFormat-byte-) | 組み込みの表示形式を取得または設定します。 |
| [calculate(boolean updateValues)](#calculate-boolean-) | cell に数式が含まれている場合、値はその数式に基づいて更新されます。 |
...
セルが配置されている worksheet の行インデックスを返します。 読み取り専用 int.
...
セルが配置されている worksheet の列インデックスを返します。 読み取り専用 int.
...
cell の値を取得または設定します。 読み書き可能 Object.
...
worksheet を取得します。 読み取り専用 [IChartDataWorksheet](../../com.aspose.slides/ichartdataworksheet)。
...
cell が非表示かどうかを判定します。 読み取り専用 boolean。
...
数値と日付のカスタム表示形式を取得または設定します。 値が空の場合は PresetNumberFormat の値が使用されます。 読み書き可能 String。
...
組み込みの表示形式を取得または設定します。 プリセット番号は [0..22] または [37..49] の範囲である必要があります。 読み書き可能 byte。
...
cell に数式が含まれている場合、値はその数式に基づいて更新されます。

**パラメータ:**
| パラメータ | 型 | 説明 |
| --- | --- | --- |
| updateValues | boolean | false の場合、実際の計算は行われません。例外チェックを行う場合は true を使用してください。 |