...
| [getVerticalJustification()](#getVerticalJustification--) | グループ文字の垂直配置。 |
| [setVerticalJustification(int value)](#setVerticalJustification-int-) | グループ文字の垂直配置。 |
### getBase() {#getBase--}
```
...
グループ文字の垂直配置。 オブジェクトのベースラインに対する配置を指定します。例えば、グループ文字がオブジェクトの上にある場合、VerticalJustification が Top のときはオブジェクトの上部がベースライン上にあり、Bottom に設定されたときはオブジェクトの下部がベースライン上にあります。 デフォルト: Position=Top の場合 Bottom、Position=Bottom の場合 Top
