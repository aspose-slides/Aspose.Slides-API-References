...
description: HTML エクスポート用にプレゼンテーションのスライド画像が保存される形式を決定します。
type: docs
...
| メソッド | 説明 |
| --- | --- |
| [svg(SVGOptions options)](#svg-com.aspose.slides.SVGOptions-) | スライドは SVG 形式に変換されます。 |
| [bitmap(float scale, int imageFormat)](#bitmap-float-int-) | スライドはラスタ画像に変換されます。 |
...
| imageFormat | int | 結果画像のフォーマット (例: PNG、JPEG)。 |
