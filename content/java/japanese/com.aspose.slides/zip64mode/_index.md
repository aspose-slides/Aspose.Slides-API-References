...
OpenXML ファイルは ZIP アーカイブであり、ファイルの非圧縮サイズ、圧縮サイズ、アーカイブ全体のサイズに対して 4 GB (2^32 バイト) の制限があり、アーカイブ内のファイル数は 65,535 (2^16-1) までに制限されています。ZIP64 形式拡張子はこれらの制限を 2^64 まで拡大します。
## フィールド

| フィールド | 説明 |
| --- | --- |
| [Never](#Never) | ZIP64 形式拡張子を使用しません。 |
