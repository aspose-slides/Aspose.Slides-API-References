...
| [getBubbleSize()](#getBubbleSize--) | チャート データ ポイントのバブルサイズを返します。 |
| [getValue()](#getValue--) | チャート データ ポイントの値を返します。 |
| [getSizeValue()](#getSizeValue--) | チャート データ ポイントのサイズ値を返します。 |
| [getColorValue()](#getColorValue--) | チャート データ ポイントの色値を返します。 |
| [getErrorBarsCustomValues()](#getErrorBarsCustomValues--) | カスタム 値タイプの場合のシリーズ エラーバー値を表します。 |
...
チャート データ ポイントのバブルサイズを返します。読み取り専用 [IDoubleChartValue](../../com.aspose.slides/idoublechartvalue)。
...
チャート データ ポイントの色値を返します。Map チャートで使用されます。読み取り専用 [IDoubleChartValue](../../com.aspose.slides/idoublechartvalue)。
...
カスタム 値タイプの場合のシリーズ エラーバー値を表します。読み取り専用 [IErrorBarsCustomValues](../../com.aspose.slides/ierrorbarscustomvalues)。
...
シリーズインデックス、データ ポイント インデックス、ParentSeriesGroup.IsColorVaried プロパティ、およびチャート スタイルに基づくデータ ポイントの自動カラーを返します。このカラーは FillType が NotDefined の場合にデフォルトで使用されます。

**戻り値:**
java.awt.Color - データ ポイントの自動カラー java.awt.Color
...
値が負の場合、データ ポイントの色を反転させることを指定します。読み書き可能な boolean。
...
データ ポイント レベルのコンテナを返します。TreeMap および Sunburst 系列に適用されます。データ ポイント レベルのインデックスは 0 から始まります。
