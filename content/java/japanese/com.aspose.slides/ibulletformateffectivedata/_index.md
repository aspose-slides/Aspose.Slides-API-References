...
description: 効果的な段落箇条書き書式プロパティを含む不変オブジェクトです。
type: docs
...
効果的な段落箇条書き書式プロパティを含む不変オブジェクトです。
...
| [getHeight()](#getHeight--) | 段落の箇条書き高さを返します。 |
| [getNumberedBulletStartWith()](#getNumberedBulletStartWith--) | 番号付き箇条書きグループで使用される最初の番号を返します。 |
...
番号付き箇条書きグループで使用される最初の番号を返します。 読み取り専用 short。
...
>  try {
>      // 最初のスライドの最初のシェイプがテキストを含むAutoShapeであると想定します...
>      // テキスト段落の箇条書き情報を出力します
