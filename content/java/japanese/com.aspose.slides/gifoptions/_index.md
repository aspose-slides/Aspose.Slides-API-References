...
>      gifOptions.setFrameSize(new Dimension(960, 720)); // 結果の GIF のサイズ
>      gifOptions.setDefaultDelay(2000); // 各スライドが次に変更されるまで表示される時間
...
| [getTransitionFps()](#getTransitionFps--) | 遷移の FPS（フレーム/秒）を取得または設定します。デフォルト値は 25 です。 |
| [setTransitionFps(int value)](#setTransitionFps-int-) | 遷移の FPS（フレーム/秒）を取得または設定します。デフォルト値は 25 です。 |
...
遷移の FPS（フレーム/秒）を取得または設定します。デフォルト値は 25 です。
