...
| [getTextFrame()](#getTextFrame--) | ノードのテキストフレームを返します。 |
...
| [getBulletFillFormat()](#getBulletFillFormat--) | ノードの箇条書き用 FillFormat オブジェクトを返します。 |
...
| [isHidden()](#isHidden--) | このノードがデータモデルで隠しノードであるかどうかを true で返します。 |
...
ノードのテキストフレームを返します。 読み取り専用 [ITextFrame](../../com.aspose.slides/itextframe)。
...
ノードをアシスタントとして取得または設定します。 読み書き可能な boolean。
...
ノードの箇条書き用 FillFormat オブジェクトを返します。 注: 一部の SmartArt レイアウトでは箇条書きが提供されないため、null を返す場合があります。 読み取り専用 [IFillFormat](../../com.aspose.slides/ifillformat)。
...
兄弟ノード間でのノードのゼロベース位置を取得または設定します。 読み書き可能  int 。
...
このノードがデータモデルで隠しノードであるかどうかを true で返します。 読み取り専用 boolean。
...
現在のノードに関連付けられた組織図レイアウトタイプを取得または設定します。 読み書き可能 [OrganizationChartLayoutType](../../com.aspose.slides/organizationchartlayouttype)。
