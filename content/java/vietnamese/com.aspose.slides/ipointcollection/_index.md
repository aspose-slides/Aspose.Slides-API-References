---
title: IPointCollection
second_title: Tham chiếu API Aspose.Slides cho Java
description: Biểu thị một tập hợp các phần.
type: docs
url: /vi/com.aspose.slides/ipointcollection/
---
**Tất cả các giao diện được triển khai:**
com.aspose.ms.System.Collections.Generic.IGenericEnumerable
```
public interface IPointCollection extends System.Collections.Generic.IGenericEnumerable<IPoint>
```

Biểu thị một tập hợp các phần.
## Phương thức

| Phương thức | Mô tả |
| --- | --- |
| [getCount()](#getCount--) | Trả về số lượng điểm trong tập hợp. |
| [get_Item(int index)](#get-Item-int-) | Trả về một điểm tại chỉ số đã chỉ định. |
### getCount() {#getCount--}
```
public abstract int getCount()
```

Trả về số lượng điểm trong tập hợp. Đọc-chỉ int.

**Trả về:**
int
### get_Item(int index) {#get-Item-int-}
```
public abstract IPoint get_Item(int index)
```

Trả về một điểm tại chỉ số đã chỉ định.

**Tham số:**
| Tham số | Kiểu | Mô tả |
| --- | --- | --- |
| index | int | Chỉ số của phần tử. |

**Trả về:**
[IPoint](../../com.aspose.slides/ipoint) - Đối tượng [IPoint](../../com.aspose.slides/ipoint).