...
| [getAdvanceOnClick()](#getAdvanceOnClick--) | Xác định liệu một cú nhấp chuột chuột có chuyển sang slide tiếp theo hay không. |
| [setAdvanceOnClick(boolean value)](#setAdvanceOnClick-boolean-) | Xác định liệu một cú nhấp chuột chuột có chuyển sang slide tiếp theo hay không. |
...
| [getAdvanceAfterTime()](#getAdvanceAfterTime--) | Xác định thời gian, tính bằng mili giây, sau đó chuyển đổi nên bắt đầu. |
| [setAdvanceAfterTime(long value)](#setAdvanceAfterTime-long-) | Xác định thời gian, tính bằng mili giây, sau đó chuyển đổi nên bắt đầu. |
...
| [getSoundName()](#getSoundName--) | Xác định tên có thể đọc được cho âm thanh của chuyển đổi. |
| [setSoundName(String value)](#setSoundName-java.lang.String-) | Xác định tên có thể đọc được cho âm thanh của chuyển đổi. |
...
Thuộc tính này xác định liệu âm thanh có lặp lại cho đến khi sự kiện âm thanh tiếp theo xảy ra trong trình chiếu. Đọc-ghi boolean.
...
Xác định liệu một cú nhấp chuột chuột có chuyển sang slide tiếp theo hay không. Nếu thuộc tính này không được chỉ định thì giả định giá trị true. Đọc-ghi boolean.
...
Xác định thời gian, tính bằng mili giây, sau đó chuyển đổi nên bắt đầu. Cài đặt này có thể được sử dụng cùng với thuộc tính advClick. Nếu thuộc tính này không được chỉ định thì giả định không có tự động chuyển tiếp. Đọc-ghi long.
...
Xác định liệu âm thanh này có phải là âm thanh tích hợp sẵn hay không. Nếu thuộc tính này được đặt thành true thì ứng dụng tạo ra sẽ được thông báo để kiểm tra thuộc tính name đã chỉ định cho âm thanh này trong danh sách âm thanh tích hợp sẵn và có thể hiển thị tên tùy chỉnh hoặc giao diện người dùng cần thiết. Đọc-ghi boolean.
...
Xác định tên có thể đọc được cho âm thanh của chuyển đổi. Thuộc tính (\#getSound.getSound/\#setSound(IAudio).setSound(IAudio)) phải được gán để lấy hoặc đặt tên âm thanh. Đọc-ghi String.
...
Tương ứng với thuộc tính p14:dur của phần tử p:transition trong sơ đồ PresentationML. Nếu không được đặt, độ dài sẽ được xác định tự động dựa trên thuộc tính \#getSpeed.getSpeed/\#setSpeed(int).setSpeed(int) và loại chuyển đổi.
