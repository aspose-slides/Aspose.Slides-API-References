---
title: MotionCommandPathType
second_title: Tham chiếu API Aspose.Slides cho Java
description: Biểu diễn các loại lệnh cho hành vi hiệu ứng chuyển động của hoạt ảnh.
type: docs
url: /vi/com.aspose.slides/motioncommandpathtype/
---
**Kế thừa:**
java.lang.Object, com.aspose.ms.System.ValueType, com.aspose.ms.System.Enum
```
public final class MotionCommandPathType extends System.Enum
```

Biểu diễn các loại lệnh cho hành vi hiệu ứng chuyển động của hoạt ảnh.
## Trường

| Trường | Mô tả |
| --- | --- |
| [MoveTo](#MoveTo) |  |
| [LineTo](#LineTo) |  |
| [CurveTo](#CurveTo) |  |
| [CloseLoop](#CloseLoop) |  |
| [End](#End) |  |
### MoveTo {#MoveTo}
```
public static final int MoveTo
```




### LineTo {#LineTo}
```
public static final int LineTo
```




### CurveTo {#CurveTo}
```
public static final int CurveTo
```




### CloseLoop {#CloseLoop}
```
public static final int CloseLoop
```




### End {#End}
```
public static final int End
```