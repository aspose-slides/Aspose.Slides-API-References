...
description: Đại diện cho một tập hợp tất cả các slide bố cục của master slide đã xác định.
type: docs
...
Đại diện cho tập hợp các slide bố cục của master slide đã định nghĩa. Kế thừa lớp LayoutSlideCollection với các phương thức để thêm/chèn/xóa/nhân bản/đổi thứ tự các slide bố cục trong ngữ cảnh của từng bộ sưu tập các slide bố cục của master.

## Phương thức

| Phương thức | Mô tả |
| --- | --- |
| [addClone(ILayoutSlide sourceLayout)](#addClone-com.aspose.slides.ILayoutSlide-) | Thêm một bản sao của slide bố cục được chỉ định vào cuối bộ sưu tập. |
| [insertClone(int index, ILayoutSlide sourceLayout)](#insertClone-int-com.aspose.slides.ILayoutSlide-) | Chèn một bản sao của slide bố cục được chỉ định vào vị trí xác định trong bộ sưu tập. |
...
| [removeAt(int index)](#removeAt-int-) | Xóa phần tử tại chỉ mục xác định trong bộ sưu tập. |
...
Bố cục mới sẽ được liên kết với master slide cha cho bộ sưu tập các slide bố cục này. Vì vậy đây là tương tự của việc sao chép/dán với tùy chọn "Use Destination Theme" trong PowerPoint.
...
| layoutName | java.lang.String | Tên cho một bố cục mới. Nếu tên đã tồn tại, sẽ ném ArgumentException. Nếu truyền null, tên sẽ được tạo tự động dựa trên kiểu bố cục đã truyền (ví dụ "Title Slide" hoặc "1_Title Slide", "2_..", v.v.). |
...
Bố cục được chèn cho giá trị SlideLayoutType.Custom của layoutType không chứa placeholder và không có hình dạng.
...
Xóa phần tử tại chỉ mục xác định trong bộ sưu tập.
...
1) Để tránh ném PptxEditException, hãy kiểm tra thuộc tính HasDependingSlides của layout trước. 2) Bạn cũng có thể dùng phương thức [ILayoutSlide.remove](../../com.aspose.slides/ilayoutslide\#remove) để đơn giản hóa mã.
...
Di chuyển slide bố cục từ bộ sưu tập tới vị trí xác định.
