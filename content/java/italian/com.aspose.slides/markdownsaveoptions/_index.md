...
description: Rappresenta le opzioni che controllano come la presentazione dovrebbe essere salvata in markdown.
type: docs
...
Rappresenta le opzioni che controllano come la presentazione dovrebbe essere salvata in markdown.
...
| [getBasePath()](#getBasePath--) | Specifica il percorso base in cui il documento con le risorse verrà salvato. |
| [setBasePath(String value)](#setBasePath-java.lang.String-) | Specifica il percorso base in cui il documento con le risorse verrà salvato. |
...
| [getNewLineType()](#getNewLineType--) | Specifica se il documento generato deve avere nuove righe \\\\r(Macintosh) di \\\\n(Unix) o \\\\r\\\\n(Windows). |
| [setNewLineType(int value)](#setNewLineType-int-) | Specifica se il documento generato deve avere nuove righe \\\\r(Macintosh) di \\\\n(Unix) o \\\\r\\\\n(Windows). |
...
| [getSlideNumberFormat()](#getSlideNumberFormat--) | Ottiene o imposta la stringa di formato usata per le intestazioni dei numeri di diapositiva nell'output Markdown. |
| [setSlideNumberFormat(String value)](#setSlideNumberFormat-java.lang.String-) | Ottiene o imposta la stringa di formato usata per le intestazioni dei numeri di diapositiva nell'output Markdown. |
| [getHandleRepeatedSpaces()](#getHandleRepeatedSpaces--) | Specifica come i caratteri di spazio regolari ripetuti devono essere gestiti durante l'esportazione Markdown. |
| [setHandleRepeatedSpaces(int value)](#setHandleRepeatedSpaces-int-) | Specifica come i caratteri di spazio regolari ripetuti devono essere gestiti durante l'esportazione Markdown. |
| [getRemoveEmptyLines()](#getRemoveEmptyLines--) | Se impostato su true, rimuove le linee vuote o contenenti solo spazi bianchi dall'output Markdown finale. |
| [setRemoveEmptyLines(boolean value)](#setRemoveEmptyLines-boolean-) | Se impostato su true, rimuove le linee vuote o contenenti solo spazi bianchi dall'output Markdown finale. |
...
Specifică la specifica markdown pentru convertire a prezentării. Valoarea implicită este  TextOnly .
...
Specifică il percorso base in cui il documento con le risorse verrà salvato. Il valore predefinito è la directory corrente dell'applicazione.
...
Specifică se il documento generato deve avere nuove righe \\\\r(Macintosh) di \\\\n(Unix) o \\\\r\\\\n(Windows). Il valore predefinito è  Unix .
...
Specifică la specifica markdown pentru convertire a prezentării. Valoarea implicită este  Multi-markdown .
...
Specifică la specifica markdown pentru convertire a prezentării. Valoarea implicită è  Multi-markdown .
...
Ottiene o imposta la stringa di formato usata per le intestazioni dei numeri di diapositiva nell'output Markdown. Il formato deve includere il "\{0\}" segnaposto, che verrà sostituito con l'indice della diapositiva durante l'esportazione. Esempio: "\# Slide \{0\}" produrrà "\# Slide 1", "\# Slide 2", ecc.
...
Specifică come i caratteri di spazio regolari ripetuti devono essere gestiti durante l'esportazione Markdown. Questa proprietà definisce se gli spazi consecutivi sono: - preservati come caratteri di spazio regolari, - alternati tra spazi regolari e entità di spazio non interrompibile (�), - o completamente sostituiti (dopo il primo) con uno spazio non interrompibile per preservare l'allineamento visivo nell'output Markdown. Il valore predefinito è [HandleRepeatedSpaces.AlternateSpacesToNbsp](../../com.aspose.slides/handlerepeatedspaces\#AlternateSpacesToNbsp).
...
Se impostato su true, rimuove le linee vuote o contenenti solo spazi bianchi dall'output Markdown finale. Il valore predefinito è false.
...
Si verifica per ogni immagine non SVG (bitmap o metafile) durante l'esportazione Markdown. Consente di personalizzare come l'immagine viene salvata e referenziata. Se non gestita, l'immagine viene salvata localmente con un collegamento relativo.
...
Si verifica per ogni immagine SVG durante l'esportazione Markdown. Consente di sovrascrivere il salvataggio predefinito e la generazione del collegamento. Se non gestita, l'SVG viene salvato localmente con un collegamento relativo.

**Parametri:**
| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| event | [MarkdownSvgImageSavingHandler](../../com.aspose.slides/markdownsvgimagesavinghandler) | Evento di salvataggio immagine SVG Markdown. |