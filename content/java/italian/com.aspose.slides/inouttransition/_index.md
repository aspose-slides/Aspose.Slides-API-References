---
title: InOutTransition
second_title: Riferimento API di Aspose.Slides per Java
description: Effetto di transizione diapositiva In-Out.
type: docs
url: /it/com.aspose.slides/inouttransition/
---
**Eredità:**  
java.lang.Object, [com.aspose.slides.TransitionValueBase](../../com.aspose.slides/transitionvaluebase)

**Tutte le interfacce implementate:**  
[com.aspose.slides.IInOutTransition](../../com.aspose.slides/iinouttransition)  
```
public class InOutTransition extends TransitionValueBase implements IInOutTransition
```

Effetto di transizione diapositiva In-Out.
## Metodi

| Metodo | Descrizione |
| --- | --- |
| [getDirection()](#getDirection--) | Direzione di un effetto di transizione. |
| [setDirection(int value)](#setDirection-int-) | Direzione di un effetto di transizione. |
### getDirection() {#getDirection--}
```
public final int getDirection()
```

Direzione di un effetto di transizione. Lettura/Scrittura [TransitionInOutDirectionType](../../com.aspose.slides/transitioninoutdirectiontype).

**Restituisce:**  
int
### setDirection(int value) {#setDirection-int-}
```
public final void setDirection(int value)
```

Direzione di un effetto di transizione. Lettura/Scrittura [TransitionInOutDirectionType](../../com.aspose.slides/transitioninoutdirectiontype).

**Parametri:**
| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| value | int |  |