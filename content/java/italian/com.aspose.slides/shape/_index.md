...
| [addPlaceholder(IPlaceholder placeholderToCopyFrom)](#addPlaceholder-com.aspose.slides.IPlaceholder-) | Aggiunge un nuovo segnaposto se non ce n'è e imposta le proprietà del segnaposto su quello specificato. |
...
| [getRawFrame()](#getRawFrame--) | Restituisce o imposta le proprietà del frame grezzo della forma. |
| [setRawFrame(IShapeFrame value)](#setRawFrame-com.aspose.slides.IShapeFrame-) | Restituisce o imposta le proprietà del frame grezzo della forma. |
...
| [getThreeDFormat()](#getThreeDFormat--) | Restituisce l'oggetto ThreeDFormat che contiene le proprietà dell'effetto 3D per una forma. |
...
| [getBlackWhiteMode()](#getBlackWhiteMode--) | La proprietà specifica come una forma verrà resa in modalità di visualizzazione in bianco e nero. |
| [setBlackWhiteMode(byte value)](#setBlackWhiteMode-byte-) | La proprietà specifica come una forma verrà resa in modalità di visualizzazione in bianco e nero. |
| [getUniqueId()](#getUniqueId--) | Restituisce un identificatore interno, limitato alla presentazione, destinato all'uso da parte di componenti aggiuntivi o altro codice. |
| [getOfficeInteropShapeId()](#getOfficeInteropShapeId--) | Restituisce un identificatore univoco limitato alla diapositiva che rimane costante per la durata della forma e consente a PowerPoint o al codice di interop di fare riferimento alla forma in modo affidabile da qualsiasi punto del documento. |
...
| [isDecorative()](#isDecorative--) | Ottiene o imposta l'opzione 'Segna come decorativa' booleano Lettura/Scrittura. |
| [setDecorative(boolean value)](#setDecorative-boolean-) | Ottiene o imposta l'opzione 'Segna come decorativa' booleano Lettura/Scrittura. |
...
Determina se la forma è TextHolder_PPT. Solo lettura booleano .
...
>      for (IShape shape : slide.getSlide().getShapes()) // Scorre la diapositiva
>      {
...
Aggiunge un nuovo segnaposto se non ce n'è e imposta le proprietà del segnaposto su quello specificato.
...
**Restituisce:**
[IPlaceholder](../../com.aspose.slides/iplaceholder) - Nuovo \#getPlaceholder.getPlaceholder.
...
> ```
> // ottieni tutti gli effetti animati (master/layout/slide) del segnaposto della forma
...
Restituisce o imposta le proprietà del frame grezzo della forma. Lettura/Scrittura [IShapeFrame](../../com.aspose.slides/ishapeframe).
...
>  //Questo codice può portare a situazioni poco chiare. Perciò sono state aggiunte restrizioni per l'uso di valori indefiniti per IShape.getFrame(). I valori di x, y, larghezza, altezza, flipH, flipV e rotationAngle devono essere definiti (non Float.NaN o NullableBool.NotDefined). Il codice di esempio sopra ora genera un'eccezione ArgumentException.
>  //Questo si applica a questi casi d'uso:
>  IShape shape = ...;
>  shape.setFrame(...); // non può essere indefinito
>  IShapeCollection shapes = ...;
>  // i parametri x, y, larghezza, altezza non possono essere Float.NaN:
>  {
...
>  }
>  //Ma le proprietà del frame IShape.RawFrame possono essere indefinite. Ha senso quando la forma è collegata a un segnaposto. In tal caso i valori indefiniti del frame della forma vengono sovrascritti dal segnaposto genitore. Se non esiste un segnaposto genitore, la forma utilizza i valori predefiniti quando valuta il frame efficace basandosi sul suo IShape.RawFrame. I valori predefiniti sono 0 e NullableBool.False per x, y, larghezza, altezza, flipH, flipV e rotationAngle. Per esempio:
>  IShape shape = ...; // shape is linked to placeholder
>  shape.setRawFrame(new ShapeFrame(Float.NaN, Float.NaN, 100, Float.NaN, NullableBool.NotDefined, NullableBool.NotDefined, 0)); // ora la forma eredita i valori x, y, altezza, flipH, flipV dal segnaposto e sovrascrive larghezza=100 e rotationAngle=0.{code}
> ```
...
>      shapes.insertVideoFrame(...);
>  }
>  //Ma le proprietà del frame IShape.RawFrame possono essere indefinite. Ha senso quando la forma è collegata a un segnaposto. In tal caso i valori indefiniti del frame della forma vengono sovrascritti dal segnaposto genitore. Se non esiste un segnaposto genitore per quella forma, la forma utilizza i valori predefiniti quando valuta il frame efficace in base al suo IShape.RawFrame. I valori predefiniti sono 0 e NullableBool.False per x, y, larghezza, altezza, flipH, flipV e rotationAngle. Per esempio:
...
Restituisce l'oggetto LineFormat che contiene le proprietà di formattazione della linea per una forma. Nota: può restituire null per alcuni tipi di forme che non hanno proprietà di linea. Solo lettura [ILineFormat](../../com.aspose.slides/ilineformat).
...
Restituisce l'oggetto ThreeDFormat che contiene le proprietà dell'effetto 3D per una forma. Nota: può restituire null per alcuni tipi di forme che non hanno proprietà 3D. Solo lettura [IThreeDFormat](../../com.aspose.slides/ithreedformat).
...
Restituisce l'oggetto EffectFormat che contiene gli effetti pixel applicati a una forma. Nota: può restituire null per alcuni tipi di forme che non hanno proprietà di effetto. Solo lettura [IEffectFormat](../../com.aspose.slides/ieffectformat).
...
Restituisce l'oggetto FillFormat che contiene le proprietà di formattazione del riempimento per una forma. Nota: può restituire null per alcuni tipi di forme che non hanno proprietà di riempimento. Solo lettura [IFillFormat](../../com.aspose.slides/ifillformat).
...
Restituisce la miniatura della forma. ShapeThumbnailBounds.Shape è il tipo di limiti di miniatura della forma usato per impostazione predefinita.
...
Determina se la forma è nascosta. Lettura/Scrittura booleano .
...
Restituisce la posizione di una forma nell'ordine Z. Shapes[0] restituisce la forma più arretrata nell'ordine Z, e Shapes[Shapes.Count - 1] restituisce la forma più avanzata nell'ordine Z. Solo lettura int .
...
```
Restituisce o imposta il numero di gradi di rotazione della forma specificata attorno all'asse z. Un valore positivo indica una rotazione in senso orario; un valore negativo indica una rotazione in senso antiorario. Lettura/scrittura float.

--------------------

Il valore restituito è sempre definito (non è Float.NaN). Il valore assegnato deve essere definito (non Float.NaN). È possibile impostare valori non definiti per le proprietà dell'istanza RawFrame.
...
Il valore restituito è sempre definito e mai Float.NaN. Il valore assegnato deve anch'esso essere definito; assegnare Float.NaN solo alle proprietà di un'istanza RawFrame.
...
La proprietà specifica come una forma verrà renderizzata in modalità bianco e nero. Lettura/scrittura [BlackWhiteMode](../../com.aspose.slides/blackwhitemode).
...
Restituisce un identificatore univoco a livello di diapositiva che rimane costante per tutta la durata della forma e permette a PowerPoint o al codice interop di fare riferimento alla forma in modo affidabile da qualsiasi punto del documento. Solo lettura long. Vedi anche \#getUniqueId.getUniqueId.
...
La proprietà \#getParentGroup.getParentGroup restituisce l'oggetto GroupShape padre se la forma è raggruppata.
...
Restituisce l'oggetto GroupShape padre se la forma è raggruppata. Altrimenti restituisce null. Solo lettura [IGroupShape](../../com.aspose.slides/igroupshape).
...
**Restituisce:**
java.awt.geom.Rectangle2D.Float - Un java.awt.geom.Rectangle2D.Float che rappresenta i limiti visivi della forma nelle coordinate della diapositiva.

--------------------

Il rettangolo restituito rappresenta i limiti allineati agli assi di tutti i contenuti prodotti dalla forma durante il rendering nello spazio delle coordinate della diapositiva. Questi limiti possono differire dai limiti del modello della forma \#getX.getX/\#setX(float).setX(float), \#getY.getY/\#setY(float).setY(float), \#getWidth.getWidth/\#setWidth(float).setWidth(float), \#getHeight.getHeight/\#setHeight(float).setHeight(float) e possono contenere coordinate negative se il contenuto renderizzato si estende oltre l'origine della diapositiva. I limiti visivi tengono conto di aspetti legati al rendering come trasformazioni (ad es. rotazione), larghezza e giunzioni del tratto, layout e overflow del testo, geometria di SmartArt e altri effetti di layout che influenzano l'aspetto finale renderizzato della forma. I limiti restituiti non sono ritagliati al rettangolo della diapositiva.
### getSlide() {#getSlide--}
```
public final IBaseSlide getSlide()
```

Restituisce la diapositiva padre di una forma. Solo lettura [IBaseSlide](../../com.aspose.slides/ibaseslide).
...
Restituisce la presentazione padre di una diapositiva. Solo lettura [IPresentation](../../com.aspose.slides/ipresentation).
