...
| [indexOf(IChartCategory value)](#indexOf-com.aspose.slides.IChartCategory-) | 搜索指定的 [ChartCategory](../../com.aspose.slides/chartcategory) 并返回其在整个 Collection 中首次出现的零基索引。 |
...
如果为 true，则工作表用于存储类别（此情况下支持多层类别）。如果为 false，则工作表不用于存储值（此情况下不支持多层类别）。可读写布尔。
...
返回使用的类别分组级别计数。对于多层类别，此值大于1。只读 int。
...
此方法添加名为 AUTO_DATA 的工作表并在其中添加所有值。如果使用 [ChartDataWorkbook](../../com.aspose.slides/chartdataworkbook) 添加或编辑单元格值，请确保不要使用此工作表。使用此方法添加的值的最大数量不得超过 16711680 |

**返回：**
[IChartCategory](../../com.aspose.slides/ichartcategory) - 已添加 [IChartCategory](../../com.aspose.slides/ichartcategory)。
...
搜索指定的 [ChartCategory](../../com.aspose.slides/chartcategory) 并返回其在整个 Collection 中首次出现的零基索引。
...
**返回：**
int - 如果找到，则返回值在整个 CollectionBase 中首次出现的零基索引；否则返回 -1。
...
**返回：**
com.aspose.ms.System.Collections.Generic.IGenericEnumerator<com.aspose.slides.IChartCategory> - 整个集合的 java.util.Iterator。
