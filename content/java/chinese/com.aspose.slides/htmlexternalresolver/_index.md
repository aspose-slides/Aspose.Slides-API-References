...
description: HTML 导入例程使用的回调对象，用于获取诸如图像等被引用的对象。
type: docs
...
HTML 导入例程使用的回调对象，用于获取诸如图像等被引用的对象。

--------------------

使用此解析器可能会导致漏洞，当客户端提供的 HTML 文件使服务器软件获取本地或网络文件时。请谨慎使用。建议根本不要指定 HtmlExternalResolver（仅会读取嵌入对象），或者创建某个子类来检查指定的 uri 是否有效。
## 构造函数
...
| 方法 | 描述 |
| --- | --- |
| [resolveUri(String baseUri, String relativeUri)](#resolveUri-java.lang.String-java.lang.String-) | 从基 URI 和相对 URI 解析出绝对 URI。 |
...
**返回值:**
java.io.InputStream - 一个 InputStream 对象；如果资源无法被流式传输，则返回 null。