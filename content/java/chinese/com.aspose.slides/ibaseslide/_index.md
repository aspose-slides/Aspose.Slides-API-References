...
| [getSlideShowTransition()](#getSlideShowTransition--) | 返回 TransitionEx 对象，其中包含指定幻灯片在幻灯片放映期间如何前进的信息。 |
...
| [joinPortionsWithSameFormatting()](#joinPortionsWithSameFormatting--) | 将所有可接受形状中所有段落的具有相同格式的文本段落合并。 |
...
返回 TransitionEx 对象，其中包含指定幻灯片在幻灯片放映期间如何前进的信息。只读 [ISlideShowTransition](../../com.aspose.slides/islideshowtransition)。
...
指定是否在幻灯片上显示母版幻灯片上的形状。对于母版幻灯片本身，此属性始终返回 false。读写 boolean。
...
确定两个 IBaseSlide 实例是否相等。返回值基于幻灯片的结构和静态内容计算。如果所有形状、样式、文本、动画和其他设置等都相等，则两个幻灯片相等。比较不考虑唯一标识符值（例如 SlideId）和动态内容（例如日期占位符中的当前日期值）。
...
**返回：**
boolean - **true** 如果指定的 IBaseSlide 等于当前 IBaseSlide；否则，**false**。