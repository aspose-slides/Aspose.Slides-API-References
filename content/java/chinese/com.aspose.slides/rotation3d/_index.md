...
| 方法 | 描述 |
| --- | --- |
| [getRotationX()](#getRotationX--) | 返回或设置围绕 X 轴的旋转角度，即 |
| [setRotationX(byte value)](#setRotationX-byte-) | 返回或设置围绕 X 轴的旋转角度，即 |
| [getRotationY()](#getRotationY--) | 返回或设置围绕 Y 轴的旋转角度，即 |
| [setRotationY(int value)](#setRotationY-int-) | 返回或设置围绕 Y 轴的旋转角度，即 |
...
返回或设置围绕 X 轴的旋转角度，即 3D 图表的 Y 方向（范围 -90 到 90 度）。该属性对应 ECMA-376 中的 21.2.2.157 rotX（X Rotation）条目，以及 PowerPoint 2007+ 中的 “Y Rotation” 选项。读/写 byte。
...
返回或设置围绕 Y 轴的旋转角度，即 3D 图表的 X 方向（范围 0 到 360 度）。该属性对应 ECMA-376 中的 21.2.2.158 rotY（Y Rotation）条目，以及 PowerPoint 2007+ 中的 “X Rotation” 选项。读/写 int。
...
返回或设置围绕 Y 轴的旋转角度，即 3D 图表的 X 方向（范围 0 到 360 度）。该属性对应 ECMA-376 中的 21.2.158 rotY（Y Rotation）条目，以及 PowerPoint 2007+ 中的 “X Rotation” 选项。读/写 int。
...
返回或设置 3D 图表的透视值（视场角），范围 0 到 240。如果 RightAngleAxes 属性为 true，则此值被忽略。读/写 byte。
...
确定图表坐标轴是否为直角，而不是透视绘制。换言之，它决定坐标轴的角度是否独立于图表的旋转或仰角。读/写 boolean。
