...
| 方法 | 描述 |
| --- | --- |
| [getStartPosAlpha()](#getStartPosAlpha--) | 指定起始 alpha 值 (percents) 在 alpha 梯度斜坡 (along the alpha gradient ramp) 上的起始位置。 |
| [setStartPosAlpha(float value)](#setStartPosAlpha-float-) | 指定起始 alpha 值 (percents) 在 alpha 梯度斜坡 (along the alpha gradient ramp) 上的起始位置。 |
| [getEndPosAlpha()](#getEndPosAlpha--) | 指定结束 alpha 值 (percents) 在 alpha 梯度斜坡 (along the alpha gradient ramp) 上的结束位置。 |
| [setEndPosAlpha(float value)](#setEndPosAlpha-float-) | 指定结束 alpha 值 (percents) 在 alpha 梯度斜坡 (along the alpha gradient ramp) 上的结束位置。 |
...
| [setRotateShadowWithShape(boolean value)](#setRotateShadowWithShape-boolean-) | 指定在形状旋转时反射是否随形状旋转。 |
...
指定起始 alpha 值 (percents) 在 alpha 梯度斜坡 (along the alpha gradient ramp) 上的起始位置。读/写 float.
...
指定结束 alpha 值 (percents) 在 alpha 梯度斜坡 (along the alpha gradient ramp) 上的结束位置。读/写 float.
...
矩形对齐。读/写 [RectangleAlignment](../../com.aspose.slides/rectanglealignment)。
...
指定水平缩放因子，负值缩放会导致翻转。(percents) 读/写 double。
...
指定垂直缩放因子，负值缩放会导致翻转。(percents) 读/写 double。
