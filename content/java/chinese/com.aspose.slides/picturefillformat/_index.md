...
| [getCropLeft()](#getCropLeft--) | 返回或设置从图片左侧裁剪掉的真实图像宽度的百分比。 |
| [setCropLeft(float value)](#setCropLeft-float-) | 返回或设置从图片左侧裁剪掉的真实图像宽度的百分比。 |
| [getCropTop()](#getCropTop--) | 返回或设置从图片顶部裁剪掉的真实图像高度的百分比。 |
| [setCropTop(float value)](#setCropTop-float-) | 返回或设置从图片顶部裁剪掉的真实图像高度的百分比。 |
| [getCropRight()](#getCropRight--) | 返回或设置从图片右侧裁剪掉的真实图像宽度的百分比。 |
| [setCropRight(float value)](#setCropRight-float-) | 返回或设置从图片右侧裁剪掉的真实图像宽度的百分比。 |
| [getCropBottom()](#getCropBottom--) | 返回或设置从图片底部裁剪掉的真实图像高度的百分比。 |
| [setCropBottom(float value)](#setCropBottom-float-) | 返回或设置从图片底部裁剪掉的真实图像高度的百分比。 |
| [deletePictureCroppedAreas()](#deletePictureCroppedAreas--) | 删除填充图片的裁剪区域。 |
| [compressImage(boolean deleteCroppedAreasOfImage, int resolution)](#compressImage-boolean-int-) | 通过根据形状大小和指定分辨率减小图像尺寸来压缩图像。 |
| [compressImage(boolean deleteCroppedAreasOfImage, float resolution)](#compressImage-boolean-float-) | 通过根据形状大小和指定分辨率减小图像尺寸来压缩图像。 |
| [getStretchOffsetLeft()](#getStretchOffsetLeft--) | 返回或设置填充矩形左边缘，以相对于形状边界框左边缘的百分比偏移定义。 |
| [setStretchOffsetLeft(float value)](#setStretchOffsetLeft-float-) | 返回或设置填充矩形左边缘，以相对于形状边界框左边缘的百分比偏移定义。 |
| [getStretchOffsetTop()](#getStretchOffsetTop--) | 返回或设置填充矩形上边缘，以相对于形状边界框上边缘的百分比偏移定义。 |
| [setStretchOffsetTop(float value)](#setStretchOffsetTop-float-) | 返回或设置填充矩形上边缘，以相对于形状边界框上边缘的百分比偏移定义。 |
| [getStretchOffsetRight()](#getStretchOffsetRight--) | 返回或设置填充矩形右边缘，以相对于形状边界框右边缘的百分比偏移定义。 |
| [setStretchOffsetRight(float value)](#setStretchOffsetRight-float-) | 返回或设置填充矩形右边缘，以相对于形状边界框右边缘的百分比偏移定义。 |
| [getStretchOffsetBottom()](#getStretchOffsetBottom--) | 返回或设置填充矩形下边缘，以相对于形状边界框下边缘的百分比偏移定义。 |
| [setStretchOffsetBottom(float value)](#setStretchOffsetBottom-float-) | 返回或设置填充矩形下边缘，以相对于形状边界框下边缘的百分比偏移定义。 |
...
| [getTileFlip()](#getTileFlip--) | 绕水平、垂直或两条轴翻转纹理平铺。 |
| [setTileFlip(int value)](#setTileFlip-int-) | 绕水平、垂直或两条轴翻转纹理平铺。 |
### getVersion() {#getVersion--}
```
...
返回或设置从图片左侧裁剪掉的真实图像宽度的百分比。读/写  float 。
...
返回或设置从图片顶部裁剪掉的真实图像高度的百分比。读/写  float 。
...
返回或设置从图片右侧裁剪掉的真实图像宽度的百分比。读/写  float 。
...
返回或设置从图片底部裁剪掉的真实图像高度的百分比。读/写  float 。
...
**返回：**
[IPPImage](../../com.aspose.slides/ippimage) - 如果不需要裁剪则返回裁剪后的图像，否则返回原始图像。
...
通过根据形状大小和指定分辨率减小图像尺寸来压缩图像。可选地，它还会删除裁剪区域。
...
> CompressImage
> ``` 方法通过设置目标分辨率并删除裁剪区域来减小演示文稿中图像的大小：
>  
>  Presentation presentation = new Presentation("demo.pptx");
>  try {
>      ISlide slide = presentation.getSlides().get_Item(0);
...
| resolution | float | 目标 DPI 分辨率。该值必须为正数，并确定图像将如何被重新调整大小。 |
...
**返回：**
boolean - 一个布尔值，指示图像是否成功压缩。如果图像已调整大小或裁剪，则返回   ，否则返回  .
