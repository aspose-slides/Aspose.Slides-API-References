...
| [getAnchorCenter()](#getAnchorCenter--) | 确定文本框是否在单元格内部居中。 |
| [setAnchorCenter(boolean value)](#setAnchorCenter-boolean-) | 确定文本框是否在单元格内部居中。 |
| [getFirstColumn()](#getFirstColumn--) | 获取单元格的第一列。 |
| [getFirstRow()](#getFirstRow--) | 获取单元格的第一行。 |
| [getColSpan()](#getColSpan--) | 返回父表格网格中当前单元格跨越的网格列数。 |
| [getRowSpan()](#getRowSpan--) | 返回合并单元格跨越的行数。 |
| [getTextFrame()](#getTextFrame--) | 返回单元格的文本框。 |
| [getTable()](#getTable--) | 返回单元格所属的父 Table 对象。 |
| [isMergedCell()](#isMergedCell--) | 如果单元格与任何已调整的单元格合并则返回 true，否则返回 false。 |
...
返回父表格网格中当前单元格跨越的网格列数。此属性允许单元格跨越其他单元格的垂直边界，从而呈现合并的外观。只读 int。
...
返回合并单元格跨越的行数。这与其他单元格的 vMerge 属性配合使用，以指定水平合并的起始单元格。只读 int。
...
如果单元格与任何已调整的单元格合并则返回 true，否则返回 false。只读 boolean。
