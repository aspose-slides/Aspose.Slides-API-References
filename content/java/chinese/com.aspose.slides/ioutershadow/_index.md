...
| [getScaleHorizontal()](#getScaleHorizontal--) | 水平缩放因子，原始尺寸的百分比。 |
| [setScaleHorizontal(double value)](#setScaleHorizontal-double-) | 水平缩放因子，原始尺寸的百分比。 |
| [getScaleVertical()](#getScaleVertical--) | 垂直缩放因子，原始尺寸的百分比。 |
| [setScaleVertical(double value)](#setScaleVertical-double-) | 垂直缩放因子，原始尺寸的百分比。 |
### getBlurRadius() {#getBlurRadius--}
```
public abstract double getBlurRadius()
```

模糊半径，单位为点。默认值 - 0 pt。可读写 double。
...
阴影方向，单位为度。默认值 - 0 °（从左到右）。可读写 float。
...
阴影的颜色。默认值 - 自动黑色（主题相关）。只读 [IColorFormat](../../com.aspose.slides/icolorformat)。
...
水平缩放因子，原始尺寸的百分比。负缩放会导致翻转。默认值 - 100%。可读写 double。
...
垂直缩放因子，原始尺寸的百分比。负缩放会导致翻转。默认值 - 100%。可读写 double。
