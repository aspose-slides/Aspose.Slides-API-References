...
此类用于返回和操作为特定段落定义的文本段落格式属性。这意味着在获取值时不应用继承，因此在大多数情况下您将得到表示“undefined”的值。

为了获取包括继承在内的有效格式参数值，您需要使用 [IPortionFormat.getEffective](../../com.aspose.slides/iportionformat\#getEffective) 方法，该方法返回一个 [IPortionFormatEffectiveData](../../com.aspose.slides/iportionformateffectivedata) 实例。
## 方法
...
| [getFontBold()](#getFontBold--) | 确定字体是否加粗。 |
| [setFontBold(byte value)](#setFontBold-byte-) | 确定字体是否加粗。 |
| [getFontItalic()](#getFontItalic--) | 确定字体是否斜体。 |
| [setFontItalic(byte value)](#setFontItalic-byte-) | 确定字体是否斜体。 |
...
| [setProofDisabled(byte value)](#setProofDisabled-byte-) | 确定文本是否不应进行校对。 |
...
| [isHardUnderlineLine()](#isHardUnderlineLine--) | 确定下划线样式是否拥有自己的 LineFormat 属性，或从文本的 LineFormat 属性继承。 |
| [setHardUnderlineLine(byte value)](#setHardUnderlineLine-byte-) | 确定下划线样式是否拥有自己的 LineFormat 属性，或从文本的 LineFormat 属性继承。 |
| [isHardUnderlineFill()](#isHardUnderlineFill--) | 确定下划线样式是否拥有自己的 FillFormat 属性，或从文本的 FillFormat 属性继承。 |
| [setHardUnderlineFill(byte value)](#setHardUnderlineFill-byte-) | 确定下划线样式是否拥有自己的 FillFormat 属性，或从文本的 FillFormat 属性继承。 |
...
| [getKerningMinimalSize()](#getKerningMinimalSize--) | 返回或设置应开启字距调整的最小字体大小。 |
| [setKerningMinimalSize(float value)](#setKerningMinimalSize-float-) | 返回或设置应开启字距调整的最小字体大小。 |
...
返回用于突出显示文本的颜色。不适用继承。只读 [IColorFormat](../../com.aspose.slides/icolorformat)。
...
确定字体是否加粗。不适用继承。读写 [NullableBool](../../com.aspose.slides/nullablebool)。
...
确定字体是否斜体。不适用继承。读写 [NullableBool](../../com.aspose.slides/nullablebool)。
...
确定数字是否应忽略文本东亚语言特定的垂直布局。不适用继承。读写 [NullableBool](../../com.aspose.slides/nullablebool)。
...
确定文本的高度是否应标准化。不适用继承。读写 [NullableBool](../../com.aspose.slides/nullablebool)。
...
确定文本是否不应进行校对。不适用继承。读写 [NullableBool](../../com.aspose.slides/nullablebool)。
...
返回或设置文本下划线类型。不适用继承。读写 [TextUnderlineType](../../com.aspose.slides/textunderlinetype)。
...
返回或设置文本大小写类型。不适用继承。读写 [TextCapType](../../com.aspose.slides/textcaptype)。
...
返回或设置文本删除线类型。不适用继承。读写 [TextStrikethroughType](../../com.aspose.slides/textstrikethroughtype)。
...
确定下划线样式是否拥有自己的 LineFormat 属性，或从文本的 LineFormat 属性继承。读写 [NullableBool](../../com.aspose.slides/nullablebool)。
...
确定下划线样式是否拥有自己的 FillFormat 属性，或从文本的 FillFormat 属性继承。读写 [NullableBool](../../com.aspose.slides/nullablebool)。
...
返回或设置段落的字体高度。**Float.NaN** 表示高度未定义，应从母版继承。读写 float。
...
返回或设置拉丁字体信息。Null 表示字体未定义，应从母版继承。读写 [IFontData](../../com.aspose.slides/ifontdata)。
...
返回或设置东亚字体信息。Null 表示字体未定义，应从母版继承。读写 [IFontData](../../com.aspose.slides/ifontdata)。
...
返回或设置复杂脚本字体信息。Null 表示字体未定义，应从母版继承。读写 [IFontData](../../com.aspose.slides/ifontdata)。
...
返回或设置符号字体信息。Null 表示字体未定义，应从母版继承。读写 [IFontData](../../com.aspose.slides/ifontdata)。
...
返回或设置上标或下标文本。值范围为 -100%（下标）至 100%（上标）。**Float.NaN** 表示值未定义，应从母版继承。读写 float。
...
返回或设置应开启字距调整的最小字体大小。**Float.NaN** 表示值未定义，应从母版继承。读写 float。
...
返回或设置校对语言的 Id。用于拼写和语法检查。读写 String。
...
返回或设置字符间距增量。**Float.NaN** 表示值未定义，应从母版继承。读写 float。
...
获取或设置一个值，指示文本段落是否启用拼写检查。当此属性设为 false 时，文本元素的拼写检查将被抑制。设为 true 时，允许拼写检查。默认值为 false。
