...
返回或设置标题的 x 坐标，作为图表宽度的比例。读/写 float.
...
返回或设置标题的 y 坐标，作为图表高度的比例。读/写 float.
...
返回或设置标题的宽度，作为图表宽度的比例。读/写 float.
...
返回或设置标题的高度，作为图表高度的比例。读/写 float.
...
可以包含富格式文本。如果此属性不为 null，则该格式化文本值会覆盖自动生成的文本。自动生成的文本是数据标签、值轴显示单位标签、轴标题、图表标题、趋势线标签的隐式属性。自动生成的文本使用 IFormattedTextContainer.TextFormat 属性进行格式化。只读 [ITextFrame](../../com.aspose.slides/itextframe)。
...
指定图表元素相对于图表左上角的实际 x 位置（左）。在获取实际值之前调用 IChart.validateChartLayout() 方法。读 float。
...
指定图表元素相对于图表左上角的实际顶部位置。在获取实际值之前调用 IChart.validateChartLayout() 方法。读 float。
...
指定图表元素的实际宽度。在获取实际值之前调用 IChart.validateChartLayout() 方法。读 float。
...
指定图表元素的实际高度。在获取实际值之前调用 IChart.validateChartLayout() 方法。读 float。
