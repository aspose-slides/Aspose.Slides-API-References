...
| [getCropLeft()](#getCropLeft--) | 返回或设置左侧裁剪的真实图像宽度的百分比。 |
| [setCropLeft(float value)](#setCropLeft-float-) | 返回或设置左侧裁剪的真实图像宽度的百分比。 |
| [getCropTop()](#getCropTop--) | 返回或设置顶部裁剪的真实图像高度的百分比。 |
| [setCropTop(float value)](#setCropTop-float-) | 返回或设置顶部裁剪的真实图像高度的百分比。 |
| [getCropRight()](#getCropRight--) | 返回或设置右侧裁剪的真实图像宽度的百分比。 |
| [setCropRight(float value)](#setCropRight-float-) | 返回或设置右侧裁剪的真实图像宽度的百分比。 |
| [getCropBottom()](#getCropBottom--) | 返回或设置底部裁剪的真实图像高度的百分比。 |
| [setCropBottom(float value)](#setCropBottom-float-) | 返回或设置底部裁剪的真实图像高度的百分比。 |
| [getStretchOffsetLeft()](#getStretchOffsetLeft--) | 返回或设置填充矩形左边缘的百分比偏移（相对于形状边界框左边缘）。 |
| [setStretchOffsetLeft(float value)](#setStretchOffsetLeft-float-) | 返回或设置填充矩形左边缘的百分比偏移（相对于形状边界框左边缘）。 |
| [getStretchOffsetTop()](#getStretchOffsetTop--) | 返回或设置填充矩形上边缘的百分比偏移（相对于形状边界框上边缘）。 |
| [setStretchOffsetTop(float value)](#setStretchOffsetTop-float-) | 返回或设置填充矩形上边缘的百分比偏移（相对于形状边界框上边缘）。 |
| [getStretchOffsetRight()](#getStretchOffsetRight--) | 返回或设置填充矩形右边缘的百分比偏移（相对于形状边界框右边缘）。 |
| [setStretchOffsetRight(float value)](#setStretchOffsetRight-float-) | 返回或设置填充矩形右边缘的百分比偏移（相对于形状边界框右边缘）。 |
| [getStretchOffsetBottom()](#getStretchOffsetBottom--) | 返回或设置填充矩形底边缘的百分比偏移（相对于形状边界框底边缘）。 |
| [setStretchOffsetBottom(float value)](#setStretchOffsetBottom-float-) | 返回或设置填充矩形底边缘的百分比偏移（相对于形状边界框底边缘）。 |
| [deletePictureCroppedAreas()](#deletePictureCroppedAreas--) | 删除填充图片的裁剪区域。 |
| [compressImage(boolean deleteCroppedAreasOfImage, int resolution)](#compressImage-boolean-int-) | 根据形状大小和指定分辨率压缩图像。 |
| [compressImage(boolean deleteCroppedAreasOfImage, float resolution)](#compressImage-boolean-float-) | 根据形状大小和指定分辨率压缩图像。 |
| [getTileOffsetX()](#getTileOffsetX--) | 返回或设置纹理相对于形状原点的水平偏移（单位：点）。 |
| [setTileOffsetX(float value)](#setTileOffsetX-float-) | 返回或设置纹理相对于形状原点的水平偏移（单位：点）。 |
| [getTileOffsetY()](#getTileOffsetY--) | 返回或设置纹理相对于形状原点的垂直偏移（单位：点）。 |
| [setTileOffsetY(float value)](#setTileOffsetY-float-) | 返回或设置纹理相对于形状原点的垂直偏移（单位：点）。 |
...
| [getTileFlip()](#getTileFlip--) | 围绕水平、垂直或两个轴翻转纹理平铺。 |
| [setTileFlip(int value)](#setTileFlip-int-) | 围绕水平、垂直或两个轴翻转纹理平铺。 |
...
返回或设置左侧裁剪的真实图像宽度的百分比。读/写 float。
...
返回或设置顶部裁剪的真实图像高度的百分比。读/写 float。
...
返回或设置右侧裁剪的真实图像宽度的百分比。读/写 float。
...
返回或设置底部裁剪的真实图像高度的百分比。读/写 float。
...
返回或设置填充矩形左边缘的百分比偏移（相对于形状边界框左边缘）。正百分比表示内缩，负百分比表示外伸。读/写 float。
...
返回或设置填充矩形上边缘的百分比偏移（相对于形状边界框上边缘）。正百分比表示内缩，负百分比表示外伸。读/写 float。
...
返回或设置填充矩形右边缘的百分比偏移（相对于形状边界框右边缘）。正百分比表示内缩，负百分比表示外伸。读/写 float。
...
返回或设置填充矩形底边缘的百分比偏移（相对于形状边界框底边缘）。正百分比表示内缩，负百分比表示外伸。读/写 float。
...
**返回值：**
[IPPImage](../../com.aspose.slides/ippimage) - 裁剪后的图像，若无需裁剪则返回原始图像。

--------------------

此方法在裁剪的同时将 WMF/EMF 元文件转换为光栅 PNG 图像。
...
根据形状大小和指定分辨率压缩图像以减小其尺寸。可选地，它还能删除裁剪区域。
...
> CompressImage
> ``` 方法通过设置目标分辨率并移除裁剪区域来减小演示文稿中图像大小的示例：
>  
>  Presentation presentation = new Presentation("demo.pptx");
>  try {
>      ISlide slide = presentation.getSlides().get_Item(0);
...
| resolution | int | 压缩的目标分辨率，指定为 [PicturesCompression](../../com.aspose.slides/picturescompression) 枚举的一个值。

--------------------

此方法会更改图像的大小和分辨率，类似于 PowerPoint 的 “Picture Format -> Compress Pictures” 功能。

**返回值：**
boolean - 一个布尔值，指示图像是否成功压缩。如果图像已被重新调整大小或裁剪，则返回 true；否则返回 false。
...
> CompressImage
> ```
 方法通过设置目标分辨率并移除裁剪区域来减小演示文稿中图像的大小：
>   
>  Presentation presentation = new Presentation("demo.pptx");
>  try {
>      ISlide slide = presentation.getSlides().get_Item(0);
...
默认是 [RectangleAlignment.TopLeft](../../com.aspose.slides/rectanglealignment\#TopLeft)。
