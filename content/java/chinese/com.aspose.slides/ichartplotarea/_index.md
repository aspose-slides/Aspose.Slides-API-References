...
| [getLayoutTargetType()](#getLayoutTargetType--) | 如果绘图区的布局是手动定义的，此属性指定是按内部布局（不包括坐标轴和坐标轴标签）还是按外部布局（包括坐标轴和坐标轴标签）。 |
| [setLayoutTargetType(int value)](#setLayoutTargetType-int-) | 如果绘图区的布局是手动定义的，此属性指定是按内部布局（不包括坐标轴和坐标轴标签）还是按外部布局（包括坐标轴和坐标轴标签）。 |
### getFormat() {#getFormat--}
```
...
如果绘图区的布局是手动定义的，此属性指定是按内部布局（不包括坐标轴和坐标轴标签）还是按外部布局（包括坐标轴和坐标轴标签）。读写 [LayoutTargetType](../../com.aspose.slides/layouttargettype)(\#getLayoutTargetType.getLayoutTargetType/\#setLayoutTargetType(int).setLayoutTargetType(int)).
