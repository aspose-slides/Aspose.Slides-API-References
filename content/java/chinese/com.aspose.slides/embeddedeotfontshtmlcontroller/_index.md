...
description: 用于在 EOT 格式中嵌入字体的格式控制器类
type: docs
...
在形状渲染之前调用。对每个形状调用一次。如果此函数向 generator 写入任何内容，当前幻灯片的图像生成将结束，插入已添加的 HTML 片段，并在前一个图像之上启动新的图像。
