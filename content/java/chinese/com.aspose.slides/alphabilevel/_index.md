...
表示 Alpha 双层效果。Alpha（不透明度）值小于阈值的将被更改为 0（完全透明），而大于或等于阈值的 Alpha 值将被更改为 100%（完全不透明）。
## 方法
...
| [getEffective()](#getEffective--) | 获取在继承应用后的有效 Alpha 双层效果数据。 |
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| obj | java.lang.Object | 用于比较的 [AlphaBiLevel](../../com.aspose.slides/alphabilevel)。 |
