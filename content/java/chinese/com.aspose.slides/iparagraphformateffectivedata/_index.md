...
此接口与 [IParagraphFormat](../../com.aspose.slides/iparagraphformat) 接口一起使用，以在应用继承后返回有效的格式化值。
## 方法
...
| [getAlignment()](#getAlignment--) | 返回段落中的文本对齐方式。 |
| [getSpaceWithin()](#getSpaceWithin--) | 返回段落基线之间的空间量。 |
| [getSpaceBefore()](#getSpaceBefore--) | 返回段落第一行之前的空间量。 |
| [getSpaceAfter()](#getSpaceAfter--) | 返回段落最后一行之后的空间量。 |
...
| [getMarginLeft()](#getMarginLeft--) | 返回段落的左边距。 |
| [getMarginRight()](#getMarginRight--) | 返回段落的右边距。 |
| [getIndent()](#getIndent--) | 返回段落的首行缩进/悬挂缩进。 |
| [getDefaultTabSize()](#getDefaultTabSize--) | 返回默认制表位大小。 |
...
返回段落中的文本对齐方式。只读 [TextAlignment](../../com.aspose.slides/textalignment)。
...
返回段落的首行缩进/悬挂缩进。悬挂缩进可用负值定义。只读 float。
