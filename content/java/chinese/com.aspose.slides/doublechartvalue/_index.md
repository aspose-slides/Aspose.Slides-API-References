...
description: 表示可以在 pptx 演示文稿中以两种方式存储的双精度值：1）在与图表相关的工作簿的单元格/单元格中；2）作为文字值。
type: docs
...
表示可以在 pptx 演示文稿中以两种方式存储的双精度值：1) 在与图表相关的工作簿的单元格/单元格中；2) 作为文字值。
## 方法
...
| [getAsLiteralDouble()](#getAsLiteralDouble--) | 返回或设置值为文字 double。 |
| [setAsLiteralDouble(double value)](#setAsLiteralDouble-double-) | 返回或设置值为文字 double。 |
