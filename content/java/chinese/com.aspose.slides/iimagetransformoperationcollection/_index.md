...
| 方法 | 描述 |
| --- | --- |
| [get_Item(int index)](#get-Item-int-) | 返回集合中索引对应的 [IImageTransformOperation](../../com.aspose.slides/iimagetransformoperation)。 |
| [removeAt(int index)](#removeAt-int-) | 从集合中指定索引处移除图像效果。 |
...
| [addBiLevelEffect(float threshold)](#addBiLevelEffect-float-) | 在集合末尾添加新的 Bi-Level（黑/白）效果。 |
...
返回集合中索引对应的 [IImageTransformOperation](../../com.aspose.slides/iimagetransformoperation)。
...
在集合末尾添加新的 Bi-Level（黑/白）效果。

**参数：**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| threshold | float | Bi-Level 效果的亮度阈值。大于等于阈值的设置为白色，小于阈值的设置为黑色。 |
...
| --- | --- | --- |
| radius | double | 模糊的半径。 |
| grow | boolean | 指定对象的边界是否因模糊而扩大。true 表示边界会扩大，false 表示不扩大。 |
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| brightness | float | 亮度变化的百分比。 |
| contrast | float | 对比度变化的百分比。 |
