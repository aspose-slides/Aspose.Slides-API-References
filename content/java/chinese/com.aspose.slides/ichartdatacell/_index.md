...
| 方法 | 描述 |
| --- | --- |
| [getRow()](#getRow--) | 返回单元格所在工作表行的索引。 |
| [getColumn()](#getColumn--) | 返回单元格所在工作表列的索引。 |
...
| [calculate(boolean updateValues)](#calculate-boolean-) | 如果单元格包含公式，则值将根据该公式进行更新。 |
### getRow() {#getRow--}
```
public abstract int getRow()
```

返回单元格所在工作表行的索引。只读 int。
...
获取或设置数字和日期的内置显示格式。Preset number 必须在 [0..22] 或 [37..49] 范围内。读/写 byte。
