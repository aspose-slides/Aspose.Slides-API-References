...
| [getGroupingLevels()](#getGroupingLevels--) | 图表类别分组层级值的托管容器。 |
...
如果为 true，则 AsCell 属性为实际。换句话说，工作表用于存储类别（此情况下支持多层级类别）。如果为 false，则 AsLiteral 属性为实际。换句话说，工作表不用于存储类别（此情况下不支持多层级类别）。只读 boolean。
...
返回或设置 IChartDataCell 对象。如果类别是多层级的，则在级别 “0” 使用 IChartDataCell 对象。读/写 [IChartDataCell](../../com.aspose.slides/ichartdatacell)。
...
如果 UseCell 为 true，则此属性表示 AsCell.Value 属性。如果 UseCell 为 false，则此属性表示 AsLiteral 属性。读/写 Object。
...
图表类别分组层级值的托管容器。多层级类别包含多个分组层级。分组层级索引从零开始。只读 [IChartCategoryLevelsManager](../../com.aspose.slides/ichartcategorylevelsmanager)。
