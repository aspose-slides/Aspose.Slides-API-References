...
| [getLightType()](#getLightType--) | 表示可应用于形状的预设光右。 |
| [setLightType(int value)](#setLightType-int-) | 表示可应用于形状的预设光右。 |
...
表示可应用于形状的预设光右。光照框表示相对于 3D 场景以特定方式定位的一组光源。读/写 [LightRigPresetType](../../com.aspose.slides/lightrigpresettype)。
...
通过使用纬度坐标、经度坐标以及围绕轴的旋转来定义旋转。返回数组的第一个元素-纬度，第二个-经度，第三个-旋转。
