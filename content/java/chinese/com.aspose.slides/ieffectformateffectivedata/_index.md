...
此接口与 [IEffectFormat](../../com.aspose.slides/ieffectformat) 接口一起使用，以返回已应用继承的有效格式化值。

## Methods

| 方法 | 描述 |
| --- | --- |
| [isNoEffects()](#isNoEffects--) | 如果所有效果均被禁用（如刚创建的默认 EffectFormat 对象），返回 true。 |
| [getBlurEffect()](#getBlurEffect--) | 模糊效果。 |
| [getFillOverlayEffect()](#getFillOverlayEffect--) | 填充覆盖效果。 |
...
如果所有效果均被禁用（如刚创建的默认 EffectFormat 对象），返回 true。只读 boolean。
...
填充覆盖效果。只读 [IFillOverlayEffectiveData](../../com.aspose.slides/ifilloverlayeffectivedata)。
...
发光效果。只读 [IGlowEffectiveData](../../com.aspose.slides/igloweffectivedata)。
