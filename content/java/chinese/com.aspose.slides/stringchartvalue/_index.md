...
description: 表示可以在 pptx 演示文稿中以两种方式存储的字符串值：1）在与图表关联的工作簿的单元格中；2）作为文字值。
type: docs
...
表示可以在 pptx 演示文稿中存储的字符串值，有两种方式：1) 在与图表关联的工作簿的单元格中；2) 作为文字值。
## 方法

| 方法 | 描述 |
| --- | --- |
| [getAsCells()](#getAsCells--) | 不允许分配空值。 |
| [setAsCells(IChartCellCollection value)](#setAsCells-com.aspose.slides.IChartCellCollection-) | 不允许分配空值。 |
| [getAsLiteralString()](#getAsLiteralString--) | 返回或设置为文字字符串。 |
| [setAsLiteralString(String value)](#setAsLiteralString-java.lang.String-) | 返回或设置为文字字符串。 |
...
不允许分配空值。返回的值始终非空。读/写 [IChartCellCollection](../../com.aspose.slides/ichartcellcollection)。
