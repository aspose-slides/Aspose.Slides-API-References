...
一个内置的[IAIWebClient](../../com.aspose.slides/iaiwebclient)实现，用于连接OpenAI API。
## 构造函数
...
创建使用外部管理的 HttpClient 的 OpenAI Web 客户端实例。提供的 HttpClient 不会被此实例释放，仍由调用方拥有。
...
使用提供的 HttpConnection 实例向 AI 模型发送聊天指令，并返回对该指令的响应消息。
