...
删除线水平（默认 false）- 指定水平删除线的隐藏或显示状态。
...
>  
>  MathBorderBox borderBox = new MathBorderBox(new MathematicalText("x"));
>  MathBorderBox.setStrikethroughVertical(true);
> ```
...
删除线左下到右上（默认 false）。指定边框箱左下角到右上角的对角删除线的隐藏或显示状态。
...
从左上角到右下角的删除线（默认 false）。指定边框箱左上角到右下角的对角删除线的隐藏或显示状态。
