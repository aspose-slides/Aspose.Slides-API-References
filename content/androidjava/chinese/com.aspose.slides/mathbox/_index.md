...
指定数学元素的逻辑装箱（封装）。例如，装箱的对象可以作为带或不带对齐点的运算符仿真器，作为换行点，或被分组以防止在其内部换行。例如，应该对 “==” 运算符进行装箱，以防止换行。
...
微分 当为 true 时，盒子充当微分 (例如 \\ud835\\udc51\\ud835\\udc65 在被积函数中)，并获得适用于数学微分的水平间距。默认值: false
