...
```
返回演示文稿的最后修改日期。值为 UTC 时间。对于 Presentation.DocumentProperties 为只读（因为在 IPresentation 对象保存过程中会在内部更新）。可以通过方法 [IPresentationInfo.readDocumentProperties](../../com.aspose.slides/ipresentationinfo#readDocumentProperties) 返回的 DocumentProperties 实例进行更改。请参见 [IPresentationInfo.updateDocumentProperties(IDocumentProperties)](../../com.aspose.slides/ipresentationinfo#updateDocumentProperties-IDocumentProperties-) 方法摘要中的示例。
...
返回演示文稿最后修改的日期。值为 UTC 时间。对于 Presentation.DocumentProperties 为只读（因为在 IPresentation 对象保存过程中会在内部更新）。可以通过方法 [IPresentationInfo.readDocumentProperties](../../com.aspose.slides/ipresentationinfo#readDocumentProperties) 返回的 DocumentProperties 实例进行更改。请参见 [IPresentationInfo.updateDocumentProperties(IDocumentProperties)](../../com.aspose.slides/ipresentationinfo#updateDocumentProperties-IDocumentProperties-) 方法摘要中的示例。
...
返回演示文稿上一次打印的日期。读写 java.util.Date.
...
返回演示文稿上一次打印的日期。读/写 java.util.Date.
...
> ```
> 以下代码演示如何将敏感度标签信息从自定义文档属性移动到现代 SensitivityLabels 集合：
>   
...
指示文档中的超链接是否是最新的。将此元素设置为 **true** 表示超链接已更新。将此元素设置为 **false** 表示超链接已过时。读/写 boolean。
...
```
指定此部分中的一个或多个超链接仅由生成者在此部分更新。下一个打开此文档的生成者应使用此部分中指定的新超链接更新超链接关系。读/写 boolean。
...
```
指示文档部件的分组以及每个组中的部件数量。只读 IHeadingPair[].
