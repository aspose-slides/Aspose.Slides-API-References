...
| [getDisableFontLigatures()](#getDisableFontLigatures--) | 取得或設定指示文字是否在渲染時不使用連字的值。 |
| [setDisableFontLigatures(boolean value)](#setDisableFontLigatures-boolean-) | 取得或設定指示文字是否在渲染時不使用連字的值。 |
...
取得或設定指示文字是否在渲染時不使用連字的值。設定為 true 時，連字將在渲染輸出中被停用。預設情況下，此屬性為 false。
