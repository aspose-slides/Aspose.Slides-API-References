...
description: "L'eccezione che viene lanciata quando un metodo è invocato su un oggetto il cui stato è incompatibile con questa chiamata. Non creare mai istanze di questa classe manualmente. Usa invece la classe InvalidOperationException. Non inserire mai le istanze della classe InvalidOperationException in System::SmartPtr."
type: docs
weight: 508
url: /it/system/details_invalidoperationexception/
---
## Details_InvalidOperationException classe


L'eccezione che viene lanciata quando un metodo è invocato su un oggetto il cui stato è incompatibile con questa chiamata. Non creare mai istanze di questa classe manualmente. Usa invece la classe InvalidOperationException. Non inserire mai le istanze della classe InvalidOperationException dentro [System::SmartPtr](../smartptr/).
...
| void [Lock](../object/lock/)() | Implementa il blocco lock() di C#. Chiama direttamente o usa l'oggetto sentinella [LockContext](../lockcontext/). |
...
| void [Unlock](../object/unlock/)() | Implementa lo sblocco della dichiarazione lock() di C#. Chiama direttamente o usa l'oggetto sentinella [LockContext](../lockcontext/). |
