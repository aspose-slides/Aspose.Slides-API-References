...
description: Apontador de dicionário ordenado com operadores de acesso. Este tipo é um ponteiro para gerenciar a exclusão de outro objeto. Deve ser alocado na pilha e passado para funções por valor ou por referência const.
type: docs
weight: 534
url: /pt/system.collections.generic/sorteddictionaryptr/
---
## SortedDictionaryPtr classe

Apontador de dicionário ordenado com operadores de acesso. Este tipo é um ponteiro para gerenciar a exclusão de outro objeto. Deve ser alocado na pilha e passado para funções por valor ou por referência const.
...
| [Pointee_](../../system/smartptr/pointee_/) * [get_shared](../../system/smartptr/get_shared/)() const | Obtém o objeto apontado, mas afirma que o ponteiro está no modo compartilhado. |
| int [get_shared_count](../../system/smartptr/get_shared_count/)() const | Obtém o número de ponteiros compartilhados existentes para o objeto referenciado, incluindo o atual. Afirma que o ponteiro atual está no modo compartilhado. |
...
| **bool** [IsAliasingPtr](../../system/smartptr/isaliasingptr/)() const | Verifica se o ponteiro aponta para outro objeto que não o de propriedade (criado por um construtor de aliasing). |
...
| V\& [operator[]](./operator[]/)(const T\&) const | Função acessora. |
| [SmartPtr_](../../system/smartptr/smartptr_/) [RemoveAliasing](../../system/smartptr/removealiasing/)() const | Remove o aliasing (criado por um construtor de aliasing) do ponteiro, garantindo que ele gerencie (se compartilhado) ou rastreie (se fraco) o mesmo objeto ao qual aponta. |
...
|  [SmartPtr](../../system/smartptr/smartptr/)([SmartPtrMode](../../system/smartptrmode/)) | Cria o objeto [SmartPtr](../../system/smartptr/) no modo requerido. |
...
|  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<Q\>\&, [SmartPtrMode](../../system/smartptrmode/)) | Constrói por cópia o objeto [SmartPtr](../../system/smartptr/). Ambos os ponteiros apontam para o mesmo objeto depois. Realiza conversão de tipo se permitida. |
|  [SmartPtr](../../system/smartptr/smartptr/)([SmartPtr_](../../system/smartptr/smartptr_/)\&&, [SmartPtrMode](../../system/smartptrmode/)) | Constrói por move o objeto [SmartPtr](../../system/smartptr/). Efetivamente, troca dois ponteiros, se ambos estiverem no mesmo modo. x pode ficar inutilizável após a chamada. |
...
|  [SortedDictionaryPtr](./sorteddictionaryptr/)() | Constrói ponteiro nulo. |
|  [SortedDictionaryPtr](./sorteddictionaryptr/)(const [SharedPtr](../../system/sharedptr/)\<[SortedDictionary](../sorteddictionary/)\<T, V\>\>\&) | Constrói ponteiro para o dicionário ordenado especificado. |
