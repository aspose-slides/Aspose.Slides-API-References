...
description: "Interface de contêiner indexado de elementos. Objetos desta classe devem ser alocados somente usando a função System::MakeObject(). Nunca crie instância deste tipo na pilha ou usando o operador new, pois isso resultará em erros de tempo de execução e/ou falhas de asserção. Sempre encapsule esta classe em um ponteiro System::SmartPtr e use esse ponteiro para passá-lo a funções como argumento."
type: docs
weight: 339
url: /pt/system.collections.generic/ilist/
---
## IList classe

Interface de contêiner indexado de elementos. Objetos desta classe devem ser alocados somente usando a função [System::MakeObject()](../../system/makeobject/). Nunca crie instância deste tipo na pilha ou usando o operador new, pois isso resultará em erros de tempo de execução e/ou falhas de asserção. Sempre envolva esta classe em um ponteiro [System::SmartPtr](../../system/smartptr/) e use esse ponteiro para passá-lo a funções como argumento.
...
| [iterator](../ienumerable/iterator/) [begin](../ienumerable/begin/)() | Obtém iterador apontando para o primeiro elemento (se houver) da coleção. Esse iterador não pode ser usado para alterar o objeto referenciado porque [GetEnumerator()](../ienumerable/getenumerator/) retorna um objeto-cópia de T. |
...
| virtual void [CopyTo](../icollection/copyto/)([System::ArrayPtr](../../system/arrayptr/)\<T\>, int) | Copia todos os elementos da coleção para elementos de array existentes. |
| [iterator](../ienumerable/iterator/) [end](../ienumerable/end/)() | Obtém iterador apontando logo após o último elemento (se houver) da coleção. Esse iterador não pode ser usado para alterar o objeto referenciado porque [GetEnumerator()](../ienumerable/getenumerator/) retorna um objeto-cópia de T. |
...
| **bool** [get_IsFixedSize](./get_isfixedsize/)() | Verifica se a coleção tem tamanho fixo. |
...
| virtual T [idx_get](./idx_get/)(int) const | Obtém o elemento no índice especificado. |
| virtual void [idx_set](./idx_set/)(int, T) | Define o elemento no índice especificado. |
| virtual int [IndexOf](./indexof/)(const T\&) const | Obtém o índice da primeira ocorrência do item no contêiner. |
| virtual void [Insert](./insert/)(int, const T\&) | Insere elemento na posição especificada, deslocando outros elementos. |
...
| virtual void [RemoveAt](./removeat/)(int) | Remove elemento no índice especificado. |
