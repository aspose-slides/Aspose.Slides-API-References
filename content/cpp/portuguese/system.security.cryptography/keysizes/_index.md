...
description: "Conjunto de tamanhos de chave aceitos por algoritmos simétricos. Objetos desta classe devem ser alocados somente usando a função System::MakeObject(). Nunca crie instância deste tipo na pilha ou usando o operador new, pois isso resultará em erros de tempo de execução e/ou falhas de asserção. Sempre envolva esta classe em um ponteiro System::SmartPtr e use esse ponteiro para passá-lo a funções como argumento."
type: docs
weight: 300
url: /pt/system.security.cryptography/keysizes/
---
## classe KeySizes

Conjunto de tamanhos de chave aceitos por algoritmos simétricos. Objetos desta classe devem ser alocados somente usando a função [System::MakeObject()](../../system/makeobject/). Nunca crie instância deste tipo na pilha ou usando o operador new, pois isso resultará em erros de tempo de execução e/ou falhas de asserção. Sempre envolva esta classe em um ponteiro [System::SmartPtr](../../system/smartptr/) e use esse ponteiro para passá-lo a funções como argumento.
...
| int [get_MaxSize](./get_maxsize/)() const | Obtém o tamanho máximo de chave válido. |
| int [get_MinSize](./get_minsize/)() const | Obtém o tamanho mínimo de chave válido. |
| int [get_SkipSize](./get_skipsize/)() const | Obtém o passo de tamanho de chave válido. |
...
|  [KeySizes](./keysizes/)(int, int, int) | Informação RTTI. |
