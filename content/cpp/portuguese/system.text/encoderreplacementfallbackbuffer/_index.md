...
description: "Buffer para substituir a estratégia de fallback de codificação. Objetos desta classe devem ser alocados apenas usando a função System::MakeObject(). Nunca crie instância desse tipo na pilha ou usando o operador new, pois isso resultará em erros de tempo de execução e/ou falhas de asserção. Sempre encapsule esta classe em um ponteiro System::SmartPtr e use esse ponteiro para passá-lo às funções como argumento."
type: docs
weight: 209
url: /pt/system.text/encoderreplacementfallbackbuffer/
---
## EncoderReplacementFallbackBuffer classe


[Buffer](../../system/buffer/) para substituir a estratégia de fallback de codificação. Objetos desta classe devem ser alocados apenas usando a função [System::MakeObject()](../../system/makeobject/). Nunca crie instância desse tipo na pilha ou usando o operador new, pois isso resultará em erros de tempo de execução e/ou falhas de asserção. Sempre encapsule esta classe em um ponteiro [System::SmartPtr](../../system/smartptr/) e use esse ponteiro para passá-lo às funções como argumento.
...
| Metodo | Descrição |
| --- | --- |
|  [EncoderReplacementFallbackBuffer](./encoderreplacementfallbackbuffer/)(const [EncoderReplacementFallbackPtr](../../system/encoderreplacementfallbackptr/)\&) | Construtor. |
...
| **bool** [Fallback](./fallback/)(char_t, int) override | Manipula falha de codificação. |
| **bool** [Fallback](./fallback/)(char_t, char_t, int) override | Manipula falha de codificação. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | Somente para uso interno. |
| int [get_Remaining](./get_remaining/)() const override | Obtém o número de caracteres restantes no buffer. |
...
| void [Reset](./reset/)() override | Redefine o buffer para o estado inicial (antes da chamada [Fallback()](./fallback/)). |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Define o n-ésimo argumento do template como um ponteiro fraco (em vez de compartilhado). Permite trocar ponteiros em contêineres para o modo fraco. |
...
* Classe [EncoderFallbackBuffer](../encoderfallbackbuffer/)
