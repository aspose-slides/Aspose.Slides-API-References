...
description: "Usado para armazenar informações serializadas sobre as instâncias da classe EndPoint. Objetos desta classe devem ser alocados somente usando a função System::MakeObject(). Nunca crie uma instância deste tipo na pilha ou usando operator new, pois isso resultará em erros de tempo de execução e/ou falhas de asserção. Sempre encapsule esta classe em um ponteiro System::SmartPtr e use esse ponteiro para passá-lo a funções como argumento."
type: docs
weight: 443
url: /pt/system.net/socketaddress/
---
## SocketAddress classe

Usado para armazenar informações serializadas sobre as instâncias da classe [EndPoint](../endpoint/). Objetos desta classe devem ser alocados somente usando a função [System::MakeObject()](../../system/makeobject/). Nunca crie uma instância deste tipo na pilha ou usando operator new, pois isso resultará em erros de tempo de execução e/ou falhas de asserção. Sempre envolva esta classe em um ponteiro [System::SmartPtr](../../system/smartptr/) e use esse ponteiro para passá-lo a funções como argumento.
...
| [Sockets::AddressFamily](../../system.net.sockets/addressfamily/) [get_Family](./get_family/)() | Retorna a família de endereços. |
| **int32_t** [get_Size](./get_size/)() | Retorna o tamanho do buffer subjacente. |
...
| **uint8_t** [idx_get](./idx_get/)(**int32_t**) | Obtém um valor do buffer subjacente no índice especificado. |
| void [idx_set](./idx_set/)(**int32_t**, **uint8_t**) | Define um valor no buffer subjacente no índice especificado. |
...
|  [SocketAddress](./socketaddress/)([Sockets::AddressFamily](../../system.net.sockets/addressfamily/)) | Constrói uma nova instância. |
|  [SocketAddress](./socketaddress/)([Sockets::AddressFamily](../../system.net.sockets/addressfamily/), **int32_t**) | Constrói uma nova instância. |
