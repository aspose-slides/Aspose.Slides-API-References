...
description: "Calendário japonês. Objetos desta classe devem ser alocados apenas usando a função System::MakeObject(). Nunca crie instância deste tipo na pilha ou usando operador new, pois resultará em erros de tempo de execução e/ou falhas de asserção. Sempre envolva esta classe em ponteiro System::SmartPtr e use esse ponteiro para passá-lo a funções como argumento."
type: docs
weight: 183
url: /pt/system.globalization/japanesecalendar/
---
## JapaneseCalendar classe

Calendário japonês. Objetos desta classe devem ser alocados apenas usando a função [System::MakeObject()](../../system/makeobject/). Nunca crie instâncias deste tipo na pilha ou usando o operador new, pois isso resultará em erros de tempo de execução e/ou falhas de asserção. Sempre envolva esta classe em um ponteiro [System::SmartPtr](../../system/smartptr/) e use esse ponteiro para passá-lo a funções como argumento.
...
| [SharedPtr](../../system/sharedptr/)\<[Object](../../system/object/)\> [Clone](./clone/)() override | Cria uma cópia do objeto atual e devolve um ponteiro compartilhado para ele. |
...
| int [GetDaysInYear](./getdaysinyear/)(int, int) const override | Obtém o número de dias em um ano específico. |
| virtual int [GetDaysInYear](./getdaysinyear/)(int) const | Obtém o número de dias em um ano específico. |
| virtual int [GetDaysInYear](./getdaysinyear/)(int, int) const | Obtém o número de dias em um ano específico. |
...
|  [JapaneseCalendar](./japanesecalendar/)() | Construtor. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | Decrementa e devolve o contador de referência compartilhado. Não deve ser chamado diretamente; em vez disso, use ponteiros inteligentes ou ThisProtector. |
