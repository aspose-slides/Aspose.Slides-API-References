...
description: "TimeZoneNotFoundException é lançada quando as informações de fuso horário não são encontradas. Nunca crie instâncias desta classe manualmente. Use a classe TimeZoneNotFoundException em vez disso. Nunca encapsule as instâncias da classe TimeZoneNotFoundException em System::SmartPtr."
type: docs
weight: 729
url: /pt/system/details_timezonenotfoundexception/
---
## Details_TimeZoneNotFoundException classe

TimeZoneNotFoundException é lançada quando as informações de fuso horário não são encontradas. Nunca crie instâncias desta classe manualmente. Use a classe TimeZoneNotFoundException em vez disso. Nunca encapsule as instâncias da classe TimeZoneNotFoundException em [System::SmartPtr](../smartptr/).
...
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Define o n-ésimo argumento de modelo como um ponteiro fraco (em vez de compartilhado). Permite trocar ponteiros em contêineres para o modo fraco. |
...
| virtual const char * [what](../details_exception/what/)() const | Implementa o método [what()](../details_exception/what/) que é chamado pela classe [ExceptionWrapper](../exceptionwrapper/). Apesar de a classe não herdar de std::exception, classes derivadas podem usar membros protegidos/privados para implementar sua lógica. Mover a implementação deste método para o [ExceptionWrapper](../exceptionwrapper/) pode quebrar essa lógica. |
