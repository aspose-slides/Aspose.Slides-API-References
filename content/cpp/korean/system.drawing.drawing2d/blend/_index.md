...
description: "LinearGradientBrush 객체에 대한 블렌드 패턴을 나타냅니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여만 할당해야 합니다. 스택이나 operator new를 사용하여 이 타입의 인스턴스를 만들지 마세요. 이는 런타임 오류 및/또는 단언 오류를 발생시킬 수 있습니다. 항상 이 클래스를 System::SmartPtr 포인터로 래핑하고 해당 포인터를 함수 인수로 전달하십시오."
type: docs
weight: 14
url: /ko/system.drawing.drawing2d/blend/
---
## Blend 클래스

[LinearGradientBrush](../lineargradientbrush/) 객체에 대한 블렌드 패턴을 나타냅니다. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여만 할당해야 합니다. 스택에 또는 operator new를 사용하여 이 타입의 인스턴스를 절대 생성하지 마세요. 이는 런타임 오류 및/또는 단언 오류를 초래합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터로 래핑하고, 해당 포인터를 인수로 함수에 전달하십시오.
...
| 메서드 | 설명 |
| --- | --- |
|  [Blend](./blend/)() | [Blend](./) 클래스의 새 인스턴스를 생성합니다. |
|  [Blend](./blend/)(int) | [Blend](./) 클래스의 새 인스턴스를 생성합니다. |
...
| [ArrayPtr](../../system/arrayptr/)\<**float**\> [get_Factors](./get_factors/)() const | 그라디언트의 블렌드 계수 배열을 반환합니다. |
| [ArrayPtr](../../system/arrayptr/)\<**float**\> [get_Positions](./get_positions/)() const | 그라디언트의 블렌드 위치 배열을 반환합니다. |
...
| void [set_Factors](./set_factors/)(const [ArrayPtr](../../system/arrayptr/)\<**float**\>\&) | 그라디언트의 블렌드 계수 배열을 설정합니다. |
| void [set_Positions](./set_positions/)(const [ArrayPtr](../../system/arrayptr/)\<**float**\>\&) | 그라디언트의 블렌드 위치 배열을 설정합니다. |
