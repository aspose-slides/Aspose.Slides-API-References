...
description: "경로를 나타내는 그래픽 데이터를 포함합니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여 할당해야 합니다. 스택이나 operator new를 사용해 이 타입의 인스턴스를 만들면 런타임 오류 및/또는 어설션 결함이 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 래핑하고 해당 포인터를 인수로 함수에 전달하십시오."
type: docs
weight: 131
url: /ko/system.drawing.drawing2d/pathdata/
---
## PathData 클래스

경로를 나타내는 그래픽 데이터를 포함합니다. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여 할당해야 합니다. 스택이나 operator new를 사용하여 이 타입의 인스턴스를 생성하면 런타임 오류 및/또는 어설션 결함이 발생합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터로 래핑하고, 해당 포인터를 인수로 함수에 전달하십시오.
...
| [ArrayPtr](../../system/arrayptr/)\<[PointF](../../system.drawing/pointf/)\> [get_Points](./get_points/)() | 경로를 구성하는 점들을 포함하는 배열을 반환합니다. |
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_Types](./get_types/)() | **Points** 배열에서 해당 점들의 유형을 지정하는 값을 포함하는 배열을 반환합니다. |
...
|  [PathData](./pathdata/)() | 빈 [PathData](./) 객체를 구성합니다. |
...
| void [set_Points](./set_points/)(const [ArrayPtr](../../system/arrayptr/)\<[PointF](../../system.drawing/pointf/)\>\&) | 경로를 구성하는 점들을 포함하는 배열을 설정합니다. |
| void [set_Types](./set_types/)(const [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>\&) | **Points** 배열에서 해당 점들의 유형을 지정하는 값을 포함하는 배열을 설정합니다. |
