...
description: "'Range' 헤더의 값에서 바이트 범위를 나타냅니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여 할당해야 합니다. 스택에 직접 인스턴스를 생성하거나 operator new를 사용해서 만들면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 감싸고 해당 포인터를 함수 인수로 전달하십시오."
type: docs
weight: 261
url: /ko/system.net.http.headers/rangeitemheadervalue/
---
## RangeItemHeaderValue 클래스

[Range](../../system/range/) 헤더 값에서 바이트 범위를 나타냅니다. 이 클래스의 객체는 반드시 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여 할당해야 합니다. 스택에 직접 생성하거나 operator new를 사용해서 인스턴스를 만들면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터로 감싸고, 해당 포인터를 함수 인수로 전달하십시오.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | C#-style 부동소수점 비교를 에뮬레이션합니다. 이 비교에서는 IEC 60559:1989에 따르면 NaN은 어떤 값과도 같지 않음에도 두 NaN을 동일하게 간주합니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | C#-style 부동소수점 비교를 에뮬레이션합니다. 이 비교에서는 IEC 60559:1989에 따르면 NaN은 어떤 값과도 같지 않음에도 두 NaN을 동일하게 간주합니다. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | 내부 용도 전용입니다. |
| [Nullable](../../system/nullable/)\<**int64_t**\> [get_From](./get_from/)() | 데이터 전송을 시작해야 하는 위치를 반환합니다. |
| [Nullable](../../system/nullable/)\<**int64_t**\> [get_To](./get_to/)() | 데이터 전송을 중지해야 하는 위치를 반환합니다. |
...
| static **int32_t** [GetRangeItemLength](./getrangeitemlength/)([String](../../system/string/), **int32_t**, [System::SharedPtr](../../system/sharedptr/)\<[RangeItemHeaderValue](./)\>\&) | 지정된 인덱스부터 전달된 문자열을 [RangeItemHeaderValue](./) 클래스의 인스턴스로 변환합니다. |
| static **int32_t** [GetRangeItemListLength](./getrangeitemlistlength/)([String](../../system/string/), **int32_t**, [System::SharedPtr](../../system/sharedptr/)\<[Collections::Generic::ICollection](../../system.collections.generic/icollection/)\<[System::SharedPtr](../../system/sharedptr/)\<[RangeItemHeaderValue](./)\>\>\>) | 지정된 위치부터 전달된 문자열을 RangeItemHeaderValue-클래스 인스턴스 컬렉션으로 변환합니다. |
...
|  [RangeItemHeaderValue](./rangeitemheadervalue/)([Nullable](../../system/nullable/)\<**int64_t**\>, [Nullable](../../system/nullable/)\<**int64_t**\>) | 새 인스턴스를 생성합니다. |
