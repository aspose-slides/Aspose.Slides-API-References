---
title: ShiftIteratorBy()
second_title: Aspose.Slides for C++ API 참조
description: 지정된 단계 수만큼 반복자를 이동합니다.
type: docs
weight: 53
url: /ko/system.collections.generic/dictionaryiterator/shiftiteratorby/
---
## DictionaryIterator::ShiftIteratorBy(std::ptrdiff_t) 메서드


지정된 단계 수만큼 반복자를 이동합니다.

```cpp
void System::Collections::Generic::DictionaryIterator<Dict>::ShiftIteratorBy(std::ptrdiff_t offset) override
```


### 인수

| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| offset | std::ptrdiff_t | 반복자를 이동시킬 단계 수. |

## 관련 항목

* 클래스 [DictionaryIterator](../)
* 네임스페이스 [System::Collections::Generic](../../)
* 라이브러리 [Aspose.Slides](../../../)