...
description: "딕셔너리 키 목록을 구현합니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여 할당해야 합니다. 스택에 직접 생성하거나 operator new를 사용해 이 타입의 인스턴스를 만들지 마십시오. 이렇게 하면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 래핑하고 해당 포인터를 함수 인자로 전달하세요."
type: docs
weight: 14
url: /ko/system.collections.generic/_keylist/
---
## _KeyList 클래스

딕셔너리 키의 목록을 구현합니다. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여 할당해야 합니다. 스택에 직접 생성하거나 operator new를 사용해 이 타입의 인스턴스를 만들지 마십시오. 그렇지 않으면 런타임 오류 및/또는 어설션 오류가 발생합니다. 이 클래스를 항상 [System::SmartPtr](../../system/smartptr/) 포인터로 래핑하고 해당 포인터를 함수 인자로 전달하세요.
...
| 메서드 | 설명 |
| --- | --- |
| [_KeyCollection](../_keycollection/_keycollection/)(const typename Dict::Ptr\&) | 지정된 딕셔너리를 참조하는 컬렉션을 초기화합니다. |
| [_KeyList](./_keylist/)(const typename Dict::Ptr\&) | 지정된 딕셔너리를 참조하는 컬렉션을 초기화합니다. |
...
| **bool** [Contains](./contains/)(const [TKey](../_keycollection/tkey/)\&) const override | 지정된 키가 컬렉션에 존재하는지 확인합니다. |
...
| [SharedPtr](../../system/sharedptr/)\<[IEnumerator](../ienumerator/)\<[TKey](../_keycollection/tkey/)\>\> [GetEnumerator](../_keycollection/getenumerator/)() override | 키를 순회하는 열거자를 반환합니다. |
...
| [TKey](../_keycollection/tkey/) [idx_get](./idx_get/)(int) const override | 지정된 위치의 키를 반환합니다. |
| void [idx_set](../ikvcollection/idx_set/)(int, T) override | 설정자 함수입니다. |
| int [IndexOf](../ikvcollection/indexof/)(const T\&) const override | 컨테이너 내 항목의 인덱스를 반환합니다. |
...
| [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<ResultType\>\> [LINQ_Select](../ienumerable/linq_select/)(const [Func](../../system/func/)\<T, **int32_t**, ResultType\>\&) | 각 요소의 인덱스를 포함하여 시퀀스의 요소를 새로운 형태로 변환합니다. |
...
| [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<T\>\> [LINQ_Skip](../ienumerable/linq_skip/)(**int32_t**) | 시퀀스 시작에서 지정된 개수만큼 연속 요소를 건너뛰고 나머지를 반환합니다. |
| [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<T\>\> [LINQ_Take](../ienumerable/linq_take/)(**int32_t**) | 시퀀스 시작에서 지정된 개수만큼 연속 요소를 반환합니다. |
...
| void [SetTemplateWeakPtr](../basekvcollection/settemplateweakptr/)(**uint32_t**) override | 컴파일을 가능하게 하지만, 이 구조체는 데이터를 소유하지 않으므로 실제로는 아무것도 하지 않습니다. |
...
| System::Details::VirtualizedIteratorBase\<[TKey](../_keycollection/tkey/)\> * [virtualizeBeginConstIterator](../_keycollection/virtualizebeginconstiterator/)() const override | 현재 컨테이너의 begin const 반복자 구현을 반환합니다. |
| System::Details::VirtualizedIteratorBase\<[TKey](../_keycollection/tkey/)\> * [virtualizeBeginIterator](../_keycollection/virtualizebeginiterator/)() override | 현재 컨테이너의 begin 반복자 구현을 반환합니다. |
| System::Details::VirtualizedIteratorBase\<[TKey](../_keycollection/tkey/)\> * [virtualizeEndConstIterator](../_keycollection/virtualizeendconstiterator/)() const override | 현재 컨테이너의 end const 반복자 구현을 반환합니다. |
| System::Details::VirtualizedIteratorBase\<[TKey](../_keycollection/tkey/)\> * [virtualizeEndIterator](../_keycollection/virtualizeenditerator/)() override | 현재 컨테이너의 end 반복자 구현을 반환합니다. |
...
* 클래스 [_KeyCollection](../_keycollection/)
