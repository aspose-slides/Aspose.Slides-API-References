...
description: "딕셔너리 값의 목록을 구현합니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여만 할당해야 합니다. 스택에 이 유형의 인스턴스를 직접 생성하거나 operator new를 사용하면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 래핑하고 해당 포인터를 함수 인자로 전달하십시오."
type: docs
weight: 40
url: /ko/system.collections.generic/_valuelist/
---
## _ValueList 클래스

딕셔너리 값의 목록을 구현합니다. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여만 할당해야 합니다. 스택에 이 유형의 인스턴스를 직접 생성하거나 operator new를 사용해서는 안 되며, 그렇지 않으면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터에 래핑하고 해당 포인터를 함수 인자로 전달하십시오.
...
| 메서드 | 설명 |
| --- | --- |
| [_ValueCollection](../_valuecollection/_valuecollection/)(const typename Dict::Ptr&) | 지정된 딕셔너리를 참조하는 컬렉션을 초기화합니다. |
| [_ValueList](./_valuelist/)(const typename Dict::Ptr&) | 지정된 딕셔너리를 참조하는 컬렉션을 초기화합니다. |
...
| **bool** [Contains](../_valuecollection/contains/)(const [TValue](../_valuecollection/tvalue/)&) const override | 컨테이너에 항목이 존재하는지 확인합니다. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const&, **float** const&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도 (NaN 포함) 같지 않지만, 두 NaN이 동일하게 간주되는 C# 스타일 부동소수점 비교를 에뮬레이트합니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const&, **double** const&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도 (NaN 포함) 같지 않지만, 두 NaN이 동일하게 간주되는 C# 스타일 부동소수점 비교를 에뮬레이트합니다. |
...
| [SharedPtr](../../system/sharedptr/)<[IEnumerator](../ienumerator/)<[TValue](../_valuecollection/tvalue/)>> [GetEnumerator](../_valuecollection/getenumerator/)() override | 값을 순회하는 열거자를 반환합니다. |
...
| virtual [TValue](../_valuecollection/tvalue/) [idx_get](./idx_get/)(int) const | 지정된 위치의 값을 반환합니다. |
...
| [SharedPtr](../../system/sharedptr/)<[Linq::IOrderedEnumerable](../../system.linq/iorderedenumerable/)<T>> [LINQ_OrderBy](../ienumerable/linq_orderby/)(const [Func](../../system/func/)<T, Key>&) | keySelector에 의해 선택된 키 값을 기준으로 시퀀스의 요소를 오름차순으로 정렬합니다. |
| [SharedPtr](../../system/sharedptr/)<[Linq::IOrderedEnumerable](../../system.linq/iorderedenumerable/)<Source>> [LINQ_OrderBy](../ienumerable/linq_orderby/)(const [Func](../../system/func/)<Source, Key>&) |  |
| [SharedPtr](../../system/sharedptr/)<[Linq::IOrderedEnumerable](../../system.linq/iorderedenumerable/)<T>> [LINQ_OrderByDescending](../ienumerable/linq_orderbydescending/)(const [Func](../../system/func/)<T, Key>&) | keySelector에 의해 선택된 키 값을 기준으로 시퀀스의 요소를 내림차순으로 정렬합니다. |
| [SharedPtr](../../system/sharedptr/)<[Linq::IOrderedEnumerable](../../system.linq/iorderedenumerable/)<Source>> [LINQ_OrderByDescending](../ienumerable/linq_orderbydescending/)(const [Func](../../system/func/)<Source, Key>&) |  |
| [SharedPtr](../../system/sharedptr/)<[IEnumerable](../ienumerable/)<T>> [LINQ_Reverse](../ienumerable/linq_reverse/)() | 시퀀스의 요소 순서를 반대로 합니다. |
...
| [Object](../../system/object/)& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const&) | 할당 연산자. 실제로 아무것도 복사하지 않으며, 새로운 객체를 초기화하고 하위 클래스의 복사 생성을 가능하게 합니다. |
...
| void [RemoveAt](../ikvcollection/removeat/)(int) override | 지정된 위치에서 항목을 제거합니다. |
| int [RemovedSharedRefs](../../system/object/removedsharedrefs/)(int) | 지정된 값만큼 공유 레퍼런스 카운트를 감소시킵니다. |
| void [SetTemplateWeakPtr](../basekvcollection/settemplateweakptr/)(**uint32_t**) override | 컴파일을 가능하게 하지만, 이 구조체가 데이터를 소유하지 않으므로 실제 동작은 없습니다. |
...
| System::Details::VirtualizedIteratorBase<[TValue](../_valuecollection/tvalue/)> * [virtualizeBeginConstIterator](../_valuecollection/virtualizebeginconstiterator/)() const override | 현재 컨테이너의 begin const 반복자 구현을 반환합니다. |
| System::Details::VirtualizedIteratorBase<[TValue](../_valuecollection/tvalue/)> * [virtualizeBeginIterator](../_valuecollection/virtualizebeginiterator/)() override | 현재 컨테이너의 begin 반복자 구현을 반환합니다. |
| System::Details::VirtualizedIteratorBase<[TValue](../_valuecollection/tvalue/)> * [virtualizeEndConstIterator](../_valuecollection/virtualizeendconstiterator/)() const override | 현재 컨테이너의 end const 반복자 구현을 반환합니다. |
| System::Details::VirtualizedIteratorBase<[TValue](../_valuecollection/tvalue/)> * [virtualizeEndIterator](../_valuecollection/virtualizeenditerator/)() override | 현재 컨테이너의 end 반복자 구현을 반환합니다. |
...
* 클래스 [_ValueCollection](../_valuecollection/)
