...
description: 액세스 연산자를 갖는 리스트 포인터입니다. 이 타입은 다른 객체의 삭제를 관리하는 포인터입니다. 스택에 할당하고 값이나 const 레퍼런스로 함수에 전달해야 합니다.
type: docs
weight: 456
url: /ko/system.collections.generic/listptr/
---
## ListPtr 클래스

[List](../list/) 포인터는 액세스 연산자를 제공합니다. 이 타입은 다른 객체의 삭제를 관리하는 포인터입니다. 스택에 할당하고 값이나 const 레퍼런스로 함수에 전달해야 합니다.
...
| [SmartPtr](../../system/smartptr/)\<Y\> [const_pointer_cast](../../system/smartptr/const_pointer_cast/)() const | const_cast를 사용하여 가리키는 객체를 다른 타입으로 캐스팅합니다. |
| [SmartPtr](../../system/smartptr/)\<Y\> [dynamic_pointer_cast](../../system/smartptr/dynamic_pointer_cast/)() const | dynamic_cast를 사용하여 가리키는 객체를 다른 타입으로 캐스팅합니다. |
...
| [Pointee_](../../system/smartptr/pointee_/) * [get_shared](../../system/smartptr/get_shared/)() const | 가리키는 객체를 가져오지만, 포인터가 공유 모드인지 확인합니다. |
| int [get_shared_count](../../system/smartptr/get_shared_count/)() const | 참조된 객체에 존재하는 공유 포인터 수를 현재 포인터를 포함하여 반환합니다. 현재 포인터가 공유 모드인지 확인합니다. |
| int [GetHashCode](../../system/smartptr/gethashcode/)() const | 가리키는 객체에서 [GetHashCode()](../../system/smartptr/gethashcode/)를 호출합니다. |
| T * [GetObjectNotNull](../../system/smartptr/getobjectnotnull/)() const | 현재 참조된 객체를 가져옵니다(존재하는 경우). 없으면 예외를 발생시킵니다. |
| [Object](../../system/object/) * [GetObjectOrNull](../../system/smartptr/getobjectornull/)() const | 가리키는 객체를 가져옵니다(존재하는 경우) 또는 nullptr를 반환합니다. [get()](../../system/smartptr/get/)와 동일합니다. |
| [Object](../../system/object/) * [GetObjectOwner](../../system/smartptr/getobjectowner/)() const | 참조된 객체를 가져옵니다. |
| [Pointee_](../../system/smartptr/pointee_/) * [GetPointer](../../system/smartptr/getpointer/)() const | 가리키는 객체를 가져옵니다(존재하는 경우) 또는 nullptr를 반환합니다. [get()](../../system/smartptr/get/)와 동일합니다. |
| **bool** [Is](../../system/smartptr/is/)(const [System::TypeInfo](../../system/typeinfo/)\&) const | 가리키는 객체가 특정 타입 또는 그 자식 타입인지 확인합니다. C#의 'is' 의미를 따릅니다. |
...
| [ListPtr](./listptr/)(std::nullptr_t) | null-pointer를 초기화합니다. |
| [ListPtr](./listptr/)(const [SharedPtr](../../system/sharedptr/)\<[List](../list/)\<T\>\>\&) | 지정된 리스트로 포인터를 초기화합니다. |
...
| [Pointee_](../../system/smartptr/pointee_/)\& [operator*](../../system/smartptr/operator_star/)() const | 가리키는 객체에 대한 참조를 반환합니다. 포인터가 null이 아닌지 확인합니다. |
...
| [SmartPtr_](../../system/smartptr/smartptr_/)\& [operator=](../../system/smartptr/operator_equal/)([SmartPtr_](../../system/smartptr/smartptr_/)\&&) | [SmartPtr](../../system/smartptr/) 객체를 이동 할당합니다. x는 사용 불가능하게 됩니다. |
...
| **bool** [operator==](./operator_equal_equal/)(std::nullptr_t) const | [List](../list/) 포인터가 null인지 확인합니다. |
| std::vector\<T\>::reference [operator[]](./operator[]/)(int) | 접근자. |
| std::vector\<T\>::const_reference [operator[]](./operator[]/)(int) const | 접근자. |
| [SmartPtr_](../../system/smartptr/smartptr_/) [RemoveAliasing](../../system/smartptr/removealiasing/)() const | 포인터에서 별칭(별칭 생성자를 통해 생성된)을 제거하고, 가리키는 동일 객체를 공유 모드이면 관리하고 약한 모드이면 추적하도록 합니다. |
...
| [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<Q\>\&, [SmartPtrMode](../../system/smartptrmode/)) | [SmartPtr](../../system/smartptr/) 객체를 복사 생성합니다. 두 포인터는 이후 동일 객체를 가리키며, 허용되는 경우 형 변환을 수행합니다. |
| [SmartPtr](../../system/smartptr/smartptr/)([SmartPtr_](../../system/smartptr/smartptr_/)\&&, [SmartPtrMode](../../system/smartptrmode/)) | [SmartPtr](../../system/smartptr/) 객체를 이동 생성합니다. 두 포인터가 동일 모드인 경우 실제로 두 포인터를 교환합니다. 호출 후 x는 사용 불가능할 수 있습니다. |
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<[Array](../../system/array/)\<Y\>\>\&, [SmartPtrMode](../../system/smartptrmode/)) | 다른 타입의 새 배열을 생성하여 참조된 배열의 타입을 변환합니다. C#에서는 지원되지만 C++에서는 지원되지 않는 배열 타입 캐스트가 필요한 경우에 유용합니다. |
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const Y\&) | 빈 배열을 초기화합니다. 일부 C# 코드 구문을 변환하는 데 사용됩니다. |
| [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<P\>\&, [Pointee_](../../system/smartptr/pointee_/) *, [SmartPtrMode](../../system/smartptrmode/)) | [SmartPtr](../../system/smartptr/)를 생성합니다. 이는 ptr의 초기값과 소유권 정보를 공유하지만, 관련 없고 관리되지 않는 포인터 p를 보유합니다. |
| [SmartPtr](../../system/smartptr/)\<Y\> [static_pointer_cast](../../system/smartptr/static_pointer_cast/)() const | static_cast를 사용하여 가리키는 객체를 다른 타입으로 캐스팅합니다. |
| [SmartPtr](../../system/smartptr/)\<[Object](../../system/object/)\> [ToObjectPtr](../../system/smartptr/toobjectptr/)() const | 어떠한 포인터 타입이든 [Object](../../system/object/) 포인터로 변환합니다. Pointee_ 타입이 완전할 필요는 없습니다. |
...
| [~SmartPtr](../../system/smartptr/~smartptr/)() | [SmartPtr](../../system/smartptr/) 객체를 파괴합니다. 필요시 가리키는 객체의 레퍼런스 카운터를 감소시키고 객체를 삭제합니다. |
