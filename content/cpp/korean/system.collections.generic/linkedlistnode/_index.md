...
description: "연결 리스트의 노드입니다. 연결 리스트에 래핑된 std::list 이터레이터에 대한 래퍼를 구현합니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여 할당해야 합니다. 스택에 이 타입의 인스턴스를 생성하거나 operator new를 사용하면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 래핑하고 해당 포인터를 함수 인수로 전달하십시오."
type: docs
weight: 417
url: /ko/system.collections.generic/linkedlistnode/
---
## LinkedListNode 클래스

연결 리스트의 노드입니다. 연결 리스트에 래핑된 std::list의 이터레이터에 대한 래퍼를 구현합니다. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수만 사용하여 할당해야 합니다. 스택에 이 타입의 인스턴스를 생성하거나 operator new를 사용해서는 안 됩니다. 그렇게 하면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터로 래핑하고, 해당 포인터를 인수로 함수에 전달하십시오.
...
| [SharedPtr](../../system/sharedptr/)\<[LinkedList](../linkedlist/)\<T\>\> [get_List](./get_list/)() const | 포함된 리스트를 반환합니다. |
| [SharedPtr](../../system/sharedptr/)\<[LinkedListNode](./)\<T\>\> [get_Next](./get_next/)() const | 다음 노드를 반환합니다. |
| [SharedPtr](../../system/sharedptr/)\<[LinkedListNode](./)\<T\>\> [get_Previous](./get_previous/)() const | 이전 노드를 반환합니다. |
| T [get_Value](./get_value/)() const | 저장된 값을 반환합니다. |
...
| void [set_Value](./set_value/)(const T\&) | 저장된 값을 설정합니다. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | n번째 템플릿 인자를 강한 포인터가 아닌 약한 포인터로 설정합니다. 컨테이너의 포인터를 약한 모드로 전환할 수 있게 합니다. |
