---
title: GetMonthsInYear()
second_title: Aspose.Slides for C++ API 레퍼런스
description: 지정된 연도의 월 수를 반환합니다.
type: docs
weight: 92
url: /ko/system.globalization/gregoriancalendar/getmonthsinyear/
---
## GregorianCalendar::GetMonthsInYear(int, int) const 메서드


지정된 연도의 월 수를 반환합니다.

```cpp
int System::Globalization::GregorianCalendar::GetMonthsInYear(int year, int era) const override
```


### 인수

| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| year | int | 월 수를 가져올 연도. |
| era | int | 시대. |

### 반환값

지정된 연도와 지정된 시대에 해당하는 월 수.

## GregorianCalendar::GetMonthsInYear(int) const 메서드


RTTI 정보.

```cpp
virtual int System::Globalization::Calendar::GetMonthsInYear(int year) const
```

## GregorianCalendar::GetMonthsInYear(int, int) const 메서드


RTTI 정보.

```cpp
virtual int System::Globalization::Calendar::GetMonthsInYear(int year, int era) const
```

## 참조

* 클래스 [GregorianCalendar](../)
* 네임스페이스 [System::Globalization](../../)
* 라이브러리 [Aspose.Slides](../../../)