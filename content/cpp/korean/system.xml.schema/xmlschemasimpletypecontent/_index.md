...
description: 간단한 유형 콘텐츠 클래스의 기본 클래스입니다.
type: docs
weight: 846
url: /ko/system.xml.schema/xmlschemasimpletypecontent/
---
## XmlSchemaSimpleTypeContent 클래스

간단한 유형 콘텐츠 클래스의 기본 클래스입니다.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값(또는 NaN)과도 같지 않지만, 두 NaN을 동일하게 간주하는 C# 스타일 부동 소수점 비교를 에뮬레이트합니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값(또는 NaN)과도 같지 않지만, 두 NaN을 동일하게 간주하는 C# 스타일 부동 소수점 비교를 에뮬레이트합니다. |
...
이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여 할당해야 합니다. 스택이나 operator new를 사용해 이 유형의 인스턴스를 생성하지 마십시오. 이렇게 하면 런타임 오류 및/또는 assert 오류가 발생할 수 있습니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터로 감싸고, 해당 포인터를 인수로 함수에 전달하십시오. 
