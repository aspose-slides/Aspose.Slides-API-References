...
description: XML 스키마에서 World Wide Web Consortium (W3C)이 지정한 length facet를 나타냅니다. 이 클래스는 데이터 유형의 simpleType 요소 길이에 대한 제한을 지정하는 데 사용할 수 있습니다.
type: docs
weight: 560
url: /ko/system.xml.schema/xmlschemalengthfacet/
---
## XmlSchemaLengthFacet 클래스

Represents the **length** facet from XML [Schema](../) as specified by the World Wide [Web](../../system.web/) 컨소시엄 (W3C). This 클래스 can be used to specify a restriction on the length of a **simpleType** element on the data type.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | 두 NaN을 동일하게 간주하는 C# 스타일 부동소수점 비교를 에뮬레이트합니다. IEC 60559:1989에 따르면 NaN은 모든 값과, NaN 포함, 같지 않지만 이를 무시합니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | 두 NaN을 동일하게 간주하는 C# 스타일 부동소수점 비교를 에뮬레이트합니다. IEC 60559:1989에 따르면 NaN은 모든 값과, NaN 포함, 같지 않지만 이를 무시합니다. |
...
| [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[XmlAttribute](../../system.xml/xmlattribute/)\>\> [get_UnhandledAttributes](../xmlschemaannotated/get_unhandledattributes/)() | 현재 schema의 target namespace에 속하지 않는 qualified attributes를 반환합니다. |
...
| void [set_UnhandledAttributes](../xmlschemaannotated/set_unhandledattributes/)(const [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[XmlAttribute](../../system.xml/xmlattribute/)\>\>\&) | 현재 schema의 target namespace에 속하지 않는 qualified attributes를 설정합니다. |
| void [set_Value](../xmlschemafacet/set_value/)(const [String](../../system/string/)\&) | facet의 **value** 속성을 설정합니다. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | n번째 템플릿 인자를 weak 포인터(공유 포인터가 아닌)로 설정합니다. 컨테이너 내 포인터를 weak 모드로 전환할 수 있습니다. |
...
|  [XmlSchemaLengthFacet](./xmlschemalengthfacet/)() | [XmlSchemaLengthFacet](./) 클래스의 새 인스턴스를 초기화합니다. |
