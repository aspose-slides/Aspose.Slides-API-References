...
description: World Wide Web Consortium (W3C)에서 정의한 XML 스키마의 ref 속성을 가진 group 요소를 나타냅니다. 이 클래스는 스키마 수준에 정의된 group을 참조하는 복합 타입 내에서 사용됩니다.
type: docs
weight: 456
url: /ko/system.xml.schema/xmlschemagroupref/
---
## XmlSchemaGroupRef 클래스
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | 두 NaN이 IEC 60559:1989에 따르면 NaN은 어떠한 값과도 같지 않지만, C# 스타일 부동소수점 비교를 에뮬레이트하여 두 NaN을 동일하게 간주합니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | 두 NaN이 IEC 60559:1989에 따르면 NaN은 어떠한 값과도 같지 않지만, C# 스타일 부동소수점 비교를 에뮬레이트하여 두 NaN을 동일하게 간주합니다. |
...
| [String](../../system/string/) [get_MaxOccursString](../xmlschemaparticle/get_maxoccursstring/)() | 숫자를 문자열 값으로 반환합니다. 파티클이 발생할 수 있는 최대 횟수. |
| [Decimal](../../system/decimal/) [get_MinOccurs](../xmlschemaparticle/get_minoccurs/)() | 파티클이 발생할 최소 횟수를 반환합니다. |
| [String](../../system/string/) [get_MinOccursString](../xmlschemaparticle/get_minoccursstring/)() | 숫자를 문자열 값으로 반환합니다. 파티클이 발생할 최소 횟수. |
...
| [SharedPtr](../../system/sharedptr/)\<[XmlSchemaGroupBase](../xmlschemagroupbase/)\> [get_Particle](./get_particle/)() | **Particle** 값의 컴파일 후 해석을 보유하는 [XmlSchemaChoice](../xmlschemachoice/), [XmlSchemaAll](../xmlschemaall/) 또는 [XmlSchemaSequence](../xmlschemasequence/) 클래스 중 하나를 반환합니다. |
| [SharedPtr](../../system/sharedptr/)\<[XmlQualifiedName](../../system.xml/xmlqualifiedname/)\> [get_RefName](./get_refname/)() | 이 스키마(또는 지정된 네임스페이스가 가리키는 다른 스키마)에서 정의된 그룹의 이름을 반환합니다. |
...
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | 대입 연산자. 실제로 아무 것도 복사하지 않으며, 새로운 객체를 초기화하고 서브클래스 복사 생성을 가능하게 합니다. |
...
| void [set_MaxOccursString](../xmlschemaparticle/set_maxoccursstring/)(const [String](../../system/string/)\&) | 숫자를 문자열 값으로 설정합니다. 파티클이 발생할 최대 횟수. |
| void [set_MinOccurs](../xmlschemaparticle/set_minoccurs/)([Decimal](../../system/decimal/)) | 파티클이 발생할 최소 횟수를 설정합니다. |
| void [set_MinOccursString](../xmlschemaparticle/set_minoccursstring/)(const [String](../../system/string/)\&) | 숫자를 문자열 값으로 설정합니다. 파티클이 발생할 최소 횟수. |
...
| void [set_RefName](./set_refname/)(const [SharedPtr](../../system/sharedptr/)\<[XmlQualifiedName](../../system.xml/xmlqualifiedname/)\>\&) | 이 스키마(또는 지정된 네임스페이스가 가리키는 다른 스키마)에서 정의된 그룹의 이름을 설정합니다. |
...
| 타입정의 | 설명 |
| --- | --- |
| [Ptr](./ptr/) | 이 클래스 인스턴스에 대한 공유 포인터에 대한 별칭입니다. |

## 비고

이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여 할당해야 합니다. 스택에 직접 생성하거나 operator new를 사용해 인스턴스를 만들면 런타임 오류 및/또는 어설션 결함이 발생합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터로 감싸고 해당 포인터를 인수로 함수에 전달하십시오. 
