...
description: "번역 후 TypeDescriptor를 사용하는 코드를 컴파일하기 위한 더미 클래스입니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여만 할당해야 합니다. 스택에 직접 인스턴스를 만들거나 operator new를 사용하면 런타임 오류 및/또는 어서션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 감싸고, 해당 포인터를 함수 인자로 전달하십시오."
type: docs
weight: 209
url: /ko/system.componentmodel/typedescriptor/
---
## TypeDescriptor 클래스

Dummy 클래스는 번역 후 TypeDescriptor를 사용하는 코드를 컴파일하기 위해 제공됩니다. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여만 할당해야 합니다. 스택에 직접 만들거나 operator new를 사용하여 인스턴스를 생성하면 런타임 오류 및/또는 어서션 오류가 발생합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터로 감싸고, 해당 포인터를 함수 인자로 전달하십시오.
...
| static [System::SharedPtr](../../system/sharedptr/)\<[TypeConverter](../typeconverter/)\> [GetConverter](./getconverter/)(const [TypeInfo](../../system/typeinfo/)\&) | RTTI 정보. |
