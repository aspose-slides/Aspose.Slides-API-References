...
description: "이 클래스의 인스턴스는 AsyncCompletedEventHandler 대리자에 인수로 전달됩니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여 할당해야 합니다. 스택에 직접 생성하거나 operator new를 사용해 인스턴스를 만들면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 래핑하고 해당 포인터를 인수로 함수에 전달하십시오."
type: docs
weight: 1
url: /ko/system.componentmodel/asynccompletedeventargs/
---
## AsyncCompletedEventArgs 클래스

이 클래스의 인스턴스는 AsyncCompletedEventHandler 대리자에 인수로 전달됩니다. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수로만 할당해야 합니다. 스택에 직접 생성하거나 operator new를 사용하여 인스턴스를 만들면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터에 래핑하고 이 포인터를 인수로 함수에 전달하십시오.
...
|  [AsyncCompletedEventArgs](./asynccompletedeventargs/)(const [System::Exception](../../system/exception/)\&, **bool**, const [System::SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\>\&) | 새로운 [System.ComponentModel.AsyncCompletedEventArgs](./) 클래스 인스턴스를 초기화합니다. |
...
| **bool** [get_Cancelled](./get_cancelled/)() const | 비동기 작업이 취소되었는지 여부를 나타내는 값을 가져옵니다. 백그라운드 작업이 취소된 경우 true, 그렇지 않으면 false. 기본값은 false입니다. |
| const [System::Exception](../../system/exception/)\& [get_Error](./get_error/)() const | 비동기 작업 중 발생한 오류를 나타내는 값을 가져옵니다. |
| [System::SharedPtr](../../system/sharedptr/)\<[Object](../../system/object/)\> [get_UserState](./get_userstate/)() const | 비동기 작업의 고유 식별자를 가져옵니다. 값이 설정되지 않은 경우 null을 반환합니다. |
