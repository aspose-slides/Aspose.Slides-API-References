---
title: virtualizeEndIterator()
second_title: Aspose.Slides for C++ API 참조
description: 현재 컨테이너에 대한 종료 반복자의 구현을 가져옵니다.
type: docs
weight: 183
url: /ko/system.net/cookiecollection/virtualizeenditerator/
---
## CookieCollection::virtualizeEndIterator() 메서드

현재 컨테이너에 대한 종료 반복자의 구현을 가져옵니다.

```cpp
System::Details::VirtualizedIteratorBase<System::SharedPtr<Cookie>> * System::Net::CookieCollection::virtualizeEndIterator() override
```

### 반환값

새로 생성된 반복자 커널 객체에 대한 포인터.

## 참고

* Typedef [SharedPtr](../../../system/sharedptr/)
* 클래스 [Cookie](../../cookie/)
* 클래스 [CookieCollection](../)
* 네임스페이스 [System::Net](../../)
* 라이브러리 [Aspose.Slides](../../../)