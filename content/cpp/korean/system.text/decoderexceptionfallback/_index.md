...
description: "예외를 발생시키는 폴백 전략을 제공합니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여 할당해야 합니다. 스택에 직접 생성하거나 operator new를 사용해 인스턴스를 만들지 말아야 하며, 그렇지 않으면 런타임 오류 및/또는 어설션 오류가 발생할 수 있습니다. 항상 이 클래스를 System::SmartPtr 포인터로 래핑하고, 해당 포인터를 함수 인수로 전달하십시오."
type: docs
weight: 27
url: /ko/system.text/decoderexceptionfallback/
---
## DecoderExceptionFallback 클래스

예외를 발생시키는 폴백 전략을 제공합니다. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여 할당해야 합니다. 스택에 직접 생성하거나 operator new를 사용해 인스턴스를 만들지 마십시오. 이렇게 하면 런타임 오류 및/또는 어설션 오류가 발생할 수 있습니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터에 래핑하고, 해당 포인터를 함수 인수로 전달하십시오.
...
| 메서드 | 설명 |
| --- | --- |
| [DecoderFallbackBufferPtr](../../system/decoderfallbackbufferptr/) [CreateFallbackBuffer](./createfallbackbuffer/)() override | 폴백 버퍼를 생성합니다. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | 두 NaN이 IEC 60559:1989에 따르면 NaN은 어떠한 값과도( NaN 포함) 같지 않음에도 불구하고 동일한 것으로 간주되는 C# 스타일 부동소수점 비교를 에뮬레이트합니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | 두 NaN이 IEC 60559:1989에 따르면 NaN은 어떠한 값과도( NaN 포함) 같지 않음에도 불구하고 동일한 것으로 간주되는 C# 스타일 부동소수점 비교를 에뮬레이트합니다. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | 내부 용도 전용입니다. |
| static const [DecoderFallbackPtr](../../system/decoderfallbackptr/) [get_ExceptionFallback](../decoderfallback/get_exceptionfallback/)() | 기본 예외 폴백 구현을 가져옵니다. |
| int [get_MaxCharCount](./get_maxcharcount/)() const override | 인스턴스가 반환할 수 있는 최대 문자 수를 가져옵니다. |
| static const [DecoderFallbackPtr](../../system/decoderfallbackptr/) [get_ReplacementFallback](../decoderfallback/get_replacementfallback/)() | 기본 교체 폴백 구현을 가져옵니다. |
| static const [DecoderFallbackPtr](../../system/decoderfallbackptr/) [get_StandardSafeFallback](../decoderfallback/get_standardsafefallback/)() | 기본 표준 안전 폴백 구현을 가져옵니다. |
