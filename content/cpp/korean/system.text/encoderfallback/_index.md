...
description: "인코딩 오류를 처리하기 위한 fallback API를 제공합니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여 할당해야 합니다. 스택에 직접 인스턴스를 생성하거나 operator new를 사용하지 마십시오. 그렇지 않으면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 감싸고 해당 포인터를 인수로 함수에 전달하십시오."
type: docs
weight: 170
url: /ko/system.text/encoderfallback/
---
## EncoderFallback 클래스

인코딩 오류를 처리하기 위한 fallback API를 제공합니다. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여 할당해야 합니다. 스택에 직접 만들거나 operator new를 사용해 인스턴스를 생성하면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터로 감싸고, 해당 포인터를 인수로 함수에 전달하십시오.
...
| 메서드 | 설명 |
| --- | --- |
| virtual [EncoderFallbackBufferPtr](../../system/encoderfallbackbufferptr/) [CreateFallbackBuffer](./createfallbackbuffer/)() | fallback 알고리즘과 연결된 버퍼를 가져옵니다. |
...
| static const [EncoderFallbackPtr](../../system/encoderfallbackptr/) [get_ExceptionFallback](./get_exceptionfallback/)() | 기본 예외 fallback 구현을 가져옵니다. |
| virtual int [get_MaxCharCount](./get_maxcharcount/)() const | fallback이 반환할 수 있는 최대 문자 수를 가져옵니다. |
| static const [EncoderFallbackPtr](../../system/encoderfallbackptr/) [get_ReplacementFallback](./get_replacementfallback/)() | 기본 교체 fallback 구현을 가져옵니다. |
| static const [EncoderFallbackPtr](../../system/encoderfallbackptr/) [get_StandardSafeFallback](./get_standardsafefallback/)() | 기본 표준 안전 fallback 구현을 가져옵니다. |
