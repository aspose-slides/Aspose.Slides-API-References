...
type: docs
weight: 66
url: /ko/system.text/icuencoder/convert/
---
## ICUEncoder::Convert(ArrayPtr\<char_t\>, int, int, ArrayPtr\<uint8_t\>, int, int, bool, int\&, int\&, bool\&) 메서드
...
| bytesUsed | int\& | 쓰기된 바이트 수를 저장할 변수에 대한 참조. |
| completed | **bool**\& | 입력 버퍼가 소진되면 true, 그렇지 않으면 false로 설정될 변수에 대한 참조. |

## ICUEncoder::Convert(const char_t *, int, uint8_t *, int, bool, int\&, int\&, bool\&) 메서드
