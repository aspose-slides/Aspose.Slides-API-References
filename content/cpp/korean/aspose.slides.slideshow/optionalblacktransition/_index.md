...
| **bool** [get_FromBlack](./get_fromblack/)() override | 이 속성은 전환이 검은 화면에서 시작할지 여부를 지정합니다(그런 다음 새 슬라이드가 검은 화면 위로 전환됩니다). 읽기 **bool**. |
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | 객체와 연결된 레퍼런스 카운터 데이터 구조를 가져옵니다. |
| **int32_t** [GetHashCode](../transitionvaluebase/gethashcode/)() const override | 특정 유형에 대한 해시 함수 역할을 하며, 해시 알고리즘 및 해시 테이블과 같은 데이터 구조에 사용하기에 적합합니다. |
...
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | 할당 연산자. 실제로는 아무 것도 복사하지 않고 새 객체를 초기화하며 하위 클래스 복사 구성을 가능하게 합니다. |
...
| void [set_FromBlack](./set_fromblack/)(**bool**) override | 이 속성은 전환이 검은 화면에서 시작할지 여부를 지정합니다(그런 다음 새 슬라이드가 검은 화면 위로 전환됩니다). 쓰기 **bool**. |
...
* 클래스 [IOptionalBlackTransition](../ioptionalblacktransition/)
