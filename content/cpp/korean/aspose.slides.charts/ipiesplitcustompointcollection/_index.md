...
description: 사용자 정의 분할이 적용된 파이-바 차트 또는 파이-파이 차트의 두 번째 파이 또는 바에 그려질 포인트 컬렉션을 나타냅니다.
type: docs
weight: 1158
url: /ko/aspose.slides.charts/ipiesplitcustompointcollection/
---
## IPieSplitCustomPointCollection 클래스
...
| 메서드 | 설명 |
| --- | --- |
| virtual void [Add](./add/)(const **int32_t**\&) | 부(parent series points collection에서 해당 인덱스로 데이터 포인트를 추가합니다. |
| virtual void [Add](../../system.collections.generic/icollection/add/)(const T\&) | 컬렉션에 요소를 추가합니다. |
| [iterator](../../system.collections.generic/ienumerable/iterator/) [begin](../../system.collections.generic/ienumerable/begin/)() | 컬렉션의 첫 번째 요소(있을 경우)를 가리키는 반복자를 가져옵니다. 이 반복자는 [GetEnumerator()](../../system.collections.generic/ienumerable/getenumerator/)가 T의 복사 객체를 반환하기 때문에 참조된 객체를 변경하는 데 사용할 수 없습니다. |
| [const_iterator](../../system.collections.generic/ienumerable/const_iterator/) [begin](../../system.collections.generic/ienumerable/begin/)() const | 컬렉션의 const 한정 인스턴스에서 첫 번째 요소(있을 경우)를 가리키는 반복자를 가져옵니다. |
| [const_iterator](../../system.collections.generic/ienumerable/const_iterator/) [cbegin](../../system.collections.generic/ienumerable/cbegin/)() const | 컬렉션에서 첫 번째 const 한정 요소(있을 경우)를 가리키는 반복자를 가져옵니다. |
| [const_iterator](../../system.collections.generic/ienumerable/const_iterator/) [cend](../../system.collections.generic/ienumerable/cend/)() const | 컬렉션의 마지막 const 한정 요소(있을 경우) 바로 뒤를 가리키는 반복자를 가져옵니다. |
...
| [iterator](../../system.collections.generic/ienumerable/iterator/) [end](../../system.collections.generic/ienumerable/end/)() | 컬렉션의 마지막 요소(있을 경우) 바로 뒤를 가리키는 반복자를 가져옵니다. 이 반복자는 [GetEnumerator()](../../system.collections.generic/ienumerable/getenumerator/)가 T의 복사 객체를 반환하기 때문에 참조된 객체를 변경하는 데 사용할 수 없습니다. |
| [const_iterator](../../system.collections.generic/ienumerable/const_iterator/) [end](../../system.collections.generic/ienumerable/end/)() const | 컬렉션의 const 한정 인스턴스에서 마지막 요소(있을 경우) 바로 뒤를 가리키는 반복자를 가져옵니다. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | C# 스타일 부동소수점 비교를 에뮬레이션합니다. 두 NaN은 IEC 60559:1989에 따르면 NaN은 어떤 값과도 같지 않음에도 불구하고 동일한 것으로 간주됩니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | C# 스타일 부동소수점 비교를 에뮬레이션합니다. 두 NaN은 IEC 60559:1989에 따르면 NaN은 어떤 값과도 같지 않음에도 불구하고 동일한 것으로 간주됩니다. |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IChartDataPoint](../ichartdatapoint/)\> [idx_get](./idx_get/)(**int32_t**) | 인덱스로 차트 데이터 포인트를 반환합니다. |
...
| virtual void [Remove](./remove/)(**int32_t**) | 부모 시리즈 포인트 컬렉션에서 인덱스로 컬렉션 항목을 제거합니다. |
