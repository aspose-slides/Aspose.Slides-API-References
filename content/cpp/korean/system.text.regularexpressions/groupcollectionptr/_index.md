...
description: 그룹 컬렉션 포인터. 이 타입은 다른 객체의 삭제를 관리하는 포인터입니다. 스택에 할당하고 값이나 const 레퍼런스로 함수에 전달해야 합니다.
type: docs
weight: 53
url: /ko/system.text.regularexpressions/groupcollectionptr/
---
## GroupCollectionPtr 클래스

[Group](../group/) 컬렉션 포인터. 이 타입은 다른 객체의 삭제를 관리하는 포인터입니다. 스택에 할당하고 값 또는 const 참조로 함수에 전달해야 합니다.
...
| int [get_shared_count](../../system/smartptr/get_shared_count/)() const | 참조된 객체에 존재하는 공유 포인터 수를 현재 포인터를 포함하여 반환합니다. 현재 포인터가 공유 모드임을 단언합니다. |
...
| [GroupCollectionPtr](./groupcollectionptr/)() | 널 포인터 생성자입니다. |
| [GroupCollectionPtr](./groupcollectionptr/)(const [SharedPtr](../../system/sharedptr/)\<[GroupCollection](../groupcollection/)\>\&) | 형 변환 생성자입니다. |
| **bool** [Is](../../system/smartptr/is/)(const [System::TypeInfo](../../system/typeinfo/)\&) const | 가리키는 객체가 특정 타입 혹은 그 하위 타입인지 확인합니다. C#의 'is' 의미와 동일합니다. |
| **bool** [IsAliasingPtr](../../system/smartptr/isaliasingptr/)() const | 포인터가 소유 객체가 아닌 다른 객체를 가리키는지 확인합니다(별칭 생성자로 생성된 경우). |
...
| explicit [operator bool](../../system/smartptr/operator_bool/)() const | 포인터가 널이 아닌지 확인합니다. |
| **bool** [operator!](../../system/smartptr/operator_not/)() const | 포인터가 널인지 확인합니다. |
| [Pointee_](../../system/smartptr/pointee_/)\& [operator*](../../system/smartptr/operator_star/)() const | 가리키는 객체에 대한 참조를 반환합니다. 포인터가 널이 아님을 단언합니다. |
...
| [GroupPtr](../groupptr/) [operator[]](./operator[]/)(size_t) const | [Group](../group/) 접근자. |
| [SmartPtr_](../../system/smartptr/smartptr_/) [RemoveAliasing](../../system/smartptr/removealiasing/)() const | 포인터에서 별칭(별칭 생성자로 생성된)을 제거하고, 가리키는 동일 객체를 (공유인 경우) 관리하거나 (약한 경우) 추적하도록 합니다. |
...
| [SmartPtr](../../system/smartptr/smartptr/)(std::nullptr_t, [SmartPtrMode](../../system/smartptrmode/)) | 필요한 모드의 널 포인터 [SmartPtr](../../system/smartptr/) 객체를 생성합니다. |
...
| [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr_](../../system/smartptr/smartptr_/)\&, [SmartPtrMode](../../system/smartptrmode/)) | [SmartPtr](../../system/smartptr/) 객체를 복사 생성합니다. 이후 두 포인터 모두 같은 객체를 가리킵니다. |
| [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<Q\>\&, [SmartPtrMode](../../system/smartptrmode/)) | [SmartPtr](../../system/smartptr/) 객체를 복사 생성합니다. 이후 두 포인터 모두 같은 객체를 가리킵니다. 허용되는 경우 타입 변환을 수행합니다. |
| [SmartPtr](../../system/smartptr/smartptr/)([SmartPtr_](../../system/smartptr/smartptr_/)\&&, [SmartPtrMode](../../system/smartptrmode/)) | [SmartPtr](../../system/smartptr/) 객체를 이동 생성합니다. 두 포인터가 동일 모드인 경우 실질적으로 포인터를 교환합니다. 호출 후 x는 사용 불가능할 수 있습니다. |
| explicit [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<[Array](../../system/array/)\<Y\>\>\&, [SmartPtrMode](../../system/smartptrmode/)) | 다른 타입의 새 배열을 생성하여 참조된 배열의 타입을 변환합니다. C#에서 지원되는 배열 타입 캐스트가 C++에서는 지원되지 않을 때 유용합니다. |
...
| static const [System::TypeInfo](../../system/typeinfo/)\& [Type](../../system/smartptr/type/)() | [System::TypeInfo](../../system/typeinfo/) 타입의 객체를 얻기 위한 단축키입니다. |
