...
description: 수학 요소의 논리적 박싱(패키징)을 지정합니다. 예를 들어, 박스된 객체는 정렬 포인트가 있거나 없거나 하는 연산자 에뮬레이터 역할을 하거나, 줄 바꿈 지점으로 사용되거나, 줄 바꿈이 허용되지 않도록 그룹화될 수 있습니다. 예를 들어, \"==\" 연산자는 줄 바꿈을 방지하기 위해 박스되어야 합니다.
type: docs
weight: 170
...
수학 요소의 논리적 박싱(포장)을 지정합니다. 예를 들어, 박스된 객체는 정렬 포인트가 있거나 없거나 하는 연산자 에뮬레이터 역할을 하거나, 줄 바꿈 지점으로 사용되거나, 줄 바꿈을 허용하지 않도록 그룹화될 수 있습니다. 예를 들어, "==" 연산자는 줄 바꿈을 방지하기 위해 박스되어야 합니다.
...
| 메서드 | 설명 |
| --- | --- |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathAccent](../imathaccent/)\> [Accent](../imathelement/accent/)(char16_t) | 이 요소 위에 억음 부호(이 요소의 최상단에 위치하는 문자)를 설정합니다. |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathDelimiter](../imathdelimiter/)\> [Enclose](../imathelement/enclose/)(char16_t, char16_t) | 이 요소를 괄호와 같은 지정된 문자 또는 다른 문자로 프레임화하여 둘러씁니다. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도(NaN 포함) 동등하지 않지만, C# 스타일 부동소수점 비교를 에뮬레이트하여 두 NaN을 동등하게 간주합니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도(NaN 포함) 동등하지 않지만, C# 스타일 부동소수점 비교를 에뮬레이트하여 두 NaN을 동등하게 간주합니다. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | 내부 용도로만 사용됩니다. |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathFunction](../imathfunction/)\> [Function](../imathelement/function/)([System::SharedPtr](../../system/sharedptr/)\<[IMathElement](../imathelement/)\>) | 이 인스턴스를 함수 이름으로 사용하여 인수를 받는 함수를 호출합니다. |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathFunction](../imathfunction/)\> [Function](../imathelement/function/)([System::String](../../system/string/)) | 이 인스턴스를 함수 이름으로 사용하여 인수를 받는 함수를 호출합니다. |
| virtual **bool** [get_AlignmentPoint](./get_alignmentpoint/)() | true이면, 이 연산자 에뮬레이터는 정렬 포인트 역할을 합니다; 즉, 다른 식에서 지정된 정렬 포인트와 정렬될 수 있습니다. 기본값: false |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathElement](../imathelement/)\> [get_Base](./get_base/)() | 기본 인수 |
| virtual **bool** [get_Differential](./get_differential/)() | 미분. true이면, 박스는 미분(예: 적분식에서 \\uD835\\uDC51\\uD835\\uDC65) 역할을 하며 수학적 미분에 적절한 수평 간격을 받습니다. 기본값: false |
| virtual **uint8_t** [get_ExplicitBreak](./get_explicitbreak/)() | 명시적 줄 바꿈은 Box 객체 시작 부분에 줄 바꿈이 있는지 지정하며, 줄이 박스 객체 시작 부분에서 래핑됩니다. 이전 수학 텍스트 줄의 연산자 번호를 지정하여 현재 수학 텍스트 줄의 정렬 포인트로 사용합니다. 가능한 값: 1..255 기본값: 0 (명시적 줄 바꿈 없음) |
| virtual **bool** [get_NoBreak](./get_nobreak/)() | 줄 바꿈 없음. 이 속성은 객체 박스의 "깨지지 않음" 속성을 지정합니다. true이면 박스 내부에서 줄 바꿈이 발생하지 않습니다. 이는 둘 이상의 이항 연산자로 구성된 연산자 에뮬레이터에 중요할 수 있습니다. 이 요소가 지정되지 않으면 박스 내부에서 줄 바꿈이 발생할 수 있습니다. 기본값: true |
| virtual **bool** [get_OperatorEmulator](./get_operatoremulator/)() | 연산자 에뮬레이터. true이면 박스와 그 내용이 단일 연산자처럼 동작하고 연산자의 속성을 상속합니다. 예를 들어, 해당 문자는 줄 바꿈 지점으로 사용될 수 있고 다른 연산자와 정렬될 수 있습니다. 연산자 에뮬레이터는 '=='와 같이 하나 이상의 글리프가 결합하여 연산자를 형성할 때 자주 사용됩니다. 기본값: false |
| virtual [System::ArrayPtr](../../system/arrayptr/)\<[System::SharedPtr](../../system/sharedptr/)\<[IMathElement](../imathelement/)\>\> [GetChildren](../imathelement/getchildren/)() | 자식 요소 가져오기 |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathGroupingCharacter](../imathgroupingcharacter/)\> [Group](../imathelement/group/)(char16_t, [MathTopBotPositions](../mathtopbotpositions/), [MathTopBotPositions](../mathtopbotpositions/)) | 하단 중괄호와 같은 그룹화 문자를 사용하거나 다른 문자를 사용하여 이 요소를 그룹에 배치합니다. |
...
| virtual void [set_AlignmentPoint](./set_alignmentpoint/)(**bool**) | true이면, 이 연산자 에뮬레이터는 정렬 포인트 역할을 합니다; 즉, 다른 식에서 지정된 정렬 포인트와 정렬될 수 있습니다. 기본값: false |
| virtual void [set_Differential](./set_differential/)(**bool**) | 미분. true이면, 박스는 미분(예: \\uD835\\uDC51\\uD835\\uDC65) 역할을 하며 수학적 미분에 적절한 수평 간격을 받습니다. 기본값: false |
| virtual void [set_ExplicitBreak](./set_explicitbreak/)(**uint8_t**) | 명시적 줄 바꿈은 Box 객체 시작 부분에 줄 바꿈이 있는지 지정하며, 줄이 박스 객체 시작 부분에서 래핑됩니다. 이전 수학 텍스트 줄의 연산자 번호를 지정하여 현재 수학 텍스트 줄의 정렬 포인트로 사용합니다. 가능한 값: 1..255 기본값: 0 (명시적 줄 바꿈 없음) |
| virtual void [set_NoBreak](./set_nobreak/)(**bool**) | 줄 바꿈 없음. 이 속성은 객체 박스의 "깨지지 않음" 속성을 지정합니다. true이면 박스 내부에서 줄 바꿈이 발생하지 않습니다. 이는 둘 이상의 이항 연산자로 구성된 연산자 에뮬레이터에 중요할 수 있습니다. 이 요소가 지정되지 않으면 박스 내부에서 줄 바꿈이 발생할 수 있습니다. 기본값: true |
| virtual void [set_OperatorEmulator](./set_operatoremulator/)(**bool**) | 연산자 에뮬레이터. true이면 박스와 그 내용이 단일 연산자처럼 동작하고 연산자의 속성을 상속합니다. 예를 들어, 해당 문자는 줄 바꿈 지점으로 사용될 수 있고 다른 연산자와 정렬될 수 있습니다. 연산자 에뮬레이터는 '=='와 같이 하나 이상의 글리프가 결합하여 연산자를 형성할 때 자주 사용됩니다. 기본값: false |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | n번째 템플릿 인수를 공유 대신 약한 포인터로 설정합니다. 컨테이너에서 포인터를 약한 모드로 전환할 수 있게 합니다. |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathBox](./)\> [ToBox](../imathelement/tobox/)() | 이 요소를 비시각적 박스(논리적 그룹화) 안에 배치합니다. 이는 수식이나 다른 수학 텍스트의 구성 요소를 그룹화하는 데 사용됩니다. 박스된 객체는 예를 들어 정렬 포인트가 있거나 없거나 하는 연산자 에뮬레이터 역할을 하거나, 줄 바꿈 지점으로 사용되거나, 내부에서 줄 바꿈이 허용되지 않도록 그룹화될 수 있습니다. |
