...
description: "Managed SHA384 해시 관리자. 구현되지 않음. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여 할당해야만 합니다. 스택에 직접 생성하거나 operator new를 사용해 인스턴스를 만들면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 감싸고, 해당 포인터를 함수 인수로 전달하십시오."
type: docs
weight: 612
url: /ko/system.security.cryptography/sha384managed/
---
## SHA384Managed 클래스
...
| static [SharedPtr](../../system/sharedptr/)\<[SHA384](../sha384/)\> [Create](../sha384/create/)() | 해시 알고리즘 인스턴스를 생성합니다. |
| static [SharedPtr](../../system/sharedptr/)\<[SHA384](../sha384/)\> [Create](../sha384/create/)(const [String](../../system/string/)\&) | 해시 알고리즘 인스턴스를 생성합니다. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | 두 NaN이 IEC 60559:1989에 따르면 어떤 값과도( NaN 포함) 동일하지 않지만, 두 NaN을 동일하게 간주하는 C# 스타일 부동소수점 비교를 구현합니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | 두 NaN이 IEC 60559:1989에 따르면 어떤 값과도( NaN 포함) 동일하지 않지만, 두 NaN을 동일하게 간주하는 C# 스타일 부동소수점 비교를 구현합니다. |
...
|  [SHA384Managed](./sha384managed/)() | 생성자. |
