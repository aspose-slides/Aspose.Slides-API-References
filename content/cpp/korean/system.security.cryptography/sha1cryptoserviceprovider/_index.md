...
description: "SHA1 CSP 구현. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여 할당해야 합니다. 스택이나 operator new를 사용하여 이 타입의 인스턴스를 생성하면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 감싸고, 해당 포인터를 함수 인자로 전달하십시오."
type: docs
weight: 547
url: /ko/system.security.cryptography/sha1cryptoserviceprovider/
---
## SHA1CryptoServiceProvider 클래스

[SHA1](../sha1/) CSP 구현. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여 할당해야 합니다. 스택이나 operator new를 사용하여 이 타입의 인스턴스를 생성하면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터로 감싸고, 해당 포인터를 함수 인자로 전달하여 사용하십시오.
...
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [ComputeHash](../hashalgorithm/computehash/)([SharedPtr](../../system/sharedptr/)\<[IO::Stream](../../system.io/stream/)\> const\&) | 스트림을 끝까지 읽고 읽은 데이터에 대해 해시를 계산합니다. |
| static [SharedPtr](../../system/sharedptr/)\<[SHA1](../sha1/)\> [Create](../sha1/create/)() | 해시 알고리즘 인스턴스를 생성합니다. |
| static [SharedPtr](../../system/sharedptr/)\<[SHA1](../sha1/)\> [Create](../sha1/create/)(const [String](../../system/string/)\&) | 해시 알고리즘 인스턴스를 생성합니다. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | C# 스타일의 부동소수점 비교를 에뮬레이트합니다. 두 NaN은 IEC 60559:1989에 따라 어떤 값과도, NaN 포함, 같지 않음에도 불구하고 동일하게 간주됩니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | C# 스타일의 부동소수점 비교를 에뮬레이트합니다. 두 NaN은 IEC 60559:1989에 따라 어떤 값과도, NaN 포함, 같지 않음에도 불구하고 동일하게 간주됩니다. |
...
|  [SHA1CryptoServiceProvider](./sha1cryptoserviceprovider/)() | 생성자. |
...
| int [TransformBlock](../hashalgorithm/transformblock/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int) override | 데이터를 블록 단위로 처리하고 데이터를 출력 배열에 복사합니다. |
