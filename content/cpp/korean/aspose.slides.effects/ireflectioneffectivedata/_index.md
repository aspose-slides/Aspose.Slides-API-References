...
---
## IReflectionEffectiveData 클래스

불변 객체는 [Reflection](../reflection/) 효과를 나타냅니다.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | 두 NaN이 IEC 60559:1989에 따라 어떤 값과도 (NaN 포함) 같지 않지만, C# 스타일 부동소수점 비교를 에뮬레이트합니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | 두 NaN이 IEC 60559:1989에 따라 어떤 값과도 (NaN 포함) 같지 않지만, C# 스타일 부동소수점 비교를 에뮬레이트합니다. |
...
| virtual **float** [get_Direction](./get_direction/)() | 반사의 방향. 읽기 전용 **float**. |
| virtual **double** [get_Distance](./get_distance/)() | 반사의 거리. 읽기 전용 **double**. |
| virtual **float** [get_EndPosAlpha](./get_endposalpha/)() | 끝 알파 값(백분율)의 최종 위치(알파 그라디언트 램프를 따라)를 지정합니다. 읽기 전용 **float**. |
| virtual **float** [get_EndReflectionOpacity](./get_endreflectionopacity/)() | 끝 반사 불투명도(백분율). 읽기 전용 **float**. |
| virtual **float** [get_FadeDirection](./get_fadedirection/)() | 반사를 오프셋할 방향(각도)을 지정합니다. 읽기 전용 **float**. |
...
| virtual **bool** [get_RotateShadowWithShape](./get_rotateshadowwithshape/)() | 도형이 회전될 때 반사도 함께 회전해야 하는지 지정합니다. 읽기 전용 **bool**. |
| virtual **double** [get_ScaleHorizontal](./get_scalehorizontal/)() | 수평 스케일링 계수를 지정합니다. 음수 스케일링은 뒤집힘을 발생시킵니다. (백분율) 읽기 전용 **double**. |
| virtual **double** [get_ScaleVertical](./get_scalevertical/)() | 수직 스케일링 계수를 지정합니다. 음수 스케일링은 뒤집힘을 발생시킵니다. (백분율) 읽기 전용 **double**. |
| virtual **double** [get_SkewHorizontal](./get_skewhorizontal/)() | 수평 스키유 각도를 지정합니다. 읽기 전용 **double**. |
| virtual **double** [get_SkewVertical](./get_skewvertical/)() | 수직 스키유 각도를 지정합니다. 읽기 전용 **double**. |
| virtual **float** [get_StartPosAlpha](./get_startposalpha/)() | 시작 알파 값(백분율)의 시작 위치(알파 그라디언트 램프를 따라)를 지정합니다. 읽기 전용 **float**. |
| virtual **float** [get_StartReflectionOpacity](./get_startreflectionopacity/)() | 시작 반사 불투명도(백분율). 읽기 전용 **float**. |
