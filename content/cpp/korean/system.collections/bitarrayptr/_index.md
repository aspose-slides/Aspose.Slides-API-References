...
description: BitArray에 대한 포인터입니다. 이 유형은 다른 객체의 삭제를 관리하는 포인터입니다. 스택에 할당하고 값으로 또는 const 참조로 함수를 통해 전달해야 합니다.
type: docs
weight: 14
url: /ko/system.collections/bitarrayptr/
---
## BitArrayPtr 클래스

[BitArray](../bitarray/)에 대한 포인터입니다. 이 유형은 다른 객체의 삭제를 관리하는 포인터입니다. 스택에 할당하고 값으로 또는 const 참조로 함수를 통해 전달해야 합니다.
...
| [BitArrayPtr](./bitarrayptr/)(const [SharedPtr](../../system/sharedptr/)\<[BitArray](../bitarray/)\>\&) | 변환 생성자입니다. |
| std::enable_if_t\<std::is_same\<Y, T\>::value, [SmartPtr](../../system/smartptr/)\<Y\>\> [Cast](../../system/smartptr/cast/)() const | 포인터를 자체 유형으로 캐스팅합니다. |
| std::enable_if_t<\!std::is_same\<Y, T\>::value\&&std::is_base_of\<Y, T\>::value, [SmartPtr](../../system/smartptr/)\<Y\>\> [Cast](../../system/smartptr/cast/)() const | static_cast를 사용하여 포인터를 기본 유형으로 캐스팅합니다. |
| std::enable_if_t\<Check::value\&&\!std::is_same\<Y, T\>::value\&&\!std::is_base_of\<Y, T\>::value, [SmartPtr](../../system/smartptr/)\<Y\>\> [Cast](../../system/smartptr/cast/)() const | dynamic_cast를 사용하여 파생 유형으로 포인터를 캐스팅합니다. |
| std::enable_if_t<\!Check::value\&&\!std::is_same\<Y, T\>::value\&&\!std::is_base_of\<Y, T\>::value, [SmartPtr](../../system/smartptr/)\<Y\>\> [Cast](../../system/smartptr/cast/)() const | dynamic_cast를 사용하여 파생 유형으로 포인터를 캐스팅합니다. |
...
| int [get_shared_count](../../system/smartptr/get_shared_count/)() const | 현재 포인터를 포함하여 참조된 객체에 존재하는 공유 포인터 수를 가져옵니다. 현재 포인터가 공유 모드임을 확인합니다. |
| int [GetHashCode](../../system/smartptr/gethashcode/)() const | [GetHashCode()](../../system/smartptr/gethashcode/)를 가리키는 객체에서 호출합니다. |
| T * [GetObjectNotNull](../../system/smartptr/getobjectnotnull/)() const | 현재 참조된 객체를 가져오며(존재하는 경우) 없으면 예외를 발생시킵니다. |
| [Object](../../system/object/) * [GetObjectOrNull](../../system/smartptr/getobjectornull/)() const | 가리키는 객체를 가져오며(존재하는 경우) 없으면 nullptr를 반환합니다. [get()](../../system/smartptr/get/)와 동일합니다. |
| [Object](../../system/object/) * [GetObjectOwner](../../system/smartptr/getobjectowner/)() const | 참조된 객체를 가져옵니다. |
| [Pointee_](../../system/smartptr/pointee_/) * [GetPointer](../../system/smartptr/getpointer/)() const | 가리키는 객체를 가져오며(존재하는 경우) 없으면 nullptr를 반환합니다. [get()](../../system/smartptr/get/)와 동일합니다. |
| **bool** [Is](../../system/smartptr/is/)(const [System::TypeInfo](../../system/typeinfo/)\&) const | 가리키는 객체가 특정 유형이거나 그 자식 유형인지 확인합니다. C#의 'is' 연산자와 동일한 의미를 가집니다. |
| **bool** [IsAliasingPtr](../../system/smartptr/isaliasingptr/)() const | 포인터가 소유된 객체가 아닌 다른 객체를 가리키는지 확인합니다(별칭 생성자로 생성된 경우). |
| **bool** [IsNull](./isnull/)() const | 특정 값이 null인지 확인합니다. |
...
| [SmartPtr_](../../system/smartptr/smartptr_/)\& [operator=](../../system/smartptr/operator_equal/)(const [SmartPtr](../../system/smartptr/)\<Q\>\&) | [SmartPtr](../../system/smartptr/) 객체를 복사 할당합니다. 필요한 유형 변환을 수행합니다. |
...
| **BitArray::Reference** [operator[]](./operator[]/)(int) const | 비트 접근자입니다. |
| [SmartPtr_](../../system/smartptr/smartptr_/) [RemoveAliasing](../../system/smartptr/removealiasing/)() const | 포인터에서 별칭(별칭 생성자로 만든)을 제거하고, 공유 모드이면 관리하고 약한 모드이면 추적하도록 같은 객체를 가리키도록 합니다. |
| void [reset](../../system/smartptr/reset/)([Pointee_](../../system/smartptr/pointee_/) *) | 가리키는 객체를 설정합니다. |
| void [reset](../../system/smartptr/reset/)() | 포인터를 nullptr를 가리키도록 합니다. |
| void [set_Mode](../../system/smartptr/set_mode/)([SmartPtrMode](../../system/smartptrmode/)) | 포인터 모드를 설정합니다. 참조된 객체의 참조 카운트가 변경될 수 있습니다. |
...
| [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<Q\>\&, [SmartPtrMode](../../system/smartptrmode/)) | [SmartPtr](../../system/smartptr/) 객체를 복사 생성합니다. 두 포인터는 이후 동일한 객체를 가리키며, 허용되는 경우 유형 변환을 수행합니다. |
| [SmartPtr](../../system/smartptr/smartptr/)([SmartPtr_](../../system/smartptr/smartptr_/)\&&, [SmartPtrMode](../../system/smartptrmode/)) | [SmartPtr](../../system/smartptr/) 객체를 이동 생성합니다. 두 포인터가 동일한 모드인 경우 사실상 두 포인터를 교환합니다. 호출 후 x는 사용 불가능할 수 있습니다. |
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<[Array](../../system/array/)\<Y\>\>\&, [SmartPtrMode](../../system/smartptrmode/)) | 다른 유형의 새 배열을 생성하여 참조된 배열의 유형을 변환합니다. C#에서 지원되지만 C++에서는 지원되지 않는 배열 형 변환에 유용합니다. |
...
| [SmartPtr](../../system/smartptr/)\<Y\> [static_pointer_cast](../../system/smartptr/static_pointer_cast/)() const | 가리키는 객체에 static_cast를 사용하여 다른 유형으로 캐스팅합니다. |
| [SmartPtr](../../system/smartptr/)\<[Object](../../system/object/)\> [ToObjectPtr](../../system/smartptr/toobjectptr/)() const | 어떤 포인터 유형이든 [Object](../../system/object/)에 대한 포인터로 변환합니다. Pointee_ 유형이 완전할 필요는 없습니다. |
| static const [System::TypeInfo](../../system/typeinfo/)\& [Type](../../system/smartptr/type/)() | [System::TypeInfo](../../system/typeinfo/) 객체를 Pointee_ 유형에 대해 얻기 위한 바로 가기입니다. |
