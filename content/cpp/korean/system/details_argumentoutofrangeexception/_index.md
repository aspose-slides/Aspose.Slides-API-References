...
description: "ArgumentOutOfRangeException은 호출된 메서드에 전달된 인수가 해당 인수에 대해 예상되는 값 범위를 벗어날 때 발생합니다. 이 클래스를 수동으로 인스턴스화하지 마십시오. 대신 ArgumentOutOfRangeException 클래스를 사용하십시오. ArgumentOutOfRangeException 클래스 인스턴스를 System::SmartPtr에 감싸지 마십시오."
type: docs
weight: 352
url: /ko/system/details_argumentoutofrangeexception/
---
## Details_ArgumentOutOfRangeException 클래스

ArgumentOutOfRangeException은 호출된 메서드에 전달된 인수가 해당 인수에 대해 예상되는 값 범위를 벗어날 때 발생합니다. 이 클래스를 수동으로 인스턴스화하지 마세요. 대신 ArgumentOutOfRangeException 클래스를 사용하십시오. ArgumentOutOfRangeException 클래스 인스턴스를 [System::SmartPtr](../smartptr/)에 감싸지 마세요.
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도(또는 NaN 자체와도) 같지 않음에도 불구하고 C#-style 부동소수점 비교를 흉내 내어 두 NaN을 동일하게 간주합니다. |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도(또는 NaN 자체와도) 같지 않음에도 불구하고 C#-style 부동소수점 비교를 흉내 내어 두 NaN을 동일하게 간주합니다. |
...
| virtual [String](../string/) [get_StackTrace](../details_exception/get_stacktrace/)() const | 스택 추적이 포함된 문자열을 반환합니다. |
...
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | n번째 템플릿 인수를 약한 포인터(공유가 아닌)로 설정합니다. 컨테이너의 포인터를 약한 모드로 전환할 수 있게 합니다. |
...
| void [Unlock](../object/unlock/)() | C# lock() 구문의 잠금을 해제합니다. 직접 호출하거나 [LockContext](../lockcontext/) 감시 객체를 사용하십시오. |
...
| virtual const char * [what](../details_exception/what/)() const | [what()](../details_exception/what/) 메서드를 구현하며, 이는 [ExceptionWrapper](../exceptionwrapper/) 클래스에 의해 호출됩니다. 이 클래스가 std::exception에서 상속되지 않았음에도 파생 클래스가 protected/private 멤버를 사용해 로직을 구현할 수 있습니다. 이 메서드 구현을 [ExceptionWrapper](../exceptionwrapper/)로 이동하면 해당 로직이 깨질 수 있습니다. |
