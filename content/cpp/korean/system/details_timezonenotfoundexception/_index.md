...
description: "TimeZoneNotFoundException은 시간대 정보가 없을 때 발생합니다. 이 클래스를 수동으로 인스턴스화하지 마세요. 대신 TimeZoneNotFoundException 클래스를 사용하십시오. TimeZoneNotFoundException 클래스 인스턴스를 System::SmartPtr에 절대 래핑하지 마세요."
type: docs
weight: 729
url: /ko/system/details_timezonenotfoundexception/
---
## Details_TimeZoneNotFoundException 클래스

TimeZoneNotFoundException은 시간대 정보가 없을 때 발생합니다. 이 클래스를 수동으로 인스턴스화하지 마세요. 대신 TimeZoneNotFoundException 클래스를 사용하십시오. TimeZoneNotFoundException 클래스 인스턴스를 [System::SmartPtr](../smartptr/)에 절대 래핑하지 마세요.
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도 같지 않지만, 두 NaN을 동일하게 간주하는 C#-style 부동소수점 비교를 에뮬레이트합니다. |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도 같지 않지만, 두 NaN을 동일하게 간주하는 C#-style 부동소수점 비교를 에뮬레이트합니다. |
...
|  [Object](../object/object/)([Object](../object/) const\&) | 복사 생성자입니다. 실제로 아무 것도 복사하지 않고, 새 객체를 초기화하며 서브클래스의 복사 생성을 가능하게 합니다. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | 할당 연산자입니다. 실제로 아무 것도 복사하지 않고, 새 객체를 초기화하며 서브클래스의 복사 생성을 가능하게 합니다. |
...
| virtual const char * [what](../details_exception/what/)() const | [what()](../details_exception/what/) 메서드를 구현합니다. 이 메서드는 [ExceptionWrapper](../exceptionwrapper/) 클래스에 의해 호출됩니다. 이 클래스가 std::exception에서 상속되지 않았음에도 파생 클래스는 보호/비공개 멤버를 사용해 로직을 구현할 수 있습니다. 이 메서드 구현을 [ExceptionWrapper](../exceptionwrapper/)로 옮기면 해당 로직이 손상될 수 있습니다. |
