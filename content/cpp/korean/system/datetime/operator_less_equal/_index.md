...
description: 현재 객체가 지정된 DateTime 객체가 나타내는 값보다 이전이거나 동일한 날짜 및 시간 값을 나타내는지 확인합니다.
type: docs
weight: 599
url: /ko/system/datetime/operator_less_equal/
---
...
현재 객체가 나타내는 날짜 및 시간 값이 **other**가 나타내는 값보다 이전이거나 동일한 경우 True, 그렇지 않으면 false

## DateTime::operator<=(std::nullptr_t) const 메서드
