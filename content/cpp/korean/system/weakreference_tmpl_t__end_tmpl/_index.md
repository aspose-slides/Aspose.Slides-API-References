...
description: 객체를 참조하지만 해당 객체가 삭제될 수 있도록 허용하는 약한 참조를 나타냅니다.
type: docs
weight: 1509
url: /ko/system/weakreference_tmpl_t__end_tmpl/
---
## WeakReference< T > 클래스
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도 (NaN 포함) 같지 않지만, 두 NaN을 동일하게 간주하는 C# 스타일 부동 소수점 비교를 에뮬레이트합니다. |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도 (NaN 포함) 같지 않지만, 두 NaN을 동일하게 간주하는 C# 스타일 부동 소수점 비교를 에뮬레이트합니다. |
...
| **bool** [operator!=](./operator_not_equal/)(std::nullptr_t) const | 참조된 객체가 null이 아닌지 확인합니다. |
| **bool** [operator!=](./operator_not_equal/)(const [WeakReference](./weakreference/)\<T\>\&) const | 참조된 객체를 다른 WeakReference 클래스 인스턴스와 비교합니다. |
...
| **bool** [operator==](./operator_equal_equal/)(std::nullptr_t) const | 참조된 객체가 null인지 확인합니다. |
| **bool** [operator==](./operator_equal_equal/)(const [WeakReference](./weakreference/)\<T\>\&) const | 참조된 객체를 다른 WeakReference 클래스 인스턴스와 비교합니다. |
...
| void [reset](./reset/)() |  |
| void [SetTarget](./settarget/)(const [SmartPtr](../smartptr/)\<T\>\&) | 현재 WeakReference 객체가 참조하는 객체(대상)를 설정합니다. |
...
| **bool** [TryGetTarget](./trygettarget/)(const [SmartPtr](../smartptr/)\<T\>\&) const | 현재 WeakReference 객체가 참조하는 객체(대상)를 가져옵니다. |
...
|  [WeakReference](./weakreference/)(std::nullptr_t) | nullptr에서 생성되는 생성자. |
|  [WeakReference](./weakreference/)(const [SmartPtr](../smartptr/)\<T\>\&) | 지정된 객체를 참조하는 WeakReference 클래스의 새 인스턴스를 초기화합니다. |
|  [WeakReference](./weakreference/)(const [SmartPtr](../smartptr/)\<T\>\&, **bool**) | 지정된 객체를 참조하는 WeakReference 클래스의 새 인스턴스를 초기화합니다. |
