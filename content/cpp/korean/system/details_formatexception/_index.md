...
description: "FormatException 은 메서드 인수의 형식이 올바르지 않을 때 발생합니다. 이 클래스를 직접 인스턴스화하지 마십시오. 대신 FormatException 클래스를 사용하십시오. FormatException 클래스 인스턴스를 System::SmartPtr 로 래핑하지 마십시오."
type: docs
weight: 469
url: /ko/system/details_formatexception/
---
## Details_FormatException 클래스

FormatException 은 메서드 인수의 형식이 올바르지 않을 때 발생합니다. 이 클래스를 직접 인스턴스화하지 마십시오. 대신 FormatException 클래스를 사용하십시오. FormatException 클래스 인스턴스를 [System::SmartPtr](../smartptr/) 로 래핑하지 마십시오.
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도 (NaN 포함) 같지 않지만, C#-스타일 부동소수점 비교를 에뮬레이션하여 두 NaN을 동일하다고 간주합니다. |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도 (NaN 포함) 같지 않지만, C#-스타일 부동소수점 비교를 에뮬레이션하여 두 NaN을 동일하다고 간주합니다. |
...
| const [System::TypeInfo](../typeinfo/)\& [GetType](./gettype/)() const override | 객체의 실제 유형을 가져옵니다. C# [System.Object.GetType()](../object/gettype/) 호출의 유사 버전입니다. |
| **bool** [Is](./is/)(const [System::TypeInfo](../typeinfo/)\&) const override |  |
| void [Lock](../object/lock/)() | C# lock() 문을 구현합니다. 직접 호출하거나 [LockContext](../lockcontext/) 감시 객체를 사용하십시오. |
| virtual [ptr](../object/ptr/) [MemberwiseClone](../object/memberwiseclone/)() const | C# [Object.MemberwiseClone()](../object/memberwiseclone/) 메서드의 유사 버전입니다. 사용자 정의 타입을 복제할 수 있게 합니다. |
...
| void [set_HResult](../details_exception/set_hresult/)(**int32_t**) | 특정 예외에 할당된 코드형 숫자 값인 HRESULT를 설정합니다. |
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | n번째 템플릿 인자를 약한 포인터로 설정합니다(공유 대신). 컨테이너 안의 포인터를 약한 모드로 전환할 수 있게 합니다. |
...
| virtual const char * [what](../details_exception/what/)() const | [what()](../details_exception/what/) 메서드를 구현하며, 이는 [ExceptionWrapper](../exceptionwrapper/) 클래스에 의해 호출됩니다. 이 클래스가 std::exception에서 상속되지 않았음에도 불구하고 파생 클래스는 보호/비공개 멤버를 사용해 로직을 구현할 수 있습니다. 이 메서드 구현을 [ExceptionWrapper](../exceptionwrapper/) 로 이동하면 해당 로직이 손상될 수 있습니다. |
