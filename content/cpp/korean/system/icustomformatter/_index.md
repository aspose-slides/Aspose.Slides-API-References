...
description: "지정된 객체가 나타내는 값의 문자열 표현을 사용자 정의 형식으로 수행하는 메서드를 정의합니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여 할당해야 합니다. 스택에 직접 인스턴스를 생성하거나 operator new를 사용하면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 감싸고 해당 포인터를 함수 인자로 전달하십시오."
type: docs
weight: 950
url: /ko/system/icustomformatter/
---
## ICustomFormatter 클래스

지정된 객체가 나타내는 값의 문자열 표현을 사용자 정의 형식으로 수행하는 메서드를 정의합니다. 이 클래스의 객체는 [System::MakeObject()](../makeobject/) 함수를 사용하여 할당해야 합니다. 스택에 직접 인스턴스를 생성하거나 operator new를 사용하면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 [System::SmartPtr](../smartptr/) 포인터로 감싸고 해당 포인터를 함수 인자로 전달하십시오.
...
| virtual [System::String](../string/) [Format](./format/)([System::String](../string/), [System::SharedPtr](../sharedptr/)\<[System::Object](../object/)\>, [System::SharedPtr](../sharedptr/)\<[System::IFormatProvider](../iformatprovider/)\>) | 지정된 형식을 사용하여 현재 객체가 나타내는 값의 문자열 표현을 반환합니다. |
...
| void [Lock](../object/lock/)() | C# lock() 구문을 구현하여 잠급니다. 직접 호출하거나 [LockContext](../lockcontext/) 감시 객체를 사용하십시오. |
...
|  [Object](../object/object/)([Object](../object/) const\&) | 복사 생성자. 실제로 아무것도 복사하지 않고, 새로운 객체를 초기화하고 서브클래스의 복사 생성을 가능하게 합니다. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | 할당 연산자. 실제로 아무것도 복사하지 않고, 새로운 객체를 초기화하고 서브클래스의 복사 생성을 가능하게 합니다. |
