...
description: "InvalidTimeZoneException은 시간대 정보가 잘못된 경우에 발생합니다. 이 클래스를 수동으로 인스턴스화하지 마십시오. 대신 InvalidTimeZoneException 클래스를 사용하십시오. InvalidTimeZoneException 클래스 인스턴스를 System::SmartPtr에 절대 래핑하지 마십시오."
type: docs
weight: 534
url: /ko/system/details_invalidtimezoneexception/
---
## Details_InvalidTimeZoneException 클래스

InvalidTimeZoneException은 시간대 정보가 잘못된 경우에 발생합니다. 이 클래스를 직접 인스턴스화하지 마십시오. 대신 InvalidTimeZoneException 클래스를 사용하십시오. InvalidTimeZoneException 클래스 인스턴스를 [System::SmartPtr](../smartptr/)에 절대 래핑하지 마십시오.
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | 두 NaN을 동일하게 간주하는 C# 스타일 부동 소수점 비교를 에뮬레이트합니다(IEC 60559:1989에 따르면 NaN은 어떤 값과도, NaN 자체와도 같지 않지만). |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | 두 NaN을 동일하게 간주하는 C# 스타일 부동 소수점 비교를 에뮬레이트합니다(IEC 60559:1989에 따르면 NaN은 어떤 값과도, NaN 자체와도 같지 않지만). |
...
| virtual **int32_t** [GetHashCode](../object/gethashcode/)() const | C# [Object.GetHashCode()](../object/gethashcode/) 메서드와 유사합니다. 사용자 정의 객체의 해시를 가능하게 합니다. |
...
| void [set_HResult](../details_exception/set_hresult/)(**int32_t**) | 특정 예외에 할당된 코드화된 숫자값인 HRESULT를 설정합니다. |
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | n번째 템플릿 인자를 공유 포인터가 아닌 약한 포인터로 설정합니다. 컨테이너의 포인터를 약한 모드로 전환할 수 있게 합니다. |
...
| virtual const char * [what](../details_exception/what/)() const | [what()](../details_exception/what/) 메서드를 구현하며, 이는 [ExceptionWrapper](../exceptionwrapper/) 클래스에 의해 호출됩니다. 이 클래스는 std::exception을 상속받지 않지만, 파생 클래스는 보호/비공개 멤버를 사용해 로직을 구현할 수 있습니다. 이 메서드 구현을 [ExceptionWrapper](../exceptionwrapper/)으로 이동하면 해당 로직이 깨질 수 있습니다. |
