...
description: "ArithmeticException 은 산술 연산이나 캐스팅 변환 작업을 수행하는 중 오류가 발생하면 발생합니다. 이 클래스를 수동으로 인스턴스화 하지 마십시오. 대신 ArithmeticException 클래스를 사용하십시오. ArithmeticException 클래스 인스턴스를 System::SmartPtr 로 절대 래핑하지 마십시오."
type: docs
weight: 365
url: /ko/system/details_arithmeticexception/
---
## Details_ArithmeticException 클래스

ArithmeticException 은 산술 연산, 캐스팅 변환 작업을 수행하는 중 오류가 발생할 때 발생합니다. 이 클래스의 인스턴스를 수동으로 생성하지 마십시오. 대신 ArithmeticException 클래스를 사용하십시오. [System::SmartPtr](../smartptr/) 로 ArithmeticException 클래스 인스턴스를 절대 래핑하지 마십시오.
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도, 심지어 NaN 자체와도 같지 않지만, 두 NaN을 동일하게 간주하는 C#-style 부동소수점 비교를 에뮬레이트합니다. |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | IEC 60559:1989에 따르면 NaN은 어떤 값과도, 심지어 NaN 자체와도 같지 않지만, 두 NaN을 동일하게 간주하는 C#-style 부동소수점 비교를 에뮬레이트합니다. |
...
| void [Lock](../object/lock/)() | C# lock() 문을 구현하여 잠급니다. 직접 호출하거나 [LockContext](../lockcontext/) 센트리 객체를 사용하십시오. |
...
|  [Object](../object/object/)([Object](../object/) const\&) | 복사 생성자. 실제로 아무것도 복사하지 않으며, 단순히 새 객체를 초기화하고 서브클래스 복사 생성을 가능하게 합니다. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | 할당 연산자. 실제로 아무것도 복사하지 않으며, 단순히 새 객체를 초기화하고 서브클래스 복사 생성을 가능하게 합니다. |
...
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | n번째 템플릿 인수를 공유가 아닌 약한 포인터로 설정합니다. 컨테이너 내 포인터를 약한 모드로 전환할 수 있게 합니다. |
...
| Detail::SmartPtrCounter * [WeakRefAdded](../object/weakrefadded/)() | 약한 참조 카운트를 증가시킵니다. 직접 호출해서는 안 되며, 스마트 포인터나 ThisProtector를 사용하십시오. |
