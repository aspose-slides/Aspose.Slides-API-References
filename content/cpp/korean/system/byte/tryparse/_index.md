...
description: 지정된 문자열(숫자의 문자열 표현을 포함)을 8비트 부호 없는 정수와 동등한 형태로 변환합니다.
type: docs
weight: 14
url: /ko/system/byte/tryparse/
---
## Byte::TryParse(const String\&, uint8_t\&) method


지정된 문자열(숫자의 문자열 표현을 포함) 을 8비트 부호 없는 정수와 동등한 형태로 변환합니다.
...
변환이 성공하면 true이며, 그렇지 않으면 false입니다.

## Byte::TryParse(const String\&, Globalization::NumberStyles, const SharedPtr\<IFormatProvider\>\&, uint8_t\&) method


지정된 문자열(숫자의 문자열 표현을 포함)을 제공된 형식 정보와 숫자 스타일을 사용하여 8비트 부호 없는 정수와 동등한 형태로 변환합니다.
