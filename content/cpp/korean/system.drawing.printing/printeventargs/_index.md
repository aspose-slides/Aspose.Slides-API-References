...
description: "BeginPrint 및 EndPrint 이벤트에 대한 데이터를 제공합니다. 이 클래스의 객체는 System::MakeObject() 함수를 사용하여만 할당해야 합니다. 스택에 직접 인스턴스를 만들거나 operator new를 사용해서 생성하면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 System::SmartPtr 포인터로 감싸서 해당 포인터를 함수 인수로 전달하십시오."
type: docs
weight: 92
url: /ko/system.drawing.printing/printeventargs/
---
## PrintEventArgs 클래스

BeginPrint 및 EndPrint 이벤트에 대한 데이터를 제공합니다. 이 클래스의 객체는 [System::MakeObject()](../../system/makeobject/) 함수를 사용하여만 할당해야 합니다. 스택에 직접 생성하거나 operator new를 사용하여 인스턴스를 만들면 런타임 오류 및/또는 어설션 오류가 발생합니다. 항상 이 클래스를 [System::SmartPtr](../../system/smartptr/) 포인터로 감싸고, 해당 포인터를 함수 인수로 전달하십시오.
...
| Method | Description |
| --- | --- |
|  [CancelEventArgs](../../system.componentmodel/canceleventargs/canceleventargs/)(**bool**) | 생성자. |
|  [CancelEventArgs](../../system.componentmodel/canceleventargs/canceleventargs/)() | 생성자; Cancel 속성을 false 로 설정합니다. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | C# 스타일 부동 소수점 비교를 에뮬레이션합니다. IEC 60559:1989에 따르면 NaN은 어떤 값과도, NaN 자체와도 같지 않지만, 두 NaN을 동일하게 간주합니다. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | C# 스타일 부동 소수점 비교를 에뮬레이션합니다. IEC 60559:1989에 따르면 NaN은 어떤 값과도, NaN 자체와도 같지 않지만, 두 NaN을 동일하게 간주합니다. |
...
| **bool** [get_Cancel](../../system.componentmodel/canceleventargs/get_cancel/)() | 이벤트를 취소해야 하는지를 나타내는 값을 반환합니다. |
| [PrintAction](../printaction/) [get_PrintAction](./get_printaction/)() | 현재 객체가 나타내는 인쇄 동작을 지정하는 값을 반환합니다. |
...
| virtual const [TypeInfo](../../system/typeinfo/)\& [GetType](../../system/object/gettype/)() const | 객체의 실제 유형을 반환합니다. C# [System.Object.GetType()](../../system/object/gettype/) 호출과 동일합니다. |
...
|  [PrintEventArgs](./printeventargs/)() | 새로운 [PrintEventArgs](./) 객체 인스턴스를 생성합니다. |
...
| void [set_Cancel](../../system.componentmodel/canceleventargs/set_cancel/)(**bool**) | 이벤트를 취소해야 하는지를 나타내는 값을 설정합니다. |
...
| Field | Description |
| --- | --- |
| static [Empty](../../system/eventargs/empty/) | 빈 [EventArgs](../../system/eventargs/) 공유 포인터(널 포인터)를 나타내는 정적 멤버입니다. |

## 참조

* 클래스 [CancelEventArgs](../../system.componentmodel/canceleventargs/)
