...
| 메서드 | 설명 |
| --- | --- |
| virtual **bool** [CheckWriteProtection](./checkwriteprotection/)([System::String](../../system/string/)) | 프레젠테이션이 수정용 비밀번호로 보호되어 있는지 여부를 결정합니다. |
| virtual void [Encrypt](./encrypt/)([System::String](../../system/string/)) | [Presentation](../presentation/)를 지정된 비밀번호로 암호화합니다. |
...
| virtual **bool** [get_EncryptDocumentProperties](./get_encryptdocumentproperties/)() | 프레젠테이션이 비밀번호로 보호된 경우에만 의미가 있습니다. true이면 문서 속성이 프레젠테이션 파일에 암호화됩니다. false이면 프레젠테이션은 암호화된 상태이지만 문서 속성은 공개됩니다. 읽기 **bool**. |
| virtual [System::String](../../system/string/) [get_EncryptionPassword](./get_encryptionpassword/)() | 암호화 비밀번호를 반환합니다. 읽기 전용 [System::String](../../system/string/). |
| virtual **bool** [get_IsEncrypted](./get_isencrypted/)() | 이 인스턴스가 암호화되었는지 여부를 가져옵니다. 읽기 전용 **bool**. |
| virtual **bool** [get_IsOnlyDocumentPropertiesLoaded](./get_isonlydocumentpropertiesloaded/)() | 프레젠테이션 파일이 비밀번호로 보호되고 해당 파일의 문서 속성이 공개된 경우에만 의미가 있습니다. true 값은 비밀번호 없이 암호화된 프레젠테이션 파일에서 문서 속성만 로드된다는 의미이며, false 값은 올바른 비밀번호를 사용하여 전체 암호화된 프레젠테이션을 로드한다는 의미입니다. 프레젠테이션이 암호화되지 않은 경우 이 속성 값은 항상 false이며, 암호화된 파일의 문서 속성이 공개되지 않은 경우에도 값은 항상 false입니다. PresentationEx.EncryptDocumentProperties가 true이면 IsOnlyDocumentPropertiesLoaded 속성 값은 항상 false입니다. 읽기 전용 **bool**. |
| virtual **bool** [get_IsWriteProtected](./get_iswriteprotected/)() | 이 프레젠테이션이 쓰기 보호되었는지 여부를 가져옵니다. 읽기 전용 **bool**. |
| virtual **bool** [get_ReadOnlyRecommended](./get_readonlyrecommended/)() | 읽기 전용 권장 사항을 가져옵니다. 읽기 **bool**. |
...
| virtual void [RemoveEncryption](./removeencryption/)() | 암호화를 제거합니다. |
| virtual void [RemoveWriteProtection](./removewriteprotection/)() | 이 프레젠테이션의 쓰기 보호를 제거합니다. |
| virtual void [set_EncryptDocumentProperties](./set_encryptdocumentproperties/)(**bool**) | 프레젠테이션이 비밀번호로 보호된 경우에만 의미가 있습니다. true이면 문서 속성이 프레젠테이션 파일에 암호화됩니다. false이면 프레젠테이션은 암호화된 상태이지만 문서 속성은 공개됩니다. 쓰기 **bool**. |
| virtual void [set_ReadOnlyRecommended](./set_readonlyrecommended/)(**bool**) | 읽기 전용 권장 사항을 설정합니다. 쓰기 **bool**. |
...
| virtual void [SetWriteProtection](./setwriteprotection/)([System::String](../../system/string/)) | 지정된 비밀번호로 이 프레젠테이션의 쓰기 보호를 설정합니다. |
