...
description: Это предупреждение указывает на то, что у читаемой презентации есть подпись, и эта подпись будет удалена во время обработки.
type: docs
weight: 40
url: /ru/aspose.slides.warnings/ipresentationsignedwarninginfo/
---
## IPresentationSignedWarningInfo класс

Это предупреждение указывает на то, что у читаемой презентации есть подпись, и эта подпись будет удалена во время обработки.
...
| virtual void [SendWarning](../iwarninginfo/sendwarning/)([System::SharedPtr](../../system/sharedptr/)\<[IWarningCallback](../iwarningcallback/)\>) | Если получатель не равен null, завершает предупреждение для указанного получателя и бросает AbortRequestedException, если получатель решил прервать операцию. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-ый шаблонный аргумент в виде слабого указателя (а не shared). Позволяет переключать указатели в контейнерах в режим слабых. |
