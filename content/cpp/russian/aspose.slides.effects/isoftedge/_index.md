...
description: Представляет эффект мягкого края. Края фигуры размыты, а заливка не изменяется.
type: docs
weight: 963
...
Представляет эффект мягкого края. Края фигуры размягчены, в то время как заливка не затронута.
...
| virtual **double** [get_Radius](./get_radius/)() | Указывает радиус размытия, применяемый к краям. Читает **double**. |
...
| virtual void [set_Radius](./set_radius/)(**double**) | Указывает радиус размытия, применяемый к краям. Записывает **double**. |
