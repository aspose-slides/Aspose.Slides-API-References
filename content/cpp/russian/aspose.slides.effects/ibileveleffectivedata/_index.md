...
description: Неизменяемый объект, представляющий двусторонний (чёрно-белый) эффект. Входные цвета, яркость которых меньше указанного порогового значения, преобразуются в чёрный. Входные цвета, яркость которых больше или равна указанному значению, устанавливаются в белый. Значения альфа-эффекта не изменяются этим эффектом.
type: docs
weight: 443
url: /ru/aspose.slides.effects/ibileveleffectivedata/
---
## IBiLevelEffectiveData класс

Неизменяемый объект, представляющий двусторонний (чёрно-белый) эффект. Входные цвета, яркость которых меньше заданного порогового значения, преобразуются в чёрный. Входные цвета, яркость которых больше либо равна указанному значению, устанавливаются в белый. Значения альфа-эффекта не изменяются этим эффектом.
...
| virtual **float** [get_Threshold](./get_threshold/)() | Возвращает пороговое значение. Только для чтения **float**. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-th параметр шаблона в слабый указатель (вместо общего). Позволяет переключать указатели в контейнерах в режим слабых ссылок. |
