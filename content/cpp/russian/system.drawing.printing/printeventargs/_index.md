...
description: "Предоставляет данные для событий BeginPrint и EndPrint. Объекты этого класса должны выделяться только с помощью функции System::MakeObject(). Никогда не создавайте экземпляр этого типа в стеке или с помощью оператора new, так как это приведёт к ошибкам времени выполнения и/или сбоям утверждений. Всегда оборачивайте этот класс в указатель System::SmartPtr и используйте этот указатель для передачи его в функции в качестве аргумента."
type: docs
weight: 92
url: /ru/system.drawing.printing/printeventargs/
---
## Класс PrintEventArgs

Предоставляет данные для событий BeginPrint и EndPrint. Объекты этого класса должны быть выделены только с помощью функции [System::MakeObject()](../../system/makeobject/). Никогда не создавайте экземпляр этого типа в стеке или с помощью оператора new, так как это приведёт к ошибкам времени выполнения и/или сбоям утверждений. Всегда оборачивайте этот класс в указатель [System::SmartPtr](../../system/smartptr/) и используйте этот указатель для передачи его в функции в качестве аргумента.
...
| Method | Описание |
| --- | --- |
|  [CancelEventArgs](../../system.componentmodel/canceleventargs/canceleventargs/)(**bool**) | Конструктор. |
|  [CancelEventArgs](../../system.componentmodel/canceleventargs/canceleventargs/)() | Конструктор; устанавливает свойство Cancel в false. |
...
| **bool** [get_Cancel](../../system.componentmodel/canceleventargs/get_cancel/)() | Возвращает значение, указывающее, следует ли отменить событие. |
| [PrintAction](../printaction/) [get_PrintAction](./get_printaction/)() | Возвращает значение, определяющее действие печати, представляемое текущим объектом. |
...
| void [set_Cancel](../../system.componentmodel/canceleventargs/set_cancel/)(**bool**) | Устанавливает значение, указывающее, следует ли отменить событие. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й аргумент шаблона как слабый указатель (вместо совместного). Позволяет переключать указатели в контейнерах в режим weak. |
...
| Поле | Описание |
| --- | --- |
| static [Empty](../../system/eventargs/empty/) | Статический член, представляющий \"пустой\" [EventArgs](../../system/eventargs/) совместный указатель (нулевой указатель). |

## См. также

* Класс [CancelEventArgs](../../system.componentmodel/canceleventargs/)
