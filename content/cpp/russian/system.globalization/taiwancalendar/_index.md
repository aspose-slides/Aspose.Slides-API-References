...
description: "Тайваньский календарь. Объекты этого класса должны создаваться только с помощью функции System::MakeObject() . Никогда не создавайте экземпляр этого типа в стеке или с помощью оператора new, так как это приведёт к ошибкам выполнения и/или сбоям утверждений. Всегда оборачивайте этот класс в указатель System::SmartPtr и используйте этот указатель для передачи его в функции в качестве аргумента."
type: docs
weight: 326
url: /ru/system.globalization/taiwancalendar/
---
...
| int [GetDaysInMonth](./getdaysinmonth/)(int, int, int) const override | Возвращает количество дней в конкретном месяце. |
| virtual int [GetDaysInMonth](./getdaysinmonth/)(int, int) const | Возвращает количество дней в конкретном месяце. |
| virtual int [GetDaysInMonth](./getdaysinmonth/)(int, int, int) const | Возвращает количество дней в конкретном месяце. |
| int [GetDaysInYear](./getdaysinyear/)(int, int) const override | Возвращает количество дней в конкретном году. |
| virtual int [GetDaysInYear](./getdaysinyear/)(int) const | Возвращает количество дней в конкретном году. |
| virtual int [GetDaysInYear](./getdaysinyear/)(int, int) const | Возвращает количество дней в конкретном году. |
...
| static [CalendarPtr](../calendarptr/) [ReadOnly](../calendar/readonly/)(const [CalendarPtr](../calendarptr/)\&) | Возвращает только читаемую версию календаря. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й шаблонный аргумент в weak pointer (а не shared). Позволяет переключать указатели в контейнерах в режим weak. |
...
| virtual int [ToFourDigitYear](../calendar/tofourdigityear/)(int) const | Преобразует год в четырёхзначный с использованием свойства TwoDigitYearMax. |
