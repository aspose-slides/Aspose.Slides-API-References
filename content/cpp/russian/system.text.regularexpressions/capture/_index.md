...
description: "Результат сопоставления одиночного подпредложения. Объекты этого класса должны создаваться только с помощью функции System::MakeObject(). Никогда не создавайте экземпляр этого типа в стеке или с помощью оператора new, так как это приведёт к ошибкам выполнения и/или сбоям проверки assert. Всегда оборачивайте этот класс в указатель System::SmartPtr и используйте этот указатель для передачи его в функции в качестве аргумента."
type: docs
weight: 1
url: /ru/system.text.regularexpressions/capture/
---
## Capture класс
...
| Метод | Описание |
| --- | --- |
|  [Capture](./capture/)(const [UStringPtr](../ustringptr/)\&, int, int) | Конструктор. |
...
| int [get_Index](./get_index/)() const | Возвращает индекс захваченной подстроки. |
| int [get_Length](./get_length/)() const | Возвращает длину захваченной подстроки. |
| [String](../../system/string/) [get_Value](./get_value/)() const | Возвращает захваченную подстроку. |
...
| [String](../../system/string/) [ToString](./tostring/)() const override | Возвращает захваченную подстроку. |
