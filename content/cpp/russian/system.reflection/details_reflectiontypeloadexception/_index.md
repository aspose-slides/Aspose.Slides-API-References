...
description: "ReflectionTypeLoadException выбрасывается методом Module.GetTypes, если любые классы в модуле не удаётся загрузить. Никогда не создавайте экземпляры этого класса вручную. Используйте класс ReflectionTypeLoadException вместо этого. Никогда не оборачивайте экземпляры класса ReflectionTypeLoadException в System::SmartPtr."
type: docs
weight: 66
url: /ru/system.reflection/details_reflectiontypeloadexception/
---
## Details_ReflectionTypeLoadException класс

ReflectionTypeLoadException выбрасывается методом Module.GetTypes, если любые классы в модуле не удаётся загрузить. Никогда не создавайте экземпляры этого класса вручную. Используйте класс ReflectionTypeLoadException вместо этого. Никогда не оборачивайте экземпляры класса ReflectionTypeLoadException в [System::SmartPtr](../../system/smartptr/).
...
| **int32_t** [get_HResult](../../system/details_exception/get_hresult/)() const | Возвращает 32-битное целое значение, представляющее код HRESULT, связанный с исключением, представленным текущим объектом. |
...
| virtual [System::ArrayPtr](../../system/arrayptr/)\<[System::Exception](../../system/exception/)\> [get_LoaderExceptions](./get_loaderexceptions/)() const | Возвращает исключения, вызвавшие это исключение. |
...
| virtual [System::ArrayPtr](../../system/arrayptr/)\<[System::TypeInfo](../../system/typeinfo/)\> [get_Types](./get_types/)() const | Возвращает информацию о типе причин исключения. |
...
| void [Lock](../../system/object/lock/)() | Реализует оператор C# lock() блокировку. Вызывайте напрямую или используйте объект-сторож [LockContext](../../system/lockcontext/). |
...
| void [set_HResult](../../system/details_exception/set_hresult/)(**int32_t**) | Устанавливает HRESULT, числовое кодированное значение, которое назначается конкретному исключению. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-ый шаблонный параметр в виде слабой ссылки (а не общей). Позволяет переключать указатели в контейнерах в слабый режим. |
