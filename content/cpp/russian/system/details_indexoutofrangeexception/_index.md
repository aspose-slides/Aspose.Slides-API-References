...
description: "IndexOutOfRangeException выбрасывается, когда попытка доступа к элементу коллекции осуществляется с индексом, выходящим за её границы. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс IndexOutOfRangeException. Никогда не оборачивайте экземпляры класса IndexOutOfRangeException в System::SmartPtr."
type: docs
weight: 482
url: /ru/system/details_indexoutofrangeexception/
---
## Details_IndexOutOfRangeException класс

IndexOutOfRangeException выбрасывается, когда пытаются получить доступ к элементу коллекции, используя индекс, находящийся за пределами её диапазона. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс IndexOutOfRangeException. Никогда не оборачивайте экземпляры класса IndexOutOfRangeException в [System::SmartPtr](../smartptr/).
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | Эмулирует сравнение чисел с плавающей точкой в стиле C#, при котором два NaN считаются равными, несмотря на то, что согласно IEC 60559:1989 NaN не равен никакому значению, включая NaN. |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | Эмулирует сравнение чисел с плавающей точкой в стиле C#, при котором два NaN считаются равными, несмотря на то, что согласно IEC 60559:1989 NaN не равен никакому значению, включая NaN. |
...
| **int32_t** [get_HResult](../details_exception/get_hresult/)() const | Возвращает 32-разрядное целое значение, которое является кодом HRESULT, связанным с исключением, представляемым текущим объектом. |
...
| virtual [ExceptionWrapper](../exceptionwrapper/)\<[Details_Exception](../details_exception/)\> [GetBaseException](../details_exception/getbaseexception/)() const | Возвращает копию объекта Exception, представляющего самое глубокое внутреннее исключение. |
...
| static std::enable_if<\![IsSmartPtr](../issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../object/referenceequals/)(T const\&, std::nullptr_t) | Сравнивает объект типa значений со значением nullptr по ссылке. |
...
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й аргумент шаблона как слабый указатель (а не shared). Позволяет переключать указатели в контейнерах в режим слабых. |
