...
description: "NotImplementedException генерируется, когда вызывается метод, который не реализован и служит заглушкой. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс NotImplementedException. Никогда не оборачивайте экземпляры класса NotImplementedException в System::SmartPtr."
type: docs
weight: 573
url: /ru/system/details_notimplementedexception/
---
## Details_NotImplementedException класс

NotImplementedException генерируется, когда вызывается метод, который не реализован и служит заглушкой. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс NotImplementedException. Никогда не оборачивайте экземпляры класса NotImplementedException в [System::SmartPtr](../smartptr/).
...
| Detail::SmartPtrCounter * [GetCounter](../object/getcounter/)() | Получает структуру данных счётчика ссылок, связанного с объектом. |
...
|  [Object](../object/object/)([Object](../object/) const\&) | Конструктор копирования. На самом деле ничего не копирует, а просто инициализирует новый объект и позволяет копировать конструкторы подклассов. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | Оператор присваивания. На самом деле ничего не копирует, а просто инициализирует новый объект и позволяет копировать конструкторы подклассов. |
...
| void [set_HResult](../details_exception/set_hresult/)(**int32_t**) | Устанавливает HRESULT, закодированное численное значение, присваиваемое определённому исключению. |
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й аргумент шаблона как слабый указатель (а не разделяемый). Позволяет переключать указатели в контейнерах в режим слабых ссылок. |
