...
description: "ArgumentException выбрасывается, когда переданный вызываемому методу аргумент недействителен. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс ArgumentException. Никогда не оборачивайте экземпляры класса ArgumentException в System::SmartPtr."
type: docs
weight: 326
url: /ru/system/details_argumentexception/
---
## Details_ArgumentException класс


ArgumentException выбрасывается, когда переданный методу аргумент недействителен. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс ArgumentException. Никогда не оборачивайте экземпляры класса ArgumentException в [System::SmartPtr](../smartptr/).
...
| void [set_HResult](../details_exception/set_hresult/)(**int32_t**) | Устанавливает HRESULT, кодированное числовое значение, присваиваемое конкретному исключению. |
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й аргумент шаблона в слабый указатель (вместо разделяемого). Позволяет переключать указатели в контейнерах в слабый режим. |
...
| virtual  [~Object](../object/~object/)() | Разрушает объект. Освобождает все внутренние структуры данных. |
## Примечания


ArgumentNullException выбрасывается, когда вызываемому методу передаётся нулевой аргумент, хотя метод ожидает ненулевое значение. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс ArgumentNullException. Никогда не оборачивайте экземпляры класса ArgumentNullException в [System::SmartPtr](../smartptr/). 
## См. также
