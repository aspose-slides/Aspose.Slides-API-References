...
description: "Исключение, которое бросается, когда метод вызывается у объекта, находящегося в состоянии, несовместимом с этим вызовом. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс InvalidOperationException. Никогда не оборачивайте экземпляры класса InvalidOperationException в System::SmartPtr."
type: docs
weight: 508
url: /ru/system/details_invalidoperationexception/
---
## Details_InvalidOperationException класс


Исключение, которое бросается, когда метод вызывается у объекта, находящегося в состоянии, несовместимом с этим вызовом. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс InvalidOperationException. Никогда не оборачивайте экземпляры класса InvalidOperationException в [System::SmartPtr](../smartptr/).
...
| **int32_t** [get_HResult](../details_exception/get_hresult/)() const | Возвращает 32-битное целочисленное значение, которое представляет собой код HRESULT, связанный с исключением, представленным текущим объектом. |
...
| static std::enable_if<\![IsSmartPtr](../issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../object/referenceequals/)(T const\&, std::nullptr_t) | Сравнивает ссылкой объект типового значения с nullptr. |
...
| void [set_HResult](../details_exception/set_hresult/)(**int32_t**) | Устанавливает HRESULT, закодированное числовое значение, назначаемое определенному исключению. |
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й аргумент шаблона как слабый указатель (вместо shared). Позволяет переключать указатели в контейнерах в режим слабых. |
