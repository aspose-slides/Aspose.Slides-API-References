...
description: "RankException выбрасывается, когда в метод передаётся массивный аргумент с количеством измерений, отличающимся от ожидаемого. Никогда не создавайте экземпляры этого класса вручную. Используйте класс RankException. Никогда не помещайте экземпляры класса RankException в System::SmartPtr."
type: docs
weight: 677
url: /ru/system/details_rankexception/
---
## Details_RankException класс

RankException выбрасывается, когда в метод передаётся массивный аргумент с количеством измерений, отличающимся от ожидаемого. Никогда не создавайте экземпляры этого класса вручную. Используйте класс RankException вместо этого. Никогда не помещайте экземпляры класса RankException в [System::SmartPtr](../smartptr/).
...
| void [Lock](../object/lock/)() | Реализует блокировку, соответствующую оператору C# lock(). Вызывайте напрямую или используйте охранный объект [LockContext](../lockcontext/). |
...
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й шаблонный аргумент слабой ссылкой (а не общей). Позволяет переключать указатели в контейнерах в режим слабых ссылок. |
...
| void [Unlock](../object/unlock/)() | Реализует разблокировку, соответствующую оператору C# lock(). Вызывайте напрямую или используйте охранный объект [LockContext](../lockcontext/). |
