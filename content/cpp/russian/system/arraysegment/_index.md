...
description: "Представляет сегмент одномерного массива. Этот тип следует выделять в стеке и передавать в функции по значению или по ссылке. Никогда не используйте класс System::SmartPtr для управления объектами этого типа."
type: docs
weight: 40
url: /ru/system/arraysegment/
---
## ArraySegment класс
...
| **int32_t** [GetHashCode](./gethashcode/)() const override | Аналог метода C# [Object.GetHashCode()](../object/gethashcode/). Позволяет хешировать пользовательские объекты. |
...
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-ый шаблонный аргумент как слабый указатель (а не shared). Позволяет переключать указатели в контейнерах в режим weak. |
...
int main()
{
  ...
  // Создать сегмент массива, содержащий весь массив.
  auto fullArray = MakeObject<ArraySegment<String>>(array);

  // Вывести элементы сегмента массива.
  Print(fullArray);

  // Создать сегмент массива.
