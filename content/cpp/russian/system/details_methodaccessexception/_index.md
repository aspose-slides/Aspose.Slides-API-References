...
description: "MemberAccessException выбрасывается, когда пытаются получить доступ к несуществующему методу или когда доступ к методу не разрешён. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс MethodAccessException. Никогда не оборачивайте экземпляры класса MethodAccessException в System::SmartPtr."
type: docs
weight: 560
url: /ru/system/details_methodaccessexception/
---
## Details_MethodAccessException класс

MemberAccessException выбрасывается, когда пытаются получить доступ к несуществующему методу или когда доступ к методу не разрешен. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс MethodAccessException. Никогда не оборачивайте экземпляры класса MethodAccessException в [System::SmartPtr](../smartptr/).
...
| **int32_t** [get_HResult](../details_exception/get_hresult/)() const | Возвращает 32-битное целое значение — код HRESULT, связанный с исключением, представленным текущим объектом. |
...
| const [System::TypeInfo](../typeinfo/)\& [GetType](./gettype/)() const override | Возвращает фактический тип объекта. Аналог вызова C# [System.Object.GetType()](../object/gettype/). |
...
| [Object](../object/object/)([Object](../object/) const\&) | Конструктор копирования. На самом деле ничего не копирует, просто инициализирует новый объект и позволяет копирующее создание подклассов. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | Оператор присваивания. На самом деле ничего не копирует, просто инициализирует новый объект и позволяет копирующее создание подклассов. |
...
| int [RemovedSharedRefs](../object/removedsharedrefs/)(int) | Уменьшает общий счётчик ссылок на указанное значение. |
...
* Класс [Details_MemberAccessException](../details_memberaccessexception/)
