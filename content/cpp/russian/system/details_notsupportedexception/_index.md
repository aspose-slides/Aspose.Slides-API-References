...
description: "NotSupportedException выбрасывается, когда вызываемый метод не поддерживается или когда попытка выполнить операцию над потоком не поддерживается. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс NotSupportedException. Никогда не оборачивайте экземпляры класса NotSupportedException в System::SmartPtr."
type: docs
weight: 586
url: /ru/system/details_notsupportedexception/
---
## Details_NotSupportedException класс

NotSupportedException выбрасывается, когда вызываемый метод не поддерживается или когда попытка выполнить операцию над потоком не поддерживается. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс NotSupportedException. Никогда не оборачивайте экземпляры класса NotSupportedException в [System::SmartPtr](../smartptr/).
...
| virtual **int32_t** [GetHashCode](../object/gethashcode/)() const | Аналог метода C# [Object.GetHashCode()](../object/gethashcode/). Позволяет выполнять хеширование пользовательских объектов. |
...
| static std::enable_if<\![IsSmartPtr](../issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../object/referenceequals/)(T const\&, std::nullptr_t) | Сравнивает объект значимого типа со ссылкой nullptr. |
...
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й аргумент шаблона как слабый указатель (а не совместный). Позволяет переключать указатели в контейнерах в режим слабых указателей. |
