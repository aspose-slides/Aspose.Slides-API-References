...
description: "ObjectDisposedException выбрасывается, когда метод вызывается у освобождённого объекта. Никогда не создавайте экземпляры этого класса вручную. Используйте класс ObjectDisposedException вместо этого. Никогда не оборачивайте экземпляры класса ObjectDisposedException в System::SmartPtr."
type: docs
weight: 612
url: /ru/system/details_objectdisposedexception/
---
## Details_ObjectDisposedException класс

ObjectDisposedException выбрасывается, когда метод вызывается у освобождённого объекта. Никогда не создавайте экземпляры этого класса вручную. Используйте класс ObjectDisposedException вместо этого. Никогда не оборачивайте экземпляры класса ObjectDisposedException в [System::SmartPtr](../smartptr/).
...
|  [Object](../object/object/)([Object](../object/) const\&) | Конструктор копирования. На самом деле ничего не копирует, просто инициализирует новый объект и позволяет копировать конструктором подклассы. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | Оператор присваивания. На самом деле ничего не копирует, просто инициализирует новый объект и позволяет копировать конструктором подклассы. |
...
| void [set_HResult](../details_exception/set_hresult/)(**int32_t**) | Устанавливает HRESULT, кодовое числовое значение, которое назначается конкретному исключению. |
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й аргумент шаблона в виде слабой ссылки (вместо разделяемой). Позволяет переключать указатели в контейнерах в слабый режим. |
...
| int [SharedRefRemovedSafe](../object/sharedrefremovedsafe/)() | Уменьшает и возвращает счётчик разделяемых ссылок. Не следует вызывать напрямую; вместо этого используйте умные указатели или ThisProtector. |
...
* Класс [Details_InvalidOperationException](../details_invalidoperationexception/)
