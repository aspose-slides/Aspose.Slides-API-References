...
description: "InvalidProgramException присутствует только по причинам совместимости. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс InvalidProgramException. Никогда не оборачивайте экземпляры класса InvalidProgramException в System::SmartPtr."
type: docs
weight: 521
url: /ru/system/details_invalidprogramexception/
---
## Details_InvalidProgramException класс


InvalidProgramException присутствует только по причинам совместимости. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс InvalidProgramException. Никогда не оборачивайте экземпляры класса InvalidProgramException в [System::SmartPtr](../smartptr/).
...
| **int32_t** [get_HResult](../details_exception/get_hresult/)() const | Возвращает 32-битное целое значение, которое является кодом HRESULT, связанным с исключением, представленным текущим объектом. |
...
| virtual [String](../string/) [get_StackTrace](../details_exception/get_stacktrace/)() const | Возвращает строку, содержащую стек вызовов. |
...
| void [Lock](../object/lock/)() | Реализует блокировку оператора C# lock(). Вызовите напрямую или используйте объект-сторож [LockContext](../lockcontext/). |
...
|  [Object](../object/object/)([Object](../object/) const\&) | Конструктор копирования. На самом деле ничего не копирует, только инициализирует новый объект и позволяет копировать конструировать подклассы. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | Оператор присваивания. На самом деле ничего не копирует, только инициализирует новый объект и позволяет копировать конструировать подклассы. |
...
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й параметр шаблона как слабую ссылку (вместо shared). Позволяет переключать ссылки в контейнерах в режим weak. |
...
| void [Unlock](../object/unlock/)() | Реализует разблокировку оператора C# lock(). Вызовите напрямую или используйте объект-сторож [LockContext](../lockcontext/). |
