...
description: "ArgumentOutOfRangeException вызывается, когда вызываемому методу передаётся аргумент, выходящий за ожидаемый диапазон значений для этого аргумента. Никогда не создавайте экземпляры этого класса вручную. Вместо этого используйте класс ArgumentOutOfRangeException. Никогда не оборачивайте экземпляры класса ArgumentOutOfRangeException в System::SmartPtr."
type: docs
weight: 352
url: /ru/system/details_argumentoutofrangeexception/
---
## Details_ArgumentOutOfRangeException класс
...
|  [Object](../object/object/)([Object](../object/) const\&) | Конструктор копирования. На самом деле ничего не копирует, а просто инициализирует новый объект и позволяет конструировать копии подклассов. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | Оператор присваивания. На самом деле ничего не копирует, а просто инициализирует новый объект и позволяет конструировать копии подклассов. |
...
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Задаёт n-й аргумент шаблона слабой указкой (а не shared). Позволяет переключать указатели в контейнерах в слабый режим. |
| int [SharedCount](../object/sharedcount/)() const | Получает текущое значение счётчика общих ссылок. |
