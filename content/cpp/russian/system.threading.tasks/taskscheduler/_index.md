...
description: Представляет объект, который обрабатывает низкоуровневую работу по постановке задач в очередь на потоки.
type: docs
weight: 79
url: /ru/system.threading.tasks/taskscheduler/
---
## TaskScheduler класс
...
| static [SharedPtr](../../system/sharedptr/)\<[TaskScheduler](./)\> [FromCurrentSynchronizationContext](./fromcurrentsynchronizationcontext/)() | Создаёт [TaskScheduler](./), связанный с текущим потоком. |
| static [SharedPtr](../../system/sharedptr/)\<[TaskScheduler](./)\> [get_Current](./get_current/)() | Получает [TaskScheduler](./), связанный с текущей выполняемой задачей. |
| static const [SharedPtr](../../system/sharedptr/)\<[TaskScheduler](./)\>\& [get_Default](./get_default/)() | Получает экземпляр [TaskScheduler](./) по умолчанию, предоставляемый фреймворком. |
| **int32_t** [get_Id](./get_id/)() const | Получает уникальный идентификатор для этого [TaskScheduler](./). |
| virtual **int32_t** [get_MaximumConcurrencyLevel](./get_maximumconcurrencylevel/)() const | Указывает максимальный уровень параллелизма, который может поддерживать этот [TaskScheduler](./). |
