...
description: Представляет различные операции с цветом, используемые для цветовых трансформаций.
type: docs
weight: 1704
url: /ru/aspose.slides/icoloroperation/
---
## IColorOperation класс

Представляет различные операции с цветом, используемые для цветовых трансформаций.
...
| virtual [ColorTransformOperation](../colortransformoperation/) [get_OperationType](./get_operationtype/)() const | Возвращает тип операции. Только для чтения [ColorTransformOperation](../colortransformoperation/). |
| virtual **float** [get_Parameter](./get_parameter/)() | Возвращает параметр операции. Только для чтения **float**. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й шаблонный аргумент в виде слабого указателя (а не общедоступного). Позволяет переключать указатели в контейнерах в режим слабых ссылок. |
