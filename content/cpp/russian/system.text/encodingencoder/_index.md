...
description: "Кодировщик, использующий объект кодирования для кодирования. Объекты этого класса должны создаваться только с помощью функции System::MakeObject(). Никогда не создавайте экземпляр этого типа в стеке или с помощью оператора new, так как это приведёт к ошибкам выполнения и/или сбоям проверок. Всегда оборачивайте этот класс в указатель System::SmartPtr и используйте этот указатель для передачи в функции в качестве аргумента."
type: docs
weight: 248
url: /ru/system.text/encodingencoder/
---
## Класс EncodingEncoder

[Encoder](../encoder/) который использует объект кодирования для кодирования. Объекты этого класса должны выделяться только с помощью функции [System::MakeObject()](../../system/makeobject/). Никогда не создавайте экземпляр этого типа на стеке или с помощью оператора new, так как это приведёт к ошибкам во время выполнения и/или сбоям проверок. Всегда оборачивайте этот класс в указатель [System::SmartPtr](../../system/smartptr/) и используйте этот указатель для передачи в функции в качестве аргумента.
...
| Метод | Описание |
| --- | --- |
| virtual void [Convert](./convert/)(const char_t *, int, **uint8_t** *, int, **bool**, int\&, int\&, **bool**\&) | Преобразует символы в байты. |
| virtual void [Convert](./convert/)([ArrayPtr](../../system/arrayptr/)\<char_t\>, int, int, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int, **bool**, int\&, int\&, **bool**\&) | Преобразует символы в байты. |
...
| [EncoderFallbackPtr](../../system/encoderfallbackptr/) [get_Fallback](../encoder/get_fallback/)() const | Получает запасной обработчик ошибок. |
| const [EncoderFallbackBufferPtr](../../system/encoderfallbackbufferptr/) [get_FallbackBuffer](../encoder/get_fallbackbuffer/)() const | Получает буфер запасного режима. |
| virtual int [GetByteCount](../encoder/getbytecount/)([ArrayPtr](../../system/arrayptr/)\<char_t\>, int, int, **bool**) | Возвращает количество байт, необходимых для кодирования буфера. |
| virtual int [GetByteCount](../encoder/getbytecount/)(const char_t *, int, **bool**) | Возвращает количество байт, необходимых для кодирования буфера. |
| virtual int [GetBytes](../encoder/getbytes/)([ArrayPtr](../../system/arrayptr/)\<char_t\>, int, int, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, **bool**) | Получает байты, полученные в результате кодирования буфера. |
| virtual int [GetBytes](../encoder/getbytes/)(const char_t *, int, **uint8_t** *, int, **bool**) | Получает байты, полученные в результате кодирования буфера. |
...
| virtual void [Reset](../encoder/reset/)() | Очищает внутреннее состояние кодера. |
| void [set_Fallback](../encoder/set_fallback/)(const [EncoderFallbackPtr](../../system/encoderfallbackptr/)\&) | Устанавливает запасной обработчик ошибок. |
...
| int [SharedCount](../../system/object/sharedcount/)() const | Возвращает текущее значение счётчика совместно используемых ссылок. |
