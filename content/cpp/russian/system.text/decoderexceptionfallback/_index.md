...
description: "Обеспечивает стратегию отката, бросающую исключения. Объекты этого класса следует создавать только с помощью функции System::MakeObject(). Никогда не создавайте экземпляр этого типа в стеке или с использованием оператора new, так как это приведёт к ошибкам выполнения и/или нарушениям утверждений. Всегда оборачивайте этот класс в указатель System::SmartPtr и используйте этот указатель для передачи его в функции в качестве аргумента."
type: docs
weight: 27
url: /ru/system.text/decoderexceptionfallback/
---
## DecoderExceptionFallback класс

Обеспечивает стратегию отката, бросающую исключения. Объекты этого класса следует создавать только с помощью функции [System::MakeObject()](../../system/makeobject/). Никогда не создавайте экземпляр этого типа в стеке или с использованием оператора new, так как это приведёт к ошибкам выполнения и/или нарушениям утверждений. Всегда оборачивайте этот класс в указатель [System::SmartPtr](../../system/smartptr/) и используйте этот указатель для передачи его в функции в качестве аргумента.
...
| Метод | Описание |
| --- | --- |
| [DecoderFallbackBufferPtr](../../system/decoderfallbackbufferptr/) [CreateFallbackBuffer](./createfallbackbuffer/)() override | Создаёт буфер отката. |
...
| static const [DecoderFallbackPtr](../../system/decoderfallbackptr/) [get_ExceptionFallback](../decoderfallback/get_exceptionfallback/)() | Возвращает реализацию отката по умолчанию для исключений. |
| int [get_MaxCharCount](./get_maxcharcount/)() const override | Возвращает максимальное количество символов, которое может вернуть экземпляр. |
| static const [DecoderFallbackPtr](../../system/decoderfallbackptr/) [get_ReplacementFallback](../decoderfallback/get_replacementfallback/)() | Возвращает реализацию отката по умолчанию для замены. |
| static const [DecoderFallbackPtr](../../system/decoderfallbackptr/) [get_StandardSafeFallback](../decoderfallback/get_standardsafefallback/)() | Возвращает реализацию стандартного безопасного отката по умолчанию. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Устанавливает n-й шаблонный аргумент как слабый указатель (вместо общего). Позволяет переключать указатели в контейнерах в режим слабых. |
...
* Класс [DecoderFallback](../decoderfallback/)
