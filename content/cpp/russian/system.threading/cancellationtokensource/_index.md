...
description: Источник токена отмены, который можно использовать для инициирования уведомлений об отмене.
type: docs
weight: 40
url: /ru/system.threading/cancellationtokensource/
---
...
| Method | Description |
| --- | --- |
| void [Cancel](./cancel/)() | Посылает запрос на отмену. |
|  [CancellationTokenSource](./cancellationtokensource/)() | Создаёт новый [CancellationTokenSource](./). |
| static [SharedPtr](../../system/sharedptr/)\<[CancellationTokenSource](./)\> [CreateLinkedTokenSource](./createlinkedtokensource/)(const [CancellationToken](../cancellationtoken/)\&, const [CancellationToken](../cancellationtoken/)\&) | Создаёт связанный источник токена, который отменяется, когда любой из предоставленных токенов отменяется. |
| void [Dispose](./dispose/)() override | Освобождает все ресурсы, используемые [CancellationTokenSource](./). |
...
| **bool** [get_IsCancellationRequested](./get_iscancellationrequested/)() const | Возвращает, была ли запрошена отмена. |
| [CancellationToken](../cancellationtoken/) [get_Token](./get_token/)() const | Получает токен отмены, связанный с этим источником. |
...
| void [Lock](../../system/object/lock/)() | Реализует блокировку оператора C# lock(). Вызывайте напрямую либо используйте объект-страж [LockContext](../../system/lockcontext/). |
...
| void [Unlock](../../system/object/unlock/)() | Реализует разблокировку оператора C# lock(). Вызывайте напрямую либо используйте объект-страж [LockContext](../../system/lockcontext/). |
