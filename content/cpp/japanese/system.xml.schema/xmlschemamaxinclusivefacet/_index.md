...
description: World Wide Web Consortium (W3C) が定める XML Schema の maxInclusive 要素を表します。このクラスは simpleType 要素の最大値に対する制限を指定するために使用できます。要素の値は maxInclusive 要素の値以下である必要があります。
type: docs
weight: 586
url: /ja/system.xml.schema/xmlschemamaxinclusivefacet/
---
## XmlSchemaMaxInclusiveFacet クラス


XML [Schema](../) の **maxInclusive** 要素を、World Wide [Web](../../system.web/) Consortium (W3C) が定義する形で表します。このクラスは **simpleType** 要素の最大値に対する制限を指定するために使用できます。要素の値は **maxInclusive** 要素の値以下でなければなりません。
...
|  [XmlSchemaMaxInclusiveFacet](./xmlschemamaxinclusivefacet/)() | [XmlSchemaMaxInclusiveFacet](./) クラスの新しいインスタンスを初期化します。 |
...
このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうするとランタイムエラーやアサーション違反が発生します。常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として渡すようにしてください。 
