...
description: XML スキーマからの単純型に対する union 要素を、World Wide Web Consortium (W3C) が定めるように表します。union データ型は simpleType の内容を指定するために使用できます。simpleType 要素の値は、union で指定された代替データ型の集合のいずれかでなければなりません。union 型は常に派生型であり、少なくとも 2 つの代替データ型を含む必要があります。
type: docs
weight: 885
url: /ja/system.xml.schema/xmlschemasimpletypeunion/
---
## XmlSchemaSimpleTypeUnion クラス


XML [Schema](../) の単純型に対する **union** 要素を、World Wide [Web](../../system.web/) Consortium (W3C) が定めるように表します。**union** データ型は **simpleType** の内容を指定するために使用できます。**simpleType** 要素の値は、union で指定された代替データ型の集合のいずれかでなければなりません。Union 型は常に派生型であり、少なくとも2つの代替データ型を含む必要があります。
...
| [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[XmlSchemaSimpleType](../xmlschemasimpletype/)\>\> [get_BaseMemberTypes](./get_basemembertypes/)() | [XmlSchemaSimpleType](../xmlschemasimpletype/) オブジェクトの配列を返します。これは **simpleType** 要素の型を表し、単純型の [XmlSchemaSimpleTypeUnion::get_BaseTypes](./get_basetypes/) と [XmlSchemaSimpleTypeUnion::get_MemberTypes](./get_membertypes/) の値に基づいています。 |
| [SharedPtr](../../system/sharedptr/)\<[XmlSchemaObjectCollection](../xmlschemaobjectcollection/)\> [get_BaseTypes](./get_basetypes/)() | 基底型のコレクションを返します。 |
| [String](../../system/string/) [get_Id](../xmlschemaannotated/get_id/)() | 文字列 ID を返します。 |
| **int32_t** [get_LineNumber](../xmlschemaobject/get_linenumber/)() | **schema** 要素が参照するファイルの行番号を返します。 |
| **int32_t** [get_LinePosition](../xmlschemaobject/get_lineposition/)() | **schema** 要素が参照するファイルの行位置を返します。 |
| [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[XmlQualifiedName](../../system.xml/xmlqualifiedname/)\>\> [get_MemberTypes](./get_membertypes/)() | このスキーマ（または指定された名前空間で示された別のスキーマ）で定義された組み込みデータ型または **simpleType** 要素の修飾されたメンバー名の配列を返します。 |
...
| void [set_MemberTypes](./set_membertypes/)(const [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[XmlQualifiedName](../../system.xml/xmlqualifiedname/)\>\>\&) | 組み込みデータ型または **simpleType** 要素の修飾されたメンバー名の配列を設定します（このスキーマまたは指定された名前空間で示された別のスキーマ）。 |
...
|  [XmlSchemaSimpleTypeUnion](./xmlschemasimpletypeunion/)() | [XmlSchemaSimpleTypeUnion](./) クラスの新しいインスタンスを初期化します。 |
...
このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使ってインスタンスを作成しないでください。そうすると実行時エラーやアサーション違反が発生します。常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを引数として関数に渡してください。 

## 参照

* クラス [XmlSchemaSimpleTypeContent](../xmlschemasimpletypecontent/)
