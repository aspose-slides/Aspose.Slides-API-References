...
description: XML スキーマの include 要素を、World Wide Web Consortium (W3C) が指定する通りに表します。このクラスは外部スキーマから宣言と定義をインクルードするために使用されます。インクルードされた宣言と定義は、包含するスキーマ内で処理できるようになります。
type: docs
weight: 495
url: /ja/system.xml.schema/xmlschemainclude/
---
## XmlSchemaInclude クラス


XML [Schema](../) の **include** 要素を、World Wide [Web](../../system.web/) Consortium (W3C) が定義する通りに表します。このクラスは外部スキーマから宣言と定義をインクルードするために使用されます。インクルードされた宣言と定義は、包含するスキーマ内で処理できるようになります。
...
| [SharedPtr](../../system/sharedptr/)\<[XmlSchemaAnnotation](../xmlschemaannotation/)\> [get_Annotation](./get_annotation/)() | **annotation** の値を返します。 |
...
| [String](../../system/string/) [get_SchemaLocation](../xmlschemaexternal/get_schemalocation/)() | スキーマの Uniform Resource Identifier (URI) の位置を返します。これによりスキーマプロセッサはスキーマが実際に存在する場所を把握できます。 |
...
| void [set_Annotation](./set_annotation/)(const [SharedPtr](../../system/sharedptr/)\<[XmlSchemaAnnotation](../xmlschemaannotation/)\>\&) | **annotation** の値を設定します。 |
...
| void [set_SchemaLocation](../xmlschemaexternal/set_schemalocation/)(const [String](../../system/string/)\&) | スキーマの Uniform Resource Identifier (URI) の位置を設定します。これによりスキーマプロセッサはスキーマが実際に存在する場所を把握できます。 |
...
このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうすると実行時エラーやアサーション失敗が発生します。常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを引数として関数に渡すようにしてください。 
