...
description: "XmlSerializer がオブジェクトのシリアライズまたはデシリアライズに使用される場合に属性のオーバーライドを可能にします。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。実行時エラーやアサーション失敗の原因となります。常にこのクラスを System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡してください。"
type: docs
weight: 14
url: /ja/system.xml.serialization/xmlattributeoverrides/
---
## XmlAttributeOverrides クラス

[XmlSerializer](../xmlserializer/) がオブジェクトのシリアライズまたはデシリアライズに使用される場合、属性のオーバーライドを可能にします。 このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。 スタック上や operator new を使用してこの型のインスタンスを作成しないでください。実行時エラーやアサーション失敗の原因となります。 常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを引数として関数に渡してください。
...
|  [XmlAttributeOverrides](./xmlattributeoverrides/)() | コンストラクタです。 |
