...
description: "自動的にリセットされない待機スレッドに通知するイベントです。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないとランタイムエラーやアサーション失敗が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡すようにしてください。"
type: docs
weight: 144
url: /ja/system.threading/manualresetevent/
---
## ManualResetEvent クラス

自動的にリセットされない待機スレッドに通知するイベントです。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないとランタイムエラーやアサーション失敗が発生します。このクラスは常に [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数の引数として渡すようにしてください。
...
| [EventWaitHandle](../eventwaithandle/eventwaithandle/)(**bool**, [EventResetMode](../eventresetmode/)) | コンストラクタ。 |
...
| [ManualResetEvent](./manualresetevent/)(**bool**) | コンストラクタ。 |
...
| virtual **bool** [Reset](../eventwaithandle/reset/)() | イベントを非シグナル状態に設定します。 |
| virtual **bool** [Set](../eventwaithandle/set/)() | イベントをシグナル状態に設定します。 |
...
| static const [TypeInfo](../../system/typeinfo/)\& [Type](../../system/object/type/)() | [System.Object](../../system/object/) の C# typeof() 構文を実装します。 |
| void [Unlock](../../system/object/unlock/)() | C# の lock() 文のアンロックを実装します。直接呼び出すか、[LockContext](../../system/lockcontext/) センチリーオブジェクトを使用してください。 |
| static **bool** [WaitAll](../waithandle/waitall/)(const [System::ArrayPtr](../../system/arrayptr/)\<[System::SharedPtr](../../system/sharedptr/)\<[WaitHandle](../waithandle/)\>\>\&, int) | すべてのハンドルがシグナルになるまで待機します。 |
| static **bool** [WaitAll](../waithandle/waitall/)(const [System::ArrayPtr](../../system/arrayptr/)\<[System::SharedPtr](../../system/sharedptr/)\<[WaitHandle](../waithandle/)\>\>\&, [TimeSpan](../../system/timespan/)) | すべてのハンドルがシグナルになるまで待機します。 |
| static **bool** [WaitAll](../waithandle/waitall/)(const [System::ArrayPtr](../../system/arrayptr/)\<[System::SharedPtr](../../system/sharedptr/)\<[WaitHandle](../waithandle/)\>\>\&) | すべてのハンドルがシグナルになるまで待機します。 |
| static int [WaitAny](../waithandle/waitany/)(const [System::ArrayPtr](../../system/arrayptr/)\<[System::SharedPtr](../../system/sharedptr/)\<[WaitHandle](../waithandle/)\>\>\&, int) | 任意のハンドルがシグナルになるまで待機します。 |
| static int [WaitAny](../waithandle/waitany/)(const [System::ArrayPtr](../../system/arrayptr/)\<[System::SharedPtr](../../system/sharedptr/)\<[WaitHandle](../waithandle/)\>\>\&, [TimeSpan](../../system/timespan/)) | 任意のハンドルがシグナルになるまで待機します。 |
| static int [WaitAny](../waithandle/waitany/)(const [System::ArrayPtr](../../system/arrayptr/)\<[System::SharedPtr](../../system/sharedptr/)\<[WaitHandle](../waithandle/)\>\>\&) | 任意のハンドルがシグナルになるまで待機します。 |
| virtual **bool** [WaitOne](../waithandle/waitone/)() | ハンドルがシグナルになるのを無制限に待機します。 |
| virtual **bool** [WaitOne](../waithandle/waitone/)(int) | ハンドルがシグナルになるまで待機します。 |
| virtual **bool** [WaitOne](../waithandle/waitone/)([TimeSpan](../../system/timespan/)) | ハンドルがシグナルになるまで待機します。 |
| virtual **bool** [WaitOne](../waithandle/waitone/)(int, **bool**) | ハンドルがシグナルになるまで待機します。 |
...
| [~EventWaitHandle](../eventwaithandle/~eventwaithandle/)() | デストラクタ。 |
...
| フィールド | 説明 |
| --- | --- |
| static [WaitTimeout](../waithandle/waittimeout/) | タイムアウトが超過し何もシグナルが無い場合、配列内のシグナルされたオブジェクトのインデックスを返す代わりに関数が返す特別な値。 |

## 参照

* クラス [EventWaitHandle](../eventwaithandle/)
