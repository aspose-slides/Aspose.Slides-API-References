...
description: "Component クラスを使用した翻訳コードをコンパイル可能にするためのダミークラスです。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や new 演算子でインスタンスを作成しないでください。そうしないと実行時エラーやアサーション障害が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数への引数として使用してください。"
type: docs
weight: 40
url: /ja/system.componentmodel/component/
---
## コンポーネント クラス


[Component](./) クラスを使用した翻訳コードをコンパイル可能にするためのダミークラスです。[System::MakeObject()](../../system/makeobject/) 関数を使用してのみ、このクラスのオブジェクトを割り当てるべきです。スタック上や new 演算子を使用してこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション失敗が発生します。常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として渡してください。
...
| メソッド | 説明 |
| --- | --- |
|  [Component](./component/)() | RTTI 情報です。 |
| void [Dispose](./dispose/)(**bool**) | Disposable パターンのサポートです。何もしません。 |
...
| **bool** [get_DesignMode](./get_designmode/)() | コンポーネントがデザインモードかどうかを確認します。 |
...
* クラス [MarshalByRefObject](../../system/marshalbyrefobject/)
* クラス [IComponent](../icomponent/)
