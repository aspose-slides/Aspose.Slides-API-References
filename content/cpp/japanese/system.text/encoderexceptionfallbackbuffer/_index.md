...
description: "例外を投げるエンコーディングフォールバック戦略用のバッファです。実際には何も格納せず、代わりに例外を投げます。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使ってインスタンスを作成しないでください。そうするとランタイムエラーやアサーションエラーが発生する可能性があります。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡してください。"
type: docs
weight: 157
url: /ja/system.text/encoderexceptionfallbackbuffer/
---
...
|  int [get_Remaining](./get_remaining/)() const override | 残り文字数を取得します。 |
...
|  virtual void [Reset](../encoderfallbackbuffer/reset/)() | バッファを初期状態にリセットします。 |
