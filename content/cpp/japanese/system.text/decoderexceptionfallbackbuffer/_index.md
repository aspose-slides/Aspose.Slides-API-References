...
description: "例外をスローするデコードフォールバック戦略用バッファです。実際には何も保存せず、代わりにスローします。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうすると実行時エラーやアサーション違反が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数への引数として渡すようにしてください。"
type: docs
weight: 40
url: /ja/system.text/decoderexceptionfallbackbuffer/
---
## DecoderExceptionFallbackBuffer クラス


[Buffer](../../system/buffer/) は例外をスローするデコードフォールバック戦略です。実際には何も保存せず、代わりにスローします。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうすると実行時エラーやアサーション違反が発生します。このクラスは常に [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として渡すようにしてください。
...
| **bool** [Fallback](./fallback/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int) override | デコード失敗を処理します。 |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | 内部用です。 |
| int [get_Remaining](./get_remaining/)() const override | 残りの文字数を取得します。 |
...
| char_t [GetNextChar](./getnextchar/)() override | 次の利用可能な文字を取得します。 |
...
| virtual void [Reset](../decoderfallbackbuffer/reset/)() | バッファを初期状態にリセットします。 |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | n 番目のテンプレート引数を弱参照ポインタに設定します（共有ではなく）。これによりコンテナ内のポインタを弱モードに切り替えることができます。 |
...
* クラス [DecoderFallbackBuffer](../decoderfallbackbuffer/)
* 名前空間 [System::Text](../)
* ライブラリ [Aspose.Slides](../../)