...
description: "ヒジュラ暦。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうすると実行時エラーやアサーション障害が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数への引数として渡すようにしてください。"
type: docs
weight: 157
url: /ja/system.globalization/hijricalendar/
---
## HijriCalendar クラス

Hijri カレンダー。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうすると実行時エラーやアサーション障害が発生します。このクラスは常に [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として渡すようにしてください。
...
| int [get_HijriAdjustment](./get_hijriadjustment/)() const | ヒジュラ調整を取得します。 |
...
| void [set_HijriAdjustment](./set_hijriadjustment/)(int) | ヒジュラ調整を設定します。 |
...
| フィールド | 説明 |
| --- | --- |
| static constexpr [HijriEra](./hijriera/) | 現在のヒジュラ時代。 |
## 参照
