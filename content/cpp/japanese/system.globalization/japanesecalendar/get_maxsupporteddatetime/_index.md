---
title: get_MaxSupportedDateTime()
second_title: Aspose.Slides for C++ API リファレンス
description: カレンダーがサポートする最大の日時です。
type: docs
weight: 40
url: /ja/system.globalization/japanesecalendar/get_maxsupporteddatetime/
---
## JapaneseCalendar::get_MaxSupportedDateTime() const メソッド


カレンダーがサポートする最大の日時です。

```cpp
DateTime System::Globalization::JapaneseCalendar::get_MaxSupportedDateTime() const override
```

## 参照

* クラス [DateTime](../../../system/datetime/)
* クラス [JapaneseCalendar](../)
* 名前空間 [System::Globalization](../../)
* ライブラリ [Aspose.Slides](../../../)