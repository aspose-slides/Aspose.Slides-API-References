...
description: "Close() または Close() メソッドが呼び出された後に、ソケットが接続されたままになるかどうかを指定します。また、データの送信が継続する場合にソケットが接続されたままである期間も指定します。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してインスタンスを作成しないでください。そうしないとランタイムエラーやアサーション違反が発生します。このクラスは必ず System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡すようにしてください。"
type: docs
weight: 27
url: /ja/system.net.sockets/lingeroption/
---
## LingerOption クラス
...
| **bool** [get_Enabled](./get_enabled/)() | 保留中のデータをすべて送信しようとする試みで、ソケットがクローズを遅延させるかどうかを示す値を取得します。 |
| **int32_t** [get_LingerTime](./get_lingertime/)() | 遅延タイムアウト（秒）を取得します。 |
...
| void [set_Enabled](./set_enabled/)(**bool**) | 保留中のデータをすべて送信しようとする試みで、ソケットがクローズを遅延させるかどうかを示す値を設定します。 |
| void [set_LingerTime](./set_lingertime/)(**int32_t**) | 遅延タイムアウト（秒）を設定します。 |
