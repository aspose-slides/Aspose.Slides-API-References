...
description: "スタックフレームのコレクションです。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうすると実行時エラーやアサーション違反が発生する可能性があります。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数への引数として渡すようにしてください。"
type: docs
weight: 66
url: /ja/system.diagnostics/stacktrace/
---
## StackTrace クラス


スタックフレームのコレクションです。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうすると実行時エラーやアサーション違反が発生する可能性があります。このクラスは常に [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として渡すようにしてください。
...
| int [get_FrameCount](./get_framecount/)() const | スタックトレース内のフレーム数を取得します。 |
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | オブジェクトに関連付けられた参照カウンタのデータ構造を取得します。 |
| virtual [SharedPtr](../../system/sharedptr/)\<[StackFrame](../stackframe/)\> [GetFrame](./getframe/)(**uint32_t**) | スタックフレームを取得します。 |
...
| const [StackTrace](./)\& [operator=](./operator_equal/)(const [StackTrace](./)\&) const | 代入はできません。 |
...
|  [StackTrace](./stacktrace/)(const [StackTrace](./)\&) | コピーは行われません。 |
