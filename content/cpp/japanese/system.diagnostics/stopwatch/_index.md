---
title: Stopwatch
second_title: C++ 向け Aspose.Slides API リファレンス
description: "時間測定を可能にします。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション障害が発生します。常にこのクラスを System::SmartPtr ポインタでラップし、そのポインタを引数として関数に渡してください。"
type: docs
weight: 79
url: /ja/system.diagnostics/stopwatch/
---
## Stopwatch クラス

時間測定を可能にします。 このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。 スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション障害が発生します。 常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを引数として関数に渡してください。
...
| [TimeSpan](../../system/timespan/) [get_Elapsed](./get_elapsed/)() const | 現在のインスタンスで測定された総経過時間を取得します。 |
| **int64_t** [get_ElapsedMilliseconds](./get_elapsedmilliseconds/)() const | 現在のインスタンスで測定された総経過時間をミリ秒単位で取得します。 |
| **int64_t** [get_ElapsedTicks](./get_elapsedticks/)() const | 現在のインスタンスで測定された総経過時間をタイマーティック単位で取得します。 |
| **bool** [get_IsRunning](./get_isrunning/)() const | ストップウォッチが実行中かどうかを確認します。 |
...
| void [Reset](./reset/)() | 時間測定を停止し、測定された間隔をゼロに設定します。 |
| void [Restart](./restart/)() | 測定された間隔をゼロに設定し、次に時間測定を開始します。 |
...
| [Object](../../system/object/) * [SharedRefAdded](../../system/object/sharedrefadded/)() | 共有リファレンスカウントをインクリメントします。直接呼び出さないでください。代わりにスマートポインタまたは ThisProtector を使用してください。 |
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | 共有リファレンスカウントをデクリメントし、返します。直接呼び出さないでください。代わりにスマートポインタまたは ThisProtector を使用してください。 |
| void [Start](./start/)() | 時間測定を開始します。 |
| static [SharedPtr](../../system/sharedptr/)\<[Stopwatch](./)\> [StartNew](./startnew/)() | 新しい [Stopwatch](./) オブジェクトを作成し、測定を開始します。 |
| void [Stop](./stop/)() | 時間測定を停止します。 |
|  [Stopwatch](./stopwatch/)() | RTTI 情報です。 |
