...
description: スライド領域のサイズを指定します（通常ビューの (restoredTop の子の場合は幅、restoredLeft の子の場合は高さ) で、領域が可変の復元サイズ（最小化でも最大化でもない）です）。
type: docs
weight: 4538
...
スライド領域のサイズを指定します（通常ビューの (restoredTop の子の場合は幅、restoredLeft の子の場合は高さ) で、領域が可変の復元サイズ（最小化でも最大化でもない））。
...
| **bool** [get_AutoAdjust](./get_autoadjust/)() override | アプリケーション内でビューを含むウィンドウのサイズを変更した際に、サイドコンテンツ領域のサイズが新しいサイズを補償すべきかどうかを指定します。Read **bool**。 |
| **float** [get_DimensionSize](./get_dimensionsize/)() override | スライド領域のサイズを指定します（RestoredTop の子の場合は幅、RestoredLeft の子の場合は高さ）。Read **float**。 |
...
| void [set_AutoAdjust](./set_autoadjust/)(**bool**) override | アプリケーション内でビューを含むウィンドウのサイズを変更した際に、サイドコンテンツ領域のサイズが新しいサイズを補償すべきかどうかを指定します。Write **bool**。 |
| void [set_DimensionSize](./set_dimensionsize/)(**float**) override | スライド領域のサイズを指定します（RestoredTop の子の場合は幅、RestoredLeft の子の場合は高さ）。Write **float**。 |
...
* クラス [INormalViewRestoredProperties](../inormalviewrestoredproperties/)
* 名前空間 [Aspose::Slides](../)
* ライブラリ [Aspose.Slides](../../)