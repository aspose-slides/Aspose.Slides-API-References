...
| [LightingDirection](../lightingdirection/) [get_Direction](./get_direction/)() override | 光の方向。[LightingDirection](../lightingdirection/) を読み取ります。 |
| [LightRigPresetType](../lightrigpresettype/) [get_LightType](./get_lighttype/)() override | シェイプに適用できるプリセット光右を表します。ライトリグは 3D シーンに対して特定の方向に向けられた光のグループを表します。[LightRigPresetType](../lightrigpresettype/) を読み取ります。 |
...
| virtual ASPOSE_SLIDES_LOCAL_API [System::SharedPtr](../../system/sharedptr/)\<[IPresentationComponent](../ipresentationcomponent/)\> [get_Parent_IPresentationComponent](../ipviobject/get_parent_ipresentationcomponent/)() | [IPresentationComponent](../ipresentationcomponent/) の親を返します。[IPresentationComponent](../ipresentationcomponent/) は読み取り専用です。 |
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | オブジェクトに関連付けられた参照カウンタデータ構造を取得します。 |
| **int32_t** [GetHashCode](../pviobject/gethashcode/)() const override | ハッシュコードを返します。 |
| [System::ArrayPtr](../../system/arrayptr/)\<**float**\> [GetRotation](./getrotation/)() override | 回転は、緯度座標、経度座標、そしてそれらの座標に対する軸周りの回転で定義されます。返却配列の最初の要素は緯度、2 番目は経度、3 番目は回転です。回転が定義されていない場合は null を返します。 |
...
| void [set_Direction](./set_direction/)([LightingDirection](../lightingdirection/)) override | 光の方向。[LightingDirection](../lightingdirection/) に書き込みます。 |
| void [set_LightType](./set_lighttype/)([LightRigPresetType](../lightrigpresettype/)) override | シェイプに適用できるプリセット光右を表します。ライトリグは 3D シーンに対して特定の方向に向けられた光のグループを表します。[LightRigPresetType](../lightrigpresettype/) に書き込みます。 |
| void [SetRotation](./setrotation/)(**float**, **float**, **float**) override | 回転は、緯度座標、経度座標、そしてそれらの座標に対する軸周りの回転で定義されます。いずれかの座標値が std::numeric_limits<float>::quiet_NaN() の場合、すべての回転は未定義となります。 |
