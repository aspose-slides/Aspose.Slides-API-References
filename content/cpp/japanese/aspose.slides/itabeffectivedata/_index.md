...
description: 有効テキストのタブストッププロパティを保持する不変オブジェクトです。
type: docs
weight: 3992
url: /ja/aspose.slides/itabeffectivedata/
---
## ITabEffectiveData クラス


有効テキストのタブストッププロパティを保持する不変オブジェクトです。
...
| メソッド | 説明 |
| --- | --- |
| virtual int [CompareTo](../../system/icomparable/compareto/)(T) | 現在のオブジェクトと指定されたオブジェクトを比較します。 |
...
| virtual [TabAlignment](../tabalignment/) [get_Alignment](./get_alignment/)() | タブの配置スタイルを返します。読み取り専用 [TabAlignment](../tabalignment/)。 |
| virtual **double** [get_Position](./get_position/)() | タブの位置を返します。このプロパティに代入するとコレクション内のタブのインデックスが変更され、Enumerator が無効になる可能性があります。読み取り専用 **double**。 |
