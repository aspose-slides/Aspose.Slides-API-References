...
description: エフェクト（グローや影など）が適用される形状に対して、平面を定義します。
type: docs
weight: 92
...
| [System::ArrayPtr](../../system/arrayptr/)\<**float**\> [get_AnchorPoint](./get_anchorpoint/)() override | 3D 空間内の点を返します。この点はバックドロップ平面をアンカーする空間上の点です。3 つの float 値の配列で X、Y、Z 座標を定義する 3D 点として表現されます。読み取りは **float**[] 。 |
| [System::ArrayPtr](../../system/arrayptr/)\<**float**\> [get_NormalVector](./get_normalvector/)() override | 法線ベクトルを返します。より正確には、この属性はバックドロップ平面の面に対して法線となるベクトルを定義します。ベクトルは X、Y、Z 座標を定義する 3 つの float 値の配列で表されます。読み取りは **float**[] 。 |
...
| [System::ArrayPtr](../../system/arrayptr/)\<**float**\> [get_UpVector](./get_upvector/)() override | 上方向を表すベクトルを返します。より正確には、この属性はバックドロップ平面の面に対して上方向を表すベクトルを定義します。ベクトルは X、Y、Z 座標を定義する 3 つの float 値の配列で表されます。読み取りは **float**[] 。 |
...
| void [set_AnchorPoint](./set_anchorpoint/)([System::ArrayPtr](../../system/arrayptr/)\<**float**\>) override | 3D 空間内の点を設定します。この点はバックドロップ平面をアンカーする空間上の点です。3 つの float 値の配列で X、Y、Z 座標を定義する 3D 点として表現されます。書き込みは **float**[] 。 |
| void [set_NormalVector](./set_normalvector/)([System::ArrayPtr](../../system/arrayptr/)\<**float**\>) override | 法線ベクトルを設定します。より正確には、この属性はバックドロップ平面の面に対して法線となるベクトルを定義します。ベクトルは X、Y、Z 座標を定義する 3 つの float 値の配列で表されます。書き込みは **float**[] 。 |
| void [set_UpVector](./set_upvector/)([System::ArrayPtr](../../system/arrayptr/)\<**float**\>) override | 上方向ベクトルを設定します。より正確には、この属性はバックドロップ平面の面に対して上方向を表すベクトルを定義します。ベクトルは X、Y、Z 座標を定義する 3 つの float 値の配列で表されます。書き込みは **float**[] 。 |
...
* クラス [IBackdrop3DScene](../ibackdrop3dscene/)
* 名前空間 [Aspose::Slides](../)
* ライブラリ [Aspose.Slides](../../)