...
description: Presentation、Slide、または TextFrame 内のテキスト検索に使用できるオプションを表します。
type: docs
weight: 4122
url: /ja/aspose.slides/itextsearchoptions/
---
## ITextSearchOptions クラス

テキスト検索に使用できるオプションを表します。[Presentation](../presentation/)、[Slide](../slide/)、[TextFrame](../textframe/)内のテキストを検索するために使用できます。
...
| virtual **bool** [get_CaseSensitive](./get_casesensitive/)() | 大文字と小文字を区別した検索を使用する場合は true に設定し、そうでない場合は false に設定します。**bool** を読み取ります。 |
| virtual **bool** [get_IncludeNotes](./get_includenotes/)() | テキスト検索、置換、またはハイライト操作を実行する際にスライドノートに含まれるテキストを含める場合は **true** に設定します。デフォルト値は **false** です。 |
| virtual **bool** [get_WholeWordsOnly](./get_wholewordsonly/)() | 完全一致（単語全体）だけをマッチさせる場合は true に設定し、そうでない場合は false に設定します。**bool** を読み取ります。 |
...
| virtual void [set_CaseSensitive](./set_casesensitive/)(**bool**) | 大文字と小文字を区別した検索を使用する場合は true に設定し、そうでない場合は false に設定します。**bool** を書き込みます。 |
| virtual void [set_IncludeNotes](./set_includenotes/)(**bool**) | テキスト検索、置換、またはハイライト操作を実行する際にスライドノートに含まれるテキストを含める場合は **true** に設定します。デフォルト値は **false** です。 |
| virtual void [set_WholeWordsOnly](./set_wholewordsonly/)(**bool**) | 完全一致（単語全体）だけをマッチさせる場合は true に設定し、そうでない場合は false に設定します。**bool** を書き込みます。 |
