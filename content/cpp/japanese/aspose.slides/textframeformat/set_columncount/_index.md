...
description: テキスト領域の列数を設定します。この値は正の数でなければなりません。そうでない場合、値は 0 に設定されます。0 は未定義の値を意味します。int32_tで書き込みます。
type: docs
weight: 261
url: /ja/aspose.slides/textframeformat/set_columncount/
---
## TextFrameFormat::set_ColumnCount(int32_t) メソッド

テキスト領域の列数を設定します。この値は正の数でなければなりません。そうでない場合、値は0に設定されます。値0は未定義の値を意味します。書き込みは **int32_t** です。
