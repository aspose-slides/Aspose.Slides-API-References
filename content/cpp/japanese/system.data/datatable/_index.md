...
description: "行と列で構成されたデータです。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new でこの型のインスタンスを作成しないでください。そうしないとランタイムエラーやアサーション失敗が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数への引数として使用してください。"
type: docs
weight: 105
url: /ja/system.data/datatable/
---
## DataTable クラス


[Data](../) は行と列で構成されています。 このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。 この型のインスタンスをスタック上または operator new で作成しないでください。そうしないとランタイムエラーやアサーション失敗が発生します。 このクラスは必ず [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として渡してください。
...
| [System::SharedPtr](../../system/sharedptr/)\<[DataColumnCollection](../datacolumncollection/)\> [get_Columns](./get_columns/)() | テーブルに存在する列を取得します。 |
| [System::SharedPtr](../../system/sharedptr/)\<[DataSet](../dataset/)\> [get_DataSet](./get_dataset/)() | テーブルが属するデータセットを取得します。 |
| [System::SharedPtr](../../system/sharedptr/)\<[DataRowCollection](../datarowcollection/)\> [get_Rows](./get_rows/)() | RTTI 情報です。 |
| [System::String](../../system/string/) [get_TableName](./get_tablename/)() | テーブル名を取得します。 |
