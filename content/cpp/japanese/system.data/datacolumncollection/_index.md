...
description: "データ列のコレクションをカプセル化します。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使ってこの型のインスタンスを作成しないでください。実行時エラーやアサーション違反が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数へ引数として渡してください。"
type: docs
weight: 14
url: /ja/system.data/datacolumncollection/
---
## DataColumnCollection クラス

データ列のコレクションをカプセル化します。  
このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。  
スタック上や operator new を使用してこの型のインスタンスを作成しないでください。実行時エラーやアサーション違反が発生します。  
...
| [System::SharedPtr](../../system/sharedptr/)\<[DataColumn](../datacolumn/)\> [idx_get](./idx_get/)(const **int32_t**) | インデックスで列を取得します。 |
| **int32_t** [IndexOf](./indexof/)(const [System::String](../../system/string/)\&) | コレクション内の列を検索します。 |
