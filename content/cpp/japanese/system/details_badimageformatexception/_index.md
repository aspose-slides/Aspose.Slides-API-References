...
description: "動的リンクライブラリ（DLL）または実行可能プログラムのファイルイメージが無効な場合にスローされる例外です。このクラスのインスタンスを手動で作成しないでください。代わりに BadImageFormatException クラスを使用してください。BadImageFormatException クラスのインスタンスを System::SmartPtr にラップしないでください。"
type: docs
weight: 378
url: /ja/system/details_badimageformatexception/
---
## Details_BadImageFormatException クラス


動的リンクライブラリ（DLL）または実行可能プログラムのファイルイメージが無効な場合にスローされる例外です。このクラスのインスタンスを手動で作成しないでください。代わりに BadImageFormatException クラスを使用してください。BadImageFormatException クラスのインスタンスを [System::SmartPtr](../smartptr/) にラップしないでください。
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | C# スタイルの浮動小数点比較をエミュレートし、IEC 60559:1989 によれば NaN は任意の値（NaN を含む）と等しくないにもかかわらず、2 つの NaN を等しいとみなします。 |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | C# スタイルの浮動小数点比較をエミュレートし、IEC 60559:1989 によれば NaN は任意の値（NaN を含む）と等しくないにもかかわらず、2 つの NaN を等しいとみなします。 |
...
| virtual [String](../string/) [get_FileName](../details_exceptionwithfilename/get_filename/)() const | この例外を引き起こすファイルの名前を取得します。 |
...
| const [System::TypeInfo](../typeinfo/)\& [GetType](../details_systemexception/gettype/)() const override | オブジェクトの実際の型を取得します。C# の [System.Object.GetType()](../object/gettype/) 呼び出しに相当します。 |
| **bool** [Is](../details_systemexception/is/)(const [System::TypeInfo](../typeinfo/)\&) const override |  |
| void [Lock](../object/lock/)() | C# の lock() 文のロック機構を実装します。直接呼び出すか、[LockContext](../lockcontext/) セントリーオブジェクトを使用してください。 |
...
| **bool** [ReferenceEquals](../object/referenceequals/)([String](../string/) const\&, [String](../string/) const\&) | [Object::ReferenceEquals](../object/referenceequals/) の、文字列の場合に対する特殊化です。 |
...
| int [SharedRefRemovedSafe](../object/sharedrefremovedsafe/)() | 共有参照カウントをデクリメントして返します。直接呼び出すべきではなく、スマートポインタまたは ThisProtector を使用してください。 |
...
| void [Unlock](../object/unlock/)() | C# の lock() 文のロック解除機構を実装します。直接呼び出すか、[LockContext](../lockcontext/) セントリーオブジェクトを使用してください。 |
...
| virtual const char * [what](../details_exception/what/)() const | [what()](../details_exception/what/) メソッドを実装し、[ExceptionWrapper](../exceptionwrapper/) クラスから呼び出されます。このクラスは std::exception から継承されていないにもかかわらず、派生クラスは protected / private メンバーを使用してロジックを実装できます。このメソッドの実装を [ExceptionWrapper](../exceptionwrapper/) に移すと、そのロジックが壊れる可能性があります。 |
| virtual  [~Object](../object/~object/)() | オブジェクトを破棄し、すべての内部データ構造を解放します。 |

## 参照

* クラス [Details_ExceptionWithFilename](../details_exceptionwithfilename/)
* 名前空間 [System](../)
* ライブラリ [Aspose.Slides](../../)