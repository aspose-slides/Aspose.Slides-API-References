...
description: "TimeoutException は、プロセスまたは操作に割り当てられた時間が期限切れかどうかを示します。このクラスのインスタンスを手動で作成しないでください。代わりに TimeoutException クラスを使用してください。TimeoutException クラスのインスタンスを System::SmartPtr にラップしないでください。"
type: docs
weight: 716
url: /ja/system/details_timeoutexception/
---
## Details_TimeoutException クラス

TimeoutException は、プロセスまたは操作に割り当てられた時間が期限切れになったかどうかを示します。 このクラスのインスタンスを手動で作成しないでください。代わりに TimeoutException クラスを使用してください。[System::SmartPtr](../smartptr/) に TimeoutException クラスのインスタンスをラップしないでください。
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | C# スタイルの浮動小数点比較をエミュレートします。IEC 60559:1989 によれば NaN は任意の値（NaN を含む）と等しくないにもかかわらず、2 つの NaN を等しいとみなします。 |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | C# スタイルの浮動小数点比較をエミュレートします。IEC 60559:1989 によれば NaN は任意の値（NaN を含む）と等しくないにもかかわらず、2 つの NaN を等しいとみなします。 |
...
| virtual [String](../string/) [get_Message](../details_exception/get_message/)() const | エラー記述を含む文字列を返します。 |
...
| Detail::SmartPtrCounter * [GetCounter](../object/getcounter/)() | オブジェクトに関連付けられた参照カウンターデータ構造体を取得します。 |
...
| void [Lock](../object/lock/)() | C# の lock() 文のロック機構を実装します。直接呼び出すか、[LockContext](../lockcontext/) センチリーオブジェクトを使用してください。 |
...
| virtual const char * [what](../details_exception/what/)() const | [what()](../details_exception/what/) メソッドを実装します。このメソッドは [ExceptionWrapper](../exceptionwrapper/) クラスから呼び出されます。このクラスは std::exception から継承していないにもかかわらず、派生クラスは protected / private メンバーを使用してロジックを実装できます。このメソッドの実装を [ExceptionWrapper](../exceptionwrapper/) に移動するとそのロジックが壊れる可能性があります。 |
