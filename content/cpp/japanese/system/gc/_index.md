...
description: 実際には何も行わないスタブのように動作するエミュレートされたガーベジコレクションを表します。インスタンスサービスを持たない静的型です。いかなる手段でもインスタンスを作成してはいけません。
type: docs
weight: 872
url: /ja/system/gc/
---
...
Emulated Garbage Collection を表します。実際には何もしないスタブのように動作します。インスタンス サービスを持たない静的型です。いかなる手段でもインスタンスを作成してはいけません。
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | C# スタイルの浮動小数点比較をエミュレートします。IEC 60559:1989 によれば NaN は任意の値（NaN を含む）と等しくないにもかかわらず、2 つの NaN が等しいとみなされます。 |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | C# スタイルの浮動小数点比較をエミュレートします。IEC 60559:1989 によれば NaN は任意の値（NaN を含む）と等しくないにもかかわらず、2 つの NaN が等しいとみなされます。 |
...
| static **int64_t** [GetTotalAllocatedBytes](./gettotalallocatedbytes/)(**bool**) | 合計割り当てバイト数を返します。 |
| static **int64_t** [GetTotalMemory](./gettotalmemory/)(**bool**) | 現在のプロセスが現在割り当てているプライベートメモリのバイト数を返します。 |
...
| void [Lock](../object/lock/)() | C# の lock() 文のロック機能を実装します。直接呼び出すか、[LockContext](../lockcontext/) 見張りオブジェクトを使用してください。 |
...
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | n 番目のテンプレート引数を弱いポインタ（共有ではなく）に設定します。コンテナ内のポインタを弱参照モードに切り替えることができます。 |
...
| virtual [String](../string/) [ToString](../object/tostring/)() const | C# [Object.ToString()](../object/tostring/) メソッドの類似です。カスタムオブジェクトを文字列に変換することが可能です。 |
| static const [TypeInfo](../typeinfo/)\& [Type](../object/type/)() | C# typeof([System.Object](../object/)) 構文を実装します。 |
| void [Unlock](../object/unlock/)() | C# の lock() 文のロック解除を実装します。直接呼び出すか、[LockContext](../lockcontext/) 見張りオブジェクトを使用してください。 |
