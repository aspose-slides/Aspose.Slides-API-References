...
description: 現在のオブジェクトが、指定された DateTimeOffset オブジェクトが表す値と同じか、それより後の日時値を表しているかどうかを判断します。
type: docs
weight: 599
url: /ja/system/datetimeoffset/operator_greater_equal/
---
## DateTimeOffset::operator>=(const DateTimeOffset\&) const メソッド


現在のオブジェクトが、指定された[DateTimeOffset](../)オブジェクトが表す値と同じか、またはそれより後の日時値を表しているかどうかを判断します。
...
現在のオブジェクトが表す日時値が **other** が表す値と同じかそれより後である場合は true、そうでない場合は false
