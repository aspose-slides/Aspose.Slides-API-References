...
description: "DivideByZeroException は、算術演算で 0 除算が試みられたときにスローされます。このクラスのインスタンスを手動で作成しないでください。代わりに DivideByZeroException クラスを使用してください。DivideByZeroException クラスのインスタンスを System::SmartPtr にラップしないでください。"
type: docs
weight: 404
url: /ja/system/details_dividebyzeroexception/
---
## Details_DivideByZeroException クラス


DivideByZeroException は、算術演算で 0 除算が試みられたときにスローされます。 このクラスのインスタンスを手動で作成しないでください。 代わりに DivideByZeroException クラスを使用してください。 DivideByZeroException クラスのインスタンスを [System::SmartPtr](../smartptr/) にラップしないでください。
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | C# スタイルの浮動小数点比較をエミュレートします。この比較では、IEC 60559:1989 によると NaN は任意の値（NaN を含む）と等しくないにもかかわらず、2 つの NaN を等しいとみなします。 |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | C# スタイルの浮動小数点比較をエミュレートします。この比較では、IEC 60559:1989 によると NaN は任意の値（NaN を含む）と等しくないにもかかわらず、2 つの NaN を等しいとみなします。 |
...
| **bool** [ReferenceEquals](../object/referenceequals/)([String](../string/) const\&, [String](../string/) const\&) | [Object::ReferenceEquals](../object/referenceequals/) の文字列のケースに対する特殊化です。 |
| int [RemovedSharedRefs](../object/removedsharedrefs/)(int) | 指定された値だけ共有参照カウントを減少させます。 |
| void [set_HResult](../details_exception/set_hresult/)(**int32_t**) | HRESULT を設定します。これは特定の例外に割り当てられる数値コードです。 |
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | n 番目のテンプレート引数を共有ではなく弱参照ポインタに設定します。コンテナ内のポインタを弱参照モードに切り替えることができます。 |
...
OutOfMemoryException は、アプリケーションのメモリが不足したときにスローされます。 このクラスのインスタンスを手動で作成しないでください。 代わりに OutOfMemoryException クラスを使用してください。 OutOfMemoryException クラスのインスタンスを [System::SmartPtr](../smartptr/) にラップしないでください。 

## 参照

* クラス [Details_ArithmeticException](../details_arithmeticexception/)
* 名前空間 [System](../)
* ライブラリ [Aspose.Slides](../../)