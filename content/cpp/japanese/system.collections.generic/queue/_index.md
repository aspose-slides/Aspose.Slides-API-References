...
description: Queue クラスの前方宣言。
type: docs
weight: 469
url: /ja/system.collections.generic/queue/
---
## Queue クラス
...
| virtual void [Clear](./clear/)() | キュー内のすべての要素を削除します。 |
| virtual **bool** [Contains](./contains/)(const T\&) const | operator == を使用して要素を比較し、キューが特定の要素を含むかどうかをチェックします。 |
| [queue_t](./queue_t/)\& [data](./data/)() | 基礎となるデータ構造へのアクセサです。 |
| const [queue_t](./queue_t/)\& [data](./data/)() const | 基礎となるデータ構造へのアクセサです。 |
| T [Dequeue](./dequeue/)() | キューの先頭からアイテムを取得します。 |
...
| [const_iterator](../ienumerable/const_iterator/) [end](../ienumerable/end/)() const | const 修飾インスタンスのコレクションの最後の要素（存在する場合）の直後を指すイテレータを取得します。 |
| void [Enqueue](./enqueue/)(const T\&) | アイテムをキューの末尾に追加します。 |
...
| virtual int [get_Count](./get_count/)() const | キュー内の要素数を取得します。 |
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | オブジェクトに関連付けられた参照カウンタデータ構造を取得します。 |
| [IEnumeratorPtr](./ienumeratorptr/) [GetEnumerator](./getenumerator/)() override | キューを反復する列挙子を取得します。 |
...
| int [LINQ_Count](../ienumerable/linq_count/)() | シーケンス内の要素数を返します（直接カウントによる計算）。 |
...
| [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<ResultType\>\> [LINQ_SelectMany](../ienumerable/linq_selectmany/)(const [Func](../../system/func/)\<T, [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<ResultType\>\>\>\&) | シーケンスの各要素を投影し、得られたシーケンスを 1 つのシーケンスに結合します。 |
...
| void [Lock](../../system/object/lock/)() | C# の lock() 文のロック機能を実装します。直接呼び出すか、[LockContext](../../system/lockcontext/) センティネリーオブジェクトを使用します。 |
...
| T [Peek](./peek/)() | キューの先頭からアイテムを取得しますが、キューからは削除しません。 |
|  [Queue](./queue/)() | 空のキューを構築します。 |
|  [Queue](./queue/)(int) | 空のキューを構築します。 |
|  [Queue](./queue/)(const [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<T\>\>\&) | コピー コンストラクタ。 |
...
| void [Unlock](../../system/object/unlock/)() | C# の lock() 文のロック解除を実装します。直接呼び出すか、[LockContext](../../system/lockcontext/) センティネリーオブジェクトを使用します。 |
...
| [queue_t](./queue_t/) | 基礎となるデータ型です。 |
| [IEnumerablePtr](./ienumerableptr/) | 同じ型要素のコンテナです。 |
| [IEnumeratorPtr](./ienumeratorptr/) | **Enumerator** 型です。 |

## 備考

[Queue](./) コンテナは STL list をラップしています。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないとランタイムエラーやアサーション失敗が発生します。常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数の引数として渡してください。
...
int main()
{
  ...
  // 最初のキュー項目を出力します。Peek メソッドはキューから項目を削除しません。
  std::cout << queue->Peek() << std::endl;
  // キューの項目を出力します。
  PrintItems(queue);

  // 最初のキュー項目を出力します。Dequeue メソッドはキューから項目を削除します。
