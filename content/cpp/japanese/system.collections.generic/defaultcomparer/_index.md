...
description: "デフォルト比較クラスです。operator < と operator == を使用して値を比較します。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使ってインスタンスを作成しないでください。これにより実行時エラーやアサーション障害が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡してください。"
type: docs
weight: 118
url: /ja/system.collections.generic/defaultcomparer/
---
## DefaultComparer クラス
...
| メソッド | 説明 |
| --- | --- |
| int [Compare](./compare/)(typename [ThisType::args_type](../icomparer/args_type/), typename [ThisType::args_type](../icomparer/args_type/)) const override | 実際のデータ比較。 |
