...
description: "2 つのオブジェクトを大小比較で比較するインターフェースです。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないとランタイムエラーやアサーション違反が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡すようにしてください。"
type: docs
weight: 261
url: /ja/system.collections.generic/icomparer/
---
## IComparer クラス

2 つのオブジェクトを大小比較で比較するインターフェースです。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないとランタイムエラーやアサーション違反が発生します。このクラスは常に [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数の引数として渡すようにしてください。
...
| メソッド | 説明 |
| --- | --- |
| virtual int [Compare](./compare/)([args_type](./args_type/), [args_type](./args_type/)) const | [Comparison](../../system/comparison/) 関数。 |
...
| 型エイリアス | 説明 |
| --- | --- |
| [args_type](./args_type/) | [Comparison](../../system/comparison/) 引数タイプ。 |
