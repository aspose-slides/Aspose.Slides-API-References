...
description: "辞書型コンテナのキーまたは値を保持するコンテナのインターフェイスです。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうすると実行時エラーやアサーション障害が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡して使用してください。"
type: docs
weight: 326
url: /ja/system.collections.generic/ikvcollection/
---
## IKVCollection クラス

辞書型コンテナのキーまたは値を保持するコンテナのインターフェイスです。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てる必要があります。スタック上や operator new を使用してインスタンスを作成しないでください。そうすると実行時エラーやアサーション障害が発生します。このクラスは常に [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数の引数として渡して使用してください。
...
| メソッド | 説明 |
| --- | --- |
| void [Add](./add/)(const T\&) override | コンテナにアイテムを追加します。 |
| [iterator](../ienumerable/iterator/) [begin](../ienumerable/begin/)() | コレクションの最初の要素（存在すれば）を指すイテレータを取得します。このイテレータは、[GetEnumerator()](../ienumerable/getenumerator/) が T のコピーオブジェクトを返すため、参照されたオブジェクトを変更するために使用できません。 |
| [const_iterator](../ienumerable/const_iterator/) [begin](../ienumerable/begin/)() const | const 修飾されたコレクションインスタンスの最初の要素（存在すれば）を指すイテレータを取得します。 |
| [const_iterator](../ienumerable/const_iterator/) [cbegin](../ienumerable/cbegin/)() const | コレクションの最初の const 修飾要素（存在すれば）を指すイテレータを取得します。 |
| [const_iterator](../ienumerable/const_iterator/) [cend](../ienumerable/cend/)() const | コレクションの最後の const 修飾要素（存在すれば）の直後を指すイテレータを取得します。 |
| void [Clear](./clear/)() override | コンテナ内のすべての要素を削除します。 |
| **bool** [Contains](./contains/)(const T\&) const override | アイテムがコンテナに存在するか確認します。 |
| virtual void [CopyTo](../icollection/copyto/)([System::ArrayPtr](../../system/arrayptr/)\<T\>, int) | すべてのコレクション要素を既存の配列要素へコピーします。 |
| [iterator](../ienumerable/iterator/) [end](../ienumerable/end/)() | コレクションの最後の要素（存在すれば）の直後を指すイテレータを取得します。このイテレータは、[GetEnumerator()](../ienumerable/getenumerator/) が T のコピーオブジェクトを返すため、参照されたオブジェクトを変更するために使用できません。 |
| [const_iterator](../ienumerable/const_iterator/) [end](../ienumerable/end/)() const | const 修飾されたコレクションインスタンスの最後の要素（存在すれば）の直後を指すイテレータを取得します。 |
...
| virtual int [get_Count](./get_count/)() const | コンテナの要素数を取得します。 |
| **bool** [get_IsFixedSize](../ilist/get_isfixedsize/)() | コレクションが固定サイズかどうかを確認します。 |
| **bool** [get_IsReadOnly](./get_isreadonly/)() const override | コンテナが読み取り専用かどうかを確認します。 |
...
| virtual [SharedPtr](../../system/sharedptr/)\<[IEnumerator](../ienumerator/)\<T\>\> [GetEnumerator](./getenumerator/)() | コンテナ用の列挙子を取得します。 |
...
| virtual T [idx_get](./idx_get/)(int) const | 取得関数。 |
| void [idx_set](./idx_set/)(int, T) override | 設定関数。 |
| int [IndexOf](./indexof/)(const T\&) const override | コンテナ内のアイテムのインデックスを取得します。 |
| void [Insert](./insert/)(int, const T\&) override | 指定した位置にアイテムを挿入します。 |
...
| **bool** [LINQ_Any](../ienumerable/linq_any/)(std::function\<**bool**(T)>) | シーケンスのいずれかの要素が存在するか、条件を満たすかどうかを判定します。 |
...
| T [LINQ_FirstOrDefault](../ienumerable/linq_firstordefault/)(std::function\<**bool**(T)>) | 条件を満たすシーケンスの最初の要素を返します。該当要素がなければデフォルト値を返します。 |
...
| [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<T\>\> [LINQ_Reverse](../ienumerable/linq_reverse/)() | シーケンスの要素順序を逆にします。 |
...
| [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<ResultType\>\> [LINQ_SelectMany](../ienumerable/linq_selectmany/)(const [Func](../../system/func/)\<T, [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<ResultType\>\>\>\&) | シーケンスの各要素を投影し、結果のシーケンスを 1 つに結合します。 |
...
| **bool** [Remove](./remove/)(const T\&) override | コンテナからアイテムを削除します。 |
| void [RemoveAt](./removeat/)(int) override | 指定した位置のアイテムを削除します。 |
