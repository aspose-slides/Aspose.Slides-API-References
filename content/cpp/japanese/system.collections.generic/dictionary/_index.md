...
description: Dictionary クラスの前方宣言です。
type: docs
weight: 144
url: /ja/system.collections.generic/dictionary/
---
## 辞書クラス
...
| [iterator](../ienumerable/iterator/) [begin](../ienumerable/begin/)() | コレクションの最初の要素（存在する場合）を指すイテレータを取得します。このイテレータは、[GetEnumerator()](../ienumerable/getenumerator/) が T のコピーオブジェクトを返すため、参照対象オブジェクトを変更するために使用できません。 |
...
| virtual **bool** [ContainsKey](../idictionary/containskey/)(const TKey\&) const | コンテナがキーを含むか確認します。 |
...
| [Dictionary](./dictionary/)() | 空の辞書を作成します。 |
| [Dictionary](./dictionary/)(const [map_t](./map_t/)\&) | マップからデータをコピーします。 |
| [Dictionary](./dictionary/)(int) | 事前に割り当てられた辞書を作成するオーバーロードです。実際には割り当てを行いません。 |
| [Dictionary](./dictionary/)(const [SharedPtr](../../system/sharedptr/)\<[IDictionary](../idictionary/)\<TKey, TValue\>\>\&) | コピーコンストラクタ。 |
| [Dictionary](./dictionary/)(const [SharedPtr](../../system/sharedptr/)\<[IDictionary](../idictionary/)\<TKey, TValue\>\>\&, const [SharedPtr](../../system/sharedptr/)\<[IEqualityComparer](../iequalitycomparer/)\<TKey\>\>\&) | コピーコンストラクタ。 |
| [Dictionary](./dictionary/)(const [SharedPtr](../../system/sharedptr/)\<[IEqualityComparer](../iequalitycomparer/)\<TKey\>\>\&) | 空の辞書を作成します。 |
| [Dictionary](./dictionary/)(int, const [SharedPtr](../../system/sharedptr/)\<[IEqualityComparer](../iequalitycomparer/)\<TKey\>\>\&) | 空の辞書を作成します。 |
| [iterator](../ienumerable/iterator/) [end](../ienumerable/end/)() | コレクションの最後の要素（存在する場合）の直後を指すイテレータを取得します。このイテレータは、[GetEnumerator()](../ienumerable/getenumerator/) が T のコピーオブジェクトを返すため、参照対象オブジェクトを変更するために使用できません。 |
...
| [IEnumeratorPtr](./ienumeratorptr/) [GetEnumerator](./getenumerator/)() override | 列挙子オブジェクトを作成します。 |
...
| virtual TValue [GetValueOrDefault](../idictionary/getvalueordefault/)(const TKey\&) const | 見つかった場合は値を返します。見つからない場合は **Value()** を返します。 |
| virtual TValue [GetValueOrDefault](../idictionary/getvalueordefault/)(const TKey\&, const TValue\&) const | 見つかった場合は値を返します。見つからない場合は **defaultValue** を返します。 |
| virtual TValue [GetValueOrNull](../idictionary/getvalueornull/)(const TKey\&) const | 見つかった場合は値を返します。見つからない場合は **null** を返します。参照型にのみ意味があります。 |
...
| T [LINQ_Aggregate](../ienumerable/linq_aggregate/)(const [Func](../../system/func/)\<T, T, T\>\&) | シーケンスに対して accumulator 関数を適用します。 |
| **bool** [LINQ_All](../ienumerable/linq_all/)(std::function\<**bool**(T)>) | シーケンスのすべての要素が条件を満たすか判定します。 |
| **bool** [LINQ_Any](../ienumerable/linq_any/)() | シーケンスが要素を含むかどうかを判定します。 |
| **bool** [LINQ_Any](../ienumerable/linq_any/)(std::function\<**bool**(T)>) | シーケンスに要素が存在するか、条件を満たすかを判定します。 |
| T [LINQ_Average](../ienumerable/linq_average/)() | 数値シーケンスの平均値を計算します。 |
...
| T [LINQ_FirstOrDefault](../ienumerable/linq_firstordefault/)(std::function\<**bool**(T)>) | 条件を満たすシーケンスの最初の要素を返します。該当要素がない場合はデフォルト値を返します。 |
...
| [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<ResultType\>\> [LINQ_OfType](../ienumerable/linq_oftype/)() | シーケンスの要素を指定された型でフィルタリングします。 |
...
| [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<T\>\> [LINQ_Where](../ienumerable/linq_where/)(std::function\<**bool**(T)>) | 指定された述語に基づきシーケンスをフィルタリングします。 |
...
| virtual **bool** [Remove](../idictionary/remove/)(const TKey\&) | キーをコンテナから削除します。 |
| virtual **bool** [Remove](../icollection/remove/)(const T\&) | 要素をコレクションから削除します。 |
...
| 型エイリアス | 説明 |
| --- | --- |
| [KeyCollection](./keycollection/) | 抽出するキーのコレクション。 |
| [ValueCollection](./valuecollection/) | 抽出する値のコレクション。 |
| [map_t](./map_t/) | 基本データ型。 |
| [Ptr](./ptr/) | ポインタ型。 |
| [KVPair](./kvpair/) | キーと値のペア型。 |
| [IEnumerablePtr](./ienumerableptr/) | 列挙可能インターフェイスへのポインタ。 |
| [IEnumeratorPtr](./ienumeratorptr/) | 列挙子へのポインタ。 |

## 備考

[Dictionary](./) は値をキーにマッピングします。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や new 演算子でインスタンスを作成しないでください。実行時エラーやアサーション失敗の原因になります。このクラスは常に [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを引数として関数に渡してください。
