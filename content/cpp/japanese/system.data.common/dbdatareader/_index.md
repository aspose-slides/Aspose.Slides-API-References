...
description: "データベースからデータを受信するための APIです。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション失敗が発生します。このクラスは必ず System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡してください。"
type: docs
weight: 40
url: /ja/system.data.common/dbdatareader/
---
## DbDataReader クラス


データベースからデータを受信するための API です。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション失敗が発生します。このクラスは必ず [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数の引数として渡してください。
...
| Method | 説明 |
| --- | --- |
| virtual void [Close](./close/)() | データ取得チャネルを閉じます。 |
...
| virtual [SharedPtr](../../system/sharedptr/)\<[Object](../../system/object/)\> [idx_get](./idx_get/)([String](../../system/string/)) | 名前付きアイテムを取得します。 |
| virtual [SharedPtr](../../system/sharedptr/)\<[Object](../../system/object/)\> [idx_get](./idx_get/)(int) | インデックスでアイテムを取得します。 |
...
| virtual **bool** [Read](./read/)() | データベースから次のレコードを読み取ります。 |
