...
description: "Windows Forms アプリケーションから印刷する際に、出力をプリンターへ送ります。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使ってこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション障害が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数への引数として使用してください。"
type: docs
weight: 53
url: /ja/system.drawing.printing/printdocument/
---
## PrintDocument クラス


このクラスは [Windows](../../system.windows/) Forms アプリケーションから印刷する際に、出力をプリンターに送ります。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上で、または operator new を使用してこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション障害が発生します。常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として使用してください。
...
| [SharedPtr](../../system/sharedptr/)\<[PrinterSettings](../printersettings/)\> [get_PrinterSettings](./get_printersettings/)() | 未実装。 |
...
| void [set_DocumentName](./set_documentname/)(const [String](../../system/string/)\&) | 未実装。 |
| void [set_PrintController](./set_printcontroller/)(const [SharedPtr](../../system/sharedptr/)\<[PrintController](../printcontroller/)\>\&) | 未実装。 |
| void [set_PrinterSettings](./set_printersettings/)(const [SharedPtr](../../system/sharedptr/)\<[PrinterSettings](../printersettings/)\>\&) | 未実装。 |
