...
description: Xslt スタイルシートの実行中にサポートする XSLT 機能を指定します。
type: docs
weight: 118
url: /ja/system.xml.xsl/xsltsettings/
---
## XsltSettings クラス

Xslt スタイルシートの実行中にサポートする XSLT 機能を指定します。
...
| static [SharedPtr](../../system/sharedptr/)\<[XsltSettings](./)\> [get_Default](./get_default/)() | [XsltSettings](./) オブジェクトをデフォルト設定で返します。XSLT **document()** 関数および埋め込みスクリプトブロックのサポートは無効化されています。 |
| **bool** [get_EnableDocumentFunction](./get_enabledocumentfunction/)() | XSLT **document()** 関数のサポートを有効にするかどうかを示す値を返します。 |
| **bool** [get_EnableScript](./get_enablescript/)() | 埋め込みスクリプトブロックのサポートを有効にするかどうかを示す値を返します。 |
| static [SharedPtr](../../system/sharedptr/)\<[XsltSettings](./)\> [get_TrustedXslt](./get_trustedxslt/)() | [XsltSettings](./) オブジェクトを返し、XSLT **document()** 関数および埋め込みスクリプトブロックのサポートを有効にします。 |
...
| void [set_EnableDocumentFunction](./set_enabledocumentfunction/)(**bool**) | XSLT **document()** 関数のサポートを有効にするかどうかを示す値を設定します。 |
| void [set_EnableScript](./set_enablescript/)(**bool**) | 埋め込みスクリプトブロックのサポートを有効にするかどうかを示す値を設定します。 |
...
|  [XsltSettings](./xsltsettings/)() | [XsltSettings](./) クラスの新しいインスタンスをデフォルト設定で初期化します。 |
|  [XsltSettings](./xsltsettings/)(**bool**, **bool**) | [XsltSettings](./) クラスの新しいインスタンスを指定された設定で初期化します。 |
...
このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタックや operator new を使用してインスタンスを作成しないでください。そうすると実行時エラーやアサーション障害が発生します。常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として渡すようにしてください。
