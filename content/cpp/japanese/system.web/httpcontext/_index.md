...
description: "ダミークラスで、HttpContext 参照とのリンクは可能にしますが、実行はしません。適切に実装されたメンバーは含まれていません。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や new 演算子でこの型のインスタンスを作成しないでください。実行時エラーやアサーション違反が発生します。常にこのクラスを System::SmartPtr ポインタでラップし、そのポインタを関数への引数として渡してください。"
type: docs
weight: 14
url: /ja/system.web/httpcontext/
---
## HttpContext クラス


このダミークラスは、[HttpContext](./) 参照とのリンクを可能にしますが、実行はしません。適切に実装されたメンバーは含まれていません。このクラスのオブジェクトは、[System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や new 演算子でこの型のインスタンスを作成しないでください。実行時エラーやアサーション違反が発生します。常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として渡してください。
...
| static [System::SharedPtr](../../system/sharedptr/)\<[HttpContext](./)\> [get_Current](./get_current/)() | 未実装です。 |
| [System::SharedPtr](../../system/sharedptr/)\<HttpRequest\> [get_Request](./get_request/)() | 未実装です。 |
...
| static void [set_Current](./set_current/)(const [System::SharedPtr](../../system/sharedptr/)\<[HttpContext](./)\>\&) | 未実装です。 |
