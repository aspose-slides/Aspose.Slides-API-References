---
title: get_IsPasswordProtected()
second_title: Aspose.Slides for C++ API リファレンス
description: VBAProject がパスワードで保護され、プロジェクト プロパティを表示できるかどうかを示します。 読み取り専用 bool.
type: docs
weight: 40
url: /ja/aspose.slides.vba/ivbaproject/get_ispasswordprotected/
---
## IVbaProject::get_IsPasswordProtected() メソッド

VBAProject がパスワードで保護され、プロジェクト プロパティを表示できないかどうかを示します。 読み取り専用 **bool**.

```cpp
virtual bool Aspose::Slides::Vba::IVbaProject::get_IsPasswordProtected()=0
```

## 備考


```cpp
auto presentation = System::MakeObject<Presentation>(path + u"demo.pptm");

if (presentation->get_VbaProject()->get_IsPasswordProtected())
{
    System::Console::WriteLine(System::String(u"The VBAProject '") + presentation->get_VbaProject()->get_Name() + u"' is protected by password to view project properties.");
}
```

## 参照

* クラス [IVbaProject](../)
* 名前空間 [Aspose::Slides::Vba](../../)
* ライブラリ [Aspose.Slides](../../../)