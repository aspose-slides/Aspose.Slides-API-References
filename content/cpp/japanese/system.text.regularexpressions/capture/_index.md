...
description: "単一サブ式のマッチング結果です。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこのタイプのインスタンスを作成しないでください。そうすると実行時エラーやアサーション障害が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数への引数として使用してください。"
type: docs
weight: 1
url: /ja/system.text.regularexpressions/capture/
---
## Capture クラス

単一サブ式のマッチングの結果です。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうすると実行時エラーやアサーション障害が発生します。このクラスは常に [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として使用してください。
...
| メソッド | 説明 |
| --- | --- |
|  [Capture](./capture/)(const [UStringPtr](../ustringptr/)\&, int, int) | コンストラクタ。 |
...
| int [get_Index](./get_index/)() const | キャプチャされたサブストリングのインデックスを取得します。 |
| int [get_Length](./get_length/)() const | キャプチャされたサブストリングの長さを取得します。 |
| [String](../../system/string/) [get_Value](./get_value/)() const | キャプチャされたサブストリングを取得します。 |
...
| [String](../../system/string/) [ToString](./tostring/)() const override | キャプチャされたサブストリングを取得します。 |
