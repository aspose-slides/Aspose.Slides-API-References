...
description: "EncoderParameter オブジェクトの配列を表します。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション障害が発生します。常にこのクラスを System::SmartPtr ポインタでラップし、そのポインタを関数への引数として使用してください。"
type: docs
weight: 79
url: /ja/system.drawing.imaging/encoderparameters/
---
## EncoderParameters クラス

[EncoderParameter](../encoderparameter/) オブジェクトの配列を表します。  
このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。  
スタック上または operator new を使用してこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション障害が発生します。  
...
| Method | 説明 |
| --- | --- |
|  [EncoderParameters](./encoderparameters/)(int) | [EncoderParameters](./) クラスの新しいインスタンスを構築します。 |
...
| [System::ArrayPtr](../../system/arrayptr/)\<[EncoderParameterPtr](../encoderparameterptr/)\> [get_Param](./get_param/)() const | 現在の [EncoderParameters](./) オブジェクトが表す [EncoderParameter](../encoderparameter/) オブジェクトの配列を返します。 |
...
| void [set_Param](./set_param/)(const [System::ArrayPtr](../../system/arrayptr/)\<[EncoderParameterPtr](../encoderparameterptr/)\>\&) | 現在の [EncoderParameters](./) オブジェクトに [EncoderParameter](../encoderparameter/) オブジェクトの配列を割り当てます。 |
