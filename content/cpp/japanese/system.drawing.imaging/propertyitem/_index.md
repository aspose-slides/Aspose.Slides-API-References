...
description: "画像ファイルに含めるメタデータプロパティを表します。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てる必要があります。スタック上でインスタンスを作成したり operator new を使用したりしないでください。そうすると実行時エラーやアサーション違反が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数への引数として渡してください。"
type: docs
weight: 170
url: /ja/system.drawing.imaging/propertyitem/
---
## PropertyItem クラス

画像ファイルに含めるメタデータプロパティを表します。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。この型のインスタンスをスタック上で作成したり operator new を使用したりしないでください。そうすると実行時エラーやアサーション違反が発生します。常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数への引数として渡すようにしてください。
...
| **int32_t** [get_Id](./get_id/)() const | 現在のオブジェクトが表すプロパティの ID を返します。 |
| **int32_t** [get_Len](./get_len/)() const | 現在のオブジェクトが表すプロパティの長さ（バイト単位）を返します。 |
| **int16_t** [get_Type](./get_type/)() const | 現在のオブジェクトが表すプロパティの型（バイト単位）を返します。 |
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_Value](./get_value/)() const | 現在のオブジェクトが表すプロパティの値（バイト単位）を返します。 |
...
| void [set_Id](./set_id/)(**int32_t**) | 現在のオブジェクトが表すプロパティの ID を設定します。 |
| void [set_Len](./set_len/)(**int32_t**) | 現在のオブジェクトが表すプロパティの長さ（バイト単位）を設定します。 |
| void [set_Type](./set_type/)(**int16_t**) | 現在のオブジェクトが表すプロパティの型（バイト単位）を設定します。 |
| void [set_Value](./set_value/)(const [System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>\&) | 現在のオブジェクトが表すプロパティの型（バイト単位）を設定します。 |
