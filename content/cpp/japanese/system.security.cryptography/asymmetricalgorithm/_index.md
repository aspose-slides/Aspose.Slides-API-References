...
description: "非対称暗号化アルゴリズム用の抽象基底クラスです。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないとランタイムエラーやアサーション失敗が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡して使用してください。"
type: docs
weight: 14
url: /ja/system.security.cryptography/asymmetricalgorithm/
---
...
| static [SharedPtr](../../system/sharedptr/)\<[AsymmetricAlgorithm](./)\> [Create](./create/)() | デフォルトのアルゴリズムを作成します。未実装です。 |
| static [SharedPtr](../../system/sharedptr/)\<[AsymmetricAlgorithm](./)\> [Create](./create/)(const [String](../../system/string/)\&) | 名前でアルゴリズムを作成します。未実装です。 |
| void [Dispose](./dispose/)() override | 現在のオブジェクトが所有するリソースを解放します。 |
...
| virtual void [FromXmlString](./fromxmlstring/)([String](../../system/string/)) | XML 文字列からアルゴリズムパラメータを読み取ります。 |
| virtual [String](../../system/string/) [get_KeyExchangeAlgorithm](./get_keyexchangealgorithm/)() | 使用する鍵交換アルゴリズムを取得します。 |
| virtual **int32_t** [get_KeySize](./get_keysize/)() | 鍵サイズを取得します。 |
| virtual [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[KeySizes](../keysizes/)\>\> [get_LegalKeySizes](./get_legalkeysizes/)() | 許可された鍵サイズの配列を取得します。 |
| virtual [String](../../system/string/) [get_SignatureAlgorithm](./get_signaturealgorithm/)() | 使用する署名アルゴリズムを取得します。 |
...
| virtual void [set_KeySize](./set_keysize/)(**int32_t**) | 鍵サイズを設定します。 |
...
| virtual [String](../../system/string/) [ToXmlString](./toxmlstring/)(**bool**) | アルゴリズムパラメータを XML 文字列に書き込みます。 |
