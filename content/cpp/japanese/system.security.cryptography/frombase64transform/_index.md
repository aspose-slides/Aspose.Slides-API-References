...
description: "CryptoStream クラスのインスタンスを Base64 から変換します。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうするとランタイムエラーやアサーション違反が発生する可能性があります。常にこのクラスを System::SmartPtr ポインタでラップし、そのポインタを関数への引数として使用してください。"
type: docs
weight: 209
url: /ja/system.security.cryptography/frombase64transform/
---
...
|  [FromBase64Transform](./frombase64transform/)([FromBase64TransformMode](../frombase64transformmode/)) | コンストラクタ。 |
| virtual **bool** [get_CanReuseTransform](./get_canreusetransform/)() | 現在の変換が再利用可能かどうかを示す値を取得します。 |
| **bool** [get_CanTransformMultipleBlocks](./get_cantransformmultipleblocks/)() | 複数のブロックを変換できるかどうかを示す値を取得します。 |
...
| virtual int [TransformBlock](../icryptotransform/transformblock/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int) | データブロックを処理し、データを出力配列にコピーします。 |
