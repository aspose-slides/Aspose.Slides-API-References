...
description: "Rijndael アルゴリズムの基底クラスです。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上または operator new を使用してこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション障害が発生します。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡すようにしてください。"
type: docs
weight: 404
url: /ja/system.security.cryptography/rijndael/
---
## Rijndael クラス

[Rijndael](./) アルゴリズムの基底クラスです。このクラスのオブジェクトは [System::MakeObject()](../../system/makeobject/) 関数を使用してのみ割り当てるべきです。スタック上または operator new を使用してこの型のインスタンスを作成しないでください。そうしないと実行時エラーやアサーション障害が発生します。常にこのクラスを [System::SmartPtr](../../system/smartptr/) ポインタでラップし、そのポインタを関数の引数として渡すようにしてください。
...
| virtual void [GenerateIV](../symmetricalgorithm/generateiv/)() | アルゴリズムの初期値をランダムに生成します。既存のものがある場合は上書きします。 |
| virtual void [GenerateKey](../symmetricalgorithm/generatekey/)() | アルゴリズムの鍵をランダムに生成します。既存のものがある場合は上書きします。 |
...
| virtual [System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_IV](../symmetricalgorithm/get_iv/)() | 暗号操作の初期値を取得します。未作成の場合は新規作成します。 |
| virtual [System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_Key](../symmetricalgorithm/get_key/)() | 暗号操作の鍵を取得します。未作成の場合は新規作成します。 |
| virtual int [get_KeySize](../symmetricalgorithm/get_keysize/)() | 暗号操作の鍵サイズを取得します。 |
...
| virtual void [set_Key](../symmetricalgorithm/set_key/)([System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>) | 暗号操作の鍵を設定します。 |
| virtual void [set_KeySize](../symmetricalgorithm/set_keysize/)(int) | 暗号操作の鍵サイズを設定します。 |
...
| **bool** [ValidKeySize](../symmetricalgorithm/validkeysize/)(int) | 鍵サイズが有効かどうかを確認します。 |
