...
description: "対称暗号アルゴリズムで受け入れられるキーサイズの集合です。このクラスのオブジェクトは System::MakeObject() 関数を使用してのみ割り当てるべきです。スタック上や operator new を使用してこの型のインスタンスを作成しないでください。そうしないとランタイムエラーやアサーション失敗が発生する可能性があります。このクラスは常に System::SmartPtr ポインタでラップし、そのポインタを関数の引数として渡してください。"
type: docs
weight: 300
url: /ja/system.security.cryptography/keysizes/
---
## KeySizes クラス
...
| int [get_MaxSize](./get_maxsize/)() const | 有効な最大キーサイズを取得します。 |
| int [get_MinSize](./get_minsize/)() const | 有効な最小キーサイズを取得します。 |
| int [get_SkipSize](./get_skipsize/)() const | 有効なキーサイズのステップを取得します。 |
...
|  [KeySizes](./keysizes/)(int, int, int) | RTTI 情報です。 |
