...
description: Alpha Floor エフェクトを表します。Alpha（不透明度）値が 100% 未満の場合は 0 に変換されます。つまり、部分的に透明なものはすべて完全に透明になります。
type: docs
weight: 287
...
アルファ フロア効果を表します。アルファ（不透明度）値が 100% 未満の場合は 0 に変換されます。つまり、部分的に透明なものはすべて完全に透明になります。
...
| void [Unlock](../../system/object/unlock/)() | C# lock() ステートメントのアンロック機能を実装します。直接呼び出すか、[LockContext](../../system/lockcontext/) セントリーオブジェクトを使用してください。 |
