...
description: Alpha Ceiling 効果を表す不変オブジェクト。Alpha（不透明度）の値がゼロより大きい場合は 100% に変更されます。言い換えれば、部分的に不透明なものはすべて完全に不透明になります。
type: docs
weight: 274
url: /ja/aspose.slides.effects/ialphaceilingeffectivedata/
---
## IAlphaCeilingEffectiveData クラス

不変オブジェクトで、Alpha Ceiling 効果を表します。Alpha（不透明度）の値がゼロより大きい場合は 100% に変更されます。言い換えれば、部分的に不透明なものはすべて完全に不透明になります。
