...
description: Alpha Ceiling エフェクトを表します。Alpha（不透明度）の値が 0 より大きい場合は 100% に変更されます。つまり、部分的に不透明なものはすべて完全に不透明になります。
type: docs
weight: 261
...
Alpha Ceiling エフェクトを表します。Alpha（不透明度）の値が 0 より大きい場合は 100% に変更されます。つまり、部分的に不透明なものはすべて完全に不透明になります。
...
| void [Lock](../../system/object/lock/)() | C# の lock() 文のロックを実装します。直接呼び出すか、[LockContext](../../system/lockcontext/) センチリオブジェクトを使用します。 |
...
| void [Unlock](../../system/object/unlock/)() | C# の lock() 文のロック解除を実装します。直接呼び出すか、[LockContext](../../system/lockcontext/) センチリオブジェクトを使用します。 |
