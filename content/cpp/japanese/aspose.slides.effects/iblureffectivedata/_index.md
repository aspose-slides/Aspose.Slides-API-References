...
description: 塗りつぶしを含むシェイプ全体に適用されるぼかし効果を表す不変オブジェクトです。アルファを含むすべてのカラーチャネルが影響を受けます。
type: docs
weight: 469
url: /ja/aspose.slides.effects/iblureffectivedata/
---
## IBlurEffectiveData クラス
...
| virtual **bool** [get_Grow](./get_grow/)() | ブラー処理の結果としてオブジェクトの境界を拡張するかどうかを決定します。true は境界が拡張されることを示し、false は拡張されないことを示します。読み取り専用 **bool**。 |
| virtual **double** [get_Radius](./get_radius/)() | ブラー半径を返します。読み取り専用 **double**。 |
...
| void [Lock](../../system/object/lock/)() | C# の lock() ステートメントによるロックを実装します。直接呼び出すか、[LockContext](../../system/lockcontext/) センチネルオブジェクトを使用してください。 |
