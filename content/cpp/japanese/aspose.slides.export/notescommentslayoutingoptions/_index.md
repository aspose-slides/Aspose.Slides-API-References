...
エクスポートされたドキュメントでノートとコメントのレイアウトの外観を制御するオプションを提供します。
...
| [System::Drawing::Color](../../system.drawing/color/) [get_CommentsAreaColor](./get_commentsareacolor/)() | コメント領域の色を取得します（コメントが右側に表示される場合にのみ適用されます）。 |
| **int32_t** [get_CommentsAreaWidth](./get_commentsareawidth/)() | コメント出力領域の幅をピクセル単位で取得します（コメントが右側に表示される場合にのみ適用されます）。 |
| [CommentsPositions](../commentspositions/) [get_CommentsPosition](./get_commentsposition/)() | ページ上のコメントの位置を取得します。 |
| [NotesPositions](../notespositions/) [get_NotesPosition](./get_notesposition/)() | ページ上のノートの位置を取得します。 |
| **bool** [get_ShowCommentsByNoAuthor](./get_showcommentsbynoauthor/)() | 作成者がないコメントの表示状態を取得します。true の場合、コメントが表示されます。（コメントが表示されている場合にのみ適用されます） |
...
| void [set_CommentsAreaColor](./set_commentsareacolor/)([System::Drawing::Color](../../system.drawing/color/)) | コメント領域の色を設定します（コメントが右側に表示される場合にのみ適用されます）。 |
| void [set_CommentsAreaWidth](./set_commentsareawidth/)(**int32_t**) | コメント出力領域の幅をピクセル単位で設定します（コメントが右側に表示される場合にのみ適用されます）。 |
| void [set_CommentsPosition](./set_commentsposition/)([CommentsPositions](../commentspositions/)) | ページ上のコメントの位置を設定します。 |
| void [set_NotesPosition](./set_notesposition/)([NotesPositions](../notespositions/)) | ページ上のノートの位置を設定します。 |
| void [set_ShowCommentsByNoAuthor](./set_showcommentsbynoauthor/)(**bool**) | 作成者がないコメントの表示状態を設定します。true の場合、コメントが表示されます。（コメントが表示されている場合にのみ適用されます） |
