...
description: "モジュール内のクラスのいずれかの読み込みに失敗した場合、Module.GetTypes メソッドによって ReflectionTypeLoadException がスローされます。このクラスのインスタンスを手動で作成しないでください。代わりに ReflectionTypeLoadException クラスを使用してください。ReflectionTypeLoadException クラスのインスタンスを System::SmartPtr にラップしないでください。"
type: docs
weight: 66
url: /ja/system.reflection/details_reflectiontypeloadexception/
---
## Details_ReflectionTypeLoadException クラス

ReflectionTypeLoadException は、モジュールのクラスのいずれかが読み込みに失敗した場合に Module.GetTypes メソッドからスローされます。 このクラスのインスタンスを手動で作成しないでください。代わりに ReflectionTypeLoadException クラスを使用してください。 ReflectionTypeLoadException クラスのインスタンスを [System::SmartPtr](../../system/smartptr/) にラップしないでください。
...
| **int32_t** [get_HResult](../../system/details_exception/get_hresult/)() const | 現在のオブジェクトが表す例外に関連付けられた HRESULT コードの 32 ビット整数値を返します。 |
...
| virtual [System::ArrayPtr](../../system/arrayptr/)\<[System::Exception](../../system/exception/)\> [get_LoaderExceptions](./get_loaderexceptions/)() const | この例外を引き起こした例外を返します。 |
...
| virtual [System::ArrayPtr](../../system/arrayptr/)\<[System::TypeInfo](../../system/typeinfo/)\> [get_Types](./get_types/)() const | 例外の原因となった型情報を返します。 |
| virtual [ExceptionWrapper](../../system/exceptionwrapper/)\<[Details_Exception](../../system/details_exception/)\> [GetBaseException](../../system/details_exception/getbaseexception/)() const | 最内側の例外を表す Exception オブジェクトのコピーを返します。 |
...
| void [Lock](../../system/object/lock/)() | C# の lock() 文のロック機構を実装します。直接呼び出すか、[LockContext](../../system/lockcontext/) センティリティオブジェクトを使用してください。 |
...
| void [set_HResult](../../system/details_exception/set_hresult/)(**int32_t**) | 特定の例外に割り当てられたコード化された数値である HRESULT を設定します。 |
...
| void [Unlock](../../system/object/unlock/)() | C# lock() 文のロック解除を実装します。直接呼び出すか、[LockContext](../../system/lockcontext/) センティリティオブジェクトを使用してください。 |
...
| virtual const char * [what](../../system/details_exception/what/)() const | [what()](../../system/details_exception/what/) メソッドを実装します。このメソッドは [ExceptionWrapper](../../system/exceptionwrapper/) クラスによって呼び出されます。std::exception から継承されていないクラスでも、保護またはプライベートメンバーを使用してロジックを実装できます。このメソッド実装を [ExceptionWrapper](../../system/exceptionwrapper/) に移動すると、そのロジックが壊れる可能性があります。 |
