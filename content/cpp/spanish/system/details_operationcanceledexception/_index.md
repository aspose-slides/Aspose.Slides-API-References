...
description: "OperationCanceledException se lanza en un hilo cuando se cancela una operación que el hilo estaba ejecutando. Nunca cree instancias de esta clase manualmente. Use la clase OperationCanceledException en su lugar. Nunca envuelva las instancias de la clase OperationCanceledException en System::SmartPtr."
type: docs
weight: 625
url: /es/system/details_operationcanceledexception/
---
## Details_OperationCanceledException clase


OperationCanceledException is thrown in a thread upon cancellation of an operation that the thread was executing. Never create instances of this class manually. Use the OperationCanceledException class instead. Never wrap the OperationCanceledException class instances into [System::SmartPtr](../smartptr/).
...
| static std::enable_if\<[IsSmartPtr](../issmartptr/)\<T1\>::value\&&[IsSmartPtr](../issmartptr/)\<T2\>::value, **bool**\>::type [Equals](../object/equals/)(T1 const\&, T2 const\&) | Compara objetos de tipo referencia en estilo C#. |
| static std::enable_if<\![IsSmartPtr](../issmartptr/)\<T1\>::value\&&\![IsSmartPtr](../issmartptr/)\<T2\>::value, **bool**\>::type [Equals](../object/equals/)(T1 const\&, T2 const\&) | Compara objetos de tipo valor en estilo C#. |
...
| Detail::SmartPtrCounter * [GetCounter](../object/getcounter/)() | Obtiene la estructura de datos del contador de referencias asociado al objeto. |
...
| void [Lock](../object/lock/)() | Implementa el bloqueo de la declaración C# lock(). Llame directamente o use el objeto centinela [LockContext](../lockcontext/). |
...
| [Object](../object/object/)([Object](../object/) const\&) | Constructor de copia. Realmente no copia nada, solo inicializa un nuevo objeto y permite la construcción de copias en subclases. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | Operador de asignación. Realmente no copia nada, solo inicializa un nuevo objeto y permite la construcción de copias en subclases. |
...
| void [Unlock](../object/unlock/)() | Implementa el desbloqueo de la declaración C# lock(). Llame directamente o use el objeto centinela [LockContext](../lockcontext/). |
...
| virtual const char * [what](../details_exception/what/)() const | Implementa el método [what()](../details_exception/what/) que es llamado por la clase [ExceptionWrapper](../exceptionwrapper/). A pesar de que esta clase no hereda de std::exception, las clases derivadas pueden usar miembros protegidos/privados para implementar su lógica. Mover la implementación de este método a [ExceptionWrapper](../exceptionwrapper/) podría romper esa lógica. |
