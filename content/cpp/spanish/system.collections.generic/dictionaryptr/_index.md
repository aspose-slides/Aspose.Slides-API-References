...
description: Clase de puntero de diccionario con sobrecargas de operadores. Este tipo es un puntero para gestionar la eliminación de otros objetos. Debe asignarse en la pila y pasarse a funciones ya sea por valor o por referencia const.
type: docs
weight: 170
url: /es/system.collections.generic/dictionaryptr/
---
## DictionaryPtr clase


[Dictionary](../dictionary/) clase de puntero con sobrecargas de operadores. Este tipo es un puntero para gestionar la eliminación de otros objetos. Debe ser asignado en la pila y pasado a funciones ya sea por valor o por referencia const.
...
| Método | Descripción |
| --- | --- |
| auto [begin](../../system/smartptr/begin/)() | Accesor para el método [begin()](../../system/smartptr/begin/) de una colección subyacente. Sólo se compila si SmartPtr_ es un tipo de especialización con el método [begin()](../../system/smartptr/begin/). |
| auto [begin](../../system/smartptr/begin/)() const | Accesor para el método [begin()](../../system/smartptr/begin/) de una colección subyacente. Sólo se compila si SmartPtr_ es un tipo de especialización con el método [begin()](../../system/smartptr/begin/). |
...
|  [DictionaryPtr](./dictionaryptr/)(const [SharedPtr](../../system/sharedptr/)\<[Dictionary](../dictionary/)\<T, V\>\>\&) | Convierte el tipo de puntero. |
...
| int [GetHashCode](../../system/smartptr/gethashcode/)() const | Llama a [GetHashCode()](../../system/smartptr/gethashcode/) sobre el objeto apuntado. |
...
| **bool** [IsAliasingPtr](../../system/smartptr/isaliasingptr/)() const | Comprueba si el puntero apunta a otro objeto que no sea el propio (creado por un constructor de aliasing). |
...
| **bool** [operator<](../../system/smartptr/operator_less/)(Y *) const | Provee semántica de comparación menor para la clase [SmartPtr](../../system/smartptr/). |
| **bool** [operator<](../../system/smartptr/operator_less/)([SmartPtr](../../system/smartptr/)\<Y\> const\&) const | Provee semántica de comparación menor para la clase [SmartPtr](../../system/smartptr/). |
...
| V\& [operator[]](./operator[]/)(const X\&) const | Operador de acceso para trabajar con la conversión del tipo de clave. |
| V\& [operator[]](./operator[]/)(const T\&) const | Operador de acceso. |
| [SmartPtr_](../../system/smartptr/smartptr_/) [RemoveAliasing](../../system/smartptr/removealiasing/)() const | Elimina el aliasing (creado por un constructor de aliasing) del puntero, asegurándose de que gestiona (si es compartido) o rastrea (si es débil) el mismo objeto al que apunta. |
...
| void [SetContainedTemplateWeakPtr](../../system/smartptr/setcontainedtemplateweakptr/)(**uint32_t**) const | Llama al método SetTemplateWeakPtr() sobre el objeto apuntado (si lo hay). |
...
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const Y\&) | Inicializa una matriz vacía. Usado para traducir algunas construcciones de código en C#. |
|  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<P\>\&, [Pointee_](../../system/smartptr/pointee_/) *, [SmartPtrMode](../../system/smartptrmode/)) | Construye un [SmartPtr](../../system/smartptr/) que comparte información de propiedad con el valor inicial de ptr, pero mantiene un puntero p no relacionado y no administrado. |
...
| static const [System::TypeInfo](../../system/typeinfo/)\& [Type](../../system/smartptr/type/)() | Atajo para obtener el objeto [System::TypeInfo](../../system/typeinfo/) del tipo Pointee_. |
