...
المثال التالي يوضح كيفية إضافة شكل ثلاثي الأبعاد في PowerPoint [Presentation](../presentation/).
```cpp
...
```
المثال التالي يوضح كيفية تطبيق تأثير التدرج على شكل ثلاثي الأبعاد في PowerPoint [Presentation](../presentation/).
```cpp
// إنشاء كائن من فئة Presentation.
auto pres = System::MakeObject<Presentation>();
auto slide = pres->get_Slides()->idx_get(0);
...
pres->Save(u"sandbox_3d.pptx", SaveFormat::Pptx);
```
المثال التالي يوضح كيفية تطبيق تأثير ثلاثي الأبعاد على النص. لإنشاء نص ثلاثي الأبعاد يمكن استخدام تأثير التحويل WordArt.
```cpp
// إنشاء كائن من فئة Presentation.
auto pres = System::MakeObject<Presentation>();
auto slide = pres->get_Slides()->idx_get(0);
