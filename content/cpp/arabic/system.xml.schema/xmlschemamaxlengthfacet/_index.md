...
description: يمثل عنصر maxLength من مخطط XML كما حدّد من قبل اتحاد الويب العالمي (W3C). يمكن استخدام هذه الفئة لتحديد قيد على الحد الأقصى لطول قيمة البيانات لعنصر simpleType. يجب أن يكون الطول أقل من قيمة عنصر maxLength.
type: docs
weight: 599
url: /ar/system.xml.schema/xmlschemamaxlengthfacet/
---
## XmlSchemaMaxLengthFacet فئة


يمثل عنصر **maxLength** من XML [Schema](../) كما هو محدد من قبل اتحاد الويب العالمي [Web](../../system.web/) (W3C). يمكن استخدام هذه الفئة لتحديد قيد على الحد الأقصى لطول قيمة البيانات لعنصر **simpleType**. يجب أن يكون الطول أقل من قيمة عنصر **maxLength**.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | يقوم بمحاكاة مقارنة النقطة العائمة بأسلوب C# حيث تُعتبر قيمتا NaN متساويتين بالرغم من أن معيار IEC 60559:1989 ينص على أن NaN لا يساوي أي قيمة، بما فيها NaN. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | يقوم بمحاكاة مقارنة النقطة العائمة بأسلوب C# حيث تُعتبر قيمتا NaN متساويتين بالرغم من أن معيار IEC 60559:1989 ينص على أن NaN لا يساوي أي قيمة، بما فيها NaN. |
...
| [String](../../system/string/) [get_Value](../xmlschemafacet/get_value/)() | يرجع الصفة **value** لل facet. |
...
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | يفحص ما إذا كان الكائن يمثل مثلاً للنوع الموصوف بواسطة targetType. تناظر لمعامل C# 'is'. |
| void [Lock](../../system/object/lock/)() | يطبق قفل عبارة C# lock(). استدعِ مباشرةً أو استخدم كائن الحراسة [LockContext](../../system/lockcontext/). |
...
| void [set_Value](../xmlschemafacet/set_value/)(const [String](../../system/string/)\&) | يضبط الصفة **value** لل facet. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | يضبط وسيط القالب رقم n إلى مؤشر ضعيف (بدلاً من المشترك). يتيح تحويل المؤشرات في الحاويات إلى وضع ضعيف. |
...
| void [Unlock](../../system/object/unlock/)() | يطبق إلغاء قفل عبارة C# lock(). استدعِ مباشرةً أو استخدم كائن الحراسة [LockContext](../../system/lockcontext/). |
...
|  [XmlSchemaFacet](../xmlschemafacet/xmlschemafacet/)() | يهيئ نسخة جديدة من الفئة [XmlSchemaFacet](../xmlschemafacet/). |
|  [XmlSchemaMaxLengthFacet](./xmlschemamaxlengthfacet/)() | يهيئ نسخة جديدة من الفئة [XmlSchemaMaxLengthFacet](./). |
...
يجب تخصيص كائنات هذه الفئة باستخدام الدالة [System::MakeObject()](../../system/makeobject/) فقط. لا تقم بإنشاء نسخ من هذا النوع على المكدس أو باستخدام معامل new، لأن ذلك سيسبّب أخطاءً وقت التشغيل أو/و أعطالًا في التحقق. دائمًا غلف هذه الفئة في مؤشر [System::SmartPtr](../../system/smartptr/) واستخدم هذا المؤشر لتمريره إلى الدوال كمعامل. 
