...
description: "IndexOutOfRangeException تُرمى عندما يتم محاولة الوصول إلى عنصر من مجموعة باستخدام فهرس خارج حدودها. لا تقم بإنشاء كائنات من هذه الفئة يدويًا. استخدم فئة IndexOutOfRangeException بدلاً من ذلك. لا تقم بتغليف كائنات فئة IndexOutOfRangeException داخل System::SmartPtr."
type: docs
weight: 482
url: /ar/system/details_indexoutofrangeexception/
---
## Details_IndexOutOfRangeException فئة

IndexOutOfRangeException تُرمى عندما يتم محاولة الوصول إلى عنصر من مجموعة باستخدام فهرس خارج حدودها. لا تقم بإنشاء كائنات من هذه الفئة يدويًا. استخدم فئة IndexOutOfRangeException بدلاً من ذلك. لا تقم بتغليف كائنات فئة IndexOutOfRangeException داخل [System::SmartPtr](../smartptr/).
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | يحاكي مقارنة النقطة العائمة بأسلوب C# حيث يُعتبر NaNانان متساويين رغم أن معيار IEC 60559:1989 يوضح أن NaN غير مساوي لأي قيمة، بما في ذلك NaN. |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | يحاكي مقارنة النقطة العائمة بأسلوب C# حيث يُعتبر NaNانان متساويين رغم أن معيار IEC 60559:1989 يوضح أن NaN غير مساوي لأي قيمة، بما في ذلك NaN. |
...
| void [Lock](../object/lock/)() | ينفّذ قفل تعبير lock() في C#. استدع مباشرة أو استخدم كائن الحراسة [LockContext](../lockcontext/). |
...
|  [Object](../object/object/)([Object](../object/) const\&) | منشئ نسخة. لا ينسخ أي شيء فعليًا، بل يهيء كائنًا جديدًا ويسمح بإنشاء نسخ من الفئات الفرعية. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | عامل الإسناد. لا ينسخ أي شيء فعليًا، بل يهيء كائنًا جديدًا ويسمح بإنشاء نسخ من الفئات الفرعية. |
...
| void [set_HResult](../details_exception/set_hresult/)(**int32_t**) | يضبط HRESULT، وهي قيمة عددية مشفرة تُعطى لاستثناء معين. |
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | يضبط الوسيط القالب رقم n كإشارة ضعيفة (بدلاً من مشتركة). يتيح تحويل المؤشرات في الحاويات إلى وضع ضعيف. |
...
| void [Unlock](../object/unlock/)() | ينفّذ إلغاء قفل تعبير lock() في C#. استدع مباشرة أو استخدم كائن الحراسة [LockContext](../lockcontext/). |
...
| virtual const char * [what](../details_exception/what/)() const | ينفّذ طريقة [what()](../details_exception/what/) التي يستدعيها فئة [ExceptionWrapper](../exceptionwrapper/). بالرغم من أن هذه الفئة لا تورّث من std::exception، إلا أن الفئات المشتقة يمكنها استخدام الأعضاء المحمية/الخاصة لتنفيذ منطقها. نقل تنفيذ هذه الطريقة إلى [ExceptionWrapper](../exceptionwrapper/) قد يعيق هذا المنطق. |
