---
title: IsLeapDay()
second_title: Aspose.Slides สำหรับ C++ เอกสารอ้างอิง API
description: ตรวจสอบว่าวันนั้นเป็นปีอธิกสุรทินหรือไม่.
type: docs
weight: 79
url: /th/system.globalization/koreanlunisolarcalendar/isleapday/
---
## KoreanLunisolarCalendar::IsLeapDay(int, int, int, int) const method

ตรวจสอบว่าวันนั้นเป็นปีอธิกสุรทินหรือไม่

```cpp
bool System::Globalization::KoreanLunisolarCalendar::IsLeapDay(int year, int month, int day, int era) const override
```

### อาร์กิวเมนต์

| Parameter | Type | Description |
| --- | --- | --- |
| year | int | ปี |
| month | int | เดือน |
| day | int | วัน |
| era | int | สมัย |

### ค่าที่ส่งกลับ

True หากวันที่ระบุเป็นปีอธิกสุรทิน, false ไม่เช่นนั้น

## KoreanLunisolarCalendar::IsLeapDay(int, int, int) const method

ตรวจสอบว่าวันนั้นเป็นปีอธิกสุรทินหรือไม่

```cpp
virtual bool System::Globalization::Calendar::IsLeapDay(int year, int month, int day) const
```

### อาร์กิวเมนต์

| Parameter | Type | Description |
| --- | --- | --- |
| year | int | ปี |
| month | int | เดือน |
| day | int | วัน |

### ค่าที่ส่งกลับ

True หากวันที่ระบุเป็นปีอธิกสุรทิน, false ไม่เช่นนั้น

## KoreanLunisolarCalendar::IsLeapDay(int, int, int, int) const method

ตรวจสอบว่าวันนั้นเป็นปีอธิกสุรทินหรือไม่

```cpp
virtual bool System::Globalization::Calendar::IsLeapDay(int year, int month, int day, int era) const=0
```

### อาร์กิวเมนต์

| Parameter | Type | Description |
| --- | --- | --- |
| year | int | ปี |
| month | int | เดือน |
| day | int | วัน |
| era | int | สมัย |

### ค่าที่ส่งกลับ

True หากวันที่ระบุเป็นปีอธิกสุรทิน, false ไม่เช่นนั้น

## ดูเพิ่มเติม

* คลาส [KoreanLunisolarCalendar](../)
* เนมสเปซ [System::Globalization](../../)
* ไลบรารี [Aspose.Slides](../../../)