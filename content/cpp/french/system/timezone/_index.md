...
description: "Représente un fuseau horaire. Les objets de cette classe ne doivent être alloués qu'en utilisant la fonction System::MakeObject(). Ne créez jamais d'instance de ce type sur la pile ou en utilisant l'opérateur new, car cela entraînera des erreurs d'exécution et/ou des fautes d'assertion. Enveloppez toujours cette classe dans un pointeur System::SmartPtr et utilisez ce pointeur pour le passer aux fonctions en tant qu'argument."
type: docs
weight: 1301
url: /fr/system/timezone/
---
## classe TimeZone
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | Émule la comparaison de nombres à virgule flottante à la C# où deux NaN sont considérés égaux bien que, selon IEC 60559:1989, NaN ne soit égal à aucune valeur, y compris NaN. |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | Émule la comparaison de nombres à virgule flottante à la C# où deux NaN sont considérés égaux bien que, selon IEC 60559:1989, NaN ne soit égal à aucune valeur, y compris NaN. |
| virtual **bool** [FastCast](../object/fastcast/)(const Details::FastRttiBase\&, void **) const | À des fins internes uniquement. |
| static [TimeZonePtr](../timezoneptr/) [get_CurrentTimeZone](./get_currenttimezone/)() | Retourne une nouvelle instance de la classe [TimeZone](./) qui représente le fuseau horaire actuel. |
| virtual [String](../string/) [get_DaylightName](./get_daylightname/)() const | Retourne un nom pour l'heure d'été du fuseau horaire représenté par l'objet courant. |
| virtual [String](../string/) [get_StandardName](./get_standardname/)() const | Retourne un nom pour l'heure standard du fuseau horaire représenté par l'objet courant. |
...
| virtual [Globalization::DaylightTimePtr](../../system.globalization/daylighttimeptr/) [GetDaylightChanges](./getdaylightchanges/)(**int32_t**) | Retourne la période d'heure d'été pour une année particulière. |
...
| virtual [TimeSpan](../timespan/) [GetUtcOffset](./getutcoffset/)([DateTime](../datetime/)) | Retourne le décalage UTC pour l'heure locale spécifiée. |
...
| virtual **bool** [IsDaylightSavingTime](./isdaylightsavingtime/)([DateTime](../datetime/)) | Détermine si la valeur date-heure représentée par l'objet [DateTime](../datetime/) spécifié se trouve dans la plage d'heure d'été du fuseau horaire représenté par l'objet [TimeZone](./) courant. |
...
|  [Object](../object/object/)([Object](../object/) const\&) | Constructeur de copie. Ne copie rien, il initialise simplement le nouvel objet et permet la construction de copies de sous-classes. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | Opérateur d'affectation. Ne copie rien, il initialise simplement le nouvel objet et permet la construction de copies de sous-classes. |
...
| int [RemovedSharedRefs](../object/removedsharedrefs/)(int) | Diminue le compteur de références partagées du montant spécifié. |
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | Définit le nᵉᵉ argument de modèle comme un pointeur faible (plutôt que partagé). Permet de basculer les pointeurs dans les conteneurs en mode faible. |
