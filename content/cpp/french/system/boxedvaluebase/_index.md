...
description: "Une classe de base qui définit une interface et implémente certaines méthodes fondamentales d'une classe dérivée qui représente une valeur empaquetée. Les objets de cette classe ne doivent être alloués qu'en utilisant la fonction System::MakeObject(). Ne créez jamais d'instance de ce type sur la pile ou en utilisant l'opérateur new, car cela entraînera des erreurs d'exécution et/ou des échecs d'assertion. Enveloppez toujours cette classe dans un pointeur System::SmartPtr et utilisez ce pointeur pour le transmettre aux fonctions en tant qu'argument."
type: docs
weight: 131
url: /fr/system/boxedvaluebase/
---
## BoxedValueBase classe

Une classe de base qui définit une interface et implémente certaines méthodes fondamentales d’une classe dérivée qui représente une valeur empaquetée. Les objets de cette classe ne doivent être alloués qu’en utilisant la fonction [System::MakeObject()](../makeobject/). Ne créez jamais d’instance de ce type sur la pile ou en utilisant l’opérateur new, car cela entraînerait des erreurs d’exécution et/ou des échecs d’assertion. Enveloppez toujours cette classe dans un pointeur [System::SmartPtr](../smartptr/) et utilisez ce pointeur pour le transmettre aux fonctions en tant qu’argument.
...
| virtual [TypeCode](../typecode/) [GetTypeCode](./gettypecode/)() const | Renvoie la valeur représentant le type de la valeur empaquetée représentée par l'objet actuel. |
| virtual **uint64_t** [GetUnsignedLongLongValue](./getunsignedlonglongvalue/)() const | Convertit la valeur empaquetée représentée par l'objet actuel en entier 64 bits. |
...
| virtual **bool** [IsBoxedEnum](./isboxedenum/)() | Détermine si l'objet actuel représente une valeur empaquetée d'un type enum. |
...
| static [SharedPtr](../sharedptr/)\<[Object](../object/)\> [Parse](./parse/)(const [TypeInfo](../typeinfo/)\&, const [String](../string/)\&, **bool**) | Emboîte la valeur d'une constante d'énumération de l'énumération spécifiée avec le nom indiqué. Un paramètre indique si la casse doit être ignorée lors de l'interprétation de la chaîne spécifiant le nom de la constante d'énumération. |
| static [SharedPtr](../sharedptr/)\<[Object](../object/)\> [Parse](./parse/)(const [TypeInfo](../typeinfo/)\&, const [String](../string/)\&) | Emboîte la valeur d'une constante d'énumération de l'énumération spécifiée avec le nom indiqué. |
...
| [System::String](../string/) [ToString](./tostring/)(const [System::String](../string/)\&) const | Convertit l'objet empaqueté en chaîne en utilisant la chaîne de format spécifiée. |
| virtual [String](../string/) [ToString](./tostring/)() const | Analogue de la méthode C# [Object.ToString()](../object/tostring/). Permet de convertir des objets personnalisés en chaîne. |
