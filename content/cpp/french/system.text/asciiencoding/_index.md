...
description: "Représente l'encodage ASCII. Les objets de cette classe ne doivent être alloués qu'en utilisant la fonction System::MakeObject() . Ne créez jamais d'instance de ce type sur la pile ou avec l'opérateur new, car cela provoquerait des erreurs d'exécution et/ou des fautes d'assertion. Enveloppez toujours cette classe dans un pointeur System::SmartPtr et utilisez ce pointeur pour le passer aux fonctions en argument."
type: docs
weight: 1
url: /fr/system.text/asciiencoding/
---
## ASCIIEncoding classe

Représente l’encodage ASCII. Les objets de cette classe ne doivent être alloués qu’en utilisant la fonction [System::MakeObject()](../../system/makeobject/). Ne créez jamais d’instance de ce type sur la pile ou en utilisant l’opérateur new, car cela entraînerait des erreurs d’exécution et/ou des violations d’assertion. Enveloppez toujours cette classe dans le pointeur [System::SmartPtr](../../system/smartptr/) et utilisez ce pointeur pour le passer aux fonctions en argument.
...
| static [EncodingPtr](../../system/encodingptr/) [get_UTF8Unmarked](../encoding/get_utf8unmarked/)() | Uniquement interne, à utiliser par les bibliothèques de classes : non marqué et ne valide pas l’entrée. |
...
| int [GetMaxByteCount](./getmaxbytecount/)(int) override | Obtient le nombre maximal d’octets possible pour contenir une chaîne d’un nombre de caractères connu. |
