...
description: "Contient le code commun pour les collections de clés ou de valeurs. Les objets de cette classe doivent être alloués uniquement à l'aide de la fonction System::MakeObject(). Ne créez jamais d'instance de ce type sur la pile ou en utilisant l'opérateur new, car cela entraînera des erreurs d'exécution et/ou des fautes d'assertion. Enveloppez toujours cette classe dans un pointeur System::SmartPtr et utilisez ce pointeur pour le passer aux fonctions en tant qu'argument."
type: docs
weight: 79
url: /fr/system.collections.generic/basekvcollection/
---
## BaseKVCollection classe

Contient le code commun pour les collections de clés ou de valeurs. Les objets de cette classe doivent être alloués uniquement à l'aide de la fonction [System::MakeObject()](../../system/makeobject/). Ne créez jamais d'instance de ce type sur la pile ou en utilisant l'opérateur new, car cela entraînera des erreurs d'exécution et/ou des échecs d'assertion. Enveloppez toujours cette classe dans un pointeur [System::SmartPtr](../../system/smartptr/) et utilisez ce pointeur pour le passer aux fonctions en tant qu'argument.
...
| [iterator](../ienumerable/iterator/) [begin](../ienumerable/begin/)() | Obtient un itérateur pointant vers le premier élément (s'il existe) de la collection. Cet itérateur ne peut pas être utilisé pour modifier l'objet référencé parce que [GetEnumerator()](../ienumerable/getenumerator/) renvoie une copie de l'objet T. |
...
| [const_iterator](../ienumerable/const_iterator/) [cbegin](../ienumerable/cbegin/)() const | Obtient un itérateur pointant vers le premier élément qualifié en const (s'il existe) de la collection. |
| [const_iterator](../ienumerable/const_iterator/) [cend](../ienumerable/cend/)() const | Obtient un itérateur pointant immédiatement après le dernier élément qualifié en const (s'il existe) de la collection. |
| void [Clear](../ikvcollection/clear/)() override | Supprime tous les éléments du conteneur. |
| **bool** [Contains](../ikvcollection/contains/)(const T\&) const override | Vérifie si l'élément est présent dans le conteneur. |
| void [CopyTo](./copyto/)([ArrayPtr](../../system/arrayptr/)\<KV\>, int) override | Copie les données dans les éléments existants du tableau. |
| [iterator](../ienumerable/iterator/) [end](../ienumerable/end/)() | Obtient un itérateur pointant immédiatement après le dernier élément (s'il existe) de la collection. Cet itérateur ne peut pas être utilisé pour modifier l'objet référencé parce que [GetEnumerator()](../ienumerable/getenumerator/) renvoie une copie de l'objet T. |
| [const_iterator](../ienumerable/const_iterator/) [end](../ienumerable/end/)() const | Obtient un itérateur pointant immédiatement après le dernier élément (s'il existe) de l'instance de la collection qualifiée en const. |
...
| virtual [SharedPtr](../../system/sharedptr/)\<[IEnumerator](../ienumerator/)\<T\>\> [GetEnumerator](../ikvcollection/getenumerator/)() | Obtient l'énumérateur du conteneur. |
...
| virtual T [idx_get](../ikvcollection/idx_get/)(int) const | Fonction d'accès (getter). |
| void [idx_set](../ikvcollection/idx_set/)(int, T) override | Fonction de modification (setter). |
| int [IndexOf](../ikvcollection/indexof/)(const T\&) const override | Obtient l'index de l'élément dans le conteneur. |
...
| void [Lock](../../system/object/lock/)() | Met en œuvre le verrouillage de la déclaration C# lock(). Appelez directement ou utilisez l'objet sentinelle [LockContext](../../system/lockcontext/). |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | Constructeur de copie. Ne copie rien, vraiment, il initialise simplement le nouvel objet et permet la copie lors de la construction des sous-classes. |
...
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | Opérateur d'affectation. Ne copie rien, vraiment, il initialise simplement le nouvel objet et permet la copie lors de la construction des sous-classes. |
...
| void [SetTemplateWeakPtr](./settemplateweakptr/)(**uint32_t**) override | Permet la compilation, mais ne fait rien car cette structure ne possède pas de données. |
...
| void [Unlock](../../system/object/unlock/)() | Met en œuvre le déverrouillage de la déclaration C# lock(). Appelez directement ou utilisez l'objet sentinelle [LockContext](../../system/lockcontext/). |
