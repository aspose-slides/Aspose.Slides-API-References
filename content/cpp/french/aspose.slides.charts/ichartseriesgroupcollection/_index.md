...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IChartSeriesGroup](../ichartseriesgroup/)\> [idx_get](./idx_get/)([System::SharedPtr](../../system/sharedptr/)\<[IChartSeries](../ichartseries/)\>) | Obtient le groupe de séries par série. |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IChartSeriesGroup](../ichartseriesgroup/)\> [idx_get](./idx_get/)(**int32_t**) | Obtient le groupe de séries par index. |
...
1) Chaque groupe de séries contient des séries avec des types combinables. Les groupes de types de séries combinables sont définis et décrits avec l'énumération CombinableSeriesTypesGroup. De plus, chaque groupe de séries contient des séries qui sont tracées soit sur les axes principaux, soit sur les axes secondaires (pas les deux cas dans un même groupe). Ainsi, le principe du regroupement des séries est un groupement par les types de groupe susmentionnés et par le type de traçage principal/secondaire.

2) Un groupe de séries contient certaines propriétés de séries communes à chaque série du groupe ("propriétés du groupe de séries"). Les "propriétés du groupe de séries" dans la classe [ChartSeriesGroup](../chartseriesgroup/) sont en lecture/écriture. Chacune des "propriétés du groupe de séries" peut avoir une projection en lecture seule dans la classe [ChartSeries](../chartseries/).
