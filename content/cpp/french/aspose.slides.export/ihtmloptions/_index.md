...
| virtual **bool** [get_DeletePicturesCroppedAreas](./get_deletepicturescroppedareas/)() | Un indicateur booléen indique si les parties recadrées restent dans le document. Si vrai, les parties recadrées seront supprimées, sinon elles seront sérialisées dans le document (ce qui peut éventuellement entraîner un fichier plus volumineux). Lit **bool**. |
| virtual **bool** [get_DisableFontLigatures](./get_disablefontligatures/)() | Obtient une valeur indiquant si le texte est rendu sans utiliser les ligatures. Lorsqu'elle est définie sur **true**, les ligatures seront désactivées dans la sortie rendue. Par défaut, cette propriété est définie sur **false**. |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IHtmlFormatter](../ihtmlformatter/)\> [get_HtmlFormatter](./get_htmlformatter/)() | Renvoie le modèle HTML. Lit [IHtmlFormatter](../ihtmlformatter/). |
...
| virtual **uint8_t** [get_JpegQuality](./get_jpegquality/)() | Renvoie une valeur déterminant la qualité des images JPEG à l'intérieur du document PDF. Lit **uint8_t**. |
| virtual [Aspose::Slides::Export::PicturesCompression](../picturescompression/) [get_PicturesCompression](./get_picturescompression/)() | Représente le niveau de compression des images. Lit [PicturesCompression](../picturescompression/). |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[ISlideImageFormat](../islideimageformat/)\> [get_SlideImageFormat](./get_slideimageformat/)() | Renvoie les options de format d'image des diapositives. Lit [ISlideImageFormat](../islideimageformat/). |
...
| virtual **bool** [get_SvgResponsiveLayout](./get_svgresponsivelayout/)() | Vrai pour exclure les attributs de largeur et de hauteur du conteneur SVG - cela rendra la mise en page réactive. Faux sinon. Lit **bool**. |
...
| virtual void [set_DeletePicturesCroppedAreas](./set_deletepicturescroppedareas/)(**bool**) | Un indicateur booléen indique si les parties recadrées restent dans le document. Si vrai, les parties recadrées seront supprimées, sinon elles seront sérialisées dans le document (ce qui peut éventuellement entraîner un fichier plus volumineux). Écrit **bool**. |
| virtual void [set_DisableFontLigatures](./set_disablefontligatures/)(**bool**) | Définit une valeur indiquant si le texte est rendu sans utiliser les ligatures. Lorsqu'elle est définie sur **true**, les ligatures seront désactivées dans la sortie rendue. Par défaut, cette propriété est définie sur **false**. |
...
| virtual void [set_HtmlFormatter](./set_htmlformatter/)([System::SharedPtr](../../system/sharedptr/)\<[IHtmlFormatter](../ihtmlformatter/)\>) | Définit le modèle HTML. Écrit [IHtmlFormatter](../ihtmlformatter/). |
| virtual void [set_JpegQuality](./set_jpegquality/)(**uint8_t**) | Définit une valeur déterminant la qualité des images JPEG à l'intérieur du document PDF. Écrit **uint8_t**. |
| virtual void [set_PicturesCompression](./set_picturescompression/)([Aspose::Slides::Export::PicturesCompression](../picturescompression/)) | Représente le niveau de compression des images. Écrit [PicturesCompression](../picturescompression/). |
...
| virtual void [set_SlideImageFormat](./set_slideimageformat/)([System::SharedPtr](../../system/sharedptr/)\<[ISlideImageFormat](../islideimageformat/)\>) | Définit les options de format d'image des diapositives. Écrit [ISlideImageFormat](../islideimageformat/). |
...
| virtual void [set_SvgResponsiveLayout](./set_svgresponsivelayout/)(**bool**) | Vrai pour exclure les attributs de largeur et de hauteur du conteneur SVG - cela rendra la mise en page réactive. Faux sinon. Écrit **bool**. |
