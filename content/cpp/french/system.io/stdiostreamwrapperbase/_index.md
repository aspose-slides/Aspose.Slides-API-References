...
description: "Représente une classe de base pour les wrappers de type System.IO.Stream. Les objets de cette classe ne doivent être alloués qu’en utilisant la fonction System::MakeObject(). Ne créez jamais d’instance de ce type sur la pile ou en utilisant l’opérateur new, car cela entraînerait des erreurs d’exécution et/ou des défauts d’assertion. Enveloppez toujours cette classe dans un pointeur System::SmartPtr et utilisez ce pointeur pour le transmettre aux fonctions en tant qu’argument."
type: docs
weight: 352
url: /fr/system.io/stdiostreamwrapperbase/
---
## STDIOStreamWrapperBase classe


Représente une classe de base pour les wrappers de type [System.IO.Stream](../stream/). Les objets de cette classe ne doivent être alloués qu’en utilisant la fonction [System::MakeObject()](../../system/makeobject/). Ne créez jamais d’instance de ce type sur la pile ou en utilisant l’opérateur new, car cela entraînerait des erreurs d’exécution et/ou des violations d’assertion. Enveloppez toujours cette classe dans un pointeur [System::SmartPtr](../../system/smartptr/) et utilisez ce pointeur pour le transmettre aux fonctions en tant qu’argument.
...
| void [CopyTo](../stream/copyto/)(const [SharedPtr](../../system/sharedptr/)\<[Stream](../stream/)\>\&, **int32_t**) | Copie des octets vers le flux spécifié, en utilisant la taille du tampon spécifiée. |
...
| virtual void [Flush](../stream/flush/)() | Vide les tampons de ce flux et écrit toutes les données tamponnées dans le support sous-jacent. |
| virtual [TaskPtr](../../system/taskptr/) [FlushAsync](../stream/flushasync/)(const [Threading::CancellationToken](../../system.threading/cancellationtoken/)\&) | Vide de façon asynchrone tous les tampons de ce flux, oblige les données tamponnées à être écrites sur le dispositif sous-jacent et surveille les demandes d'annulation. |
| [TaskPtr](../../system/taskptr/) [FlushAsync](../stream/flushasync/)() | Vide de façon asynchrone tous les tampons de ce flux, oblige les données tamponnées à être écrites sur le dispositif sous-jacent et surveille les demandes d'annulation. |
| **bool** [get_CanRead](./get_canread/)() const override | Détermine si le flux prend en charge la lecture. |
| **bool** [get_CanSeek](./get_canseek/)() const override | Détermine si le flux prend en charge le repositionnement. |
| virtual **bool** [get_CanTimeout](../stream/get_cantimeout/)() const | Obtient une valeur qui détermine si le flux actuel peut expirer. |
| **bool** [get_CanWrite](./get_canwrite/)() const override | Détermine si le flux prend en charge l'écriture. |
| **int64_t** [get_Length](./get_length/)() const override | Renvoie la longueur du flux. |
...
| [STDIOStreamWrapperBase](./)\& [operator=](./operator_equal/)(const [STDIOStreamWrapperBase](./)\&) | Opérateur d'affectation par copie. Supprimé. |
...
| virtual **int32_t** [Read](../stream/read/)(const [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>\&, **int32_t**, **int32_t**) | Lit le nombre spécifié d'octets depuis le flux et les écrit dans le tableau d'octets spécifié. |
| virtual **int32_t** [Read](../stream/read/)(const System::Details::ArrayView\<**uint8_t**\>\&, **int32_t**, **int32_t**) | Lit le nombre spécifié d'octets depuis le flux et les écrit dans le tableau d'octets spécifié. |
| **int32_t** [Read](../stream/read/)(const System::Details::StackArray\<**uint8_t**, N\>\&, **int32_t**, **int32_t**) | Lit le nombre spécifié d'octets depuis le flux et les écrit dans le tableau d'octets spécifié. |
| virtual **int32_t** [Read](../stream/read/)(const [System::Span](../../system/span/)\<**uint8_t**\>\&) | Lit le nombre spécifié d'octets depuis le flux et les écrit dans la séquence d'octets spécifiée. |
...
| virtual int [ReadByte](../stream/readbyte/)() | Lit un octet unique du flux et renvoie une valeur entière 32 bits équivalente à la valeur de l'octet lu. |
...
|  [RTTI_INFO_TEMPLATE_CLASS](./rtti_info_template_class/)([ThisType](./thistype/), [ThisTypeBaseTypesInfo](./thistypebasetypesinfo/)) | Information RTTI. |
...
| virtual void [SetLength](../stream/setlength/)(**int64_t**) | Définit la longueur du flux représenté par l'objet actuel. |
...
| virtual void [Write](../stream/write/)(const [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>\&, **int32_t**, **int32_t**) | Écrit la sous-plage d'octets spécifiée du tableau d'octets spécifié dans le flux. |
| virtual void [Write](../stream/write/)(const System::Details::ArrayView\<**uint8_t**\>\&, **int32_t**, **int32_t**) | Écrit la sous-plage d'octets spécifiée du tableau d'octets spécifié dans le flux. |
...
| virtual void [Write](../stream/write/)(const [System::ReadOnlySpan](../../system/readonlyspan/)\<**uint8_t**\>\&) | Écrit la sous-plage d'octets spécifiée de la séquence d'octets spécifiée dans le flux. |
...
| [TaskPtr](../../system/taskptr/) [WriteAsync](../stream/writeasync/)(const [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>\&, **int32_t**, **int32_t**) | Écrit de façon asynchrone une séquence d'octets dans le flux actuel, avance la position actuelle dans ce flux du nombre d'octets écrits, et surveille les demandes d'annulation. |
| virtual void [WriteByte](../stream/writebyte/)(**uint8_t**) | Écrit la valeur entière non signée de 8 bits spécifiée dans le flux. |
