...
description: "دستور پایگاه داده. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از اپراتور new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا شکست‌های اعتبارسنجی می‌شود. همیشه این کلاس را در اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای عبور به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 1
url: /fa/system.data.common/dbcommand/
---
## DbCommand کلاس

دستور پایگاه داده. اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از اپراتور new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا شکست‌های اعتبارسنجی می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای عبور به توابع به عنوان آرگومان استفاده کنید.
...
| virtual int [ExecuteNonQuery](./executenonquery/)() | دستور غیر-پرسشی را اجرا می‌کند. |
| virtual [SharedPtr](../../system/sharedptr/)\<[DbDataReader](../dbdatareader/)\> [ExecuteReader](./executereader/)() | دستور پرسشی را اجرا می‌کند. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای استفاده داخلی. |
| virtual [String](../../system/string/) [get_CommandText](./get_commandtext/)() const | اطلاعات RTTI. |
| virtual [SharedPtr](../../system/sharedptr/)\<[System::Data::Common::DbConnection](../dbconnection/)\> [get_Connection](./get_connection/)() const | اتصال پایگاه داده مرتبط با دستور را دریافت می‌کند. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری با عبارت lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء نگهبانی [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازندهٔ نسخه‌برداری. در واقع چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌نماید و امکان ساخت کلاس‌های فرزند را از طریق نسخه‌برداری فراهم می‌کند. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | اپراتور انتساب. در واقع چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌نماید و امکان ساخت کلاس‌های فرزند را از طریق نسخه‌برداری فراهم می‌کند. |
...
| virtual void [set_CommandText](./set_commandtext/)([String](../../system/string/)) const | متن دستور پایگاه داده را تنظیم می‌کند. |
| virtual void [set_Connection](./set_connection/)([SharedPtr](../../system/sharedptr/)\<[System::Data::Common::DbConnection](../dbconnection/)\>) | اتصال پایگاه داده مرتبط با دستور را دریافت می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌برداری با عبارت lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء نگهبانی [LockContext](../../system/lockcontext/) استفاده کنید. |
