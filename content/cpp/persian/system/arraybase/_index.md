...
description: نمونه ساختگی برای کلاس System.Array (کلاس پایهٔ انتزاعی برای همهٔ آرایه‌ها) که ممکن است بر حسب درخواست با عملکرد پر شود.
type: docs
weight: 27
url: /fa/system/arraybase/
---
## ArrayBase کلاس

نمونه ساختگی برای کلاس [System.Array](../array/) (کلاس پایهٔ انتزاعی برای همهٔ آرایه‌ها) ممکن است با عملکرد بر حسب درخواست پر شود.
...
| متد | توضیح |
| --- | --- |
| virtual **bool** [Equals](../object/equals/)([ptr](../object/ptr/)) | اشیاء را با استفاده از معنی‌سازی C# [Object.Equals](../object/equals/) مقایسه می‌کند. |
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | شبیه‌سازی مقایسهٔ نقطه‌اعشاری به سبک C# که در آن دو NaN برابر درنظر گرفته می‌شوند حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | شبیه‌سازی مقایسهٔ نقطه‌اعشاری به سبک C# که در آن دو NaN برابر درنظر گرفته می‌شوند حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| virtual **bool** [FastCast](../object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای اهداف داخلی. |
| virtual **int32_t** [get_Length](./get_length/)() const | عدد صحیح ۳۲ بیتی را برمی‌گرداند که مجموع تمام عناصر در همهٔ ابعاد آرایه را نشان می‌دهد. |
...
| virtual **bool** [Is](../object/is/)(const [TypeInfo](../typeinfo/)\&) const | بررسی می‌کند که آیا شیء نمونه‌ای از نوعی است که توسط targetType توضیح داده شده است. معادل عملگر 'is' در C#. |
| void [Lock](../object/lock/)() | پیاده‌سازی قفل‌کردن بیانیهٔ lock() در C#. به‌صورت مستقیم فراخوانی کنید یا از شیء نگهبان [LockContext](../lockcontext/) استفاده کنید. |
...
|  [Object](../object/object/)([Object](../object/) const\&) | سازندهٔ کپی. در واقع هیچ داده‌ای را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساختکلاس‌های فرزند از طریق کپی را فراهم می‌سازد. |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | عملگر انتساب. در واقع هیچ داده‌ای را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساختکلاس‌های فرزند از طریق کپی را فراهم می‌سازد. |
| virtual void * [raw_data_ptr](./raw_data_ptr/)() | نشانگر به اولین عنصر آرایهٔ تک‌بعد را برمی‌گرداند. برای آرایه‌های چندبعدی نتیجه نامشخص است. |
...
| int [SharedRefRemovedSafe](../object/sharedrefremovedsafe/)() | شمارندهٔ مرجع مشترک را کاهش داده و مقدار آن را برمی‌گرداند. نباید به‌صورت مستقیم فراخوانی شود؛ به‌جای آن از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
...
| void [Unlock](../object/unlock/)() | پیاده‌سازی بازکردن قفل بیانیهٔ lock() در C#. به‌صورت مستقیم فراخوانی کنید یا از شیء نگهبان [LockContext](../lockcontext/) استفاده کنید. |
