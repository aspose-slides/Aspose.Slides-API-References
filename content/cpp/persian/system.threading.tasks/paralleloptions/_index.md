...
description: گزینه‌هایی را که عملکرد متدهای کلاس Parallel را تنظیم می‌کند، ذخیره می‌کند.
type: docs
weight: 27
url: /fa/system.threading.tasks/paralleloptions/
---
## کلاس ParallelOptions

گزینه‌هایی را که عملکرد متدها را روی کلاس [Parallel](../parallel/) تنظیم می‌کند، ذخیره می‌کند.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسه نقطه‌اعشار به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، اگرچه بر اساس IEC 60559:1989 NaN با هیچ مقداری، از جمله NaN، برابر نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه نقطه‌اعشار به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، اگرچه بر اساس IEC 60559:1989 NaN با هیچ مقداری، از جمله NaN، برابر نیست. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای مقاصد داخلی. |
| const [CancellationToken](../../system.threading/cancellationtoken/)\& [get_CancellationToken](./get_cancellationtoken/)() | [CancellationToken](../../system.threading/cancellationtoken/) مرتبط با این نمونه [ParallelOptions](./) را دریافت می‌کند. |
| **int32_t** [get_MaxDegreeOfParallelism](./get_maxdegreeofparallelism/)() | حداکثر درجه موازی‌سازی فعال شده توسط این نمونه [ParallelOptions](./) را دریافت می‌کند. |
| const [SharedPtr](../../system/sharedptr/)\<[TaskScheduler](../taskscheduler/)\>\& [get_TaskScheduler](./get_taskscheduler/)() | [TaskScheduler](../taskscheduler/) مرتبط با این نمونه [ParallelOptions](./) را دریافت می‌کند. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری بیان lock() در C# را پیاده‌سازی می‌کند. مستقیماً فراخوانی شود یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازنده کپی. در واقع چیزی کپی نمی‌کند، فقط شیء جدید را مقداردهی می‌کند و امکان ساخت کپی برای کلاس‌های فرعی را فراهم می‌کند. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر انتساب. در واقع چیزی کپی نمی‌کند، فقط شیء جدید را مقداردهی می‌کند و امکان ساخت کپی برای کلاس‌های فرعی را فراهم می‌کند. |
|  [ParallelOptions](./paralleloptions/)() | یک نمونه [ParallelOptions](./) را با مقادیر پیش‌فرض می‌سازد. |
...
| void [set_CancellationToken](./set_cancellationtoken/)(const [CancellationToken](../../system.threading/cancellationtoken/)\&) | [CancellationToken](../../system.threading/cancellationtoken/) مرتبط با این نمونه [ParallelOptions](./) را تنظیم می‌کند. |
| void [set_MaxDegreeOfParallelism](./set_maxdegreeofparallelism/)(**int32_t**) | حداکثر درجه موازی‌سازی فعال شده توسط این نمونه [ParallelOptions](./) را تنظیم می‌کند. |
| void [set_TaskScheduler](./set_taskscheduler/)(const [SharedPtr](../../system/sharedptr/)\<[TaskScheduler](../taskscheduler/)\>\&) | [TaskScheduler](../taskscheduler/) مرتبط با این نمونه [ParallelOptions](./) را تنظیم می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌برداری بیان lock() در C# را پیاده‌سازی می‌کند. مستقیماً فراخوانی شود یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
