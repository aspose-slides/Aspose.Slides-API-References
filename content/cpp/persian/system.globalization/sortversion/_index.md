...
description: "اطلاعاتی دربارهٔ نسخهٔ یونیکد مورد استفاده برای مقایسه و ترتیب رشته‌ها فراهم می‌کند. اشیای این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با اپراتور new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای اعتبارسنجی می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای ارسال به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 300
url: /fa/system.globalization/sortversion/
---
## SortVersion کلاس
...
| متد | توضیح |
| --- | --- |
| **bool** [Equals](./equals/)([SharedPtr](../../system/sharedptr/)\<[SortVersion](./)\>) override | بررسی می‌کند که آیا نمونهٔ فعلی [SortVersion](./) برابر با شیء [SortVersion](./) مشخص‌شده است. |
| **bool** [Equals](./equals/)([SharedPtr](../../system/sharedptr/)\<[Object](../../system/object/)\>) override | بررسی می‌کند که آیا نمونهٔ فعلی [SortVersion](./) برابر با شیء [SortVersion](./) مشخص‌شده است. |
| virtual **bool** [Equals](../../system/iequatable/equals/)(T) | تعیین می‌کند که آیا اشیای فعلی و مشخص‌شده برابر هستند. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسهٔ نقطه‌اعشار به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، هرچند برطبق IEC 60559:1989 NaN برابر با هیچ مقداری نیست، حتی NaN. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسهٔ نقطه‌اعشار به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، هرچند برطبق IEC 60559:1989 NaN برابر با هیچ مقداری نیست، حتی NaN. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای مقاصد داخلی. |
| int [get_FullVersion](./get_fullversion/)() | شمارهٔ کامل نسخه را برمی‌گرداند. |
| [Guid](../../system/guid/) [get_SortId](./get_sortid/)() | شناسهٔ یکتا برای این شیء را برمی‌گرداند. |
...
| void [Lock](../../system/object/lock/)() | اجرای قفل‌گذاری عبارت lock() در C# را پیاده‌سازی می‌کند. مستقیماً فراخوانی کنید یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| **bool** [operator!=](./operator_not_equal/)(const [SortVersion](./)\&) | بررسی می‌کند که آیا نمونهٔ فعلی [SortVersion](./) برابر با شیء [SortVersion](./) مشخص‌شده نیست. |
...
| **bool** [operator==](./operator_equal_equal/)(const [SortVersion](./)\&) | بررسی می‌کند که آیا نمونهٔ فعلی [SortVersion](./) برابر با شیء [SortVersion](./) مشخص‌شده است. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان nام قالب را به یک اشاره‌گر ضعیف تنظیم می‌کند (نه مشترک). امکان تغییر اشاره‌گرها در ساختمان‌ها به حالت ضعیف را فراهم می‌کند. |
...
|  [SortVersion](./sortversion/)(int, const [Guid](../../system/guid/)\&) | اطلاعات RTTI. |
...
| void [Unlock](../../system/object/unlock/)() | اجرای بازکردن قفل عبارت lock() در C# را پیاده‌سازی می‌کند. مستقیماً فراخوانی کنید یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
