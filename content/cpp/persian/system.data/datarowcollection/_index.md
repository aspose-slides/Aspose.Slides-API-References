...
description: "مجموعه‌ای از ردیف‌های داده. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای اطمینان‌سنجی می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچانید و از این اشاره‌گر برای انتقال به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 66
url: /fa/system.data/datarowcollection/
---
## DataRowCollection کلاس
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسه نقطه‌فلزی به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند هرچند طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه نقطه‌فلزی به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند هرچند طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای استفاده داخلی. |
| **int32_t** [get_Count](./get_count/)() | تعداد ردیف‌های موجود در مجموعه را دریافت می‌کند. |
...
| [System::SharedPtr](../../system/sharedptr/)\<[DataRow](../datarow/)\> [idx_get](./idx_get/)(const **int32_t**) | اطلاعات RTTI. |
...
| void [Lock](../../system/object/lock/)() | اجرای قفل‌گذاری دستور C# lock(). به‌صورت مستقیم فراخوانی کنید یا از شیء نگهدارنده [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | اجرای بازکردن قفل دستور C# lock(). به‌صورت مستقیم فراخوانی کنید یا از شیء نگهدارنده [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual  [~Object](../../system/object/~object/)() | شیء را نابودی می‌کند. تمام ساختارهای داده داخلی را آزاد می‌سازد. |
