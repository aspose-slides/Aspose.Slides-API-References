...
---
## IMathNaryOperatorProperties کلاس


ویژگی‌های [IMathNaryOperator](../imathnaryoperator/) را مشخص می‌کند
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسه نقطه‌ی شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر درنظر گرفته می‌شوند، اگرچه بر اساس IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه نقطه‌ی شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر درنظر گرفته می‌شوند، اگرچه بر اساس IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN نیست. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای مقاصد داخلی. |
| virtual **bool** [get_GrowToMatchOperandHeight](./get_growtomatchoperandheight/)() | کاراکتر عملگر به صورت عمودی رشد می‌کند تا با ارتفاع عملوند مطابقت داشته باشد |
| virtual **bool** [get_HideSubscript](./get_hidesubscript/)() | پایین‌نویس را مخفی می‌کند |
| virtual **bool** [get_HideSuperscript](./get_hidesuperscript/)() | بالا‌نویس را مخفی می‌کند |
| virtual [MathLimitLocations](../mathlimitlocations/) [get_LimitLocation](./get_limitlocation/)() | محل محدودیت‌ها (پایین‌نویس و بالا‌نویس) |
| virtual char16_t [get_Operator](./get_operator/)() | کاراکتر عملگر Nary. برای مثال: '\\u2211', '\\u222B' |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری با دستور lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual void [set_GrowToMatchOperandHeight](./set_growtomatchoperandheight/)(**bool**) | کاراکتر عملگر به صورت عمودی رشد می‌کند تا با ارتفاع عملوند مطابقت داشته باشد |
| virtual void [set_HideSubscript](./set_hidesubscript/)(**bool**) | پایین‌نویس را مخفی می‌کند |
| virtual void [set_HideSuperscript](./set_hidesuperscript/)(**bool**) | بالا‌نویس را مخفی می‌کند |
| virtual void [set_LimitLocation](./set_limitlocation/)([MathLimitLocations](../mathlimitlocations/)) | محل محدودیت‌ها (پایین‌نویس و بالا‌نویس) |
| virtual void [set_Operator](./set_operator/)(char16_t) | کاراکتر عملگر Nary. برای مثال: '\\u2211', '\\u222B' |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارندهٔ مرجع مشترک را کاهش می‌دهد و برمی‌گرداند. نباید به‌صورت مستقیم صدا زده شود؛ به‌جای آن از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گشایی با دستور lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
