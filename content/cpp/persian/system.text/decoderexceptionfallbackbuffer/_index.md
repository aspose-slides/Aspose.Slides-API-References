...
description: "بافر برای استراتژی fallback رمزگشایی با پرتاب استثنا. در واقع هیچ چیزی ذخیره نمی‌کند، بلکه به‌جای آن پرتاب می‌کند. اشیای این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا می‌تواند منجر به خطاهای زمان اجرا و/یا نقص‌های ادعایی شود. همیشه این کلاس را در اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای ارسال به توابع به‌عنوان آرگومان استفاده کنید."
type: docs
weight: 40
url: /fa/system.text/decoderexceptionfallbackbuffer/
---
## DecoderExceptionFallbackBuffer کلاس


[Buffer](../../system/buffer/) برای استراتژی fallback رمزگشایی با پرتاب استثنا. در واقع چیزی ذخیره نمی‌کند، بلکه پرتاب می‌کند. اشیا این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های ادعایی می‌شود. همیشه این کلاس را در اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای پاس دادن آن به توابع به‌عنوان آرگومان استفاده کنید.
...
| متد | توضیح |
| --- | --- |
|  [DecoderExceptionFallbackBuffer](./decoderexceptionfallbackbuffer/)() | سازنده. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسه شناور سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، اگرچه طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه شناور سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، اگرچه طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| **bool** [Fallback](./fallback/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int) override | در صورت شکست رمزگشایی، آن را مدیریت می‌کند. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای مقاصد داخلی. |
| int [get_Remaining](./get_remaining/)() const override | تعداد کاراکترهای باقی‌مانده را دریافت می‌کند. |
...
| char_t [GetNextChar](./getnextchar/)() override | کاراکتر بعدی در دسترس را دریافت می‌کند. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری با عبارت lock() در C# را پیاده‌سازی می‌کند. مستقیم صدا بزنید یا از شیء نظارت [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| **bool** [MovePrevious](./moveprevious/)() override | به کاراکتر قبلی می‌رود. |
|  [Object](../../system/object/object/)() | شیء را ایجاد می‌کند. تمام ساختارهای داده داخلی را مقداردهی اولیه می‌کند. |
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازنده کپی. در واقع هیچ چیز را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی برای زیرکلاس‌ها را می‌دهد. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | اپراتور انتساب. در واقع هیچ چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی برای زیرکلاس‌ها را می‌دهد. |
...
| virtual void [Reset](../decoderfallbackbuffer/reset/)() | بافر را به حالت اولیه بازنشانی می‌کند. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان n‌ام قالب را به یک اشاره‌گر ضعیف تنظیم می‌کند (به‌جای مشترک). امکان تغییر اشاره‌گرها در محفظه‌ها به حالت ضعیف را فراهم می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌برداری با عبارت lock() در C# را پیاده‌سازی می‌کند. مستقیم صدا بزنید یا از شیء نظارت [LockContext](../../system/lockcontext/) استفاده کنید. |
...
* کلاس [DecoderFallbackBuffer](../decoderfallbackbuffer/)
