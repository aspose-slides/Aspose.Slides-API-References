...
description: "استراتژی جایگزینی برای تعویض نمادهای نادرست با یک بدنه (stub) فراهم می‌کند. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() ساخته شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های اعتبارسنجی می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای عبور به توابع به‌عنوان آرگومان استفاده کنید."
type: docs
weight: 79
url: /fa/system.text/decoderreplacementfallback/
---
## DecoderReplacementFallback کلاس
...
| Method | Description |
| --- | --- |
| [DecoderFallbackBufferPtr](../../system/decoderfallbackbufferptr/) [CreateFallbackBuffer](./createfallbackbuffer/)() override | یک بافر fallback ایجاد می‌کند. |
|  [DecoderReplacementFallback](./decoderreplacementfallback/)() | سازنده‌ای که از رشته جایگزین \"?\" پیش‌فرض استفاده می‌کند. |
|  [DecoderReplacementFallback](./decoderreplacementfallback/)(const [String](../../system/string/)\&) | سازنده. |
...
| const [String](../../system/string/)\& [get_DefaultString](./get_defaultstring/)() const | رشته جایگزین را دریافت می‌کند. |
| static const [DecoderFallbackPtr](../../system/decoderfallbackptr/) [get_ExceptionFallback](../decoderfallback/get_exceptionfallback/)() | پیاده‌سازی پیش‌فرض fallback استثنا را دریافت می‌کند. |
| int [get_MaxCharCount](./get_maxcharcount/)() const override | حداکثر تعداد کاراکترهایی که شیء می‌تواند بازگرداند را دریافت می‌کند. |
| static const [DecoderFallbackPtr](../../system/decoderfallbackptr/) [get_ReplacementFallback](../decoderfallback/get_replacementfallback/)() | پیاده‌سازی پیش‌فرض fallback جایگزینی را دریافت می‌کند. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری عبارت C# lock() را پیاده‌سازی می‌کند. مستقیم فراخوانی کنید یا از شیء sentry [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| int [RemovedSharedRefs](../../system/object/removedsharedrefs/)(int) | شمارنده مرجع مشترک را به میزان مقدار مشخص کاهش می‌دهد. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گذاری عبارت C# lock() را باز می‌کند. مستقیم فراخوانی کنید یا از شیء sentry [LockContext](../../system/lockcontext/) استفاده کنید. |
