...
description: این کلاس عنصر keyref را از XMLSchema مطابق با کنسرسیوم جهانی وب (W3C) نشان می‌دهد.
type: docs
weight: 547
url: /fa/system.xml.schema/xmlschemakeyref/
---
## XmlSchemaKeyref کلاس

این کلاس عنصر **keyref** را از XMLSchema مطابق با کنسرسیوم جهانی [Web](../../system.web/) (W3C) نشان می‌دهد.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | شبیه‌سازی مقایسه نقطه شناور به سبک C# که در آن دو NaN برابر در نظر گرفته می‌شوند، حتی با این‌که بر اساس IEC 60559:1989 NaN برابر هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | شبیه‌سازی مقایسه نقطه شناور به سبک C# که در آن دو NaN برابر در نظر گرفته می‌شوند، حتی با این‌که بر اساس IEC 60559:1989 NaN برابر هیچ مقداری، از جمله NaN، نیست. |
...
| [SharedPtr](../../system/sharedptr/)\<[XmlSchemaObjectCollection](../xmlschemaobjectcollection/)\> [get_Fields](../xmlschemaidentityconstraint/get_fields/)() | مجموعه فیلدهایی که به‌عنوان فرزندان برای انتخابگر عبارات XML Path Language ([XPath](../../system.xml.xpath/)) اعمال می‌شوند را برمی‌گرداند. |
...
| [String](../../system/string/) [get_Name](../xmlschemaidentityconstraint/get_name/)() | نام محدودیت هویت را برمی‌گرداند. |
...
| [SharedPtr](../../system/sharedptr/)\<[XmlQualifiedName](../../system.xml/xmlqualifiedname/)\> [get_QualifiedName](../xmlschemaidentityconstraint/get_qualifiedname/)() | نام qualified محدودیت هویت را برمی‌گرداند که تفسیر پس از کامپایل مقدار **QualifiedName** را نگه می‌دارد. |
| [SharedPtr](../../system/sharedptr/)\<[XmlQualifiedName](../../system.xml/xmlqualifiedname/)\> [get_Refer](./get_refer/)() | نام کلیدی که این محدودیت به آن در نوع ساده یا پیچیده دیگری اشاره می‌کند را برمی‌گرداند. |
| [SharedPtr](../../system/sharedptr/)\<[XmlSchemaXPath](../xmlschemaxpath/)\> [get_Selector](../xmlschemaidentityconstraint/get_selector/)() | عنصر **selector** عبارت [XPath](../../system.xml.xpath/) را برمی‌گرداند. |
...
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | ساختار داده شمارنده ارجاع مرتبط با شیء را به‌دست می‌آورد. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری با عبارت C# lock() را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء نظارتی [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازنده کپی. در واقع هیچ چیز را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی برای زیردموی‌ها را فراهم می‌کند. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | اپراتور تخصیص. در واقع هیچ‌چیز را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی برای زیردموی‌ها را فراهم می‌کند. |
| static **bool** [ReferenceEquals](../../system/object/referenceequals/)([ptr](../../system/object/ptr/) const\&, [ptr](../../system/object/ptr/) const\&) | شیء‌ها را به‌صورت ارجاعی مقایسه می‌کند. |
| static std::enable_if<\![IsSmartPtr](../../system/issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../../system/object/referenceequals/)(T const\&, T const\&) | شیء‌ها را به‌صورت ارجاعی مقایسه می‌کند. |
...
| void [set_Name](../xmlschemaidentityconstraint/set_name/)(const [String](../../system/string/)\&) | نام محدودیت هویت را تنظیم می‌کند. |
...
| void [set_Refer](./set_refer/)(const [SharedPtr](../../system/sharedptr/)\<[XmlQualifiedName](../../system.xml/xmlqualifiedname/)\>\&) | نام کلیدی که این محدودیت به آن در نوع ساده یا پیچیده دیگری ارجاع دارد را تنظیم می‌کند. |
| void [set_Selector](../xmlschemaidentityconstraint/set_selector/)(const [SharedPtr](../../system/sharedptr/)\<[XmlSchemaXPath](../xmlschemaxpath/)\>\&) | عنصر **selector** عبارت [XPath](../../system.xml.xpath/) را تنظیم می‌کند. |
...
| int [SharedCount](../../system/object/sharedcount/)() const | مقدار فعلی شمارنده ارجاع مشترک را به‌دست می‌آورد. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارنده ارجاع مشترک را کاهش می‌دهد و برمی‌گرداند. نباید به‌صورت مستقیم فراخوانی شود؛ در عوض، از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گذاری با عبارت C# lock() را از حالت قفل خارج می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء نظارتی [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [XmlSchemaIdentityConstraint](../xmlschemaidentityconstraint/xmlschemaidentityconstraint/)() | یک نمونه جدید از کلاس [XmlSchemaIdentityConstraint](../xmlschemaidentityconstraint/) را مقداردهی اولیه می‌کند. |
|  [XmlSchemaKeyref](./xmlschemakeyref/)() | یک نمونه جدید از کلاس [XmlSchemaKeyref](./) را مقداردهی اولیه می‌کند. |
...
شیء‌های این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص یابند. هرگز نمونه‌های این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا اشکالات ادعا می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای گذراندن به توابع به‌عنوان آرگومان استفاده کنید.

## موارد مرتبط

* کلاس [XmlSchemaIdentityConstraint](../xmlschemaidentityconstraint/)
