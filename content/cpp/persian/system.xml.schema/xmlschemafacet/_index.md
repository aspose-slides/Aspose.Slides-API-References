...
description: یک کلاس پایه برای تمام فاست‌ها که هنگام محدود کردن انواع ساده استفاده می‌شوند.
type: docs
weight: 404
url: /fa/system.xml.schema/xmlschemafacet/
---
## XmlSchemaFacet کلاس

یک کلاس پایه برای تمام فاست‌ها که هنگام محدود کردن انواع ساده استفاده می‌شوند.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسهٔ نقطه شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN به عنوان برابر در نظر گرفته می‌شوند، حتی اگر طبق IEC 60559:1989 NaN برابر هیچ مقداری نیست، از جمله NaN. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسهٔ نقطه شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN به عنوان برابر در نظر گرفته می‌شوند، حتی اگر طبق IEC 60559:1989 NaN برابر هیچ مقداری نیست، از جمله NaN. |
...
| virtual **bool** [get_IsFixed](./get_isfixed/)() | اطلاعاتی را برمی‌گرداند که نشان می‌دهد این فاست ثابت است. |
...
| [String](../../system/string/) [get_SourceUri](../xmlschemaobject/get_sourceuri/)() | موقعیت منبع برای فایلی که طرح‌واره را بارگذاری کرده است را برمی‌گرداند. |
| [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[XmlAttribute](../../system.xml/xmlattribute/)\>\> [get_UnhandledAttributes](../xmlschemaannotated/get_unhandledattributes/)() | صفات مشروطی که به فضای‌نام هدف طرح‌وارهٔ فعلی تعلق ندارند را برمی‌گرداند. |
| [String](../../system/string/) [get_Value](./get_value/)() | صفت **value** فاست را برمی‌گرداند. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری توسط عبارت lock() در C# را پیاده‌سازی می‌کند. مستقیماً صدا بزنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازندهٔ کپی. در واقع هیچ‌چیزی کپی نمی‌کند، فقط شیء جدید را مقداردهی می‌کند و امکان ساخت کپی از زیرکلاس‌ها را می‌دهد. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر انتساب. در واقع هیچ‌چیزی کپی نمی‌کند، فقط شیء جدید را مقداردهی می‌کند و امکان ساخت کپی از زیرکلاس‌ها را می‌دهد. |
...
| virtual void [set_IsFixed](./set_isfixed/)(**bool**) | اطلاعاتی را تنظیم می‌کند که نشان می‌دهد این فاست ثابت است. |
...
| void [set_UnhandledAttributes](../xmlschemaannotated/set_unhandledattributes/)(const [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[XmlAttribute](../../system.xml/xmlattribute/)\>\>\&) | صفات مشروطی که به فضای‌نام هدف طرح‌وارهٔ فعلی تعلق ندارند را تنظیم می‌کند. |
| void [set_Value](./set_value/)(const [String](../../system/string/)\&) | صفت **value** فاست را تنظیم می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌برداری توسط عبارت lock() در C# را پیاده‌سازی می‌کند. مستقیماً صدا بزنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [XmlSchemaFacet](./xmlschemafacet/)() | نمونهٔ جدیدی از کلاس [XmlSchemaFacet](./) را مقداردهی اولیه می‌کند. |
...
| تعریف | توضیح |
| --- | --- |
| [Ptr](./ptr/) | نام مستعاری برای اشاره‌گر مشترک به یک نمونه از این کلاس است. |

## نکات

اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌های این نوع را بر روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا اشکالات ادعا می‌شود. همیشه این کلاس را درون اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای عبور به توابع به عنوان آرگومان استفاده کنید. 
