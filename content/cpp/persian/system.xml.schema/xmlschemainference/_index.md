...
description: یک طرح‌واره XML Schema Definition Language (XSD) را از یک سند XML استنتاج می‌کند. کلاس XmlSchemaInference قابل ارث‌برداری نیست.
type: docs
weight: 508
url: /fa/system.xml.schema/xmlschemainference/
---
## XmlSchemaInference کلاس

از یک سند XML، یک طرح‌واره XML [Schema](../) Definition Language (XSD) را استنتاج می‌کند. کلاس [XmlSchemaInference](./) نمی‌تواند ارث‌برداری شود.
...
| [XmlSchemaInference::InferenceOption](./inferenceoption/) [get_Occurrence](./get_occurrence/)() | مقدار [XmlSchemaInference::InferenceOption](./inferenceoption/) را که بر اعلان‌های رخداد طرح‌واره استنتاج شده از سند XML تأثیر می‌گذارد، برمی‌گرداند. |
| [XmlSchemaInference::InferenceOption](./inferenceoption/) [get_TypeInference](./get_typeinference/)() | مقدار [XmlSchemaInference::InferenceOption](./inferenceoption/) را که بر انواع استنتاج شده از سند XML تأثیر می‌گذارد، برمی‌گرداند. |
...
| [SharedPtr](../../system/sharedptr/)\<[XmlSchemaSet](../xmlschemaset/)\> [InferSchema](./inferschema/)(const [SharedPtr](../../system/sharedptr/)\<[XmlReader](../../system.xml/xmlreader/)\>\&) | یک طرح‌واره XML [Schema](../) Definition Language (XSD) را از سند XML موجود در شیء [XmlReader](../../system.xml/xmlreader/) مشخص‌شده استنتاج می‌کند. |
| [SharedPtr](../../system/sharedptr/)\<[XmlSchemaSet](../xmlschemaset/)\> [InferSchema](./inferschema/)(const [SharedPtr](../../system/sharedptr/)\<[XmlReader](../../system.xml/xmlreader/)\>\&, [SharedPtr](../../system/sharedptr/)\<[XmlSchemaSet](../xmlschemaset/)\>) | یک طرح‌واره XML [Schema](../) Definition Language (XSD) را از سند XML موجود در شیء [XmlReader](../../system.xml/xmlreader/) مشخص‌شده استنتاج می‌کند، و طرح‌وارهٔ استنتاج‌شده را با استفاده از یک طرح‌وارهٔ موجود در شیء [XmlSchemaSet](../xmlschemaset/) که فضای‌نام هدف یکسان دارد، تصحیح می‌کند. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری دستور lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| void [set_Occurrence](./set_occurrence/)([XmlSchemaInference::InferenceOption](./inferenceoption/)) | مقدار [XmlSchemaInference::InferenceOption](./inferenceoption/) را که بر اعلان‌های رخداد طرح‌واره استنتاج‌شده از سند XML تأثیر می‌گذارد، تنظیم می‌کند. |
| void [set_TypeInference](./set_typeinference/)([XmlSchemaInference::InferenceOption](./inferenceoption/)) | مقدار [XmlSchemaInference::InferenceOption](./inferenceoption/) را که بر انواع استنتاج‌شده از سند XML تأثیر می‌گذارد، تنظیم می‌کند. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارندهٔ مرجع مشترک را کاهش داده و بر می‌گرداند. نباید مستقیماً فراخوانی شود؛ در عوض، از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌برداری دستور lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [XmlSchemaInference](./xmlschemainference/)() | یک نمونهٔ جدید از کلاس [XmlSchemaInference](./) را مقداردهی اولیه می‌کند. |
...
| Enum | توضیح |
| --- | --- |
| [InferenceOption](./inferenceoption/) | بر اطلاعات رخداد و نوع که توسط کلاس [XmlSchemaInference](./) برای عناصر و ویژگی‌های یک سند XML استنتاج می‌شود، تأثیر می‌گذارد. |
...
شیءهای این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌های این نوع را بر روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا اشکالات ادعا می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای عبور به توابع به‌عنوان آرگومان استفاده کنید.
