...
description: "نمایش‌دهندهٔ خواننده‌ای است که کاراکترها را از یک رشته می‌خواند. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای اعتبارسنجی می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای عبور به‌عنوان آرگومان به توابع استفاده کنید."
type: docs
weight: 404
url: /fa/system.io/stringreader/
---
## کلاس StringReader

نمایش‌دهندهٔ خواننده‌ای است که کاراکترها را از یک رشته می‌خواند. اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای اعتبارسنجی می‌شود. همواره این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای پاس دادن به توابع به عنوان آرگومان استفاده کنید.
...
| void [Dispose](./dispose/)(**bool**) override | کاری انجام نمی‌دهد. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسه نقطه شناور سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند حتی اگر بر اساس IEC 60559:1989 NaN برابر با هیچ مقداری نیست، از جمله NaN. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه نقطه شناور سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند حتی اگر بر اساس IEC 60559:1989 NaN برابر با هیچ مقداری نیست، از جمله NaN. |
...
| void [Lock](../../system/object/lock/)() | اجرای قفل‌گذاری بیان lock() در C#. به‌صورت مستقیم فراخوانی شود یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| int [Peek](./peek/)() override | یک کاراکتر واحد را از جریان می‌خواند بدون اینکه موقعیت جریان را تغییر دهد. |
| int [Read](./read/)() override | یک کاراکتر واحد را از جریان می‌خواند. |
| int [Read](./read/)([ArrayPtr](../../system/arrayptr/)\<char_t\>, int, int) override | تعداد مشخصی کاراکتر را از جریان به آرایه کاراکتری مشخص خوانده و در موقعیت مشخص‌شده آغاز می‌کند. |
| virtual int [ReadBlock](../textreader/readblock/)([ArrayPtr](../../system/arrayptr/)\<char_t\>, int, int) | حداکثر تعداد مشخصی کاراکتر را از خوانندهٔ متن فعلی می‌خواند و داده‌ها را در یک بافر می‌نویسد، شروع از اندیس مشخص‌شده. |
| [String](../../system/string/) [ReadLine](./readline/)() override | کاراکترها را از جریان می‌خواند تا پایان خط فعلی. |
| [String](../../system/string/) [ReadToEnd](./readtoend/)() override | کاراکترها را از جریان می‌خواند تا پایان جریان. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان نهم قالب را به یک اشاره‌گر ضعیف (به جای مشترک) تنظیم می‌کند. امکان تغییر اشاره‌گرها در محفظه‌ها به حالت ضعیف را فراهم می‌کند. |
...
|  [StringReader](./stringreader/)(const [String](../../system/string/)\&) | یک نمونه جدید از کلاس [StringReader](./) که کاراکترها را از رشتهٔ مشخص‌شده می‌خواند، می‌سازد. |
| virtual [String](../../system/string/) [ToString](../../system/object/tostring/)() const | معادل متد C# [Object.ToString()](../../system/object/tostring/). امکان تبدیل اشیاء سفارشی به رشته را فراهم می‌کند. |
| static const [TypeInfo](../../system/typeinfo/)\& [Type](../../system/object/type/)() | ساختار typeof([System.Object](../../system/object/)) در C# را پیاده‌سازی می‌کند. |
| void [Unlock](../../system/object/unlock/)() | اجرای باز کردن قفل بیان lock() در C#. به‌صورت مستقیم فراخوانی شود یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
