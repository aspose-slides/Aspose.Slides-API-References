...
description: یک رابط فراهم می‌کند تا یک کلاس بتواند اطلاعات خط و موقعیت را برگرداند.
type: docs
weight: 40
url: /fa/system.xml/ixmllineinfo/
---
## IXmlLineInfo کلاس

یک رابط فراهم می‌کند تا یک کلاس بتواند اطلاعات خط و موقعیت را برگرداند.
...
| virtual **int32_t** [get_LineNumber](./get_linenumber/)() | شماره خط فعلی را برمی‌گرداند. |
| virtual **int32_t** [get_LinePosition](./get_lineposition/)() | موقعیت خط فعلی را برمی‌گرداند. |
...
| virtual **bool** [HasLineInfo](./haslineinfo/)() | مقداری را برمی‌گرداند که نشان می‌دهد آیا کلاس می‌تواند اطلاعات خط را برگرداند یا نه. |
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | بررسی می‌کند که آیا شی نمونه‌ای از نوعی است که توسط targetType توصیف شده است. معادل عملگر C# 'is'. |
| void [Lock](../../system/object/lock/)() | اجرای قفل کردن عبارت C# lock() را پیاده‌سازی می‌کند. به‌طور مستقیم فراخوانی کنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| static std::enable_if<\![IsSmartPtr](../../system/issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../../system/object/referenceequals/)(T const\&, std::nullptr_t) | شی از نوع مقدار را با nullptr به‌صورت مرجع مقایسه می‌کند. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان nام قالب را به یک اشاره‌گر ضعیف تنظیم می‌کند (به‌جای مشترک). امکان تغییر اشاره‌گرها در محفظه‌ها به حالت ضعیف را فراهم می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | اجرای رفع قفل عبارت C# lock() را پیاده‌سازی می‌کند. به‌طور مستقیم فراخوانی کنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
