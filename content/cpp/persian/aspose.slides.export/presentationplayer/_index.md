...
description: پخش‌کنندهٔ انیمیشن‌های مرتبط با ارائه را نمایندگی می‌کند.
type: docs
weight: 625
...
نمایش‌کنندهٔ پخش انیمیشن‌های مرتبط با [Presentation](../../aspose.slides/presentation/) را نمایندگی می‌کند.
...
| متد | توضیح |
| --- | --- |
| void [Dispose](./dispose/)() override | نمونهٔ [PresentationPlayer](./) را آزاد می‌کند. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسه نقطه شناور شبیه به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، اگرچه طبق IEC 60559:1989 NaN برابر هیچ مقداری نیست، از جمله NaN. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه نقطه شناور شبیه به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، اگرچه طبق IEC 60559:1989 NaN برابر هیچ مقداری نیست، از جمله NaN. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای موارد داخلی. |
| **int32_t** [get_FrameIndex](./get_frameindex/)() | شاخص فریم را دریافت می‌کند. |
...
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | بررسی می‌کند که آیا شی یک نمونه از نوعی که توسط targetType توصیف شده است یا خیر. معادل عملگر 'is' در C#. |
| void [Lock](../../system/object/lock/)() | قفل‌گذاری بیانیه lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء حفاظت‌کننده [LockContext](../../system/lockcontext/) استفاده نمایید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازندهٔ کپی. در واقع هیچ چیزی را کپی نمی‌کند، فقط شی جدید را مقداردهی اولیه می‌کند و امکان ساخت کلاس‌های فرزند از طریق کپی را فراهم می‌آورد. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | اپراتور انتساب. در واقع هیچ چیزی را کپی نمی‌کند، فقط شی جدید را مقداردهی اولیه می‌کند و امکان ساخت کلاس‌های فرزند از طریق کپی را فراهم می‌آورد. |
|  [PresentationPlayer](./presentationplayer/)([System::SharedPtr](../../system/sharedptr/)\<[PresentationAnimationsGenerator](../presentationanimationsgenerator/)\>, **double**) | نمونهٔ جدیدی از [PresentationPlayer](./) ایجاد می‌کند. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارندهٔ ارجاع مشترک را کاهش می‌دهد و مقدار آن را باز می‌گرداند. نباید به‌صورت مستقیم فراخوانی شود؛ به‌جای آن، از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | باز کردن قفل بیانیه lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء حفاظت‌کننده [LockContext](../../system/lockcontext/) استفاده نمایید. |
...
| virtual  [~Object](../../system/object/~object/)() | شی را حذف می‌کند. تمام ساختارهای داده داخلی را آزاد می‌سازد. |
## تعاریف نوع

| تعریف نوع | توضیح |
| --- | --- |
| [FrameTickHandler](./frametickhandler/) | نمایندهٔ دستگیرهٔ تیک فریم رویداد **PresentationPlayer::FrameTick** است. |
## ملاحظات
