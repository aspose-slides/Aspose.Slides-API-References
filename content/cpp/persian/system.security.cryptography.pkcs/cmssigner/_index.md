...
description: "APIی برای امضای اشیا با استفاده از CMS را فراهم می‌کند. خود اشیا را امضا نمی‌کند؛ برای این کار از کلاس SignedCMS استفاده کنید. پیاده‌سازی نشده است. اشیای این کلاس باید فقط با تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با عملگر new ایجاد نکنید، زیرا به خطاهای زمان اجرا و/یا شکست‌های ادعا منجر می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای ارسال به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 1
url: /fa/system.security.cryptography.pkcs/cmssigner/
---
## CmsSigner کلاس

API برای امضای اشیا با استفاده از CMS را فراهم می‌کند. خود اشیا را امضا نمی‌کند؛ برای این کار از کلاس SignedCMS استفاده کنید. پیاده‌سازی نشده است. اشیا از این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا شکست‌های ادعا می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای پاس دادن به توابع به عنوان آرگومان استفاده کنید.
...
| متد | توضیح |
| --- | --- |
|  [CmsSigner](./cmssigner/)(const [SharedPtr](../../system/sharedptr/)\<[X509Certificates::X509Certificate2](../../system.security.cryptography.x509certificates/x509certificate2/)\>\&) | امضاکننده را با گواهی X509 مقداردهی اولیه می‌کند. |
| virtual **bool** [Equals](../../system/object/equals/)([ptr](../../system/object/ptr/)) | اشیا را با استفاده از معناشناسی [Object.Equals](../../system/object/equals/) زبان C# مقایسه می‌کند. |
...
| [SharedPtr](../../system/sharedptr/)\<[Oid](../../system.security.cryptography/oid/)\> [get_DigestAlgorithm](./get_digestalgorithm/)() const | الگوریتم هش مورد استفاده با امضا را دریافت می‌کند. |
| [X509Certificates::X509IncludeOption](../../system.security.cryptography.x509certificates/x509includeoption/) [get_IncludeOption](./get_includeoption/)() const | بررسی می‌کند کدام گواهی‌ها از زنجیره در امضا گنجانده می‌شوند. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری با بیان lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر تخصیص. در واقع هیچ چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی در زیرکلاس‌ها را فراهم می‌کند. |
...
| void [set_DigestAlgorithm](./set_digestalgorithm/)(const [SharedPtr](../../system/sharedptr/)\<[Oid](../../system.security.cryptography/oid/)\>\&) | الگوریتم هش مورد استفاده با امضا را تنظیم می‌کند. |
| void [set_IncludeOption](./set_includeoption/)([X509Certificates::X509IncludeOption](../../system.security.cryptography.x509certificates/x509includeoption/)) | مشخص می‌کند کدام گواهی‌ها از زنجیره در امضا گنجانده شوند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گذاری با بیان lock() در C# را باز می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
