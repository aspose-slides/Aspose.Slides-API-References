...
description: یک رابط پایه برای همه هشدارها را نشان می‌دهد.
type: docs
weight: 66
...
نمایانگر یک رابط پایه برای همه هشدارها.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسهٔ نقطه شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، حتی اگر بر طبق IEC 60559:1989 NaN برابر هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسهٔ نقطه شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، حتی اگر بر طبق IEC 60559:1989 NaN برابر هیچ مقداری، از جمله NaN، نیست. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای مقاصد داخلی. |
| virtual [System::String](../../system/string/) [get_Description](./get_description/)() | توضیحی قابل خواندن برای انسان از این هشدار را برمی‌گرداند. فقط خواندنی [System::String](../../system/string/). |
| virtual [Aspose::Slides::Warnings::WarningType](../warningtype/) [get_WarningType](./get_warningtype/)() | نوعی از هشدار را برمی‌گرداند. فقط خواندنی [IWarningInfo::get_WarningType](./get_warningtype/). |
...
| void [Lock](../../system/object/lock/)() | بیان قفل‌گذاری دستور C# lock() را پیاده‌سازی می‌کند. مستقیم فراخوانی کنید یا از شیء محافظ [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual void [SendWarning](./sendwarning/)([System::SharedPtr](../../system/sharedptr/)\<[IWarningCallback](../iwarningcallback/)\>) | اگر دریافت‌کننده صفر نباشد، هشدار را به دریافت‌کنندهٔ مشخص شده ارسال می‌کند و در صورتی که دریافت‌کننده تصمیم به لغو عملیات بگیرد، استثنای AbortRequestedException را پرتاب می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | بیان بازقفل‌گذاری دستور C# lock() را پیاده‌سازی می‌کند. مستقیم فراخوانی کنید یا از شیء محافظ [LockContext](../../system/lockcontext/) استفاده کنید. |
