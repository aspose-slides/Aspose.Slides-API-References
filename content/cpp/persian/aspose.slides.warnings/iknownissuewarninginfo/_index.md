...
description: هشداری دربارهٔ مشکلی شناخته‌شده که در آینده نزدیک رفع نخواهد شد.
type: docs
weight: 1
url: /fa/aspose.slides.warnings/iknownissuewarninginfo/
---
## IKnownIssueWarningInfo کلاس

هشداری در مورد مشکلی شناخته‌شده که در آینده نزدیک برطرف نخواهد شد را نشان می‌دهد.
...
| virtual [System::String](../../system/string/) [get_Description](../iwarninginfo/get_description/)() | توضیح خوانا برای انسان از این هشدار را برمی‌گرداند. فقط‌خواندنی [System::String](../../system/string/). |
| virtual [Aspose::Slides::Warnings::WarningType](../warningtype/) [get_WarningType](../iwarninginfo/get_warningtype/)() | نوع هشدار را برمی‌گرداند. فقط‌خواندنی [IWarningInfo::get_WarningType](../iwarninginfo/get_warningtype/). |
...
| void [Lock](../../system/object/lock/)() | قفل کردن با استفاده از عبارت lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازندهٔ کپی. در واقع چیزی کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی برای زیرکلاس‌ها را فراهم می‌سازد. |
...
| static std::enable_if<\![IsSmartPtr](../../system/issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../../system/object/referenceequals/)(T const\&, std::nullptr_t) | مقایسهٔ مرجع برای شیء از نوع مقداری با nullptr انجام می‌دهد. |
...
| virtual void [SendWarning](../iwarninginfo/sendwarning/)([System::SharedPtr](../../system/sharedptr/)\<[IWarningCallback](../iwarningcallback/)\>) | اگر گیرنده null نباشد، هشدار را به گیرندهٔ مشخص‌شده خاتمه می‌دهد و در صورت تصمیم گیرنده برای قطع عملیات، استثنای AbortRequestedException را پرتاب می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌برداری با استفاده از عبارت lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
| Detail::SmartPtrCounter * [WeakRefAdded](../../system/object/weakrefadded/)() | شمارندهٔ مرجع ضعیف را افزایش‌دهد. نباید به‌صورت مستقیم فراخوانی شود؛ به‌جای آن از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
