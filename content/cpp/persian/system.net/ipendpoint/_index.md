...
description: "نمایشگر یک نقطه انتهایی شبکه است که شامل یک آدرس IP و یک پورت می‌شود. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را بر روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعا می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای ارسال به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 339
url: /fa/system.net/ipendpoint/
---
## IPEndPoint کلاس


نمایشگر یک نقطه انتهایی شبکه است که شامل یک آدرس IP و یک پورت می‌شود. اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعا می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای ارسال به توابع به عنوان آرگومان استفاده کنید.
...
| متد | توضیح |
| --- | --- |
| [System::SharedPtr](../../system/sharedptr/)\<[EndPoint](../endpoint/)\> [Create](./create/)([System::SharedPtr](../../system/sharedptr/)\<[SocketAddress](../socketaddress/)\>) override | یک نمونه جدید از کلاس [EndPoint](../endpoint/) را با استفاده از آدرس سوکت مشخص ایجاد می‌کند. |
...
| [System::SharedPtr](../../system/sharedptr/)\<[IPAddress](../ipaddress/)\> [get_Address](./get_address/)() | آدرس نقطه انتهایی را دریافت می‌کند. |
| [System::Net::Sockets::AddressFamily](../../system.net.sockets/addressfamily/) [get_AddressFamily](./get_addressfamily/)() override | خانواده آدرس را که نمونه فعلی به آن تعلق دارد برمی‌گرداند. |
| **int32_t** [get_Port](./get_port/)() | شماره پورت را دریافت می‌کند. |
...
| [ImplPtr](../endpoint/implptr/) [GetImpl](./getimpl/)() const override | یک اشاره‌گر به پیاده‌سازی را برمی‌گرداند. |
| virtual const [TypeInfo](../../system/typeinfo/)\& [GetType](../../system/object/gettype/)() const | نوع واقعی شیء را دریافت می‌کند. معادل فراخوانی C# [System.Object.GetType()](../../system/object/gettype/). |
|  [IPEndPoint](./ipendpoint/)(**int64_t**, **int32_t**) | یک نمونه جدید می‌سازد. |
|  [IPEndPoint](./ipendpoint/)([System::SharedPtr](../../system/sharedptr/)\<[IPAddress](../ipaddress/)\>, **int32_t**) | یک نمونه جدید می‌سازد. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری عملگر lock() در C# را پیاده‌سازی می‌کند. مستقیماً فراخوانی کنید یا از شیء محافظ [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| void [set_Address](./set_address/)([System::SharedPtr](../../system/sharedptr/)\<[IPAddress](../ipaddress/)\>) | آدرس نقطه انتهایی را تنظیم می‌کند. |
| void [set_Port](./set_port/)(**int32_t**) | شماره پورت را تنظیم می‌کند. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارنده مرجع مشترک را کاهش داده و برمی‌گرداند. نباید مستقیماً صدا زده شود؛ در عوض، از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گذاری معکوس عملگر lock() در C# را پیاده‌سازی می‌کند. مستقیماً فراخوانی کنید یا از شیء محافظ [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| فیلد | توضیح |
| --- | --- |
| static [Any](./any/) | نقطه انتهایی برای هر آدرس IPv4 و هر پورت. |
| static [AnyPort](./anyport/) | مقداری که نشان می‌دهد آیا هر پورت می‌تواند استفاده شود. |
| static [IPv6Any](./ipv6any/) | نقطه انتهایی برای هر آدرس IPv6 و هر پورت. |
| static [MaxPort](./maxport/) | حداکثر شماره پورت. |
| static [MinPort](./minport/) | حداقل شماره پورت. |
