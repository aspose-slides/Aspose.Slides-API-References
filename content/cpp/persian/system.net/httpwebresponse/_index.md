...
description: "نمایانگر پاسخ وب HTTP است. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از operator new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های اعتبارسنجی می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای ارسال به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 287
url: /fa/system.net/httpwebresponse/
---
## HttpWebResponse کلاس

نمایانگر پاسخ وب HTTP است. اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از operator new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های اعتبارسنجی می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای ارسال به توابع به عنوان آرگومان استفاده کنید.
...
| [System::String](../../system/string/) [get_CharacterSet](./get_characterset/)() | پیاده‌سازی نشده. |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[CookieCollection](../cookiecollection/)\> [get_Cookies](./get_cookies/)() | کلوچک‌های مرتبط با پاسخ وب را برمی‌گرداند. |
| [System::SharedPtr](../../system/sharedptr/)\<[WebHeaderCollection](../webheadercollection/)\> [get_Headers](./get_headers/)() override | مجموعهٔ سرصفحه‌هایی که با پاسخ جاری مرتبط هستند را برمی‌گرداند. |
| virtual [String](../../system/string/) [get_Method](./get_method/)() | متد HTTP را برمی‌گرداند. |
| [System::SharedPtr](../../system/sharedptr/)\<[Uri](../../system/uri/)\> [get_ResponseUri](./get_responseuri/)() override | URI منبع را برمی‌گرداند. |
| virtual [HttpStatusCode](../httpstatuscode/) [get_StatusCode](./get_statuscode/)() | کد وضعیت HTTP مرتبط با پاسخ وب را برمی‌گرداند. |
| virtual [String](../../system/string/) [get_StatusDescription](./get_statusdescription/)() | نمایش رشته‌ای کد وضعیت را برمی‌گرداند. |
| **bool** [get_SupportsHeaders](./get_supportsheaders/)() override | یک مقدار که نشان می‌دهد آیا پاسخ جاری از سرصفحه‌ها پشتیبانی می‌کند را برمی‌گرداند. |
...
| [String](../../system/string/) [GetResponseHeader](./getresponseheader/)([String](../../system/string/)) | مقدار متناظر برای نام سرصفحهٔ مشخص‌شده را برمی‌گرداند. |
...
|  [HttpWebResponse](./httpwebresponse/)([System::SharedPtr](../../system/sharedptr/)\<[Http::HttpResponseMessage](../../system.net.http/httpresponsemessage/)\>, [System::SharedPtr](../../system/sharedptr/)\<[Uri](../../system/uri/)\>, [System::SharedPtr](../../system/sharedptr/)\<[CookieContainer](../cookiecontainer/)\>) | یک نمونهٔ جدید می‌سازد. |
...
| int [RemovedSharedRefs](../../system/object/removedsharedrefs/)(int) | شمارندهٔ مرجع اشتراکی را به میزان مقدار مشخص‌شده کاهش می‌دهد. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان nام قالب را به یک اشاره‌گر ضعیف (به‌جای shared) تنظیم می‌کند. اجازه می‌دهد اشاره‌گرها در محفظه‌ها به حالت ضعیف تغییر کنند. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارندهٔ مرجع اشتراکی را کاهش می‌دهد و بازمی‌گرداند. نباید به‌صورت مستقیم فراخوانی شود؛ به‌جای آن، از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
