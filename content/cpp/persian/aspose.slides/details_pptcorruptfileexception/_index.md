...
description: استثنائی که هنگامیکه فایل ارائه احتمالاً خراب باشد پرتاب می‌شود.
type: docs
weight: 651
url: /fa/aspose.slides/details_pptcorruptfileexception/
---
## Details_PptCorruptFileException کلاس

استثنائی که هنگامیکه فایل ارائه احتمالاً خراب باشد پرتاب می‌شود.
...
| **int32_t** [get_HResult](../../system/details_exception/get_hresult/)() const | مقدار عدد صحیح ۳۲ بیتی که کد HRESULT مرتبط با استثنای نمایان‌شده توسط شیء جاری است را برمی‌گرداند. |
...
| virtual [String](../../system/string/) [get_Message](../../system/details_exception/get_message/)() const | رشته‌ای را برمی‌گرداند که شامل توضیح خطا است. |
| virtual [String](../../system/string/) [get_StackTrace](../../system/details_exception/get_stacktrace/)() const | رشته‌ای را برمی‌گرداند که شامل رد‌پیشینهٔ پشته است. |
...
| void [Lock](../../system/object/lock/)() | اجرای قفل‌گذاری با دستور lock() C#. مستقیماً صدا بزنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازندهٔ کپی. در واقع هیچ چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی برای کلاس‌های فرعی را فراهم می‌سازد. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر انتساب. در واقع هیچ چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی برای کلاس‌های فرعی را فراهم می‌کند. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان nام الگوی قالب را به یک اشاره‌گر ضعیف (به جای اشتراکی) تنظیم می‌کند. امکان تغییر حالت اشاره‌گرها در کانتینرها به حالت ضعیف را فراهم می‌سازد. |
...
| void [Unlock](../../system/object/unlock/)() | اجرای آزادسازی قفل با دستور lock() C#. مستقیماً صدا بزنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual const char * [what](../../system/details_exception/what/)() const | پیاده‌سازی متد [what()](../../system/details_exception/what/) که توسط کلاس [ExceptionWrapper](../../system/exceptionwrapper/) فراخوانی می‌شود. علیرغم اینکه این کلاس از std::exception ارث‌بری نمی‌کند، کلاس‌های مشتق‌شده می‌توانند از اعضای protected/private برای پیاده‌سازی منطق خود استفاده کنند. انتقال این پیاده‌سازی به [ExceptionWrapper](../../system/exceptionwrapper/) ممکن است آن منطق را بشکند. |
