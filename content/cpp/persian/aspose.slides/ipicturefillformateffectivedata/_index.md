...
description: شیء تغییرناپذیری که خصوصیات پر کردن تصویر را شامل می‌شود.
type: docs
weight: 3238
url: /fa/aspose.slides/ipicturefillformateffectivedata/
---
## IPictureFillFormatEffectiveData کلاس

شیء تغییرناپذیری که خصوصیات پر کردن تصویر را شامل می‌شود.
...
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه عدد شناور دو برابر (double) به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند، حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نباشد. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای استفاده داخلی. |
| virtual **float** [get_CropBottom](./get_cropbottom/)() | تعداد درصدهای ارتفاع واقعی تصویر که از پایین تصویر برش خورده است را بر می‌گرداند. فقط خواندنی **float**. |
| virtual **float** [get_CropLeft](./get_cropleft/)() | تعداد درصدهای عرض واقعی تصویر که از سمت چپ تصویر برش خورده است را بر می‌گرداند. فقط خواندنی **float**. |
| virtual **float** [get_CropRight](./get_cropright/)() | تعداد درصدهای عرض واقعی تصویر که از سمت راست تصویر برش خورده است را بر می‌گرداند. فقط خواندنی **float**. |
| virtual **float** [get_CropTop](./get_croptop/)() | تعداد درصدهای ارتفاع واقعی تصویر که از بالای تصویر برش خورده است را بر می‌گرداند. فقط خواندنی **float**. |
| virtual **int32_t** [get_Dpi](./get_dpi/)() | dpi که برای پر کردن تصویر استفاده می‌شود را بر می‌گرداند. فقط خواندنی **int32_t**. |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IPictureEffectiveData](../ipictureeffectivedata/)\> [get_Picture](./get_picture/)() | تصویر را بر می‌گرداند. فقط خواندنی [IPictureEffectiveData](../ipictureeffectivedata/). |
| virtual [Aspose::Slides::PictureFillMode](../picturefillmode/) [get_PictureFillMode](./get_picturefillmode/)() | حالت پر کردن تصویر را بر می‌گرداند. فقط خواندنی [Slides::PictureFillMode](../picturefillmode/). |
...
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر تخصیص. در واقع چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی از زیرکلاس‌ها را می‌دهد. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان قالب nام را به یک اشاره‌گر ضعیف (به‌جای مشترک) تنظیم می‌کند. امکان سوئیچ کردن اشاره‌گرها در محفظه‌ها به حالت ضعیف را می‌دهد. |
...
| void [Unlock](../../system/object/unlock/)() | رفع قفل دستور lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
