...
مقدار تنظیم یک شکل هندسی را نشان می‌دهد. این مقادیر بر فرم شکل تأثیر می‌گذارند.
...
| **float** [get_AngleValue](./get_anglevalue/)() override | مقدار را باز می‌گرداند و به‌عنوان زاویه‌ای در درجه تفسیر می‌کند. خواند **float**. |
| [System::String](../../system/string/) [get_Name](./get_name/)() override | نام این مقدار تنظیم را برمی‌گرداند. فقط خواندنی [System::String](../../system/string/). |
| **int64_t** [get_RawValue](./get_rawvalue/)() override | مقدار تنظیم را «همان‌طور» برمی‌گرداند. خواند **int64_t**. |
| [ShapeAdjustmentType](../shapeadjustmenttype/) [get_Type](./get_type/)() override | نوع تنظیم شکل را برمی‌گرداند. فقط خواندنی [ShapeAdjustmentType](../shapeadjustmenttype/). |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری عبارت C# lock() را پیاده‌سازی می‌کند. مستقیماً فراخوانی کنید یا از شیء نگهبانی [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| void [set_AngleValue](./set_anglevalue/)(**float**) override | مقدار را تنظیم می‌کند و به‌عنوان زاویه‌ای در درجه تفسیر می‌کند. نوشت **float**. |
| void [set_RawValue](./set_rawvalue/)(**int64_t**) override | مقدار تنظیم را «همان‌طور» تنظیم می‌کند. نوشت **int64_t**. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان قالب nام را به یک اشاره‌گر ضعیف تنظیم می‌کند (به‌جای مشترک). امکان تبدیل اشاره‌گرها در مخازن به حالت ضعیف را فراهم می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گذاری معکوس عبارت C# lock() را پیاده‌سازی می‌کند. مستقیماً فراخوانی کنید یا از شیء نگهبانی [LockContext](../../system/lockcontext/) استفاده کنید. |
...
* کلاس [IAdjustValue](../iadjustvalue/)
