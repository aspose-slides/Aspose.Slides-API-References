...
description: شیء غیرقابل تغییر که یک نقطه‌ی گرادیان را نمایندگی می‌کند.
type: docs
weight: 2445
url: /fa/aspose.slides/igradientstopeffectivedata/
---
## IGradientStopEffectiveData کلاس

شیء غیرقابل تغییر که یک نقطه‌ی گرادیان را نمایندگی می‌کند.
...
| virtual [System::Drawing::Color](../../system.drawing/color/) [get_Color](./get_color/)() | رنگ یک نقطه‌ی گرادیان را برمی‌گرداند. فقط-خواندنی [System::Drawing::Color](../../system.drawing/color/). |
| virtual **float** [get_Position](./get_position/)() | موقعیت (0..1) یک نقطه‌ی گرادیان را برمی‌گرداند. فقط-خواندنی **float**. |
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | ساختار داده‌ی شمارش‌گر مرجع مرتبط با شیء را دریافت می‌کند. |
...
| void [Lock](../../system/object/lock/)() | اجرای قفل کردن با استفاده از دستور lock() C# را پیاده‌سازی می‌کند. مستقیم صدا بزنید یا از شیء محافظ [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان nام الگوی template را به یک اشاره‌گر ضعیف (به جای مشترک) تنظیم می‌کند. امکان تغییر اشاره‌گرها در کانتینرها به حالت ضعیف را فراهم می‌آورد. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارنده‌ی مرجع مشترک را کاهش می‌دهد و برمی‌گرداند. نباید مستقیماً فراخوانی شود؛ در عوض، از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | اجرای بازکردن قفل با استفاده از دستور lock() C# را پیاده‌سازی می‌کند. مستقیم صدا بزنید یا از شیء محافظ [LockContext](../../system/lockcontext/) استفاده کنید. |
