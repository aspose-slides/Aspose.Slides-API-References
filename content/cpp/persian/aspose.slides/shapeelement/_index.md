...
description: نمایانگر بخشی از شکل با همان ویژگی‌های خطوط مرزی و پر کردن.
type: docs
weight: 5123
...
نمایانگر بخشی از شکل با همان ویژگی‌های پیرامون و پر کردن.

```cpp
class ShapeElement : public Aspose::Slides::IShapeElement
```
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسه نقطه شناور به سبک C# را شبیه‌سازی می‌کند به‌طوری که دو NaN برابر در نظر گرفته شوند حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه نقطه شناور به سبک C# را شبیه‌سازی می‌کند به‌طوری که دو NaN برابر در نظر گرفته شوند حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای مقاصد داخلی. |
| [ShapeElementFillSource](../shapeelementfillsource/) [get_FillSource](./get_fillsource/)() | اطلاعاتی دربارهٔ نحوه پر کردن یک عنصر را برمی‌گرداند. فقط-خواندنی [ShapeElementFillSource](../shapeelementfillsource/). |
| [System::SharedPtr](../../system/sharedptr/)\<[Shape](../shape/)\> [get_ParentShape](./get_parentshape/)() | یک Shape_PPT را برمی‌گرداند که عنصر برای آن ایجاد شده است. فقط-خواندنی [Shape](../shape/). |
| [System::ArrayPtr](../../system/arrayptr/)\<[System::Drawing::PointF](../../system.drawing/pointf/)\> [get_PathPoints](./get_pathpoints/)() | یک آرایه از نقاطی که هندسه مسیر عنصر را تعریف می‌کنند، دریافت می‌کند. |
| [System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_PathTypes](./get_pathtypes/)() | یک آرایه از مقادیر بایت که نوع هر نقطه در مسیر عنصر را مشخص می‌کنند، دریافت می‌کند. |
| [ShapeElementStrokeSource](../shapeelementstrokesource/) [get_StrokeSource](./get_strokesource/)() | اطلاعاتی دربارهٔ نحوه خط‌کشی یک عنصر را برمی‌گرداند. فقط-خواندنی [ShapeElementStrokeSource](../shapeelementstrokesource/). |
...
| void [Lock](../../system/object/lock/)() | قفل کردن عبارت C# lock() را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان nام الگو را به یک اشاره‌گر ضعیف (به‌جای مشترک) تنظیم می‌کند. امکان تغییر اشاره‌گرها در سازنده‌ها به حالت ضعیف را می‌دهد. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌برداری عبارت C# lock() را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
...
* کلاس [IShapeElement](../ishapeelement/)
