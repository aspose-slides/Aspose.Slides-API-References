...
description: "اطلاعات مربوط به یک فریم پشته منفرد را دریافت می‌کند. فقط برای MSVS. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعایی می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای انتقال به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 53
url: /fa/system.diagnostics/stackframe/
---
## کلاس StackFrame

اطلاعات مربوط به یک فریم پشته را دریافت می‌کند. فقط برای MSVS. اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا اشکالات ادعایی می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای عبور به توابع به عنوان آرگومان استفاده کنید.
...
| virtual int [GetFileColumnNumber](./getfilecolumnnumber/)() | عدد شماره ستون را دریافت می‌کند. |
| virtual int [GetFileLineNumber](./getfilelinenumber/)() | عدد شماره خط را دریافت می‌کند. |
| virtual [System::String](../../system/string/) [GetFileName](./getfilename/)() | نام فایل را دریافت می‌کند. |
...
| [System::SharedPtr](../../system/sharedptr/)\<[System::Reflection::MethodBase](../../system.reflection/methodbase/)\> [GetMethod](./getmethod/)() | اطلاعات متد را دریافت می‌کند. |
...
| void [Lock](../../system/object/lock/)() | قفل کردن با عبارت lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| const [StackFrame](./)\& [operator=](./operator_equal/)(const [StackFrame](./)\&) const | بدون تغییر. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان nام قالب را به یک اشاره‌گر ضعیف (به‌جای مشترک) تنظیم می‌کند. امکان تعویض اشاره‌گرها در محفظه‌ها به حالت ضعیف را می‌دهد. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارنده مرجع مشترک را کاهش می‌دهد و مقدار آن را بر می‌گرداند. نباید به‌صورت مستقیم فراخوانی شود؛ به‌جای آن از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
|  [StackFrame](./stackframe/)(int) | یک فریم پشته را در جابجایی فعلی پشته ایجاد می‌کند. |
|  [StackFrame](./stackframe/)(const [StackFrame](./)\&) | بدون کپی. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گشایی با عبارت lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual  [~StackFrame](./~stackframe/)() | متد مخرب. |
