...
description: "اطلاعات پایه درباره متد. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از operator new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های اطمینان می‌شود. همیشه این کلاس را در اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای پاس دادن به توابع به‌عنوان آرگومان استفاده کنید."
type: docs
weight: 118
url: /fa/system.reflection/methodbase/
---
## MethodBase کلاس

اطلاعات پایه درباره متد. اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از operator new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های اطمینان می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای عبور به عنوان آرگومان به توابع استفاده کنید.
...
| متد | توضیح |
| --- | --- |
| void [AddAttribute](../memberinfo/addattribute/)(const [ObjectPtr](../memberinfo/objectptr/)\&) | خاصیت را به مجموعه اضافه می‌کند. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | امولاتوری برای مقایسه عدد شناور به سبک C# است که در آن دو NaN برابر در نظر گرفته می‌شوند حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | امولاتوری برای مقایسه عدد شناور به سبک C# است که در آن دو NaN برابر در نظر گرفته می‌شوند حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
...
| [MemberTypes](../membertypes/) [get_MemberType](./get_membertype/)() const override | نوع عضو را نشان می‌دهد - متد، سازنده، رویداد، و غیره. |
...
| static [System::SharedPtr](../../system/sharedptr/)\<[MemberInfo](../memberinfo/)\> [GetCurrentMethod](./getcurrentmethod/)(const [String](../../system/string/)\&) | این متد امکان دریافت نام متد فعلی را فراهم می‌کند. مترجم به‌صورت خودکار ASPOSE_CURRENT_FUNCTION را به عنوان پارامتر جایگزین می‌کند. |
| [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[Object](../../system/object/)\>\> [GetCustomAttributes](../memberinfo/getcustomattributes/)(const [TypeInfo](../../system/typeinfo/)\&, **bool**) const | یک آرایه حاوی اشیائی که تمام ویژگی‌های سفارشی اعمال‌شده بر نوع نمایان‌شده توسط شیء فعلی را نشان می‌دهد، برمی‌گرداند. |
| [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[Object](../../system/object/)\>\> [GetCustomAttributes](../memberinfo/getcustomattributes/)(**bool**) const | یک آرایه حاوی اشیائی که تمام ویژگی‌های سفارشی اعمال‌شده بر نوع نمایان‌شده توسط شیء فعلی را نشان می‌دهد، برمی‌گرداند. |
...
|  [MethodBase](./methodbase/)() | نمونه جدیدی از کلاس [MethodBase](./) را مقداردهی اولیه می‌کند. |
...
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر انتساب. در واقع هیچ چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت زیرکلاس‌ها به‌صورت کپی را فراهم می‌کند. |
...
| static std::enable_if<\![IsSmartPtr](../../system/issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../../system/object/referenceequals/)(T const\&, std::nullptr_t) | مقابله مرجع‌گونه شیء نوع مقداری با nullptr. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گشایی عبارت lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
