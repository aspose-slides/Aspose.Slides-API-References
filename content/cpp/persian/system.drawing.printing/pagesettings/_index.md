...
description: "تنظیمات صفحهٔ چاپ‌شده را نمایش می‌دهد. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() اختصاص یابند. هرگز نمونه‌ای از این نوع را در پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای اظهاری می‌شود. همواره این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای انتقال به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 14
url: /fa/system.drawing.printing/pagesettings/
---
## PageSettings کلاس

اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) اختصاص یابند. هرگز نمونه‌ای از این نوع را در پشته یا با استفاده از operator new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای اظهاری می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای انتقال آن به توابع به عنوان آرگومان استفاده کنید.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسه ممیز شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN به عنوان برابر در نظر گرفته می‌شوند، اگرچه بر اساس IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه ممیز شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN به عنوان برابر در نظر گرفته می‌شوند، اگرچه بر اساس IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای موارد داخلی استفاده می‌شود. |
| [Rectangle](../../system.drawing/rectangle/) [get_Bounds](./get_bounds/)() | پیاده‌سازی نشده. |
| **float** [get_HardMarginX](./get_hardmarginx/)() | پیاده‌سازی نشده. |
| **float** [get_HardMarginY](./get_hardmarginy/)() | پیاده‌سازی نشده. |
| [SharedPtr](../../system/sharedptr/)\<[PrinterResolution](../printerresolution/)\> [get_PrinterResolution](./get_printerresolution/)() | پیاده‌سازی نشده. |
...
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | بررسی می‌کند که آیا شیء نمونه‌ای از نوع توصیف‌شده توسط targetType است. معادل عملگر 'is' در C# است. |
| void [Lock](../../system/object/lock/)() | قفل‌گذاری عبارت lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء محافظ [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [PageSettings](./pagesettings/)() | یک نمونهٔ جدید از کلاس [PageSettings](./) می‌سازد. |
|  [PageSettings](./pagesettings/)(const [SharedPtr](../../system/sharedptr/)\<[PrinterSettings](../printersettings/)\>\&) | یک نمونهٔ جدید از کلاس [PageSettings](./) می‌سازد. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گشایی عبارت lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء محافظ [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| تعریف‌نوع | توضیح |
| --- | --- |
| [ptr](./ptr/) | یک نام مستعار برای یک اشاره‌گر مشترک به نمونه‌ای از این کلاس. |
