...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسهٔ نقاط شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند حتی اگر طبق IEC 60559:1989 NaN برابر هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسهٔ نقاط شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند حتی اگر طبق IEC 60559:1989 NaN برابر هیچ مقداری، از جمله NaN، نیست. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای مقاصد داخلی استفاده می‌شود. |
| [System::SharedPtr](../../system/sharedptr/)\<[IFormat](../iformat/)\> [get_DownBarsFormat](./get_downbarsformat/)() override | قالب DownBars را برمی‌گرداند. فقط-خواندنی [IFormat](../iformat/). |
| **int32_t** [get_GapWidth](./get_gapwidth/)() override | GapWidth. خواندن **int32_t**. |
| **bool** [get_HasUpDownBars](./get_hasupdownbars/)() override | وجود UpDownBars. خواندن **bool**. |
| [System::SharedPtr](../../system/sharedptr/)\<[IFormat](../iformat/)\> [get_UpBarsFormat](./get_upbarsformat/)() override | قالب UpBars را برمی‌گرداند. فقط-خواندنی [IFormat](../iformat/). |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری با بیان lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده شود. |
...
|  [Object](../../system/object/object/)() | شیء جدیدی ایجاد می‌کند. تمام ساختارهای داخلی را مقداردهی اولیه می‌کند. |
...
| void [set_GapWidth](./set_gapwidth/)(**int32_t**) override | GapWidth. نوشتن **int32_t**. |
| void [set_HasUpDownBars](./set_hasupdownbars/)(**bool**) override | وجود UpDownBars. نوشتن **bool**. |
| void [SetTemplateWeakPtr](./settemplateweakptr/)(**uint32_t**) override | متغیر قالب nام را به یک اشاره‌گر ضعیف (به‌جای اشتراکی) تنظیم می‌کند. امکان تغییر اشاره‌گرها در کانتینرها به حالت ضعیف را فراهم می‌سازد. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارندهٔ مرجع مشترک را کاهش داده و مقدار آن را باز می‌گرداند. نباید به‌صورت مستقیم فراخوانی شود؛ به‌جای آن از نشانگرهای هوشمند یا ThisProtector استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گشایی با بیان lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده شود. |
...
* کلاس [IUpDownBarsManager](../iupdownbarsmanager/)
