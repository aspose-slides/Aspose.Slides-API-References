...
description: نمایشگر ورق کاری مرتبط با IChartDataCell
type: docs
weight: 209
...
نمایش می‌دهد ورق کاری مرتبط با [IChartDataCell](../ichartdatacell/)
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | تطبیق مقایسهٔ نقطه شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | تطبیق مقایسهٔ نقطه شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر در نظر گرفته می‌شوند حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN، نیست. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای مقاصد داخلی. |
| **int32_t** [get_Index](./get_index/)() override | اندیس ورق کاری را دریافت می‌کند. فقط خواندنی **int32_t**. |
| [System::String](../../system/string/) [get_Name](./get_name/)() override | نام ورق کاری را بر می‌گرداند. فقط خواندنی [System::String](../../system/string/). |
...
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | بررسی می‌کند آیا شیء نمونه‌ای از نوع توصیف‌شده توسط targetType است. معادل اپراتور C# 'is'. |
| void [Lock](../../system/object/lock/)() | قفل‌گذاری با جملهٔ C# lock() را پیاده‌سازی می‌کند. مستقیم فراخوانی کنید یا از شیء پایدار [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازندهٔ کپی. واقعاً چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه کرده و امکان ساخت کپی زیرکلاس‌ها را می‌دهد. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | اپراتور انتساب. واقعاً چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه کرده و امکان ساخت کپی زیرکلاس‌ها را می‌دهد. |
...
| static std::enable_if<\![IsSmartPtr](../../system/issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../../system/object/referenceequals/)(T const\&, std::nullptr_t) | شیء از نوع مقدار را با nullptr بر مبنای مرجع مقایسه می‌کند. |
...
| void [SetTemplateWeakPtr](./settemplateweakptr/)(**uint32_t**) override | آرگومان nام قالب را به یک اشاره‌گر ضعیف تنظیم می‌کند (به جای مشترک). اجازه می‌دهد اشاره‌گرها در مخازن به حالت ضعیف سوئیچ کنند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گذاری معکوس جملهٔ C# lock() را پیاده‌سازی می‌کند. مستقیم فراخوانی کنید یا از شیء پایدار [LockContext](../../system/lockcontext/) استفاده کنید. |
...
* کلاس [IChartDataWorksheet](../ichartdataworksheet/)
