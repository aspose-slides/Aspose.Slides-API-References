...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسه نقطه‌ی شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN به‌عنوان برابر در نظر گرفته می‌شوند، حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه نقطه‌ی شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN به‌عنوان برابر در نظر گرفته می‌شوند، حتی اگر طبق IEC 60559:1989 NaN برابر با هیچ مقداری، از جمله NaN نیست. |
...
| [System::SharedPtr](../../system/sharedptr/)\<[IFormat](../iformat/)\> [get_Format](./get_format/)() override | سبک‌های خط، پرشدگی و اثر یک شیء را برمی‌گرداند. فقط-خواندنی [IFormat](../iformat/). |
| **bool** [get_HasBorderHorizontal](./get_hasborderhorizontal/)() override | اگر جدول داده نمودار مرزهای افقی سلول داشته باشد، درست است. خواند **bool**. |
| **bool** [get_HasBorderOutline](./get_hasborderoutline/)() override | اگر جدول داده نمودار مرزهای کلی داشته باشد، درست است. خواند **bool**. |
| **bool** [get_HasBorderVertical](./get_hasbordervertical/)() override | اگر جدول داده نمودار مرزهای عمودی سلول دارد، درست است. خواند **bool**. |
| **bool** [get_ShowLegendKey](./get_showlegendkey/)() override | اگر کلید راهنمای برچسب داده قابل مشاهده باشد، درست است. خواند **bool**. |
| [System::SharedPtr](../../system/sharedptr/)\<[IChartTextFormat](../icharttextformat/)\> [get_TextFormat](./get_textformat/)() override | فرمت متن را برمی‌گرداند. فقط-خواندنی [IChartTextFormat](../icharttextformat/). |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری دستور lock() در C# را پیاده‌سازی می‌کند. مستقیماً فراخوانی کنید یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| void [set_HasBorderHorizontal](./set_hasborderhorizontal/)(**bool**) override | اگر جدول داده نمودار مرزهای افقی سلول داشته باشد، درست است. نوشتن **bool**. |
| void [set_HasBorderOutline](./set_hasborderoutline/)(**bool**) override | اگر جدول داده نمودار مرزهای کلی داشته باشد، درست است. نوشتن **bool**. |
| void [set_HasBorderVertical](./set_hasbordervertical/)(**bool**) override | اگر جدول داده نمودار مرزهای عمودی سلول دارد، درست است. نوشتن **bool**. |
| void [set_ShowLegendKey](./set_showlegendkey/)(**bool**) override | اگر کلید راهنمای برچسب داده قابل مشاهده باشد، درست است. نوشتن **bool**. |
| void [SetTemplateWeakPtr](./settemplateweakptr/)(**uint32_t**) override | آرگومان تمپلیت nام را به یک اشاره‌گر ضعیف (به‌جای مشترک) تنظیم می‌کند. امکان تغییر اشاره‌گرها در کانتینرها به حالت ضعیف را فراهم می‌آورد. |
