...
description: "الگوریتم MD5 سازگار با CSP. پیاده‌سازی نشده. اشیای این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعای (assertion) می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای پاس دادن به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 326
url: /fa/system.security.cryptography/md5cryptoserviceprovider/
---
## کلاس MD5CryptoServiceProvider

الگوریتم [MD5](../md5/) سازگار با CSP. پیاده‌سازی نشده. اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعای (assertion) می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچانید و از این اشاره‌گر برای عبور به توابع به عنوان آرگومان استفاده کنید.
```cpp
...
| متد | توضیح |
| --- | --- |
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [ComputeHash](../hashalgorithm/computehash/)(const [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>\&) | هش بافر. |
...
| static [SharedPtr](../../system/sharedptr/)\<[MD5](../md5/)\> [Create](../md5/create/)() | الگوریتم [MD5](../md5/) را ایجاد می‌کند. |
| static [SharedPtr](../../system/sharedptr/)\<[MD5](../md5/)\> [Create](../md5/create/)(const [String](../../system/string/)\&) | الگوریتم [MD5](../md5/) را ایجاد می‌کند. |
...
| virtual int [get_HashSize](../hashalgorithm/get_hashsize/)() | اندازه مقدار هش محاسبه‌شده را بر اساس بایت دریافت می‌کند. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری بیانیه lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
|  [MD5CryptoServiceProvider](./md5cryptoserviceprovider/)() | سازنده. پیاده‌سازی نشده. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازندهٔ کپی. در واقع هیچ‌ چیزی کپی نمی‌کند، فقط شیء جدید را مقداردهی می‌کند و امکان ساخت کپی برای زیردست‌ها را می‌دهد. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر انتساب. در واقع هیچ‌ چیزی کپی نمی‌کند، فقط شیء جدید را مقداردهی می‌کند و امکان ساخت کپی برای زیردست‌ها را می‌دهد. |
...
| int [RemovedSharedRefs](../../system/object/removedsharedrefs/)(int) | شمارش مرجع مشترک را با مقدار مشخص کاهش می‌دهد. |
...
| int [TransformBlock](../hashalgorithm/transformblock/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int) override | بخش داده را پردازش می‌کند و داده‌ها را به آرایه خروجی کپی می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گذاری بیانیه lock() در C# را باز می‌کند. به‌صورت مستقیم صدا زده شود یا از شیء نگهبان [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual  [~HashAlgorithm](../hashalgorithm/~hashalgorithm/)() | دستگاه تخریب. |
