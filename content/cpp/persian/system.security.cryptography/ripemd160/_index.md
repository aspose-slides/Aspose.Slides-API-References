...
description: "هش RIPEMD160 را برای داده‌های ورودی محاسبه می‌کند. در C++ RIPEMD160 یک کلاس انتزاعی نیست. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های اعتبارسنجی می‌شود. همواره این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای عبور به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 430
url: /fa/system.security.cryptography/ripemd160/
---
## RIPEMD160 کلاس

برای داده‌های ورودی، هش [RIPEMD160](./) را محاسبه می‌کند. در C++، [RIPEMD160](./) یک کلاس انتزاعی نیست. اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های اعتبارسنجی می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) قرار داده و از این اشاره‌گر برای ارسال به توابع به عنوان آرگومان استفاده کنید.
...
| static [SharedPtr](../../system/sharedptr/)\<[RIPEMD160](./)\> [Create](./create/)() | یک نمونه از الگوریتم هش ایجاد می‌کند. |
| static [SharedPtr](../../system/sharedptr/)\<[RIPEMD160](./)\> [Create](./create/)(const [String](../../system/string/)\&) | یک نمونه از الگوریتم هش ایجاد می‌کند. |
...
| virtual int [get_HashSize](../hashalgorithm/get_hashsize/)() | اندازۀ مقدار هش محاسبه‌شده را بر حسب بایت دریافت می‌کند. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان الگوئی nام را به یک اشاره‌گر ضعیف (به جای مشترک) تنظیم می‌کند. امکان تغییر اشاره‌گرها در کانتینرها به حالت ضعیف را فراهم می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گذاری با عبارت C# lock() را باز می‌کند. مستقیماً فراخوانی کنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual  [~Object](../../system/object/~object/)() | شیء را می‌نویسد. تمام ساختارهای داده داخلی را آزاد می‌کند. |
