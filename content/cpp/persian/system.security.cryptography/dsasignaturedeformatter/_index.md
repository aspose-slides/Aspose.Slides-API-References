...
description: "برای تأیید امضای DSA استفاده می‌شود. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعا می‌شود. همیشه این کلاس را درون اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای انتقال به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 157
url: /fa/system.security.cryptography/dsasignaturedeformatter/
---
## DSASignatureDeformatter کلاس

برای تأیید امضای [DSA](../dsa/) استفاده می‌شود. اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعا می‌شود. همیشه این کلاس را درون اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای انتقال به توابع به عنوان آرگومان استفاده کنید.
...
| virtual **int32_t** [GetHashCode](../../system/object/gethashcode/)() const | معادل متد C# [Object.GetHashCode()](../../system/object/gethashcode/). قابلیت هش‌گذاری اشیاء سفارشی را فراهم می‌کند. |
...
| void [Lock](../../system/object/lock/)() | بیان قفل‌گذاری در عبیه C# lock() را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| static std::enable_if<\![IsSmartPtr](../../system/issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../../system/object/referenceequals/)(T const\&, std::nullptr_t) | به‌صورت مرجعی شیء نوع مقدار را با nullptr مقایسه می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | بیان باز کردن قفل در عبیه C# lock() را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
| **bool** [VerifySignature](./verifysignature/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>) override | پیاده‌سازی نشده. |
| virtual **bool** [VerifySignature](../asymmetricsignaturedeformatter/verifysignature/)([System::SharedPtr](../../system/sharedptr/)\<[HashAlgorithm](../hashalgorithm/)\>, [System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>) | امضا را روی داده‌ها تأیید می‌کند. پیاده‌سازی نشده. |
