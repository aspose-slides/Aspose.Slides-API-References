...
description: "اطلاعاتی درباره یک میزبان را نشان می‌دهد. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص یابند. هرگز نمونه‌ای از این نوع را در پشته یا با استفاده از اپراتور new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های ادعا می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای ارسال به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 1
url: /fa/system.net.networkinformation/hostinformation/
---
## HostInformation کلاس

اطلاعاتی درباره یک میزبان را نشان می‌دهد. شی‌های این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص یابند. هرگز نمونه‌ای از این نوع را در پشته یا با استفاده از عامل new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های ادعا می‌شود. همواره این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای ارسال به توابع به عنوان آرگومان استفاده کنید.
...
| static [String](../../system/string/) [get_DomainName](./get_domainname/)() | دامنه‌ای که کامپیوتر محلی در آن ثبت شده است را برمی‌گرداند. |
| static [String](../../system/string/) [get_HostName](./get_hostname/)() | نام میزبان کامپیوتر محلی را برمی‌گرداند. |
...
| void [Lock](../../system/object/lock/)() | قفل‌کردن بیان lock() در C# را اجرا می‌کند. به‌صورت مستقیم صدا زده یا از شی مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازندهٔ copy. در واقع هیچ‌چیزی را کپی نمی‌کند، فقط شی جدید را مقداردهی اولیه می‌کند و امکان ساختن زیرکلاس‌ها از طریق copy را فراهم می‌کند. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر انتساب. در واقع هیچ‌چیزی را کپی نمی‌کند، فقط شی جدید را مقداردهی اولیه می‌کند و امکان ساختن زیرکلاس‌ها از طریق copy را فراهم می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | باز کردن قفل بیان lock() در C# را اجرا می‌کند. به‌صورت مستقیم صدا زده یا از شی مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
...
* فضای نام [System::Net::NetworkInformation](../)
