...
description: "خصوصیاتی را فراهم می‌کند که ابعاد فریم یک تصویر را به‌دست می‌آورد. اشیای این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا اشکالات assert می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچانید و از این اشاره‌گر برای عبور به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 92
url: /fa/system.drawing.imaging/framedimension/
---
## FrameDimension کلاس

خصوصیاتی را فراهم می‌کند که ابعاد فریم یک تصویر را به‌دست می‌آورد. اشیای این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا باعث خطاهای زمان اجرا و/یا شکست‌های assert می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای عبور به توابع به عنوان آرگومان استفاده کنید.
...
| Method | Description |
| --- | --- |
| **bool** [Equals](./equals/)([FrameDimensionPtr](../framedimensionptr/)) | مشخص می‌کند که آیا شیء [FrameDimension](./) مشخص شده معادل شیء جاری است. |
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | مقایسه عدد شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر درنظر گرفته می‌شوند، اگرچه بر اساس IEC 60559:1989 NaN برابر هیچ مقداری، از جمله NaN، نیست. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | مقایسه عدد شناور به سبک C# را شبیه‌سازی می‌کند که در آن دو NaN برابر درنظر گرفته می‌شوند، اگرچه بر اساس IEC 60559:1989 NaN برابر هیچ مقداری، از جمله NaN، نیست. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای مقاصد داخلی. |
| [FrameDimension](./framedimension/)(const [System::Guid](../../system/guid/)\&) | یک شیء جدید [FrameDimension](./) را ساخته و آن را با GUID مشخص شده مقداردهی اولیه می‌کند. |
| [System::Guid](../../system/guid/) [get_Guid](./get_guid/)() const | GUID مربوط به شیء جاری را برمی‌گرداند. |
| static [FrameDimensionPtr](../framedimensionptr/) [get_Page](./get_page/)() | بعد صفحه را برمی‌گرداند. |
| static [FrameDimensionPtr](../framedimensionptr/) [get_Resolution](./get_resolution/)() | بعد وضوح را برمی‌گرداند. |
| static [FrameDimensionPtr](../framedimensionptr/) [get_Time](./get_time/)() | بعد زمان را برمی‌گرداند. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری با بیان C# lock() را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء نگهبانی [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گذاری معکوس بیان C# lock() را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی کنید یا از شیء نگهبانی [LockContext](../../system/lockcontext/) استفاده کنید. |
