...
description: شیء غیرقابل تغییر که اثر Alpha Replace را نشان می‌دهد. مقادیر آلفای اثر (شفافیت) با یک آلفای ثابت جایگزین می‌شوند.
type: docs
weight: 404
url: /fa/aspose.slides.effects/ialphareplaceeffectivedata/
---
## IAlphaReplaceEffectiveData کلاس

Immutable object که نمایانگر اثر Alpha Replace است. مقادیر آلفای اثر (شفافیت) با یک آلفای ثابت جایگزین می‌شوند.
...
| virtual **float** [get_Alpha](./get_alpha/)() | مقدار آلفای جدید را در بازه [0..1] برمی‌گرداند. فقط خواندنی **float**. |
...
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | بررسی می‌کند که آیا آبجکت نمونه‌ای از نوع توصیف‌شده توسط targetType است. معادل عملگر 'is' C#. |
| void [Lock](../../system/object/lock/)() | اجرای قفل (lock()) در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازنده کپی. در واقع هیچ چیزی را کپی نمی‌کند، فقط آبجکت جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی در زیرکلاس‌ها را فراهم می‌آورد. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر تخصیص. در واقع هیچ چیزی را کپی نمی‌کند، فقط آبجکت جدید را مقداردهی اولیه می‌کند و امکان ساخت کپی در زیرکلاس‌ها را فراهم می‌آورد. |
...
| static std::enable_if<\![IsSmartPtr](../../system/issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../../system/object/referenceequals/)(T const\&, std::nullptr_t) | مقایسه مرجع‌ ای آبجکت نوع مقدار با nullptr. |
...
| void [Unlock](../../system/object/unlock/)() | اجرای بازکردن قفل (lock()) در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم فراخوانی شود یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
