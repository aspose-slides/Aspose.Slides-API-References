...
description: شیء غیرقابل تغییر که یک اثر جایگزینی رنگ را نشان می‌دهد. تمام رنگ‌های اثر به یک رنگ ثابت تغییر می‌کنند. مقادیر آلفا تحت تأثیر قرار نمی‌گیرند.
type: docs
weight: 547
...
شیء غیرقابل تغییر که یک اثر جایگزینی رنگ را نشان می‌دهد. تمام رنگ‌های اثر به یک رنگ ثابت تغییر می‌کنند. مقادیر آلفا تحت تأثیر قرار نمی‌گیرند.
...
| virtual [System::Drawing::Color](../../system.drawing/color/) [get_Color](./get_color/)() | قالب رنگ را که رنگ هر پیکسل را جایگزین خواهد کرد برمی‌گرداند. فقط-خواندنی [System::Drawing::Color](../../system.drawing/color/). |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری عبارت C# lock() را پیاده‌سازی می‌کند. مستقیم فراخوانی شود یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازنده کپی. در واقع هیچ چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه کرده و امکان ساختن کپی از کلاس‌های فرزند را فراهم می‌کند. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر تخصیص. در واقع هیچ چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه کرده و امکان ساختن کپی از کلاس‌های فرزند را فراهم می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌برداری عبارت C# lock() را پیاده‌سازی می‌کند. مستقیم فراخوانی شود یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
