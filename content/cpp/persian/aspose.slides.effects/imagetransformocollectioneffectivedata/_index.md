...
description: شیء غیرقابل تغییر که یک مجموعه فقط-خواندنی از افکت‌های مؤثر تبدیل تصویر را نمایان می‌کند.
type: docs
weight: 820
...
شیء غیرقابل تغییر که نمایانگر یک مجموعه فقط‌خواندنی از افکت‌های تبدیل تصویر مؤثر است.
...
| Method | Description |
| --- | --- |
| [iterator](./iterator/) [begin](./begin/)() | دوراندازی که به اولین عنصر (if any) مجموعه اشاره می‌کند را می‌گیرد. |
| [const_iterator](./const_iterator/) [begin](./begin/)() const | دوراندازی که به اولین عنصر (if any) از نمونه const-qualified مجموعه اشاره می‌کند را می‌گیرد. |
| [const_iterator](./const_iterator/) [cbegin](./cbegin/)() const | دوراندازی که به اولین عنصر const-qualified (if any) مجموعه اشاره می‌کند را می‌گیرد. |
| [const_iterator](./const_iterator/) [cend](./cend/)() const | دوراندازی که درست پس از آخرین عنصر const-qualified (if any) مجموعه اشاره می‌کند را می‌گیرد. |
| void [CopyTo](./copyto/)([System::ArrayPtr](../../system/arrayptr/)\<[System::SharedPtr](../../system/sharedptr/)\<[IEffectEffectiveData](../ieffecteffectivedata/)\>\>, **int32_t**) override | تمام عناصر مجموعه را در آرایهٔ مشخص شده کپی می‌کند. |
| [iterator](./iterator/) [end](./end/)() | دوراندازی که درست پس از آخرین عنصر (if any) مجموعه اشاره می‌کند را می‌گیرد. |
| [const_iterator](./const_iterator/) [end](./end/)() const | دوراندازی که درست پس از آخرین عنصر (if any) از نمونه const-qualified مجموعه اشاره می‌کند را می‌گیرد. |
| **bool** [Equals](./equals/)([System::SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\>) override | تعیین می‌کند که آیا شیء مشخص‌شده با شیء فعلی برابر است یا نه. |
...
| **int32_t** [get_Count](./get_count/)() override | تعداد افکت‌های تصویر در یک مجموعه را برمی‌گرداند. فقط-خواندنی **int32_t**. |
| **bool** [get_IsSynchronized](./get_issynchronized/)() override | مقداری را برمی‌گرداند که نشان می‌دهد دسترسی به مجموعه همگام‌سازی شده (ایمن برای رشته) است یا نه. فقط-خواندنی **bool**. |
...
| [System::SharedPtr](../../system/sharedptr/)\<[System::Collections::Generic::IEnumerator](../../system.collections.generic/ienumerator/)\<[System::SharedPtr](../../system/sharedptr/)\<[IEffectEffectiveData](../ieffecteffectivedata/)\>\>\> [GetEnumerator](./getenumerator/)() override | یک شمارندهٔ که از طریق مجموعه تکرار می‌کند را برمی‌گرداند. |
| **int32_t** [GetHashCode](./gethashcode/)() const override | به عنوان تابع هش برای یک نوع خاص عمل می‌کند که برای الگوریتم‌های هش و ساختارهای داده‌ای مانند جدول هش مناسب است. |
...
| [System::SharedPtr](../../system/sharedptr/)\<[IEffectEffectiveData](../ieffecteffectivedata/)\> [idx_get](./idx_get/)(**int32_t**) override | عنصر را بر اساس اندیس برمی‌گرداند. |
|  [ImageTransformOCollectionEffectiveData](./imagetransformocollectioneffectivedata/)() |  |
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | بررسی می‌کند که آیا شیء نشان‌دهندهٔ نمونه‌ای از نوعی است که توسط targetType توصیف شده است. معادل عملگر 'is' در C#. |
...
| **bool** [LINQ_Any](../../system.collections.generic/ienumerable/linq_any/)() | تعیین می‌کند که آیا یک دنباله شامل هر عنصر دیگری است یا نه. |
...
| ResultType [LINQ_Average](../../system.collections.generic/ienumerable/linq_average/)(const [Func](../../system/func/)\<T, ResultType\>\&) | میانگین یک دنباله از مقادیر را که با فراخوانی یک تابع تبدیل بر هر عنصر از دنباله ورودی به دست می‌آید، محاسبه می‌کند. |
...
| T [LINQ_ElementAt](../../system.collections.generic/ienumerable/linq_elementat/)(int) | عنصر در اندیس مشخصی از یک دنباله را برمی‌گرداند. |
| T [LINQ_ElementAtOrDefault](../../system.collections.generic/ienumerable/linq_elementatordefault/)(int) | عنصر در اندیس مشخصی از یک دنباله را برمی‌گرداند. |
...
| [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../../system.collections.generic/ienumerable/)\<T\>\> [LINQ_Skip](../../system.collections.generic/ienumerable/linq_skip/)(**int32_t**) | تعداد مشخصی از عناصر متوالی را از ابتدای یک دنباله عبور داده و بقیه را برمی‌گرداند. |
...
| void [Lock](../../system/object/lock/)() | عملکرد قفل‌گذاری توسط عبارت lock() در C# را پیاده‌سازی می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان قالب nام را به یک اشاره‌گر ضعیف (به‌جای اشتراکی) تنظیم می‌کند. امکان تغییر اشاره‌گرها در مخازن به حالت ضعیف را فراهم می‌کند. |
| int [SharedCount](../../system/object/sharedcount/)() const | مقدار فعلی شمارشگر مرجع مشترک را می‌گیرد. |
| [Object](../../system/object/) * [SharedRefAdded](../../system/object/sharedrefadded/)() | شمارشگر مرجع مشترک را افزایش می‌دهد. نباید به‌صورت مستقیم صدا زده شود؛ به‌جای آن از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارشگر مرجع مشترک را کاهش داده و آن را برمی‌گرداند. نباید به‌صورت مستقیم صدا زده شود؛ به‌جای آن از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گذاری توسط عبارت lock() در C# را باز می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
| [virtualized_iterator](./virtualized_iterator/) * [virtualizeBeginConstIterator](./virtualizebeginconstiterator/)() const override | دوراندازی که به اولین عنصر (if any) از نمونه const-qualified مجموعه اشاره می‌کند را می‌گیرد. |
| [virtualized_iterator](./virtualized_iterator/) * [virtualizeBeginIterator](./virtualizebeginiterator/)() override | دوراندازی که به اولین عنصر (if any) مجموعه اشاره می‌کند را می‌گیرد. |
| [virtualized_iterator](./virtualized_iterator/) * [virtualizeEndConstIterator](./virtualizeendconstiterator/)() const override | دوراندازی که درست پس از آخرین عنصر (if any) از نمونه const-qualified مجموعه اشاره می‌کند را می‌گیرد. |
| [virtualized_iterator](./virtualized_iterator/) * [virtualizeEndIterator](./virtualizeenditerator/)() override | دوراندازی که درست پس از آخرین عنصر (if any) مجموعه اشاره می‌کند را می‌گیرد. |
| Detail::SmartPtrCounter * [WeakRefAdded](../../system/object/weakrefadded/)() | شمارشگر مرجع ضعیف را افزایش می‌دهد. نباید به‌صورت مستقیم صدا زده شود؛ به‌جای آن از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
| void [WeakRefRemoved](../../system/object/weakrefremoved/)() | شمارشگر مرجع ضعیف را کاهش می‌دهد. نباید به‌صورت مستقیم صدا زده شود؛ به‌جای آن از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
| virtual  [~Object](../../system/object/~object/)() | شیء را از بین می‌برد. تمام ساختارهای داده داخلی را آزاد می‌کند. |

## Typedefها
| Typedef | Description |
| --- | --- |
| [iterator_holder_type](./iterator_holder_type/) | یک نوع مجموعه که نوع‌های iterator آن به عنوان iteratorهای مجموعهٔ فعلی استفاده می‌شود. |
...
نام IImageTransformOperationCollectionEffectiveData به [IImageTransformOCollectionEffectiveData](../iimagetransformocollectioneffectivedata/) کوتاه شد زیرا طول نام‌های COM نمی‌تواند بیش از ۳۹ باشد.

## موارد مرتبط

* کلاس [IEffectiveData](../../aspose.slides/ieffectivedata/)
* کلاس [IImageTransformOCollectionEffectiveData](../iimagetransformocollectioneffectivedata/)
