...
description: شیء غیرقابل تغییر که نمایانگر اثر محو است که بر تمام شکل، از جمله پر کردن آن اعمال می‌شود. تمام کانال‌های رنگی، از جمله آلفا، تحت تأثیر قرار می‌گیرند.
type: docs
weight: 469
url: /fa/aspose.slides.effects/iblureffectivedata/
---
## IBlurEffectiveData کلاس

شیء غیرقابل تغییر که نمایانگر اثر [Blur](../blur/) است که بر تمام شکل، از جمله پر کردن آن اعمال می‌شود. تمام کانال‌های رنگی، از جمله آلفا، تحت تأثیر قرار می‌گیرند.
...
| virtual **bool** [get_Grow](./get_grow/)() | تعیین می‌کند آیا محدوده‌های شیء باید به دلیل محو شدن بزرگ شوند یا خیر. مقدار true نشان‌دهنده بزرگ شدن محدوده‌ها است و false نشان‌دهنده عدم بزرگ شدن. فقط-خواندنی **bool**. |
| virtual **double** [get_Radius](./get_radius/)() | شعاع محو را برمی‌گرداند. فقط-خواندنی **double**. |
...
| void [Lock](../../system/object/lock/)() | قفل کردن با عبارت lock() در C# را پیاده‌سازی می‌کند. مستقیماً فراخوانی کنید یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان nام قالب را به یک اشاره‌گر ضعیف تنظیم می‌کند (به جای مشترک). امکان تغییر اشاره‌گرها در سازندها به حالت ضعیف را فراهم می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گشایی با عبارت lock() در C# را پیاده‌سازی می‌کند. مستقیماً فراخوانی کنید یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
