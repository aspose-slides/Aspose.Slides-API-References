...
description: "آرایه‌ای از بیت‌ها که می‌توان با ایندکس به آن‌ها دسترسی داشت. اشیای این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای assert می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچانید و از این اشاره‌گر برای عبور به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 1
url: /fa/system.collections/bitarray/
---
## BitArray کلاس

[Array](../../system/array/) از بیت‌هایی که می‌توان با اندیس به آن‌ها دسترسی پیدا کرد. اشیای این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، چون منجر به خطاهای زمان اجرا و/یا اخطارهای استثنا می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچانید و از این اشاره‌گر برای انتقال به توابع به عنوان آرگومان استفاده کنید.
...
int main()
{
  // یک نمونه جدید از کلاس BitArray را می‌سازد.
  auto bitArray = MakeObject<System::Collections::BitArray>(3);

  // مقادیر را چاپ می‌کند.
  Print(bitArray);
