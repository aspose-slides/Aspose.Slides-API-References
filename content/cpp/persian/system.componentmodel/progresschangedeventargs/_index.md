...
description: "یک نمونه از این کلاس به عنوان آرگومان به نماینده ProgressChangedEventHandler پاس داده می‌شود. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های assert می‌شود. همیشه این کلاس را درون اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای پاس کردن آن به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 157
url: /fa/system.componentmodel/progresschangedeventargs/
---
## ProgressChangedEventArgs کلاس

یک نمونه از این کلاس به عنوان آرگومان به نماینده ProgressChangedEventHandler پاس داده می‌شود. اشیاء این کلاس باید تنها با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا نقص‌های assert می‌شود. همیشه این کلاس را درون اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای پاس کردن آن به توابع به عنوان آرگومان استفاده کنید.
...
|  [EventArgs](../../system/eventargs/eventargs/)() | سازنده. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | فقط برای مقاصد داخلی. |
| int [get_ProgressPercentage](./get_progresspercentage/)() const | درصد پیشرفت کار غیرهمزمان را دریافت می‌کند. |
| [System::SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\> [get_UserState](./get_userstate/)() const | یک وضعیت کاربری یکتا را به‌دست می‌آورد. |
...
| void [Lock](../../system/object/lock/)() | قفل‌کردن توسط دستور C# lock() را پیاده‌سازی می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء محافظ [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | سازندهٔ کپی. در واقع هیچ‌چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت نسخهٔ کپی از زیرکلاس‌ها را فراهم می‌سازد. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | عملگر انتساب. در واقع هیچ‌چیزی را کپی نمی‌کند، فقط شیء جدید را مقداردهی اولیه می‌کند و امکان ساخت نسخهٔ کپی از زیرکلاس‌ها را فراهم می‌سازد. |
|  [ProgressChangedEventArgs](./progresschangedeventargs/)(int, [System::SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\>) | سازنده. |
...
| static std::enable_if<\![IsSmartPtr](../../system/issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../../system/object/referenceequals/)(T const\&, std::nullptr_t) | مقابله مرجع‌گونه شیء نوع مقدار با nullptr. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | آرگومان nام قالب را به یک اشاره‌گر ضعیف (به جای shared) تنظیم می‌کند. امکان تغییر اشاره‌گرها در کانتینرها به حالت weak را فراهم می‌کند. |
...
| void [Unlock](../../system/object/unlock/)() | قفل‌گشایی توسط دستور C# lock() را پیاده‌سازی می‌کند. به‌صورت مستقیم صدا بزنید یا از شیء محافظ [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| فیلد | توضیح |
| --- | --- |
| static [Empty](../../system/eventargs/empty/) | یک عضو ثابت که یک اشاره‌گر مشترک "خالی" [EventArgs](../../system/eventargs/) (null-pointer) را نمایندگی می‌کند. |

## موارد مرتبط

* کلاس [EventArgs](../../system/eventargs/)
