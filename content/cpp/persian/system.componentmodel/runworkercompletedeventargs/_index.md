...
description: "یک نمونه از این کلاس به عنوان آرگومان به نماینده RunWorkerCompletedEventHandler پاس داده می‌شود. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعا می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای انتقال به توابع به عنوان آرگومان استفاده کنید."
type: docs
weight: 183
url: /fa/system.componentmodel/runworkercompletedeventargs/
---
## RunWorkerCompletedEventArgs کلاس

یک نمونه از این کلاس به عنوان آرگومان به نماینده RunWorkerCompletedEventHandler پاس داده می‌شود. اشیاء این کلاس باید فقط با استفاده از تابع [System::MakeObject()](../../system/makeobject/) اختصاص داده شوند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعا می‌شود. همیشه این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای پاس دادن به توابع به عنوان آرگومان استفاده کنید.
...
| متد | توضیح |
| --- | --- |
|  [AsyncCompletedEventArgs](../asynccompletedeventargs/asynccompletedeventargs/)() | سازنده. |
|  [AsyncCompletedEventArgs](../asynccompletedeventargs/asynccompletedeventargs/)(const [System::Exception](../../system/exception/)\&, **bool**, const [System::SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\>\&) | یک نمونه جدید از کلاس [System.ComponentModel.AsyncCompletedEventArgs](../asynccompletedeventargs/) را مقداردهی اولیه می‌کند. |
...
| **bool** [get_Cancelled](../asynccompletedeventargs/get_cancelled/)() const | یک مقدار دریافت می‌کند که نشان‌دهنده این است که آیا یک عملیات ناهمزمان لغو شده است یا نه. true اگر عملیات پس‌زمینه لغو شده باشد؛ در غیر این صورت false. مقدار پیش‌فرض false است. |
| const [System::Exception](../../system/exception/)\& [get_Error](../asynccompletedeventargs/get_error/)() const | یک مقدار دریافت می‌کند که نشان می‌دهد کدام خطا در طول یک عملیات ناهمزمان رخ داده است. |
| [System::SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\> [get_Result](./get_result/)() const | یک مقدار دریافت می‌کند که نمایانگر نتیجه یک عملیات ناهمزمان است. |
| [System::SharedPtr](../../system/sharedptr/)\<[Object](../../system/object/)\> [get_UserState](../asynccompletedeventargs/get_userstate/)() const | شناسهٔ یکتای کار ناهمزمان را دریافت می‌کند. یک ارجاع به شیء که به طور یکتا کار ناهمزمان را شناسایی می‌کند؛ در غیر این صورت، اگر مقداری تنظیم نشده باشد، null برمی‌گردد. |
...
| void [Lock](../../system/object/lock/)() | اجرای قفل‌گذاری بیان lock() در C#. مستقیماً صدا بزنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
|  [RunWorkerCompletedEventArgs](./runworkercompletedeventargs/)(const [System::SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\>\&, const [System::Exception](../../system/exception/)\&, **bool**) | سازنده. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارندهٔ مرجع اشتراکی را کاهش می‌دهد و بازمی‌گرداند. نباید مستقیماً فراخوانی شود؛ در عوض، از اشاره‌گرهای هوشمند یا ThisProtector استفاده کنید. |
...
| void [Unlock](../../system/object/unlock/)() | اجرای آزادسازی بیان lock() در C#. مستقیماً صدا بزنید یا از شیء [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| فیلد | توضیح |
| --- | --- |
| static [Empty](../../system/eventargs/empty/) | یک عضو ایستاتیک که یک اشاره‌گر اشتراکی \"خالی\" [EventArgs](../../system/eventargs/) (null-pointer) را نمایان می‌کند. |

## همچنین ببینید

* کلاس [AsyncCompletedEventArgs](../asynccompletedeventargs/)
