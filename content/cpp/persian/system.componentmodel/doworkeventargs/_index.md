...
description: "آرگومان‌های رویداد DoWork. اشیاء این کلاس باید فقط با استفاده از تابع System::MakeObject() تخصیص یابند. هرگز نمونه‌ای از این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعایی می‌شود. همیشه این کلاس را در یک اشاره‌گر System::SmartPtr بپیچید و از این اشاره‌گر برای عبور به عنوان آرگومان به توابع استفاده کنید."
type: docs
weight: 92
url: /fa/system.componentmodel/doworkeventargs/
---
## کلاس DoWorkEventArgs

آرگومان‌های رویداد DoWork. اشیاء این کلاس باید تنها با استفاده از تابع [System::MakeObject()](../../system/makeobject/) تخصیص یابند. هرگز نمونه این نوع را روی پشته یا با استفاده از عملگر new ایجاد نکنید، زیرا منجر به خطاهای زمان اجرا و/یا خطاهای ادعایی می‌شود. همواره این کلاس را در یک اشاره‌گر [System::SmartPtr](../../system/smartptr/) بپیچید و از این اشاره‌گر برای انتقال به توابع به عنوان آرگومان استفاده کنید.
...
| Method | Description |
| --- | --- |
|  [CancelEventArgs](../canceleventargs/canceleventargs/)(**bool**) | سازنده. |
|  [CancelEventArgs](../canceleventargs/canceleventargs/)() | سازنده؛ خاصیت Cancel را به false تنظیم می‌کند. |
|  [DoWorkEventArgs](./doworkeventargs/)(const [SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\>\&) | یک نمونه از آرگومان‌ها را می‌سازد. |
...
| [SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\> [get_Argument](./get_argument/)() | خاصیت Argument را دریافت می‌کند؛ پیاده‌سازی نشده. |
| **bool** [get_Cancel](../canceleventargs/get_cancel/)() | مقدارئی را برمی‌گرداند که نشان می‌دهد آیا رویداد باید لغو شود. |
| [SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\> [get_Result](./get_result/)() | خاصیت Result را دریافت می‌کند؛ پیاده‌سازی نشده. |
...
| void [Lock](../../system/object/lock/)() | قفل‌گذاری با بیان lock() در C# را پیاده‌سازی می‌کند. مستقیم فراخوانی کنید یا از شیء مراقب [LockContext](../../system/lockcontext/) استفاده کنید. |
...
| void [set_Cancel](../canceleventargs/set_cancel/)(**bool**) | مقدارئی را تنظیم می‌کند که نشان می‌دهد آیا رویداد باید لغو شود. |
| void [set_Result](./set_result/)(const [SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\>\&) | خاصیت Result را تنظیم می‌کند؛ پیاده‌سازی نشده. |
...
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | شمارنده مرجع مشترک را کاهش می‌دهد و بر می‌گرداند. نباید به طور مستقیم فراخانی شود؛ به‌جای آن از smart pointers یا ThisProtector استفاده کنید. |
...
| Field | Description |
| --- | --- |
| static [Empty](../../system/eventargs/empty/) | یک عضو ثابت که یک اشاره‌گر مشترک "خالی" [EventArgs](../../system/eventargs/) (null-pointer) را نماینده می‌شود. |

## موارد مرتبط

* کلاس [CancelEventArgs](../canceleventargs/)
