...
description: "Καθορίζει εάν ένα socket θα παραμείνει συνδεδεμένο μετά από κλήση στις μεθόδους Close() ή Close(). Επίσης καθορίζει την περίοδο που το socket θα παραμείνει συνδεδεμένο εάν η αποστολή των δεδομένων συνεχιστεί. Αντικείμενα αυτής της κλάσης πρέπει να εκχωρούνται μόνο χρησιμοποιώντας τη συνάρτηση System::MakeObject(). Ποτέ μην δημιουργείτε παρουσία αυτού του τύπου στη στοίβα ή χρησιμοποιώντας τον τελεστή new, καθώς θα οδηγήσει σε σφάλματα χρόνου εκτέλεσης και/ή σφάλματα ελέγχου. Πάντα τυλίξτε αυτήν την κλάση σε δείκτη System::SmartPtr και χρησιμοποιήστε αυτόν τον δείκτη για να τον περάσετε σε συναρτήσεις ως όρισμα."
type: docs
weight: 27
url: /el/system.net.sockets/lingeroption/
---
## LingerOption κλάση

Καθορίζει εάν ένα socket θα παραμείνει συνδεδεμένο μετά από κλήση στις μεθόδους Close() ή Close(). Επίσης καθορίζει την περίοδο που το socket θα παραμείνει συνδεδεμένο εάν η αποστολή των δεδομένων συνεχιστεί. Αντικείμενα αυτής της κλάσης πρέπει να εκχωρούνται μόνο χρησιμοποιώντας τη συνάρτηση [System::MakeObject()](../../system/makeobject/). Ποτέ μην δημιουργείτε παρουσία αυτού του τύπου στη στοίβα ή χρησιμοποιώντας τον τελεστή new, καθώς θα προκαλέσει σφάλματα χρόνου εκτέλεσης και/ή σφαλμάτων ελέγχου. Πάντα τυλίξτε αυτήν την κλάση σε δείκτη [System::SmartPtr](../../system/smartptr/) και χρησιμοποιήστε αυτόν τον δείκτη για να τον περάσετε σε συναρτήσεις ως όρισμα.
...
| **bool** [get_Enabled](./get_enabled/)() | Λαμβάνει μια τιμή που υποδεικνύει εάν το socket θα καθυστερήσει το κλείσιμο σε προσπάθεια αποστολής όλων των εκκρεμών δεδομένων. |
| **int32_t** [get_LingerTime](./get_lingertime/)() | Λαμβάνει το χρονικό όριο καθυστέρησης σε δευτερόλεπτα. |
...
| void [set_Enabled](./set_enabled/)(**bool**) | Ορίζει μια τιμή που υποδεικνύει εάν το socket θα καθυστερήσει το κλείσιμο σε προσπάθεια αποστολής όλων των εκκρεμών δεδομένων. |
| void [set_LingerTime](./set_lingertime/)(**int32_t**) | Ορίζει το χρονικό όριο καθυστέρησης σε δευτερόλεπτα. |
