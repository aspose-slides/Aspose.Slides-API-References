...
---
## IRevealTransition κλάση

Ανακάλυψη εφέ μετάβασης διαφάνειας.
...
| virtual [TransitionLeftRightDirectionType](../transitionleftrightdirectiontype/) [get_Direction](./get_direction/)() | Κατεύθυνση της μετάβασης. Ανάγνωση [TransitionLeftRightDirectionType](../transitionleftrightdirectiontype/). |
| virtual **bool** [get_ThroughBlack](./get_throughblack/)() | Καθορίζει αν η μετάβαση εξασθενεί μέσω μαύρου. Ανάγνωση **bool**. |
...
| virtual **int32_t** [GetHashCode](../../system/object/gethashcode/)() const | Αντίστοιχο της μεθόδου C# [Object.GetHashCode()](../../system/object/gethashcode/). Ενεργοποιεί την κατασκευή hash προσαρμοσμένων αντικειμένων. |
...
| virtual void [set_Direction](./set_direction/)([TransitionLeftRightDirectionType](../transitionleftrightdirectiontype/)) | Κατεύθυνση της μετάβασης. Εγγραφή [TransitionLeftRightDirectionType](../transitionleftrightdirectiontype/). |
| virtual void [set_ThroughBlack](./set_throughblack/)(**bool**) | Καθορίζει αν η μετάβαση εξασθενεί μέσω μαύρου. Εγγραφή **bool**. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει το n'th όρισμα προτύπου ως αδυνατό δείκτη (αντί για κοινό). Επιτρέπει την εναλλαγή δεικτών σε containers σε αδύνατη λειτουργία. |
| int [SharedCount](../../system/object/sharedcount/)() const | Λαμβάνει την τρέχουσα τιμή του κοινόμεσου μετρητή αναφορών. |
| [Object](../../system/object/) * [SharedRefAdded](../../system/object/sharedrefadded/)() | Αυξάνει τον κοινόμεσο μετρητή αναφορών. Δεν πρέπει να κληθεί άμεσα· αντί για αυτό, χρησιμοποιήστε έξυπνους δεικτικούς ή το ThisProtector. |
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | Μειώνει και επιστρέφει τον κοινόμεσο μετρητή αναφορών. Δεν πρέπει να κληθεί άμεσα· αντί για αυτό, χρησιμοποιήστε έξυπνους δεικτικούς ή το ThisProtector. |
...
| void [Unlock](../../system/object/unlock/)() | Υλοποιεί το statement lock() της C# για ξεγρίψιμο. Κλήση άμεσα ή χρήση του αντικειμένου sentry [LockContext](../../system/lockcontext/). |
| Detail::SmartPtrCounter * [WeakRefAdded](../../system/object/weakrefadded/)() | Αυξάνει τον αδύνατο μετρητή αναφορών. Δεν πρέπει να κληθεί άμεσα· αντί για αυτό, χρησιμοποιήστε έξυπνους δεικτικούς ή το ThisProtector. |
| void [WeakRefRemoved](../../system/object/weakrefremoved/)() | Μειώνει τον αδύνατο μετρητή αναφορών. Δεν πρέπει να κληθεί άμεσα· αντί για αυτό, χρησιμοποιήστε έξυπνους δεικτικούς ή το ThisProtector. |
