...
description: Αναπαριστά το σημείο ενός chert.
type: docs
weight: 1301
...
Αναπαριστά το σημείο ενός chert.
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | Προσομοιώνει τη σύγκριση κινητής υποδιαστολής σε στυλ C# όπου δύο NaNs θεωρούνται ίσα παρόλο που σύμφωνα με IEC 60559:1989 το NaN δεν είναι ίσο με κάποια τιμή, συμπεριλαμβανομένου του NaN. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | Προσομοιώνει τη σύγκριση κινητής υποδιαστολής σε στυλ C# όπου δύο NaNs θεωρούνται ίσα παρόλο που σύμφωνα με IEC 60559:1989 το NaN δεν είναι ίσο με κάποια τιμή, συμπεριλαμβανομένου του NaN. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | Μόνο για εσωτερική χρήση. |
| [System::SharedPtr](../../system/sharedptr/)\<[IFormat](../iformat/)\> [get_Format](./get_format/)() override | Λαμβάνει το γέμισμα του δείκτη. Μόνο-ανάγνωση [IFormat](../iformat/). |
| **int32_t** [get_Size](./get_size/)() override | Αναπαριστά το μέγεθος του δείκτη σε γράφημα γραμμής, διάσπαρτης ή ράντας. Ανάγνωση **int32_t**. |
| [MarkerStyleType](../markerstyletype/) [get_Symbol](./get_symbol/)() override | Αναπαριστά το στυλ του δείκτη σε γράφημα γραμμής, διάσπαρτης ή ράντας. Ανάγνωση [MarkerStyleType](../markerstyletype/). |
...
| void [set_Size](./set_size/)(**int32_t**) override | Αναπαριστά το μέγεθος του δείκτη σε γράφημα γραμμής, διάσπαρτης ή ράντας. Εγγραφή **int32_t**. |
| void [set_Symbol](./set_symbol/)([MarkerStyleType](../markerstyletype/)) override | Αναπαριστά το στυλ του δείκτη σε γράφημα γραμμής, διάσπαρτης ή ράντας. Εγγραφή [MarkerStyleType](../markerstyletype/). |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει το n-οστό όρισμα προτύπου ως αδύναμο δείκτη (αντί για κοινό). Επιτρέπει την εναλλαγή δεικτών σε περιέκτες σε αδύναμη κατάσταση. |
| int [SharedCount](../../system/object/sharedcount/)() const | Λαμβάνει την τρέχουσα τιμή του κοινόμετρου αναφοράς. |
| [Object](../../system/object/) * [SharedRefAdded](../../system/object/sharedrefadded/)() | Αυξάνει το κοινόμετρο αναφοράς. Δεν πρέπει να κληθεί άμεσα· αντί αυτού, χρησιμοποιήστε έξυπνους δείκτες ή ThisProtector. |
