---
title: GetRange()
second_title: Aspose.Slides για C++ – Αναφορά API
description: Αποκτά την περιοχή δεδομένων του διαγράμματος.
type: docs
weight: 170
url: /el/aspose.slides.charts/ichartdata/getrange/
---
## IChartData::GetRange() μέθοδος

Αποκτά την περιοχή δεδομένων του διαγράμματος.

```cpp
virtual System::String Aspose::Slides::Charts::IChartData::GetRange()=0
```

### Τιμή επιστροφής

Formula της περιοχής δεδομένων των κελιών. Π.χ.: "Sheet1!$A$1:$C$4"

## Παρατηρήσεις

```cpp
auto pres = MakeObject<Presentation>();
auto chart = pres->get_Slides()->idx_get(0)->get_Shapes()->AddChart(Charts::ChartType::PercentsStackedBar, 100.0f, 100.0f, 500.0f, 400.0f);
String result = (AsCast<Charts::ChartData>(chart->get_ChartData()))->GetRange();
```

## Δείτε επίσης

* Κλάση [String](../../../system/string/)
* Κλάση [IChartData](../)
* Χώρος ονομάτων [Aspose::Slides::Charts](../../)
* Βιβλιοθήκη [Aspose.Slides](../../../)