...
description: Αντιπροσωπεύει μια προειδοποίηση σχετικά με γνωστή μη υλοποιημένη λειτουργία που δεν θα υλοποιηθεί στο εγγύς μέλλον.
type: docs
weight: 14
url: /el/aspose.slides.warnings/inotimplementedwarninginfo/
---
## INotImplementedWarningInfo κλάση

Αντιπροσωπεύει μια προειδοποίηση σχετικά με γνωστή μη υλοποιημένη δυνατότητα που δεν θα υλοποιηθεί στο εγγύς μέλλον.
...
| virtual [System::String](../../system/string/) [get_Description](../iwarninginfo/get_description/)() | Επιστρέφει μια ανθρώπινα αναγνώσιμη περιγραφή αυτής της προειδοποίησης. Μόνο ανάγνωση [System::String](../../system/string/). |
| virtual [Aspose::Slides::Warnings::WarningType](../warningtype/) [get_WarningType](../iwarninginfo/get_warningtype/)() | Επιστρέφει έναν τύπο προειδοποίησης. Μόνο ανάγνωση [IWarningInfo::get_WarningType](../iwarninginfo/get_warningtype/). |
...
| [Object](../../system/object/object/)([Object](../../system/object/) const\&) | Κατασκευαστής αντιγραφής. Στην πραγματικότητα δεν αντιγράφει τίποτα, απλώς αρχικοποιεί νέο αντικείμενο και ενεργοποιεί τη δημιουργία αντιγράφων σε υποκατηγορίες. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | Τελεστή ανάθεσης. Στην πραγματικότητα δεν αντιγράφει τίποτα, απλώς αρχικοποιεί νέο αντικείμενο και ενεργοποιεί τη δημιουργία αντιγράφων σε υποκατηγορίες. |
...
| virtual void [SendWarning](../iwarninginfo/sendwarning/)([System::SharedPtr](../../system/sharedptr/)\<[IWarningCallback](../iwarningcallback/)\>) | Εάν ο παραλήπτης δεν είναι null, λήγει την προειδοποίηση σε έναν καθορισμένο παραλήπτη και ρίχνει την AbortRequestedException αν ο παραλήπτης αποφάσισε να ακυρώσει μια λειτουργία. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει το n'th ορίσμα προτύπου ως αδύναμο δείκτη (αντί για κοινόμο). Επιτρέπει την αλλαγή δεικτών σε containers σε αδύναμη λειτουργία. |
