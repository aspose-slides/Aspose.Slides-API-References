...
---
## IObsoletePresLockingBehaviorWarningInfo κλάση
...
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | Λαμβάνει τη δομή δεδομένων καταγραφής αναφορών που σχετίζεται με το αντικείμενο. |
...
| virtual void [SendWarning](../iwarninginfo/sendwarning/)([System::SharedPtr](../../system/sharedptr/)\<[IWarningCallback](../iwarningcallback/)\>) | Εάν ο παραλήπτης δεν είναι null, τερματίζει την προειδοποίηση σε έναν καθορισμένο παραλήπτη και ρίχνει την εξαίρεση AbortRequestedException εάν ο παραλήπτης αποφάσισε να ακυρώσει μια λειτουργία. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει το ν-οστό όρισμα του προτύπου ως αδύναμη δείκτη (αντί για κοινόχρηστο). Επιτρέπει την εναλλαγή δεικτών σε containers σε αδύναμη λειτουργία. |
...
* Ονομαχώρος [Aspose::Slides::Warnings](../)
