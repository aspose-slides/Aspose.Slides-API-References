...
description: Αναπαριστά την εξαίρεση που δημιουργείται όταν σφάλματα και προειδοποιήσεις επικύρωσης σχήματος XML Schema Definition Language (XSD) εντοπίζονται σε ένα έγγραφο XML που υποβάλλεται σε επικύρωση.
type: docs
weight: 27
url: /el/system.xml.schema/details_xmlschemavalidationexception/
---
## Details_XmlSchemaValidationException κλάση

Αναπαριστά την εξαίρεση που δημιουργείται όταν σφάλματα και προειδοποιήσεις επικύρωσης σχήματος XML [Schema](../) Definition Language (XSD) εντοπίζονται σε ένα έγγραφο XML που υποβάλλεται σε επικύρωση.
...
| **int32_t** [get_LinePosition](../details_xmlschemaexception/get_lineposition/)() | Επιστρέφει τη θέση στη γραμμή που υποδεικνύει πού συνέβη το σφάλμα. |
...
| [SharedPtr](../../system/sharedptr/)\<[Object](../../system/object/)\> [get_SourceObject](./get_sourceobject/)() | Επιστρέφει τον κόμβο XML που προκάλεσε αυτή την XmlSchemaValidationException. |
...
| const [System::TypeInfo](../../system/typeinfo/)\& [GetType](../../system/details_systemexception/gettype/)() const override | Αποκτά τον πραγματικό τύπο του αντικειμένου. Αναλογία κλήσης C# [System.Object.GetType()](../../system/object/gettype/). |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | Κατασκευαστής αντιγραφής. Στην πραγματικότητα δεν αντιγράφει τίποτα, μόνο αρχικοποιεί νέο αντικείμενο και επιτρέπει τη δημιουργία αντιγράφων υποκλάσεων. |
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | Τελεστής ανάθεσης. Στην πραγματικότητα δεν αντιγράφει τίποτα, μόνο αρχικοποιεί νέο αντικείμενο και επιτρέπει τη δημιουργία αντιγράφων υποκλάσεων. |
...
| void [WeakRefRemoved](../../system/object/weakrefremoved/)() | Μειώνει τον αδύναμο μετρητή αναφοράς. Δεν πρέπει να καλείται απευθεία· χρησιμοποιήστε έξυπνους δείκτες ή ThisProtector. |
| virtual const char * [what](../../system/details_exception/what/)() const | Υλοποιεί τη μέθοδο [what()](../../system/details_exception/what/) η οποία καλείται από την κλάση [ExceptionWrapper](../../system/exceptionwrapper/). Παρά το γεγονός ότι αυτή η κλάση δεν κληρονομεί από std::exception, οι παράγωγες κλάσεις μπορούν να χρησιμοποιήσουν προστατευμένα/ιδιωτικά μέλη για να υλοποιήσουν τη λογική τους. Η μετακίνηση της υλοποίησης αυτής της μεθόδου στο [ExceptionWrapper](../../system/exceptionwrapper/) μπορεί να διαταράξει αυτή τη λογική. |
...
| --- | --- |
| [Ptr](./ptr/) | Ένα ψευδώνυμο για κοινό δείκτη σε ένα παράδειγμα αυτής της κλάσης. |
