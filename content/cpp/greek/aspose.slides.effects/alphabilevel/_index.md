...
description: Απεικονίζει ένα εφέ Alpha Bi-Level. Τιμές Alpha (Διαφάνεια) μικρότερες από το όριο αλλάζονται σε 0 (πλήρως διαφανές) και τιμές alpha μεγαλύτερες ή ίσες με το όριο αλλάζονται στο 100% (πλήρως αδιαφανές).
type: docs
weight: 1
...
| Method | Description |
| --- | --- |
| **bool** [Equals](./equals/)([System::SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\>) override | Καθορίζει εάν το καθορισμένο [AlphaBiLevel](./) είναι ίσο με το τρέχον [AlphaBiLevel](./). |
...
| **float** [get_Threshold](./get_threshold/)() override | Επιστρέφει το όριο του εφέ. Διαβάζει **float**. |
...
| [System::SharedPtr](../../system/sharedptr/)\<[IAlphaBiLevelEffectiveData](../ialphabileveleffectivedata/)\> [GetEffective](./geteffective/)() override | Λαμβάνει τα δεδομένα του αποτελεσματικού εφέ Alpha Bi-Level με την κληρονομικότητα εφαρμοσμένη. |
...
| void [Lock](../../system/object/lock/)() | Εφαρμόζει το statement lock() της C#. Κάλεσε απευθείας ή χρησιμοποίησε το αντικείμενο sentry [LockContext](../../system/lockcontext/). |
...
| [Object](../../system/object/object/)([Object](../../system/object/) const\&) | Κατασκευαστής αντιγραφής. Δεν αντιγράφει τίποτα, πραγματικά, απλώς αρχικοποιεί νέο αντικείμενο και ενεργοποιεί την αντιγραφή κατασκευής υποκατηγοριών. |
...
| void [set_Threshold](./set_threshold/)(**float**) override | Επιστρέφει το όριο του εφέ. Εγγραφή **float**. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει το n'th όρισμα προτύπου ως αδυνατό δείκτη (αντί για κοινό). Επιτρέπει την αλλαγή δεικτών σε συλλογές σε αδυνατό τρόπο. |
| int [SharedCount](../../system/object/sharedcount/)() const | Λαμβάνει την τρέχουσα τιμή του μετρητή κοινών αναφορών. |
| [Object](../../system/object/) * [SharedRefAdded](../../system/object/sharedrefadded/)() | Αυξάνει το μετρητή κοινών αναφορών. Δεν θα πρέπει να κληθεί άμεσα· αντίθετα, χρησιμοποιήστε έξυπνους δείκτες ή ThisProtector. |
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | Μειώνει και επιστρέφει το μετρητή κοινών αναφορών. Δεν θα πρέπει να κληθεί άμεσα· αντίθετα, χρησιμοποιήστε έξυπνους δείκτες ή ThisProtector. |
...
| void [Unlock](../../system/object/unlock/)() | Υλοποιεί το statement lock() της C# για ξεκλείδωμα. Κάλεσε απευθείας ή χρησιμοποίησε το αντικείμενο sentry [LockContext](../../system/lockcontext/). |
| Detail::SmartPtrCounter * [WeakRefAdded](../../system/object/weakrefadded/)() | Αυξάνει το μετρητή αδυνατών αναφορών. Δεν θα πρέπει να κληθεί άμεσα· αντίθετα, χρησιμοποιήστε έξυπνους δείκτες ή ThisProtector. |
| void [WeakRefRemoved](../../system/object/weakrefremoved/)() | Μειώνει το μετρητή αδυνατών αναφορών. Δεν θα πρέπει να κληθεί άμεσα· αντίθετα, χρησιμοποιήστε έξυπνους δείκτες ή ThisProtector. |
...
* Κλάση [IAlphaBiLevel](../ialphabilevel/)
