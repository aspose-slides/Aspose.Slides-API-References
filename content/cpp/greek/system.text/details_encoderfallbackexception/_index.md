...
description: "Εξαίρεση που ρίχνεται από EncoderExceptionFallback όταν η κωδικοποίηση αποτυγχάνει. Ποτέ μην δημιουργείτε στιγμές αυτής της κλάσης χειροκίνητα. Χρησιμοποιήστε την κλάση EncoderFallbackException αντί αυτού. Ποτέ μην ενσωματώνετε τις στιγμές της κλάσης EncoderFallbackException στο System::SmartPtr."
type: docs
weight: 118
url: /el/system.text/details_encoderfallbackexception/
---
## Details_EncoderFallbackException κλάση

Εξαίρεση που ρίχνεται από [EncoderExceptionFallback](../encoderexceptionfallback/) όταν αποτυγχάνει η κωδικοποίηση. Ποτέ μην δημιουργείτε στιγμές αυτής της κλάσης χειροκίνητα. Χρησιμοποιήστε την κλάση EncoderFallbackException αντί αυτού. Ποτέ μην ενσωματώνετε τις στιγμές της κλάσης EncoderFallbackException στο [System::SmartPtr](../../system/smartptr/).
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | Εξομοιώνει τη σύγκριση κινητών σημείων σε στυλ C# όπου δύο NaN θεωρούνται ίσα, παρόλο που σύμφωνα με το IEC 60559:1989 το NaN δεν είναι ίσο με καμία τιμή, συμπεριλαμβανομένου του NaN. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | Εξομοιώνει τη σύγκριση κινητών σημείων σε στυλ C# όπου δύο NaN θεωρούνται ίσα, παρόλο που σύμφωνα με το IEC 60559:1989 το NaN δεν είναι ίσο με καμία τιμή, συμπεριλαμβανομένου του NaN. |
| virtual **bool** [FastCast](../../system/object/fastcast/)(const Details::FastRttiBase\&, void **) const | Μόνο για εσωτερική χρήση. |
| char_t [get_CharUnknown](./get_charunknown/)() | Αποκτά το χαρακτήρα που προκάλεσε το σφάλμα. |
| char_t [get_CharUnknownHigh](./get_charunknownhigh/)() | Αποκτά τον υψηλό χαρακτήρα του ζεύγους που προκάλεσε το σφάλμα. |
| char_t [get_CharUnknownLow](./get_charunknownlow/)() | Αποκτά τον χαμηλό χαρακτήρα του ζεύγους που προκάλεσε το σφάλμα. |
...
| **int32_t** [get_HResult](../../system/details_exception/get_hresult/)() const | Επιστρέφει τιμή 32-bit ακεραίου που είναι κωδικός HRESULT συνδεδεμένος με την εξαίρεση που αντιπροσωπεύεται από το τρέχον αντικείμενο. |
| int [get_Index](./get_index/)() | Αποκτά τη θέση του χαρακτήρα που προκάλεσε το σφάλμα στον πίνακα εισόδου. |
...
| virtual [String](../../system/string/) [get_StackTrace](../../system/details_exception/get_stacktrace/)() const | Επιστρέφει τη συμβολοσειρά που περιέχει το αρχείο καταγραφής στοίβας. |
...
| const [System::TypeInfo](../../system/typeinfo/)\& [GetType](../../system/details_argumentexception/gettype/)() const override | Αποκτά τον πραγματικό τύπο του αντικειμένου. Αναλογία κλήσης C# [System.Object.GetType()](../../system/object/gettype/). |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει το n-οστό όρισμα προτύπου ως αδύναμη αναφορά (αντί για κοινόχρηστη). Επιτρέπει την αλλαγή δεικτών σε containers σε αδύναμη λειτουργία. |
...
| virtual const char * [what](../../system/details_exception/what/)() const | Υλοποιεί τη μέθοδο [what()](../../system/details_exception/what/) η οποία καλείται από την κλάση [ExceptionWrapper](../../system/exceptionwrapper/). Παρά το γεγονός ότι αυτή η κλάση δεν κληρονομείται από std::exception, παράγωγες κλάσεις μπορούν να χρησιμοποιήσουν προστατευμένα/ιδιωτικά μέλη για να υλοποιήσουν τη λογική τους. Η μεταφορά αυτής της υλοποίησης μεθόδου στο [ExceptionWrapper](../../system/exceptionwrapper/) μπορεί να σπάσει αυτή τη λογική. |
