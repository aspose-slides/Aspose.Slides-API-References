...
description: "Παρέχει API για υπογραφή αντικειμένων χρησιμοποιώντας CMS. Δεν υπογράφει τα αντικείμενα από μόνο του· χρησιμοποιήστε την κλάση SignedCMS για αυτό. Δεν υλοποιείται. Τα αντικείμενα αυτής της κλάσης πρέπει να δημιουργούνται μόνο με τη συνάρτηση System::MakeObject(). Ποτέ μην δημιουργείτε μια παρουσία αυτού του τύπου στο stack ή με τον τελεστή new, καθώς θα προκαλέσει σφάλματα χρόνου εκτέλεσης και/ή σφάλματα ασφάλισης. Πάντα τυλίξτε αυτήν την κλάση σε δείκτη System::SmartPtr και χρησιμοποιήστε αυτόν τον δείκτη για να το περάσετε στις συναρτήσεις ως όρισμα."
type: docs
weight: 1
url: /el/system.security.cryptography.pkcs/cmssigner/
---
## CmsSigner κλάση

Παρέχει API για υπογραφή αντικειμένων χρησιμοποιώντας CMS. Δεν υπογράφει τα αντικείμενα από μόνο του· χρησιμοποιήστε την κλάση SignedCMS για αυτό. Δεν υλοποιείται. Τα αντικείμενα αυτής της κλάσης πρέπει να δημιουργούνται μόνο με τη συνάρτηση [System::MakeObject()](../../system/makeobject/). Ποτέ μην δημιουργείτε ένα στιγμιότυπο αυτού του τύπου στο stack ή με τον τελεστή new, καθώς θα προκαλέσει σφάλματα χρόνου εκτέλεσης και/ή σφάλματα ασφάλισης. Πάντα τυλίξτε αυτήν την κλάση σε δείκτη [System::SmartPtr](../../system/smartptr/) και χρησιμοποιήστε αυτόν τον δείκτη για να το περάσετε στις συναρτήσεις ως όρισμα.
...
| Method | Description |
| --- | --- |
|  [CmsSigner](./cmssigner/)(const [SharedPtr](../../system/sharedptr/)\<[X509Certificates::X509Certificate2](../../system.security.cryptography.x509certificates/x509certificate2/)\>\&) | Αρχικοποιεί το υπογράφον με πιστοποιητικό X509. |
...
| [SharedPtr](../../system/sharedptr/)\<[Oid](../../system.security.cryptography/oid/)\> [get_DigestAlgorithm](./get_digestalgorithm/)() const | Λαμβάνει τον αλγόριθμο κατακερματισμού που χρησιμοποιείται με την υπογραφή. |
| [X509Certificates::X509IncludeOption](../../system.security.cryptography.x509certificates/x509includeoption/) [get_IncludeOption](./get_includeoption/)() const | Ελέγχει ποια πιστοποιητικά από την αλυσίδα θα συμπεριληφθούν στην υπογραφή. |
...
| virtual **int32_t** [GetHashCode](../../system/object/gethashcode/)() const | Αναλογικό της μεθόδου C# [Object.GetHashCode()](../../system/object/gethashcode/). Ενεργοποιεί τον κατακερματισμό προσαρμοσμένων αντικειμένων. |
...
| void [Lock](../../system/object/lock/)() | Υλοποιεί την κλειδωτική δήλωση C# lock(). Κλήση απευθείας ή χρήση του αντικειμένου φρουρού [LockContext](../../system/lockcontext/). |
| virtual [ptr](../../system/object/ptr/) [MemberwiseClone](../../system/object/memberwiseclone/)() const | Αναλογικό της μεθόδου C# [Object.MemberwiseClone()](../../system/object/memberwiseclone/). Ενεργοποιεί την κλωνοποίηση προσαρμοσμένων τύπων. |
|  [Object](../../system/object/object/)() | Δημιουργεί αντικείμενο. Αρχικοποιεί όλες τις εσωτερικές δομές δεδομένων. |
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | Κατασκευή αντιγράφου. Στην πραγματικότητα δεν αντιγράφει τίποτα, απλώς αρχικοποιεί νέο αντικείμενο και επιτρέπει την κατασκευή αντιγράφων των υποκλάσεων. |
...
| void [set_DigestAlgorithm](./set_digestalgorithm/)(const [SharedPtr](../../system/sharedptr/)\<[Oid](../../system.security.cryptography/oid/)\>\&) | Ορίζει τον αλγόριθμο κατακερματισμού που χρησιμοποιείται με την υπογραφή. |
| void [set_IncludeOption](./set_includeoption/)([X509Certificates::X509IncludeOption](../../system.security.cryptography.x509certificates/x509includeoption/)) | Καθορίζει ποια πιστοποιητικά από την αλυσίδα θα συμπεριληφθούν στην υπογραφή. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει το n'th όρισμα προτύπου σε αδύναμο δείκτη (αντί για κοινό). Επιτρέπει την αλλαγή δεικτών στα containers σε αδύναμη λειτουργία. |
...
* Χώρος ονομάτων [System::Security::Cryptography::Pkcs](../)
