...
description: "Δεδομένα κωδικοποιημένα σε ASN.1. Τα αντικείμενα αυτής της κλάσης πρέπει να δημιουργούνται μόνο χρησιμοποιώντας τη συνάρτηση System::MakeObject(). Ποτέ μην δημιουργείτε μια παρουσία αυτού του τύπου στη στοίβα ή χρησιμοποιώντας τον τελεστή new, καθώς θα οδηγήσει σε σφάλματα χρόνου εκτέλεσης και/ή σφάλματα αστοχίας. Πάντα να τυλίγετε αυτήν την κλάση σε δείκτη System::SmartPtr και να χρησιμοποιείτε αυτόν τον δείκτη για να τη μεταβιβάζετε σε συναρτήσεις ως όρισμα."
type: docs
weight: 1
url: /el/system.security.cryptography/asnencodeddata/
---
## AsnEncodedData κλάση

ASN.1-encoded data. Objects of this κλάσης should only be allocated using [System::MakeObject()](../../system/makeobject/) function. Never create instance of this type on stack or using operator new, as it will result in runtime errors and/or assertion faults. Always wrap this κλάση into [System::SmartPtr](../../system/smartptr/) pointer and use this pointer to pass it to functions as argument.
...
| Μέθοδος | Περιγραφή |
| --- | --- |
|  [AsnEncodedData](./asnencodeddata/)(const [SharedPtr](../../system/sharedptr/)\<[AsnEncodedData](./)\>\&) | Κατασκευαστής αντιγραφής. |
|  [AsnEncodedData](./asnencodeddata/)(const [ByteArrayPtr](../../system/bytearrayptr/)\&) | Κατασκευαστής. |
|  [AsnEncodedData](./asnencodeddata/)(const [SharedPtr](../../system/sharedptr/)\<[Oid](../oid/)\>\&, const [ByteArrayPtr](../../system/bytearrayptr/)\&) | Κατασκευαστής. |
|  [AsnEncodedData](./asnencodeddata/)(const [String](../../system/string/)\&, const [ByteArrayPtr](../../system/bytearrayptr/)\&) | Κατασκευαστής. |
| virtual void [CopyFrom](./copyfrom/)(const [SharedPtr](../../system/sharedptr/)\<[AsnEncodedData](./)\>\&) | Αντιγράφει δεδομένα από διαφορετικό αντικείμενο. |
...
| virtual [String](../../system/string/) [Format](./format/)(**bool**) const | Μορφοποιεί τα δεδομένα σε αναγνώσιμη μορφή. |
| [SharedPtr](../../system/sharedptr/)\<[Oid](../oid/)\> [get_Oid](./get_oid/)() const | Λαμβάνει το αναγνωριστικό αντικειμένου των κωδικοποιημένων δεδομένων. |
| [ByteArrayPtr](../../system/bytearrayptr/) [get_RawData](./get_rawdata/)() const | Λαμβάνει τα ακατέργαστα κωδικοποιημένα δεδομένα. |
...
| void [Lock](../../system/object/lock/)() | Υλοποιεί την κλειδώση της δήλωσης C# lock(). Καλείται απευθείας ή χρησιμοποιήστε το αντικείμενο φρουρού [LockContext](../../system/lockcontext/). |
...
| void [set_Oid](./set_oid/)(const [SharedPtr](../../system/sharedptr/)\<[Oid](../oid/)\>\&) | Ορίζει το αναγνωριστικό αντικειμένου των κωδικοποιημένων δεδομένων. |
| void [set_RawData](./set_rawdata/)(const [ByteArrayPtr](../../system/bytearrayptr/)\&) | Ορίζει τα ακατέργαστα κωδικοποιημένα δεδομένα. |
