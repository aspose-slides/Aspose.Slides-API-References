...
description: "Κωδικός ελεγχοποιητικού μηνύματος βασισμένος σε hash που χρησιμοποιεί τη συνάρτηση hash SHA1. Δεν υλοποιήθηκε. Τα αντικείμενα αυτής της κλάσης πρέπει να δημιουργούνται μόνο μέσω της συνάρτησης System::MakeObject(). Ποτέ μην δημιουργείτε αντίτυπο αυτού του τύπου στη στοίβα ή χρησιμοποιώντας τον τελεστή new, καθώς θα προξενήσει σφάλματα χρόνου εκτέλεσης και/ή αποτυχίες ελέγχων. Πάντοτε τυλίξτε αυτήν την κλάση σε δείκτη System::SmartPtr και χρησιμοποιήστε αυτόν τον δείκτη για να το περάσετε ως όρισμα σε συναρτήσεις."
type: docs
weight: 248
url: /el/system.security.cryptography/hmacsha1/
---
## HMACSHA1 κλάση
...
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [ComputeHash](../hashalgorithm/computehash/)([SharedPtr](../../system/sharedptr/)\<[IO::Stream](../../system.io/stream/)\> const\&) | Αναγίνει ροή μέχρι το τέλος και υπολογίζει το hash των δεδομένων που διαβάστηκαν. |
| static [SharedPtr](../../system/sharedptr/)\<[HashAlgorithm](../hashalgorithm/)\> [Create](../hashalgorithm/create/)(const [String](../../system/string/)\&) | Δημιουργεί αλγόριθμο hash βάσει ονόματος. |
...
| virtual [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_Hash](../hashalgorithm/get_hash/)() | Παίρνει την τιμή του υπολογισμένου κώδικα hash. |
| virtual int [get_HashSize](../hashalgorithm/get_hashsize/)() | Παίρνει το μέγεθος της υπολογισμένης τιμής hash σε bytes. |
...
|  [HMACSHA1](./hmacsha1/)(const [System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>\&) | Κατασκευαστής. |
| virtual void [Initialize](../hashalgorithm/initialize/)() | Επαναφέρει το hasher στην αρχική κατάσταση. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει το n'th όρισμα πρότυπου ως αδυνατό δείκτη (αντί για κοινό). Επιτρέπει την εναλλαγή δεικτών σε containers σε αδυνατή λειτουργία. |
...
| int [TransformBlock](../hashalgorithm/transformblock/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int) override | Επεξεργάζεται μπλοκ δεδομένων και αντιγράφει τα δεδομένα στον πίνακα εξόδου. |
...
| void [Unlock](../../system/object/unlock/)() | Υλοποιεί το statement ξεκλειδώματος C# lock(). Κλήση απευθείας ή χρήση του αντικειμένου φρουρού [LockContext](../../system/lockcontext/). |
| Detail::SmartPtrCounter * [WeakRefAdded](../../system/object/weakrefadded/)() | Αυξάνει τον μετρητή αδυνατής αναφοράς. Δεν πρέπει να καλείται απευθείας· χρησιμοποιήστε έξυπνους δείκτες ή ThisProtector. |
| void [WeakRefRemoved](../../system/object/weakrefremoved/)() | Μειώνει τον μετρητή αδυνατής αναφοράς. Δεν πρέπει να καλείται απευθείας· χρησιμοποιήστε έξυπνους δείκτες ή ThisProtector. |
