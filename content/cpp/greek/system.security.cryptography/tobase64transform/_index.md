...
description: "Μετατρέπει το στιγμιότυπο της κλάσης CryptoStream σε base 64. Τα αντικείμενα αυτής της κλάσης πρέπει να εκχωρούνται μόνο χρησιμοποιώντας τη συνάρτηση System::MakeObject(). Ποτέ μην δημιουργείτε ένα στιγμιότυπο αυτού του τύπου στο stack ή χρησιμοποιώντας τον τελεστή new, καθώς θα προκαλέσει σφάλματα χρόνου εκτέλεσης και/ή σφάλματα ελέγχου. Πάντα τυλίξτε αυτήν την κλάση σε δείκτη System::SmartPtr και χρησιμοποιήστε αυτόν τον δείκτη για να τον περάσετε σε συναρτήσεις ως όρισμα."
type: docs
weight: 664
url: /el/system.security.cryptography/tobase64transform/
---
## ToBase64Transform κλάση
...
| virtual **bool** [get_CanReuseTransform](./get_canreusetransform/)() | Επιστρέφει μια τιμή που υποδεικνύει αν η τρέχουσα μετατροπή μπορεί να επαναχρησιμοποιηθεί. |
| **bool** [get_CanTransformMultipleBlocks](./get_cantransformmultipleblocks/)() | Επιστρέφει μια τιμή που υποδεικνύει αν μπορούν να μετασχηματιστούν πολλαπλά μπλοκ. |
| virtual **int32_t** [get_InputBlockSize](./get_inputblocksize/)() | Μέγεθος εισαγωγικού μπλοκ. |
| virtual **int32_t** [get_OutputBlockSize](./get_outputblocksize/)() | Μέγεθος εξαγωγικού μπλοκ. |
...
| virtual **int32_t** [GetHashCode](../../system/object/gethashcode/)() const | Αναλογίας με τη μέθοδο C# [Object.GetHashCode()](../../system/object/gethashcode/). Ενεργοποιεί τη δημιουργία κατακερματισμού προσαρμοσμένων αντικειμένων. |
| virtual const [TypeInfo](../../system/typeinfo/)\& [GetType](../../system/object/gettype/)() const | Επιστρέφει τον πραγματικό τύπο του αντικειμένου. Αναλογία με την κλήση C# [System.Object.GetType()](../../system/object/gettype/). |
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | Ελέγχει αν το αντικείμενο αποτελεί εμφάνιση τύπου που περιγράφεται από το targetType. Αναλογίας με τον τελεστή C# 'is'. |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει το n'th όρισμα προτύπου ως αδύναμο δείκτη (αντί για κοινόχρηστο). Επιτρέπει την αλλαγή των δεικτών σε κοντέινερ σε αδύναμη λειτουργία. |
...
| **int32_t** [TransformBlock](./transformblock/)([System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, **int32_t**, **int32_t**, [System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, **int32_t**) | Επεξεργάζεται ένα μπλοκ δεδομένων και αντιγράφει τα δεδομένα σε πίνακα εξόδου. |
| virtual int [TransformBlock](../icryptotransform/transformblock/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int) | Επεξεργάζεται ένα μπλοκ δεδομένων και αντιγράφει τα δεδομένα σε πίνακα εξόδου. |
| [System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [TransformFinalBlock](./transformfinalblock/)([System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, **int32_t**, **int32_t**) | Επεξεργάζεται το τελευταίο μπλοκ δεδομένων και υπολογίζει την τιμή εξόδου. |
| virtual [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [TransformFinalBlock](../icryptotransform/transformfinalblock/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int) | Επεξεργάζεται το τελευταίο μπλοκ δεδομένων και υπολογίζει την τιμή εξόδου. |
