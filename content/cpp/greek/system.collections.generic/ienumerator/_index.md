...
description: "Διεπαφή του επαναλήπτη που μπορεί να χρησιμοποιηθεί για επανάληψη μέσω ορισμένων στοιχείων. Τα αντικείμενα αυτής της κλάσης πρέπει να εκχωρούνται μόνο χρησιμοποιώντας τη συνάρτηση System::MakeObject(). Ποτέ μην δημιουργείτε παρουσία αυτού του τύπου στο στοίβο ή χρησιμοποιώντας τον τελεστή new, καθώς αυτό θα οδηγήσει σε σφάλματα χρόνου εκτέλεσης και/ή σφάλματα ελέγχου. Πάντα να τυλίγετε αυτή την κλάση σε δείκτη System::SmartPtr και να χρησιμοποιείτε αυτόν τον δείκτη για τη μεταβίβαση του σε λειτουργίες ως όρισμα."
type: docs
weight: 300
url: /el/system.collections.generic/ienumerator/
---
## IEnumerator κλάση
...
| Method | Description |
| --- | --- |
| [IEnumerator](./) * [AsVirtualizedIterator](./asvirtualizediterator/)() | Προετοιμάζει τον επαναλήπτη για χρήση από την κλάση VirtualizedIterator. |
| System::Details::VirtualizedIteratorBase\<T\> * [CloneIterator](./cloneiterator/)() const override | Δημιουργεί αντίγραφο του τρέχοντος επαναλήπτη. |
| virtual [MakeConstRef_t](../../system/makeconstref_t/)\<T\> [Current](./current/)() const | Επιστρέφει το τρέχον στοιχείο. |
...
| virtual [MakeConstRef_t](../../system/makeconstref_t/)\<T\> [get_Current](./get_current/)() const | Επιστρέφει το τρέχον στοιχείο. |
...
| virtual **int32_t** [GetHashCode](../../system/object/gethashcode/)() const | Αναλογική με τη μέθοδο C# [Object.GetHashCode()](../../system/object/gethashcode/). Επιτρέπει τη δημιουργία κατακερματισμού προσαρμοσμένων αντικειμένων. |
...
| void [IncrementIterator](./incrementiterator/)() override | Μετακινεί τον επαναλήπτη ένα βήμα μπροστά. |
| void [InitializeIterator](./initializeiterator/)() override | Κάνει την πρώτη κλήση [MoveNext()](./movenext/) και προετοιμάζει το αντικείμενο enumerator για χρήση από το VirtualizedIterator. |
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | Ελέγχει αν το αντικείμενο αντιπροσωπεύει μια παρουσία τύπου που περιγράφεται από targetType. Αναλογική με το τελεστή C# 'is'. |
...
| void [MarkOwnedByVirtualizedIterator](./markownedbyvirtualizediterator/)() | Σημαδεύει τον enumerator που ανήκει στον virtualized iterator. |
...
| virtual **bool** [MoveNext](./movenext/)() | Μετακινεί τον enumerator στο επόμενο στοιχείο. Εάν δεν είχε προηγούμενα αναφερθεί στοιχείο, ορίζει αναφορά στο πρώτο διαθέσιμο στοιχείο. Εάν φτάσει στο τέλος του container, δεν κάνει τίποτα. |
...
| virtual void [Reset](./reset/)() | Επαναφέρει τον enumerator στη θέση πριν από το πρώτο στοιχείο. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει την n'tη παράμετρο προτύπου ως αδύναμη δείκτη (αντί για κοινόχρηστη). Επιτρέπει την αλλαγή δεικτών σε containers σε αδύναμη λειτουργία. |
| int [SharedCount](../../system/object/sharedcount/)() const | Επιστρέφει την τρέχουσα τιμή του μετρητή κοινόχρηστης αναφοράς. |
| [Object](../../system/object/) * [SharedRefAdded](../../system/object/sharedrefadded/)() | Αυξάνει τον μετρητή κοινόχρηστης αναφοράς. Δεν πρέπει να κληθεί άμεσα· αντί αυτού, χρησιμοποιήστε έξυπνους δείκτες ή ThisProtector. |
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | Μειώνει και επιστρέφει τον μετρητή κοινόχρηστης αναφοράς. Δεν πρέπει να κληθεί άμεσα· αντί αυτού, χρησιμοποιήστε έξυπνους δείκτες ή ThisProtector. |
...
int main()
{
  // Δημιουργήστε το αντικείμενο της κλάσης List.
  ...
  // Αποκτήστε τον επαναλήπτη της λίστας.
