...
description: Enumerator που χρησιμοποιείται από τη μέθοδο επέκτασης IEnumerable.Select().
type: docs
weight: 79
url: /el/system.collections.generic.details/enumeratorselectadapter/
---
## EnumeratorSelectAdapter κλάση
...
| static **bool** [Equals](../../system/object/equals/)(**float** const\&, **float** const\&) | Προσομοιώνει σύγκριση κινητής υποδιαστολής στυλ C# όπου δύο NaN θεωρούνται ίσα παρόλο που σύμφωνα με IEC 60559:1989 το NaN δεν είναι ίσο με καμία τιμή, συμπεριλαμβανομένου του NaN. |
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | Προσομοιώνει σύγκριση κινητής υποδιαστολής στυλ C# όπου δύο NaN θεωρούνται ίσα παρόλο που σύμφωνα με IEC 60559:1989 το NaN δεν είναι ίσο με καμία τιμή, συμπεριλαμβανομένου του NaN. |
...
| void [IncrementIterator](../../system.collections.generic/ienumerator/incrementiterator/)() override | Μετακινεί τον επαναλήπτη ένα βήμα εμπρός. |
...
| void [Lock](../../system/object/lock/)() | Υλοποιεί το κλείδωμα της δήλωσης C# lock(). Καλέστε απευθείας ή χρησιμοποιήστε το αντικείμενο φύλακας [LockContext](../../system/lockcontext/). |
...
| **bool** [MoveNext](./movenext/)() override | Μετακινεί τον enumerator στο επόμενο στοιχείο. Εάν δεν είχε προηγουμένως αναφερθεί στοιχείο, ορίζει την αναφορά στο πρώτο διαθέσιμο στοιχείο. Εάν φτάσει στο τέλος του κοντέινερ, δεν κάνει τίποτα. |
...
| static std::enable_if<\![IsSmartPtr](../../system/issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../../system/object/referenceequals/)(T const\&, std::nullptr_t) | Συγκρίνει σύμφωνα με αναφορά αντικείμενο τύπου τιμής με nullptr. |
...
| void [Reset](./reset/)() override | Επαναφέρει τον enumerator σε θέση πριν το πρώτο στοιχείο. |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | Ορίζει το n'th όρισμα προτύπου ως αδύναμο δείκτη (αντί για κοινόχρηστο). Επιτρέπει την αλλαγή δεικτών στα κοντέινερ σε αδύναμη λειτουργία. |
...
| void [Unlock](../../system/object/unlock/)() | Υλοποιεί το ξεκλείδωμα της δήλωσης C# lock(). Καλέστε απευθείας ή χρησιμοποιήστε το αντικείμενο φύλακας [LockContext](../../system/lockcontext/). |
