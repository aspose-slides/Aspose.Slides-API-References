...
description: Η εξαίρεση που προκαλείται όταν συμβαίνει σφάλμα κατά την επεξεργασία μιας μετατροπής XSLT.
type: docs
weight: 14
url: /el/system.xml.xsl/details_xsltexception/
---
## Details_XsltException κλάση

Η εξαίρεση που προκαλείται όταν συμβαίνει σφάλμα κατά την επεξεργασία μιας μετατροπής XSLT.
...
| **int32_t** [get_HResult](../../system/details_exception/get_hresult/)() const | Επιστρέφει τιμή ακέραιου 32-bit που είναι κωδικός HRESULT συνδεδεμένος με την εξαίρεση που αντιπροσωπεύεται από το τρέχον αντικείμενο. |
...
| virtual **int32_t** [get_LineNumber](./get_linenumber/)() | Επιστρέφει τον αριθμό γραμμής που υποδεικνύει πού συνέβη το σφάλμα στο φύλλο στυλ. |
| virtual **int32_t** [get_LinePosition](./get_lineposition/)() | Επιστρέφει τη θέση της γραμμής που υποδεικνύει πού συνέβη το σφάλμα στο φύλλο στυλ. |
| [String](../../system/string/) [get_Message](./get_message/)() const override | Επιστρέφει το μορφοποιημένο μήνυμα σφάλματος που περιγράφει την τρέχουσα εξαίρεση. |
| virtual [String](../../system/string/) [get_SourceUri](./get_sourceuri/)() | Επιστρέφει τη διαδρομή τοποθεσίας του φύλλου στυλ. |
...
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | Αποκτά τη δομή δεδομένων του μετρητή αναφορών συνδεδεμένη με το αντικείμενο. |
...
| const [System::TypeInfo](../../system/typeinfo/)\& [GetType](../../system/details_systemexception/gettype/)() const override | Αποκτά τον πραγματικό τύπο του αντικειμένου. Αναλογικό κλήσης C# [System.Object.GetType()](../../system/object/gettype/). |
...
| static std::enable_if<\![IsSmartPtr](../../system/issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../../system/object/referenceequals/)(T const\&, std::nullptr_t) | Συγκρίνει αναφορά το αντικείμενο τύπου τιμής με nullptr. |
...
| virtual const char * [what](../../system/details_exception/what/)() const | Εφαρμόζει τη μέθοδο [what()](../../system/details_exception/what/) που καλείται από την κλάση [ExceptionWrapper](../../system/exceptionwrapper/). Παρότι αυτή η κλάση δεν κληρονομείται από std::exception, οι παράγωγες κλάσεις μπορούν να χρησιμοποιήσουν προστατευμένα/ιδιωτικά μέλη για την υλοποίηση της λογικής τους. Η μετακίνηση της υλοποίησης αυτής της μεθόδου στο [ExceptionWrapper](../../system/exceptionwrapper/) μπορεί να διασπά αυτή τη λογική. |
