...
| [CameraPresetType](../camerapresettype/) [get_CameraType](./get_cameratype/)() override | [Camera](./) typ. Číst [CameraPresetType](../camerapresettype/). |
| **float** [get_FieldOfViewAngle](./get_fieldofviewangle/)() override | [Camera](./) FOV (0-180 stupňů, field of View). Číst **float**. |
...
| **float** [get_Zoom](./get_zoom/)() override | [Camera](./) zoom (kladná hodnota v procentech). Číst **float**. |
...
| [System::ArrayPtr](../../system/arrayptr/)\<**float**\> [GetRotation](./getrotation/)() override | Rotace je definována pomocí zeměpisné šířky, zeměpisné délky a otáčení kolem osy jako zeměpisná šířka a délka. první prvek v návratovém poli – latitude, druhý – longitude, třetí – revolution. Vrací null, pokud není definována žádná rotace. |
...
| void [set_CameraType](./set_cameratype/)([CameraPresetType](../camerapresettype/)) override | [Camera](./) typ. Zápis [CameraPresetType](../camerapresettype/). |
| void [set_FieldOfViewAngle](./set_fieldofviewangle/)(**float**) override | [Camera](./) FOV (0-180 stupňů, field of View). Zápis **float**. |
| void [set_Zoom](./set_zoom/)(**float**) override | [Camera](./) zoom (kladná hodnota v procentech). Zápis **float**. |
| void [SetRotation](./setrotation/)(**float**, **float**, **float**) override | Rotace je definována pomocí souřadnice zeměpisné šířky, souřadnice zeměpisné délky a otáčení kolem osy jako souřadnice šířky a délky. Pokud je některá hodnota souřadnice std::numeric_limits<float>::quiet_NaN(), je celá rotace nedefinovaná. |
