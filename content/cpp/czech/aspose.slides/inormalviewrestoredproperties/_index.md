...
description: Určuje velikost oblasti snímku ((šířka, když je potomkem restoredTop, výška, když je potomkem restoredLeft) normálního pohledu, když má oblast proměnnou obnovovanou velikost (ani minimalizovanou, ani maximalizovanou).
type: docs
weight: 2991
url: /cs/aspose.slides/inormalviewrestoredproperties/
---
## INormalViewRestoredProperties třída


Určuje velikost oblasti snímku ((šířka, když je potomkem restoredTop, výška, když je potomkem restoredLeft) normálního pohledu, když má oblast proměnnou obnovovanou velikost (ne ani minimalizovanou, ani maximalizovanou).
...
| virtual **bool** [get_AutoAdjust](./get_autoadjust/)() | Určuje, zda má velikost bočního obsahu kompenzovat novou velikost při změně velikosti okna obsahujícího pohled v aplikaci. Čtení **bool**. |
| virtual **float** [get_DimensionSize](./get_dimensionsize/)() | Určuje velikost oblasti snímku (šířka, když je potomkem RestoredTop, výška, když je potomkem RestoredLeft). Čtení **float**. |
...
| virtual void [set_AutoAdjust](./set_autoadjust/)(**bool**) | Určuje, zda má velikost bočního obsahu kompenzovat novou velikost při změně velikosti okna obsahujícího pohled v aplikaci. Zápis **bool**. |
| virtual void [set_DimensionSize](./set_dimensionsize/)(**float**) | Určuje velikost oblasti snímku (šířka, když je potomkem RestoredTop, výška, když je potomkem RestoredLeft). Zápis **float**. |
