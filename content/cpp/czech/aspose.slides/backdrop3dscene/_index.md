...
description: Definuje rovinu, ve které jsou efekty, jako je záře a stín, aplikovány vzhledem k tvaru, na který jsou aplikovány.
type: docs
weight: 92
...
Definuje rovinu, ve které jsou efekty, jako je záře a stín, aplikovány vzhledem k tvaru, na který jsou aplikovány.
...
| [System::ArrayPtr](../../system/arrayptr/)\<**float**\> [get_AnchorPoint](./get_anchorpoint/)() override | Vrací bod ve 3D prostoru. Tento bod je bodem ve prostoru, který ukotvuje rovinu pozadí. 3D bod je reprezentován polem tří hodnot typu **float**, které definují souřadnice X, Y a Z. Pouze pro čtení **float**[]. |
| [System::ArrayPtr](../../system/arrayptr/)\<**float**\> [get_NormalVector](./get_normalvector/)() override | Vrací normálový vektor. Přesněji řečeno, tento atribut definuje vektor kolmo na plochu roviny pozadí. Vektor je reprezentován polem tří hodnot typu **float**, které definují souřadnice X, Y a Z. Pouze pro čtení **float**[]. |
...
| [System::ArrayPtr](../../system/arrayptr/)\<**float**\> [get_UpVector](./get_upvector/)() override | Vrací vektor představující směr nahoru. Přesněji řečeno, tento atribut definuje vektor představující směr nahoru vzhledem k ploše roviny pozadí. Vektor je reprezentován polem tří hodnot typu **float**, které definují souřadnice X, Y a Z. Pouze pro čtení **float**[]. |
...
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | Ověřuje, zda objekt představuje instanci typu popsaného pomocí targetType. Analogie operátoru C# 'is'. |
...
| void [set_AnchorPoint](./set_anchorpoint/)([System::ArrayPtr](../../system/arrayptr/)\<**float**\>) override | Nastavuje bod ve 3D prostoru. Tento bod je bodem ve prostoru, který ukotvuje rovinu pozadí. 3D bod je reprezentován polem tří hodnot typu **float**, které definují souřadnice X, Y a Z. Pouze pro zápis **float**[]. |
| void [set_NormalVector](./set_normalvector/)([System::ArrayPtr](../../system/arrayptr/)\<**float**\>) override | Nastavuje normálový vektor. Přesněji řečeno, tento atribut definuje vektor kolmo na plochu roviny pozadí. Vektor je representován polem tří hodnot typu **float**, které definují souřadnice X, Y a Z. Pouze pro zápis **float**[]. |
| void [set_UpVector](./set_upvector/)([System::ArrayPtr](../../system/arrayptr/)\<**float**\>) override | Nastavuje vektor představující směr nahoru. Přesněji řečeno, tento atribut definuje vektor představující směr nahoru vzhledem k ploše roviny pozadí. Vektor je reprezentován polem tří hodnot typu **float**, které definují souřadnice X, Y a Z. Pouze pro zápis **float**[]. |
...
* třída [IBackdrop3DScene](../ibackdrop3dscene/)
