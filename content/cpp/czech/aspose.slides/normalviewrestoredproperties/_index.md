...
description: Určuje velikost oblasti snímku ((šířka, když je podřízená restoredTop, výška, když je podřízená restoredLeft) normálního zobrazení, když je oblast proměnné obnovené velikosti (ani minimalizovaná, ani maximalizovaná)).
type: docs
weight: 4538
...
Určuje velikost oblasti snímku ((šířka, když je podřízená restoredTop, výška, když je podřízená restoredLeft) normálního zobrazení, když je oblast o proměnné obnovené velikosti (ani minimalizovaná, ani maximalizovaná).
...
| **bool** [get_AutoAdjust](./get_autoadjust/)() override | Určuje, zda velikost boční oblasti obsahu má kompenzovat novou velikost při změně velikosti okna obsahujícího pohled v aplikaci. Číst **bool**. |
| **float** [get_DimensionSize](./get_dimensionsize/)() override | Určuje velikost oblasti snímku (šířka, když je podřízená RestoredTop, výška, když je podřízená RestoredLeft). Číst **float**. |
...
| void [Lock](../../system/object/lock/)() | Implementuje zamykání pomocí příkazu C# lock(). Volat přímo nebo použít objekt [LockContext](../../system/lockcontext/). |
...
| void [set_AutoAdjust](./set_autoadjust/)(**bool**) override | Určuje, zda velikost boční oblasti obsahu má kompenzovat novou velikost při změně velikosti okna obsahujícího pohled v aplikaci. Zapsat **bool**. |
| void [set_DimensionSize](./set_dimensionsize/)(**float**) override | Určuje velikost oblasti snímku (šířka, když je podřízená RestoredTop, výška, když je podřízená RestoredLeft). Zapsat **float**. |
...
| void [Unlock](../../system/object/unlock/)() | Implementuje odemčení pomocí příkazu C# lock(). Volat přímo nebo použít objekt [LockContext](../../system/lockcontext/). |
...
* Třída [INormalViewRestoredProperties](../inormalviewrestoredproperties/)
