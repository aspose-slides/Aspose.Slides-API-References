...
description: "Reprezentuje vzor rozostření pro objekt LinearGradientBrush. Objektům této třídy by se mělo alokovat pouze pomocí funkce System::MakeObject() . Nikdy nevytvářejte instanci tohoto typu na zásobníku ani pomocí operátoru new, protože to povede k chybám za běhu a/nebo selháním aserce. Vždy zabalte tuto třídu do ukazatele System::SmartPtr a použijte tento ukazatel k předání do funkcí jako argument."
type: docs
weight: 14
url: /cs/system.drawing.drawing2d/blend/
---
## Blend třída

Reprezentuje vzor rozostření pro objekt [LinearGradientBrush](../lineargradientbrush/). Objektům této třídy by se mělo alokovat pouze pomocí funkce [System::MakeObject()](../../system/makeobject/). Nikdy nevytvářejte instanci tohoto typu na zásobníku ani pomocí operátoru new, protože to povede k chybám za běhu a/nebo selháním aserce. Vždy zabalte tuto třídu do ukazatele [System::SmartPtr](../../system/smartptr/) a použijte tento ukazatel k předání do funkcí jako argument.
...
| Method | Description |
| --- | --- |
|  [Blend](./blend/)() | Vytvoří novou instanci třídy [Blend](./). |
|  [Blend](./blend/)(int) | Vytvoří novou instanci třídy [Blend](./). |
...
| [ArrayPtr](../../system/arrayptr/)\<**float**\> [get_Factors](./get_factors/)() const | Vrací pole blend faktorů gradientu. |
| [ArrayPtr](../../system/arrayptr/)\<**float**\> [get_Positions](./get_positions/)() const | Vrací pole pozic blend gradientu. |
...
| void [Lock](../../system/object/lock/)() | Implementuje zamykání C# výrazu lock(). Zavolejte přímo nebo použijte objekt strážce [LockContext](../../system/lockcontext/). |
...
| void [set_Factors](./set_factors/)(const [ArrayPtr](../../system/arrayptr/)\<**float**\>\&) | Nastaví pole blend faktorů gradientu. |
| void [set_Positions](./set_positions/)(const [ArrayPtr](../../system/arrayptr/)\<**float**\>\&) | Nastaví pole pozic blend gradientu. |
...
| void [Unlock](../../system/object/unlock/)() | Implementuje odemykání C# výrazu lock(). Zavolejte přímo nebo použijte objekt strážce [LockContext](../../system/lockcontext/). |
