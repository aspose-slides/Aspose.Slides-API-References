...
description: "TimeoutException naznačuje, zda uplynula časová lhůta přidělená pro proces nebo operaci. Nikdy nevytvářejte instance této třídy ručně. Použijte místo toho třídu TimeoutException. Nikdy nezabalujte instance třídy TimeoutException do System::SmartPtr."
type: docs
weight: 716
url: /cs/system/details_timeoutexception/
---
## Details_TimeoutException třída

TimeoutException naznačuje, zda vypršela doba přidělená pro proces nebo operaci. Nikdy nevytvářejte instance této třídy ručně. Místo toho použijte třídu TimeoutException. Nikdy nezabalujte instance třídy TimeoutException do [System::SmartPtr](../smartptr/).
...
| void [Lock](../object/lock/)() | Implementuje zamykání příkazem C# lock(). Volat přímo nebo použít objekt strážce [LockContext](../lockcontext/). |
...
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | Přiřazovací operátor. Ve skutečnosti nic nekopíruje, pouze inicializuje nový objekt a umožňuje kopírování podtříd. |
...
| void [set_HResult](../details_exception/set_hresult/)(**int32_t**) | Nastavuje HRESULT, kódované číselné hodnoty přiřazené konkrétní výjimce. |
...
| [Object](../object/) * [SharedRefAdded](../object/sharedrefadded/)() | Zvyšuje sdílený čítač referencí. Není vhodné volat přímo; místo toho použijte chytré ukazatele nebo ThisProtector. |
| int [SharedRefRemovedSafe](../object/sharedrefremovedsafe/)() | Snižuje a vrací sdílený čítač referencí. Není vhodné volat přímo; místo toho použijte chytré ukazatele nebo ThisProtector. |
...
| void [Unlock](../object/unlock/)() | Implementuje odemknutí příkazem C# lock(). Volat přímo nebo použít objekt strážce [LockContext](../lockcontext/). |
| Detail::SmartPtrCounter * [WeakRefAdded](../object/weakrefadded/)() | Zvyšuje slabý čítač referencí. Není vhodné volat přímo; místo toho použijte chytré ukazatele nebo ThisProtector. |
| void [WeakRefRemoved](../object/weakrefremoved/)() | Snižuje slabý čítač referencí. Není vhodné volat přímo; místo toho použijte chytré ukazatele nebo ThisProtector. |
| virtual const char * [what](../details_exception/what/)() const | Implementuje metodu [what()](../details_exception/what/), která je volána třídou [ExceptionWrapper](../exceptionwrapper/). Navzdory tomu, že tato třída není zděděna ze std::exception, odvozené třídy mohou používat chráněné/soukromé členy k implementaci své logiky. Přesunutí implementace této metody do [ExceptionWrapper](../exceptionwrapper/) může tuto logiku narušit. |
