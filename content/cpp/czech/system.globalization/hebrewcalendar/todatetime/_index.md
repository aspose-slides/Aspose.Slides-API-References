---
title: ToDateTime()
second_title: Aspose.Slides pro C++ - referenční příručka API
description: Vytvoří objekt DateTime z komponent.
type: docs
weight: 183
url: /cs/system.globalization/hebrewcalendar/todatetime/
---
## HebrewCalendar::ToDateTime(int, int, int, int, int, int, int, int) const metoda

Vytváří [DateTime](../../../system/datetime/) objekt z komponent.

```cpp
DateTime System::Globalization::HebrewCalendar::ToDateTime(int year, int month, int day, int hour, int minute, int second, int millisecond, int era) const override
```

### Argumenty

| Parametr | Typ | Popis |
| --- | --- | --- |
| year | int | Year. |
| month | int | Month. |
| day | int | Day. |
| hour | int | Hour. |
| minute | int | Minute. |
| second | int | Second. |
| millisecond | int | Millisecond. |
| era | int | Era. |

### Návratová hodnota

[DateTime](../../../system/datetime/) objekt, který má určené komponenty.

## HebrewCalendar::ToDateTime(int, int, int, int, int, int, int) const metoda

Vytváří [DateTime](../../../system/datetime/) objekt z komponent.

```cpp
virtual DateTime System::Globalization::Calendar::ToDateTime(int year, int month, int day, int hour, int minute, int second, int millisecond) const
```

### Argumenty

| Parametr | Typ | Popis |
| --- | --- | --- |
| year | int | Year. |
| month | int | Month. |
| day | int | Day. |
| hour | int | Hour. |
| minute | int | Minute. |
| second | int | Second. |
| millisecond | int | Millisecond. |

### Návratová hodnota

[DateTime](../../../system/datetime/) objekt, který má určené komponenty.

## HebrewCalendar::ToDateTime(int, int, int, int, int, int, int, int) const metoda

Vytváří [DateTime](../../../system/datetime/) objekt z komponent.

```cpp
virtual DateTime System::Globalization::Calendar::ToDateTime(int year, int month, int day, int hour, int minute, int second, int millisecond, int era) const
```

### Argumenty

| Parametr | Typ | Popis |
| --- | --- | --- |
| year | int | Year. |
| month | int | Month. |
| day | int | Day. |
| hour | int | Hour. |
| minute | int | Minute. |
| second | int | Second. |
| millisecond | int | Millisecond. |
| era | int | Era. |

### Návratová hodnota

[DateTime](../../../system/datetime/) objekt, který má určené komponenty.

## Viz také

* Třída [DateTime](../../../system/datetime/)
* Třída [HebrewCalendar](../)
* Jmenný prostor [System::Globalization](../../)
* Knihovna [Aspose.Slides](../../../)