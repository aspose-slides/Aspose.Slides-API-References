...
description: "Kalendář Um Al Qura. Není implementován. Objektům této třídy by se mělo alokovat pouze pomocí funkce System::MakeObject() . Nikdy nevytvářejte instanci tohoto typu na zásobníku ani pomocí operátoru new, protože to povede k chybám za běhu a/nebo selháním aserčních kontrol. Vždy zabalte tuto třídu do ukazatele System::SmartPtr a použijte tento ukazatel k předávání do funkcí jako argument."
type: docs
weight: 391
url: /cs/system.globalization/umalquracalendar/
---
...
| [ArrayPtr](../../system/arrayptr/)\<int\> [get_Eras](./get_eras/)() const override | Získá seznam ér existujících v kalendáři. |
...
| virtual int [get_TwoDigitYearMax](../calendar/get_twodigityearmax/)() const | Získá poslední rok, který lze vyjádřit dvouciferně. |
...
| virtual int [GetDaysInMonth](../calendar/getdaysinmonth/)(int, int, int) const | Získá počet dnů ve specifickém měsíci. |
...
| void [Lock](../../system/object/lock/)() | Implementuje zamykání pomocí C# lock(). Volat přímo nebo použít objekt hlídkového [LockContext](../../system/lockcontext/). |
...
| void [set_TwoDigitYearMax](./set_twodigityearmax/)(int) override | Nastaví poslední rok, který lze vyjádřit dvouciferně. |
...
| void [Unlock](../../system/object/unlock/)() | Implementuje odemykání pomocí C# lock(). Volat přímo nebo použít objekt hlídkového [LockContext](../../system/lockcontext/). |
...
| Pole | Popis |
| --- | --- |
| static constexpr [UmAlQuraEra](./umalquraera/) | Aktuální éra UmAlQura. |
