...
description: "Základní třída pro hashovací algoritmy. Objektů této třídy by měly být alokovány pouze pomocí funkce System::MakeObject(). Nikdy nevytvářejte instanci tohoto typu na zásobníku ani pomocí operátoru new, protože to může vést k chybám za běhu a/nebo poruše aserce. Vždy zabalte tuto třídu do ukazatele System::SmartPtr a použijte tento ukazatel k předání jako argument funkcím."
type: docs
weight: 222
url: /cs/system.security.cryptography/hashalgorithm/
---
## HashAlgorithm třída

Základní třída pro hashovací algoritmy. Objektů této třídy by měly být alokovány pouze pomocí funkce [System::MakeObject()](../../system/makeobject/). Nikdy nevytvářejte instanci tohoto typu na zásobníku ani pomocí operátoru new, protože to může vést k chybám za běhu a/nebo poruše aserce. Vždy zabalte tuto třídu do ukazatele [System::SmartPtr](../../system/smartptr/) a použijte tento ukazatel k předání jako argument funkcím.
...
| Metoda | Popis |
| --- | --- |
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [ComputeHash](./computehash/)(const [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>\&) | Hashuje buffer. |
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [ComputeHash](./computehash/)(const [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>\&, int, int) | Hashuje část bufferu. |
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [ComputeHash](./computehash/)([SharedPtr](../../system/sharedptr/)\<[IO::Stream](../../system.io/stream/)\> const\&) | Čte proud až do konce a vypočítá hash pro přečtená data. |
| static [SharedPtr](../../system/sharedptr/)\<[HashAlgorithm](./)\> [Create](./create/)(const [String](../../system/string/)\&) | Vytvoří hashovací algoritmus na základě názvu. |
...
| virtual [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_Hash](./get_hash/)() | Vrací hodnotu vypočteného hash kódu. |
| virtual int [get_HashSize](./get_hashsize/)() | Vrací velikost vypočtené hash hodnoty v bajtech. |
| int [get_InputBlockSize](./get_inputblocksize/)() override | Velikost vstupního bloku. |
| int [get_OutputBlockSize](./get_outputblocksize/)() override | Velikost výstupního bloku. |
...
| virtual void [Initialize](./initialize/)() | Resetuje hasher do počátečního stavu. |
...
| int [TransformBlock](./transformblock/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int) override | Zpracovává blok dat a kopíruje data do výstupního pole. |
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [TransformFinalBlock](./transformfinalblock/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int) override | Zpracovává poslední blok dat a vypočítá hash. |
