...
description: 创建一个新的 Summary Zoom 框并将其插入到指定索引的形状集合中。
type: docs
weight: 157
...
创建一个新的 Summary Zoom 框并将其插入到指定索引的形状集合中。
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| index | **int32_t** | 要插入 Summary Zoom 框的零基索引。 |
| x | **float** | 新 Summary Zoom 框的 x 坐标，单位为点。 |
| y | **float** | 新 Summary Zoom 框的 y 坐标，单位为点。 |
| width | **float** | 新 Summary Zoom 框的宽度，单位为点。 |
| height | **float** | 新 Summary Zoom 框的高度，单位为点。 |
...
此方法创建一个 Summary Zoom 框，用于聚合演示文稿中所有章节的摘要链接。
