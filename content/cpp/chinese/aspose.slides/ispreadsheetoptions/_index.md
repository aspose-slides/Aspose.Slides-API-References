...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[System::Globalization::CultureInfo](../../system.globalization/cultureinfo/)\> [get_PreferredCulture](./get_preferredculture/)() | 获取首选的区域设置信息，用于计算旨在与使用双字节字符集 (DBCS) 的语言一起使用的某些函数。 |
| virtual **bool** [get_RecoverWorkbookFromChartCache](./get_recoverworkbookfromchartcache/)() | 如果图表的数据源是外部工作簿且不可用，则会从图表缓存中恢复。 |
...
| virtual void [set_PreferredCulture](./set_preferredculture/)([System::SharedPtr](../../system/sharedptr/)\<[System::Globalization::CultureInfo](../../system.globalization/cultureinfo/)\>) | 设置首选的区域设置信息，用于计算旨在与使用双字节字符集 (DBCS) 的语言一起使用的某些函数。 |
| virtual void [set_RecoverWorkbookFromChartCache](./set_recoverworkbookfromchartcache/)(**bool**) | 如果图表的数据源是外部工作簿且不可用，则会从图表缓存中恢复。 |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | 将第 n 个模板参数设为弱指针（而不是共享指针）。允许将容器中的指针切换为弱模式。 |
