...
description: 指定普通视图中幻灯片区域的大小（当为 restoredTop 的子项时为宽度，当为 restoredLeft 的子项时为高度），当该区域具有可变的已恢复大小（既未最小化也未最大化）时。
type: docs
weight: 2991
url: /zh/aspose.slides/inormalviewrestoredproperties/
---
## INormalViewRestoredProperties 类


指定普通视图中幻灯片区域的大小（当为 restoredTop 的子项时为宽度，当为 restoredLeft 的子项时为高度），当该区域具有可变的已恢复大小（既未最小化也未最大化）时。
...
| virtual **bool** [get_AutoAdjust](./get_autoadjust/)() | 指定在调整包含视图的窗口大小时，侧内容区域的尺寸是否应补偿新尺寸。读取 **bool**。 |
| virtual **float** [get_DimensionSize](./get_dimensionsize/)() | 指定幻灯片区域的尺寸（当为 RestoredTop 的子项时为宽度，当为 RestoredLeft 的子项时为高度）。读取 **float**。 |
...
| virtual void [set_AutoAdjust](./set_autoadjust/)(**bool**) | 指定在调整包含视图的窗口大小时，侧内容区域的尺寸是否应补偿新尺寸。写入 **bool**。 |
| virtual void [set_DimensionSize](./set_dimensionsize/)(**float**) | 指定幻灯片区域的尺寸（当为 RestoredTop 的子项时为宽度，当为 RestoredLeft 的子项时为高度）。写入 **float**。 |
