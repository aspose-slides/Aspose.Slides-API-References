...
| virtual [CameraPresetType](../camerapresettype/) [get_CameraType](./get_cameratype/)() | [Camera](../camera/) 类型 读取 [CameraPresetType](../camerapresettype/)。 |
| virtual **float** [get_FieldOfViewAngle](./get_fieldofviewangle/)() | [Camera](../camera/) 视场 (0-180 度, field of View) 读取 **float**。 |
| virtual **float** [get_Zoom](./get_zoom/)() | [Camera](../camera/) 缩放 (百分比的正值) 读取 **float**。 |
...
| virtual [System::ArrayPtr](../../system/arrayptr/)\<**float**\> [GetRotation](./getrotation/)() | 旋转通过使用纬度坐标、经度坐标以及围绕轴的旋转来定义，纬度和经度坐标即为上述坐标。返回数组的第一个元素 - 纬度，第二个 - 经度，第三个 - 旋转。如果未定义旋转则返回 null。 |
...
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | 检查对象是否表示由 targetType 描述的类型实例。C# 'is' 操作符的类比。 |
...
| virtual void [set_CameraType](./set_cameratype/)([CameraPresetType](../camerapresettype/)) | [Camera](../camera/) 类型 写入 [CameraPresetType](../camerapresettype/)。 |
| virtual void [set_FieldOfViewAngle](./set_fieldofviewangle/)(**float**) | [Camera](../camera/) 视场 (0-180 度, field of View) 写入 **float**。 |
| virtual void [set_Zoom](./set_zoom/)(**float**) | [Camera](../camera/) 缩放 (百分比的正值) 写入 **float**。 |
| virtual void [SetRotation](./setrotation/)(**float**, **float**, **float**) | 旋转通过使用纬度坐标、经度坐标以及围绕轴的旋转来定义。如果任意坐标值为 std::numeric_limits<float>::quiet_NaN()，则所有旋转未定义。 |
