...
description: "在 Windows Forms 应用程序中打印时，将输出发送到打印机。此类的对象只能通过 System::MakeObject() 函数分配。不要在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言失败。始终将此类包装为 System::SmartPtr 指针，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 53
url: /zh/system.drawing.printing/printdocument/
---
## PrintDocument 类

在来自 [Windows](../../system.windows/) Forms 应用程序的打印过程中，将输出发送到打印机。此类的对象应仅通过 [System::MakeObject()](../../system/makeobject/) 函数分配。不要在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装为 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针将其作为参数传递给函数。
...
| [SharedPtr](../../system/sharedptr/)\<[PrinterSettings](../printersettings/)\> [get_PrinterSettings](./get_printersettings/)() | 未实现。 |
...
| void [set_DocumentName](./set_documentname/)(const [String](../../system/string/)\&) | 未实现。 |
| void [set_PrintController](./set_printcontroller/)(const [SharedPtr](../../system/sharedptr/)\<[PrintController](../printcontroller/)\>\&) | 未实现。 |
| void [set_PrinterSettings](./set_printersettings/)(const [SharedPtr](../../system/sharedptr/)\<[PrinterSettings](../printersettings/)\>\&) | 未实现。 |
