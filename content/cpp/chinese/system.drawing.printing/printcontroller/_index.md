...
description: "控制文档的打印方式，当从 Windows Forms 应用程序进行打印时。此类的对象应仅使用 System::MakeObject() 函数分配。永远不要在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 System::SmartPtr 指针中，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 40
url: /zh/system.drawing.printing/printcontroller/
---
## PrintController 类

控制文档的打印方式，当从 [Windows](../../system.windows/) Forms 应用程序进行打印时。此类的对象应仅通过 [System::MakeObject()](../../system/makeobject/) 函数分配。永远不要在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 [System::SmartPtr](../../system/smartptr/) 指针中，并使用该指针将其作为参数传递给函数。
