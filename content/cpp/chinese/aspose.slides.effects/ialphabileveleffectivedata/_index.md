...
description: 不可变对象，表示 Alpha 双层效果。阈值以下的 Alpha（不透明度）值被更改为 0（完全透明），阈值以上或等于阈值的 Alpha 值被更改为 100%（完全不透明）。
type: docs
weight: 248
url: /zh/aspose.slides.effects/ialphabileveleffectivedata/
---
## IAlphaBiLevelEffectiveData 类

不可变对象，表示 Alpha 双层效果。阈值以下的 Alpha（不透明度）值将被更改为 0（完全透明），阈值以上或等于阈值的 Alpha 值将被更改为 100%（完全不透明）。
...
| virtual **float** [get_Threshold](./get_threshold/)() | 返回效果阈值。只读 **float**。 |
