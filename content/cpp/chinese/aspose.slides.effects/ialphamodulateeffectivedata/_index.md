...
description: 不可变对象，表示 Alpha Modulate 效果。效果的 alpha（不透明度）值乘以固定的百分比。效果容器指定包含要调制的 alpha 值的效果。
type: docs
weight: 352
url: /zh/aspose.slides.effects/ialphamodulateeffectivedata/
---
## IAlphaModulateEffectiveData 类

不可变对象，表示 Alpha Modulate 效果。效果的 alpha（不透明度）值乘以固定百分比。效果容器指定包含要调制的 alpha 值的效果。
...
| void [Lock](../../system/object/lock/)() | 实现 C# lock() 语句的加锁。直接调用或使用 [LockContext](../../system/lockcontext/) 护卫对象。 |
...
| void [Unlock](../../system/object/unlock/)() | 实现 C# lock() 语句的解锁。直接调用或使用 [LockContext](../../system/lockcontext/) 护卫对象。 |
