...
description: 不可变对象，表示 Alpha Ceiling 效果。Alpha（不透明度）大于零的值被更改为 100%。换句话说，任何部分不透明的内容都会变为完全不透明。
type: docs
weight: 274
url: /zh/aspose.slides.effects/ialphaceilingeffectivedata/
---
## IAlphaCeilingEffectiveData 类

不可变对象，表示 Alpha Ceiling 效果。Alpha（不透明度）大于零的值将被更改为 100%。换句话说，任何部分不透明的内容都会变为完全不透明。
