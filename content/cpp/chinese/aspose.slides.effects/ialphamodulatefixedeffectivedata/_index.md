...
description: 不可变对象，表示 Alpha Modulate Fixed 效果。效果的 alpha（不透明度）值乘以固定的百分比。
type: docs
weight: 378
url: /zh/aspose.slides.effects/ialphamodulatefixedeffectivedata/
---
## IAlphaModulateFixedEffectiveData 类


不可变对象，表示 Alpha Modulate Fixed 效果。效果的 alpha（不透明度）值乘以固定的百分比。
...
| virtual **float** [get_Amount](./get_amount/)() | 返回效果的百分比。只读 **float**。 |
