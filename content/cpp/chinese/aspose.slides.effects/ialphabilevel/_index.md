...
description: 表示 Alpha 双层效果。Alpha（不透明度）值小于阈值的被更改为 0（完全透明），Alpha 值大于或等于阈值的被更改为 100%（完全不透明）。
type: docs
weight: 235
...
表示 Alpha 双层效果。小于阈值的 Alpha（不透明度）值会被更改为 0（完全透明），大于或等于阈值的 Alpha 值会被更改为 100%（完全不透明）。
...
| virtual **float** [get_Threshold](./get_threshold/)() | 返回效果阈值。读取 **float**。 |
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | 获取与对象关联的引用计数数据结构。 |
| virtual T [GetEffective](../../aspose.slides/iaccessiblepviobject/geteffective/)() | 在应用继承后获取有效数据。 |
...
| virtual void [set_Threshold](./set_threshold/)(**float**) | 返回效果阈值。写入 **float**。 |
...
使用 [ImageTransformOperationFactory](../imagetransformoperationfactory/) 在 COM 中创建实例。
