...
description: 表示 Alpha Modulate 效果。效果的 Alpha（不透明度）值会被固定百分比相乘。效果容器指定一个包含要调制的 Alpha 值的效果。
type: docs
weight: 339
...
表示 Alpha Modulate 效果。效果的 Alpha（不透明度）值会被固定百分比相乘。效果容器指定一个包含要调制的 Alpha 值的效果。
...
| virtual **bool** [Is](../../system/object/is/)(const [TypeInfo](../../system/typeinfo/)\&) const | 检查对象是否为目标类型描述的实例。等同于 C# 'is' 运算符。 |
