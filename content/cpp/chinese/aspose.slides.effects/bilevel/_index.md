...
description: 表示一种 Bi-Level（黑/白）效果。亮度低于指定阈值的输入颜色会被更改为黑色。亮度大于等于指定值的输入颜色会被设置为白色。alpha 效果值不受此效果影响。
type: docs
weight: 92
...
表示一种 Bi-Level（黑/白）效果。亮度低于指定阈值的输入颜色会被更改为黑色。亮度大于等于指定值的输入颜色会被设为白色。alpha 效果值不受此效果影响。
...
| 方法 | 描述 |
| --- | --- |
| **bool** [Equals](./equals/)([System::SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\>) override | 确定指定的 [BiLevel](./) 是否等于当前的 [BiLevel](./)。 |
...
| [System::SharedPtr](../../system/sharedptr/)\<[IBiLevelEffectiveData](../ibileveleffectivedata/)\> [GetEffective](./geteffective/)() override | 获取应用继承后的有效 Bi-Level 效果数据。 |
| **int32_t** [GetHashCode](./gethashcode/)() const override | 用作特定类型的哈希函数。 |
