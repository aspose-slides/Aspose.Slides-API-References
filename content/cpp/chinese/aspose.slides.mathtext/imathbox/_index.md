...
description: 指定数学元素的逻辑盒装（包装）。例如，盒装对象可以作为带或不带对齐点的运算符仿真器、作为换行点，或进行分组以防止内部换行。例如，应该对 \"==\" 运算符进行盒装以防止换行。
type: docs
weight: 170
...
| Method | 描述 |
| --- | --- |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathAccent](../imathaccent/)\> [Accent](../imathelement/accent/)(char16_t) | 设置重音标记（该元素顶部的字符） |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathFunction](../imathfunction/)\> [AsArgumentOfFunction](../imathelement/asargumentoffunction/)([MathFunctionsOfTwoArguments](../mathfunctionsoftwoarguments/), [System::SharedPtr](../../system/sharedptr/)\<[IMathElement](../imathelement/)\>) | 使用此实例作为参数并传入指定的附加参数调用指定函数 |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathFunction](../imathfunction/)\> [AsArgumentOfFunction](../imathelement/asargumentoffunction/)([MathFunctionsOfTwoArguments](../mathfunctionsoftwoarguments/), [System::String](../../system/string/)) | 使用此实例作为参数并传入指定的附加参数调用指定函数 |
...
| virtual **bool** [get_AlignmentPoint](./get_alignmentpoint/)() | 为 true 时，此运算符仿真器充当对齐点；即其他方程中指定的对齐点可以与其对齐。默认值：false |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathElement](../imathelement/)\> [get_Base](./get_base/)() | 基础参数 |
| virtual **bool** [get_Differential](./get_differential/)() | 微分。当为 true 时，框充当微分（例如，积分被积中的 \\uD835\\uDC51\\uD835\\uDC65），并获得适当的水平间距以表示数学微分。默认值：false |
| virtual **uint8_t** [get_ExplicitBreak](./get_explicitbreak/)() | 显式换行指定在 Box 对象起始处是否有换行，即行在框对象起始处换行。指定用于当前数学文本行对齐点的前一行数学文本中运算符的编号，取值范围：1..255。默认值：0（无显式换行） |
| virtual **bool** [get_NoBreak](./get_nobreak/)() | 不换行。此属性指定对象框的“不可换行”属性。为 true 时，框内不能出现换行。这对于由多个二元运算符组成的运算符仿真器尤为重要。如未指定此元素，框内可以出现换行。默认值：true |
| virtual **bool** [get_OperatorEmulator](./get_operatoremulator/)() | 运算符仿真器。为 true 时，框及其内容表现为单一运算符并继承运算符的属性。这意味着，例如，该字符可以用作换行点并可与其他运算符对齐。运算符仿真器常用于当一个或多个字形组合形成运算符（如 '=='）的场景。默认值：false |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathGroupingCharacter](../imathgroupingcharacter/)\> [Group](../imathelement/group/)() | 使用底部花括号将此元素放入分组 |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathGroupingCharacter](../imathgroupingcharacter/)\> [Group](../imathelement/group/)(char16_t, [MathTopBotPositions](../mathtopbotpositions/), [MathTopBotPositions](../mathtopbotpositions/)) | 使用分组字符（如底部花括号或其他字符）将此元素放入分组 |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathBlock](../imathblock/)\> [Join](../imathelement/join/)([System::SharedPtr](../../system/sharedptr/)\<[IMathElement](../imathelement/)\>) | 合并数学元素并形成数学块 |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathBlock](../imathblock/)\> [Join](../imathelement/join/)([System::String](../../system/string/)) | 合并数学文本并形成数学块 |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathRadical](../imathradical/)\> [Radical](../imathelement/radical/)([System::SharedPtr](../../system/sharedptr/)\<[IMathElement](../imathelement/)\>) | 指定给定次数的数学根号，基于指定的参数。 |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathRadical](../imathradical/)\> [Radical](../imathelement/radical/)([System::String](../../system/string/)) | 指定给定次数的数学根号，基于指定的参数。 |
...
| virtual void [set_AlignmentPoint](./set_alignmentpoint/)(**bool**) | 为 true 时，此运算符仿真器充当对齐点；即其他方程中指定的对齐点可以与其对齐。默认值：false |
| virtual void [set_Differential](./set_differential/)(**bool**) | 微分。当为 true 时，框充当微分（例如，积分被积中的 \\uD835\\uDC51\\uD835\\uDC65），并获得适当的水平间距以表示数学微分。默认值：false |
| virtual void [set_ExplicitBreak](./set_explicitbreak/)(**uint8_t**) | 显式换行指定在 Box 对象起始处是否有换行，即行在框对象起始处换行。指定用于当前数学文本行对齐点的前一行数学文本中运算符的编号，取值范围：1..255。默认值：0（无显式换行） |
| virtual void [set_NoBreak](./set_nobreak/)(**bool**) | 不换行。此属性指定对象框的“不可换行”属性。为 true 时，框内不能出现换行。这对于由多个二元运算符组成的运算符仿真器尤为重要。如未指定此元素，框内可以出现换行。默认值：true |
| virtual void [set_OperatorEmulator](./set_operatoremulator/)(**bool**) | 运算符仿真器。为 true 时，框及其内容表现为单一运算符并继承运算符的属性。这意味着，例如，该字符可以用作换行点并可与其他运算符对齐。运算符仿真器常用于当一个或多个字形组合形成运算符（如 '=='）的场景。默认值：false |
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IMathBox](./)\> [ToBox](../imathelement/tobox/)() | 将此元素放入非可视框（逻辑分组），用于对方程或其他数学文本实例的组成部分进行分组。例如，盒装对象可以充当带或不带对齐点的运算符仿真器、充当换行点，或进行分组以防止内部换行。 |
