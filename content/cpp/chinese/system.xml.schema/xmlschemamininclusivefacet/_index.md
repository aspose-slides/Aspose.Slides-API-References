...
description: 表示由万维网联盟（W3C）规定的 XML Schema 中的 minInclusive 元素。此类可用于指定对 simpleType 元素最小值的限制。元素值必须大于或等于 minInclusive 元素的值。
type: docs
weight: 625
url: /zh/system.xml.schema/xmlschemamininclusivefacet/
---
## XmlSchemaMinInclusiveFacet 类

表示来自 XML [Schema](../) 的 **minInclusive** 元素，符合万维网 [Web](../../system.web/) 联盟（W3C）的规范。此类可用于指定对 simpleType 元素的最小值的限制。元素值必须大于或等于 **minInclusive** 元素的值。
...
应仅使用 [System::MakeObject()](../../system/makeobject/) 函数分配此 类的对象。切勿在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言错误。始终将此 类包装为 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针将其作为参数传递给函数。
