...
description: 表示由万维网联盟 (W3C) 指定的 XML Schema 中的 maxLength 元素。此类可用于对 simpleType 元素的数据值的最大长度施加限制。长度必须小于 maxLength 元素的值。
type: docs
weight: 599
url: /zh/system.xml.schema/xmlschemamaxlengthfacet/
---
## XmlSchemaMaxLengthFacet 类


表示来自 XML [Schema](../) 的 **maxLength** 元素，按照万维网 [Web](../../system.web/) 联盟 (W3C) 的规定。此 类可用于指定对 **simpleType** 元素的数据值的最大长度的限制。长度必须小于 **maxLength** 元素的值。
...
| virtual **bool** [get_IsFixed](../xmlschemafacet/get_isfixed/)() | 返回指示此 facet 已固定的信息。 |
...
| virtual void [set_IsFixed](../xmlschemafacet/set_isfixed/)(**bool**) | 设置指示此 facet 已固定的信息。 |
...
|  [XmlSchemaMaxLengthFacet](./xmlschemamaxlengthfacet/)() | 初始化 [XmlSchemaMaxLengthFacet](./) 类的新实例。 |
...
此类的对象只能通过 [System::MakeObject()](../../system/makeobject/) 函数分配。绝不要在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装为 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针将其作为参数传递给函数。
