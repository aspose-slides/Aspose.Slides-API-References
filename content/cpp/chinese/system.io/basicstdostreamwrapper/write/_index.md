...
description: 如果包装模式为 binary，则将指定字节数组中的指定子范围的字节写入流；否则将指定字节数组中的指定子范围的字节转换为 char_type 类型后再将结果写入流。
type: docs
weight: 79
url: /zh/system.io/basicstdostreamwrapper/write/
---
## BasicSTDOStreamWrapper::Write(const ArrayPtr\<uint8_t\>\&, int32_t, int32_t) 方法

如果包装模式是 binary，则将指定字节数组中的指定子范围的字节写入流；否则将指定字节数组中的指定子范围的字节转换为 char_type 类型后再将结果写入流。
