...
description: 创建新的数据点并将其添加到集合的末尾。适用于 chartType 为 Stock 子类型之一的系列（另请参见 ChartTypeCharacterizer.IsChartTypeStock(ChartType) 方法）。
type: docs
weight: 144
...
创建新的数据点并将其添加到集合的末尾。适用于 chartType 为 Stock 子类型之一的系列（另请参见 [ChartTypeCharacterizer.IsChartTypeStock(ChartType)](../../charttypecharacterizer/ischarttypestock/) 方法）。
