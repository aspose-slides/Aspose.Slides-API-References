...
description: 创建新的数据点并将其添加到集合的末尾。适用于 chartType 为 Column 或 Bar 子类型的系列（另请参阅 ChartTypeCharacterizer.IsChartTypeColumn(ChartType) 和 ChartTypeCharacterizer.IsChartTypeBar(ChartType) 方法）。
type: docs
weight: 196
...
创建新的数据点并将其添加到集合的末尾。适用于 chartType 为 [Column](../../../aspose.slides/column/) 或 Bar 子类型的系列（另请参阅 [ChartTypeCharacterizer.IsChartTypeColumn(ChartType)](../../charttypecharacterizer/ischarttypecolumn/) 和 [ChartTypeCharacterizer.IsChartTypeBar(ChartType)](../../charttypecharacterizer/ischarttypebar/) 方法）。
