...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IChartSeriesGroup](../ichartseriesgroup/)\> [idx_get](./idx_get/)([System::SharedPtr](../../system/sharedptr/)\<[IChartSeries](../ichartseries/)\>) | 根据系列获取系列组。 |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IChartSeriesGroup](../ichartseriesgroup/)\> [idx_get](./idx_get/)(**int32_t**) | 根据索引获取系列组。 |
...
1) 每个系列组包含具有可组合类型的系列。可组合系列类型的组由 CombinableSeriesTypesGroup 枚举定义并描述。此外，每个系列组包含的系列只能绘制在主坐标轴或次坐标轴上（同一组中不会同时出现两种情况）。因此，系列分组的原则是按上述类型组以及主/次绘图类型进行分组。

2) 系列组包含一些对组内每个系列都通用的系列属性（“系列组属性”）。在 [ChartSeriesGroup](../chartseriesgroup/) 类中，“系列组属性”是读写的。每个“系列组属性”在 [ChartSeries](../chartseries/) 类中可以有只读的投影。
