...
description: 确定当前对象表示的日期时间值是否晚于由指定的 DateTimeOffset 对象表示的值。
type: docs
weight: 573
url: /zh/system/datetimeoffset/operator_greater/
---
## DateTimeOffset::operator>(const DateTimeOffset\&) const 方法

确定当前对象表示的日期时间值是否晚于由指定的 [DateTimeOffset](../) 对象表示的值。
...
如果当前对象表示的日期时间值晚于 **other** 所表示的值，则为 True；否则为 false
