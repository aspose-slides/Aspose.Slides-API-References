...
description: "TimeoutException 表示已分配给进程或操作的时间已到期。切勿手动创建此类实例。请改用 TimeoutException 类。切勿将 TimeoutException 类实例包装到 System::SmartPtr 中。"
type: docs
weight: 716
url: /zh/system/details_timeoutexception/
---
## Details_TimeoutException 类

TimeoutException 表示为进程或操作分配的时间已到期。不要手动创建此类的实例。请改用 TimeoutException 类。不要将 TimeoutException 类实例包装到 [System::SmartPtr](../smartptr/) 中。
