...
description: "在尝试对 null 引用进行解引用时会抛出 NullReferenceException。不要手动创建此类的实例。请改用 NullReferenceException 类。永远不要将 NullReferenceException 类实例包装到 System::SmartPtr 中。"
type: docs
weight: 599
url: /zh/system/details_nullreferenceexception/
---
## Details_NullReferenceException 类

NullReferenceException 在尝试对 null 引用进行解引用时抛出。不要手动创建此类的实例。请改用 NullReferenceException 类。永远不要将 NullReferenceException 类实例包装到 [System::SmartPtr](../smartptr/) 中。
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | 模拟 C# 风格的浮点比较，其中即使根据 IEC 60559:1989 标准 NaN 不等于任何值（包括 NaN），两个 NaN 仍被视为相等。 |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | 模拟 C# 风格的浮点比较，其中即使根据 IEC 60559:1989 标准 NaN 不等于任何值（包括 NaN），两个 NaN 仍被视为相等。 |
...
| static std::enable_if<\![IsSmartPtr](../issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../object/referenceequals/)(T const\&, std::nullptr_t) | 使用引用方式将值类型对象与 nullptr 比较。 |
