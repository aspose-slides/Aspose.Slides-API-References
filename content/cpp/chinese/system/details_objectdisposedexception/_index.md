...
description: "ObjectDisposedException 在对已释放的对象调用方法时抛出。不要手动创建此类的实例。请改用 ObjectDisposedException 类。不要将 ObjectDisposedException 类实例包装到 System::SmartPtr 中。"
type: docs
weight: 612
url: /zh/system/details_objectdisposedexception/
---
## Details_ObjectDisposedException 类

ObjectDisposedException 在对已释放的对象调用方法时抛出。不要手动创建此类的实例。请改用 ObjectDisposedException 类。不要将 ObjectDisposedException 类实例包装到 [System::SmartPtr](../smartptr/) 中。
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | 模拟 C# 风格的浮点比较，其中两个 NaN 被视为相等，尽管根据 IEC 60559:1989 标准 NaN 与任何值（包括 NaN）都不相等。 |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | 模拟 C# 风格的浮点比较，其中两个 NaN 被视为相等，尽管根据 IEC 60559:1989 标准 NaN 与任何值（包括 NaN）都不相等。 |
...
| **bool** [ReferenceEquals](../object/referenceequals/)([String](../string/) const\&, std::nullptr_t) | 针对字符串和 nullptr 情况的 [Object::ReferenceEquals](../object/referenceequals/) 特化。 |
...
| virtual const char * [what](../details_exception/what/)() const | 实现 [what()](../details_exception/what/) 方法，该方法被 [ExceptionWrapper](../exceptionwrapper/) 类调用。尽管此类未从 std::exception 继承，但派生类可以使用受保护/私有成员来实现其逻辑。将此方法实现移动到 [ExceptionWrapper](../exceptionwrapper/) 可能会破坏该逻辑。 |
| virtual  [~Object](../object/~object/)() | 销毁对象。释放所有内部数据结构。 |

## 参见

* 类 [Details_InvalidOperationException](../details_invalidoperationexception/)
* 命名空间 [System](../)
* 库 [Aspose.Slides](../../)