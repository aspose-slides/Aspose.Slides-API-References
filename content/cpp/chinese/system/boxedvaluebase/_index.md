...
description: "一个基类，定义接口并实现表示装箱值的派生类的一些基本方法。此类的对象应仅使用 System::MakeObject() 函数分配。不要在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装为 System::SmartPtr 指针，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 131
url: /zh/system/boxedvaluebase/
---
## BoxedValueBase 类

一个基类，定义接口并实现表示装箱值的派生类的一些基本方法。此类的对象应仅使用 [System::MakeObject()](../makeobject/) 函数分配。不要在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装为 [System::SmartPtr](../smartptr/) 指针，并使用该指针将其作为参数传递给函数。
...
| virtual [TypeCode](../typecode/) [GetTypeCode](./gettypecode/)() const | 返回表示当前对象所包装值的类型的值。 |
| virtual **uint64_t** [GetUnsignedLongLongValue](./getunsignedlonglongvalue/)() const | 将当前对象所包装的值转换为 64 位整数。 |
| virtual **bool** [Is](../object/is/)(const [TypeInfo](../typeinfo/)\&) const | 检查对象是否表示 targetType 描述的类型实例。相当于 C# 的 'is' 运算符。 |
| virtual **bool** [IsBoxedEnum](./isboxedenum/)() | 确定当前对象是否表示枚举类型的包装值。 |
...
|  [Object](../object/object/)([Object](../object/) const\&) | 复制构造函数。实际上不复制任何内容，只是初始化新对象并允许子类的复制构造。 |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | 赋值运算符。实际上不复制任何内容，只是初始化新对象并允许子类的复制构造。 |
| static [SharedPtr](../sharedptr/)\<[Object](../object/)\> [Parse](./parse/)(const [TypeInfo](../typeinfo/)\&, const [String](../string/)\&, **bool**) | 将指定枚举中具有指定名称的枚举常量的值装箱。参数指定在解释指定枚举常量名称的字符串时是否应忽略大小写。 |
| static [SharedPtr](../sharedptr/)\<[Object](../object/)\> [Parse](./parse/)(const [TypeInfo](../typeinfo/)\&, const [String](../string/)\&) | 将指定枚举中具有指定名称的枚举常量的值装箱。 |
...
| [System::String](../string/) [ToString](./tostring/)(const [System::String](../string/)\&) const | 使用指定的格式字符串将装箱对象转换为字符串。 |
| virtual [String](../string/) [ToString](./tostring/)() const | 相当于 C# [Object.ToString()](../object/tostring/) 方法。支持将自定义对象转换为字符串。 |
