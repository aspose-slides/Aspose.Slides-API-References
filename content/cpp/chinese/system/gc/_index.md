...
description: 表示一种模拟的垃圾回收，实际上更像一个不执行任何操作的存根。这是一个静态类型，没有实例服务。绝不应以任何方式创建其实例。
type: docs
weight: 872
url: /zh/system/gc/
---
...
表示一种模拟的垃圾回收，实际上更像一个不执行任何操作的存根。这是一个静态类型，没有实例服务。绝不应以任何方式创建其实例。
...
| static **int64_t** [GetTotalAllocatedBytes](./gettotalallocatedbytes/)(**bool**) | 返回已分配的总字节数。 |
| static **int64_t** [GetTotalMemory](./gettotalmemory/)(**bool**) | 返回当前进程当前已分配的私有内存字节数。 |
| virtual const [TypeInfo](../typeinfo/)\& [GetType](../object/gettype/)() const | 获取对象的实际类型。相当于 C# [System.Object.GetType()](../object/gettype/) 调用。 |
| virtual **bool** [Is](../object/is/)(const [TypeInfo](../typeinfo/)\&) const | 检查对象是否为目标类型 targetType 所描述的实例。相当于 C# 的 'is' 运算符。 |
...
| **bool** [ReferenceEquals](../object/referenceequals/)([String](../string/) const\&, [String](../string/) const\&) | [Object::ReferenceEquals](../object/referenceequals/) 在字符串情况中的特化。 |
| int [RemovedSharedRefs](../object/removedsharedrefs/)(int) | 按指定值减少共享引用计数。 |
| virtual void [SetTemplateWeakPtr](../object/settemplateweakptr/)(**uint32_t**) | 将第 n 个模板参数设为弱指针（而不是共享指针）。允许在容器中将指针切换为弱模式。 |
