...
description: 检查引用的对象是否不为 null。
type: docs
weight: 66
url: /zh/system/weakreference_tmpl_t__end_tmpl/operator_not_equal/
---
...
如果引用的对象不为 null 则返回 true，否则返回 false。

## WeakReference< T >::operator!=(const WeakReference\<T\>\&) const 方法

将引用的对象与另一个 WeakReference 类的实例进行比较。
...
如果比较的对象引用不同的对象则返回 true，如果对象相同则返回 false。
