...
description: 将包含数字字符串表示形式的指定字符串转换为等效的 8 位无符号整数。
type: docs
weight: 1
url: /zh/system/byte/parse/
---
## Byte::Parse(const String\&) 方法

将包含数字字符串表示形式的指定字符串转换为等效的 8 位无符号整数。
...
将包含数字字符串表示形式的指定字符串转换为等效的 8 位无符号整数，使用提供的格式化信息。
...
将包含数字字符串表示形式的指定字符串转换为等效的 8 位无符号整数，使用提供的格式化信息和数字样式。
