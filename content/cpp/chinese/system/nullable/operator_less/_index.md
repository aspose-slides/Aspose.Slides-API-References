...
type: docs
weight: 170
url: /zh/system/nullable/operator_less/
---
...
确定当前对象表示的值是否小于通过对这些值应用 [operator<()](./) 得到的指定值。
...
如果当前对象表示的值小于指定值，则返回 true；否则为 false

## Nullable::operator<(const Nullable\<T1\>\&) const 方法

确定当前对象表示的值是否小于通过对这些值应用 [operator<()](./) 得到的指定 [Nullable](../) 对象表示的值。
...
如果当前对象表示的值小于指定 [Nullable](../) 对象表示的值，则返回 true；否则为 false
