...
description: "当操作系统因 I/O 错误或安全错误而拒绝访问时，会抛出 UnauthorizedAccessException。不要手动创建此类的实例。请改用 UnauthorizedAccessException 类。不要将 UnauthorizedAccessException 类的实例包装到 System::SmartPtr 中。"
type: docs
weight: 755
url: /zh/system/details_unauthorizedaccessexception/
---
## Details_UnauthorizedAccessException 类

UnauthorizedAccessException 在操作系统因 I/O 错误或安全错误而拒绝访问时抛出。不要手动创建此类的实例。请改用 UnauthorizedAccessException 类。不要将 UnauthorizedAccessException 类的实例包装到 [System::SmartPtr](../smartptr/) 中。
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | 模拟 C# 样式的浮点比较，其中两个 NaN 被视为相等，即使根据 IEC 60559:1989 标准，NaN 不等于任何值，包括 NaN。 |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | 模拟 C# 样式的浮点比较，其中两个 NaN 被视为相等，即使根据 IEC 60559:1989 标准，NaN 不等于任何值，包括 NaN。 |
...
| virtual const char * [what](../details_exception/what/)() const | 实现 [what()](../details_exception/what/) 方法，该方法由 [ExceptionWrapper](../exceptionwrapper/) 类调用。尽管此类未从 std::exception 继承，派生类仍可使用受保护/私有成员实现其逻辑。将此方法实现移动到 [ExceptionWrapper](../exceptionwrapper/) 可能会破坏该逻辑。 |
