...
description: "ArgumentException 在调用的方法传入的参数无效时抛出。请勿手动创建此类实例。请改用 ArgumentException 类。不要将 ArgumentException 类实例包装到 System::SmartPtr 中。"
type: docs
weight: 326
url: /zh/system/details_argumentexception/
---
## Details_ArgumentException 类

ArgumentException 在调用的方法传入的参数无效时抛出。请勿手动创建此类实例。请改用 ArgumentException 类。不要将 ArgumentException 类实例包装到 [System::SmartPtr](../smartptr/) 中。
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | 模拟 C# 风格的浮点比较，在该比较中，即使根据 IEC 60559:1989，NaN 不等于任何值（包括 NaN），两个 NaN 仍被视为相等。 |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | 模拟 C# 风格的浮点比较，在该比较中，即使根据 IEC 60559:1989，NaN 不等于任何值（包括 NaN），两个 NaN 仍被视为相等。 |
...
| **int32_t** [get_HResult](../details_exception/get_hresult/)() const | 返回一个 32 位整数值，即与当前对象所表示的异常关联的 HRESULT 代码。 |
...
| virtual **int32_t** [GetHashCode](../object/gethashcode/)() const | 相当于 C# [Object.GetHashCode()](../object/gethashcode/) 方法。启用自定义对象的哈希计算。 |
...
|  [Object](../object/object/)([Object](../object/) const\&) | 复制构造函数。实际上不拷贝任何内容，只是初始化新对象并允许子类的复制构造。 |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | 赋值运算符。实际上不拷贝任何内容，只是初始化新对象并允许子类的复制构造。 |
...
| virtual const char * [what](../details_exception/what/)() const | 实现 [what()](../details_exception/what/) 方法，该方法由 [ExceptionWrapper](../exceptionwrapper/) 类调用。尽管此类未从 std::exception 派生，派生类仍可使用受保护/私有成员来实现其逻辑。将此方法的实现移动到 [ExceptionWrapper](../exceptionwrapper/) 可能会破坏该逻辑。 |
| virtual  [~Object](../object/~object/)() | 销毁对象。释放所有内部数据结构。 |

## 备注

ArgumentNullException 在调用的方法传入 null 参数而该方法期望非 null 值时抛出。请勿手动创建此类实例。请改用 ArgumentNullException 类。不要将 ArgumentNullException 类实例包装到 [System::SmartPtr](../smartptr/) 中。
