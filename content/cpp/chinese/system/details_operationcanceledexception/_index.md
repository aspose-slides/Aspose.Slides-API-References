...
description: "OperationCanceledException 在线程执行的操作被取消时抛出。切勿手动创建此类的实例。请改用 OperationCanceledException 类。切勿将 OperationCanceledException 类的实例包装到 System::SmartPtr 中。"
type: docs
weight: 625
url: /zh/system/details_operationcanceledexception/
---
## Details_OperationCanceledException 类

OperationCanceledException 在线程执行的操作被取消时抛出。切勿手动创建此类的实例。请改用 OperationCanceledException 类。切勿将 OperationCanceledException 类的实例包装到 [System::SmartPtr](../smartptr/) 中。
...
| static **bool** [Equals](../object/equals/)(**float** const\&, **float** const\&) | 模拟 C# 风格的浮点比较，在此比较中，即使根据 IEC 60559:1989，NaN 与任何值（包括 NaN）都不相等，也将两个 NaN 视为相等。 |
| static **bool** [Equals](../object/equals/)(**double** const\&, **double** const\&) | 模拟 C# 风格的浮点比较，在此比较中，即使根据 IEC 60559:1989，NaN 与任何值（包括 NaN）都不相等，也将两个 NaN 视为相等。 |
...
|  [Object](../object/object/)([Object](../object/) const\&) | 复制构造函数。实际上不复制任何内容，只是初始化新对象并允许为子类进行复制构造。 |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | 赋值运算符。实际上不复制任何内容，只是初始化新对象并允许为子类进行复制构造。 |
...
| virtual const char * [what](../details_exception/what/)() const | 实现 [what()](../details_exception/what/) 方法，该方法由 [ExceptionWrapper](../exceptionwrapper/) 类调用。尽管此类未从 std::exception 派生，派生类仍可使用受保护/私有成员来实现其逻辑。将此方法实现移动到 [ExceptionWrapper](../exceptionwrapper/) 可能会破坏该逻辑。 |
