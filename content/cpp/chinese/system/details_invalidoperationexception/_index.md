...
description: "当对状态与此调用不一致的对象调用方法时抛出的异常。不要手动创建此类的实例。请改用 InvalidOperationException 类。切勿将 InvalidOperationException 类的实例包装进 System::SmartPtr。"
type: docs
weight: 508
url: /zh/system/details_invalidoperationexception/
---
## Details_InvalidOperationException 类

该异常在对状态与此调用不一致的对象调用方法时抛出。不要手动创建此类的实例。请使用 InvalidOperationException 类。永不将 InvalidOperationException 类的实例包装进 [System::SmartPtr](../smartptr/)。
...
|  [Object](../object/object/)([Object](../object/) const\&) | 复制构造函数。实际上并不复制任何内容，只是初始化新对象并支持子类的复制构造。 |
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | 赋值运算符。实际上并不复制任何内容，只是初始化新对象并支持子类的复制构造。 |
