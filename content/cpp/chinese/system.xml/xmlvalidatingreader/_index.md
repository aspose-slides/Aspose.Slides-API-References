...
description: 表示提供文档类型定义（DTD）、XML-Data Reduced（XDR）模式以及 XML Schema 定义语言（XSD）验证的读取器。
type: docs
weight: 547
url: /zh/system.xml/xmlvalidatingreader/
---
## XmlValidatingReader 类


表示提供文档类型定义 (DTD)、XML-Data Reduced (XDR) 架构以及 XML [Schema](../../system.xml.schema/) 定义语言 (XSD) 验证的读取器。
...
已弃用
:   此类已过时。建议使用 [XmlReaderSettings](../xmlreadersettings/) 类和 [XmlReader::Create](../xmlreader/create/) 方法来创建验证 XML 读取器。

该类的对象应仅通过 [System::MakeObject()](../../system/makeobject/) 函数分配。切勿在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 [System::SmartPtr](../../system/smartptr/) 指针中，并使用该指针将其作为参数传递给函数。
