---
title: ReadElementContentAsLong()
second_title: Aspose.Slides C++ API 参考
description: 读取当前元素并将内容作为 64 位有符号整数返回。
type: docs
weight: 560
url: /zh/system.xml/xmlreader/readelementcontentaslong/
---
## XmlReader::ReadElementContentAsLong() 方法

读取当前元素并将内容作为 64 位有符号整数返回。

```cpp
virtual int64_t System::Xml::XmlReader::ReadElementContentAsLong()
```

### 返回值

元素内容作为 64 位有符号整数。

## XmlReader::ReadElementContentAsLong(String, String) 方法

检查指定的本地名称和命名空间 URI 是否与当前元素匹配，然后读取当前元素并将内容作为 64 位有符号整数返回。

```cpp
virtual int64_t System::Xml::XmlReader::ReadElementContentAsLong(String localName, String namespaceURI)
```

### 参数

| Parameter | Type | Description |
| --- | --- | --- |
| localName | [String](../../../system/string/) | 元素的本地名称。 |
| namespaceURI | [String](../../../system/string/) | 元素的命名空间 URI。 |

### 返回值

元素内容作为 64 位有符号整数。

## 另见

* 类 [XmlReader](../)
* 类 [String](../../../system/string/)
* 命名空间 [System::Xml](../../)
* 库 [Aspose.Slides](../../../)