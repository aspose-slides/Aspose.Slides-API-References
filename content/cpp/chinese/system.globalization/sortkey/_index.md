...
description: "将字符串映射为其排序键。此类的对象只能使用 System::MakeObject() 函数分配。永远不要在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言错误。始终将此类包装成 System::SmartPtr 指针，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 287
url: /zh/system.globalization/sortkey/
---
## SortKey 类

将字符串映射为其排序键。此类的对象只能使用 [System::MakeObject()](../../system/makeobject/) 函数分配。永远不要在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言错误。始终将此类包装成 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针将其作为参数传递给函数。
...
| Method | Description |
| --- | --- |
| static int [Compare](./compare/)(const [SortKeyPtr](../sortkeyptr/)\&, const [SortKeyPtr](../sortkeyptr/)\&) | 比较两个排序键。 |
| **bool** [Equals](./equals/)([SharedPtr](../../system/sharedptr/)\<[Object](../../system/object/)\>) override | 检查指定的对象是否等于当前 [SortKey](./) 对象。 |
...
| virtual [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_KeyData](./get_keydata/)() | 获取表示当前对象的字节数组。 |
| virtual [String](../../system/string/) [get_OriginalString](./get_originalstring/)() | 获取用于创建此对象的原始字符串。 |
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | 获取与对象关联的引用计数数据结构。 |
| int [GetHashCode](./gethashcode/)() const override | 获取当前 [SortKey](./) 对象的哈希码。 |
...
| [String](../../system/string/) [ToString](./tostring/)() const override | 将当前对象转换为其字符串表示。 |
