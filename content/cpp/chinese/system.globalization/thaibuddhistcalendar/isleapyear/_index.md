---
title: IsLeapYear()
second_title: Aspose.Slides for C++ API 参考
description: 检查年份是否为闰年。
type: docs
weight: 196
url: /zh/system.globalization/thaibuddhistcalendar/isleapyear/
---
## ThaiBuddhistCalendar::IsLeapYear(int, int) const 方法

检查年份是否为闰年。

```cpp
bool System::Globalization::ThaiBuddhistCalendar::IsLeapYear(int year, int era) const override
```

### 参数

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| year | int | 年份。 |
| era | int | 纪元。 |

### 返回值

如果指定的年份是闰年则返回 true，false 否则。

## ThaiBuddhistCalendar::IsLeapYear(int) const 方法

检查年份是否为闰年。

```cpp
virtual bool System::Globalization::Calendar::IsLeapYear(int year) const
```

### 参数

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| year | int | 年份。 |

### 返回值

如果指定的年份是闰年则返回 true，false 否则。

## ThaiBuddhistCalendar::IsLeapYear(int, int) const 方法

检查年份是否为闰年。

```cpp
virtual bool System::Globalization::Calendar::IsLeapYear(int year, int era) const=0
```

### 参数

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| year | int | 年份。 |
| era | int | 纪元。 |

### 返回值

如果指定的年份是闰年则返回 true，false 否则。

## 另请参见

* 类 [ThaiBuddhistCalendar](../)
* 命名空间 [System::Globalization](../../)
* 库 [Aspose.Slides](../../../)