...
description: "泰国佛教历。此类的对象应仅使用 System::MakeObject() 函数分配。切勿在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 System::SmartPtr 指针中，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 378
url: /zh/system.globalization/thaibuddhistcalendar/
---
## ThaiBuddhistCalendar 类

泰国佛教历。此类的对象应仅使用 [System::MakeObject()](../../system/makeobject/) 函数分配。切勿在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装为 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针将其作为参数传递给函数。
...
| [DateTime](../../system/datetime/) [get_MaxSupportedDateTime](./get_maxsupporteddatetime/)() const override | 日历所支持的最大时间点。 |
| [DateTime](../../system/datetime/) [get_MinSupportedDateTime](./get_minsupporteddatetime/)() const override | 日历所支持的最小时间点。 |
...
| virtual void [set_TwoDigitYearMax](../calendar/set_twodigityearmax/)(int) | 设置可以用两位数字表示的最后一年。 |
...
| 字段 | 描述 |
| --- | --- |
| static constexpr [ThaiBuddhistEra](./thaibuddhistera/) | 当前泰国佛教时代。 |
