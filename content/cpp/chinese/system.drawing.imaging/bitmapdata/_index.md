...
description: "表示位图图像的一组属性。此类的对象只能使用 System::MakeObject() 函数分配。不要在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装成 System::SmartPtr 指针，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 1
url: /zh/system.drawing.imaging/bitmapdata/
---
## BitmapData 类

表示位图图像的一组属性。此类的对象只能使用 [System::MakeObject()](../../system/makeobject/) 函数分配。不要在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装成 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针将其作为参数传递给函数。
...
| int [get_Height](./get_height/)() const | 返回图像的像素高度。 |
| [PixelFormat](../pixelformat/) [get_PixelFormat](./get_pixelformat/)() const | 返回位图图像的像素格式。 |
| IntPtr [get_Scan0](./get_scan0/)() const | 返回位图中第一个像素数据的地址。 |
| int [get_Stride](./get_stride/)() const | 返回图像的步幅宽度（字节）。 |
| int [get_Width](./get_width/)() const | 返回图像的像素宽度。 |
...
| void [set_Height](./set_height/)(int) | 设置图像的像素高度。 |
| void [set_PixelFormat](./set_pixelformat/)([PixelFormat](../pixelformat/)) | 设置位图图像的像素格式。 |
| void [set_Scan0](./set_scan0/)(IntPtr) | 设置位图中第一个像素数据的地址。 |
| void [set_Stride](./set_stride/)(int) | 设置图像的步幅宽度（字节）。 |
| void [set_Width](./set_width/)(int) | 设置图像的像素宽度。 |
