...
description: "表示 EncoderParameter 对象的数组。此类对象应仅使用 System::MakeObject() 函数进行分配。切勿在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装成 System::SmartPtr 指针，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 79
url: /zh/system.drawing.imaging/encoderparameters/
---
## EncoderParameters 类

表示 [EncoderParameter](../encoderparameter/) 对象的数组。此类对象只能使用 [System::MakeObject()](../../system/makeobject/) 函数进行分配。切勿在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装成 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针将其作为参数传递给函数。
...
| [System::ArrayPtr](../../system/arrayptr/)\<[EncoderParameterPtr](../encoderparameterptr/)\> [get_Param](./get_param/)() const | 返回由当前 [EncoderParameters](./) 对象表示的 [EncoderParameter](../encoderparameter/) 对象数组。 |
...
| void [set_Param](./set_param/)(const [System::ArrayPtr](../../system/arrayptr/)\<[EncoderParameterPtr](../encoderparameterptr/)\>\&) | 将 [EncoderParameter](../encoderparameter/) 对象数组分配给当前 [EncoderParameters](./) 对象。 |
