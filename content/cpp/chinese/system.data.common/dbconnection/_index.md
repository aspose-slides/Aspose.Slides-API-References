...
description: "数据库连接。此类的对象应仅使用 System::MakeObject() 函数分配。切勿在栈上或使用 operator new 创建此类型的实例，因为这将导致运行时错误和/或断言失败。始终将此类包装为 System::SmartPtr 指针，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 14
url: /zh/system.data.common/dbconnection/
---
## DbConnection 类


数据库连接。此类的对象应仅使用 [System::MakeObject()](../../system/makeobject/) 函数分配。切勿在栈上或使用 operator new 创建此类型的实例，因为这将导致运行时错误和/或断言失败。始终将此类包装为 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针将其作为参数传递给函数。
...
| virtual [String](../../system/string/) [get_ConnectionString](./get_connectionstring/)() const | RTTI 信息。 |
...
| virtual void [Open](./open/)() | 打开到数据库的连接。 |
...
| virtual void [set_ConnectionString](./set_connectionstring/)([String](../../system/string/)) const | 设置连接信息（例如服务器和端口）。 |
