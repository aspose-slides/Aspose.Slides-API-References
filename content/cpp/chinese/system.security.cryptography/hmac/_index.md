...
description: "所有 Hash-based Message Authentication Code (HMAC) 的实现必须继承此抽象类。该类的对象只能使用 System::MakeObject() 函数分配。绝不能在栈上创建此类型的实例或使用 new 运算符，否则会导致运行时错误和/或断言失败。始终将此类包装为 System::SmartPtr 指针，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 235
url: /zh/system.security.cryptography/hmac/
---
## HMAC 类
...
| static [SharedPtr](../../system/sharedptr/)\<[HMAC](./)\> [Create](./create/)() | 未实现。 |
| static [SharedPtr](../../system/sharedptr/)\<[HashAlgorithm](../hashalgorithm/)\> [Create](../hashalgorithm/create/)(const [String](../../system/string/)\&) | 根据名称创建哈希算法。 |
...
| virtual [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_Hash](../hashalgorithm/get_hash/)() | 获取已计算的哈希码值。 |
| virtual int [get_HashSize](../hashalgorithm/get_hashsize/)() | 获取已计算的哈希值的字节大小。 |
