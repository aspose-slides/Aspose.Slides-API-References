...
description: "受管理的 TripleDES 实现。仅支持带有 None 填充的 ECB 和 CFB 模式，以及带有 None、Zeros 和 PKCS7 填充的 CBC 模式。此类的对象应仅使用 System::MakeObject() 函数进行分配。切勿在栈上或使用 operator new 创建此类型的实例，因为会导致运行时错误和/或断言故障。始终将此类包装在 System::SmartPtr 指针中，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 690
url: /zh/system.security.cryptography/tripledesmanaged/
---
## TripleDESManaged 类

受管理的 [TripleDES](../tripledes/) 实现。仅支持带有 None 填充的 ECB 和 CFB 模式，以及带有 None、Zeros 和 PKCS7 填充的 CBC 模式。此类的对象应仅使用 [System::MakeObject()](../../system/makeobject/) 函数进行分配。切勿在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 [System::SmartPtr](../../system/smartptr/) 指针中，并使用该指针将其作为参数传递给函数。
...
| static **bool** [Equals](../../system/object/equals/)(**double** const\&, **double** const\&) | 模拟 C# 风格的双精度浮点比较，其中两个 NaN 被视为相等，尽管根据 IEC 60559:1989，NaN 不等于任何值，包括 NaN 本身。 |
