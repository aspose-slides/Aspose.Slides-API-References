...
description: "对称算法接受的密钥大小集合。此类的对象只能使用 System::MakeObject() 函数分配。切勿在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言失败。始终将此类包装为 System::SmartPtr 指针，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 300
url: /zh/system.security.cryptography/keysizes/
---
## KeySizes 类

对称算法接受的密钥大小集合。此类的对象只能使用 [System::MakeObject()](../../system/makeobject/) 函数分配。切勿在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言失败。始终将此类包装为 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针将其作为参数传递给函数。
...
| int [get_MaxSize](./get_maxsize/)() const | 获取最大的有效密钥大小。 |
| int [get_MinSize](./get_minsize/)() const | 获取最小的有效密钥大小。 |
| int [get_SkipSize](./get_skipsize/)() const | 获取有效密钥大小的步长。 |
