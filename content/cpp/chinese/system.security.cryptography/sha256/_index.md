...
description: "计算输入数据的 SHA256 哈希。在 C++ 中，SHA256 不是抽象类。该类的对象应只能使用 System::MakeObject() 函数分配。切勿在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 System::SmartPtr 指针中，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 573
url: /zh/system.security.cryptography/sha256/
---
## SHA256 类

计算 [SHA256](./) 哈希以用于输入数据。在 C++ 中，[SHA256](./) 不是抽象类。此类的对象只能通过 [System::MakeObject()](../../system/makeobject/) 函数分配。绝不能在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 [System::SmartPtr](../../system/smartptr/) 指针中，并使用该指针将其作为参数传递给函数。
...
| static [SharedPtr](../../system/sharedptr/)\<[SHA256](./)\> [Create](./create/)() | 创建哈希算法实例。 |
| static [SharedPtr](../../system/sharedptr/)\<[SHA256](./)\> [Create](./create/)(const [String](../../system/string/)\&) | 创建哈希算法实例。 |
...
| virtual [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_Hash](../hashalgorithm/get_hash/)() | 获取计算得到的哈希代码的值。 |
