...
description: Group collection 指针。此类型是用于管理其他对象删除的指针。它应在栈上分配，并以值或 const 引用的方式传递给函数。
type: docs
weight: 53
url: /zh/system.text.regularexpressions/groupcollectionptr/
---
## GroupCollectionPtr 类

[Group](../group/) 集合指针。此类型是一种用于管理其他对象删除的指针。它应该在栈上分配，并以值或 const 引用的方式传递给函数。
...
| 方法 | 描述 |
| --- | --- |
| auto [begin](../../system/smartptr/begin/)() | 访问底层集合的 [begin()](../../system/smartptr/begin/) 方法的访问器。仅当 SmartPtr_ 为具有 [begin()](../../system/smartptr/begin/) 方法的特化类型时才会编译。 |
| auto [begin](../../system/smartptr/begin/)() const | 访问底层集合的 [begin()](../../system/smartptr/begin/) 方法的访问器。仅当 SmartPtr_ 为具有 [begin()](../../system/smartptr/begin/) 方法的特化类型时才会编译。 |
...
| std::enable_if_t\<Check::value\&&\!std::is_same\<Y, T\>::value\&&\!std::is_base_of\<Y, T\>::value, [SmartPtr](../../system/smartptr/)\<Y\>\> [Cast](../../system/smartptr/cast/)() const | 使用 dynamic_cast 将指针转换为派生类类型。 |
| std::enable_if_t<\!Check::value\&&\!std::is_same\<Y, T\>::value\&&\!std::is_base_of\<Y, T\>::value, [SmartPtr](../../system/smartptr/)\<Y\>\> [Cast](../../system/smartptr/cast/)() const | 使用 dynamic_cast 将指针转换为派生类类型。 |
| auto [cbegin](../../system/smartptr/cbegin/)() const | 访问底层集合的 [cbegin()](../../system/smartptr/cbegin/) 方法的访问器。仅当 SmartPtr_ 为具有 [cbegin()](../../system/smartptr/cbegin/) 方法的特化类型时才会编译。 |
| auto [cend](../../system/smartptr/cend/)() const | 访问底层集合的 [cend()](../../system/smartptr/cend/) 方法的访问器。仅当 SmartPtr_ 为具有 [cend()](../../system/smartptr/cend/) 方法的特化类型时才会编译。 |
...
| auto [end](../../system/smartptr/end/)() | 访问底层集合的 [end()](../../system/smartptr/end/) 方法的访问器。仅当 SmartPtr_ 为具有 [end()](../../system/smartptr/end/) 方法的特化类型时才会编译。 |
| auto [end](../../system/smartptr/end/)() const | 访问底层集合的 [end()](../../system/smartptr/end/) 方法的访问器。仅当 SmartPtr_ 为具有 [end()](../../system/smartptr/end/) 方法的特化类型时才会编译。 |
...
|  [GroupCollectionPtr](./groupcollectionptr/)(const [SharedPtr](../../system/sharedptr/)\<[GroupCollection](../groupcollection/)\>\&) | 类型转换构造函数。 |
| **bool** [Is](../../system/smartptr/is/)(const [System::TypeInfo](../../system/typeinfo/)\&) const | 检查指向的对象是否为特定类型或其子类型。遵循 C# 的 'is' 语义。 |
| **bool** [IsAliasingPtr](../../system/smartptr/isaliasingptr/)() const | 检查指针是否指向除自身所有对象之外的其他对象（由别名构造函数创建）。 |
...
| [GroupPtr](../groupptr/) [operator[]](./operator[]/)(size_t) const | [Group](../group/) 访问器。 |
| [SmartPtr_](../../system/smartptr/smartptr_/) [RemoveAliasing](../../system/smartptr/removealiasing/)() const | 移除指针的别名（由别名构造函数创建），确保它在共享模式下管理或在弱模式下跟踪相同的指向对象。 |
...
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<[Array](../../system/array/)\<Y\>\>\&, [SmartPtrMode](../../system/smartptrmode/)) | 通过创建不同类型的新数组来转换引用数组的类型。如果在 C# 中存在不受 C++ 支持的数组类型转换，则特别有用。 |
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const Y\&) | 初始化空数组。用于翻译某些 C# 代码构造。 |
|  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<P\>\&, [Pointee_](../../system/smartptr/pointee_/) *, [SmartPtrMode](../../system/smartptrmode/)) | 构造一个 [SmartPtr](../../system/smartptr/)，其所有权信息与指针的初始值共享，但持有一个不相关且未管理的指针 p。 |
...
| static const [System::TypeInfo](../../system/typeinfo/)\& [Type](../../system/smartptr/type/)() | 获取 Pointee_ 类型对应的 [System::TypeInfo](../../system/typeinfo/) 对象的快捷方式。 |
|  [~SmartPtr](../../system/smartptr/~smartptr/)() | 销毁 [SmartPtr](../../system/smartptr/) 对象。如有需要，减少指向对象的引用计数并删除对象。 |
