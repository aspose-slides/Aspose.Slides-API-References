...
description: "用于使使用 Control 类的翻译代码能够编译的虚拟类。方法未实现。此类的对象只能通过 System::MakeObject() 函数分配。切勿在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装成 System::SmartPtr 指针，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 1
url: /zh/system.windows.forms/control/
---
## Control 类

虚拟类，用于使使用 [Control](./) 类的翻译代码能够编译。方法未实现。此类的对象只能通过 [System::MakeObject()](../../system/makeobject/) 函数分配。切勿在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装成 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针将其作为参数传递给函数。
...
| 方法 | 描述 |
| --- | --- |
| [System::SharedPtr](../../system/sharedptr/)\<[System::Drawing::Graphics](../../system.drawing/graphics/)\> [CreateGraphics](./creategraphics/)() | RTTI 信息。 |
