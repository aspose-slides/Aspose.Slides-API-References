...
description: HashSet 类的前向声明。
type: docs
weight: 222
url: /zh/system.collections.generic/hashset/
---
## HashSet 类

Forward declaration of [HashSet](./) 类.
...
| [iterator](../ienumerable/iterator/) [begin](../ienumerable/begin/)() | 获取指向集合中第一个元素（如果有）的迭代器。由于 [GetEnumerator()](../ienumerable/getenumerator/) 返回 T 的拷贝对象，此迭代器不能用于更改引用的对象。 |
...
| virtual void [CopyTo](../icollection/copyto/)([System::ArrayPtr](../../system/arrayptr/)\<T\>, int) | 将集合的所有元素复制到现有数组元素中。 |
| [iterator](../ienumerable/iterator/) [end](../ienumerable/end/)() | 获取指向集合中最后一个元素（如果有）之后的迭代器。由于 [GetEnumerator()](../ienumerable/getenumerator/) 返回 T 的拷贝对象，此迭代器不能用于更改引用的对象。 |
...
|  [HashSet](./hashset/)(const [SharedPtr](../../system/sharedptr/)\<[IEqualityComparer](../iequalitycomparer/)\<T\>\>\&) | 创建使用指定相等比较器的空集合。 |
|  [HashSet](./hashset/)(const [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<T\>\>\&) | 基于可枚举值创建 hashset。 |
...
集合实现基于哈希。此类的对象只能使用 [System::MakeObject()](../../system/makeobject/) 函数分配。切勿在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言失败。始终将此类包装在 [System::SmartPtr](../../system/smartptr/) 指针中，并使用该指针将其作为参数传递给函数。
