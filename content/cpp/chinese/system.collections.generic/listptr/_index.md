...
description: 带有访问运算符的列表指针。此类型是用于管理其他对象删除的指针。应在栈上分配，并通过值或 const 引用传递给函数。
type: docs
weight: 456
url: /zh/system.collections.generic/listptr/
---
## ListPtr 类

[List](../list/) 指针，带有访问运算符。此类型是用于管理其他对象删除的指针。它应当在栈上分配，并通过值或 const 引用传递给函数。
...
| 方法 | 描述 |
| --- | --- |
| auto [begin](../../system/smartptr/begin/)() | 对底层集合的 [begin()](../../system/smartptr/begin/) 方法的访问器。仅在 SmartPtr_ 为具有 [begin()](../../system/smartptr/begin/) 方法的特化类型时编译。 |
| auto [begin](../../system/smartptr/begin/)() const | 对底层集合的 [begin()](../../system/smartptr/begin/) 方法的访问器。仅在 SmartPtr_ 为具有 [begin()](../../system/smartptr/begin/) 方法的特化类型时编译。 |
| std::enable_if_t\<std::is_same\<Y, T\>::value, [SmartPtr](../../system/smartptr/)\<Y\>\> [Cast](../../system/smartptr/cast/)() const | 将指针转换为其自身类型的指针。 |
...
| std::enable_if_t<\!Check::value\&&\!std::is_same\<Y, T\>::value\&&\!std::is_base_of\<Y, T\>::value, [SmartPtr](../../system/smartptr/)\<Y\>\> [Cast](../../system/smartptr/cast/)() const | 使用 dynamic_cast 将指针转换为派生类型。 |
| auto [cbegin](../../system/smartptr/cbegin/)() const | 对底层集合的 [cbegin()](../../system/smartptr/cbegin/) 方法的访问器。仅在 SmartPtr_ 为具有 [cbegin()](../../system/smartptr/cbegin/) 方法的特化类型时编译。 |
| auto [cend](../../system/smartptr/cend/)() const | 对底层集合的 [cend()](../../system/smartptr/cend/) 方法的访问器。仅在 SmartPtr_ 为具有 [cend()](../../system/smartptr/cend/) 方法的特化类型时编译。 |
| [SmartPtr](../../system/smartptr/)\<Y\> [const_pointer_cast](../../system/smartptr/const_pointer_cast/)() const | 使用 const_cast 对指向的对象进行转换，将指针转换为不同的类型。 |
| [SmartPtr](../../system/smartptr/)\<Y\> [dynamic_pointer_cast](../../system/smartptr/dynamic_pointer_cast/)() const | 使用 dynamic_cast 对指向的对象进行转换，将指针转换为不同的类型。 |
| auto [end](../../system/smartptr/end/)() | 对底层集合的 [end()](../../system/smartptr/end/) 方法的访问器。仅在 SmartPtr_ 为具有 [end()](../../system/smartptr/end/) 方法的特化类型时编译。 |
| auto [end](../../system/smartptr/end/)() const | 对底层集合的 [end()](../../system/smartptr/end/) 方法的访问器。仅在 SmartPtr_ 为具有 [end()](../../system/smartptr/end/) 方法的特化类型时编译。 |
...
|  [ListPtr](./listptr/)(const [SharedPtr](../../system/sharedptr/)\<[List](../list/)\<T\>\>\&) | 将指针初始化为指定的列表。 |
...
| [SmartPtr_](../../system/smartptr/smartptr_/)\& [operator=](../../system/smartptr/operator_equal/)([SmartPtr_](../../system/smartptr/smartptr_/)\&&) | 移动赋值 [SmartPtr](../../system/smartptr/) 对象。x 将变得不可使用。 |
| [SmartPtr_](../../system/smartptr/smartptr_/)\& [operator=](../../system/smartptr/operator_equal/)(const [SmartPtr_](../../system/smartptr/smartptr_/)\&) | 拷贝赋值 [SmartPtr](../../system/smartptr/) 对象。 |
| [SmartPtr_](../../system/smartptr/smartptr_/)\& [operator=](../../system/smartptr/operator_equal/)(const [SmartPtr](../../system/smartptr/)\<Q\>\&) | 拷贝赋值 [SmartPtr](../../system/smartptr/) 对象。进行必要的类型转换。 |
...
| **bool** [operator==](./operator_equal_equal/)(std::nullptr_t) const | 检查 [List](../list/) 指针是否为空。 |
| std::vector\<T\>::reference [operator[]](./operator[]/)(int) | 访问器。 |
| std::vector\<T\>::const_reference [operator[]](./operator[]/)(int) const | 访问器。 |
| [SmartPtr_](../../system/smartptr/smartptr_/) [RemoveAliasing](../../system/smartptr/removealiasing/)() const | 从指针中移除别名（由别名构造函数创建），确保它管理（如果是共享）或跟踪（如果是弱）指向的同一对象。 |
...
| void [SetContainedTemplateWeakPtr](../../system/smartptr/setcontainedtemplateweakptr/)(**uint32_t**) const | 在指向的对象上调用 SetTemplateWeakPtr() 方法（如果存在）。 |
...
|  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<Q\>\&, [SmartPtrMode](../../system/smartptrmode/)) | 拷贝构造 [SmartPtr](../../system/smartptr/) 对象。之后两个指针指向同一对象。如允许，执行类型转换。 |
...
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<[Array](../../system/array/)\<Y\>\>\&, [SmartPtrMode](../../system/smartptrmode/)) | 通过创建不同类型的新数组来转换引用数组的类型。如果在 C# 中有数组类型转换但在 C++ 不支持，此方法很有用。 |
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const Y\&) | 初始化空数组。用于翻译某些 C# 代码结构。 |
|  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<P\>\&, [Pointee_](../../system/smartptr/pointee_/) *, [SmartPtrMode](../../system/smartptrmode/)) | 构造一个 [SmartPtr](../../system/smartptr/)，它共享 ptr 的所有权信息，但持有一个不相关且未管理的指针 p。 |
| [SmartPtr](../../system/smartptr/)\<Y\> [static_pointer_cast](../../system/smartptr/static_pointer_cast/)() const | 使用 static_cast 对指向对象进行转换，将指针转换为不同类型。 |
| [SmartPtr](../../system/smartptr/)\<[Object](../../system/object/)\> [ToObjectPtr](../../system/smartptr/toobjectptr/)() const | 将任意指针类型转换为指向 [Object](../../system/object/) 的指针。不要求 Pointee_ 类型完整。 |
