...
description: 队列指针。此类型是用于管理其他对象删除的指针。它应在栈上分配，并以值或 const 引用方式传递给函数。
type: docs
weight: 482
url: /zh/system.collections.generic/queueptr/
---
## QueuePtr 类

[Queue](../queue/) 指针。此类型是用于管理其他对象删除的指针。它应在栈上分配，并以值传递或以 const 引用传递给函数。
...
| 方法 | 描述 |
| --- | --- |
| auto [begin](../../system/smartptr/begin/)() | 访问底层集合的 [begin()](../../system/smartptr/begin/) 方法的访问器。仅在 SmartPtr_ 为特化类型且具有 [begin()](../../system/smartptr/begin/) 方法时编译。 |
| auto [begin](../../system/smartptr/begin/)() const | 访问底层集合的 [begin()](../../system/smartptr/begin/) 方法的访问器。仅在 SmartPtr_ 为特化类型且具有 [begin()](../../system/smartptr/begin/) 方法时编译。 |
| std::enable_if_t\<std::is_same\<Y, T\>::value, [SmartPtr](../../system/smartptr/)\<Y\>\> [Cast](../../system/smartptr/cast/)() const | 将指针转换为其本身类型。 |
...
| std::enable_if_t<\!Check::value\&&\!std::is_same\<Y, T\>::value\&&\!std::is_base_of\<Y, T\>::value, [SmartPtr](../../system/smartptr/)\<Y\>\> [Cast](../../system/smartptr/cast/)() const | 使用 dynamic_cast 将指针转换为派生类型。 |
| auto [cbegin](../../system/smartptr/cbegin/)() const | 访问底层集合的 [cbegin()](../../system/smartptr/cbegin/) 方法的访问器。仅在 SmartPtr_ 为特化类型且具有 [cbegin()](../../system/smartptr/cbegin/) 方法时编译。 |
| auto [cend](../../system/smartptr/cend/)() const | 访问底层集合的 [cend()](../../system/smartptr/cend/) 方法的访问器。仅在 SmartPtr_ 为特化类型且具有 [cend()](../../system/smartptr/cend/) 方法时编译。 |
...
| auto [end](../../system/smartptr/end/)() | 访问底层集合的 [end()](../../system/smartptr/end/) 方法的访问器。仅在 SmartPtr_ 为特化类型且具有 [end()](../../system/smartptr/end/) 方法时编译。 |
| auto [end](../../system/smartptr/end/)() const | 访问底层集合的 [end()](../../system/smartptr/end/) 方法的访问器。仅在 SmartPtr_ 为特化类型且具有 [end()](../../system/smartptr/end/) 方法时编译。 |
...
| [SmartPtr_](../../system/smartptr/smartptr_/)\& [operator=](../../system/smartptr/operator_equal/)([SmartPtr_](../../system/smartptr/smartptr_/)\&&) | 对 [SmartPtr](../../system/smartptr/) 对象进行移动赋值。x 将变为不可用。 |
...
|  [QueuePtr](./queueptr/)(const [SharedPtr](../../system/sharedptr/)\<[Queue](../queue/)\<T\>\>\&) | 构造指向特定队列的指针。 |
| [SmartPtr_](../../system/smartptr/smartptr_/) [RemoveAliasing](../../system/smartptr/removealiasing/)() const | 从指针中移除别名（由别名构造函数创建），确保它（若为共享则管理，若为弱则跟踪）指向相同的对象。 |
...
|  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<Q\>\&, [SmartPtrMode](../../system/smartptrmode/)) | 拷贝构造 [SmartPtr](../../system/smartptr/) 对象。之后两个指针指向同一对象。如果允许，则执行类型转换。 |
...
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<[Array](../../system/array/)\<Y\>\>\&, [SmartPtrMode](../../system/smartptrmode/)) | 通过创建不同类型的新数组来转换引用数组的类型。如果在 C# 中有数组类型转换而 C++ 不支持，此功能非常有用。 |
