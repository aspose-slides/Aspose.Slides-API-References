...
description: 带有运算符重载的字典指针类。此类型是用于管理其他对象删除的指针。它应在栈上分配，并通过值或 const 引用传递给函数。
type: docs
weight: 170
url: /zh/system.collections.generic/dictionaryptr/
---
## DictionaryPtr 类

[Dictionary](../dictionary/) 指针类，带有运算符重载。此类型用于管理其他对象的删除。它应在栈上分配，并以值或 const 引用方式传递给函数。
...
|  [DictionaryPtr](./dictionaryptr/)() | 初始化为空指针。 |
|  [DictionaryPtr](./dictionaryptr/)(const [SharedPtr](../../system/sharedptr/)\<[Dictionary](../dictionary/)\<T, V\>\>\&) | 转换指针类型。 |
...
| [SmartPtr_](../../system/smartptr/smartptr_/)\& [operator=](../../system/smartptr/operator_equal/)(const [SmartPtr](../../system/smartptr/)\<Q\>\&) | 对 [SmartPtr](../../system/smartptr/) 对象进行拷贝赋值，并进行必要的类型转换。 |
...
| V\& [operator[]](./operator[]/)(const X\&) const | 访问运算符，用于键类型转换。 |
| V\& [operator[]](./operator[]/)(const T\&) const | 访问运算符。 |
| [SmartPtr_](../../system/smartptr/smartptr_/) [RemoveAliasing](../../system/smartptr/removealiasing/)() const | 移除指针的别名（由别名构造函数创建），确保其（在共享模式下）管理或（在弱引用模式下）跟踪相同的指向对象。 |
...
| void [SetContainedTemplateWeakPtr](../../system/smartptr/setcontainedtemplateweakptr/)(**uint32_t**) const | 在指向的对象（若存在）上调用 SetTemplateWeakPtr() 方法。 |
...
|  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr_](../../system/smartptr/smartptr_/)\&, [SmartPtrMode](../../system/smartptrmode/)) | 拷贝构造 [SmartPtr](../../system/smartptr/) 对象。两个指针随后指向同一对象。 |
|  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<Q\>\&, [SmartPtrMode](../../system/smartptrmode/)) | 拷贝构造 [SmartPtr](../../system/smartptr/) 对象。两个指针随后指向同一对象。如允许，进行类型转换。 |
...
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const [SmartPtr](../../system/smartptr/)\<[Array](../../system/array/)\<Y\>\>\&, [SmartPtrMode](../../system/smartptrmode/)) | 通过创建不同类型的新数组来转换引用数组的类型。若在 C# 中存在数组类型转换但 C++ 不支持时，这很有用。 |
| explicit  [SmartPtr](../../system/smartptr/smartptr/)(const Y\&) | 初始化为空数组。用于翻译某些 C# 代码结构。 |
