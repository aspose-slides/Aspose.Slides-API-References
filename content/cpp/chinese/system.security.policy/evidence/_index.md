...
description: "用于编译使用 Evidence 类的翻译代码的存根类。此类的对象只能通过 System::MakeObject() 函数分配。永远不要在栈上或使用 new 运算符创建此类型的实例，因为会导致运行时错误和/或断言失败。始终将此类包装为 System::SmartPtr 指针，并使用该指针作为参数传递给函数。"
type: docs
weight: 1
url: /zh/system.security.policy/evidence/
---
## Evidence 类


用于编译使用 [Evidence](./) 类的翻译代码的存根类。此类的对象只能通过 [System::MakeObject()](../../system/makeobject/) 函数分配。永远不要在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装为 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针作为参数传递给函数。
