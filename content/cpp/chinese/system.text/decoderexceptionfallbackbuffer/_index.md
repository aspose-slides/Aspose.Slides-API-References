...
description: "用于抛出异常的解码回退策略的缓冲区。实际上不存储任何内容，而是直接抛出异常。此类的对象只能使用 System::MakeObject() 函数进行分配。切勿在栈上或使用 operator new 创建此类型的实例，否则会导致运行时错误和/或断言错误。应始终将此类包装为 System::SmartPtr 指针，并使用该指针作为参数传递给函数。"
type: docs
weight: 40
url: /zh/system.text/decoderexceptionfallbackbuffer/
---
## DecoderExceptionFallbackBuffer 类

[Buffer](../../system/buffer/) 用于抛出异常的解码回退策略。实际上不存储任何内容，而是直接抛出异常。此类的对象只能使用 [System::MakeObject()](../../system/makeobject/) 函数进行分配。切勿在栈上或使用 operator new 创建此类型的实例，否则会导致运行时错误和/或断言故障。请始终将此类包装为 [System::SmartPtr](../../system/smartptr/) 指针，并使用该指针作为参数传递给函数。
...
| **bool** [Fallback](./fallback/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int) override | 处理解码失败。 |
...
| virtual void [Reset](../decoderfallbackbuffer/reset/)() | 将缓冲区重置为初始状态。 |
