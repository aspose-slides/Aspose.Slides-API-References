...
description: "表示 ASCII 编码。此类的对象只能使用 System::MakeObject() 函数分配。永远不要在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 System::SmartPtr 指针中，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 1
url: /zh/system.text/asciiencoding/
---
## ASCIIEncoding 类

表示 ASCII 编码。此类的对象只能使用 [System::MakeObject()](../../system/makeobject/) 函数分配。永远不要在栈上或使用 operator new 创建此类型的实例，因为这将导致运行时错误和/或断言故障。始终将此类包装在 [System::SmartPtr](../../system/smartptr/) 指针中，并使用该指针将其作为参数传递给函数。
...
| [DecoderPtr](../../system/decoderptr/) [GetDecoder](../icuencoding/getdecoder/)() override | 获取将请求转发给此对象的解码器。 |
| [EncoderPtr](../../system/encoderptr/) [GetEncoder](../icuencoding/getencoder/)() override | 获取将请求转发给此对象的编码器。 |
...
| int [GetMaxByteCount](./getmaxbytecount/)(int) override | 获取已知字符数的字符串可能占用的最大字节数。 |
| int [GetMaxCharCount](./getmaxcharcount/)(int) override | 获取解码指定字节数所需的最大字符数。 |
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [GetPreamble](../icuencoding/getpreamble/)() override | 返回表示编码的字节序列（例如 BOM）。 |
...
| Field | Description |
| --- | --- |
| static constexpr [ASCII_CODE_PAGE](./ascii_code_page/) | 运行时类型信息。 |
