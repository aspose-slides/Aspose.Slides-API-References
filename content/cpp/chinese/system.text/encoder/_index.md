...
description: "封装将字符序列编码为字节序列的功能。该类的对象只能使用 System::MakeObject() 函数分配。切勿在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 System::SmartPtr 指针中，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 131
url: /zh/system.text/encoder/
---
## Encoder 类

封装将字符序列编码为字节序列的功能。此类的对象只能使用 [System::MakeObject()](../../system/makeobject/) 函数分配。切勿在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 [System::SmartPtr](../../system/smartptr/) 指针中，并使用该指针将其作为参数传递给函数。
...
| [EncoderFallbackPtr](../../system/encoderfallbackptr/) [get_Fallback](./get_fallback/)() const | 获取错误处理回退。 |
| const [EncoderFallbackBufferPtr](../../system/encoderfallbackbufferptr/) [get_FallbackBuffer](./get_fallbackbuffer/)() const | 获取回退缓冲区。 |
| virtual int [GetByteCount](./getbytecount/)([ArrayPtr](../../system/arrayptr/)\<char_t\>, int, int, **bool**) | 获取对缓冲区进行编码所需的字节数。 |
| virtual int [GetByteCount](./getbytecount/)(const char_t *, int, **bool**) | 获取对缓冲区进行编码所需的字节数。 |
| virtual int [GetBytes](./getbytes/)([ArrayPtr](../../system/arrayptr/)\<char_t\>, int, int, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, **bool**) | 获取对缓冲区编码后产生的字节。 |
| virtual int [GetBytes](./getbytes/)(const char_t *, int, **uint8_t** *, int, **bool**) | 获取对缓冲区编码后产生的字节。 |
...
| virtual void [Reset](./reset/)() | 清理编码器内部状态。 |
| void [set_Fallback](./set_fallback/)(const [EncoderFallbackPtr](../../system/encoderfallbackptr/)\&) | 设置错误处理回退。 |
