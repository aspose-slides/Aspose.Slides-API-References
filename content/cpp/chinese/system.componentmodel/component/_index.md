...
description: "用于使使用 Component 类的翻译代码能够编译的虚拟类。此类的对象只能通过 System::MakeObject() 函数分配。切勿在栈上或使用 new 操作符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 System::SmartPtr 指针中，并使用该指针将其作为参数传递给函数。"
type: docs
weight: 40
url: /zh/system.componentmodel/component/
---
## 组件类


虚拟类，用于使使用 [Component](./) 类的翻译代码能够编译。此类的对象只能通过 [System::MakeObject()](../../system/makeobject/) 函数分配。切勿在栈上或使用 new 运算符创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 [System::SmartPtr](../../system/smartptr/) 指针中，并使用该指针将其作为参数传递给函数。
...
| void [Dispose](./dispose/)(**bool**) | 一次性模式支持；不执行任何操作。 |
...
| **bool** [get_DesignMode](./get_designmode/)() | 检查组件是否处于设计模式。 |
...
* 类 [MarshalByRefObject](../../system/marshalbyrefobject/)
* 类 [IComponent](../icomponent/)
