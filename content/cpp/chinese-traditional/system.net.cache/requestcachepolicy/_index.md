...
description: "用於 Http、FTP 等的通用請求快取策略。此類別的物件應僅使用 System::MakeObject() 函式進行配置。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。請始終將此類別包裝成 System::SmartPtr 指標，並使用該指標作為參數傳遞給函式。"
type: docs
weight: 14
url: /zh-hant/system.net.cache/requestcachepolicy/
---
## RequestCachePolicy 類別


用於 [Http](../../system.net.http/)、FTP 等的共通請求快取策略。此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式進行配置。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。請始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標作為參數傳遞給函式。
...
| [RequestCacheLevel](../requestcachelevel/) [get_Level](./get_level/)() | 取得指定的 RequestCacheLevel 值。 |
...
|  [RequestCachePolicy](./requestcachepolicy/)() | 建立新的實例。 |
|  [RequestCachePolicy](./requestcachepolicy/)([RequestCacheLevel](../requestcachelevel/)) | 以指定的 RequestCacheLevel 值建立新實例。 |
