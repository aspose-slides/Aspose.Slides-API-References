...
description: "加密轉換器的基底類別。此類別的物件應僅使用 System::MakeObject() 函式進行配置。切勿在堆疊上或使用 new 運算子建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。始終將此類別包裝為 System::SmartPtr 指標，並使用該指標將其作為參數傳遞給函式。"
type: docs
weight: 274
url: /zh-hant/system.security.cryptography/icryptotransform/
---
## ICryptoTransform 類別
...
| virtual int [get_InputBlockSize](./get_inputblocksize/)() | 輸入區塊大小。 |
| virtual int [get_OutputBlockSize](./get_outputblocksize/)() | 輸出區塊大小。 |
...
| virtual int [TransformBlock](./transformblock/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int, [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int) | 處理資料區塊，並將資料複製到輸出陣列。 |
| virtual [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [TransformFinalBlock](./transformfinalblock/)([ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, int, int) | 處理最後一個資料區塊並計算輸出值。 |
