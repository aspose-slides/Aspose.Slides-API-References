...
description: "受管理的 Rijndael 演算法。僅支援使用 None 填充的 ECB 與 CFB 模式，以及使用 None 與 Zeros 填充的 CBC 模式。此類別的物件應僅透過 System::MakeObject() 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 System::SmartPtr 指標，並使用此指標將其作為參數傳遞給函式。"
type: docs
weight: 417
url: /zh-hant/system.security.cryptography/rijndaelmanaged/
---
## RijndaelManaged 類別


受管理的 [Rijndael](../rijndael/) 演算法。僅支援使用 None 填充的 ECB 和 CFB 模式，以及使用 None 與 Zeros 填充的 CBC 模式。此類別的物件應僅透過 [System::MakeObject()](../../system/makeobject/) 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用此指標將其作為參數傳遞給函式。
...
| virtual [SharedPtr](../../system/sharedptr/)\<[ICryptoTransform](../icryptotransform/)\> [CreateDecryptor](./createdecryptor/)() | 使用演算法物件定義的參數建立解密器物件。 |
| virtual [SharedPtr](../../system/sharedptr/)\<[ICryptoTransform](../icryptotransform/)\> [CreateDecryptor](./createdecryptor/)([System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, [System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>) | 使用演算法物件定義的參數建立解密器物件。 |
...
| virtual [SharedPtr](../../system/sharedptr/)\<[ICryptoTransform](../icryptotransform/)\> [CreateEncryptor](./createencryptor/)() | 使用演算法物件定義的參數建立加密器物件。 |
| virtual [SharedPtr](../../system/sharedptr/)\<[ICryptoTransform](../icryptotransform/)\> [CreateEncryptor](./createencryptor/)([System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>, [System::ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>) | 使用演算法物件定義的參數建立加密器物件。 |
...
| virtual int [get_KeySize](../symmetricalgorithm/get_keysize/)() | 取得金鑰大小。 |
...
| virtual void [set_KeySize](../symmetricalgorithm/set_keysize/)(int) | 設定金鑰大小。 |
