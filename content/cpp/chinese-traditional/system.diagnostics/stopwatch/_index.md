...
description: "允許時間測量。此類別的物件應僅透過 System::MakeObject() 函式分配。切勿在堆疊上或使用 new 運算子建立此類型的實例，否則會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 System::SmartPtr 指標，並使用該指標作為參數傳遞給函式。"
type: docs
weight: 79
url: /zh-hant/system.diagnostics/stopwatch/
---
## Stopwatch 類別

允許時間測量。此類別的物件應僅透過 [System::MakeObject()](../../system/makeobject/) 函式分配。切勿在堆疊上或使用 new 運算子建立此類型的實例，否則會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標作為參數傳遞給函式。
...
| [TimeSpan](../../system/timespan/) [get_Elapsed](./get_elapsed/)() const | 取得目前實例所測量的總經過時間。 |
| **int64_t** [get_ElapsedMilliseconds](./get_elapsedmilliseconds/)() const | 取得目前實例所測量的總經過時間（毫秒）。 |
| **int64_t** [get_ElapsedTicks](./get_elapsedticks/)() const | 取得目前實例所測量的總經過時間（計時器刻度）。 |
| **bool** [get_IsRunning](./get_isrunning/)() const | 檢查計時器是否正在執行。 |
...
| void [Reset](./reset/)() | 停止時間測量，將測量區間設為零。 |
| void [Restart](./restart/)() | 將測量區間設為零，然後開始時間測量。 |
...
| void [Start](./start/)() | 開始時間測量。 |
| static [SharedPtr](../../system/sharedptr/)\<[Stopwatch](./)\> [StartNew](./startnew/)() | 建立新的 [Stopwatch](./) 物件並開始測量。 |
| void [Stop](./stop/)() | 停止時間測量。 |
|  [Stopwatch](./stopwatch/)() | RTTI 資訊。 |
...
| virtual  [~Stopwatch](./~stopwatch/)() | 解構函式。 |
