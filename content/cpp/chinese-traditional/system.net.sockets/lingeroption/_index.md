...
description: "指定套接字在呼叫 Close() 或 Close() 方法後是否保持連線。亦指定在持續傳送資料時套接字保持連線的時間。此類別的物件應僅使用 System::MakeObject() 函式分配。切勿在堆疊上或使用 new 運算子建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。請始終將此類別包裝成 System::SmartPtr 指標，並使用該指標作為參數傳遞給函式。"
type: docs
weight: 27
url: /zh-hant/system.net.sockets/lingeroption/
---
## LingerOption 類別


指定套接字在呼叫 Close() 或 Close() 方法後是否保持連線。它還指定在持續傳送資料時套接字保持連線的時間。此類別的物件只能使用 [System::MakeObject()](../../system/makeobject/) 函式分配。切勿在堆疊上或使用 new 運算子建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。請始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標作為參數傳遞給函式。
...
| **bool** [get_Enabled](./get_enabled/)() | 取得一個值，指示套接字是否會延遲關閉以嘗試傳送所有待處理資料。 |
| **int32_t** [get_LingerTime](./get_lingertime/)() | 取得延遲逾時時間（秒）。 |
...
|  [LingerOption](./lingeroption/)(**bool**, **int32_t**) | 建立新實例。 |
...
|  [Object](../../system/object/object/)([Object](../../system/object/) const\&) | 複製建構函式。實際上不會複製任何東西，只是初始化新物件並允許子類別的拷貝建構。 |
...
| void [set_Enabled](./set_enabled/)(**bool**) | 設定指示套接字是否會延遲關閉以嘗試傳送所有待處理資料的值。 |
| void [set_LingerTime](./set_lingertime/)(**int32_t**) | 設定延遲逾時時間（秒）。 |
