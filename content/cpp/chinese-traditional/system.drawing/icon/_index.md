...
description: "表示一個 Windows 圖示。此類別的物件只能使用 System::MakeObject() 函式配置。絕不要在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。應始終將此類別包裝成 System::SmartPtr 指標，並使用此指標作為參數傳遞給函式。"
type: docs
weight: 131
url: /zh-hant/system.drawing/icon/
---
## 圖示類別


表示一個 [Windows](../../system.windows/) 圖示。這個類別的物件只能使用 [System::MakeObject()](../../system/makeobject/) 函式來配置。絕不要在堆疊上或使用 operator new 建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。總是將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用此指標作為參數傳遞給函式。
...
| **int32_t** [get_Height](./get_height/)() const | 返回圖示的高度。 |
| **int32_t** [get_Width](./get_width/)() const | 返回圖示的寬度。 |
...
|  [Icon](./icon/)(const [String](../../system/string/)\&) | 從指定檔案建立一個代表圖示的 [Icon](./) 類別新實例。 |
...
| [SharedPtr](../../system/sharedptr/)\<[Bitmap](../bitmap/)\> [ToBitmap](./tobitmap/)() | 將目前物件轉換為 [Bitmap](../bitmap/) 物件。 |
