...
description: 表示 XML Schema 中的 simpleType 元素，用於簡單內容，依照萬維網聯盟 (W3C) 的規範。此類別定義了一種簡單類型。簡單類型可為僅含文字內容的屬性或元素的值指定資訊與限制。
type: docs
weight: 833
url: /zh-hant/system.xml.schema/xmlschemasimpletype/
---
## XmlSchemaSimpleType 類別


代表 XML [Schema](../) 中的 **simpleType** 元素（簡單內容），其規範由全球 [Web](../../system.web/) 聯盟 (W3C) 指定。此類別定義了一種簡單型別。簡單型別可以為屬性或僅含文字內容的元素的值指定資訊和限制。
...
此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式分配。絕不要在堆疊上或使用 new 運算子建立此型別的實例，因為這會導致執行時錯誤和/或斷言失敗。應始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標作為參數傳遞給函式。
