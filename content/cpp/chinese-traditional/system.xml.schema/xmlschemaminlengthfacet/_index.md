...
description: 代表由全球資訊網聯盟 (W3C) 所定義的 XML Schema 中的 minLength 元素。此類別可用於對 simpleType 元素的資料值的最小長度設置限制。長度必須大於 minLength 元素的值。
type: docs
weight: 638
url: /zh-hant/system.xml.schema/xmlschemaminlengthfacet/
---
## XmlSchemaMinLengthFacet class

代表來自 XML [Schema](../) 的 **minLength** 元素，依據全球資訊網 [Web](../../system.web/) 聯盟（W3C）的規範。此類別可用於對 **simpleType** 元素的資料值之最小長度設定限制。長度必須大於 **minLength** 元素的值。
...
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | 賦值運算子。實際上不會複製任何資料，只是初始化新物件並允許子類別的複製建構。 |
...
|  [XmlSchemaMinLengthFacet](./xmlschemaminlengthfacet/)() | 初始化 [XmlSchemaMinLengthFacet](./) 類別的新執行個體。 |
...
此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式進行配置。切勿在堆疊上或使用 new 運算子建立此類型的實例，否則會導致執行時錯誤和/或斷言失敗。應始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標作為參數傳遞給函式。
