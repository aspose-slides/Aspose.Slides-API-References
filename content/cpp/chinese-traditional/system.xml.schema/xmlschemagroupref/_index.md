...
description: 表示根據萬維網聯盟（W3C）所指定的 XML Schema 中具有 ref 屬性的 group 元素。此類別用於參照在 schema 級別定義的 group 的複雜型別中。
type: docs
weight: 456
url: /zh-hant/system.xml.schema/xmlschemagroupref/
---
## XmlSchemaGroupRef 類別


表示來自 XML [Schema](../)、依照 World Wide [Web](../../system.web/) Consortium (W3C) 所指定的 **group** 元素，其具有 **ref** 屬性。此類別用於在參照於 **schema** 級別所定義的 **group** 的複雜型別中。
...
| [SharedPtr](../../system/sharedptr/)\<[XmlSchemaGroupBase](../xmlschemagroupbase/)\> [get_Particle](./get_particle/)() | 傳回 [XmlSchemaChoice](../xmlschemachoice/)、[XmlSchemaAll](../xmlschemaall/) 或 [XmlSchemaSequence](../xmlschemasequence/) 類別之一，此類別保存 **Particle** 值的編譯後詮釋。 |
| [SharedPtr](../../system/sharedptr/)\<[XmlQualifiedName](../../system.xml/xmlqualifiedname/)\> [get_RefName](./get_refname/)() | 傳回此 schema 中定義之群組的名稱（或由指定命名空間指示的其他 schema 中的群組名稱）。 |
...
| void [set_RefName](./set_refname/)(const [SharedPtr](../../system/sharedptr/)\<[XmlQualifiedName](../../system.xml/xmlqualifiedname/)\>\&) | 設定此 schema 中定義之群組的名稱（或由指定命名空間指示的其他 schema 中的群組名稱）。 |
...
|  [XmlSchemaGroupRef](./xmlschemagroupref/)() | 初始化 [XmlSchemaGroupRef](./) 類別的新執行個體。 |
...
此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式配置。切勿在堆疊上或使用 new 運算子建立此型別的實例，否則會導致執行時錯誤和/或斷言失敗。始終將此類別封裝於 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標作為參數傳遞給函式。
