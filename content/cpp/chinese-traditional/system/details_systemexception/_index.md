...
description: "一個用於表示系統（而非應用程式）例外的基礎類別。切勿手動建立此類別的實例。請改用 SystemException 類別。切勿將 SystemException 類別的實例包裝成 System::SmartPtr。"
type: docs
weight: 703
url: /zh-hant/system/details_systemexception/
---
## Details_SystemException 類別

一個用於表示系統（而非應用程式）例外的基礎類別。切勿手動建立此類別的實例。請改用 SystemException 類別。切勿將 SystemException 類別的實例包裝成 [System::SmartPtr](../smartptr/)。
...
| void [Lock](../object/lock/)() | 實作 C# lock() 陳述式的上鎖。直接呼叫或使用 [LockContext](../lockcontext/) 哨兵物件。 |
...
|  [Object](../object/object/)([Object](../object/) const\&) | 拷貝建構子。實際上不會拷貝任何內容，只是初始化新物件並允許子類別的拷貝建構。 |
