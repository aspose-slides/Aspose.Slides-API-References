...
description: "當傳遞給方法的陣列參數其維度數量不同於預期時，會拋出 RankException。切勿手動建立此類別的實例。請改用 RankException 類別。切勿將 RankException 類別的實例包裝成 System::SmartPtr。"
type: docs
weight: 677
url: /zh-hant/system/details_rankexception/
---
## Details_RankException 類別


當傳遞給方法的陣列參數其維度數量與預期不同時，會拋出 RankException。切勿手動建立此類別的實例。請改用 RankException 類別。切勿將 RankException 類別的實例包裝成 [System::SmartPtr](../smartptr/)。
...
| **int32_t** [get_HResult](../details_exception/get_hresult/)() const | 傳回 32 位元整數值，該值為與目前物件所代表的例外相關的 HRESULT 代碼。 |
...
| [Object](../object/)\& [operator=](../object/operator_equal/)([Object](../object/) const\&) | 指派運算子。實際上不會複製任何內容，只是初始化新物件並允許子類別的拷貝建構。 |
