...
description: "當呼叫未實作且作為存根的方法時，會拋出 NotImplementedException。絕不可手動建立此類別的實例。請改用 NotImplementedException 類別。切勿將 NotImplementedException 類別的實例包裝到 System::SmartPtr 中。"
type: docs
weight: 573
url: /zh-hant/system/details_notimplementedexception/
---
## Details_NotImplementedException 類別


NotImplementedException 會在未實作且作為存根的方法被呼叫時拋出。永遠不要手動建立此類別的實例。請改為使用 NotImplementedException 類別。永遠不要將 NotImplementedException 類別的實例包裝到 [System::SmartPtr](../smartptr/) 中。
...
| **int32_t** [get_HResult](../details_exception/get_hresult/)() const | 傳回 32 位元整數值，該值是與目前物件所代表例外相關的 HRESULT 代碼。 |
...
| static std::enable_if<\![IsSmartPtr](../issmartptr/)\<T\>::value, **bool**\>::type [ReferenceEquals](../object/referenceequals/)(T const\&, std::nullptr_t) | 使用參考方式比較值型別物件與 nullptr。 |
