...
description: 將簡報的所有投影片以指定的格式儲存至檔案。
type: docs
weight: 443
...
以下範例說明如何使用 C++ 將 PowerPoint 轉換為 HTML。 
```cpp
// 實例化一個表示簡報檔案（例如 PPT、PPTX、ODP 等）的簡報物件。
...
```
 以下範例說明如何使用 C++ 將 PowerPoint 轉換為回應式 HTML。 
```cpp
...
```
 以下範例說明如何使用 C++ 將 PowerPoint 轉換為含註解的 HTML。 
```cpp
...
```
 以下範例說明如何使用 C++ 將 PowerPoint 轉換為保留原始字型的 HTML。 
```cpp
...
```
 以下範例說明如何使用 C++ 將 PowerPoint 轉換為 150 DPI 高畫質影像的 HTML。 
```cpp
...
```
 以下範例說明如何使用 C++ 將 PowerPoint 轉換為完整品質影像的 HTML。 
```cpp
auto pres = System::MakeObject<Presentation>(u"InputDoc.pptx");
...
```
 以下範例說明如何使用 C++ 將 [Slide](../../slide/) 轉換為 HTML。 
```cpp
```
