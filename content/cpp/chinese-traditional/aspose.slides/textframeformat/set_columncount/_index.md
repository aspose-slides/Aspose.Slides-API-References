...
description: 設定文字區域中的欄位數量。此值必須為正數。否則，該值將被設定為零。值 0 表示未定義的值。寫入 int32_t。
type: docs
weight: 261
url: /zh-hant/aspose.slides/textframeformat/set_columncount/
---
## TextFrameFormat::set_ColumnCount(int32_t) 方法


設定文字區域中的欄位數量。此值必須為正數。否則，該值將被設定為零。值 0 表示未定義的值。寫入 **int32_t**。
...
以下範例程式碼示範如何在 PowerPoint [Presentation](../../presentation/) 中的文字框新增欄位。 
```cpp
