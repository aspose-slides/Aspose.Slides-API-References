...
| [LightingDirection](../lightingdirection/) [get_Direction](./get_direction/)() override | 光線方向。讀取 [LightingDirection](../lightingdirection/)。 |
| [LightRigPresetType](../lightrigpresettype/) [get_LightType](./get_lighttype/)() override | 表示可套用於形狀的預設光線設定。LightRig 表示相對於 3D 場景以特定方式定位的一組光源。讀取 [LightRigPresetType](../lightrigpresettype/)。 |
...
| [System::ArrayPtr](../../system/arrayptr/)\<**float**\> [GetRotation](./getrotation/)() override | 旋轉透過緯度座標、經度座標以及繞軸的旋轉來定義，緯度與經度座標即為此旋轉的座標。回傳陣列的第一個元素為緯度，第二個為經度，第三個為繞軸旋轉。如未定義旋轉，回傳 null。 |
...
| void [set_Direction](./set_direction/)([LightingDirection](../lightingdirection/)) override | 光線方向。寫入 [LightingDirection](../lightingdirection/)。 |
| void [set_LightType](./set_lighttype/)([LightRigPresetType](../lightrigpresettype/)) override | 表示可套用於形狀的預設光線設定。LightRig 表示相對於 3D 場景以特定方式定位的一組光源。寫入 [LightRigPresetType](../lightrigpresettype/)。 |
| void [SetRotation](./setrotation/)(**float**, **float**, **float**) override | 旋轉透過緯度座標、經度座標以及繞軸的旋轉來定義。若任一座標值為 std::numeric_limits<float>::quiet_NaN()，則全部旋轉皆未定義。 |
