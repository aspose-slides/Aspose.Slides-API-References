...
description: "提供存取資料庫的提供者。此類別的物件應僅使用 System::MakeObject() 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 System::SmartPtr 指標，並使用此指標作為參數傳遞給函式。"
type: docs
weight: 66
url: /zh-hant/system.data.common/dbproviderfactory/
---
## DbProviderFactory 類別


提供存取資料庫的提供者。此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用此指標作為參數傳遞給函式。
...
| 方法 | 說明 |
| --- | --- |
| virtual [SharedPtr](../../system/sharedptr/)\<[DbCommand](../dbcommand/)\> [CreateCommand](./createcommand/)() | RTTI 資訊。 |
| virtual [SharedPtr](../../system/sharedptr/)\<[DbConnection](../dbconnection/)\> [CreateConnection](./createconnection/)() | 建立資料庫連線。 |
