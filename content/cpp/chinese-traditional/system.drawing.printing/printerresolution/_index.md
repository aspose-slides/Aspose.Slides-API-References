...
description: "表示印表機的解析度。此類別的物件應僅使用 System::MakeObject() 函式配置。切勿在堆疊上或使用 operator new 建立此型別的實例，因為會導致執行時錯誤和/或斷言錯誤。請始終將此類別包裝成 System::SmartPtr 指標，並使用此指標作為參數傳遞給函式。"
type: docs
weight: 66
url: /zh-hant/system.drawing.printing/printerresolution/
---
## PrinterResolution 類別


此類別的物件代表印表機的解析度。此類別的物件應該只使用 [System::MakeObject()](../../system/makeobject/) 函式配置。切勿在堆疊上或使用 operator new 建立此型別的實例，因為會導致執行時錯誤和/或斷言錯誤。請始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標作為參數傳遞給函式。
...
| int [get_X](./get_x/)() const | 傳回水平解析度。 |
| int [get_Y](./get_y/)() const | 傳回垂直解析度。 |
...
| void [set_X](./set_x/)(int) | 設定水平解析度。 |
| void [set_Y](./set_y/)(int) | 設定垂直解析度。 |
