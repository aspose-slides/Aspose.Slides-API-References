...
description: "包含表示路徑的圖形資料。此類別的物件應僅使用 System::MakeObject() 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。請始終將此類別包裝成 System::SmartPtr 指標，並使用此指標將其作為參數傳遞給函式。"
type: docs
weight: 131
url: /zh-hant/system.drawing.drawing2d/pathdata/
---
## PathData 類別

包含表示路徑的圖形資料。此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。請始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用此指標將其作為參數傳遞給函式。
...
| [ArrayPtr](../../system/arrayptr/)\<[PointF](../../system.drawing/pointf/)\> [get_Points](./get_points/)() | 回傳包含組成路徑之點的陣列。 |
| [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\> [get_Types](./get_types/)() | 回傳包含指定 **Points** 陣列中相對應點類型之值的陣列。 |
...
|  [PathData](./pathdata/)() | 建構一個空的 [PathData](./) 物件。 |
...
| void [set_Points](./set_points/)(const [ArrayPtr](../../system/arrayptr/)\<[PointF](../../system.drawing/pointf/)\>\&) | 設定包含組成路徑之點的陣列。 |
| void [set_Types](./set_types/)(const [ArrayPtr](../../system/arrayptr/)\<**uint8_t**\>\&) | 設定包含在 **Points** 陣列中相對應點類型之值的陣列。 |
