...
description: "此類別為已安裝與私有字型集合的基底類別。此類別的物件應僅使用 System::MakeObject() 函式分配。切勿在堆疊上或以 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 System::SmartPtr 指標，並使用該指標作為參數傳遞給函式。"
type: docs
weight: 1
url: /zh-hant/system.drawing.text/fontcollection/
---
## FontCollection 類別

一個用於已安裝和私有字型集合的基礎類別。此類別的物件應僅透過 [System::MakeObject()](../../system/makeobject/) 函式分配。絕不要在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標將其作為參數傳遞給函式。
...
| 方法 | 說明 |
| --- | --- |
| void [Dispose](./dispose/)() override | 釋放目前物件取得的作業系統資源。 |
...
| virtual [ArrayPtr](../../system/arrayptr/)\<[SharedPtr](../../system/sharedptr/)\<[FontFamily](../../system.drawing/fontfamily/)\>\> [get_Families](./get_families/)() | 傳回與目前物件所代表的字型集合相關聯的 [FontFamily](../../system.drawing/fontfamily/) 物件陣列。 |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | 将第 n 個模板參數設定為弱指標（而非共享）。允許在容器中將指標切換至弱模式。 |
