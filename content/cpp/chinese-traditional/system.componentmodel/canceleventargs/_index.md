...
description: "可取消事件的參數。此類別的物件應僅使用 System::MakeObject() 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 System::SmartPtr 指標，並使用該指標將其作為參數傳遞給函式。"
type: docs
weight: 27
url: /zh-hant/system.componentmodel/canceleventargs/
---
## CancelEventArgs 類別

可取消事件的參數。此類別的物件應僅透過 [System::MakeObject()](../../system/makeobject/) 函式分配。絕不可在堆疊上或使用 operator new 建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用此指標將其作為參數傳遞給函式。
...
| Method | 說明 |
| --- | --- |
|  [CancelEventArgs](./canceleventargs/)(**bool**) | 建構函式。 |
|  [CancelEventArgs](./canceleventargs/)() | 建構函式；將 Cancel 屬性設為 false。 |
...
| **bool** [get_Cancel](./get_cancel/)() | 取得表示事件是否應取消的值。 |
...
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | 賦值運算子。實際上不會複製任何內容，只是初始化新物件並允許為子類別執行複製建構。 |
...
| void [set_Cancel](./set_cancel/)(**bool**) | 設定指示事件是否應取消的值。 |
...
| 欄位 | 說明 |
| --- | --- |
| static [Empty](../../system/eventargs/empty/) | 靜態成員，代表一個「empty」的 [EventArgs](../../system/eventargs/) 共享指標（null-pointer）。 |
