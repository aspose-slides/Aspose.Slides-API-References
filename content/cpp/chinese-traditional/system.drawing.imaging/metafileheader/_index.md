...
description: "包含與圖形圖元檔相關的屬性。此類別的物件應僅使用 System::MakeObject() 函式分配。絕不應在堆疊上或使用 operator new 建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。應始終將此類別包裝成 System::SmartPtr 指標，並使用該指標將其作為參數傳遞給函式。"
type: docs
weight: 157
url: /zh-hant/system.drawing.imaging/metafileheader/
---
## MetafileHeader 類別


包含與圖形圖元檔相關的屬性。此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式分配。絕不應在堆疊上或使用 operator new 建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。應始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標將其作為參數傳遞給函式。
...
| [Rectangle](../../system.drawing/rectangle/) [get_Bounds](./get_bounds/)() const | 傳回與目前物件相關聯的圖元檔的邊界矩形。 |
| **float** [get_DpiX](./get_dpix/)() const | 未實作。 |
| **float** [get_DpiY](./get_dpiy/)() const | 未實作。 |
...
| void [set_Bounds](./set_bounds/)(const [Rectangle](../../system.drawing/rectangle/)\&) | 設定與目前物件相關聯的圖元檔的邊界矩形。 |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | 將第 n 個模板參數設定為弱指標（而非共享）。允許在容器中將指標切換為弱模式。 |
| int [SharedCount](../../system/object/sharedcount/)() const | 取得共用參考計數器的目前值。 |
| [Object](../../system/object/) * [SharedRefAdded](../../system/object/sharedrefadded/)() | 遞增共用參考計數。不應直接呼叫；請改為使用智慧指標或 ThisProtector。 |
| int [SharedRefRemovedSafe](../../system/object/sharedrefremovedsafe/)() | 遞減並傳回共用參考計數。不應直接呼叫；請改為使用智慧指標或 ThisProtector。 |
