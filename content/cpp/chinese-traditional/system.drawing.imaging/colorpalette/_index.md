...
description: "表示構成調色盤的 32 位 ARGB 顏色集合。此類別的物件應僅使用 System::MakeObject() 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 System::SmartPtr 指標，並使用該指標作為參數傳遞給函式。"
type: docs
weight: 40
url: /zh-hant/system.drawing.imaging/colorpalette/
---
## ColorPalette 類別


表示構成調色盤的 32 位 ARGB 顏色集合。此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標作為參數傳遞給函式。
...
| [System::ArrayPtr](../../system/arrayptr/)\<[Color](../../system.drawing/color/)\> [get_Entries](./get_entries/)() const | 傳回由當前物件表示的 [Color](../../system.drawing/color/) 物件陣列。 |
| int [get_Flags](./get_flags/)() const | 傳回一個值，指定如何解釋顏色陣列中的顏色值。 |
