...
description: "描述資料中的欄位。此類別的物件應僅使用 System::MakeObject() 函式進行分配。絕不可在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 System::SmartPtr 指標，並使用此指標作為參數傳遞給函式。"
type: docs
weight: 1
url: /zh-hant/system.data/datacolumn/
---
## DataColumn 類別


描述資料中的欄位。此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式來分配。絕不可在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用此指標作為參數傳遞給函式。
...
| [System::String](../../system/string/) [get_ColumnName](./get_columnname/)() | RTTI 資訊。 |
