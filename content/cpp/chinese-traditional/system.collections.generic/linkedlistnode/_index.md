...
description: "鏈結串列的節點。實作了對 std::list 之迭代器的封裝，該迭代器被包裹在鏈結串列中。本類別的物件只能使用 System::MakeObject() 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。應始終將本類別包裝成 System::SmartPtr 指標，並使用此指標作為參數傳遞給函式。"
type: docs
weight: 417
url: /zh-hant/system.collections.generic/linkedlistnode/
---
## LinkedListNode 類別

鏈結串列的節點。實作了對 std::list 之迭代器的封裝，該迭代器被包裹在鏈結串列中。本類別的物件只能使用 [System::MakeObject()](../../system/makeobject/) 函式分配。切勿在堆疊上或使用 operator new 建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。應始終將本類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用此指標作為參數傳遞給函式。
...
| [SharedPtr](../../system/sharedptr/)\<[LinkedList](../linkedlist/)\<T\>\> [get_List](./get_list/)() const | 取得所屬串列。 |
| [SharedPtr](../../system/sharedptr/)\<[LinkedListNode](./)\<T\>\> [get_Next](./get_next/)() const | 取得下一個節點。 |
| [SharedPtr](../../system/sharedptr/)\<[LinkedListNode](./)\<T\>\> [get_Previous](./get_previous/)() const | 取得前一個節點。 |
| T [get_Value](./get_value/)() const | 取得儲存的值。 |
...
| [LinkedListNode](./linkedlistnode/)(const T\&) | 建構函式。 |
...
| int [RemovedSharedRefs](../../system/object/removedsharedrefs/)(int) | 將共用參考計數降低指定的值。 |
| void [set_Value](./set_value/)(const T\&) | 設定儲存的值。 |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | 將第 n 個範本參數設為弱指標（而非共用）。允許在容器中將指標切換為弱模式。 |
