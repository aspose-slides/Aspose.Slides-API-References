...
description: "介面提供比較兩個物件相等性的方式。此類別的物件應僅使用 System::MakeObject() 函式配置。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。務必將此類別包裝成 System::SmartPtr 指標，並使用此指標將其作為參數傳遞給函式。"
type: docs
weight: 313
url: /zh-hant/system.collections.generic/iequalitycomparer/
---
## IEqualityComparer 類別

介面提供比較兩個物件相等性的方式。此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式來配置。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。務必將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用此指標將其作為參數傳遞給函式。
...
| Method | Description |
| --- | --- |
| virtual **bool** [Equals](./equals/)(T, T) const | 檢查兩個物件是否相等。 |
...
| virtual int [GetHashCode](./gethashcode/)(T) const | 取得某個物件的雜湊碼。 |
...
|  [RTTI_INFO_TEMPLATE_CLASS](./rtti_info_template_class/)([System::Collections::Generic::IEqualityComparer](./)\<T\>, System::BaseTypesInfo\<[System::Object](../../system/object/)\>) | RTTI 資訊。 |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | 將第 n 個範本參數設為弱指標（而非共用指標）。允許在容器中將指標切換為弱模式。 |
