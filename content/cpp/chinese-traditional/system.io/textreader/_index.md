...
description: "一個基底類別，供表示從不同來源讀取字元序列的閱讀器類別使用。此類別的物件只能透過 System::MakeObject() 函式配置。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤或斷言失敗。始終將此類別包裝成 System::SmartPtr 指標，並使用該指標作為參數傳遞給函式。"
type: docs
weight: 430
url: /zh-hant/system.io/textreader/
---
## TextReader 類別


一個基底類別，供代表從不同來源讀取字元序列的閱讀器類別使用。此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式配置。切勿在堆疊上或使用 operator new 建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標將其作為參數傳遞給函式。
...
| Method | Description |
| --- | --- |
| virtual void [Close](./close/)() | 關閉串流並釋放已取得的資源。 |
...
| virtual int [Peek](./peek/)() | 從串流讀取單一字元而不改變串流的讀取指標。 |
| virtual int [Read](./read/)() | 從串流讀取單一字元。 |
| virtual int [Read](./read/)([ArrayPtr](../../system/arrayptr/)\<char_t\>, int, int) | 從串流讀取指定數量的字元，並寫入指定的字元陣列，從指定位置開始。 |
| virtual int [ReadBlock](./readblock/)([ArrayPtr](../../system/arrayptr/)\<char_t\>, int, int) | 從目前的文字閱讀器讀取最多指定數量的字元，並將資料寫入緩衝區，從指定索引開始。 |
| virtual [System::String](../../system/string/) [ReadLine](./readline/)() | 從串流讀取字元，直至目前行的結尾。 |
| virtual [System::String](../../system/string/) [ReadToEnd](./readtoend/)() | 從串流讀取字元，直至串流結束。 |
