...
description: "提供有關簽署者轉換資料的資訊。此類別的物件只能使用 System::MakeObject() 函式分配。切勿在堆疊上或使用 new 運算子建立此類型的實例，否則會導致執行時錯誤和/或斷言失敗。請始終將此類別包裝成 System::SmartPtr 指標，並使用該指標作為參數傳遞給函式。"
type: docs
weight: 131
url: /zh-hant/system.security.cryptography.xml/transform/
---
## Transform 類別

提供關於簽署者轉換資料的資訊。此類別的物件只能使用 [System::MakeObject()](../../system/makeobject/) 函式來分配。切勿在堆疊上或使用 new 運算子建立此類型的實例，因為這會導致執行時錯誤和/或斷言失敗。始終將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標將其作為參數傳遞給函式。
...
| [Object](../../system/object/)\& [operator=](../../system/object/operator_equal/)([Object](../../system/object/) const\&) | 賦值運算子。實際上並未複製任何內容，只是初始化新物件並允許子類別的拷貝建構。 |
...
| void [Unlock](../../system/object/unlock/)() | 實作 C# lock() 陳述式的解除鎖定功能。直接呼叫或使用 [LockContext](../../system/lockcontext/) 監視物件。 |
