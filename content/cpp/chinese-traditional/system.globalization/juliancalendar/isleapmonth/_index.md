---
title: IsLeapMonth()
second_title: Aspose.Slides for C++ API 參考
description: 檢查該月份是否為閏月。
type: docs
weight: 144
url: /zh-hant/system.globalization/juliancalendar/isleapmonth/
---
## JulianCalendar::IsLeapMonth(int, int, int) const 方法

檢查該月份是否為閏月。

```cpp
bool System::Globalization::JulianCalendar::IsLeapMonth(int year, int month, int era) const override
```

### 參數

| Parameter | Type | Description |
| --- | --- | --- |
| year | int | 年份。 |
| month | int | 月份。 |
| era | int | 紀元。 |

### 返回值

True if specified month is leap, false otherwise.

## JulianCalendar::IsLeapMonth(int, int) const 方法

檢查該月份是否為閏月。

```cpp
virtual bool System::Globalization::Calendar::IsLeapMonth(int year, int month) const
```

### 參數

| Parameter | Type | Description |
| --- | --- | --- |
| year | int | 年份。 |
| month | int | 月份。 |

### 返回值

True if specified month is leap, false otherwise.

## JulianCalendar::IsLeapMonth(int, int, int) const 方法

檢查該月份是否為閏月。

```cpp
virtual bool System::Globalization::Calendar::IsLeapMonth(int year, int month, int era) const
```

### 參數

| Parameter | Type | Description |
| --- | --- | --- |
| year | int | 年份。 |
| month | int | 月份。 |
| era | int | 紀元。 |

### 返回值

True if specified month is leap, false otherwise.

## 另見

* 類別 [JulianCalendar](../)
* 命名空間 [System::Globalization](../../)
* 函式庫 [Aspose.Slides](../../../)