...
description: "虛擬類別，使得能夠將翻譯後的程式碼與 DataGrid 參照連結起來，但不執行它。未包含任何正確實作的成員。此類別的物件應僅使用 System::MakeObject() 函式來分配。切勿在堆疊上或使用 new 運算子建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。必須將此類別包裝成 System::SmartPtr 指標，並使用該指標作為參數傳遞給函式。"
type: docs
weight: 1
url: /zh-hant/system.web.ui.webcontrols/datagrid/
---
## DataGrid 類別


虛擬類別，使得能夠將翻譯後的程式碼與 [DataGrid](./) 參照連結起來，但不執行它。未包含任何正確實作的成員。此類別的物件應僅使用 [System::MakeObject()](../../system/makeobject/) 函式來分配。切勿在堆疊上或使用 new 運算子建立此類型的實例，因為會導致執行時錯誤和/或斷言失敗。必須將此類別包裝成 [System::SmartPtr](../../system/smartptr/) 指標，並使用該指標作為參數傳遞給函式。
