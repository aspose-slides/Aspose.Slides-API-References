...
description: 不可變物件，代表套用於整個形狀（包括填充）的模糊效果。所有色彩頻道，包括 alpha，都受到影響。
type: docs
weight: 469
url: /zh-hant/aspose.slides.effects/iblureffectivedata/
---
## IBlurEffectiveData 類別

不可變物件，代表套用於整個形狀（包括填充）的 [Blur](../blur/) 效果。所有色彩頻道，包括 alpha，都受到影響。
...
| virtual **bool** [get_Grow](./get_grow/)() | 決定因模糊而是否應擴大物件的邊界。True 表示邊界會擴大，false 表示不會。唯讀 **bool**。 |
| virtual **double** [get_Radius](./get_radius/)() | 回傳模糊半徑。唯讀 **double**。 |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | 將 n'th 模板參數設定為弱指標（而非共享）。允許在容器中將指標切換為弱模式。 |
