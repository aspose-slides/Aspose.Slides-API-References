...
description: 不可變的物件，表示二階層（黑/白）效果。亮度低於指定閾值的輸入顏色會被變為黑色。亮度大於或等於指定值的輸入顏色會被設定為白色。此效果不會影響 alpha 效果值。
type: docs
weight: 443
url: /zh-hant/aspose.slides.effects/ibileveleffectivedata/
---
## IBiLevelEffectiveData 類別
...
| virtual **float** [get_Threshold](./get_threshold/)() | 傳回閾值。唯讀 **float**。 |
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | 將 n'th 模板參數設定為弱指標（而非共享）。允許在容器中將指標切換至弱模式。 |
