...
description: 表示一個 Alpha Bi-Level 效果。Alpha（不透明度）值低於閾值的會被更改為 0（完全透明），而大於或等於閾值的 Alpha 值則會被更改為 100%（完全不透明）。
type: docs
weight: 1
...
代表一個 Alpha Bi-Level 效果。Alpha（不透明度）值小於閾值的會被更改為 0（完全透明），而大於或等於閾值的 Alpha 值則會被更改為 100%（完全不透明）。
...
| 方法 | 說明 |
| --- | --- |
| **bool** [Equals](./equals/)([System::SharedPtr](../../system/sharedptr/)\<[System::Object](../../system/object/)\>) override | 判斷指定的 [AlphaBiLevel](./) 是否等於目前的 [AlphaBiLevel](./)。 |
...
| **float** [get_Threshold](./get_threshold/)() override | 傳回效果閾值。讀取 **float**。 |
| Detail::SmartPtrCounter * [GetCounter](../../system/object/getcounter/)() | 取得與物件關聯的參考計數器資料結構。 |
| [System::SharedPtr](../../system/sharedptr/)\<[IAlphaBiLevelEffectiveData](../ialphabileveleffectivedata/)\> [GetEffective](./geteffective/)() override | 取得套用繼承後的有效 Alpha Bi-Level 效果資料。 |
...
| void [set_Threshold](./set_threshold/)(**float**) override | 傳回效果閾值。寫入 **float**。 |
...
* 類別 [IAlphaBiLevel](../ialphabilevel/)
