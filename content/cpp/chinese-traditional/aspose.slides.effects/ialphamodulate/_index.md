...
description: 表示 Alpha Modulate 效果。效果的 alpha（不透明度）值會乘以固定的百分比。效果容器指定一個包含 alpha 值以調變的效果。
type: docs
weight: 339
...
表示 Alpha Modulate 效果。效果的 alpha（不透明度）值會乘以固定的百分比。效果容器指定要調變的包含 alpha 值的效果。
