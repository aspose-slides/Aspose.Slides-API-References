---
title: set_ObjectDistribution()
second_title: Aspose.Slides for C++ API 參考
description: 物件分佈 為 true 時，陣列的內容會以陣列物件的最大寬度進行間距分配。
type: docs
weight: 79
url: /zh-hant/aspose.slides.mathtext/imatharray/set_objectdistribution/
---
## IMathArray::set_ObjectDistribution(bool) 方法

Object Distribution 為 true 時，陣列的內容會以陣列物件的最大寬度進行間距分配。

```cpp
virtual void Aspose::Slides::MathText::IMathArray::set_ObjectDistribution(bool value)=0
```

## 備註

範例：
```cpp
auto mathArray = System::MakeObject<MathArray>(System::MakeObject<MathematicalText>(u"item1"));
mathArray->set_ObjectDistribution(true);
```

## 另見

* 類別 [IMathArray](../)
* 命名空間 [Aspose::Slides::MathText](../../)
* 函式庫 [Aspose.Slides](../../../)