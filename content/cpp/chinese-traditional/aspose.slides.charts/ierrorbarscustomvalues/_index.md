...
description: 指定誤差棒的值。僅在錯誤棒值類型為 Custom 時使用。
type: docs
weight: 1015
url: /zh-hant/aspose.slides.charts/ierrorbarscustomvalues/
---
## IErrorBarsCustomValues 類別

指定誤差棒值。僅在錯誤棒值類型為 Custom 時使用。
...
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IDoubleChartValue](../idoublechartvalue/)\> [get_XMinus](./get_xminus/)() | 指定負向的誤差棒值。若錯誤棒值類型為 Custom 且允許 ErrorBarsXFormat，則可用。否則此屬性返回 null。

 唯讀 [IDoubleChartValue](../idoublechartvalue/)。 |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IDoubleChartValue](../idoublechartvalue/)\> [get_XPlus](./get_xplus/)() | 指定正向的誤差棒值。若錯誤棒值類型為 Custom 且允許 ErrorBarsXFormat，則可用。否則此屬性返回 null。

 唯讀 [IDoubleChartValue](../idoublechartvalue/)。 |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IDoubleChartValue](../idoublechartvalue/)\> [get_YMinus](./get_yminus/)() | 指定負向的誤差棒值。若錯誤棒值類型為 Custom 且允許 ErrorBarsYFormat，則可用。否則此屬性返回 null。

 唯讀 [IDoubleChartValue](../idoublechartvalue/)。 |
| virtual [System::SharedPtr](../../system/sharedptr/)\<[IDoubleChartValue](../idoublechartvalue/)\> [get_YPlus](./get_yplus/)() | 指定正向的誤差棒值。若錯誤棒值類型為 Custom 且允許 ErrorBarsYFormat，則可用。否則此屬性返回 null。
...
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | 將第 n 個模板參數設為弱指標（而非共用）。允許在容器中將指標切換至弱模式。 |
