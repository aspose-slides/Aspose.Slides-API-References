---
title: IsLeapYear()
second_title: Aspose.Slides für C++ API-Referenz
description: Überprüft, ob das Jahr ein Schaltjahr ist.
type: docs
weight: 144
url: /de/system.globalization/hebrewcalendar/isleapyear/
---
## HebrewCalendar::IsLeapYear(int, int) const Methode


Überprüft, ob das Jahr ein Schaltjahr ist.

```cpp
bool System::Globalization::HebrewCalendar::IsLeapYear(int year, int era) const override
```


### Argumente

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| year | int | Jahr. |
| era | int | Ära. |

### Rückgabewert

True, wenn das angegebene Jahr ein Schaltjahr ist, false sonst.

## HebrewCalendar::IsLeapYear(int) const Methode


Überprüft, ob das Jahr ein Schaltjahr ist.

```cpp
virtual bool System::Globalization::Calendar::IsLeapYear(int year) const
```


### Argumente

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| year | int | Jahr. |

### Rückgabewert

True, wenn das angegebene Jahr ein Schaltjahr ist, false sonst.

## HebrewCalendar::IsLeapYear(int, int) const Methode


Überprüft, ob das Jahr ein Schaltjahr ist.

```cpp
virtual bool System::Globalization::Calendar::IsLeapYear(int year, int era) const=0
```


### Argumente

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| year | int | Jahr. |
| era | int | Ära. |

### Rückgabewert

True, wenn das angegebene Jahr ein Schaltjahr ist, false sonst.

## Siehe auch

* Klasse [HebrewCalendar](../)
* Namensraum [System::Globalization](../../)
* Bibliothek [Aspose.Slides](../../../)