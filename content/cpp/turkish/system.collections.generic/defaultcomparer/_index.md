...
description: "Varsayılan karşılaştırıcı sınıfı. Değerleri karşılaştırmak için operator < ve operator == kullanır. Bu sınıfın nesneleri yalnızca System::MakeObject() işlevi kullanılarak ayrılmalıdır. Bu tipin örneğini yığına (stack) veya operator new ile oluşturmayın, çünkü çalışma zamanı hatalarına ve/veya doğrulama hatalarına neden olur. Her zaman bu sınıfı System::SmartPtr göstericisine sarın ve bu göstericiyi fonksiyonlara argüman olarak geçirin."
type: docs
weight: 118
url: /tr/system.collections.generic/defaultcomparer/
---
## DefaultComparer sınıfı
...
| Yöntem | Açıklama |
| --- | --- |
| int [Compare](./compare/)(typename [ThisType::args_type](../icomparer/args_type/), typename [ThisType::args_type](../icomparer/args_type/)) const override | Gerçek veri karşılaştırması. |
