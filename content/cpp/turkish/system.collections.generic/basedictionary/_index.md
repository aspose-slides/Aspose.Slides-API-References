...
description: "Sözlük benzeri çeşitli veri yapıları (örn. Dictionary, SortedDictionary) için ortak kodu uygular. Kapsayıcılar tanımlanırken kalıtım dışında doğrudan kullanılmamalıdır. Bu sınıfın nesneleri yalnızca System::MakeObject() işlevi kullanılarak tahsis edilmelidir. Bu tipin bir örneğini yığına (stack) veya new operatörüyle asla oluşturmayın, çünkü çalışma zamanı hatalarına ve/veya doğrulama hatalarına neden olur. Bu sınıfı her zaman System::SmartPtr işaretçisine sarın ve bu işaretçiyi fonksiyonlara argüman olarak geçirmek için kullanın."
type: docs
weight: 53
url: /tr/system.collections.generic/basedictionary/
---
## BaseDictionary sınıfı

Çeşitli sözlük benzeri veri yapıları (örnek olarak [Dictionary](../dictionary/), [SortedDictionary](../sorteddictionary/)) için ortak kodu uygular. Doğrudan kullanılmamalıdır, yalnızca kapsayıcıları tanımlarken kalıtım için kullanılabilir. Bu sınıfın nesneleri yalnızca [System::MakeObject()](../../system/makeobject/) işlevi kullanılarak tahsis edilmelidir. Bu tür bir örnek asla yığına (stack) veya new operatörüyle yaratılmamalıdır, çünkü çalışma zamanı hatalarına ve/veya doğrulama hatalarına yol açar. Bu sınıfı her zaman [System::SmartPtr](../../system/smartptr/) işaretçisine sarın ve bu işaretçiyi fonksiyonlara argüman olarak geçirin.
...
| Yöntem | Açıklama |
| --- | --- |
| void [_add_range](./_add_range/)(std::initializer_list\<typename Map::value_type\>) | C++'a özgü. |
| void [Add](./add/)(const key_t\&, const mapped_t\&) override | Sözlüğe bir anahtar-değer çifti ekler. |
| [BaseDictionary](./basedictionary/)() | Boş bir veri yapısı oluşturur. |
| [BaseDictionary](./basedictionary/)(int, const Args\&...) | Temel harita yapıcısına argümanları iletmek için yönlendirme yapıcı. |
| [BaseDictionary](./basedictionary/)([BaseType](./basetype/) *, const Args\&...) | Kopyalama yapıcı. |
| [BaseDictionary](./basedictionary/)([BaseType](./basetype/) *) | Kopyalama yapıcı. |
| [const_iterator](./const_iterator/) [begin](./begin/)() const | Konteynerin anahtar-değer öğesi için KVPair sarmalayıcısına bir yineleyici döndürür. C# tarzında uygulanır - yineleyici, get_Key() ve get_Value() arabirimine sahip KVPair nesnesini döndürmelidir. Konteyner boşsa, döndürülen yineleyici [end()](../ienumerable/end/) ile eşit olacaktır. |
| [iterator](../ienumerable/iterator/) [begin](../ienumerable/begin/)() | Koleksiyonun (varsa) ilk öğesine işaret eden bir yineleyici alır. Bu yineleyici, [GetEnumerator()](../ienumerable/getenumerator/) T'nin bir kopya nesnesi döndürdüğü için referans alınan nesneyi değiştirmek için kullanılamaz. |
| stl_const_iterator [cbegin](./cbegin/)() const | Konteynerin ilk öğesine bir yineleyici döndürür. STL tarzında uygulanır. Konteyner boşsa, döndürülen yineleyici [end()](../ienumerable/end/) ile eşit olur. |
| stl_const_iterator [cend](./cend/)() const | Konteynerin son öğesinin sonrasındaki öğeye bir yineleyici döndürür. STL tarzında uygulanır. Bu öğe bir yer tutucu görevi görür; ona erişmeye çalışmak tanımsız davranışa yol açar. |
| void [Clear](./clear/)() override | Tüm öğeleri siler. |
| **bool** [ContainsKey](./containskey/)(const key_t\&) const override | Anahtarın sözlükte bulunup bulunmadığını kontrol eder. |
| **bool** [ContainsValue](./containsvalue/)(const mapped_t\&) | Değerin sözlükte bulunup bulunmadığını kontrol eder. Değerleri karşılaştırmak için == operatörünü kullanır. |
...
| Map\& [data](./data/)() | Temel veri depolama erişimcisi. |
| const Map\& [data](./data/)() const | Temel veri depolama erişimcisi. |
| [const_iterator](./const_iterator/) [end](./end/)() const | Konteynerin son öğesinin sonrasındaki anahtar-değer öğesi için KVPair sarmalayıcısına bir yineleyici döndürür. C# tarzında uygulanır - yineleyici, get_Key() ve get_Value() arabirimine sahip KVPair nesnesini döndürmelidir. Bu öğe bir yer tutucu görevi görür; ona erişmeye çalışmak tanımsız davranışa yol açar. |
| [iterator](../ienumerable/iterator/) [end](../ienumerable/end/)() | Koleksiyonun (varsa) son öğesinin hemen sonrasına işaret eden bir yineleyici alır. Bu yineleyici, [GetEnumerator()](../ienumerable/getenumerator/) T'nin bir kopya nesnesi döndürdüğü için referans alınan nesneyi değiştirmek için kullanılamaz. |
...
| **int32_t** [get_Count](./get_count/)() const override | Öğe sayısını alır. |
...
| virtual [SharedPtr](../../system/sharedptr/)\<[IEnumerator](../ienumerator/)\<[KeyValuePair](../keyvaluepair/)\<key_t, mapped_t\>\>\> [GetEnumerator](./getenumerator/)() | Enumerator örneği oluşturur, alt sınıf tarafından uygulanmalıdır. |
...
| mapped_t [GetValueOrDefault](./getvalueordefault/)(const key_t\&) const override | Bulunursa değeri döndürür; aksi takdirde **Value()** döndürür. |
| mapped_t [GetValueOrDefault](./getvalueordefault/)(const key_t\&, const mapped_t\&) const override | Bulunursa değeri döndürür; aksi takdirde **defaultValue** döndürür. |
| mapped_t [GetValueOrNull](./getvalueornull/)(const key_t\&) const override | Bulunursa değeri döndürür; aksi takdirde **null** döndürür. Yalnızca referans tipleri için anlamlıdır. |
| [ICollection](../icollection/icollection/)() | Varsayılan yapıcı. |
| [ICollection](../icollection/icollection/)(const [ICollection](../icollection/)\&) | Kopyalama yapıcı. |
| [ICollection](../icollection/icollection/)([ICollection](../icollection/)\&&) | Taşıma yapıcı. |
| mapped_t [idx_get](./idx_get/)(const key_t\&) const override | Anahtar tabanlı alıcı işlevi. |
| void [idx_set](./idx_set/)(const key_t\&, mapped_t) override | Anahtar tabanlı ayarlayıcı işlev. Öğeyi değiştirir veya oluşturur. |
...
| T [LINQ_ElementAt](../ienumerable/linq_elementat/)(int) | Dizide belirtilen indeksteki öğeyi döndürür. |
| T [LINQ_ElementAtOrDefault](../ienumerable/linq_elementatordefault/)(int) | Dizide belirtilen indeksteki öğeyi döndürür. |
| T [LINQ_First](../ienumerable/linq_first/)() | Dizinin ilk öğesini döndürür. |
...
| T [LINQ_FirstOrDefault](../ienumerable/linq_firstordefault/)() | Dizinin ilk öğesini döndürür; dizi boşsa varsayılan bir değer döndürür. |
| T [LINQ_FirstOrDefault](../ienumerable/linq_firstordefault/)(std::function\<**bool**(T)>) | Bir koşulu sağlayan dizinin ilk öğesini, eğer böyle bir öğe bulunamazsa varsayılan bir değer döndürür. |
...
| T [LINQ_Last](../ienumerable/linq_last/)() | Dizinin son öğesini döndürür. |
| T [LINQ_LastOrDefault](../ienumerable/linq_lastordefault/)() | Dizinin son öğesini döndürür; dizi boşsa varsayılan bir değer döndürür. |
| ResultType [LINQ_Max](../ienumerable/linq_max/)(const [Func](../../system/func/)\<T, ResultType\>\&) | Genel bir dizideki her öğeye dönüşüm işlevi uygular ve elde edilen maksimum değeri döndürür. |
| ResultType [LINQ_Max](../ienumerable/linq_max/)(const [Func](../../system/func/)\<Source, ResultType\>\&) |  |
| ResultType [LINQ_Min](../ienumerable/linq_min/)(const [Func](../../system/func/)\<T, ResultType\>\&) | Genel bir dizideki her öğeye dönüşüm işlevi uygular ve elde edilen minimum değeri döndürür. |
...
| [SharedPtr](../../system/sharedptr/)\<[IEnumerable](../ienumerable/)\<ResultType\>\> [LINQ_Select](../ienumerable/linq_select/)(const [Func](../../system/func/)\<T, **int32_t**, ResultType\>\&) | Bir dizinin her öğesini, öğenin dizinini dahil ederek yeni bir biçime dönüştürür. |
...
| [Object](../../system/object/object/)([Object](../../system/object/) const\&) | Kopyalama yapıcı. Aslında hiçbir şeyi kopyalamaz, sadece yeni nesneyi başlatır ve alt sınıfların kopya oluşturmasını sağlar. |
...
| virtual mapped_t\& [operator[]](./operator[]/)(const key_t\&) | Erişim işlevi. |
...
| **bool** [ReferenceEquals](../../system/object/referenceequals/)([String](../../system/string/) const\&, std::nullptr_t) | [Object::ReferenceEquals](../../system/object/referenceequals/)'nin string ve nullptr durumu için özelleştirilmiş hâli. |
...
| **bool** [Remove](./remove/)(const key_t\&) override | Sözlükten belirli anahtarı kaldırır. |
| int [RemovedSharedRefs](../../system/object/removedsharedrefs/)(int) | Paylaşılan referans sayacını belirtilen değer kadar azaltır. |
| void [SetTemplateWeakPtr](./settemplateweakptr/)(unsigned int) override |  |
| virtual void [SetTemplateWeakPtr](../../system/object/settemplateweakptr/)(**uint32_t**) | n'inci şablon argümanını zayıf işaretçi (shared değil) olarak ayarlar. Kapsayıcılardaki işaretçileri zayıf moda geçişine izin verir. |
...
| **bool** [TryGetValue](./trygetvalue/)(const key_t\&, mapped_t\&) const override | Anahtarlı değeri arar ve bulunursa getirir. |
| static const [TypeInfo](../../system/typeinfo/)\& [Type](../../system/object/type/)() | C# typeof([System.Object](../../system/object/)) yapısını uygular. |
| void [Unlock](../../system/object/unlock/)() | C# lock() ifadesinin kilit kaldırmasını uygular. Doğrudan çağırın veya [LockContext](../../system/lockcontext/) gözlem nesnesini kullanın. |
| System::Details::VirtualizedIteratorBase\<[KVPair](./kvpair/)\> * [virtualizeBeginConstIterator](./virtualizebeginconstiterator/)() const override | Mevcut konteyner için begin const yineleyicisinin uygulamasını alır. |
| System::Details::VirtualizedIteratorBase\<[KVPair](./kvpair/)\> * [virtualizeBeginIterator](./virtualizebeginiterator/)() override | Mevcut konteyner için begin yineleyicisinin uygulamasını alır. |
| System::Details::VirtualizedIteratorBase\<[KVPair](./kvpair/)\> * [virtualizeEndConstIterator](./virtualizeendconstiterator/)() const override | Mevcut konteyner için end const yineleyicisinin uygulamasını alır. |
| System::Details::VirtualizedIteratorBase\<[KVPair](./kvpair/)\> * [virtualizeEndIterator](./virtualizeenditerator/)() override | Mevcut konteyner için end yineleyicisinin uygulamasını alır. |
...
| [KeyCollection](./keycollection/) | Temel depolama tipiyle doğru ayırıcıyı kullandığınızdan emin olun. |
| [ValueCollection](./valuecollection/) | Değer koleksiyonu. |
