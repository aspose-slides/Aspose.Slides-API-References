...
description: "Bir görüntü kodlayıcı parametre kümesiyle ilişkili bir GUID'i temsil eder. Bu sınıfın nesneleri yalnızca System::MakeObject() işlevi kullanılarak ayrılmalıdır. Bu tipin örneğini yığıt üzerinde ya da new operatörüyle oluşturmayın, çünkü çalışma zamanı hatalarına ve/veya doğrulama hatalarına yol açar. Bu sınıfı her zaman System::SmartPtr işaretçisine sarın ve bu işaretçiyi işlevlere argüman olarak geçirin."
type: docs
weight: 53
url: /tr/system.drawing.imaging/encoder/
---
## Encoder sınıfı

Görsel kodlayıcı parametrelerinin bir kümesiyle ilişkili bir GUID'i temsil eder. Bu sınıfın nesneleri yalnızca [System::MakeObject()](../../system/makeobject/) işlevi kullanılarak ayrılmalıdır. Bu türün örneğini yığıt üzerinde ya da yeni operatörüyle oluşturmayın, çünkü bu çalışma zamanı hatalarına ve/veya doğrulama hatalarına yol açar. Bu sınıfı her zaman [System::SmartPtr](../../system/smartptr/) işaretçisine sarın ve bu işaretçiyi işlevlere argüman olarak geçirin.
...
| Yöntem | Açıklama |
| --- | --- |
|  [Encoder](./encoder/)(const [Guid](../../system/guid/)\&) | [Encoder](./) sınıfının yeni bir örneğini oluşturur. |
...
| [Guid](../../system/guid/) [get_Guid](./get_guid/)() const | Geçerli nesnenin temsil ettiği bir dizi görüntü kodlayıcı parametresini belirten GUID'i döndürür. |
...
| Alan | Açıklama |
| --- | --- |
| static [ChrominanceTable](./chrominancetable/) | [Encoder](./) sınıfının krominans tablosu parametre kategorisini temsil eden bir örneği. |
| static [ColorDepth](./colordepth/) | [Encoder](./) sınıfının renk derinliği parametre kategorisini temsil eden bir örneği. |
| static [Compression](./compression/) | [Encoder](./) sınıfının sıkıştırma parametre kategorisini temsil eden bir örneği. |
| static [LuminanceTable](./luminancetable/) | [Encoder](./) sınıfının ışıklık tablosu parametre kategorisini temsil eden bir örneği. |
| static [Quality](./quality/) | [Encoder](./) sınıfının kalite parametre kategorisini temsil eden bir örneği. |
| static [RenderMethod](./rendermethod/) | [Encoder](./) sınıfının render yöntemi parametre kategorisini temsil eden bir örneği. |
| static [SaveFlag](./saveflag/) | [Encoder](./) sınıfının kaydetme bayrağı parametre kategorisini temsil eden bir örneği. |
| static [ScanMethod](./scanmethod/) | [Encoder](./) sınıfının tarama yöntemi parametre kategorisini temsil eden bir örneği. |
| static [Transformation](./transformation/) | [Encoder](./) sınıfının dönüşüm parametre kategorisini temsil eden bir örneği. |
| static [Version](./version/) | [Encoder](./) sınıfının sürüm parametre kategorisini temsil eden bir örneği. |
## Ayrıca Bakınız
