---
title: create_shape_elements method
second_title: Referencia de API de Aspose.Slides para Python vía .NET
description: 
type: docs
url: /es/aspose.slides.smartart/smartartshape/create_shape_elements/
weight: 20
---
## create_shape_elements(self) {#}
Crea y devuelve una matriz de los elementos de shape.

### Devuelve

Matriz de [`ShapeElement`](/slides/python-net/es/aspose.slides/shapeelement)



```python
def create_shape_elements(self):
    ...
```




### Ver también
* clase [`ShapeElement`](/slides/python-net/es/aspose.slides/shapeelement)
* clase [`SmartArtShape`](/slides/python-net/es/aspose.slides.smartart/smartartshape)
* módulo [`aspose.slides.smartart`](/slides/python-net/es/aspose.slides.smartart)
* biblioteca [`Aspose.Slides`](/slides/python-net)