---
title: back_wall property
second_title: Aspose.Slides para Python vía .NET Referencia de API
description: 
type: docs
url: /es/aspose.slides.charts/chart/back_wall/
weight: 200
---
## back_wall propiedad
Devuelve un objeto que permite cambiar el formato de la pared trasera de un gráfico 3D.
Solo lectura [`IChartWall`](/slides/python-net/es/aspose.slides.charts/ichartwall).

### Definición:
```python
@property
def back_wall(self):
    ...
```

### Ver también
* clase [`Chart`](/slides/python-net/es/aspose.slides.charts/chart)
* clase [`IChartWall`](/slides/python-net/es/aspose.slides.charts/ichartwall)
* módulo [`aspose.slides.charts`](/slides/python-net/es/aspose.slides.charts)
* biblioteca [`Aspose.Slides`](/slides/python-net)