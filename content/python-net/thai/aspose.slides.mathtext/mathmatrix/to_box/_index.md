---
title: to_box method
second_title: Aspose.Slides สำหรับ Python ผ่าน .NET เอกสารอ้างอิง API
description: 
type: docs
url: /th/aspose.slides.mathtext/mathmatrix/to_box/
weight: 300
---
## to_box(self) {#}
Places this element in a non-visual box (logical grouping) 
            which is used to group components of an equation or other instance of mathematical text.
            A boxed object can (for example) serve as an operator emulator with or without an alignment point, 
            serve as a line break point, or be grouped such such as not to allow line breaks within.

### Returns

Logical box with this element placed inside



```python
def to_box(self):
    ...
```




### See Also
* class [`IMathBox`](/slides/python-net/th/aspose.slides.mathtext/imathbox)
* class [`MathMatrix`](/slides/python-net/th/aspose.slides.mathtext/mathmatrix)
* module [`aspose.slides.mathtext`](/slides/python-net/th/aspose.slides.mathtext)
* library [`Aspose.Slides`](/slides/python-net)