---
title: radical method
second_title: Aspose.Slides สำหรับ Python ผ่าน .NET API Reference
description: 
type: docs
url: /th/aspose.slides.mathtext/mathradical/radical/
weight: 130
---
## radical(self, degree) {#imathelement}
ระบุรากทางคณิตศาสตร์ของดีกรีที่กำหนดจากอาร์กิวเมนต์ที่ระบุ.

### คืนค่า

อินสแตนซ์ใหม่ของประเภท [`IMathRadical`](/slides/python-net/th/aspose.slides.mathtext/imathradical)



```python
def radical(self, degree):
    ...
```


| พารามิเตอร์ | ประเภท | คำอธิบาย |
| :- | :- | :- |
| degree | [`IMathElement`](/slides/python-net/th/aspose.slides.mathtext/imathelement) | อาร์กิวเมนต์ของ Radical |


## radical(self, degree) {#str}
ระบุรากทางคณิตศาสตร์ของดีกรีที่กำหนดจากอาร์กิวเมนต์ที่ระบุ.

### คืนค่า

อินสแตนซ์ใหม่ของประเภท [`IMathRadical`](/slides/python-net/th/aspose.slides.mathtext/imathradical)



```python
def radical(self, degree):
    ...
```


| พารามิเตอร์ | ประเภท | คำอธิบาย |
| :- | :- | :- |
| degree | **str** | อาร์กิวเมนต์ของ Radical |



### ดูเพิ่มเติม
* คลาส [`IMathElement`](/slides/python-net/th/aspose.slides.mathtext/imathelement)
* คลาส [`IMathRadical`](/slides/python-net/th/aspose.slides.mathtext/imathradical)
* คลาส [`MathRadical`](/slides/python-net/th/aspose.slides.mathtext/mathradical)
* โมดูล [`aspose.slides.mathtext`](/slides/python-net/th/aspose.slides.mathtext)
* ไลบรารี [`Aspose.Slides`](/slides/python-net)