---
title: to_border_box method
second_title: مرجع API Aspose.Slides برای Python از طریق .NET
description: 
type: docs
url: /fa/aspose.slides.mathtext/mathematicaltext/to_border_box/
weight: 200
---
## to_border_box(self) {#}
این عنصر را در یک border-box قرار می‌دهد

### بازمی‌گرداند

border-boxی که این عنصر درون آن قرار گرفته است



```python
def to_border_box(self):
    ...
```



## to_border_box(self, hide_top, hide_bottom, hide_left, hide_right, strikethrough_horizontal, strikethrough_vertical, strikethrough_bottom_left_to_top_right, strikethrough_top_left_to_bottom_right) {#bool-bool-bool-bool-bool-bool-bool-bool}
این عنصر را در یک border-box قرار می‌دهد

### بازمی‌گرداند

border-boxی که این عنصر درون آن قرار گرفته است



```python
def to_border_box(self, hide_top, hide_bottom, hide_left, hide_right, strikethrough_horizontal, strikethrough_vertical, strikethrough_bottom_left_to_top_right, strikethrough_top_left_to_bottom_right):
    ...
```


| پارامتر | نوع | توضیح |
| :- | :- | :- |
| hide_top | **bool** | پنهان‌کردن لبه بالایی |
| hide_bottom | **bool** | پنهان‌کردن لبه پایین |
| hide_left | **bool** | پنهان‌کردن لبه چپ |
| hide_right | **bool** | پنهان‌کردن لبه راست |
| strikethrough_horizontal | **bool** | خط‌خورده افقی جعبه‌مرزبندی |
| strikethrough_vertical | **bool** | خط‌خورده عمودی جعبه‌مرzbندی |
| strikethrough_bottom_left_to_top_right | **bool** | خط‌خورده جعبه‌مرزبندی از پایین-چپ به بالا-راست |
| strikethrough_top_left_to_bottom_right | **bool** | خط‌خورده جعبه‌مرزبندی از بالای-چپ به پایین-راست |



### موارد مرتبط
* کلاس [`IMathBorderBox`](/slides/python-net/fa/aspose.slides.mathtext/imathborderbox)
* کلاس [`MathematicalText`](/slides/python-net/fa/aspose.slides.mathtext/mathematicaltext)
* ماژول [`aspose.slides.mathtext`](/slides/python-net/fa/aspose.slides.mathtext)
* کتابخانه [`Aspose.Slides`](/slides/python-net)