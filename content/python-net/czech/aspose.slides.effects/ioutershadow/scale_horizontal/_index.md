---
title: scale_horizontal property
second_title: Aspose.Slides pro Python pomocí .NET API Reference
description: 
type: docs
url: /cs/aspose.slides.effects/ioutershadow/scale_horizontal/
weight: 80
---
## scale_horizontal vlastnost
Horizontal scaling factor, in percent of the original size.
            Negativní škálování způsobí otočení.
            Výchozí hodnota – 100 %.
            Čtení/zápis **float**.

### Definice:
```python
@property
def scale_horizontal(self):
    ...

@scale_horizontal.setter
def scale_horizontal(self, value):
    ...
```


### Viz také
* třída [`IOuterShadow`](/slides/python-net/cs/aspose.slides.effects/ioutershadow)
* modul [`aspose.slides.effects`](/slides/python-net/cs/aspose.slides.effects)
* knihovna [`Aspose.Slides`](/slides/python-net)