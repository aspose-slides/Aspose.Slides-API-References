---
title: function method
second_title: Aspose.Slides for Python via .NET API リファレンス
description: 
type: docs
url: /ja/aspose.slides.mathtext/imathfraction/function/
weight: 50
---
## 関数(self, function_argument) {#imathelement}



```python
def function(self, function_argument):
    ...
```


| パラメータ | 型 | 説明 |
| :- | :- | :- |
| function_argument | [`IMathElement`](/slides/python-net/ja/aspose.slides.mathtext/imathelement) |  |


## 関数(self, function_argument) {#str}



```python
def function(self, function_argument):
    ...
```


| パラメータ | 型 | 説明 |
| :- | :- | :- |
| function_argument | **str** |  |



### 参照
* クラス [`IMathElement`](/slides/python-net/ja/aspose.slides.mathtext/imathelement)
* クラス [`IMathFraction`](/slides/python-net/ja/aspose.slides.mathtext/imathfraction)
* クラス [`IMathFunction`](/slides/python-net/ja/aspose.slides.mathtext/imathfunction)
* モジュール [`aspose.slides.mathtext`](/slides/python-net/ja/aspose.slides.mathtext)
* ライブラリ [`Aspose.Slides`](/slides/python-net)