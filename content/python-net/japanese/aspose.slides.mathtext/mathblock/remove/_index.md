...
### 戻り値
`item` がコレクションから正常に削除された場合は true、そうでない場合は false。元のコレクションに `item` が見つからない場合も false を返します。
...
| パラメータ | 型 | 説明 |
| :- | :- | :- |
| item | [`IMathElement`](/slides/python-net/ja/aspose.slides.mathtext/imathelement) | コレクションから削除するオブジェクト。 |
