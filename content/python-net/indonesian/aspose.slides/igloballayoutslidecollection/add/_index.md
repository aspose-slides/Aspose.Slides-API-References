...
| layout_name | **str** | Nama untuk tata letak baru. Jika nama yang diberikan sudah digunakan, ArgumentException akan dilempar.<br/><br/>            Jika parameter None diberikan, maka nama dihasilkan secara otomatis sesuai dengan tipe tata letak yang diberikan<br/><br/>            (misalnya "Title Slide" atau "1_Title Slide", "2_..", dll.). |

### Catatan

1) Tata letak yang ditambahkan untuk nilai SlideLayoutType.Custom dari `layout_type` tidak berisi placeholder maupun shape.  
2) Analogi metode ini adalah metode **Aspose.Slides.IMasterLayoutSlideCollection.Add(Aspose.Slides.SlideLayoutType,Syste** yang diakses melalui properti [`IMasterSlide.layout_slides`](/slides/python-net/id/aspose.slides/imasterslide/layout_slides).
...
| **RuntimeError(Proxy error(ArgumentException))** | dilempar jika `master` termasuk dalam presentasi lain. |
| **RuntimeError(Proxy error(ArgumentException))** | dilempar jika nilai nama tata letak `layout_name` sudah digunakan dalam <br/>            koleksi tata letak dari `master`. |
