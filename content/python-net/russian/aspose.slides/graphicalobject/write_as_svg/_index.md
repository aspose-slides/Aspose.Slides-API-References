---
title: write_as_svg method
second_title: Aspose.Slides для Python через .NET справочник API
description: 
type: docs
url: /ru/aspose.slides/graphicalobject/write_as_svg/
weight: 60
---
## write_as_svg(self, stream) {#iorawiobase}
Сохраняет содержимое Shape в файле SVG.

```python
def write_as_svg(self, stream):
    ...
```

| Параметр | Тип | Описание |
| :- | :- | :- |
| stream | **io.RawIOBase** | Целевой поток |

## write_as_svg(self, stream, svg_options) {#iorawiobase-asposeslidesexportisvgoptions}
Сохраняет содержимое Shape в файле SVG.

```python
def write_as_svg(self, stream, svg_options):
    ...
```

| Параметр | Тип | Описание |
| :- | :- | :- |
| stream | **io.RawIOBase** | Целевой поток |
| svg_options | [`ISVGOptions`](/slides/python-net/ru/aspose.slides.export/isvgoptions) | Параметры генерации SVG |

### См. также
* класс [`GraphicalObject`](/slides/python-net/ru/aspose.slides/graphicalobject)
* класс [`ISVGOptions`](/slides/python-net/ru/aspose.slides.export/isvgoptions)
* модуль [`aspose.slides`](/slides/python-net/ru/aspose.slides)
* библиотека [`Aspose.Slides`](/slides/python-net)