---
title: join method
second_title: Aspose.Slides для Python через .NET Справочник API
description: 
type: docs
url: /ru/aspose.slides.mathtext/mathgroupingcharacter/join/
weight: 100
---
## join(self, math_element) {#imathelement}
Объединяет математический элемент и создает математический блок

### Возвращаемое значение
Новый IMathBlock, содержащий этот экземпляр и указанный аргумент



```python
def join(self, math_element):
    ...
```


| Параметр | Тип | Описание |
| :- | :- | :- |
| math_element | [`IMathElement`](/slides/python-net/ru/aspose.slides.mathtext/imathelement) | Элемент для объединения |


## join(self, math_text) {#str}
Объединяет математический текст и создает математический блок

### Возвращаемое значение
Новый IMathBlock, содержащий этот экземпляр и указанный аргумент



```python
def join(self, math_text):
    ...
```


| Параметр | Тип | Описание |
| :- | :- | :- |
| math_text | **str** | Математический текст для объединения |



### См. также
* класс [`IMathBlock`](/slides/python-net/ru/aspose.slides.mathtext/imathblock)
* класс [`IMathElement`](/slides/python-net/ru/aspose.slides.mathtext/imathelement)
* класс [`MathGroupingCharacter`](/slides/python-net/ru/aspose.slides.mathtext/mathgroupingcharacter)
* модуль [`aspose.slides.mathtext`](/slides/python-net/ru/aspose.slides.mathtext)
* библиотека [`Aspose.Slides`](/slides/python-net)