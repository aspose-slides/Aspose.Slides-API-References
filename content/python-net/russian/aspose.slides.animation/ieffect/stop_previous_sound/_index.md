---
title: stop_previous_sound property
second_title: Aspose.Slides для Python через .NET справочник API
description: 
type: docs
url: /ru/aspose.slides.animation/ieffect/stop_previous_sound/
weight: 90
---
## stop_previous_sound свойство
Этот атрибут определяет, останавливает ли эффект анимации предыдущий звук.
            Чтение/запись **bool**.

### Определение:
```python
@property
def stop_previous_sound(self):
    ...

@stop_previous_sound.setter
def stop_previous_sound(self, value):
    ...
```


### См. также
* класс [`IEffect`](/slides/python-net/ru/aspose.slides.animation/ieffect)
* модуль [`aspose.slides.animation`](/slides/python-net/ru/aspose.slides.animation)
* библиотека [`Aspose.Slides`](/slides/python-net)