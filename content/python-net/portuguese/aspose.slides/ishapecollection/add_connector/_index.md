...
description: 
type: docs
url: /pt/aspose.slides/ishapecollection/add_connector/
weight: 70
---
## add_connector(self, shape_type, x, y, width, height) {#shapetype-float-float-float-float}
Cria uma nova forma de conector com a estilização de modelo padrão e a adiciona ao final da
            coleção de formas.
...
Cria uma nova forma de conector e a adiciona ao final da coleção de formas,
            aplicando opcionalmente a estilização de modelo padrão.
...
| width | **float** | A largura da moldura do conector, em pontos. |
| height | **float** | A altura da moldura do conector, em pontos. |
| create_from_template | **bool** | True para aplicar a estilização de modelo padrão (nome não vazio, estilo simples); <br/><br/> false para criar o conector com valores de propriedade padrão. |
