---
title: insert_section_zoom_frame method
second_title: Aspose.Slides for Python 透過 .NET API 參考
description: 
type: docs
url: /zh-hant/aspose.slides/ishapecollection/insert_section_zoom_frame/
weight: 300
---
## insert_section_zoom_frame(self, index, x, y, width, height, section) {#int-float-float-float-float-isection}
建立一個新的 Section Zoom 框，並將其插入至形狀集合中的
            指定索引位置。

### 回傳值

新建立的 [`ISectionZoomFrame`](/slides/python-net/zh-hant/aspose.slides/isectionzoomframe)。

```python
def insert_section_zoom_frame(self, index, x, y, width, height, section):
    ...
```

| 參數 | 類型 | 描述 |
| :- | :- | :- |
| index | **int** | 要插入 Section Zoom 框的零基索引。 |
| x | **float** | 新 Section Zoom 框的 x 座標（以點為單位）。 |
| y | **float** | 新 Section Zoom 框的 y 座標（以點為單位）。 |
| width | **float** | 新 Section Zoom 框的寬度（以點為單位）。 |
| height | **float** | 新 Section Zoom 框的高度（以點為單位）。 |
| section | [`ISection`](/slides/python-net/zh-hant/aspose.slides/isection) | 由 Section Zoom 框參照的 [`ISection`](/slides/python-net/zh-hant/aspose.slides/isection);<br/><br/>            必須屬於此簡報且至少包含一張投影片。 |

### 例外

| 例外 | 描述 |
| :- | :- |
| **RuntimeError(Proxy error(ArgumentException))** | 如果參照的節段不屬於目前的簡報或不包含任何投影片，則拋出此例外。 |

## insert_section_zoom_frame(self, index, x, y, width, height, section, image) {#int-float-float-float-float-isection-ippimage}
建立一個帶有預先定義影像的新的 Section Zoom 框，並將其插入至形狀集合中的
            指定索引位置。

### 回傳值

新建立的 [`ISectionZoomFrame`](/slides/python-net/zh-hant/aspose.slides/isectionzoomframe)。

```python
def insert_section_zoom_frame(self, index, x, y, width, height, section, image):
    ...
```

| 參數 | 類型 | 描述 |
| :- | :- | :- |
| index | **int** | 要插入 Section Zoom 框的零基索引。 |
| x | **float** | 新 Section Zoom 框的 x 座標（以點為單位）。 |
| y | **float** | 新 Section Zoom 框的 y 座標（以點為單位）。 |
| width | **float** | 新 Section Zoom 框的寬度（以點為單位）。 |
| height | **float** | 新 Section Zoom 框的高度（以點為單位）。 |
| section | [`ISection`](/slides/python-net/zh-hant/aspose.slides/isection) | 由 Section Zoom 框參照的 [`ISection`](/slides/python-net/zh-hant/aspose.slides/isection);<br/><br/>            必須屬於此簡報且至少包含一張投影片。 |
| image | [`IPPImage`](/slides/python-net/zh-hant/aspose.slides/ippimage) | 要在 Section Zoom 框中顯示的影像。 |

### 例外

| 例外 | 描述 |
| :- | :- |
| **RuntimeError(Proxy error(ArgumentException))** | 如果參照的節段不屬於目前的簡報或不包含任何投影片，則拋出此例外。 |

### 另請參閱
* 類別 [`IPPImage`](/slides/python-net/zh-hant/aspose.slides/ippimage)
* 類別 [`ISection`](/slides/python-net/zh-hant/aspose.slides/isection)
* 類別 [`ISectionZoomFrame`](/slides/python-net/zh-hant/aspose.slides/isectionzoomframe)
* 類別 [`IShapeCollection`](/slides/python-net/zh-hant/aspose.slides/ishapecollection)
* 模組 [`aspose.slides`](/slides/python-net/zh-hant/aspose.slides)
* 函式庫 [`Aspose.Slides`](/slides/python-net)