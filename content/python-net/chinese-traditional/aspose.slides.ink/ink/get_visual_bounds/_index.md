...
一個 [`RectangleF`](/slides/python-net/zh-hant/aspose.slides/rectanglef)，代表形狀在投影片座標系統中的視覺邊界。
...
返回的矩形代表在投影片座標空間中，形狀於渲染過程中產生的所有內容的軸對齊邊界。
...
視覺邊界會考慮到渲染相關的因素，例如變換（例如旋轉）、筆畫寬度與接合、文字版面配置與溢位、SmartArt 幾何形狀，以及其他影響形狀最終渲染外觀的版面效果。
