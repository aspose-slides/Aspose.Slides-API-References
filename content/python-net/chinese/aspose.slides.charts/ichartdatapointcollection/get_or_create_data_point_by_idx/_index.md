...
description: 
type: docs
url: /zh/aspose.slides.charts/ichartdatapointcollection/get_or_create_data_point_by_idx/
weight: 190
---
## get_or_create_data_point_by_idx(self, index) {#int}
如果集合已经包含索引为 `index` 的数据点，则返回该数据点。
            如果集合不包含索引为 `index`==N 的数据点
            (当此集合中的数据点数量小于或等于 N 时)
            则添加缺失的数据点并返回最后一个（它具有请求的索引）。
