---
title: end_pos_alpha property
second_title: Aspose.Slides for Python via .NET API 参考
description: 
type: docs
url: /zh/aspose.slides.effects/reflection/end_pos_alpha/
weight: 60
---
## end_pos_alpha 属性
指定终止位置（沿 alpha 渐变坡道）对应的终止 alpha 值（百分比）。
            读写 **float**.

### 定义:
```python
@property
def end_pos_alpha(self):
    ...

@end_pos_alpha.setter
def end_pos_alpha(self, value):
    ...
```


### 另请参见
* 类 [`Reflection`](/slides/python-net/zh/aspose.slides.effects/reflection)
* 模块 [`aspose.slides.effects`](/slides/python-net/zh/aspose.slides.effects)
* 库 [`Aspose.Slides`](/slides/python-net)