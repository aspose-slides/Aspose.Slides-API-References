...
description: 
type: docs
url: /zh/aspose.slides/shapecollection/add_ole_object_frame/
weight: 100
---
...
| 参数 | 类型 | 描述 |
| :- | :- | :- |
| x | **float** | 新 OLE 框的 x 坐标，单位为 points。 |
| y | **float** | 新 OLE 框的 y 坐标，单位为 points。 |
| width | **float** | 新 OLE 框的宽度，单位为 points。 |
| height | **float** | 新 OLE 框的高度，单位为 points。 |
| data_info | [`IOleEmbeddedDataInfo`](/slides/python-net/zh/aspose.slides/ioleembeddeddatainfo) | 关于嵌入的 OLE 数据的信息（[`IOleEmbeddedDataInfo`](/slides/python-net/zh/aspose.slides/ioleembeddeddatainfo)）。 |
...
| path | **str** | 链接文件的路径。 <br/><br/>此路径会原样存储在演示文稿中。<br/><br/>            如果指定了相对路径，则在从不同目录打开演示文稿时，文件将不可访问。 |
