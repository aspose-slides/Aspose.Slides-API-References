...
description: 
type: docs
url: /zh/aspose.slides/shapecollection/add_connector/
weight: 70
---
...
创建一个新连接器形状并将其添加到形状集合的末尾，可选地应用默认模板样式。
...
| width | **float** | 连接器框架的宽度，单位为点。 |
| height | **float** | 连接器框架的高度，单位为点。 |
| create_from_template | **bool** | True 表示应用默认模板样式（非空名称，简单样式）；<br/><br/>false 表示使用默认属性值创建连接器。 |
