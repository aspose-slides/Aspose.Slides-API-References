...
返回一个具有指定大小的 Thumbnail Image 对象。
...
返回一个具有指定参数的 Thumbnail tiff 图像对象。
...
| 异常 | 描述 |
| :- | :- |
| **RuntimeError(Proxy error(InvalidOperationException))** | 当 options.SlideLayoutOption 为 NotesCommentsLayoutingOptions 且其属性 NotesPosition 取值为 NotesPositions.BottomFull 时抛出。 |
...
| 参数 | 类型 | 描述 |
| :- | :- | :- |
| options | [`IRenderingOptions`](/slides/python-net/zh/aspose.slides.export/irenderingoptions) | Rendering 选项。 |
...
返回一个带自定义缩放的 Thumbnail Image 对象。
