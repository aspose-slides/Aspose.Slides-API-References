...
description: 
type: docs
url: /zh/aspose.slides/ishapecollection/insert_auto_shape/
weight: 230
---
...
The newly created [`IAutoShape`](/slides/python-net/zh/aspose.slides/iautoshape).
...
| 参数 | 类型 | 描述 |
| :- | :- | :- |
| index | **int** | 要插入自动形状的基于零的索引。 |
| shape_type | [`ShapeType`](/slides/python-net/zh/aspose.slides/shapetype) | 要插入的自动形状的[`ShapeType`](/slides/python-net/zh/aspose.slides/shapetype)。 |
| x | **float** | 形状框架的 x 坐标（以点为单位）。 |
| y | **float** | 形状框架的 y 坐标（以点为单位）。 |
| width | **float** | 形状框架的宽度（以点为单位）。 |
| height | **float** | 形状框架的高度（以点为单位）。 |
| create_from_template | **bool** | True 表示应用默认模板样式（包括非空名称、简单样式和居中文本）；<br/><br/>false 表示创建形状时所有属性均使用默认值。 |
