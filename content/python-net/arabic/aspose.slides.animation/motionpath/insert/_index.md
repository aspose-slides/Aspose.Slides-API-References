---
title: insert method
second_title: Aspose.Slides لبايثون عبر .NET مرجع API
description: 
type: docs
url: /ar/aspose.slides.animation/motionpath/insert/
weight: 40
---
## insert(self, index, type, pts, pts_type, b_relative_coord) {#int-motioncommandpathtype-listasposepydrawingpointf-motionpathpointstype-bool}
إدراج أمر جديد إلى المسار


```python
def insert(self, index, type, pts, pts_type, b_relative_coord):
    ...
```


| المعامل | النوع | الوصف |
| :- | :- | :- |
| index | **int** |  |
| type | [`MotionCommandPathType`](/slides/python-net/ar/aspose.slides.animation/motioncommandpathtype) |  |
| pts | **List[aspose.slides.PointF]** |  |
| pts_type | [`MotionPathPointsType`](/slides/python-net/ar/aspose.slides.animation/motionpathpointstype) |  |
| b_relative_coord | **bool** |  |



### انظر أيضًا
* تعداد [`MotionCommandPathType`](/slides/python-net/ar/aspose.slides.animation/motioncommandpathtype)
* فئة [`MotionPath`](/slides/python-net/ar/aspose.slides.animation/motionpath)
* تعداد [`MotionPathPointsType`](/slides/python-net/ar/aspose.slides.animation/motionpathpointstype)
* وحدة [`aspose.slides.animation`](/slides/python-net/ar/aspose.slides.animation)
* مكتبة [`Aspose.Slides`](/slides/python-net)