---
title: write_slide_start method
second_title: Aspose.Slides للبايثون عبر .NET مرجع API
description: 
type: docs
url: /ar/aspose.slides.export/embeddedeotfontshtmlcontroller/write_slide_start/
weight: 70
---
## write_slide_start(self, generator, slide) {#ihtmlgenerator-islide}}

```python
def write_slide_start(self, generator, slide):
    ...
```

| المعامل | النوع | الوصف |
| :- | :- | :- |
| generator | [`IHtmlGenerator`](/slides/python-net/ar/aspose.slides.export/ihtmlgenerator) |  |
| slide | [`ISlide`](/slides/python-net/ar/aspose.slides/islide) |  |

### انظر أيضًا
* فئة [`EmbeddedEotFontsHtmlController`](/slides/python-net/ar/aspose.slides.export/embeddedeotfontshtmlcontroller)
* فئة [`IHtmlGenerator`](/slides/python-net/ar/aspose.slides.export/ihtmlgenerator)
* فئة [`ISlide`](/slides/python-net/ar/aspose.slides/islide)
* وحدة [`aspose.slides.export`](/slides/python-net/ar/aspose.slides.export)
* مكتبة [`Aspose.Slides`](/slides/python-net)