---
title: to_border_box method
second_title: Αναφορά API Aspose.Slides για Python μέσω .NET
description: 
type: docs
url: /el/aspose.slides.mathtext/mathbar/to_border_box/
weight: 200
---
## to_border_box(self) {#}
Τοποθετεί αυτό το στοιχείο σε πλαίσιο-περιθώριο

### Επιστρέφει
Πλαίσιο-περιθώριο με αυτό το στοιχείο τοποθετημένο μέσα


```python
def to_border_box(self):
    ...
```


## to_border_box(self, hide_top, hide_bottom, hide_left, hide_right, strikethrough_horizontal, strikethrough_vertical, strikethrough_bottom_left_to_top_right, strikethrough_top_left_to_bottom_right) {#bool-bool-bool-bool-bool-bool-bool-bool}
Τοποθετεί αυτό το στοιχείο σε πλαίσιο-περιθώριο

### Επιστρέφει
Πλαίσιο-περιθώριο με αυτό το στοιχείο τοποθετημένο μέσα


```python
def to_border_box(self, hide_top, hide_bottom, hide_left, hide_right, strikethrough_horizontal, strikethrough_vertical, strikethrough_bottom_left_to_top_right, strikethrough_top_left_to_bottom_right):
    ...
```


| Παράμετρος | Τύπος | Περιγραφή |
| :- | :- | :- |
| hide_top | **bool** | Απόκρυψη άνω άκρου |
| hide_bottom | **bool** | Απόκρυψη κάτω άκρου |
| hide_left | **bool** | Απόκρυψη αριστερού άκρου |
| hide_right | **bool** | Απόκρυψη δεξιού άκρου |
| strikethrough_horizontal | **bool** | Διαγραφή οριζόντιας γραμμής πλαισίου-περιθωρίου |
| strikethrough_vertical | **bool** | Διαγραφή κάθετης γραμμής πλαισίου-περιθωρίου |
| strikethrough_bottom_left_to_top_right | **bool** | Διαγραφή πλαισίου-περιθωρίου από κάτω-αριστερά προς πάνω-δεξιά |
| strikethrough_top_left_to_bottom_right | **bool** | Διαγραφή πλαισίου-περιθωρίου από πάνω-αριστερά προς κάτω-δεξιά |


### Δείτε επίσης
* κλάση [`IMathBorderBox`](/slides/python-net/el/aspose.slides.mathtext/imathborderbox)
* κλάση [`MathBar`](/slides/python-net/el/aspose.slides.mathtext/mathbar)
* μονάδα [`aspose.slides.mathtext`](/slides/python-net/el/aspose.slides.mathtext)
* βιβλιοθήκη [`Aspose.Slides`](/slides/python-net)