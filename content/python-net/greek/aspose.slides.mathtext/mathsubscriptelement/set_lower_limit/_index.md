---
title: set_lower_limit method
second_title: Aspose.Slides για Python μέσω .NET API Αναφορά
description: 
type: docs
url: /el/aspose.slides.mathtext/mathsubscriptelement/set_lower_limit/
weight: 140
---
## set_lower_limit(self, limit) {#imathelement}
Λαμβάνει το κατώτατο όριο

### Επιστρέφει

Νέα παρουσία τύπου [`IMathLimit`](/slides/python-net/el/aspose.slides.mathtext/imathlimit)



```python
def set_lower_limit(self, limit):
    ...
```


| Παράμετρος | Τύπος | Περιγραφή |
| :- | :- | :- |
| limit | [`IMathElement`](/slides/python-net/el/aspose.slides.mathtext/imathelement) | όριο |


## set_lower_limit(self, limit) {#str}
Λαμβάνει το κατώτατο όριο

### Επιστρέφει

Νέα παρουσία τύπου [`IMathLimit`](/slides/python-net/el/aspose.slides.mathtext/imathlimit)



```python
def set_lower_limit(self, limit):
    ...
```


| Παράμετρος | Τύπος | Περιγραφή |
| :- | :- | :- |
| limit | **str** | όριο |



### Δείτε επίσης
* κλάση [`IMathElement`](/slides/python-net/el/aspose.slides.mathtext/imathelement)
* κλάση [`IMathLimit`](/slides/python-net/el/aspose.slides.mathtext/imathlimit)
* κλάση [`MathSubscriptElement`](/slides/python-net/el/aspose.slides.mathtext/mathsubscriptelement)
* μονάδα [`aspose.slides.mathtext`](/slides/python-net/el/aspose.slides.mathtext)
* βιβλιοθήκη [`Aspose.Slides`](/slides/python-net)