---
title: get_by_type method
second_title: Tham chiếu API Aspose.Slides cho Python qua .NET
description: 
type: docs
url: /vi/aspose.slides/layoutslidecollection/get_by_type/
weight: 10
---
## get_by_type(self, type) {#slidelayouttype}
Trả về slide bố cục đầu tiên có loại được chỉ định.
            Một loại slide bố cục để tìm.[`LayoutSlide`](/slides/python-net/vi/aspose.slides/layoutslide) với loại được chỉ định hoặc None nếu không tìm thấy bố cục nào.


```python
def get_by_type(self, type):
    ...
```


| Tham số | Kiểu | Mô tả |
| :- | :- | :- |
| type | [`SlideLayoutType`](/slides/python-net/vi/aspose.slides/slidelayouttype) |  |



### Xem thêm
* lớp [`ILayoutSlide`](/slides/python-net/vi/aspose.slides/ilayoutslide)
* lớp [`LayoutSlide`](/slides/python-net/vi/aspose.slides/layoutslide)
* lớp [`LayoutSlideCollection`](/slides/python-net/vi/aspose.slides/layoutslidecollection)
* liệt kê [`SlideLayoutType`](/slides/python-net/vi/aspose.slides/slidelayouttype)
* mô-đun [`aspose.slides`](/slides/python-net/vi/aspose.slides)
* thư viện [`Aspose.Slides`](/slides/python-net)