---
title: join method
second_title: Tham chiếu API Aspose.Slides cho Python thông qua .NET
description: 
type: docs
url: /vi/aspose.slides.mathtext/imathelement/join/
weight: 90
---
## join(self, math_element) {#imathelement}
Kết hợp một phần tử toán học và tạo thành một khối toán học

### Trả về

Một IMathBlock mới chứa thể hiện này và đối số đã chỉ định



```python
def join(self, math_element):
    ...
```


| Tham số | Kiểu | Mô tả |
| :- | :- | :- |
| math_element | [`IMathElement`](/slides/python-net/vi/aspose.slides.mathtext/imathelement) | Phần tử cần được kết hợp |


## join(self, math_text) {#str}
Kết hợp một đoạn văn bản toán học và tạo thành một khối toán học

### Trả về

Một IMathBlock mới chứa thể hiện này và đối số đã chỉ định



```python
def join(self, math_text):
    ...
```


| Tham số | Kiểu | Mô tả |
| :- | :- | :- |
| math_text | **str** | Đoạn văn bản toán học cần được kết hợp |



### Xem thêm
* lớp [`IMathBlock`](/slides/python-net/vi/aspose.slides.mathtext/imathblock)
* lớp [`IMathElement`](/slides/python-net/vi/aspose.slides.mathtext/imathelement)
* mô-đun [`aspose.slides.mathtext`](/slides/python-net/vi/aspose.slides.mathtext)
* thư viện [`Aspose.Slides`](/slides/python-net)