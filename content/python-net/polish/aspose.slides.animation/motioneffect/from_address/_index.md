---
title: from_address property
second_title: Aspose.Slides dla Pythona poprzez .NET – odniesienie API
description: 
type: docs
url: /pl/aspose.slides.animation/motioneffect/from_address/
weight: 70
---
## from_address właściwość
Określa współrzędną x/y, od której rozpocząć animację (w procentach). 
            Odczyt/zapis [`PointF`](/slides/python-net/pl/aspose.slides/pointf).

### Definicja:
```python
@property
def from_address(self):
    ...

@from_address.setter
def from_address(self, value):
    ...
```


### Zobacz także
* klasa [`MotionEffect`](/slides/python-net/pl/aspose.slides.animation/motioneffect)
* klasa [`PointF`](/slides/python-net/pl/aspose.slides/pointf)
* moduł [`aspose.slides.animation`](/slides/python-net/pl/aspose.slides.animation)
* biblioteka [`Aspose.Slides`](/slides/python-net)