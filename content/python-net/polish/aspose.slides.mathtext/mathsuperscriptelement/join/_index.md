---
title: join method
second_title: Aspose.Slides dla Pythona przez .NET – dokumentacja API
description: 
type: docs
url: /pl/aspose.slides.mathtext/mathsuperscriptelement/join/
weight: 100
---
## join(self, math_element) {#imathelement}
Dołącza element matematyczny i tworzy blok matematyczny

### Zwraca

Nowy IMathBlock zawierający tę instancję i określony argument



```python
def join(self, math_element):
    ...
```


| Parametr | Typ | Opis |
| :- | :- | :- |
| math_element | [`IMathElement`](/slides/python-net/pl/aspose.slides.mathtext/imathelement) | Element do połączenia |


## join(self, math_text) {#str}
Dołącza tekst matematyczny i tworzy blok matematyczny

### Zwraca

Nowy IMathBlock zawierający tę instancję i określony argument



```python
def join(self, math_text):
    ...
```


| Parametr | Typ | Opis |
| :- | :- | :- |
| math_text | **str** | Tekst matematyczny do połączenia |



### Zobacz także
* klasa [`IMathBlock`](/slides/python-net/pl/aspose.slides.mathtext/imathblock)
* klasa [`IMathElement`](/slides/python-net/pl/aspose.slides.mathtext/imathelement)
* klasa [`MathSuperscriptElement`](/slides/python-net/pl/aspose.slides.mathtext/mathsuperscriptelement)
* moduł [`aspose.slides.mathtext`](/slides/python-net/pl/aspose.slides.mathtext)
* biblioteka [`Aspose.Slides`](/slides/python-net)