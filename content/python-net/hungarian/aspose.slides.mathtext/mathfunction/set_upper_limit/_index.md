---
title: set_upper_limit method
second_title: Aspose.Slides a Python számára .NET API-referencián keresztül
description: 
type: docs
url: /hu/aspose.slides.mathtext/mathfunction/set_upper_limit/
weight: 190
---
## set_upper_limit(self, limit) {#imathelement}
Elfogadja a felső határt

### Visszatér

Új példány a(z) [`IMathLimit`](/slides/python-net/hu/aspose.slides.mathtext/imathlimit) típusú



```python
def set_upper_limit(self, limit):
    ...
```


| Paraméter | Típus | Leírás |
| :- | :- | :- |
| limit | [`IMathElement`](/slides/python-net/hu/aspose.slides.mathtext/imathelement) | limit |


## set_upper_limit(self, limit) {#str}
Elfogadja a felső határt

### Visszatér

Új példány a(z) [`IMathLimit`](/slides/python-net/hu/aspose.slides.mathtext/imathlimit) típusú



```python
def set_upper_limit(self, limit):
    ...
```


| Paraméter | Típus | Leírás |
| :- | :- | :- |
| limit | **str** | limit |


### Lásd még
* osztály [`IMathElement`](/slides/python-net/hu/aspose.slides.mathtext/imathelement)
* osztály [`IMathLimit`](/slides/python-net/hu/aspose.slides.mathtext/imathlimit)
* osztály [`MathFunction`](/slides/python-net/hu/aspose.slides.mathtext/mathfunction)
* modul [`aspose.slides.mathtext`](/slides/python-net/hu/aspose.slides.mathtext)
* könyvtár [`Aspose.Slides`](/slides/python-net)