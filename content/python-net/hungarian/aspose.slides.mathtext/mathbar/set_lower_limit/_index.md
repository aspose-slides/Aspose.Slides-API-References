---
title: set_lower_limit method
second_title: Aspose.Slides for Python a .NET API referencia
description: 
type: docs
url: /hu/aspose.slides.mathtext/mathbar/set_lower_limit/
weight: 140
---
## set_lower_limit(self, limit) {#imathelement}
Alacsony határértéket vesz fel

### Visszatér

Új példány a(z) [`IMathLimit`](/slides/python-net/hu/aspose.slides.mathtext/imathlimit) típusból



```python
def set_lower_limit(self, limit):
    ...
```


| Paraméter | Típus | Leírás |
| :- | :- | :- |
| limit | [`IMathElement`](/slides/python-net/hu/aspose.slides.mathtext/imathelement) | határ |


## set_lower_limit(self, limit) {#str}
Alacsony határértéket vesz fel

### Visszatér

Új példány a(z) [`IMathLimit`](/slides/python-net/hu/aspose.slides.mathtext/imathlimit) típusból



```python
def set_lower_limit(self, limit):
    ...
```


| Paraméter | Típus | Leírás |
| :- | :- | :- |
| limit | **str** | határ |



### Lásd még
* osztály [`IMathElement`](/slides/python-net/hu/aspose.slides.mathtext/imathelement)
* osztály [`IMathLimit`](/slides/python-net/hu/aspose.slides.mathtext/imathlimit)
* osztály [`MathBar`](/slides/python-net/hu/aspose.slides.mathtext/mathbar)
* modul [`aspose.slides.mathtext`](/slides/python-net/hu/aspose.slides.mathtext)
* könyvtár [`Aspose.Slides`](/slides/python-net)