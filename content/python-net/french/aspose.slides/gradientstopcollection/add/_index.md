---
title: add method
second_title: Référence de l'API Aspose.Slides pour Python via .NET
description: 
type: docs
url: /fr/aspose.slides/gradientstopcollection/add/
weight: 10
---
## add(self, position, color) {#float-asposepydrawingcolor}
Crée le nouveau point d'arrêt de dégradé et l'ajoute à la fin de la collection.

### Retour
Indice du nouveau point d'arrêt de dégradé dans la collection.



```python
def add(self, position, color):
    ...
```


| Paramètre | Type | Description |
| :- | :- | :- |
| position | **float** | Position du nouveau point d'arrêt de dégradé. |
| color | **aspose.slides.Color** | Couleur du nouveau point d'arrêt de dégradé. |


## add(self, position, preset_color) {#float-presetcolor}
Crée le nouveau point d'arrêt de dégradé et l'ajoute à la fin de la collection.

### Retour
Indice du nouveau point d'arrêt de dégradé dans la collection.



```python
def add(self, position, preset_color):
    ...
```


| Paramètre | Type | Description |
| :- | :- | :- |
| position | **float** | Position du nouveau point d'arrêt de dégradé. |
| preset_color | [`PresetColor`](/slides/python-net/fr/aspose.slides/presetcolor) | Couleur du nouveau point d'arrêt de dégradé. |


## add(self, position, scheme_color) {#float-schemecolor}
Crée le nouveau point d'arrêt de dégradé et l'ajoute à la fin de la collection.

### Retour
Indice du nouveau point d'arrêt de dégradé dans la collection.



```python
def add(self, position, scheme_color):
    ...
```


| Paramètre | Type | Description |
| :- | :- | :- |
| position | **float** | Position du nouveau point d'arrêt de dégradé. |
| scheme_color | [`SchemeColor`](/slides/python-net/fr/aspose.slides/schemecolor) | Couleur du nouveau point d'arrêt de dégradé. |



### Voir aussi
* class [`GradientStopCollection`](/slides/python-net/fr/aspose.slides/gradientstopcollection)
* class [`IGradientStop`](/slides/python-net/fr/aspose.slides/igradientstop)
* enumeration [`PresetColor`](/slides/python-net/fr/aspose.slides/presetcolor)
* enumeration [`SchemeColor`](/slides/python-net/fr/aspose.slides/schemecolor)
* module [`aspose.slides`](/slides/python-net/fr/aspose.slides)
* library [`Aspose.Slides`](/slides/python-net)