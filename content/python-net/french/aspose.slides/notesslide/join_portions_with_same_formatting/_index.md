---
title: join_portions_with_same_formatting method
second_title: Référence API Aspose.Slides pour Python via .NET
description: 
type: docs
url: /fr/aspose.slides/notesslide/join_portions_with_same_formatting/
weight: 40
---
## join_portions_with_same_formatting(self) {#}
Réunit les portions avec la même mise en forme dans tous les paragraphes de toutes les formes acceptables.

```python
def join_portions_with_same_formatting(self):
    ...
```

## join_portions_with_same_formatting(self, collection) {#ishapecollection}
Réunit les portions avec la même mise en forme dans tous les paragraphes de toutes les formes acceptables.

```python
def join_portions_with_same_formatting(self, collection):
    ...
```

| Paramètre | Type | Description |
| :- | :- | :- |
| collection | [`IShapeCollection`](/slides/python-net/fr/aspose.slides/ishapecollection) |  |

### Voir aussi
* classe [`IShapeCollection`](/slides/python-net/fr/aspose.slides/ishapecollection)
* classe [`NotesSlide`](/slides/python-net/fr/aspose.slides/notesslide)
* module [`aspose.slides`](/slides/python-net/fr/aspose.slides)
* bibliothèque [`Aspose.Slides`](/slides/python-net)