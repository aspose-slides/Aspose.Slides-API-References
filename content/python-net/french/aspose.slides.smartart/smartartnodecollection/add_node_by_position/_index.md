---
title: add_node_by_position method
second_title: Référence de l'API Aspose.Slides pour Python via .NET
description: 
type: docs
url: /fr/aspose.slides.smartart/smartartnodecollection/add_node_by_position/
weight: 20
---
## add_node_by_position(self, position) {#int}
Ajoute un nouveau nœud à la position sélectionnée de la collection de nœuds

### Valeur de retour

Nœud ajouté



```python
def add_node_by_position(self, position):
    ...
```


| Paramètre | Type | Description |
| :- | :- | :- |
| position | **int** | Position du nœud à base zéro |

### Exceptions

| Exception | Description |
| :- | :- |
| **RuntimeError(Proxy error(ArgumentOutOfRangeException))** | la position est inférieure à 0 |



### Voir aussi
* classe [`ISmartArtNode`](/slides/python-net/fr/aspose.slides.smartart/ismartartnode)
* classe [`SmartArtNodeCollection`](/slides/python-net/fr/aspose.slides.smartart/smartartnodecollection)
* module [`aspose.slides.smartart`](/slides/python-net/fr/aspose.slides.smartart)
* bibliothèque [`Aspose.Slides`](/slides/python-net)