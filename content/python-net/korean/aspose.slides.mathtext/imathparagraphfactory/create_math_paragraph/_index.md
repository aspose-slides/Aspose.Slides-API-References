---
title: create_math_paragraph method
second_title: Aspose.Slides for Python via .NET API 레퍼런스
description: 
type: docs
url: /ko/aspose.slides.mathtext/imathparagraphfactory/create_math_paragraph/
weight: 10
---
## create_math_paragraph(self) {#}
빈 수학 단락 만들기

### Returns

새 수학 단락



```python
def create_math_paragraph(self):
    ...
```



## create_math_paragraph(self, math_block) {#imathblock}
수학 단락을 만들고 지정된 수학 블록을 그 안에 배치합니다

### Returns

새 수학 단락



```python
def create_math_paragraph(self, math_block):
    ...
```


| 매개변수 | 유형 | 설명 |
| :- | :- | :- |
| math_block | [`IMathBlock`](/slides/python-net/ko/aspose.slides.mathtext/imathblock) | 단락에 넣을 수학 블록 |



### 관련 항목
* 클래스 [`IMathBlock`](/slides/python-net/ko/aspose.slides.mathtext/imathblock)
* 클래스 [`IMathParagraph`](/slides/python-net/ko/aspose.slides.mathtext/imathparagraph)
* 클래스 [`IMathParagraphFactory`](/slides/python-net/ko/aspose.slides.mathtext/imathparagraphfactory)
* 모듈 [`aspose.slides.mathtext`](/slides/python-net/ko/aspose.slides.mathtext)
* 라이브러리 [`Aspose.Slides`](/slides/python-net)