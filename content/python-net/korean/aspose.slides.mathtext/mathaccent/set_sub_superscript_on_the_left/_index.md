---
title: set_sub_superscript_on_the_left method
second_title: Aspose.Slides Python용 .NET API 참조
description: 
type: docs
url: /ko/aspose.slides.mathtext/mathaccent/set_sub_superscript_on_the_left/
weight: 150
---
## set_sub_superscript_on_the_left(self, subscript, superscript) {#imathelement-imathelement}
왼쪽에 첨자와 위첨자를 생성합니다

### 반환값

새 수학 요소 유형 [`IMathLeftSubSuperscriptElement`](/slides/python-net/ko/aspose.slides.mathtext/imathleftsubsuperscriptelement)



```python
def set_sub_superscript_on_the_left(self, subscript, superscript):
    ...
```


| 매개변수 | 형식 | 설명 |
| :- | :- | :- |
| subscript | [`IMathElement`](/slides/python-net/ko/aspose.slides.mathtext/imathelement) | 첨자 (왼쪽에 낮은 인덱스) |
| superscript | [`IMathElement`](/slides/python-net/ko/aspose.slides.mathtext/imathelement) | 위첨자 (왼쪽에 높은 인덱스) |


## set_sub_superscript_on_the_left(self, subscript, superscript) {#str-str}
왼쪽에 첨자와 위첨자를 생성합니다

### 반환값

새 수학 요소 유형 [`IMathLeftSubSuperscriptElement`](/slides/python-net/ko/aspose.slides.mathtext/imathleftsubsuperscriptelement)



```python
def set_sub_superscript_on_the_left(self, subscript, superscript):
    ...
```


| 매개변수 | 형식 | 설명 |
| :- | :- | :- |
| subscript | **str** | 첨자 (왼쪽에 낮은 인덱스) |
| superscript | **str** | 위첨자 (왼쪽에 높은 인덱스) |



### 참고
* 클래스 [`IMathElement`](/slides/python-net/ko/aspose.slides.mathtext/imathelement)
* 클래스 [`IMathLeftSubSuperscriptElement`](/slides/python-net/ko/aspose.slides.mathtext/imathleftsubsuperscriptelement)
* 클래스 [`MathAccent`](/slides/python-net/ko/aspose.slides.mathtext/mathaccent)
* 모듈 [`aspose.slides.mathtext`](/slides/python-net/ko/aspose.slides.mathtext)
* 라이브러리 [`Aspose.Slides`](/slides/python-net)