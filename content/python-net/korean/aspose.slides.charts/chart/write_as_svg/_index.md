---
title: write_as_svg method
second_title: Aspose.Slides for Python via .NET API 레퍼런스
description: 
type: docs
url: /ko/aspose.slides.charts/chart/write_as_svg/
weight: 80
---
## write_as_svg(self, stream) {#iorawiobase}
Shape의 콘텐츠를 SVG 파일로 저장합니다.


```python
def write_as_svg(self, stream):
    ...
```


| 매개변수 | 형식 | 설명 |
| :- | :- | :- |
| stream | **io.RawIOBase** | 대상 스트림 |


## write_as_svg(self, stream, svg_options) {#iorawiobase-asposeslidesexportisvgoptions}
Shape의 콘텐츠를 SVG 파일로 저장합니다.


```python
def write_as_svg(self, stream, svg_options):
    ...
```


| 매개변수 | 형식 | 설명 |
| :- | :- | :- |
| stream | **io.RawIOBase** | 대상 스트림 |
| svg_options | [`ISVGOptions`](/slides/python-net/ko/aspose.slides.export/isvgoptions) | SVG 생성 옵션 |



### 참조
* 클래스 [`Chart`](/slides/python-net/ko/aspose.slides.charts/chart)
* 클래스 [`ISVGOptions`](/slides/python-net/ko/aspose.slides.export/isvgoptions)
* 모듈 [`aspose.slides.charts`](/slides/python-net/ko/aspose.slides.charts)
* 라이브러리 [`Aspose.Slides`](/slides/python-net)