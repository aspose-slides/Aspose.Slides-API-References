---
title: image_size property
second_title: Aspose.Slides for Python via .NET API 레퍼런스
description: 
type: docs
url: /ko/aspose.slides.export/itiffoptions/image_size/
weight: 80
---
## image_size 속성
생성된 TIFF 이미지의 크기를 지정합니다.
            기본값은 0x0이며, 이는 생성된 이미지 크기가 프레젠테이션 슬라이드 크기 값을 기준으로 계산됨을 의미합니다.
            읽기/쓰기 [`Size`](/slides/python-net/ko/aspose.slides/size).

### 정의:
```python
@property
def image_size(self):
    ...

@image_size.setter
def image_size(self, value):
    ...
```

### 참고
* 클래스 [`ITiffOptions`](/slides/python-net/ko/aspose.slides.export/itiffoptions)
* 클래스 [`Size`](/slides/python-net/ko/aspose.slides/size)
* 모듈 [`aspose.slides.export`](/slides/python-net/ko/aspose.slides.export)
* 라이브러리 [`Aspose.Slides`](/slides/python-net)