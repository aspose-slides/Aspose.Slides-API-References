---
title: transition_duration property
second_title: Aspose.Slides Python용 .NET API 레퍼런스
description: 
type: docs
url: /ko/aspose.slides/zoomframe/transition_duration/
weight: 450
---
## transition_duration 속성
Zoom과 슬라이드 사이 전환의 지속 시간을 가져오거나 설정합니다.
            읽기/쓰기 **float**.
            기본값: 1.0f


### 비고

만약 지정되지 않으면 (TransitionDur = 0), 대상 슬라이드 전환과 해당 전환에 연결된 타이밍을 사용합니다.

### 정의:
```python
@property
def transition_duration(self):
    ...

@transition_duration.setter
def transition_duration(self, value):
    ...
```


### 참고
* 클래스 [`ZoomFrame`](/slides/python-net/ko/aspose.slides/zoomframe)
* 모듈 [`aspose.slides`](/slides/python-net/ko/aspose.slides)
* 라이브러리 [`Aspose.Slides`](/slides/python-net)