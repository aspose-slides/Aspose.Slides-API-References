...
description:
type: docs
url: /ko/aspose.slides/picturefillformat/compress_image/
weight: 10
---
## compress_image(self, delete_cropped_areas_of_image, resolution) {#bool-asposeslidesexportpicturescompression}
이미지의 크기를 도형 크기와 지정된 해상도에 따라 감소시켜 압축합니다. 선택적으로 잘린 영역을 삭제합니다.

### 반환
**bool**는 이미지가 성공적으로 압축되었는지를 나타냅니다. 이미지가 크기가 조정되었거나 잘린 경우 **True**, 그렇지 않으면 **False**를 반환합니다.
...
| 매개변수 | 형식 | 설명 |
| :- | :- | :- |
| delete_cropped_areas_of_image | **bool** | true인 경우, 메서드는 이미지의 잘린 영역을 제거하여 크기를 더욱 줄일 수 있습니다. |
...
| 예외 | 설명 |
| :- | :- |
| **RuntimeError(Proxy error(ArgumentException))** | 해상도가 유효한 값이 아닌 경우 발생합니다. |
...
| :- | :- | :- |
| delete_cropped_areas_of_image | **bool** | true인 경우, 메서드는 이미지의 잘린 영역을 제거하여 크기를 더욱 줄일 수 있습니다. |
| resolution | **float** | DPI 단위의 목표 해상도입니다. 이 값은 양수여야 하며 이미지가 어떻게 크기 조정될지를 정의합니다. |
...
| 예외 | 설명 |
| :- | :- |
| **RuntimeError(Proxy error(ArgumentException))** | 해상도가 양수가 아닌 경우 발생합니다. |

### 참조
* 클래스 [`PictureFillFormat`](/slides/python-net/ko/aspose.slides/picturefillformat)
