...
렌더링된 단락을 포함하는 이미지, 또는 **None** 을 반환합니다. 단락을 상위 컬렉션에서 찾을 수 없거나, 유효한 렌더링 경계가 없거나, 이미지를 렌더링하는 동안 오류가 발생한 경우 **None** 을 반환합니다.
...
| 매개변수 | 형식 | 설명 |
| :- | :- | :- |
| scale_x | **float** | 단락 이미지에 적용되는 가로 스케일 계수. |
| scale_y | **float** | 단락 이미지에 적용되는 세로 스케일 계수. |
