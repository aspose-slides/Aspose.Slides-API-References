---
title: add_placeholder method
second_title: Aspose.Slides for Python via .NET API 레퍼런스
description: 
type: docs
url: /ko/aspose.slides/summaryzoomsection/add_placeholder/
weight: 10
---
## add_placeholder(self, placeholder_to_copy_from) {#iplaceholder}
새 자리 표시자를 추가하고 지정된 자리 표시자의 속성을 설정합니다.

### 반환값
새로운 [`Shape.placeholder`](/slides/python-net/ko/aspose.slides/shape/placeholder).



```python
def add_placeholder(self, placeholder_to_copy_from):
    ...
```


| 매개변수 | 유형 | 설명 |
| :- | :- | :- |
| placeholder_to_copy_from | [`IPlaceholder`](/slides/python-net/ko/aspose.slides/iplaceholder) | 내용을 복사할 자리 표시자. |



### 참고
* 클래스 [`IPlaceholder`](/slides/python-net/ko/aspose.slides/iplaceholder)
* 클래스 [`SummaryZoomSection`](/slides/python-net/ko/aspose.slides/summaryzoomsection)
* 모듈 [`aspose.slides`](/slides/python-net/ko/aspose.slides)
* 라이브러리 [`Aspose.Slides`](/slides/python-net)