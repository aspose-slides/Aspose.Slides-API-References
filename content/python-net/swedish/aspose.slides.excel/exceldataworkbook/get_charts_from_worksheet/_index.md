---
title: get_charts_from_worksheet method
second_title: Aspose.Slides for Python via .NET API Reference
description: 
type: docs
url: /sv/aspose.slides.excel/exceldataworkbook/get_charts_from_worksheet/
weight: 40
---
## get_charts_from_worksheet(self, worksheet_name) {#str}
Hämtar en ordbok som innehåller index och namn för alla diagram i det angivna kalkylbladet i en Excel-arbetsbok.

### Returnerar

En lista med [`PositionedString`](/slides/python-net/sv/aspose.slides/positionedstring) där nyckeln är diagramindexet och värdet är diagramnamnet.



```python
def get_charts_from_worksheet(self, worksheet_name):
    ...
```


| Parameter | Typ | Beskrivning |
| :- | :- | :- |
| worksheet_name | **str** | Namnet på kalkylbladet där diagram ska sökas. |



### Se även
* klass [`ExcelDataWorkbook`](/slides/python-net/sv/aspose.slides.excel/exceldataworkbook)
* klass [`PositionedString`](/slides/python-net/sv/aspose.slides/positionedstring)
* modul [`aspose.slides.excel`](/slides/python-net/sv/aspose.slides.excel)
* bibliotek [`Aspose.Slides`](/slides/python-net)