---
title: set_lower_limit method
second_title: Aspose.Slides för Python via .NET API-referens
description: 
type: docs
url: /sv/aspose.slides.mathtext/mathsuperscriptelement/set_lower_limit/
weight: 140
---
## set_lower_limit(self, limit) {#imathelement}
Tar lägre gränsvärde

### Returnerar

Ny instans av typ [`IMathLimit`](/slides/python-net/sv/aspose.slides.mathtext/imathlimit)



```python
def set_lower_limit(self, limit):
    ...
```


| Parameter | Typ | Beskrivning |
| :- | :- | :- |
| limit | [`IMathElement`](/slides/python-net/sv/aspose.slides.mathtext/imathelement) | limit |


## set_lower_limit(self, limit) {#str}
Tar lägre gränsvärde

### Returnerar

Ny instans av typ [`IMathLimit`](/slides/python-net/sv/aspose.slides.mathtext/imathlimit)



```python
def set_lower_limit(self, limit):
    ...
```


| Parameter | Typ | Beskrivning |
| :- | :- | :- |
| limit | **str** | limit |



### Se också
* klass [`IMathElement`](/slides/python-net/sv/aspose.slides.mathtext/imathelement)
* klass [`IMathLimit`](/slides/python-net/sv/aspose.slides.mathtext/imathlimit)
* klass [`MathSuperscriptElement`](/slides/python-net/sv/aspose.slides.mathtext/mathsuperscriptelement)
* modul [`aspose.slides.mathtext`](/slides/python-net/sv/aspose.slides.mathtext)
* bibliotek [`Aspose.Slides`](/slides/python-net)