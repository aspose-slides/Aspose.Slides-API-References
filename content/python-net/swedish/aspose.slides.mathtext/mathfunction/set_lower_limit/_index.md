---
title: set_lower_limit method
second_title: Aspose.Slides för Python via .NET API-referens
description: 
type: docs
url: /sv/aspose.slides.mathtext/mathfunction/set_lower_limit/
weight: 140
---
## set_lower_limit(self, limit) {#imathelement}
Tar den nedre gränsen

### Returnerar

Ny instans av typen [`IMathLimit`](/slides/python-net/sv/aspose.slides.mathtext/imathlimit)

```python
def set_lower_limit(self, limit):
    ...
```

| Parameter | Typ | Beskrivning |
| :- | :- | :- |
| limit | [`IMathElement`](/slides/python-net/sv/aspose.slides.mathtext/imathelement) | gräns |

## set_lower_limit(self, limit) {#str}
Tar den nedre gränsen

### Returnerar

Ny instans av typen [`IMathLimit`](/slides/python-net/sv/aspose.slides.mathtext/imathlimit)

```python
def set_lower_limit(self, limit):
    ...
```

| Parameter | Typ | Beskrivning |
| :- | :- | :- |
| limit | **str** | gräns |

### Se även
* klass [`IMathElement`](/slides/python-net/sv/aspose.slides.mathtext/imathelement)
* klass [`IMathLimit`](/slides/python-net/sv/aspose.slides.mathtext/imathlimit)
* klass [`MathFunction`](/slides/python-net/sv/aspose.slides.mathtext/mathfunction)
* modul [`aspose.slides.mathtext`](/slides/python-net/sv/aspose.slides.mathtext)
* bibliotek [`Aspose.Slides`](/slides/python-net)