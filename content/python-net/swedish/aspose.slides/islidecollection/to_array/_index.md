---
title: to_array method
second_title: Aspose.Slides för Python via .NET API-referens
description: 
type: docs
url: /sv/aspose.slides/islidecollection/to_array/
weight: 120
---
## to_array(self) {#}
Skapar och returnerar en array med alla bilder i den.

### Returnerar

Array av [`ISlide`](/slides/python-net/sv/aspose.slides/islide)



```python
def to_array(self):
    ...
```



## to_array(self, start_index, count) {#int-int}
Skapar och returnerar en array med alla bilder från det angivna intervallet i den.

### Returnerar

Array av [`ISlide`](/slides/python-net/sv/aspose.slides/islide)



```python
def to_array(self, start_index, count):
    ...
```


| Parameter | Typ | Beskrivning |
| :- | :- | :- |
| start_index | **int** | Ett index för den första bilden att lägga till. |
| count | **int** | Antalet bilder att lägga till. |



### Se även
* klass [`ISlide`](/slides/python-net/sv/aspose.slides/islide)
* klass [`ISlideCollection`](/slides/python-net/sv/aspose.slides/islidecollection)
* modul [`aspose.slides`](/slides/python-net/sv/aspose.slides)
* bibliotek [`Aspose.Slides`](/slides/python-net)