---
title: insert_empty_slide method
second_title: Aspose.Slides för Python via .NET API-referens
description: 
type: docs
url: /sv/aspose.slides/slidecollection/insert_empty_slide/
weight: 70
---
## insert_empty_slide(self, index, layout) {#int-ilayoutslide}
Infogar en kopia av en specificerad bild på en specificerad position i samlingen.

### Returnerar

Infogad bild.



```python
def insert_empty_slide(self, index, layout):
    ...
```


| Parameter | Typ | Beskrivning |
| :- | :- | :- |
| index | **int** | Index för en ny bild. |
| layout | [`ILayoutSlide`](/slides/python-net/sv/aspose.slides/ilayoutslide) | Layout för en bild. |



### Se även
* klass [`ILayoutSlide`](/slides/python-net/sv/aspose.slides/ilayoutslide)
* klass [`ISlide`](/slides/python-net/sv/aspose.slides/islide)
* klass [`SlideCollection`](/slides/python-net/sv/aspose.slides/slidecollection)
* modul [`aspose.slides`](/slides/python-net/sv/aspose.slides)
* bibliotek [`Aspose.Slides`](/slides/python-net)