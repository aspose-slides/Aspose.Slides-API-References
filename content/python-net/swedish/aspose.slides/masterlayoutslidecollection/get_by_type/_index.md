---
title: get_by_type method
second_title: Aspose.Slides för Python via .NET API-referens
description: 
type: docs
url: /sv/aspose.slides/masterlayoutslidecollection/get_by_type/
weight: 30
---
## get_by_type(self, type) {#slidelayouttype}
Returnerar den första layoutbilden av angiven typ.
            En typ av layoutbild att hitta.[`LayoutSlide`](/slides/python-net/sv/aspose.slides/layoutslide) med angiven typ eller None om inga layouter hittades.

```python
def get_by_type(self, type):
    ...
```

| Parameter | Typ | Beskrivning |
| :- | :- | :- |
| type | [`SlideLayoutType`](/slides/python-net/sv/aspose.slides/slidelayouttype) |  |

### Se även
* klass [`ILayoutSlide`](/slides/python-net/sv/aspose.slides/ilayoutslide)
* klass [`LayoutSlide`](/slides/python-net/sv/aspose.slides/layoutslide)
* klass [`MasterLayoutSlideCollection`](/slides/python-net/sv/aspose.slides/masterlayoutslidecollection)
* enumeration [`SlideLayoutType`](/slides/python-net/sv/aspose.slides/slidelayouttype)
* modul [`aspose.slides`](/slides/python-net/sv/aspose.slides)
* library [`Aspose.Slides`](/slides/python-net)