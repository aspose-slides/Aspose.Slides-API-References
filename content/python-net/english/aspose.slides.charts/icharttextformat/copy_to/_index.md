---
title: copy_to method
second_title: Aspose.Slides for Python via .NET API Reference
description: 
type: docs
url: /aspose.slides.charts/icharttextformat/copy_to/
weight: 20
---


## copy_to {#itextframe}
Copies text format to specified text frame.


```python
def copy_to(self, dest_text_frame):
    ...
```


| Parameter | Type | Description |
| :- | :- | :- |
| dest_text_frame | [`ITextFrame`](/slides/python-net/aspose.slides/itextframe) | Text frame to copy text format to. |



### See Also
* class [`IChartTextFormat`](/slides/python-net/aspose.slides.charts/icharttextformat)
* class [`ITextFrame`](/slides/python-net/aspose.slides/itextframe)
* module [`aspose.slides.charts`](/slides/python-net/aspose.slides.charts)
* library [`Aspose.Slides`](/slides/python-net)

