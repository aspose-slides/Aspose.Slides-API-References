---
title: format_text method
second_title: Aspose.Slides for Python via .NET API Reference
description: 
type: docs
url: /aspose.slides.export/isvgshapeandtextformattingcontroller/format_text/
weight: 20
---


## format_text {#isvgtspan-iportion-itextframe}
This function is called before rendering of text portion to SVG to allow user to control resulting SVG.


```python
def format_text(self, svg_t_span, portion, text_frame):
    ...
```


| Parameter | Type | Description |
| :- | :- | :- |
| svg_t_span | [`ISvgTSpan`](/slides/python-net/aspose.slides.export/isvgtspan) | Object to control SVG tspan generation. |
| portion | [`IPortion`](/slides/python-net/aspose.slides/iportion) | Source portion. |
| text_frame | [`ITextFrame`](/slides/python-net/aspose.slides/itextframe) | Source portion text frame. |



### See Also
* class [`IPortion`](/slides/python-net/aspose.slides/iportion)
* class [`ISvgShapeAndTextFormattingController`](/slides/python-net/aspose.slides.export/isvgshapeandtextformattingcontroller)
* class [`ISvgTSpan`](/slides/python-net/aspose.slides.export/isvgtspan)
* class [`ITextFrame`](/slides/python-net/aspose.slides/itextframe)
* module [`aspose.slides.export`](/slides/python-net/aspose.slides.export)
* library [`Aspose.Slides`](/slides/python-net)

