---
title: FontSubstRule constructor
second_title: Aspose.Slides for Python via .NET API Reference
description: 
type: docs
url: /aspose.slides/fontsubstrule/__init__/
weight: 10
---


## __init__ {#ifontdata-ifontdata}
Creates new instance.


```python
def __init__(self, source_font, dest_font):
    ...
```


| Parameter | Type | Description |
| :- | :- | :- |
| source_font | [`IFontData`](/slides/python-net/aspose.slides/ifontdata) | Source font. |
| dest_font | [`IFontData`](/slides/python-net/aspose.slides/ifontdata) | Destination font. |



## __init__ {#ifontdata-ifontdata-fontsubstcondition}
Creates new instance.


```python
def __init__(self, source_font, dest_font, font_subst_rule):
    ...
```


| Parameter | Type | Description |
| :- | :- | :- |
| source_font | [`IFontData`](/slides/python-net/aspose.slides/ifontdata) | Source font. |
| dest_font | [`IFontData`](/slides/python-net/aspose.slides/ifontdata) | Destination font. |
| font_subst_rule | [`FontSubstCondition`](/slides/python-net/aspose.slides/fontsubstcondition) | Font subst rule. |



### See Also
* enumeration [`FontSubstCondition`](/slides/python-net/aspose.slides/fontsubstcondition)
* class [`FontSubstRule`](/slides/python-net/aspose.slides/fontsubstrule)
* class [`IFontData`](/slides/python-net/aspose.slides/ifontdata)
* module [`aspose.slides`](/slides/python-net/aspose.slides)
* library [`Aspose.Slides`](/slides/python-net)
