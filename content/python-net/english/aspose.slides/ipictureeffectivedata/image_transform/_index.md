---
title: image_transform property
second_title: Aspose.Slides for Python via .NET API Reference
description: 
type: docs
url: /aspose.slides/ipictureeffectivedata/image_transform/
weight: 20
---


## image_transform property
Returns the collection of image transform effects.
            Read-only [`IImageTransformOCollectionEffectiveData`](/slides/python-net/aspose.slides.effects/iimagetransformocollectioneffectivedata).

### Definition:
```python
@property
def image_transform(self):
    ...
```


### See Also
* class [`IImageTransformOCollectionEffectiveData`](/slides/python-net/aspose.slides.effects/iimagetransformocollectioneffectivedata)
* class [`IPictureEffectiveData`](/slides/python-net/aspose.slides/ipictureeffectivedata)
* module [`aspose.slides`](/slides/python-net/aspose.slides)
* library [`Aspose.Slides`](/slides/python-net)

