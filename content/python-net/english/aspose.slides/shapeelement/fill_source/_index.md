---
title: fill_source property
second_title: Aspose.Slides for Python via .NET API Reference
description: 
type: docs
url: /aspose.slides/shapeelement/fill_source/
weight: 10
---


## fill_source property
Returns information about how to fill an element.
            Read-only [`ShapeElementFillSource`](/slides/python-net/aspose.slides/shapeelementfillsource).

### Definition:
```python
@property
def fill_source(self):
    ...
```

### See Also
* class [`ShapeElement`](/slides/python-net/aspose.slides/shapeelement)
* enumeration [`ShapeElementFillSource`](/slides/python-net/aspose.slides/shapeelementfillsource)
* module [`aspose.slides`](/slides/python-net/aspose.slides)
* library [`Aspose.Slides`](/slides/python-net)
