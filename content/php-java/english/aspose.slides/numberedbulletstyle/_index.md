---
title: NumberedBulletStyle
second_title: Aspose.Sildes for PHP via Java API Reference
description: 
type: docs

url: /aspose.slides/numberedbulletstyle/
---

## NumberedBulletStyle class

 Represents the style of the numbered bullets.
 

## Constants

| Name | Value | Description |
| --- | --- | --- |
[NotDefined](#NotDefined) | -1 | Not defined. |
[BulletAlphaLCPeriod](#BulletAlphaLCPeriod) | 0 | Lowercase alphabetical characters with a period. Sample: a., b., c., ... |
[BulletAlphaUCPeriod](#BulletAlphaUCPeriod) | 1 | Uppercase alphabetical characters with a period. Sample: A., B., C., ... |
[BulletArabicParenRight](#BulletArabicParenRight) | 2 | Arabic numerals with closing parenthesis. Sample: 1), 2), 3), ... |
[BulletArabicPeriod](#BulletArabicPeriod) | 3 | Arabic numerals with a period. Sample: 1., 2., 3., ... |
[BulletRomanLCParenBoth](#BulletRomanLCParenBoth) | 4 | Lowercase Roman numerals with both parentheses. Sample: (i), (ii), (iii), ... |
[BulletRomanLCParenRight](#BulletRomanLCParenRight) | 5 | Lowercase Roman numerals with closing parenthesis. Sample: i), ii), iii), ... |
[BulletRomanLCPeriod](#BulletRomanLCPeriod) | 6 | Lowercase Roman numerals with period. Sample: i., ii., iii., ... |
[BulletRomanUCPeriod](#BulletRomanUCPeriod) | 7 | Uppercase Roman numerals with period. Sample: I., II., III., ... |
[BulletAlphaLCParenBoth](#BulletAlphaLCParenBoth) | 8 | Lowercase alphabetical characters with both parentheses. Sample: (a), (b), (c), ... |
[BulletAlphaLCParenRight](#BulletAlphaLCParenRight) | 9 | Lowercase alphabetical characters with closing parenthesis. Sample: a), b), c), ... |
[BulletAlphaUCParenBoth](#BulletAlphaUCParenBoth) | 10 | Uppercase alphabetical characters with both parentheses. Sample: (A), (B), (C), ... |
[BulletAlphaUCParenRight](#BulletAlphaUCParenRight) | 11 | Uppercase alphabetical characters with closing parenthesis. Sample: A), B), C), ... |
[BulletArabicParenBoth](#BulletArabicParenBoth) | 12 | Arabic numerals with both parentheses. Sample: (1), (2), (3), ... |
[BulletArabicPlain](#BulletArabicPlain) | 13 | Arabic numerals. Sample: 1, 2, 3, ... |
[BulletRomanUCParenBoth](#BulletRomanUCParenBoth) | 14 | Uppercase Roman numerals with both parentheses. Sample: (I), (II), (III), ... |
[BulletRomanUCParenRight](#BulletRomanUCParenRight) | 15 | Uppercase Roman numerals with closing parenthesis. Sample: I), II), III), ... |
[BulletSimpChinPlain](#BulletSimpChinPlain) | 16 | Simplified Chinese without a period. |
[BulletSimpChinPeriod](#BulletSimpChinPeriod) | 17 | Simplified Chinese with a period. |
[BulletCircleNumDBPlain](#BulletCircleNumDBPlain) | 18 | Double-byte circled number for values up to 10, from 11 - arabic numbers. |
[BulletCircleNumWDWhitePlain](#BulletCircleNumWDWhitePlain) | 19 | Text colored number with same color circle drawn around it (Wingdings white circle numbers). From 11 - arabic numbers. |
[BulletCircleNumWDBlackPlain](#BulletCircleNumWDBlackPlain) | 20 | Shadow color number with circular background of normal text color (Wingdings black circle numbers). |
[BulletTradChinPlain](#BulletTradChinPlain) | 21 | Traditional Chinese without a period. |
[BulletTradChinPeriod](#BulletTradChinPeriod) | 22 | Traditional Chinese with a period. |
[BulletArabicAlphaDash](#BulletArabicAlphaDash) | 23 | Arabic language alphabetical characters with a dash. |
[BulletArabicAbjadDash](#BulletArabicAbjadDash) | 24 | Arabic Abjad alphabets with a dash. |
[BulletHebrewAlphaDash](#BulletHebrewAlphaDash) | 25 | Hebrew language alphabetical characters with a dash. |
[BulletKanjiKoreanPlain](#BulletKanjiKoreanPlain) | 26 | Japanese/Korean numbers without a period. |
[BulletKanjiKoreanPeriod](#BulletKanjiKoreanPeriod) | 27 | Japanese/Korean numbers with a period. |
[BulletArabicDBPlain](#BulletArabicDBPlain) | 28 | Double-byte Arabic numbering scheme (no punctuation). |
[BulletArabicDBPeriod](#BulletArabicDBPeriod) | 29 | Double-byte Arabic numbering scheme with double-byte period. |
[BulletThaiAlphaPeriod](#BulletThaiAlphaPeriod) | 30 | Thai alphabet period. |
[BulletThaiAlphaParenRight](#BulletThaiAlphaParenRight) | 31 | Thai alphabet parentheses - right. |
[BulletThaiAlphaParenBoth](#BulletThaiAlphaParenBoth) | 32 | Thai alphabet parentheses - both. |
[BulletThaiNumPeriod](#BulletThaiNumPeriod) | 33 | Thai numerical period. |
[BulletThaiNumParenRight](#BulletThaiNumParenRight) | 34 | Thai numerical parentheses - right. |
[BulletThaiNumParenBoth](#BulletThaiNumParenBoth) | 35 | Thai numerical parentheses - both. |
[BulletHindiAlphaPeriod](#BulletHindiAlphaPeriod) | 36 | Hindi alphabet period - vowels. |
[BulletHindiNumPeriod](#BulletHindiNumPeriod) | 37 | Hindi numerical period. |
[BulletKanjiSimpChinDBPeriod](#BulletKanjiSimpChinDBPeriod) | 38 | Kanji Simple Chinese DBPeriod. |
[BulletHindiNumParenRight](#BulletHindiNumParenRight) | 39 | Hindi numerical parentheses - right. |
[BulletHindiAlpha1Period](#BulletHindiAlpha1Period) | 40 | Hindi alphabet period - consonants. |


---


### NotDefined {#NotDefined}
Not defined.

---

### BulletAlphaLCPeriod {#BulletAlphaLCPeriod}
Lowercase alphabetical characters with a period. Sample: a., b., c., ...

---

### BulletAlphaUCPeriod {#BulletAlphaUCPeriod}
Uppercase alphabetical characters with a period. Sample: A., B., C., ...

---

### BulletArabicParenRight {#BulletArabicParenRight}
Arabic numerals with closing parenthesis. Sample: 1), 2), 3), ...

---

### BulletArabicPeriod {#BulletArabicPeriod}
Arabic numerals with a period. Sample: 1., 2., 3., ...

---

### BulletRomanLCParenBoth {#BulletRomanLCParenBoth}
Lowercase Roman numerals with both parentheses. Sample: (i), (ii), (iii), ...

---

### BulletRomanLCParenRight {#BulletRomanLCParenRight}
Lowercase Roman numerals with closing parenthesis. Sample: i), ii), iii), ...

---

### BulletRomanLCPeriod {#BulletRomanLCPeriod}
Lowercase Roman numerals with period. Sample: i., ii., iii., ...

---

### BulletRomanUCPeriod {#BulletRomanUCPeriod}
Uppercase Roman numerals with period. Sample: I., II., III., ...

---

### BulletAlphaLCParenBoth {#BulletAlphaLCParenBoth}
Lowercase alphabetical characters with both parentheses. Sample: (a), (b), (c), ...

---

### BulletAlphaLCParenRight {#BulletAlphaLCParenRight}
Lowercase alphabetical characters with closing parenthesis. Sample: a), b), c), ...

---

### BulletAlphaUCParenBoth {#BulletAlphaUCParenBoth}
Uppercase alphabetical characters with both parentheses. Sample: (A), (B), (C), ...

---

### BulletAlphaUCParenRight {#BulletAlphaUCParenRight}
Uppercase alphabetical characters with closing parenthesis. Sample: A), B), C), ...

---

### BulletArabicParenBoth {#BulletArabicParenBoth}
Arabic numerals with both parentheses. Sample: (1), (2), (3), ...

---

### BulletArabicPlain {#BulletArabicPlain}
Arabic numerals. Sample: 1, 2, 3, ...

---

### BulletRomanUCParenBoth {#BulletRomanUCParenBoth}
Uppercase Roman numerals with both parentheses. Sample: (I), (II), (III), ...

---

### BulletRomanUCParenRight {#BulletRomanUCParenRight}
Uppercase Roman numerals with closing parenthesis. Sample: I), II), III), ...

---

### BulletSimpChinPlain {#BulletSimpChinPlain}
Simplified Chinese without a period.

---

### BulletSimpChinPeriod {#BulletSimpChinPeriod}
Simplified Chinese with a period.

---

### BulletCircleNumDBPlain {#BulletCircleNumDBPlain}
Double-byte circled number for values up to 10, from 11 - arabic numbers.

---

### BulletCircleNumWDWhitePlain {#BulletCircleNumWDWhitePlain}
Text colored number with same color circle drawn around it (Wingdings white circle numbers). From 11 - arabic numbers.

---

### BulletCircleNumWDBlackPlain {#BulletCircleNumWDBlackPlain}
Shadow color number with circular background of normal text color (Wingdings black circle numbers).

---

### BulletTradChinPlain {#BulletTradChinPlain}
Traditional Chinese without a period.

---

### BulletTradChinPeriod {#BulletTradChinPeriod}
Traditional Chinese with a period.

---

### BulletArabicAlphaDash {#BulletArabicAlphaDash}
Arabic language alphabetical characters with a dash.

---

### BulletArabicAbjadDash {#BulletArabicAbjadDash}
Arabic Abjad alphabets with a dash.

---

### BulletHebrewAlphaDash {#BulletHebrewAlphaDash}
Hebrew language alphabetical characters with a dash.

---

### BulletKanjiKoreanPlain {#BulletKanjiKoreanPlain}
Japanese/Korean numbers without a period.

---

### BulletKanjiKoreanPeriod {#BulletKanjiKoreanPeriod}
Japanese/Korean numbers with a period.

---

### BulletArabicDBPlain {#BulletArabicDBPlain}
Double-byte Arabic numbering scheme (no punctuation).

---

### BulletArabicDBPeriod {#BulletArabicDBPeriod}
Double-byte Arabic numbering scheme with double-byte period.

---

### BulletThaiAlphaPeriod {#BulletThaiAlphaPeriod}
Thai alphabet period.

---

### BulletThaiAlphaParenRight {#BulletThaiAlphaParenRight}
Thai alphabet parentheses - right.

---

### BulletThaiAlphaParenBoth {#BulletThaiAlphaParenBoth}
Thai alphabet parentheses - both.

---

### BulletThaiNumPeriod {#BulletThaiNumPeriod}
Thai numerical period.

---

### BulletThaiNumParenRight {#BulletThaiNumParenRight}
Thai numerical parentheses - right.

---

### BulletThaiNumParenBoth {#BulletThaiNumParenBoth}
Thai numerical parentheses - both.

---

### BulletHindiAlphaPeriod {#BulletHindiAlphaPeriod}
Hindi alphabet period - vowels.

---

### BulletHindiNumPeriod {#BulletHindiNumPeriod}
Hindi numerical period.

---

### BulletKanjiSimpChinDBPeriod {#BulletKanjiSimpChinDBPeriod}
Kanji Simple Chinese DBPeriod.

---

### BulletHindiNumParenRight {#BulletHindiNumParenRight}
Hindi numerical parentheses - right.

---

### BulletHindiAlpha1Period {#BulletHindiAlpha1Period}
Hindi alphabet period - consonants.

---


