---
title: EmbeddedEotFontsHtmlController
second_title: Aspose.Sildes για PHP μέσω Java API Αναφορά
description: 
type: docs
url: /el/aspose.slides/embeddedeotfontshtmlcontroller/
---
## EmbeddedEotFontsHtmlController κλάση

Η κλάση ελεγκτή μορφοποίησης για ενσωμάτωση γραμματοσειρών σε μορφή EOT

### EmbeddedEotFontsHtmlController {#EmbeddedEotFontsHtmlController}

| Όνομα | Περιγραφή |
| --- | --- |
| EmbeddedEotFontsHtmlController() | Δημιουργεί νέο στιγμιότυπο. |

**Επιστρέφει:**
EmbeddedEotFontsHtmlController


---

### EmbeddedEotFontsHtmlController {#EmbeddedEotFontsHtmlController}

| Όνομα | Περιγραφή |
| --- | --- |
| EmbeddedEotFontsHtmlController([EmbeddedWoffFontsHtmlController](../embeddedwofffontshtmlcontroller)) | Δημιουργεί νέο στιγμιότυπο. |

**Παράμετροι:**

| Όνομα | Τύπος | Περιγραφή |
| --- | --- | --- |
| controller | [EmbeddedWoffFontsHtmlController](../embeddedwofffontshtmlcontroller) | HTML ελεγκτής μορφοποίησης. |

**Επιστρέφει:**
EmbeddedEotFontsHtmlController


---

### EmbeddedEotFontsHtmlController {#EmbeddedEotFontsHtmlController}

| Όνομα | Περιγραφή |
| --- | --- |
| EmbeddedEotFontsHtmlController([VideoPlayerHtmlController](../videoplayerhtmlcontroller)) | Δημιουργεί νέο στιγμιότυπο. |

**Παράμετροι:**

| Όνομα | Τύπος | Περιγραφή |
| --- | --- | --- |
| controller | [VideoPlayerHtmlController](../videoplayerhtmlcontroller) | HTML ελεγκτής μορφοποίησης. |

**Επιστρέφει:**
EmbeddedEotFontsHtmlController


---

### EmbeddedEotFontsHtmlController {#EmbeddedEotFontsHtmlController}

| Όνομα | Περιγραφή |
| --- | --- |
| EmbeddedEotFontsHtmlController([EmbeddedEotFontsHtmlController](../embeddedeotfontshtmlcontroller)) | Δημιουργεί νέο στιγμιότυπο. |

**Παράμετροι:**

| Όνομα | Τύπος | Περιγραφή |
| --- | --- | --- |
| controller | [EmbeddedEotFontsHtmlController](../embeddedeotfontshtmlcontroller) | HTML ελεγκτής μορφοποίησης. |

**Επιστρέφει:**
EmbeddedEotFontsHtmlController


---

### EmbeddedEotFontsHtmlController {#EmbeddedEotFontsHtmlController}

| Όνομα | Περιγραφή |
| --- | --- |
| EmbeddedEotFontsHtmlController([ResponsiveHtmlController](../responsivehtmlcontroller)) | Δημιουργεί νέο στιγμιότυπο. |

**Παράμετροι:**

| Όνομα | Τύπος | Περιγραφή |
| --- | --- | --- |
| controller | [ResponsiveHtmlController](../responsivehtmlcontroller) | HTML ελεγκτής μορφοποίησης. |

**Επιστρέφει:**
EmbeddedEotFontsHtmlController


---

### EmbeddedEotFontsHtmlController {#EmbeddedEotFontsHtmlController}

| Όνομα | Περιγραφή |
| --- | --- |
| EmbeddedEotFontsHtmlController([EmbedAllFontsHtmlController](../embedallfontshtmlcontroller)) | Δημιουργεί νέο στιγμιότυπο. |

**Παράμετροι:**

| Όνομα | Τύπος | Περιγραφή |
| --- | --- | --- |
| controller | [EmbedAllFontsHtmlController](../embedallfontshtmlcontroller) | HTML ελεγκτής μορφοποίησης. |

**Επιστρέφει:**
EmbeddedEotFontsHtmlController


---

### writeDocumentEnd {#writeDocumentEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeDocumentEnd ([HtmlGenerator](../htmlgenerator), [Presentation](../presentation)) |  |

**Επιστρέφει:**
void


---

### writeDocumentStart {#writeDocumentStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeDocumentStart ([HtmlGenerator](../htmlgenerator), [Presentation](../presentation)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [GraphicalObject](../graphicalobject)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [Connector](../connector)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [Shape](../shape)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [SmartArtShape](../smartartshape)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [Table](../table)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [Ink](../ink)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [SummaryZoomFrame](../summaryzoomframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [GeometryShape](../geometryshape)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [SummaryZoomSection](../summaryzoomsection)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [ZoomFrame](../zoomframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [OleObjectFrame](../oleobjectframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [VideoFrame](../videoframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [SmartArt](../smartart)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [GroupShape](../groupshape)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [InkActions](../inkactions)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [AutoShape](../autoshape)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [PictureFrame](../pictureframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [SectionZoomFrame](../sectionzoomframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [Chart](../chart)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [AudioFrame](../audioframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [ZoomObject](../zoomobject)) |  |

**Επιστρέφει:**
void


---

### writeShapeEnd {#writeShapeEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeEnd ([HtmlGenerator](../htmlgenerator), [LegacyDiagram](../legacydiagram)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [GraphicalObject](../graphicalobject)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [Connector](../connector)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [Shape](../shape)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [SmartArtShape](../smartartshape)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [Table](../table)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [Ink](../ink)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [SummaryZoomFrame](../summaryzoomframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [GeometryShape](../geometryshape)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [SummaryZoomSection](../summaryzoomsection)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [ZoomFrame](../zoomframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [OleObjectFrame](../oleobjectframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [VideoFrame](../videoframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [SmartArt](../smartart)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [GroupShape](../groupshape)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [InkActions](../inkactions)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [AutoShape](../autoshape)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [PictureFrame](../pictureframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [SectionZoomFrame](../sectionzoomframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [Chart](../chart)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [AudioFrame](../audioframe)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [ZoomObject](../zoomobject)) |  |

**Επιστρέφει:**
void


---

### writeShapeStart {#writeShapeStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeShapeStart ([HtmlGenerator](../htmlgenerator), [LegacyDiagram](../legacydiagram)) |  |

**Επιστρέφει:**
void


---

### writeSlideEnd {#writeSlideEnd}

| Όνομα | Περιγραφή |
| --- | --- |
| writeSlideEnd ([HtmlGenerator](../htmlgenerator), [Slide](../slide)) |  |

**Επιστρέφει:**
void


---

### writeSlideStart {#writeSlideStart}

| Όνομα | Περιγραφή |
| --- | --- |
| writeSlideStart ([HtmlGenerator](../htmlgenerator), [Slide](../slide)) |  |

**Επιστρέφει:**
void


---