...
description: 指定分数对象由分数条分隔的分子和分母组成 分数条可以是水平的或对角的具体取决于分数的属性 分数对象也用于表示堆栈函数其中放置一个元素在另一个之上没有分数栏
type: docs
weight: 7970
...
指定分数对象，由分数条分隔的分子和分母组成。 分数条可以是水平的或对角的，具体取决于分数的属性。 分数对象也用于表示堆栈函数，其中放置一个元素在另一个之上，没有分数栏。
