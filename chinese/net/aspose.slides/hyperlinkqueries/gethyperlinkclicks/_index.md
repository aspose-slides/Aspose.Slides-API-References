---
title: GetHyperlinkClicks
second_title: Aspose.Slides for .NET API 参考
description: 获取所有包含非空 HyperlinkClick 的 IHyperlinkContainer 子对象 使用给定的 IHyperlinkContainer 对象您可以管理其超链接读取更新或删除请参阅 IHyperlinkContainer 接口
type: docs
weight: 20
url: /zh/net/aspose.slides/hyperlinkqueries/gethyperlinkclicks/
---
## HyperlinkQueries.GetHyperlinkClicks method

获取所有包含非空 HyperlinkClick 的 IHyperlinkContainer 子对象。 使用给定的 IHyperlinkContainer 对象，您可以管理其超链接（读取、更新或删除）。请参阅 IHyperlinkContainer 接口。

```csharp
public IList<IHyperlinkContainer> GetHyperlinkClicks()
```

### 也可以看看

* interface [IHyperlinkContainer](../../ihyperlinkcontainer)
* class [HyperlinkQueries](../../hyperlinkqueries)
* 命名空间 [Aspose.Slides](../../hyperlinkqueries)
* 部件 [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->
