...
| 姓名 | 价值 | 描述 |
| --- | --- | --- |
| Tint | `0` | 着色颜色。参数在 0（原始颜色）和 1（白色）之间的范围内。 |
...
| SetAlpha | `5` | 定义颜色的 alpha 分量。参数在 0（透明）和 1（不透明）之间的范围内。 |
| AddAlpha | `6` | 将参数值添加到颜色的 alpha 分量。参数在 -1 和 1 之间的范围内。 |
| MultiplyAlpha | `7` | 将 alpha 分量与参数值相乘。 |
| SetHue | `8` | 将颜色的色调分量更改为参数值。参数在 0 到 360 之间。 |
| AddHue | `9` | 将参数值添加到颜色的色调分量。参数在 -360 和 360 之间的范围内。 |
| MultiplyHue | `10` | 将色调分量与参数值相乘。 |
| SetSaturation | `11` | 将颜色的饱和度分量更改为参数值。参数在 0 和 1 之间的范围内。 |
| AddSaturation | `12` | 将参数值添加到颜色的饱和度分量。参数在 -1 和 1 之间的范围内。 |
| MultiplySaturation | `13` | 将饱和分量乘以参数值。 |
| SetLuminance | `14` | 将颜色的亮度分量更改为参数值。参数在 0 和 1 之间的范围内。 |
| AddLuminance | `15` | 将参数值添加到颜色的亮度分量。参数在 -1 和 1 之间的范围内。 |
| MultiplyLuminance | `16` | 将亮度分量乘以参数值。 |
| SetRed | `17` | 将颜色的红色分量更改为参数值。参数在 0 和 1 之间的范围内。 |
| AddRed | `18` | 将参数值添加到颜色的红色分量。参数在 -1 和 1 之间的范围内。 |
| MultiplyRed | `19` | 将红色分量与参数相乘。 |
| SetGreen | `20` | 将颜色的绿色分量更改为参数的值。参数在 0 和 1 之间的范围内。 |
| AddGreen | `21` | 将参数添加到颜色的绿色分量。参数在 -1 和 1 之间的范围内。 |
| MultiplyGreen | `22` | 将颜色的绿色分量乘以参数值。 |
| SetBlue | `23` | 将颜色的蓝色分量更改为参数值。参数在 0 到 360 之间。 |
