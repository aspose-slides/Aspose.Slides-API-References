...
description: 创建新数据点并将其添加到集合的末尾 适用于 chartType 是 Area 子类型之一的系列另请参见 ChartTypeCharacterizer.IsChartTypeAreaChartType 方法
type: docs
weight: 70
url: /zh/net/aspose.slides.charts/ichartdatapointcollection/adddatapointforareaseries/
---
## AddDataPointForAreaSeries(IChartDataCell) {#adddatapointforareaseries}

创建新数据点并将其添加到集合的末尾。 适用于 chartType 是 Area 子类型之一的系列（另请参见 ChartTypeCharacterizer.IsChartTypeArea(ChartType) 方法）。
