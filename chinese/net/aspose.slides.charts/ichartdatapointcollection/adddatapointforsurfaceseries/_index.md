---
title: AddDataPointForSurfaceSeries
second_title: Aspose.Slides for .NET API 参考
description: 创建新数据点并将其添加到集合的末尾 适用于 chartType 是 Surface 子类型之一的系列另请参见 ChartTypeCharacterizer.IsChartTypeSurfaceChartType 方法
type: docs
weight: 210
url: /zh/net/aspose.slides.charts/ichartdatapointcollection/adddatapointforsurfaceseries/
---
## AddDataPointForSurfaceSeries(IChartDataCell) {#adddatapointforsurfaceseries}

创建新数据点并将其添加到集合的末尾。 适用于 chartType 是 Surface 子类型之一的系列（另请参见 ChartTypeCharacterizer.IsChartTypeSurface(ChartType) 方法）。

```csharp
public IChartDataPoint AddDataPointForSurfaceSeries(IChartDataCell value)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| value | IChartDataCell | 数据点值 |

### 返回值

新数据点。

### 也可以看看

* interface [IChartDataPoint](../../ichartdatapoint)
* interface [IChartDataCell](../../ichartdatacell)
* interface [IChartDataPointCollection](../../ichartdatapointcollection)
* 命名空间 [Aspose.Slides.Charts](../../ichartdatapointcollection)
* 部件 [Aspose.Slides](../../../)

---

## AddDataPointForSurfaceSeries(double) {#adddatapointforsurfaceseries_1}

创建新数据点并将其添加到集合的末尾。 适用于 chartType 是 Surface 子类型之一的系列（另请参见 ChartTypeCharacterizer.IsChartTypeSurface(ChartType) 方法）。

```csharp
public IChartDataPoint AddDataPointForSurfaceSeries(double value)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| value | Double | 数据点值 |

### 返回值

新数据点。

### 也可以看看

* interface [IChartDataPoint](../../ichartdatapoint)
* interface [IChartDataPointCollection](../../ichartdatapointcollection)
* 命名空间 [Aspose.Slides.Charts](../../ichartdatapointcollection)
* 部件 [Aspose.Slides](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Slides.dll -->
